...
static void caja_location_widget_provider_base_init(gpointer g_class) {}

GType caja_location_widget_provider_get_type(void) {
  static GType type = 0;

  if (!type) {
    const GTypeInfo info = {sizeof(CajaLocationWidgetProviderIface),
    ...
    type = g_type_register_static(G_TYPE_INTERFACE,
                                  "CajaLocationWidgetProvider", &info, 0);
...
GtkWidget *caja_location_widget_provider_get_widget(
  ...
  g_return_val_if_fail(
      CAJA_LOCATION_WIDGET_PROVIDER_GET_IFACE(provider)->get_widget != NULL,
      NULL);
