...
void caja_menu_append_item(CajaMenu *menu, CajaMenuItem *item) {
  ...
  menu->priv->item_list =
      g_list_append(menu->priv->item_list, g_object_ref(item));
}
 ...
 * @menu: a #CajaMenu
 *
 * Returns: (element-type CajaMenuItem) (transfer full): the provided
 * #CajaMenuItem list
 */
...
static void caja_menu_finalize(GObject *object) {
...
static void caja_menu_init(CajaMenu *menu) {
  menu->priv = caja_menu_get_instance_private(menu);

  menu->priv->item_list = NULL;
}

static void caja_menu_class_init(CajaMenuClass *klass) {
