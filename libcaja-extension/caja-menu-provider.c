...
static void caja_menu_provider_base_init(gpointer g_class) {
  static gboolean initialized = FALSE;

  if (!initialized) {
    /* This signal should be emited each time the extension modify the list of
     * menu items */
    g_signal_new("items_updated", CAJA_TYPE_MENU_PROVIDER, G_SIGNAL_RUN_LAST, 0,
...
GType caja_menu_provider_get_type(void) {
  ...
  if (!type) {
    const GTypeInfo info = {sizeof(CajaMenuProviderIface),
    ...
    type =
        g_type_register_static(G_TYPE_INTERFACE, "CajaMenuProvider", &info, 0);
 ...
 *
 * Returns: (element-type CajaMenuItem) (transfer full): the provided list of
 * #CajaMenuItem
 */
