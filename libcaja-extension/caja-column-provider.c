...
static void caja_column_provider_base_init(gpointer g_class) {}

GType caja_column_provider_get_type(void) {
  static GType type = 0;

  if (!type) {
    const GTypeInfo info = {sizeof(CajaColumnProviderIface),
    ...
    type = g_type_register_static(G_TYPE_INTERFACE, "CajaColumnProvider", &info,
                                  0);
 ...
 *
 * Returns: (element-type CajaColumn) (transfer full): the provided #CajaColumn
 * objects
 */
GList *caja_column_provider_get_columns(CajaColumnProvider *provider) {
  ...
  g_return_val_if_fail(
      CAJA_COLUMN_PROVIDER_GET_IFACE(provider)->get_columns != NULL, NULL);

  return CAJA_COLUMN_PROVIDER_GET_IFACE(provider)->get_columns(provider);
}
