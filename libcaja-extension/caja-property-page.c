...
enum { PROP_0, PROP_NAME, PROP_LABEL, PROP_PAGE, LAST_PROP };
...
CajaPropertyPage *caja_property_page_new(const char *name, GtkWidget *label,
  ...
  g_return_val_if_fail(page_widget != NULL && GTK_IS_WIDGET(page_widget), NULL);

  page = g_object_new(CAJA_TYPE_PROPERTY_PAGE, "name", name, "label", label,
                      "page", page_widget, NULL);

  return page;
}

static void caja_property_page_get_property(GObject *object, guint param_id,
...
static void caja_property_page_set_property(GObject *object, guint param_id,
                                            const GValue *value,
                                            GParamSpec *pspec) {
  CajaPropertyPage *page;
...
static void caja_property_page_dispose(GObject *object) {
  CajaPropertyPage *page;
...
static void caja_property_page_finalize(GObject *object) {
  CajaPropertyPage *page;
...
static void caja_property_page_instance_init(CajaPropertyPage *page) {
  page->details = g_new0(CajaPropertyPageDetails, 1);
}

static void caja_property_page_class_init(CajaPropertyPageClass *class) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string(
          "name", "Name", "Name of the page", NULL,
  ...
  g_object_class_install_property(
      G_OBJECT_CLASS(class), PROP_PAGE,
      g_param_spec_object("page", "Page", "Widget for the property page",
                          GTK_TYPE_WIDGET, G_PARAM_READWRITE));
}
...
GType caja_property_page_get_type(void) {
  ...
  if (!type) {
    const GTypeInfo info = {sizeof(CajaPropertyPageClass),
                            NULL,
                            NULL,
    ...
    type = g_type_register_static(G_TYPE_OBJECT, "CajaPropertyPage", &info, 0);
  }
