 ...
 * files. When caja_info_provider_update_file_info() is called by the
 * application, extensions will know that it's time to add extra information to
 * the provided #CajaFileInfo.
 */

static void caja_info_provider_base_init(gpointer g_class) {}

GType caja_info_provider_get_type(void) {
  static GType type = 0;

  if (!type) {
    const GTypeInfo info = {sizeof(CajaInfoProviderIface),
    ...
    type =
        g_type_register_static(G_TYPE_INTERFACE, "CajaInfoProvider", &info, 0);
...
CajaOperationResult caja_info_provider_update_file_info(
  ...
  g_return_val_if_fail(CAJA_IS_INFO_PROVIDER(provider), CAJA_OPERATION_FAILED);
  g_return_val_if_fail(
      CAJA_INFO_PROVIDER_GET_IFACE(provider)->update_file_info != NULL,
...
void caja_info_provider_cancel_update(CajaInfoProvider *provider,
  ...
  g_return_if_fail(CAJA_INFO_PROVIDER_GET_IFACE(provider)->cancel_update !=
                   NULL);
  g_return_if_fail(handle != NULL);

  CAJA_INFO_PROVIDER_GET_IFACE(provider)->cancel_update(provider, handle);
}
