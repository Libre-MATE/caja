 ...
 *
 * Creates a new menu item that can be added to the toolbar or to a contextual
 * menu.
 *
 * Returns: a newly create #CajaMenuItem
 */
CajaMenuItem *caja_menu_item_new(const char *name, const char *label,
  ...
  item = g_object_new(CAJA_TYPE_MENU_ITEM, "name", name, "label", label, "tip",
                      tip, "icon", icon, NULL);
...
static void caja_menu_item_get_property(GObject *object, guint param_id,
...
static void caja_menu_item_set_property(GObject *object, guint param_id,
                                        const GValue *value,
                                        GParamSpec *pspec) {
  CajaMenuItem *item;
...
static void caja_menu_item_finalize(GObject *object) {
  CajaMenuItem *item;
...
static void caja_menu_item_instance_init(CajaMenuItem *item) {
  item->details = g_new0(CajaMenuItemDetails, 1);
  item->details->sensitive = TRUE;
  item->details->menu = NULL;
}

static void caja_menu_item_class_init(CajaMenuItemClass *class) {
  ...
  signals[ACTIVATE] =
      g_signal_new("activate", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(CajaMenuItemClass, activate), NULL, NULL,
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string("label", "Label", "Label to display to the user",
                          NULL, G_PARAM_READWRITE));
  g_object_class_install_property(
      G_OBJECT_CLASS(class), PROP_TIP,
      g_param_spec_string("tip", "Tip", "Tooltip for the menu item", NULL,
                          G_PARAM_READWRITE));
  g_object_class_install_property(
      G_OBJECT_CLASS(class), PROP_ICON,
      g_param_spec_string("icon", "Icon",
                          "Name of the icon to display in the menu item", NULL,
                          G_PARAM_READWRITE));

  g_object_class_install_property(
      G_OBJECT_CLASS(class), PROP_SENSITIVE,
      g_param_spec_boolean("sensitive", "Sensitive",
                           "Whether the menu item is sensitive", TRUE,
                           G_PARAM_READWRITE));
  g_object_class_install_property(
      G_OBJECT_CLASS(class), PROP_PRIORITY,
      g_param_spec_boolean("priority", "Priority",
                           "Show priority text in toolbars", TRUE,
                           G_PARAM_READWRITE));
  g_object_class_install_property(
      G_OBJECT_CLASS(class), PROP_MENU,
      g_param_spec_object("menu", "Menu",
                          "The menu belonging to this item. May be null.",
                          CAJA_TYPE_MENU, G_PARAM_READWRITE));
}
...
GType caja_menu_item_get_type(void) {
  ...
  if (!type) {
    const GTypeInfo info = {sizeof(CajaMenuItemClass),
                            NULL,
                            NULL,
