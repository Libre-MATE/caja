...
static void timed_wait_delayed_close_destroy_dialog_callback(
    GtkWidget *object, gpointer callback_data) {
  g_source_remove(GPOINTER_TO_UINT(callback_data));
}

static gboolean timed_wait_delayed_close_timeout_callback(
    gpointer callback_data) {
  guint handler_id;
...
GtkWidget *eel_dialog_add_button(GtkDialog *dialog, const gchar *button_text,
...
static GtkWidget *eel_file_chooser_dialog_new_valist(
  ...
  while (button_text) {
    ...
    if (g_strcmp0(button_text, "process-stop") == 0)
      eel_dialog_add_button(GTK_DIALOG(result), _("_Cancel"), button_text,
                            response_id);
    else if (g_strcmp0(button_text, "document-open") == 0)
      eel_dialog_add_button(GTK_DIALOG(result), _("_Open"), button_text,
                            response_id);
    else if (g_strcmp0(button_text, "document-revert") == 0)
      eel_dialog_add_button(GTK_DIALOG(result), _("_Revert"), button_text,
                            response_id);
    else
...
GtkWidget *eel_file_chooser_dialog_new(const gchar *title, GtkWindow *parent,
...
static gboolean timed_wait_callback(gpointer callback_data) {
  ...
  if (g_strcmp0(button, "process-stop") == 0)
    eel_dialog_add_button(GTK_DIALOG(dialog), _("_Cancel"), button,
                          GTK_RESPONSE_OK);
  else
    eel_dialog_add_button(GTK_DIALOG(dialog), _("_OK"), button,
                          GTK_RESPONSE_OK);
...
int eel_run_simple_dialog(GtkWidget *parent, gboolean ignore_close_box,
  ...
  while (1) {
    ...
    if (g_strcmp0(button_title, "process-stop") == 0)
      eel_dialog_add_button(GTK_DIALOG(dialog), _("_Cancel"), button_title,
                            response_id);
    else
      eel_dialog_add_button(GTK_DIALOG(dialog), _("_OK"), button_title,
                            response_id);
...
static GtkDialog *show_message_dialog(
    const char *primary_text, const char *secondary_text, GtkMessageType type,
    GtkButtonsType buttons_type, const char *details_text, GtkWindow *parent) {
  GtkDialog *dialog;
...
GtkDialog *eel_show_info_dialog_with_details(const char *primary_text,
  ...
  dialog = show_message_dialog(primary_text, secondary_text, GTK_MESSAGE_INFO,
                               GTK_BUTTONS_OK, detailed_info, parent);
