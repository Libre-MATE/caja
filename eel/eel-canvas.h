...
#define EEL_CANVAS_COLOR(r, g, b)                      \
  ((((int)(r)&0xff) << 24) | (((int)(g)&0xff) << 16) | \
   (((int)(b)&0xff) << 8) | 0xff)

#define EEL_CANVAS_COLOR_A(r, g, b, a)                 \
  ((((int)(r)&0xff) << 24) | (((int)(g)&0xff) << 16) | \
...
enum { EEL_CANVAS_UPDATE_REQUESTED = 1 << 0, EEL_CANVAS_UPDATE_DEEP = 1 << 1 };
...
struct _EelCanvasItemClass {
   ...
   */
  void (*bounds)(EelCanvasItem *item, double *x1, double *y1, double *x2,
                 double *y2);
 ...
 * grab. event is the event, triggering the grab. Returns the same values as
 * gdk_seat_grab().
 */
...
struct _EelCanvasClass {
   ...
   */
  void (*draw_background)(EelCanvas *canvas, cairo_t *cr);
