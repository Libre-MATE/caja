...
static GdkPixbuf *create_new_pixbuf(GdkPixbuf *src) {
  g_assert(gdk_pixbuf_get_colorspace(src) == GDK_COLORSPACE_RGB);
  g_assert(
      (!gdk_pixbuf_get_has_alpha(src) && gdk_pixbuf_get_n_channels(src) == 3) ||
      (gdk_pixbuf_get_has_alpha(src) && gdk_pixbuf_get_n_channels(src) == 4));
...
GdkPixbuf *eel_create_spotlight_pixbuf(GdkPixbuf *src) {
  GdkPixbuf *dest;
  int i, j;
...
cairo_surface_t *eel_create_spotlight_surface(cairo_surface_t *src, int scale) {
  GdkPixbuf *pixbuf;
  cairo_surface_t *dest;

  pixbuf =
      gdk_pixbuf_get_from_surface(src, 0, 0, cairo_image_surface_get_width(src),
...
/* the following routine was stolen from the panel to darken a pixbuf, by
 * manipulating the saturation */

/* saturation is 0-255, darken is 0-255 */

GdkPixbuf *eel_create_darkened_pixbuf(GdkPixbuf *src, int saturation,
                                      int darken) {
  gint i, j;
...
/* this routine colorizes the passed-in pixbuf by multiplying each pixel with
 * the passed in color */

GdkPixbuf *eel_create_colorized_pixbuf(GdkPixbuf *src, GdkRGBA *color) {
  int i, j;
  int width, height, has_alpha, src_row_stride, dst_row_stride;
...
static void draw_frame_row(GdkPixbuf *frame_image, int target_width,
                           int source_width, int source_v_position,
                           int dest_v_position, GdkPixbuf *result_pixbuf,
...
static void draw_frame_column(GdkPixbuf *frame_image, int target_height,
                              int source_height, int source_h_position,
                              int dest_h_position, GdkPixbuf *result_pixbuf,
...
/* draw an arbitrary frame around an image, with the result passed back in a
 * newly allocated pixbuf */
GdkPixbuf *eel_embed_image_in_frame(GdkPixbuf *source_image,
  ...
  result_pixbuf = eel_stretch_frame_image(frame_image, left_offset, top_offset,
