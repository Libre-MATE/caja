...
#define EEL_BOILERPLATE(class_name, corba_name, prefix, parent_class_type, \
  ...
  GType prefix##_get_type(void) {                                          \
    ...
    static GType type;                                                     \
                                                                           \
    if (type == 0) {                                                       \
      static GTypeInfo info = {sizeof(class_name##Class),                  \
                               NULL,                                       \
                               NULL,                                       \
 ...
 * class initialization function. The companion macro
 *EEL_IMPLEMENT_MUST_OVERRIDE_SIGNAL must be used earlier in the file. Called
 *like this:
 *
...
#define EEL_IMPLEMENT_MUST_OVERRIDE_SIGNAL(prefix, signal)        \
                                                                  \
  static void prefix##_unimplemented_##signal(void) {             \
    g_warning("failed to override signal " #prefix "->" #signal); \
  }

#else /* G_DISABLE_ASSERT */

#define EEL_DEFINE_MUST_OVERRIDE_SIGNAL(class_cast_macro, class_pointer, \
                                        prefix, signal)
#define EEL_IMPLEMENT_MUST_OVERRIDE_SIGNAL(prefix, signal)
#define EEL_ASSIGN_MUST_OVERRIDE_SIGNAL(class_pointer, prefix, signal)
...
#define EEL_INVOKE_METHOD_IF(class_cast_macro, object, method, parameters) \
  (EEL_ACCESS_METHOD(class_cast_macro, object, method)                     \
       ? 0                                                                 \
