...
size_t eel_strlen(const char *string) {
...
gboolean eel_str_is_empty(const char *string_or_null) {
  return eel_strcmp(string_or_null, NULL) == 0;
}

gboolean eel_str_has_prefix(const char *haystack, const char *needle) {
  return g_str_has_prefix(haystack == NULL ? "" : haystack,
                          needle == NULL ? "" : needle);
}

gboolean eel_istr_has_prefix(const char *haystack, const char *needle) {
  const char *h, *n;
  char hc, nc;
...
char *eel_str_double_underscores(const char *string) {
  int underscores;
  const char *p;
  char *q;
  char *escaped;
...
char *eel_str_capitalize(const char *string) {
  char *capitalized;
 ...
 */
char *eel_str_middle_truncate(const char *string, guint truncate_length) {
  char *truncated;
  guint length;
  ...
  g_strlcat(truncated,
            g_utf8_offset_to_pointer(string, length - num_right_chars),
            (truncate_length + 1));

  return truncated;
}

char *eel_str_strip_substring_and_after(const char *string,
  ...
  return g_strndup(string, substring_position - string);
}

char *eel_str_replace_substring(const char *string, const char *substring,
...
static const char *get_position(const char *format, int *i) {
  const char *p;
...
static gboolean is_flag(char c) { return strchr("#0- +'I", c) != NULL; }

static gboolean is_length_modifier(char c) {
  return strchr("hlLjzt", c) != NULL;
}

static ArgType get_arg_type_from_format(EelPrintfHandler *custom_handlers,
...
static void skip_argv(va_list *va, ArgType type,
...
static void skip_to_arg(va_list *va, ArgType *types,
                        EelPrintfHandler *custom_handlers, int n) {
  int i;
  for (i = 0; i < n; i++) {
...
char *eel_strdup_vprintf_with_custom(EelPrintfHandler *custom,
                                     const char *format, va_list va_orig) {
  va_list va;
  const char *p;
  int num_args, i, j;
  ArgType *args;
  ...
  for (i = 0; i < num_args; i++) {
    ...
    if (conversions[i].precision_pos != -1) {
      ...
      g_string_insert(conversions[i].format,
                      conversions[i].precision_format_index, val);
    ...
    if (conversions[i].width_pos != -1) {
      ...
      g_string_insert(conversions[i].format, conversions[i].width_format_index,
                      val);
...
static void verify_printf(const char *format, ...) {
  va_list va;
  char *orig, *new;
...
static char *custom1_to_string(char *format, va_list va) {
  int i;
...
static void custom1_skip(va_list *va) { (void)va_arg(*va, int); }

static char *custom2_to_string(char *format, va_list va) {
  char *s;
...
static void custom2_skip(va_list *va) { (void)va_arg(*va, char *); }

static EelPrintfHandler handlers[] = {{'N', custom1_to_string, custom1_skip},
                                      {'Y', custom2_to_string, custom2_skip},
                                      {0}};

static void verify_custom(const char *orig, const char *format, ...) {
  char *new;
  va_list va;
...
void eel_self_check_string(void) {
  ...
  EEL_CHECK_STRING_RESULT(eel_str_double_underscores("foo_bar_2"),
                          "foo__bar__2");
  ...
  EEL_CHECK_STRING_RESULT(eel_str_middle_truncate("a_much_longer_foo", 0),
                          "a_much_longer_foo");
  EEL_CHECK_STRING_RESULT(eel_str_middle_truncate("a_much_longer_foo", 1),
                          "a_much_longer_foo");
  EEL_CHECK_STRING_RESULT(eel_str_middle_truncate("a_much_longer_foo", 2),
                          "a_much_longer_foo");
  EEL_CHECK_STRING_RESULT(eel_str_middle_truncate("a_much_longer_foo", 3),
                          "a_much_longer_foo");
  EEL_CHECK_STRING_RESULT(eel_str_middle_truncate("a_much_longer_foo", 4),
                          "a_much_longer_foo");
  EEL_CHECK_STRING_RESULT(eel_str_middle_truncate("a_much_longer_foo", 5),
                          "a...o");
  EEL_CHECK_STRING_RESULT(eel_str_middle_truncate("a_much_longer_foo", 6),
                          "a...oo");
  EEL_CHECK_STRING_RESULT(eel_str_middle_truncate("a_much_longer_foo", 7),
                          "a_...oo");
  EEL_CHECK_STRING_RESULT(eel_str_middle_truncate("a_much_longer_foo", 8),
                          "a_...foo");
  EEL_CHECK_STRING_RESULT(eel_str_middle_truncate("a_much_longer_foo", 9),
                          "a_m...foo");
  EEL_CHECK_STRING_RESULT(eel_str_middle_truncate("something_even", 8),
                          "so...ven");
  EEL_CHECK_STRING_RESULT(eel_str_middle_truncate("something_odd", 8),
                          "so...odd");
  EEL_CHECK_STRING_RESULT(eel_str_middle_truncate("something_even", 9),
                          "som...ven");
  EEL_CHECK_STRING_RESULT(eel_str_middle_truncate("something_odd", 9),
                          "som...odd");
  EEL_CHECK_STRING_RESULT(eel_str_middle_truncate("something_even", 10),
                          "som...even");
  EEL_CHECK_STRING_RESULT(eel_str_middle_truncate("something_odd", 10),
                          "som..._odd");
  EEL_CHECK_STRING_RESULT(eel_str_middle_truncate("something_even", 11),
                          "some...even");
  EEL_CHECK_STRING_RESULT(eel_str_middle_truncate("something_odd", 11),
                          "some..._odd");
  EEL_CHECK_STRING_RESULT(eel_str_middle_truncate("something_even", 12),
                          "some..._even");
  EEL_CHECK_STRING_RESULT(eel_str_middle_truncate("something_odd", 12),
                          "some...g_odd");
  EEL_CHECK_STRING_RESULT(eel_str_middle_truncate("something_even", 13),
                          "somet..._even");
  EEL_CHECK_STRING_RESULT(eel_str_middle_truncate("something_odd", 13),
                          "something_odd");
  EEL_CHECK_STRING_RESULT(eel_str_middle_truncate("something_even", 14),
                          "something_even");
  EEL_CHECK_STRING_RESULT(eel_str_middle_truncate("something_odd", 13),
                          "something_odd");
  ...
  EEL_CHECK_STRING_RESULT(eel_str_strip_substring_and_after("foo", "bar"),
                          "foo");
  EEL_CHECK_STRING_RESULT(eel_str_strip_substring_and_after("foo bar", "bar"),
                          "foo ");
  EEL_CHECK_STRING_RESULT(
      eel_str_strip_substring_and_after("foo bar xxx", "bar"), "foo ");
  ...
  EEL_CHECK_STRING_RESULT(eel_str_replace_substring("xxx", "x", "foo"),
                          "foofoofoo");
  EEL_CHECK_STRING_RESULT(eel_str_replace_substring("fff", "f", "foo"),
                          "foofoofoo");
  EEL_CHECK_STRING_RESULT(eel_str_replace_substring("foofoofoo", "foo", "f"),
                          "fff");
  EEL_CHECK_STRING_RESULT(eel_str_replace_substring("foofoofoo", "f", ""),
                          "oooooo");

  verify_printf("%.*s", 2, "foo");
  verify_printf("%*.*s", 2, 4, "foo");
  verify_printf("before %5$*1$.*2$s between %6$*3$.*4$d after", 4, 5, 6, 7,
                "foo", G_PI);
