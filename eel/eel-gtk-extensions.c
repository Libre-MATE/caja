...
/* Used for window position & size sanity-checking. The sizes are big enough to
 * prevent at least normal-sized mate panels from obscuring the window at the
 * screen edges.
 */
...
char *eel_gtk_window_get_geometry_string(GtkWindow *window) {
  ...
  g_return_val_if_fail(gtk_window_get_gravity(window) == GDK_GRAVITY_NORTH_WEST,
                       NULL);
...
static void sanity_check_window_position(int *left, int *top) {
  GdkScreen *screen;
  gint scale;
  ...
  *left = CLAMP(*left, 0,
                WidthOfScreen(gdk_x11_screen_get_xscreen(screen)) / scale -
                    MINIMUM_ON_SCREEN_WIDTH);
}

static void sanity_check_window_dimensions(guint *width, guint *height) {
  GdkScreen *screen;
  int screen_width;
  int screen_height;
  int scale;
...
void eel_gtk_window_set_initial_geometry(GtkWindow *window,
  ...
  if ((geometry_flags & EEL_GDK_X_VALUE) &&
      (geometry_flags & EEL_GDK_Y_VALUE)) {
    GdkScreen *screen;
...
void eel_gtk_window_set_initial_geometry_from_string(
  ...
  geometry_flags =
      eel_gdk_parse_geometry(geometry_string, &left, &top, &width, &height);
  ...
  eel_gtk_window_set_initial_geometry(window, geometry_flags, left, top, width,
                                      height);
}
...
static gboolean tree_view_button_press_callback(GtkWidget *tree_view,
...
void eel_gtk_tree_view_set_activate_on_single_click(GtkTreeView *tree_view,
  ...
  button_press_id = GPOINTER_TO_UINT(
      g_object_get_data(G_OBJECT(tree_view), "eel-tree-view-activate"));

  if (button_press_id && !should_activate) {
    g_signal_handler_disconnect(tree_view, button_press_id);
    g_object_set_data(G_OBJECT(tree_view), "eel-tree-view-activate", NULL);
  } else if (!button_press_id && should_activate) {
    button_press_id =
        g_signal_connect(tree_view, "button_press_event",
                         G_CALLBACK(tree_view_button_press_callback), NULL);
    g_object_set_data(G_OBJECT(tree_view), "eel-tree-view-activate",
...
GtkWidget *eel_image_menu_item_new_from_icon(const gchar *icon_name,
  ...
  if ((icon_name) &&
      (g_settings_get_boolean(icon_settings, "menus-have-icons")))
