...
static void pixbuf_loader_size_prepared(GdkPixbufLoader *loader, int width,
                                        int height, gpointer desired_size_ptr) {
  ...
  if (size != desired_size) {
    ...
    gdk_pixbuf_loader_set_size(loader, floor(scale * width + 0.5),
...
double eel_gdk_scale_to_fit_factor(int width, int height, int max_width,
                                   int max_height, int *scaled_width,
                                   int *scaled_height) {
  double scale_factor;
 ...
 * be scaled as large as possible without exceeding the specified width and
 * height.
 */
GdkPixbuf *eel_gdk_pixbuf_scale_to_fit(GdkPixbuf *pixbuf, int max_width,
                                       int max_height) {
  int scaled_width;
  int scaled_height;

  eel_gdk_scale_to_fit_factor(gdk_pixbuf_get_width(pixbuf),
                              gdk_pixbuf_get_height(pixbuf), max_width,
                              max_height, &scaled_width, &scaled_height);

  return gdk_pixbuf_scale_simple(pixbuf, scaled_width, scaled_height,
                                 GDK_INTERP_BILINEAR);
}
 ...
 */
GdkPixbuf *eel_gdk_pixbuf_scale_down_to_fit(GdkPixbuf *pixbuf, int max_width,
                                            int max_height) {
  int scaled_width;
  int scaled_height;
  ...
  scale_factor = eel_gdk_scale_to_fit_factor(
      gdk_pixbuf_get_width(pixbuf), gdk_pixbuf_get_height(pixbuf), max_width,
      max_height, &scaled_width, &scaled_height);
...
double eel_gdk_scale_to_min_factor(int width, int height, int min_width,
                                   int min_height, int *scaled_width,
                                   int *scaled_height) {
  double scale_factor;
...
/* Returns a scaled copy of pixbuf, preserving aspect ratio. The copy will
 * be scaled as small as possible without going under the specified width and
 * height.
 */
GdkPixbuf *eel_gdk_pixbuf_scale_to_min(GdkPixbuf *pixbuf, int min_width,
                                       int min_height) {
  int scaled_width;
  int scaled_height;

  eel_gdk_scale_to_min_factor(gdk_pixbuf_get_width(pixbuf),
                              gdk_pixbuf_get_height(pixbuf), min_width,
                              min_height, &scaled_width, &scaled_height);
...
gboolean eel_gdk_pixbuf_save_to_file(const GdkPixbuf *pixbuf,
  ...
  return gdk_pixbuf_save((GdkPixbuf *)pixbuf, file_name, "png", NULL, NULL);
}
...
GdkPixbuf *eel_gdk_pixbuf_scale_down(GdkPixbuf *pixbuf, int dest_width,
                                     int dest_height) {
  ...
  dest_pixbuf =
      gdk_pixbuf_new(GDK_COLORSPACE_RGB, has_alpha, 8, dest_width, dest_height);
