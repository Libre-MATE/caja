...
static void eel_editable_label_set_property(GObject *object, guint prop_id,
                                            const GValue *value,
                                            GParamSpec *pspec);
static void eel_editable_label_get_property(GObject *object, guint prop_id,
...
static gint eel_editable_label_draw(GtkWidget *widget, cairo_t *cr);
...
static gint eel_editable_label_button_release(GtkWidget *widget,
                                              GdkEventButton *event);
static gint eel_editable_label_motion(GtkWidget *widget, GdkEventMotion *event);
static gint eel_editable_label_key_press(GtkWidget *widget, GdkEventKey *event);
...
static void eel_editable_label_preedit_changed_cb(GtkIMContext *context,
                                                  EelEditableLabel *label);
static gboolean eel_editable_label_retrieve_surrounding_cb(
    GtkIMContext *context, EelEditableLabel *label);
static gboolean eel_editable_label_delete_surrounding_cb(
    GtkIMContext *slave, gint offset, gint n_chars, EelEditableLabel *label);
...
static void editable_delete_text_emit(GtkEditable *editable, gint start_pos,
                                      gint end_pos);
static void editable_insert_text(GtkEditable *editable, const gchar *new_text,
                                 gint new_text_length, gint *position);
static void editable_delete_text(GtkEditable *editable, gint start_pos,
                                 gint end_pos);
static gchar *editable_get_chars(GtkEditable *editable, gint start_pos,
                                 gint end_pos);
static void editable_set_selection_bounds(GtkEditable *editable, gint start,
                                          gint end);
static gboolean editable_get_selection_bounds(GtkEditable *editable,
                                              gint *start, gint *end);
static void editable_real_set_position(GtkEditable *editable, gint position);
static gint editable_get_position(GtkEditable *editable);

G_DEFINE_TYPE_WITH_CODE(
    EelEditableLabel, eel_editable_label, GTK_TYPE_WIDGET,
...
static void add_move_binding(GtkBindingSet *binding_set, guint keyval,
  ...
  gtk_binding_entry_add_signal(binding_set, keyval, modmask | GDK_SHIFT_MASK,
                               "move_cursor", 3, G_TYPE_ENUM, step, G_TYPE_INT,
                               count, G_TYPE_BOOLEAN, TRUE);
}

static void eel_editable_label_class_init(EelEditableLabelClass *class) {
  ...
  gtk_widget_class_set_accessible_type(
      widget_class, eel_editable_label_accessible_get_type());
  ...
  signals[MOVE_CURSOR] =
      g_signal_new("move_cursor", G_TYPE_FROM_CLASS(class),
                   G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                   G_STRUCT_OFFSET(EelEditableLabelClass, move_cursor), NULL,
                   NULL, eel_marshal_VOID__ENUM_INT_BOOLEAN, G_TYPE_NONE, 3,
                   GTK_TYPE_MOVEMENT_STEP, G_TYPE_INT, G_TYPE_BOOLEAN);

  signals[COPY_CLIPBOARD] =
      g_signal_new("copy_clipboard", G_TYPE_FROM_CLASS(class),
                   G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                   G_STRUCT_OFFSET(EelEditableLabelClass, copy_clipboard), NULL,
                   NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  signals[POPULATE_POPUP] = g_signal_new(
      "populate_popup", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(EelEditableLabelClass, populate_popup), NULL, NULL,
      g_cclosure_marshal_VOID__OBJECT, G_TYPE_NONE, 1, GTK_TYPE_MENU);

  signals[DELETE_FROM_CURSOR] =
      g_signal_new("delete_from_cursor", G_TYPE_FROM_CLASS(class),
                   G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                   G_STRUCT_OFFSET(EelEditableLabelClass, delete_from_cursor),
                   NULL, NULL, eel_marshal_VOID__ENUM_INT, G_TYPE_NONE, 2,
                   GTK_TYPE_DELETE_TYPE, G_TYPE_INT);

  signals[CUT_CLIPBOARD] =
      g_signal_new("cut_clipboard", G_TYPE_FROM_CLASS(class),
                   G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                   G_STRUCT_OFFSET(EelEditableLabelClass, cut_clipboard), NULL,
                   NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  signals[PASTE_CLIPBOARD] =
      g_signal_new("paste_clipboard", G_TYPE_FROM_CLASS(class),
                   G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                   G_STRUCT_OFFSET(EelEditableLabelClass, paste_clipboard),
                   NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  signals[TOGGLE_OVERWRITE] =
      g_signal_new("toggle_overwrite", G_TYPE_FROM_CLASS(class),
                   G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
  ...
  g_object_class_install_property(
      ...
      g_param_spec_string("text", _("Text"), _("The text of the label."), NULL,
                          G_PARAM_READWRITE));
  g_object_class_install_property(
      gobject_class, PROP_JUSTIFY,
      g_param_spec_enum(
          "justify", _("Justification"),
          _("The alignment of the lines in the text of the label relative to "
  ...
  g_object_class_install_property(
      ...
      g_param_spec_boolean(
          "wrap", _("Line wrap"),
          _("If set, wrap lines if the text becomes too wide."), FALSE,
          G_PARAM_READWRITE));

  g_object_class_install_property(
      gobject_class, PROP_CURSOR_POSITION,
      g_param_spec_int(
          "cursor_position", _("Cursor Position"),
          _("The current position of the insertion cursor in chars."), 0,
          G_MAXINT, 0, G_PARAM_READABLE));

  g_object_class_install_property(
      gobject_class, PROP_SELECTION_BOUND,
      g_param_spec_int("selection_bound", _("Selection Bound"),
                       _("The position of the opposite end of the selection "
                         "from the cursor in chars."),
  ...
  add_move_binding(binding_set, GDK_KEY_Right, 0, GTK_MOVEMENT_VISUAL_POSITIONS,
                   1);

  add_move_binding(binding_set, GDK_KEY_Left, 0, GTK_MOVEMENT_VISUAL_POSITIONS,
                   -1);
  ...
  add_move_binding(binding_set, GDK_KEY_f, GDK_MOD1_MASK, GTK_MOVEMENT_WORDS,
                   1);

  add_move_binding(binding_set, GDK_KEY_b, GDK_MOD1_MASK, GTK_MOVEMENT_WORDS,
                   -1);

  add_move_binding(binding_set, GDK_KEY_Home, 0, GTK_MOVEMENT_DISPLAY_LINE_ENDS,
                   -1);

  add_move_binding(binding_set, GDK_KEY_End, 0, GTK_MOVEMENT_DISPLAY_LINE_ENDS,
                   1);
  ...
  add_move_binding(binding_set, GDK_KEY_KP_Up, 0, GTK_MOVEMENT_DISPLAY_LINES,
                   -1);

  add_move_binding(binding_set, GDK_KEY_Down, 0, GTK_MOVEMENT_DISPLAY_LINES, 1);

  add_move_binding(binding_set, GDK_KEY_KP_Down, 0, GTK_MOVEMENT_DISPLAY_LINES,
                   1);
 ...
 **/
GtkWidget *eel_editable_label_new(const gchar *str) {
  EelEditableLabel *label;

  label = g_object_new(EEL_TYPE_EDITABLE_LABEL, NULL);

  if (str && *str) eel_editable_label_set_text(label, str);
 ...
 *
 * Sets the text within the #EelEditableLabel widget.  It overwrites any text
 *that was there before.
 *
 * This will also clear any previously set mnemonic accelerators.
 **/
void eel_editable_label_set_text(EelEditableLabel *label, const gchar *str) {
  ...
  if (strcmp(label->text, str) == 0) return;
 ...
 * @wrap: the setting
 *
 * Toggles line wrapping within the #EelEditableLabel widget.  %TRUE makes it
 ...
 **/
void eel_editable_label_set_line_wrap(EelEditableLabel *label, gboolean wrap) {
  g_return_if_fail(EEL_IS_EDITABLE_LABEL(label));
...
void eel_editable_label_set_font_description(EelEditableLabel *label,
  ...
  if (label->font_desc) pango_font_description_free(label->font_desc);
...
static void label_wrap_width_free(gpointer data) {
  LabelWrapWidth *wrap_width = data;
  pango_font_description_free(wrap_width->font_desc);
  g_free(wrap_width);
}

static gint get_label_wrap_width(EelEditableLabel *label) {
  PangoLayout *layout;
  GtkStyleContext *style = gtk_widget_get_style_context(GTK_WIDGET(label));
  PangoFontDescription *desc;

  LabelWrapWidth *wrap_width =
      g_object_get_data(G_OBJECT(style), "gtk-label-wrap-width");
  if (!wrap_width) {
    wrap_width = g_new0(LabelWrapWidth, 1);
    g_object_set_data_full(G_OBJECT(style), "gtk-label-wrap-width", wrap_width,
  ...
  if (wrap_width->font_desc &&
      pango_font_description_equal(wrap_width->font_desc, desc))
    goto out;

  if (wrap_width->font_desc) pango_font_description_free(wrap_width->font_desc);
...
static void eel_editable_label_ensure_layout(EelEditableLabel *label,
  ...
  if (label->layout == NULL) {
    ...
    if (include_preedit) {
      gtk_im_context_get_preedit_string(label->im_context, &preedit_string,
                                        &preedit_attrs, NULL);
    ...
    if (preedit_length) {
      ...
      pango_attr_list_splice(tmp_attrs, preedit_attrs, label->selection_anchor,
                             preedit_length);
    ...
    if (label->wrap) {
      ...
      if (set_width > 0)
        ...
        /* Unfortunately, the above may leave us with a very unbalanced looking
         * paragraph, so we try short search for a narrower width that leaves us
          ...
          if (perfect_width < width) {
            ...
            if (logical_rect.height <= height)
              ...
              if (mid_width > perfect_width) {
                ...
                if (logical_rect.height <= height) width = logical_rect.width;
              }
            }
          }
        }
...
static void eel_editable_label_size_request(GtkWidget *widget,
  ...
  if (label->wrap) eel_editable_label_recompute(label);
...
static void eel_editable_label_size_allocate(GtkWidget *widget,
                                             GtkAllocation *allocation) {
  (*GTK_WIDGET_CLASS(eel_editable_label_parent_class)->size_allocate)(
      widget, allocation);
}
...
static void eel_editable_label_state_flags_changed(GtkWidget *widget,
  ...
  if (GTK_WIDGET_CLASS(eel_editable_label_parent_class)->state_flags_changed)
    GTK_WIDGET_CLASS(eel_editable_label_parent_class)
...
static void eel_editable_label_direction_changed(
    GtkWidget *widget, GtkTextDirection previous_dir) {
  EelEditableLabel *label = EEL_EDITABLE_LABEL(widget);

  if (label->layout) pango_layout_context_changed(label->layout);
...
static void get_layout_location(EelEditableLabel *label, gint *xp, gint *yp) {
  GtkWidget *widget;
  ...
  x = floor(0.5 + xpad + ((allocation.width - req.width) * xalign) + 0.5);

  y = floor(0.5 + ypad + ((allocation.height - req.height) * yalign) + 0.5);
...
static gint eel_editable_label_get_cursor_pos(EelEditableLabel *label,
  ...
  index = label->selection_anchor +
          g_utf8_offset_to_pointer(preedit_text, label->preedit_cursor) -
          preedit_text;
...
static gboolean eel_editable_label_get_block_cursor_location(
    EelEditableLabel *label, gint *index, PangoRectangle *pos,
  ...
  if (pos->width != 0) {
    if (at_line_end) *at_line_end = FALSE;
  ...
  if (layout_line == NULL) return FALSE;
  ...
  if (layout_line->length) {
    if (layout_line->resolved_dir == PANGO_DIRECTION_RTL) {
      ...
      for (i = 0; i < line_no; i++) pango_layout_iter_next_line(iter);
  ...
  if (!font_desc) font_desc = pango_context_get_font_description(context);
  ...
  if (rtl) pos->x -= pos->width - 1;

  if (at_line_end) *at_line_end = TRUE;
...
static void eel_editable_label_draw_cursor(EelEditableLabel *label, cairo_t *cr,
  ...
  if (gtk_widget_is_drawable(GTK_WIDGET(label))) {
    ...
    if (label->overwrite_mode &&
        eel_editable_label_get_block_cursor_location(label, range, &strong_pos,
                                                     &block_at_line_end))
      block = TRUE;
    else
      block = FALSE;
    ...
    if (!block) {
      gtk_render_insertion_cursor(
          context, cr, xoffset, yoffset, label->layout, index,
    ...
    } else /* Block cursor */
    {
      ...
      cairo_rectangle(cr, xoffset + PANGO_PIXELS(strong_pos.x),
      ...
      if (!block_at_line_end) {
        ...
        clip = gdk_pango_layout_get_clip_region(label->layout, xoffset, yoffset,
                                                range, 1);
...
static gint eel_editable_label_draw(GtkWidget *widget, cairo_t *cr) {
  ...
  if (gtk_widget_get_visible(widget) && gtk_widget_get_mapped(widget) &&
    ...
    if (label->selection_anchor != label->selection_end) {
      ...
      if (label->preedit_length > 0 && range[1] > label->selection_anchor) {
        const char *text;

        text = pango_layout_get_text(label->layout) + label->selection_anchor;
        range[1] +=
            g_utf8_offset_to_pointer(text, label->preedit_length) - text;
      }
      ...
      gtk_style_context_get(style, state, GTK_STYLE_PROPERTY_BACKGROUND_COLOR,
                            &c, NULL);
      background_color = *c;
      gdk_rgba_free(c);
...
static void eel_editable_label_realize(GtkWidget *widget) {
  ...
  gtk_im_context_set_client_window(label->im_context,
...
static void window_to_layout_coords(EelEditableLabel *label, gint *x, gint *y) {
  gint lx, ly;

  /* get layout location in gtk_widget_get_window (widget) coords */
  get_layout_location(label, &lx, &ly);

  if (x) *x -= lx; /* go to layout */

  if (y) *y -= ly; /* go to layout */
}

static void get_layout_index(EelEditableLabel *label, gint x, gint y,
                             gint *index) {
  gint trailing = 0;
  const gchar *cluster;
  ...
  *index = 0;
  ...
  pango_layout_xy_to_index(label->layout, x, y, index, &trailing);
...
static void eel_editable_label_select_word(EelEditableLabel *label) {
  gint min, max;

  gint start_index =
      eel_editable_label_move_backward_word(label, label->selection_end);
  gint end_index =
      eel_editable_label_move_forward_word(label, label->selection_end);

  min = MIN(label->selection_anchor, label->selection_end);
  max = MAX(label->selection_anchor, label->selection_end);
...
static gint eel_editable_label_motion(GtkWidget *widget,
  ...
  gdk_window_get_device_position(gtk_widget_get_window(widget), event->device,
                                 &x, &y, NULL);

  get_layout_index(label, x, y, &index);

  eel_editable_label_select_region_index(label, label->selection_anchor, index);
...
static void get_text_callback(GtkClipboard *clipboard,
  ...
  if ((label->selection_anchor != label->selection_end) && label->text) {
    gint start, end;
    gint len;

    start = MIN(label->selection_anchor, label->selection_end);
    end = MAX(label->selection_anchor, label->selection_end);
    ...
    gtk_selection_data_set_text(selection_data, label->text + start,
                                end - start);
  }
}
...
static void eel_editable_label_select_region_index(EelEditableLabel *label,
  ...
  if (anchor_index != end_index) {
    ...
    gtk_clipboard_set_with_owner(clipboard, targets, n_targets,
                                 get_text_callback, clear_text_callback,
                                 G_OBJECT(label));
...
void eel_editable_label_select_region(EelEditableLabel *label,
  ...
  if (label->text) {
    if (start_offset < 0) start_offset = g_utf8_strlen(label->text, -1);

    if (end_offset < 0) end_offset = g_utf8_strlen(label->text, -1);
...
gboolean eel_editable_label_get_selection_bounds(EelEditableLabel *label,
  ...
  start_index = MIN(label->selection_anchor, label->selection_end);
  end_index = MAX(label->selection_anchor, label->selection_end);
  ...
  if (start) *start = start_offset;

  if (end) *end = end_offset;
 ...
 *
 **/
void eel_editable_label_get_layout_offsets(EelEditableLabel *label, gint *x,
                                           gint *y) {
  g_return_if_fail(EEL_IS_EDITABLE_LABEL(label));
...
static gint eel_editable_label_key_press(GtkWidget *widget,
  ...
  if (GTK_WIDGET_CLASS(eel_editable_label_parent_class)
...
static gint eel_editable_label_key_release(GtkWidget *widget,
  ...
  return GTK_WIDGET_CLASS(eel_editable_label_parent_class)
      ->key_release_event(widget, event);
}

static void eel_editable_label_keymap_direction_changed(
    GdkKeymap *keymap, EelEditableLabel *label) {
  gtk_widget_queue_draw(GTK_WIDGET(label));
}
...
static gint eel_editable_label_focus_in(GtkWidget *widget,
  ...
  g_signal_connect(gdk_keymap_get_for_display(gtk_widget_get_display(widget)),
                   "direction_changed",
                   G_CALLBACK(eel_editable_label_keymap_direction_changed),
                   label);
...
static gint eel_editable_label_focus_out(GtkWidget *widget,
  ...
  g_signal_handlers_disconnect_by_func(
      gdk_keymap_get_for_display(gtk_widget_get_display(widget)),
      (gpointer)eel_editable_label_keymap_direction_changed, label);

  return FALSE;
}

static void eel_editable_label_delete_text(EelEditableLabel *label,
                                           int start_pos, int end_pos) {
  if (start_pos < 0) start_pos = 0;
  if (end_pos < 0 || end_pos > label->n_bytes) end_pos = label->n_bytes;

  if (start_pos < end_pos) {
    int anchor, end;

    memmove(label->text + start_pos, label->text + end_pos,
            label->n_bytes + 1 - end_pos);
    label->n_bytes -= (end_pos - start_pos);

    anchor = label->selection_anchor;
    if (anchor > start_pos) anchor -= MIN(anchor, end_pos) - start_pos;

    end = label->selection_end;
    if (end > start_pos) end -= MIN(end, end_pos) - start_pos;
...
static void eel_editable_label_insert_text(EelEditableLabel *label,
                                           const gchar *new_text,
                                           gint new_text_length, gint *index) {
  ...
  memmove(label->text + *index + new_text_length, label->text + *index,
          label->n_bytes - *index);
...
static void eel_editable_label_enter_text(EelEditableLabel *label,
                                          const gchar *str) {
...
static void eel_editable_label_preedit_changed_cb(GtkIMContext *context,
  ...
  gtk_im_context_get_preedit_string(label->im_context, &preedit_string, NULL,
                                    &cursor_pos);
...
static gboolean eel_editable_label_retrieve_surrounding_cb(
    GtkIMContext *context, EelEditableLabel *label) {
  gtk_im_context_set_surrounding(context, label->text, strlen(label->text) + 1,
                                 label->selection_end);
...
static gboolean eel_editable_label_delete_surrounding_cb(
  ...
  current_pos = g_utf8_pointer_to_offset(label->text,
                                         label->text + label->selection_anchor);
  gtk_editable_delete_text(GTK_EDITABLE(label), current_pos + offset,
 ...
 * right arrow key.
 */
static void get_better_cursor(EelEditableLabel *label, gint index, gint *x,
                              gint *y) {
  GtkTextDirection keymap_direction =
      (gdk_keymap_get_direction(gdk_keymap_get_for_display(
           gtk_widget_get_display(GTK_WIDGET(label)))) == PANGO_DIRECTION_LTR)
          ? GTK_TEXT_DIR_LTR
          : GTK_TEXT_DIR_RTL;
  GtkTextDirection widget_direction =
      gtk_widget_get_direction(GTK_WIDGET(label));
  gboolean split_cursor;
  PangoRectangle strong_pos, weak_pos;

  g_object_get(gtk_widget_get_settings(GTK_WIDGET(label)), "gtk-split-cursor",
               &split_cursor, NULL);
...
static gint eel_editable_label_move_logically(EelEditableLabel *label,
                                              gint start, gint count) {
  gint offset = g_utf8_pointer_to_offset(label->text, label->text + start);
...
static gint eel_editable_label_move_visually(EelEditableLabel *label,
                                             gint start, gint count) {
  gint index;
  ...
  while (count != 0) {
    ...
    if (count > 0) {
      pango_layout_move_cursor_visually(label->layout, strong, index, 0, 1,
...
static gint eel_editable_label_move_line(EelEditableLabel *label, gint start,
                                         gint count) {
  ...
  if (i == n_lines) i = n_lines - 1;

  i += count;
  i = CLAMP(i, 0, n_lines - 1);

  line = pango_layout_get_line(label->layout, i);
  if (pango_layout_line_x_to_index(line, x, &index, NULL))
    return index;
  else {
    if (i == n_lines - 1)
...
static gint eel_editable_label_move_forward_word(EelEditableLabel *label,
                                                 gint start) {
  gint new_pos = g_utf8_pointer_to_offset(label->text, label->text + start);
  gint length;
...
static void eel_editable_label_move_cursor(EelEditableLabel *label,
  ...
  if (label->selection_end != label->selection_anchor && !extend_selection) {
    ...
    switch (step) {
      ...
      case GTK_MOVEMENT_VISUAL_POSITIONS: {
        ...
        end_is_left =
            (end_y < anchor_y) || (end_y == anchor_y && end_x < anchor_x);

        if (count < 0)
          new_pos =
              end_is_left ? label->selection_end : label->selection_anchor;
        else
          new_pos =
              !end_is_left ? label->selection_end : label->selection_anchor;
  ...
  if (extend_selection)
    eel_editable_label_select_region_index(label, label->selection_anchor,
                                           new_pos);
  else
...
static void eel_editable_label_delete_from_cursor(EelEditableLabel *label,
  ...
  switch (type) {
    case GTK_DELETE_CHARS:
      ...
      start_pos =
          g_utf8_pointer_to_offset(label->text, label->text + start_pos);
      end_pos = g_utf8_pointer_to_offset(label->text, label->text + end_pos);
      gtk_editable_delete_text(GTK_EDITABLE(label), MIN(start_pos, end_pos),
                               MAX(start_pos, end_pos));
      break;
    case GTK_DELETE_WORDS:
      if (count < 0) {
        /* Move to end of current word, or if not on a word, end of previous
         * word */
        end_pos = eel_editable_label_move_backward_word(label, end_pos);
        end_pos = eel_editable_label_move_forward_word(label, end_pos);
      } else if (count > 0) {
        /* Move to beginning of current word, or if not on a word, begining of
         * next word */
    ...
    case GTK_DELETE_PARAGRAPH_ENDS:
      end_pos = g_utf8_pointer_to_offset(label->text,
                                         label->text + label->selection_anchor);
      if (count < 0)
...
static void paste_received(GtkClipboard *clipboard, const gchar *text,
                           gpointer data) {
...
static void eel_editable_label_paste(EelEditableLabel *label,
  ...
  gtk_clipboard_request_text(
      gtk_widget_get_clipboard(GTK_WIDGET(label), selection), paste_received,
      label);
}
 ...
 */
static void activate_cb(GtkWidget *menuitem, EelEditableLabel *label) {
  const gchar *signal = g_object_get_data(G_OBJECT(menuitem), "gtk-signal");
  g_signal_emit_by_name(label, signal);
}

static void append_action_signal(EelEditableLabel *label, GtkWidget *menu,
                                 const gchar *icon_name,
                                 const gchar *label_name, const gchar *signal,
                                 gboolean sensitive) {
  GtkWidget *menuitem =
      eel_image_menu_item_new_from_icon(icon_name, label_name);
...
static void popup_targets_received(GtkClipboard *clipboard,
                                   GtkSelectionData *data, gpointer user_data) {
  ...
  if (gtk_widget_get_realized(GTK_WIDGET(label))) {
    ...
    if (label->popup_menu) gtk_widget_destroy(label->popup_menu);

    label->popup_menu = gtk_menu_new();

    gtk_menu_set_reserve_toggle_size(GTK_MENU(label->popup_menu), FALSE);

    gtk_menu_attach_to_widget(GTK_MENU(label->popup_menu), GTK_WIDGET(label),
                              popup_menu_detach);

    have_selection = label->selection_anchor != label->selection_end;

    clipboard_contains_text = gtk_selection_data_targets_include_text(data);

    append_action_signal(label, label->popup_menu, "edit-cut", _("Cu_t"),
                         "cut_clipboard", have_selection);
    append_action_signal(label, label->popup_menu, "edit-copy", _("_Copy"),
                         "copy_clipboard", have_selection);
    append_action_signal(label, label->popup_menu, "edit-paste", _("_Paste"),
                         "paste_clipboard", clipboard_contains_text);

    menuitem =
        eel_image_menu_item_new_from_icon("edit-select-all", _("Select All"));
    ...
    g_signal_emit(label, signals[POPULATE_POPUP], 0, label->popup_menu);
...
static void eel_editable_label_do_popup(EelEditableLabel *label,
  ...
  gtk_clipboard_request_contents(
      ...
      gdk_atom_intern("TARGETS", FALSE), popup_targets_received, info);
}
...
static void editable_insert_text_emit(GtkEditable *editable,
  ...
  if (*position < 0 || *position > text_length) *position = text_length;
...
static void editable_delete_text_emit(GtkEditable *editable, gint start_pos,
  ...
  if (end_pos < 0 || end_pos > text_length) end_pos = text_length;
  if (start_pos < 0) start_pos = 0;
...
static void editable_insert_text(GtkEditable *editable, const gchar *new_text,
  ...
  eel_editable_label_insert_text(label, new_text, new_text_length, &index);
...
static void editable_set_selection_bounds(GtkEditable *editable, gint start,
  ...
  if (end < 0 || end > text_length) end = text_length;
  if (start < 0) start = text_length;
...
static gboolean editable_get_selection_bounds(GtkEditable *editable,
  ...
  *start = g_utf8_pointer_to_offset(label->text,
                                    label->text + label->selection_anchor);
  *end =
      g_utf8_pointer_to_offset(label->text, label->text + label->selection_end);
...
static void editable_real_set_position(GtkEditable *editable, gint position) {
  ...
  if (index != label->selection_anchor || index != label->selection_end) {
    eel_editable_label_select_region_index(label, index, index);
  }
}
...
static const char *eel_editable_label_accessible_data =
    "eel-editable-label-accessible-data";
...
static gchar *eel_editable_label_accessible_get_text(AtkText *text,
                                                     gint start_pos,
                                                     gint end_pos) {
  GtkWidget *widget;
...
static gunichar eel_editable_label_accessible_get_character_at_offset(
...
static gchar *eel_editable_label_accessible_get_text_before_offset(
  ...
  return gail_text_util_get_text(
      priv->textutil, eel_editable_label_get_layout(label), GAIL_BEFORE_OFFSET,
      boundary_type, offset, start_offset, end_offset);
}

static gchar *eel_editable_label_accessible_get_text_at_offset(
    AtkText *text, gint offset, AtkTextBoundary boundary_type,
    gint *start_offset, gint *end_offset) {
  GtkWidget *widget;
  EelEditableLabel *label;
  ...
  priv = g_object_get_data(G_OBJECT(text), eel_editable_label_accessible_data);
  return gail_text_util_get_text(
      priv->textutil, eel_editable_label_get_layout(label), GAIL_AT_OFFSET,
      boundary_type, offset, start_offset, end_offset);
}

static gchar *eel_editable_label_accessible_get_text_after_offset(
    AtkText *text, gint offset, AtkTextBoundary boundary_type,
    gint *start_offset, gint *end_offset) {
  GtkWidget *widget;
  EelEditableLabel *label;
  ...
  priv = g_object_get_data(G_OBJECT(text), eel_editable_label_accessible_data);
  return gail_text_util_get_text(
      priv->textutil, eel_editable_label_get_layout(label), GAIL_AFTER_OFFSET,
      boundary_type, offset, start_offset, end_offset);
}

static gint eel_editable_label_accessible_get_caret_offset(AtkText *text) {
  GtkWidget *widget;
...
static gboolean eel_editable_label_accessible_set_caret_offset(AtkText *text,
                                                               gint offset) {
  GtkWidget *widget;
...
static gint eel_editable_label_accessible_get_character_count(AtkText *text) {
  GtkWidget *widget;
  EelEditableLabel *label;
...
static gint eel_editable_label_accessible_get_n_selections(AtkText *text) {
  GtkWidget *widget;
  EelEditableLabel *label;
...
static gchar *eel_editable_label_accessible_get_selection(AtkText *text,
                                                          gint selection_num,
                                                          gint *start_pos,
                                                          gint *end_pos) {
  GtkWidget *widget;
...
static gboolean eel_editable_label_accessible_add_selection(AtkText *text,
                                                            gint start_pos,
                                                            gint end_pos) {
  GtkWidget *widget;
  EelEditableLabel *label;
...
static gboolean eel_editable_label_accessible_remove_selection(
  ...
  if (selection_num != 0) return FALSE;
...
static gboolean eel_editable_label_accessible_set_selection(AtkText *text,
                                                            gint selection_num,
                                                            gint start_pos,
                                                            gint end_pos) {
  GtkWidget *widget;
...
static AtkAttributeSet *eel_editable_label_accessible_get_run_attributes(
  ...
  if (dir == GTK_TEXT_DIR_RTL) {
    at_set = gail_misc_add_attribute(
        at_set, ATK_TEXT_ATTR_DIRECTION,
        g_strdup(atk_text_attribute_get_value(ATK_TEXT_ATTR_DIRECTION, dir)));
  }

  at_set = gail_misc_layout_get_run_attributes(
      at_set, eel_editable_label_get_layout(label), label->text, offset,
      start_offset, end_offset);
  return at_set;
}

static AtkAttributeSet *eel_editable_label_accessible_get_default_attributes(
    AtkText *text) {
  GtkWidget *widget;
  EelEditableLabel *label;
  AtkAttributeSet *at_set = NULL;
  ...
  at_set = gail_misc_get_default_attributes(
      at_set, eel_editable_label_get_layout(label), widget);
  return at_set;
}

static void eel_editable_label_accessible_get_character_extents(
  ...
  if (index > cursor_index) index += label->preedit_length;
  pango_layout_index_to_pos(eel_editable_label_get_layout(label), index,
                            &char_rect);

  gail_misc_get_extents_from_pango_rectangle(
      widget, &char_rect, x_layout, y_layout, x, y, width, height, coords);
}

static gint eel_editable_label_accessible_get_offset_at_point(
  ...
  index = gail_misc_get_index_at_point_in_layout(
      widget, eel_editable_label_get_layout(label), x_layout, y_layout, x, y,
      coords);
  if (index == -1) {
...
static void eel_editable_label_accessible_insert_text(AtkEditableText *text,
                                                      const gchar *string,
                                                      gint length,
                                                      gint *position) {
  GtkWidget *widget;
...
static void eel_editable_label_accessible_copy_text(AtkEditableText *text,
                                                    gint start_pos,
                                                    gint end_pos) {
  GtkWidget *widget;
  EelEditableLabel *label;
...
static void eel_editable_label_accessible_cut_text(AtkEditableText *text,
                                                   gint start_pos,
                                                   gint end_pos) {
  GtkWidget *widget;
  EelEditableLabel *label;
...
static void eel_editable_label_accessible_delete_text(AtkEditableText *text,
                                                      gint start_pos,
                                                      gint end_pos) {
  GtkWidget *widget;
  EelEditableLabel *label;
...
static void eel_editable_label_accessible_paste_text(AtkEditableText *text,
                                                     gint position) {
  GtkWidget *widget;
  GtkEditable *editable;
  EelEditableLabelAccessiblePaste paste_struct;
  ...
  if (!gtk_editable_get_editable(editable)) return;
...
static void eel_editable_label_accessible_notify_insert(AtkObject *accessible) {
  EelEditableLabelAccessiblePrivate *priv;

  priv = g_object_get_data(G_OBJECT(accessible),
                           eel_editable_label_accessible_data);
  if (priv->signal_name) {
    g_signal_emit_by_name(accessible, priv->signal_name, priv->position,
                          priv->length);
    priv->signal_name = NULL;
  }
}

static gboolean eel_editable_label_accessible_idle_notify_insert(
    gpointer data) {
 ...
 */
static void eel_editable_label_accessible_insert_text_cb(
    EelEditableLabel *label, gchar *arg1, gint arg2, gpointer arg3) {
 ...
 */
static void eel_editable_label_accessible_delete_text_cb(
    EelEditableLabel *label, gint arg1, gint arg2) {
  AtkObject *accessible;
   ...
   */
  if (arg2 - arg1 == 0) return;

  g_signal_emit_by_name(accessible, "text_changed::delete", arg1, arg2 - arg1);
}

static void eel_editable_label_accessible_changed_cb(EelEditableLabel *label) {
  AtkObject *accessible;
  EelEditableLabelAccessiblePrivate *priv;
...
static gboolean check_for_selection_change(AtkObject *accessible,
                                           GtkWidget *widget) {
  EelEditableLabelAccessiblePrivate *priv;
  EelEditableLabel *label;
...
static void eel_editable_label_accessible_notify_gtk(GObject *obj,
                                                     GParamSpec *pspec) {
  GtkWidget *widget;
  AtkObject *accessible;
  EelEditableLabel *label;
...
static void eel_editable_label_accessible_initialize(AtkObject *accessible,
                                                     gpointer widget) {
  EelEditableLabelAccessiblePrivate *priv;
  EelEditableLabel *label;
  ...
  gail_text_util_text_setup(
      priv->textutil, eel_editable_label_get_text(EEL_EDITABLE_LABEL(widget)));
  priv->selection_anchor = label->selection_anchor;
  priv->selection_end = label->selection_end;
  g_object_set_data(G_OBJECT(accessible), eel_editable_label_accessible_data,
                    priv);
  g_signal_connect(widget, "insert-text",
                   G_CALLBACK(eel_editable_label_accessible_insert_text_cb),
                   NULL);
  g_signal_connect(widget, "delete-text",
                   G_CALLBACK(eel_editable_label_accessible_delete_text_cb),
                   NULL);
  g_signal_connect(widget, "changed",
                   G_CALLBACK(eel_editable_label_accessible_changed_cb), NULL);

  g_signal_connect(widget, "notify",
                   G_CALLBACK(eel_editable_label_accessible_notify_gtk), NULL);
  atk_object_set_role(accessible, ATK_ROLE_TEXT);
}

static const gchar *eel_editable_label_accessible_get_name(
    AtkObject *accessible) {
...
static AtkStateSet *eel_editable_label_accessible_ref_state_set(
    AtkObject *accessible) {
...
static void eel_editable_label_accessible_finalize(GObject *object) {
  EelEditableLabelAccessiblePrivate *priv;
...
static void eel_editable_label_accessible_class_init(
    EelEditableLabelAccessibleClass *klass) {
...
static void eel_editable_label_accessible_init(
    EelEditableLabelAccessible *accessible) {}
