...
static EelDimensions wrap_table_get_max_child_dimensions(
    const EelWrapTable *wrap_table);
static EelDimensions wrap_table_get_content_dimensions(
    const EelWrapTable *wrap_table);
...
static void eel_wrap_table_init(EelWrapTable *wrap_table) {
...
static void eel_wrap_table_set_property(GObject *object, guint property_id,
                                        const GValue *value,
                                        GParamSpec *pspec) {
  EelWrapTable *wrap_table;
...
static void eel_wrap_table_get_property(GObject *object, guint property_id,
...
static void eel_wrap_table_get_preferred_width(GtkWidget *widget,
                                               gint *minimum_width,
                                               gint *natural_width) {
  GtkRequisition req;
...
static void eel_wrap_table_get_preferred_height(GtkWidget *widget,
                                                gint *minimum_height,
                                                gint *natural_height) {
  GtkRequisition req;
  eel_wrap_table_size_request(widget, &req);
...
static int eel_wrap_table_draw(GtkWidget *widget, cairo_t *cr) {
  EelWrapTable *wrap_table;
  GList *iterator;
  ...
  for (iterator = wrap_table->details->children; iterator;
       iterator = iterator->next) {
    g_assert(GTK_IS_WIDGET(iterator->data));
    gtk_container_propagate_draw(GTK_CONTAINER(widget),
                                 GTK_WIDGET(iterator->data), cr);
  }
...
static void eel_wrap_table_add(GtkContainer *container, GtkWidget *child) {
  EelWrapTable *wrap_table;
  GtkWidget *widget;
  ...
  wrap_table->details->children =
      g_list_append(wrap_table->details->children, child);
  ...
  if (wrap_table->details->is_scrolled) {
    g_signal_connect(child, "focus_in_event",
                     G_CALLBACK(wrap_table_child_focus_in), wrap_table);
  }
}

static void eel_wrap_table_remove(GtkContainer *container, GtkWidget *child) {
  EelWrapTable *wrap_table;
  ...
  wrap_table->details->children =
      g_list_remove(wrap_table->details->children, child);
...
static void eel_wrap_table_class_init(EelWrapTableClass *wrap_table_class) {
  ...
  g_object_class_install_property(
      gobject_class, PROP_X_SPACING,
      g_param_spec_uint("x_spacing", NULL, NULL, 0, G_MAXINT, 0,
                        G_PARAM_READWRITE));

  g_object_class_install_property(
      gobject_class, PROP_Y_SPACING,
      g_param_spec_uint("y_spacing", NULL, NULL, 0, G_MAXINT, 0,
                        G_PARAM_READWRITE));

  g_object_class_install_property(
      gobject_class, PROP_X_JUSTIFICATION,
      g_param_spec_enum("x_justification", NULL, NULL, GTK_TYPE_JUSTIFICATION,
                        GTK_JUSTIFY_LEFT, G_PARAM_READWRITE));

  g_object_class_install_property(
      gobject_class, PROP_Y_JUSTIFICATION,
      g_param_spec_enum("y_justification", NULL, NULL, GTK_TYPE_JUSTIFICATION,
                        GTK_JUSTIFY_LEFT, G_PARAM_READWRITE));

  g_object_class_install_property(
      gobject_class, PROP_HOMOGENEOUS,
      g_param_spec_boolean("homogeneous", NULL, NULL, FALSE,
                           G_PARAM_READWRITE));
}

/* Private EelWrapTable methods */
static int wrap_table_get_num_fitting(int available, int spacing,
                                      int max_child_size) {
  int num;
...
static void wrap_table_layout(EelWrapTable *wrap_table) {
  ...
  num_cols = wrap_table_get_num_fitting(
      allocation.width -
          gtk_container_get_border_width(GTK_CONTAINER(wrap_table)) * 2,
      wrap_table->details->x_spacing, max_child_dimensions.width);
    ...
    if (gtk_widget_get_visible(item)) {
      ...
      if (wrap_table->details->homogeneous) {
        ...
        if ((pos.x + max_child_dimensions.width) > content_bounds.x1) {
          pos.x = content_bounds.x0 + wrap_table->details->x_spacing +
                  max_child_dimensions.width;
          pos.y +=
              (max_child_dimensions.height + wrap_table->details->y_spacing);
        ...
        switch (wrap_table->details->x_justification) {
          case EEL_JUSTIFICATION_MIDDLE:
            item_allocation.x +=
                (max_child_dimensions.width - (int)item_allocation.width) / 2;
            break;
        ...
        switch (wrap_table->details->y_justification) {
          case EEL_JUSTIFICATION_MIDDLE:
            item_allocation.y +=
                (max_child_dimensions.height - (int)item_allocation.height) / 2;
            break;
          case EEL_JUSTIFICATION_END:
            item_allocation.y +=
...
static EelDimensions wrap_table_get_max_child_dimensions(
  ...
  for (iterator = wrap_table->details->children; iterator;
    ...
    if (gtk_widget_get_visible(child)) {
      ...
      max_dimensions =
          wrap_table_irect_max_dimensions(&child_dimensions, &max_dimensions);
    }
  }
...
static EelDimensions wrap_table_get_content_dimensions(
  ...
  if (num_children > 0) {
    ...
    content_dimensions.height +=
        (num_rows - 1) * wrap_table->details->y_spacing;
  }
...
static gboolean wrap_table_child_focus_in(GtkWidget *widget,
  ...
  if (!wrap_table_child_visible_in(widget, viewport)) {
    ...
    gtk_adjustment_set_value(hadj,
                             MIN(x, gtk_adjustment_get_upper(hadj) -
                                        gtk_adjustment_get_page_size(hadj)));
    gtk_adjustment_set_value(vadj,
                             MIN(y, gtk_adjustment_get_upper(vadj) -
 ...
 *
 */
void eel_wrap_table_set_x_spacing(EelWrapTable *wrap_table, guint x_spacing) {
 ...
 *
 */
void eel_wrap_table_set_y_spacing(EelWrapTable *wrap_table, guint y_spacing) {
  g_return_if_fail(EEL_IS_WRAP_TABLE(wrap_table));
 ...
 */
GtkWidget *eel_wrap_table_find_child_at_event_point(
    const EelWrapTable *wrap_table, int x, int y) {
  GList *iterator;
 ...
 */
EelJustification eel_wrap_table_get_x_justification(
    const EelWrapTable *wrap_table) {
  g_return_val_if_fail(EEL_IS_WRAP_TABLE(wrap_table), 0);
 ...
 * Returns: The horizontal justification between wraps.
 */
EelJustification eel_wrap_table_get_y_justification(
    const EelWrapTable *wrap_table) {
  g_return_val_if_fail(EEL_IS_WRAP_TABLE(wrap_table), 0);
 ...
 */
void eel_wrap_table_reorder_child(EelWrapTable *wrap_table, GtkWidget *child,
                                  int position) {
  GList *node;
  ...
  g_return_if_fail((guint)position <
                   g_list_length(wrap_table->details->children));
  ...
  wrap_table->details->children =
      g_list_remove(wrap_table->details->children, child);
  wrap_table->details->children =
      g_list_insert(wrap_table->details->children, child, position);
...
GtkWidget *eel_scrolled_wrap_table_new(gboolean homogeneous,
  ...
  gtk_viewport_set_shadow_type(GTK_VIEWPORT(viewport), shadow_type);

  gtk_container_add(GTK_CONTAINER(scrolled_window), viewport);

  wrap_table = eel_wrap_table_new(homogeneous);
  gtk_container_add(GTK_CONTAINER(viewport), wrap_table);
