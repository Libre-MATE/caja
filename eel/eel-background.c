...
enum { APPEARANCE_CHANGED, SETTINGS_CHANGED, RESET, LAST_SIGNAL };
...
static void free_fade(EelBackground *self) {
...
static void free_background_surface(EelBackground *self) {
...
static void eel_background_finalize(GObject *object) {
  ...
  if (self->details->is_desktop) {
    desktop_bg_objects = g_list_remove(desktop_bg_objects, G_OBJECT(self));
  }

  G_OBJECT_CLASS(eel_background_parent_class)->finalize(object);
}

static void eel_background_unrealize(EelBackground *self) {
  free_background_surface(self);
...
static void make_color_inactive(EelBackground *self, GdkRGBA *color) {
...
static void set_image_properties(EelBackground *self) {
  GdkRGBA c;
...
void eel_background_set_color(EelBackground *self, const gchar *color) {
...
static void drawable_get_adjusted_size(EelBackground *self, int *width,
                                       int *height) {
  if (self->details->is_desktop) {
...
static gboolean eel_background_ensure_realized(EelBackground *self) {
  int width, height;
  GdkWindow *window;
  ...
  self->details->bg_surface = mate_bg_create_surface(
      self->details->bg, window, width, height, self->details->is_desktop);
...
static void set_root_surface(EelBackground *self, GdkWindow *window,
                             GdkScreen *screen) {
  eel_background_ensure_realized(self);
  ...
  if (self->details->use_common_surface) {
    ...
    int width, height;
    drawable_get_adjusted_size(self, &width, &height);
    self->details->bg_surface =
        mate_bg_create_surface(self->details->bg, window, width, height, TRUE);
  }
...
static void init_fade(EelBackground *self) {
  GSettings *mate_background_preferences;
  GtkWidget *widget = self->details->widget;
  gboolean do_fade;

  if (!self->details->is_desktop || widget == NULL ||
  ...
  if (self->details->fade == NULL) {
    ...
    if (width == self->details->bg_entire_width &&
      ...
      g_signal_connect_swapped(self->details->fade, "finished",
                               G_CALLBACK(free_fade), self);
    }
  }
  ...
  if (self->details->fade != NULL &&
    ...
    if (self->details->bg_surface == NULL) {
      ...
      start_surface =
          mate_bg_get_surface_from_root(gtk_widget_get_screen(widget));
      mate_bg_crossfade_set_start_surface(self->details->fade, start_surface);
      cairo_surface_destroy(start_surface);
    } else {
      mate_bg_crossfade_set_start_surface(self->details->fade,
                                          self->details->bg_surface);
    }
  }
}

static void on_fade_finished(MateBGCrossfade *fade, GdkWindow *window,
                             gpointer user_data) {
...
static gboolean fade_to_surface(EelBackground *self, GtkWidget *widget,
                                cairo_surface_t *surface) {
  if (self->details->fade == NULL ||
  ...
  if (!mate_bg_crossfade_is_started(self->details->fade)) {
    ...
    if (self->details->is_desktop) {
      g_signal_connect(self->details->fade, "finished",
...
static void eel_background_set_up_widget(EelBackground *self) {
  GtkWidget *widget = self->details->widget;
  ...
  if (self->details->bg_surface == NULL) return;
...
static gboolean background_changed_cb(EelBackground *self) {
...
static void widget_queue_background_change(GtkWidget *widget,
                                           gpointer user_data) {
  EelBackground *self = EEL_BACKGROUND(user_data);
 ...
 */
static void widget_style_updated_cb(GtkWidget *widget, gpointer user_data) {
  widget_queue_background_change(widget, user_data);
}

static void eel_background_changed(MateBG *bg, gpointer user_data) {
  EelBackground *self = EEL_BACKGROUND(user_data);
...
static void eel_background_transitioned(MateBG *bg, gpointer user_data) {
  EelBackground *self = EEL_BACKGROUND(user_data);
...
static void screen_size_changed(GdkScreen *screen, EelBackground *background) {
  int w, h;
...
static void widget_realized_setup(GtkWidget *widget, EelBackground *self) {
  if (!self->details->is_desktop) {
    return;
  }
  ...
  self->details->screen_size_handler = g_signal_connect(
      screen, "size_changed", G_CALLBACK(screen_size_changed), self);

  if (self->details->screen_monitors_handler > 0) {
    g_signal_handler_disconnect(screen, self->details->screen_monitors_handler);
  }
  self->details->screen_monitors_handler = g_signal_connect(
      screen, "monitors-changed", G_CALLBACK(screen_size_changed), self);

  self->details->use_common_surface =
      (gdk_window_get_visual(window) == gtk_widget_get_visual(widget)) ? TRUE
                                                                       : FALSE;

  init_fade(self);
}

static void widget_realize_cb(GtkWidget *widget, gpointer user_data) {
  EelBackground *self = EEL_BACKGROUND(user_data);
...
static void widget_unrealize_cb(GtkWidget *widget, gpointer user_data) {
  EelBackground *self = EEL_BACKGROUND(user_data);
...
EelBackground *eel_get_widget_background(GtkWidget *widget) {
  ...
  g_object_set_data_full(G_OBJECT(widget), "eel_background", self,
                         g_object_unref);

  g_signal_connect_object(widget, "destroy", G_CALLBACK(on_widget_destroyed),
                          self, 0);
  g_signal_connect_object(widget, "realize", G_CALLBACK(widget_realize_cb),
                          self, 0);
  g_signal_connect_object(widget, "unrealize", G_CALLBACK(widget_unrealize_cb),
                          self, 0);
  ...
  g_signal_connect_object(self, "appearance_changed",
                          G_CALLBACK(widget_queue_background_change), widget,
                          G_CONNECT_SWAPPED);
...
static void eel_background_class_init(EelBackgroundClass *klass) {
  ...
  signals[SETTINGS_CHANGED] = g_signal_new(
      "settings_changed", G_TYPE_FROM_CLASS(object_class),
      G_SIGNAL_RUN_LAST | G_SIGNAL_NO_RECURSE,
      G_STRUCT_OFFSET(EelBackgroundClass, settings_changed), NULL, NULL,
      g_cclosure_marshal_VOID__INT, G_TYPE_NONE, 1, G_TYPE_INT);

  signals[RESET] =
      g_signal_new("reset", G_TYPE_FROM_CLASS(object_class),
                   G_SIGNAL_RUN_LAST | G_SIGNAL_NO_RECURSE,
                   G_STRUCT_OFFSET(EelBackgroundClass, reset), NULL, NULL,
                   g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  object_class->finalize = eel_background_finalize;
}

static void eel_background_init(EelBackground *self) {
...
void eel_background_set_desktop(EelBackground *self, gboolean is_desktop) {
  ...
  if (is_desktop) {
    ...
    desktop_bg_objects = g_list_prepend(desktop_bg_objects, G_OBJECT(self));
...
/* determine if a background is darker or lighter than average, to help clients
   know what colors to draw on top with */
...
static void eel_bg_set_image_uri_helper(EelBackground *self,
 ...
 */
static void eel_bg_set_image_uri_and_color(EelBackground *self,
...
void eel_bg_set_placement(EelBackground *self, MateBGPlacement placement) {
  if (self->details->bg) mate_bg_set_placement(self->details->bg, placement);
}

void eel_bg_save_to_gsettings(EelBackground *self, GSettings *settings) {
  if (self->details->bg) mate_bg_save_to_gsettings(self->details->bg, settings);
}
...
void eel_bg_load_from_gsettings(EelBackground *self, GSettings *settings) {
  ...
  picture_filename =
      g_settings_get_string(settings, MATE_BG_KEY_PICTURE_FILENAME);
  ...
  if (self->details->bg)
    mate_bg_load_from_gsettings(self->details->bg, settings);
}

void eel_bg_load_from_system_gsettings(EelBackground *self, GSettings *settings,
                                       gboolean apply) {
  if (self->details->bg)
    mate_bg_load_from_system_gsettings(self->details->bg, settings, apply);
}
...
void eel_background_set_dropped_color(EelBackground *self, GtkWidget *widget,
  ...
  channels = (guint16 *)gtk_selection_data_get_data(
      (GtkSelectionData *)selection_data);
  color_spec = g_strdup_printf("#%02X%02X%02X", channels[0] >> 8,
                               channels[1] >> 8, channels[2] >> 8);

  /* Figure out if the color was dropped close enough to an edge to create a
     gradient. For the moment, this is hard-wired, but later the widget will
     have to have some say in where the borders are.
  */
  ...
  if (drop_location_x < left_border && drop_location_x <= right_border) {
    new_gradient_spec =
        eel_gradient_set_left_color_spec(gradient_spec, color_spec);
  } else if (drop_location_x >= left_border && drop_location_x > right_border) {
    new_gradient_spec =
        eel_gradient_set_right_color_spec(gradient_spec, color_spec);
  } else if (drop_location_y < top_border && drop_location_y <= bottom_border) {
    new_gradient_spec =
        eel_gradient_set_top_color_spec(gradient_spec, color_spec);
  } else if (drop_location_y >= top_border && drop_location_y > bottom_border) {
    new_gradient_spec =
        eel_gradient_set_bottom_color_spec(gradient_spec, color_spec);
  } else {
...
void eel_self_check_background(void) {
