...
static void eel_canvas_re_set_property(GObject *object, guint param_id,
                                       const GValue *value, GParamSpec *pspec);
static void eel_canvas_re_get_property(GObject *object, guint param_id,
                                       GValue *value, GParamSpec *pspec);

static void eel_canvas_re_update_shared(EelCanvasItem *item, double i2w_dx,
...
static void eel_canvas_re_bounds(EelCanvasItem *item, double *x1, double *y1,
                                 double *x2, double *y2);
static void eel_canvas_re_translate(EelCanvasItem *item, double dx, double dy);
static void eel_canvas_rect_update(EelCanvasItem *item, double i2w_dx,
                                   double i2w_dy, int flags);
...
GType eel_canvas_re_get_type(void) {
  ...
  if (!re_type) {
    ...
    re_type = g_type_register_static(eel_canvas_item_get_type(), "EelCanvasRE",
                                     &re_info, 0);
  }
...
static void eel_canvas_re_class_init(EelCanvasREClass *klass) {
  ...
  g_object_class_install_property(
      gobject_class, PROP_X1,
      g_param_spec_double("x1", NULL, NULL, -G_MAXDOUBLE, G_MAXDOUBLE, 0,
                          G_PARAM_READWRITE));
  g_object_class_install_property(
      gobject_class, PROP_Y1,
      g_param_spec_double("y1", NULL, NULL, -G_MAXDOUBLE, G_MAXDOUBLE, 0,
                          G_PARAM_READWRITE));
  g_object_class_install_property(
      gobject_class, PROP_X2,
      g_param_spec_double("x2", NULL, NULL, -G_MAXDOUBLE, G_MAXDOUBLE, 0,
                          G_PARAM_READWRITE));
  g_object_class_install_property(
      gobject_class, PROP_Y2,
      g_param_spec_double("y2", NULL, NULL, -G_MAXDOUBLE, G_MAXDOUBLE, 0,
                          G_PARAM_READWRITE));
  g_object_class_install_property(
      gobject_class, PROP_FILL_COLOR_RGBA,
      g_param_spec_boxed("fill-color-rgba", NULL, NULL, GDK_TYPE_RGBA,

                         G_PARAM_READWRITE));
  g_object_class_install_property(
      gobject_class, PROP_OUTLINE_COLOR_RGBA,
      g_param_spec_boxed("outline-color-rgba", NULL, NULL, GDK_TYPE_RGBA,
                         G_PARAM_READWRITE));

  g_object_class_install_property(
      gobject_class, PROP_OUTLINE_STIPPLING,
      g_param_spec_boolean("outline-stippling", NULL, NULL, FALSE,
                           G_PARAM_READWRITE));
  g_object_class_install_property(
      gobject_class, PROP_WIDTH_PIXELS,
      g_param_spec_uint("width-pixels", NULL, NULL, 0, G_MAXUINT, 0,
                        G_PARAM_READWRITE));
  g_object_class_install_property(
      gobject_class, PROP_WIDTH_UNITS,
      g_param_spec_double("width-units", NULL, NULL, 0.0, G_MAXDOUBLE, 0.0,
                          G_PARAM_READWRITE));
...
static void eel_canvas_re_set_fill(EelCanvasRE *re, gboolean fill_set) {
...
static void eel_canvas_re_set_outline(EelCanvasRE *re, gboolean outline_set) {
...
#endif
  if (re_parent_class->realize) (*re_parent_class->realize)(item);
}

static void eel_canvas_re_unrealize(EelCanvasItem *item) {
  if (re_parent_class->unrealize) (*re_parent_class->unrealize)(item);
}
...
static void eel_canvas_rect_draw(EelCanvasItem *item, cairo_t *cr,
                                 cairo_region_t *region);

static double eel_canvas_rect_point(EelCanvasItem *item, double x, double y,
                                    int cx, int cy,
...
static void eel_canvas_set_source_color(cairo_t *cr, GdkRGBA *rgba) {
  ...
  if (re->outline_set) {
    ...
    if (re->width_pixels) {
      ...
      cairo_set_line_width(
          cr, (int)(re->width * re->item.canvas->pixels_per_unit + 0.5));
    }
    ...
    cairo_rectangle(cr, cx1 + 0.5, cy1 + 0.5, cx2 - cx1, cy2 - cy1);
    cairo_stroke(cr);
  }
  cairo_restore(cr);
}
...
static double eel_canvas_rect_point(EelCanvasItem *item, double x, double y,
  ...
  *actual_item = item;
  ...
  if ((x >= x1) && (y >= y1) && (x <= x2) && (y <= y2)) {
    ...
    if (re->fill_set || !re->outline_set) return 0.0;
...
static void request_redraw_borders(EelCanvas *canvas, Rect *update_rect,
                                   int width) {
  eel_canvas_request_redraw(canvas, update_rect->x0, update_rect->y0,
                            update_rect->x1, update_rect->y0 + width);
  eel_canvas_request_redraw(canvas, update_rect->x0, update_rect->y1 - width,
                            update_rect->x1, update_rect->y1);
  eel_canvas_request_redraw(canvas, update_rect->x0, update_rect->y0,
                            update_rect->x0 + width, update_rect->y1);
  eel_canvas_request_redraw(canvas, update_rect->x1 - width, update_rect->y0,
                            update_rect->x1, update_rect->y1);
}
...
static void eel_canvas_rect_update(EelCanvasItem *item, double i2w_dx,
  ...
  diff_rects(update_rect, priv->last_update_rect, &repaint_rects_count,
             repaint_rects);
  for (i = 0; i < repaint_rects_count; i++) {
    eel_canvas_request_redraw(item->canvas, repaint_rects[i].x0,
                              repaint_rects[i].y0, repaint_rects[i].x1,
                              repaint_rects[i].y1);
  }
  ...
  if (re->outline_set) {
    ...
    request_redraw_borders(item->canvas, &update_rect, (width_lt + width_rb));
...
static int rect_empty(const Rect *src) {
...
static gboolean rects_intersect(Rect r1, Rect r2) {
  if (r1.x0 >= r2.x1) {
    return FALSE;
  }
  if (r2.x0 >= r1.x1) {
...
static void diff_rects_guts(Rect ra, Rect rb, int *count, Rect result[4]) {
  if (ra.x0 < rb.x0) {
