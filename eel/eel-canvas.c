 ...
 *
 * - Allow to specify whether EelCanvasImage sizes are in units or pixels (scale
 * or don't scale).
 *
 * - Implement a flag for eel_canvas_item_reparent() that tells the function to
 * keep the item visually in the same place, that is, to keep it in the same
 * place with respect to the canvas origin.
 *
 * - GC put functions for items.
 *
 * - Widget item (finish it).
 *
 * - GList *eel_canvas_gimme_all_items_contained_in_this_area (EelCanvas
...
static void group_add(EelCanvasGroup *group, EelCanvasItem *item);
static void group_remove(EelCanvasGroup *group, EelCanvasItem *item);
...
enum { ITEM_PROP_0, ITEM_PROP_PARENT, ITEM_PROP_VISIBLE };

enum { ITEM_DESTROY, ITEM_EVENT, ITEM_LAST_SIGNAL };
 ...
 **/
GType eel_canvas_item_get_type(void) {
  ...
  if (!canvas_item_type) {
    ...
    canvas_item_type = g_type_register_static(
        G_TYPE_INITIALLY_UNOWNED, "EelCanvasItem", &canvas_item_info, 0);
  }
 ...
 * group, etc.)
 */
static void item_post_create_setup(EelCanvasItem *item) {
...
static void eel_canvas_item_set_property(GObject *gobject, guint param_id,
                                         const GValue *value,
                                         GParamSpec *pspec) {
  EelCanvasItem *item;
  ...
  switch (param_id) {
    case ITEM_PROP_PARENT:
      if (item->parent != NULL) {
        g_warning(
            "Cannot set `parent' argument after item has "
...
static void eel_canvas_item_get_property(GObject *gobject, guint param_id,
...
static void eel_canvas_item_dispose(GObject *object) {
  EelCanvasItem *item;
  ...
  if (item->canvas) {
    ...
    if (item == item->canvas->focused_item) item->canvas->focused_item = NULL;
    ...
    if (item->parent) group_remove(EEL_CANVAS_GROUP(item->parent), item);
...
static void eel_canvas_item_unrealize(EelCanvasItem *item) {
  if (item->flags & EEL_CANVAS_ITEM_MAPPED)
    (*EEL_CANVAS_ITEM_GET_CLASS(item)->unmap)(item);
...
static void eel_canvas_item_map(EelCanvasItem *item) {
  item->flags |= EEL_CANVAS_ITEM_MAPPED;
}

/* Unmap handler for canvas items */
static void eel_canvas_item_unmap(EelCanvasItem *item) {
  item->flags &= ~(EEL_CANVAS_ITEM_MAPPED);
}

/* Update handler for canvas items */
static void eel_canvas_item_update(EelCanvasItem *item, double i2w_dx,
                                   double i2w_dy, int flags) {
...
static void eel_canvas_item_invoke_update(EelCanvasItem *item, double i2w_dx,
                                          double i2w_dy, int flags) {
  int child_flags;
  ...
  if (child_flags & GCI_UPDATE_MASK) {
    if (EEL_CANVAS_ITEM_GET_CLASS(item)->update)
      EEL_CANVAS_ITEM_GET_CLASS(item)->update(item, i2w_dx, i2w_dy,
                                              child_flags);
  }
...
static double eel_canvas_item_invoke_point(EelCanvasItem *item, double x,
                                           double y, int cx, int cy,
                                           EelCanvasItem **actual_item) {
  /* Calculate x & y in item local coordinates */

  if (EEL_CANVAS_ITEM_GET_CLASS(item)->point)
    return EEL_CANVAS_ITEM_GET_CLASS(item)->point(item, x, y, cx, cy,
                                                  actual_item);
 ...
 * @args: The list of object argument name/value pairs used to configure the
 *item.
 *
 * Configures a canvas item.  The arguments in the item are set to the specified
 * values, and the item is repainted as appropriate.
...
void eel_canvas_item_move(EelCanvasItem *item, double dx, double dy) {
  ...
  if (item->flags & EEL_CANVAS_ITEM_MAPPED) item->canvas->need_repick = TRUE;
...
static void eel_canvas_queue_resize(EelCanvas *canvas) {
 ...
 */
static gboolean put_item_after(GList *link, GList *before) {
  ...
  if (before == NULL) {
    if (link == parent->item_list) return FALSE;
    ...
    if ((link == parent->item_list_end) &&
        (before == parent->item_list_end->prev))
      return FALSE;
...
void eel_canvas_item_raise(EelCanvasItem *item, int positions) {
  ...
  if (!item->parent || positions == 0) return;

  parent = EEL_CANVAS_GROUP(item->parent);
  link = g_list_find(parent->item_list, item);
  g_assert(link != NULL);

  for (before = link; positions && before; positions--) before = before->next;

  if (!before) before = parent->item_list_end;
 ...
 */
static void seat_grab_prepare_window(GdkSeat *seat, GdkWindow *window,
                                     gpointer user_data) {
 ...
 *eel_canvas_item_hide(), then it returns %GDK_GRAB_NOT_VIEWABLE. Else, it
 *returns the result of calling gdk_seat_grab().
 **/
GdkGrabStatus eel_canvas_item_grab(EelCanvasItem *item, GdkEventMask event_mask,
                                   GdkCursor *cursor, const GdkEvent *event) {
  ...
  if (item->canvas->grabbed_item) return GDK_GRAB_ALREADY_GRABBED;

  if (!(item->flags & EEL_CANVAS_ITEM_MAPPED)) return GDK_GRAB_NOT_VIEWABLE;

  display = gtk_widget_get_display(GTK_WIDGET(item->canvas));
  seat = gdk_display_get_default_seat(display);

  retval =
      gdk_seat_grab(seat, gtk_layout_get_bin_window(GTK_LAYOUT(item->canvas)),
                    GDK_SEAT_CAPABILITY_ALL_POINTING, FALSE, cursor, event,
                    seat_grab_prepare_window, NULL);

  if (retval != GDK_GRAB_SUCCESS) return retval;
...
void eel_canvas_item_ungrab(EelCanvasItem *item) {
  ...
  if (item->canvas->grabbed_item != item) return;
...
static int is_descendant(EelCanvasItem *item, EelCanvasItem *parent) {
 ...
 **/
void eel_canvas_item_get_bounds(EelCanvasItem *item, double *x1, double *y1,
                                double *x2, double *y2) {
...
void eel_canvas_item_request_update(EelCanvasItem *item) {
  if (NULL == item->canvas) return;

  g_return_if_fail(!item->canvas->doing_update);

  if (item->flags & EEL_CANVAS_ITEM_NEED_UPDATE) return;
  ...
  if (item->parent != NULL) {
    ...
    /* Have reached the top of the tree, make sure the update call gets
     * scheduled. */
    eel_canvas_request_update(item->canvas);
  }
}
...
void eel_canvas_item_request_redraw(EelCanvasItem *item) {
  if (item->flags & EEL_CANVAS_ITEM_MAPPED)
    eel_canvas_request_redraw(item->canvas, item->x1, item->y1, item->x2 + 1,
                              item->y2 + 1);
}

/*** EelCanvasGroup ***/

enum { GROUP_PROP_0, GROUP_PROP_X, GROUP_PROP_Y };

static void eel_canvas_group_class_init(EelCanvasGroupClass *klass);
static void eel_canvas_group_init(EelCanvasGroup *group);
static void eel_canvas_group_set_property(GObject *object, guint param_id,
                                          const GValue *value,
                                          GParamSpec *pspec);
static void eel_canvas_group_get_property(GObject *object, guint param_id,
                                          GValue *value, GParamSpec *pspec);

static void eel_canvas_group_destroy(EelCanvasItem *object);

static void eel_canvas_group_update(EelCanvasItem *item, double i2w_dx,
                                    double i2w_dy, int flags);
...
static void eel_canvas_group_draw(EelCanvasItem *item, cairo_t *cr,
                                  cairo_region_t *region);
static double eel_canvas_group_point(EelCanvasItem *item, double x, double y,
                                     int cx, int cy,
                                     EelCanvasItem **actual_item);
static void eel_canvas_group_translate(EelCanvasItem *item, double dx,
                                       double dy);
 ...
 **/
GType eel_canvas_group_get_type(void) {
...
static void eel_canvas_group_class_init(EelCanvasGroupClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_double("x", _("X"), _("X"), -G_MAXDOUBLE, G_MAXDOUBLE, 0.0,
                          G_PARAM_READWRITE));
  g_object_class_install_property(
      gobject_class, GROUP_PROP_Y,
      g_param_spec_double("y", _("Y"), _("Y"), -G_MAXDOUBLE, G_MAXDOUBLE, 0.0,
                          G_PARAM_READWRITE));
...
static void eel_canvas_group_set_property(GObject *gobject, guint param_id,
                                          const GValue *value,
                                          GParamSpec *pspec) {
  EelCanvasItem *item;
  ...
  switch (param_id) {
    case GROUP_PROP_X:
      ...
      if (old != group->xpos) moved = TRUE;
      break;

    case GROUP_PROP_Y:
      old = group->ypos;
      group->ypos = g_value_get_double(value);
      if (old != group->ypos) moved = TRUE;
      break;
...
static void eel_canvas_group_get_property(GObject *gobject, guint param_id,
                                          GValue *value, GParamSpec *pspec) {
  EelCanvasGroup *group;
...
static void eel_canvas_group_update(EelCanvasItem *item, double i2w_dx,
  ...
  for (list = group->item_list; list; list = list->next) {
    ...
    eel_canvas_item_invoke_update(i, i2w_dx + group->xpos, i2w_dy + group->ypos,
                                  flags);
...
static void eel_canvas_group_draw(EelCanvasItem *item, cairo_t *cr,
  ...
  for (list = group->item_list; list; list = list->next) {
    ...
    if ((child->flags & EEL_CANVAS_ITEM_MAPPED) &&
      ...
      if (cairo_region_contains_rectangle(region, &child_rect) !=
...
static double eel_canvas_group_point(EelCanvasItem *item, double x, double y,
  ...
  *actual_item = NULL;
  ...
  for (list = group->item_list; list; list = list->next) {
    ...
    if ((child->x1 > x2) || (child->y1 > y2) || (child->x2 < x1) ||
        (child->y2 < y1))
      continue;
...
void eel_canvas_group_translate(EelCanvasItem *item, double dx, double dy) {
  EelCanvasGroup *group;
...
static void eel_canvas_group_bounds(EelCanvasItem *item, double *x1, double *y1,
  ...
  for (; list; list = list->next) {
    ...
    if (!(child->flags & EEL_CANVAS_ITEM_MAPPED)) continue;
...
enum { DRAW_BACKGROUND, LAST_SIGNAL };
...
static gint eel_canvas_button(GtkWidget *widget, GdkEventButton *event);
static gint eel_canvas_motion(GtkWidget *widget, GdkEventMotion *event);
static gint eel_canvas_draw(GtkWidget *widget, cairo_t *cr);
static gint eel_canvas_key(GtkWidget *widget, GdkEventKey *event);
static gint eel_canvas_crossing(GtkWidget *widget, GdkEventCrossing *event);
static gint eel_canvas_focus_in(GtkWidget *widget, GdkEventFocus *event);
static gint eel_canvas_focus_out(GtkWidget *widget, GdkEventFocus *event);
static void eel_canvas_request_update_real(EelCanvas *canvas);
static void eel_canvas_draw_background(EelCanvas *canvas, cairo_t *cr);
 ...
 **/
GType eel_canvas_get_type(void) {
  ...
  if (!canvas_type) {
    ...
    canvas_type = g_type_register_static(gtk_layout_get_type(), "EelCanvas",
                                         &canvas_info, 0);
  }
...
static void eel_canvas_get_property(GObject *object, guint prop_id,
                                    GValue *value, GParamSpec *pspec) {
  switch (prop_id) {
    default:
...
static void eel_canvas_set_property(GObject *object, guint prop_id,
...
static void eel_canvas_accessible_adjustment_changed(GtkAdjustment *adjustment,
                                                     gpointer data) {
  AtkObject *atk_obj;
...
static void eel_canvas_accessible_initialize(AtkObject *obj, gpointer data) {
  EelCanvas *canvas;
  ...
  g_signal_connect(gtk_scrollable_get_hadjustment(GTK_SCROLLABLE(canvas)),
                   "value_changed",
                   G_CALLBACK(eel_canvas_accessible_adjustment_changed), obj);
...
static gint eel_canvas_accessible_get_n_children(AtkObject *obj) {
...
static AtkObject *eel_canvas_accessible_ref_child(AtkObject *obj, gint i) {
  GtkAccessible *accessible;
  GtkWidget *widget;
  EelCanvas *canvas;
  EelCanvasGroup *root_group;
...
G_DEFINE_TYPE(EelCanvasAccessible, eel_canvas_accessible,
              GTK_TYPE_CONTAINER_ACCESSIBLE)

static void eel_canvas_accessible_class_init(EelCanvasAccessibleClass *klass) {
...
static void eel_canvas_accessible_init(EelCanvasAccessible *accessible) {}
...
static void eel_canvas_class_init(EelCanvasClass *klass) {
  ...
  canvas_signals[DRAW_BACKGROUND] = g_signal_new(
      "draw_background", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(EelCanvasClass, draw_background), NULL, NULL,
...
static void eel_canvas_init(EelCanvas *canvas) {
  ...
  canvas->root =
      EEL_CANVAS_ITEM(g_object_new(eel_canvas_group_get_type(), NULL));
...
static void remove_idle(EelCanvas *canvas) {
...
static void shutdown_transients(EelCanvas *canvas) {
...
static void eel_canvas_destroy(GtkWidget *object) {
  ...
  if (canvas->root_destroy_id) {
    g_signal_handler_disconnect(G_OBJECT(canvas->root),
                                canvas->root_destroy_id);
 ...
 */
static void scroll_to(EelCanvas *canvas, int cx, int cy) {
  ...
  scroll_width = floor(
      (canvas->scroll_x2 - canvas->scroll_x1) * canvas->pixels_per_unit + 0.5);
  scroll_height = floor(
      (canvas->scroll_y2 - canvas->scroll_y1) * canvas->pixels_per_unit + 0.5);
  ...
  if (changed_x) gtk_adjustment_set_value(hadjustment, (gdouble)cx);

  adjustment_value = gtk_adjustment_get_value(vadjustment);
  changed_y = ((int)adjustment_value) != cy;
  if (changed_y) gtk_adjustment_set_value(vadjustment, (gdouble)cy);

  gtk_layout_get_size(&canvas->layout, &width, &height);
  if ((scroll_width != (int)width) || (scroll_height != (int)height)) {
    gtk_layout_set_size(GTK_LAYOUT(canvas), (guint)scroll_width,
                        (guint)scroll_height);
  }

  /* Signal GtkLayout that it should do a redraw. */
  if (changed_x) g_signal_emit_by_name(hadjustment, "value_changed");
  if (changed_y) g_signal_emit_by_name(vadjustment, "value_changed");
}
...
static void eel_canvas_size_allocate(GtkWidget *widget,
  ...
  scroll_to(canvas, gtk_adjustment_get_value(hadjustment),
...
static int emit_event(EelCanvas *canvas, GdkEvent *event) {
  ...
  switch (ev.type) {
    case GDK_ENTER_NOTIFY:
    case GDK_LEAVE_NOTIFY:
      eel_canvas_window_to_world(canvas, ev.crossing.x, ev.crossing.y,
    ...
    case GDK_3BUTTON_PRESS:
    case GDK_BUTTON_RELEASE:
      eel_canvas_window_to_world(canvas, ev.motion.x, ev.motion.y, &ev.motion.x,
                                 &ev.motion.y);
      break;
 ...
 */
static int pick_current_item(EelCanvas *canvas, GdkEvent *event) {
  int button_down;
  double x, y;
  int retval;
   ...
   */
  button_down =
      canvas->state & (GDK_BUTTON1_MASK | GDK_BUTTON2_MASK | GDK_BUTTON3_MASK |
  ...
  if ((canvas->new_current_item != canvas->current_item) &&
      (canvas->current_item != NULL) && !canvas->left_grabbed_item) {
    GdkEvent new_event;
  ...
  /* new_current_item may have been set to NULL during the call to emit_event()
   * above */
...
static gint eel_canvas_button(GtkWidget *widget, GdkEventButton *event) {
  ...
  if (event->button > 5) return FALSE;
   ...
   */
  if (!canvas->grabbed_item &&
      event->window != gtk_layout_get_bin_window(GTK_LAYOUT(canvas)))
    return retval;
...
static gint eel_canvas_key(GtkWidget *widget, GdkEventKey *event) {
  ...
  if (emit_event(canvas, (GdkEvent *)event)) return TRUE;
  if (event->type == GDK_KEY_RELEASE)
    return GTK_WIDGET_CLASS(canvas_parent_class)
...
static cairo_region_t *eel_cairo_get_clip_region(cairo_t *cr) {
  ...
  if (list->status == CAIRO_STATUS_CLIP_NOT_REPRESENTABLE) {
    ...
    if (!gdk_cairo_get_clip_rectangle(cr, &clip_rect)) return NULL;
  ...
  for (i = list->num_rectangles - 1; i >= 0; --i) {
    ...
    if (cairo_region_union_rectangle(region, &clip_rect) !=
...
static gboolean eel_canvas_draw(GtkWidget *widget, cairo_t *cr) {
  EelCanvas *canvas = EEL_CANVAS(widget);
  GdkWindow *bin_window;
  cairo_region_t *region;

  if (!gdk_cairo_get_clip_rectangle(cr, NULL)) return FALSE;

  bin_window = gtk_layout_get_bin_window(GTK_LAYOUT(widget));

  if (!gtk_cairo_should_draw_window(cr, bin_window)) return FALSE;
  ...
  g_signal_emit(G_OBJECT(canvas), canvas_signals[DRAW_BACKGROUND], 0, cr);
...
static void eel_canvas_draw_background(EelCanvas *canvas, cairo_t *cr) {
  ...
  if (!gdk_cairo_get_clip_rectangle(cr, &rect)) return;
...
static void do_update(EelCanvas *canvas) {
...
static void add_idle(EelCanvas *canvas) {
  if (!canvas->idle_id) {
     ...
     */
    canvas->idle_id =
        g_idle_add_full(GDK_PRIORITY_REDRAW - 20, idle_handler, canvas, NULL);
  }
}
 ...
 **/
void eel_canvas_set_scroll_region(EelCanvas *canvas, double x1, double y1,
                                  double x2, double y2) {
  double wxofs, wyofs;
  int xofs, yofs;
  GtkAdjustment *vadjustment, *hadjustment;
  ...
  /*
   * Set the new scrolling region.  If possible, do not move the visible
 ...
 **/
void eel_canvas_get_scroll_region(EelCanvas *canvas, double *x1, double *y1,
                                  double *x2, double *y2) {
  g_return_if_fail(EEL_IS_CANVAS(canvas));

  if (x1) *x1 = canvas->scroll_x1;

  if (y1) *y1 = canvas->scroll_y1;

  if (x2) *x2 = canvas->scroll_x2;

  if (y2) *y2 = canvas->scroll_y2;
}

void eel_canvas_set_center_scroll_region(EelCanvas *canvas,
...
void eel_canvas_set_pixels_per_unit(EelCanvas *canvas, double n) {
  ...
  hadjustment = gtk_scrollable_get_hadjustment(GTK_SCROLLABLE(canvas));
  vadjustment = gtk_scrollable_get_vadjustment(GTK_SCROLLABLE(canvas));
  cx = (gtk_adjustment_get_value(hadjustment) + center_x) /
           canvas->pixels_per_unit +
       canvas->scroll_x1 + canvas->zoom_xofs;
  cy = (gtk_adjustment_get_value(vadjustment) + center_y) /
           canvas->pixels_per_unit +
...
void eel_canvas_get_scroll_offsets(EelCanvas *canvas, int *cx, int *cy) {
  ...
  if (cx) *cx = gtk_adjustment_get_value(hadjustment);

  if (cy) *cy = gtk_adjustment_get_value(vadjustment);
}
 ...
 **/
void eel_canvas_request_redraw(EelCanvas *canvas, int x1, int y1, int x2,
                               int y2) {
  GdkRectangle bbox;
 ...
 **/
void eel_canvas_w2c_rect_d(EelCanvas *canvas, double *x1, double *y1,
                           double *x2, double *y2) {
  eel_canvas_w2c_d(canvas, *x1, *y1, x1, y1);
  eel_canvas_w2c_d(canvas, *x2, *y2, x2, y2);
}
 ...
 **/
void eel_canvas_w2c_d(EelCanvas *canvas, double wx, double wy, double *cx,
                      double *cy) {
  double zoom;
  ...
  if (cx) *cx = (wx - canvas->scroll_x1) * zoom + canvas->zoom_xofs;
  if (cy) *cy = (wy - canvas->scroll_y1) * zoom + canvas->zoom_yofs;
}
...
void eel_canvas_c2w(EelCanvas *canvas, int cx, int cy, double *wx, double *wy) {
  ...
  if (wx) *wx = (cx - canvas->zoom_xofs) / zoom + canvas->scroll_x1;
  if (wy) *wy = (cy - canvas->zoom_yofs) / zoom + canvas->scroll_y1;
}
 ...
 **/
void eel_canvas_window_to_world(EelCanvas *canvas, double winx, double winy,
                                double *worldx, double *worldy) {
  g_return_if_fail(EEL_IS_CANVAS(canvas));

  if (worldx)
    *worldx = canvas->scroll_x1 +
              ((winx - canvas->zoom_xofs) / canvas->pixels_per_unit);

  if (worldy)
    *worldy = canvas->scroll_y1 +
              ((winy - canvas->zoom_yofs) / canvas->pixels_per_unit);
}
 ...
 * function is here for backwards compatibility reasons.
 **/
void eel_canvas_world_to_window(EelCanvas *canvas, double worldx, double worldy,
...
static guint eel_canvas_item_accessible_add_focus_handler(
  ...
  if (!g_signal_handler_find(component, match_type, signal_id, 0, NULL,
    ...
    return g_signal_connect_closure_by_id(
        ...
        g_cclosure_new(G_CALLBACK(handler), NULL, (GClosureNotify)NULL), FALSE);
  }
  return 0;
}

static void eel_canvas_item_accessible_get_item_extents(EelCanvasItem *item,
                                                        GdkRectangle *rect) {
...
static gboolean eel_canvas_item_accessible_is_item_in_window(
    EelCanvasItem *item, GdkRectangle *rect) {
  GtkWidget *widget;
  gboolean retval;
...
static void eel_canvas_item_accessible_get_extents(AtkComponent *component,
                                                   gint *x, gint *y,
...
static gint eel_canvas_item_accessible_get_mdi_zorder(AtkComponent *component) {
  AtkGObjectAccessible *atk_gobj;
  GObject *g_obj;
  EelCanvasItem *item;
...
static gboolean eel_canvas_item_accessible_grab_focus(AtkComponent *component) {
  AtkGObjectAccessible *atk_gobj;
  GObject *obj;
  EelCanvasItem *item;
  GtkWidget *toplevel;
...
static void eel_canvas_item_accessible_remove_focus_handler(
    AtkComponent *component, guint handler_id) {
  g_signal_handler_disconnect(component, handler_id);
}

static void eel_canvas_item_accessible_component_interface_init(
...
static gboolean eel_canvas_item_accessible_is_item_on_screen(
    EelCanvasItem *item) {
  GdkRectangle rect;
...
static void eel_canvas_item_accessible_initialize(AtkObject *obj,
                                                  gpointer data) {
...
static AtkStateSet *eel_canvas_item_accessible_ref_state_set(
  ...
  state_set =
      ATK_OBJECT_CLASS(accessible_item_parent_class)->ref_state_set(accessible);
...
static void eel_canvas_item_accessible_class_init(
    EelCanvasItemAccessibleClass *klass) {
  AtkObjectClass *atk_class = ATK_OBJECT_CLASS(klass);
...
static void eel_canvas_item_accessible_init(
    EelCanvasItemAccessible *accessible) {}

static AtkObject *eel_canvas_item_accessible_create(GObject *for_object) {
  GType type;
  AtkObject *accessible;
  EelCanvasItem *item;
...
static GType eel_canvas_item_accessible_factory_get_accessible_type(void) {
...
G_DEFINE_TYPE(EelCanvasItemAccessibleFactory,
              eel_canvas_item_accessible_factory, ATK_TYPE_OBJECT_FACTORY)
...
static void eel_canvas_item_accessible_factory_init(
    EelCanvasItemAccessibleFactory *accessible) {}
...
static void eel_canvas_item_class_init(EelCanvasItemClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object("parent", NULL, NULL, EEL_TYPE_CANVAS_ITEM,
                          G_PARAM_READWRITE));
  ...
  item_signals[ITEM_EVENT] = g_signal_new(
      "event", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(EelCanvasItemClass, event), boolean_handled_accumulator,
      NULL, eel_marshal_BOOLEAN__BOXED, G_TYPE_BOOLEAN, 1,
      GDK_TYPE_EVENT | G_SIGNAL_TYPE_STATIC_SCOPE);

  item_signals[ITEM_DESTROY] = g_signal_new(
      "destroy", G_TYPE_FROM_CLASS(klass),
      G_SIGNAL_RUN_CLEANUP | G_SIGNAL_NO_RECURSE | G_SIGNAL_NO_HOOKS,
      G_STRUCT_OFFSET(EelCanvasItemClass, destroy), NULL, NULL,
