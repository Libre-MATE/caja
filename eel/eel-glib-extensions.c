...
gboolean eel_g_list_equal(GList *list_a, GList *list_b) {
  ...
  for (p = list_a, q = list_b; p != NULL && q != NULL;
       p = p->next, q = q->next) {
    if (p->data != q->data) {
      return FALSE;
    }
 ...
 **/
gboolean eel_g_str_list_equal(GList *list_a, GList *list_b) {
  GList *p, *q;
...
gboolean eel_g_lists_sort_and_check_for_intersection(GList **list_1,
                                                     GList **list_2)
...
GList *eel_g_list_partition(GList *list, EelPredicateFunction predicate,
                            gpointer user_data, GList **failed) {
...
static void print_key_string(gpointer key, gpointer value,
...
static void free_hash_tables_at_exit(void) {
  GList *p;
  guint size;
  ...
  for (p = hash_tables_to_free_at_exit; p != NULL; p = p->next) {
    ...
    if (size != 0) {
      if (hash_table_to_free->keys_known_to_be_strings) {
        ...
        g_hash_table_foreach(hash_table_to_free->hash_table, print_key_string,
                             NULL);
      }
      g_warning(
          "\"%s\" hash table still has %u element%s at quit time%s",
          hash_table_to_free->display_name, size, size == 1 ? "" : "s",
          hash_table_to_free->keys_known_to_be_strings ? " (keys above)" : "");
    }
...
GHashTable *eel_g_hash_table_new_free_at_exit(GHashFunc hash_func,
  ...
  hash_table_to_free->keys_known_to_be_strings = hash_func == g_str_hash;

  hash_tables_to_free_at_exit =
      g_list_prepend(hash_tables_to_free_at_exit, hash_table_to_free);
...
static void flatten_hash_table_element(gpointer key, gpointer value,
                                       gpointer callback_data) {
  FlattenedHashTable *flattened_table;

  flattened_table = callback_data;
  flattened_table->keys = g_list_prepend(flattened_table->keys, key);
  flattened_table->values = g_list_prepend(flattened_table->values, value);
}
...
void eel_g_hash_table_safe_for_each(GHashTable *hash_table, GHFunc callback,
  ...
  g_hash_table_foreach(hash_table, flatten_hash_table_element, &flattened);

  for (p = flattened.keys, q = flattened.values; p != NULL;
       p = p->next, q = q->next) {
...
void eel_add_weak_pointer(gpointer pointer_location) {
  ...
  g_object_add_weak_pointer(G_OBJECT(*object_location), object_location);
}
...
void eel_remove_weak_pointer(gpointer pointer_location) {
  ...
  g_object_remove_weak_pointer(G_OBJECT(*object_location), object_location);

  *object_location = NULL;
}

static void update_auto_boolean(GSettings *settings, const gchar *key,
                                gpointer user_data) {
...
void eel_g_settings_add_auto_boolean(GSettings *settings, const char *key,
  ...
  *storage = g_settings_get_boolean(settings, key);
  ...
  g_signal_connect(settings, signal, G_CALLBACK(update_auto_boolean), storage);

  g_free(signal);
}

static void update_auto_int(GSettings *settings, const gchar *key,
                            gpointer user_data) {
  int *storage = user_data;
...
static void update_auto_enum(GSettings *settings, const gchar *key,
                             gpointer user_data) {
  int *storage = user_data;
...
void eel_g_settings_add_auto_enum(GSettings *settings, const char *key,
  ...
  *storage = g_settings_get_enum(settings, key);
  signal = g_strconcat("changed::", key, NULL);
  g_signal_connect(settings, signal, G_CALLBACK(update_auto_enum), storage);

  g_free(signal);
}

static void update_auto_strv_as_quarks(GSettings *settings, const gchar *key,
                                       gpointer user_data) {
...
static void update_auto_strv(GSettings *settings, const gchar *key,
                             gpointer user_data) {
...
void eel_g_settings_add_auto_strv(GSettings *settings, const char *key,
  ...
  *storage = NULL;
  ...
  signal = g_strconcat("changed::", key, NULL);
  g_signal_connect(settings, signal, G_CALLBACK(update_auto_strv), storage);
...
void eel_g_settings_add_auto_strv_as_quarks(GSettings *settings,
  ...
  *storage = NULL;
  ...
  signal = g_strconcat("changed::", key, NULL);
  g_signal_connect(settings, signal, G_CALLBACK(update_auto_strv_as_quarks),
                   storage);
...
static gboolean eel_test_predicate(gpointer data, gpointer callback_data) {
...
void eel_self_check_glib_extensions(void) {
  ...
  EEL_CHECK_BOOLEAN_RESULT(eel_g_str_list_equal(compare_list_1, compare_list_2),
                           TRUE);
  EEL_CHECK_BOOLEAN_RESULT(eel_g_str_list_equal(compare_list_1, compare_list_3),
                           FALSE);
  EEL_CHECK_BOOLEAN_RESULT(eel_g_str_list_equal(compare_list_1, compare_list_4),
                           FALSE);
  EEL_CHECK_BOOLEAN_RESULT(eel_g_str_list_equal(compare_list_1, compare_list_5),
                           FALSE);
  ...
  actual_passed = eel_g_list_partition(list_to_partition, eel_test_predicate,
                                       "m", &actual_failed);

  EEL_CHECK_BOOLEAN_RESULT(eel_g_str_list_equal(expected_passed, actual_passed),
                           TRUE);
  EEL_CHECK_BOOLEAN_RESULT(eel_g_str_list_equal(expected_failed, actual_failed),
                           TRUE);
