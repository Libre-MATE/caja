...
/* Arguments */
enum { ARG_0, ARG_CHILD_UNDER_POINTER };
...
static void signal_connect_while_realized(GtkWidget *object, const char *name,
                                          GCallback callback,
...
static int ancestor_motion_notify_event(GtkWidget *widget,
                                        ...
                                        gpointer event_data);
static int ancestor_button_press_event(GtkWidget *widget, GdkEventButton *event,
                                       gpointer event_data);
...
static void eel_image_table_init(EelImageTable *image_table) {
...
static void eel_image_table_realize(GtkWidget *widget) {
  ...
  signal_connect_while_realized(windowed_ancestor, "enter_notify_event",
                                G_CALLBACK(ancestor_enter_notify_event), widget,
                                widget);

  signal_connect_while_realized(windowed_ancestor, "leave_notify_event",
                                G_CALLBACK(ancestor_leave_notify_event), widget,
                                widget);

  signal_connect_while_realized(windowed_ancestor, "motion_notify_event",
                                G_CALLBACK(ancestor_motion_notify_event),
                                widget, widget);

  signal_connect_while_realized(windowed_ancestor, "button_press_event",
                                G_CALLBACK(ancestor_button_press_event), widget,
                                widget);

  signal_connect_while_realized(windowed_ancestor, "button_release_event",
                                G_CALLBACK(ancestor_button_release_event),
                                widget, widget);
}

/* GtkContainerClass methods */
static void eel_image_table_remove(GtkContainer *container, GtkWidget *child) {
...
static void image_table_emit_signal(EelImageTable *image_table,
                                    GtkWidget *child, guint signal_index, int x,
                                    int y, int button, guint state,
                                    GdkEvent *gdk_event) {
  ...
  g_signal_emit(image_table, image_table_signals[signal_index], 0, child,
                &event);
}
...
static void eel_image_table_class_init(EelImageTableClass *image_table_class) {
  ...
  /* Signals */
  image_table_signals[CHILD_ENTER] = g_signal_new(
      "child_enter", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(EelImageTableClass, child_enter), NULL, NULL,
      eel_marshal_VOID__OBJECT_POINTER, G_TYPE_NONE, 2, GTK_TYPE_WIDGET,
      G_TYPE_POINTER);
  image_table_signals[CHILD_LEAVE] = g_signal_new(
      "child_leave", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(EelImageTableClass, child_leave), NULL, NULL,
      eel_marshal_VOID__OBJECT_POINTER, G_TYPE_NONE, 2, GTK_TYPE_WIDGET,
      G_TYPE_POINTER);
  image_table_signals[CHILD_PRESSED] = g_signal_new(
      "child_pressed", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(EelImageTableClass, child_pressed), NULL, NULL,
      eel_marshal_VOID__OBJECT_POINTER, G_TYPE_NONE, 2, GTK_TYPE_WIDGET,
      G_TYPE_POINTER);
  image_table_signals[CHILD_RELEASED] = g_signal_new(
      "child_released", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(EelImageTableClass, child_released), NULL, NULL,
      eel_marshal_VOID__OBJECT_POINTER, G_TYPE_NONE, 2, GTK_TYPE_WIDGET,
      G_TYPE_POINTER);
  image_table_signals[CHILD_CLICKED] = g_signal_new(
      "child_clicked", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(EelImageTableClass, child_clicked), NULL, NULL,
      eel_marshal_VOID__OBJECT_POINTER, G_TYPE_NONE, 2, GTK_TYPE_WIDGET,
      G_TYPE_POINTER);
}

static void image_table_handle_motion(EelImageTable *image_table, int x, int y,
                                      GdkEvent *event) {
  GtkWidget *child;
  ...
  child = eel_wrap_table_find_child_at_event_point(EEL_WRAP_TABLE(image_table),
                                                   x, y);
  ...
  if (leave_emit_child != NULL) {
    image_table_emit_signal(image_table, leave_emit_child, CHILD_LEAVE, x, y, 0,
                            0, (GdkEvent *)event);
  }

  if (enter_emit_child != NULL) {
    image_table_emit_signal(image_table, enter_emit_child, CHILD_ENTER, x, y, 0,
                            0, (GdkEvent *)event);
  }
}
...
static void while_realized_disconnecter(GObject *object,
  ...
  g_signal_handler_disconnect(info->realized_widget,
 ...
 * useful for non windowed widgets that need to monitor events in their
 *ancestored windowed widget.  The signal is automatically disconnected when
 *&widget is unrealized.  Also, the signal is automatically disconnected when
 *either &object or &widget are destroyed.
 **/
static void signal_connect_while_realized(GtkWidget *object, const char *name,
                                          GCallback callback,
                                          gpointer callback_data,
...
static int ancestor_enter_notify_event(GtkWidget *widget,
  ...
  image_table_handle_motion(EEL_IMAGE_TABLE(event_data), event->x, event->y,
                            (GdkEvent *)event);
...
static int ancestor_leave_notify_event(GtkWidget *widget,
  ...
  image_table_handle_motion(EEL_IMAGE_TABLE(event_data), x, y,
                            (GdkEvent *)event);
...
static int ancestor_motion_notify_event(GtkWidget *widget,
  ...
  image_table_handle_motion(EEL_IMAGE_TABLE(event_data), (int)event->x,
                            (int)event->y, (GdkEvent *)event);
...
static int ancestor_button_press_event(GtkWidget *widget, GdkEventButton *event,
  ...
  if (child != NULL) {
    if (child == image_table->details->child_under_pointer) {
      ...
      image_table_emit_signal(image_table, child, CHILD_PRESSED, event->x,
                              event->y, event->button, event->state,
                              (GdkEvent *)event);
    }
  }
...
static int ancestor_button_release_event(GtkWidget *widget,
  ...
  if (released_emit_child != NULL) {
    image_table_emit_signal(image_table, released_emit_child, CHILD_RELEASED,
                            event->x, event->y, event->button, event->state,
                            (GdkEvent *)event);
  }

  if (clicked_emit_child != NULL) {
    image_table_emit_signal(image_table, clicked_emit_child, CHILD_CLICKED,
                            event->x, event->y, event->button, event->state,
                            (GdkEvent *)event);
  }
...
GtkWidget *eel_image_table_new(gboolean homogeneous) {
  ...
  image_table =
      EEL_IMAGE_TABLE(gtk_widget_new(eel_image_table_get_type(), NULL));
