...
static void copy_done(GHashTable *debuting_uris, gpointer data) {
  g_print("Copy done\n");
}

static void changed_cb(CajaProgressInfo *info, gpointer data) {
  g_print("Changed: %s -- %s\n", caja_progress_info_get_status(info),
          caja_progress_info_get_details(info));
}

static void progress_changed_cb(CajaProgressInfo *info, gpointer data) {
  g_print("Progress changed: %f\n", caja_progress_info_get_progress(info));
}

static void finished_cb(CajaProgressInfo *info, gpointer data) {
  g_print("Finished\n");
  gtk_main_quit();
}
...
int main(int argc, char *argv[]) {
  ...
  caja_file_operations_copy(sources, NULL /* GArray *relative_item_points */,
                            dest, GTK_WINDOW(window), copy_done, NULL);
