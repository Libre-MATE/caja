...
static void hits_added_cb(CajaSearchEngine *engine, GSList *hits) {
  g_print("hits added\n");
  while (hits) {
    g_print(" - %s\n", (char *)hits->data);
    hits = hits->next;
...
static void hits_subtracted_cb(CajaSearchEngine *engine, GSList *hits) {
...
static void finished_cb(CajaSearchEngine *engine) {
  g_print("finished!\n");
  //	gtk_main_quit ();
}
...
int main(int argc, char *argv[]) {
  ...
  g_signal_connect(engine, "hits-subtracted", G_CALLBACK(hits_subtracted_cb),
                   NULL);
