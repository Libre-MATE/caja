...
G_DEFINE_TYPE_WITH_CODE(CajaHistorySidebar, caja_history_sidebar,
                        GTK_TYPE_SCROLLED_WINDOW,
                        G_IMPLEMENT_INTERFACE(CAJA_TYPE_SIDEBAR,
                                              caja_history_sidebar_iface_init));

G_DEFINE_TYPE_WITH_CODE(CajaHistorySidebarProvider,
                        caja_history_sidebar_provider, G_TYPE_OBJECT,
                        G_IMPLEMENT_INTERFACE(CAJA_TYPE_SIDEBAR_PROVIDER,
                                              sidebar_provider_iface_init));

static void update_history(CajaHistorySidebar *sidebar) {
  GtkListStore *store;
  ...
  for (l = history; l != NULL; l = l->next) {
    ...
    gtk_list_store_set(store, &iter, HISTORY_SIDEBAR_COLUMN_ICON, surface,
                       HISTORY_SIDEBAR_COLUMN_NAME, name,
                       HISTORY_SIDEBAR_COLUMN_BOOKMARK, bookmark, -1);
...
static void history_changed_callback(GObject *signaller,
                                     CajaHistorySidebar *sidebar) {
  update_history(sidebar);
}

static void open_selected_item(CajaHistorySidebar *sidebar, GtkTreePath *path,
  ...
  gtk_tree_model_get(model, &iter, HISTORY_SIDEBAR_COLUMN_BOOKMARK, &bookmark,
                     -1);
...
static gboolean button_press_event_callback(GtkWidget *widget,
  ...
  if (event->button == 2 && event->type == GDK_BUTTON_PRESS) {
    ...
    if (gtk_tree_view_get_path_at_pos(sidebar->tree_view, event->x, event->y,
                                      &path, NULL, NULL, NULL)) {
      open_selected_item(sidebar, path, CAJA_WINDOW_OPEN_FLAG_NEW_TAB);
...
static void update_click_policy(CajaHistorySidebar *sidebar) {
  int policy;
...
static void caja_history_sidebar_init(CajaHistorySidebar *sidebar) {
  ...
  gtk_tree_view_column_set_attributes(col, cell, "text",
                                      HISTORY_SIDEBAR_COLUMN_NAME, NULL);
  ...
  g_signal_connect_swapped(caja_preferences,
                           ...
                           G_CALLBACK(click_policy_changed_callback), sidebar);
  update_click_policy(sidebar);
}

static void caja_history_sidebar_finalize(GObject *object) {
  CajaHistorySidebar *sidebar;
...
static void caja_history_sidebar_class_init(CajaHistorySidebarClass *class) {
...
static const char *caja_history_sidebar_get_sidebar_id(CajaSidebar *sidebar) {
  return CAJA_HISTORY_SIDEBAR_ID;
}

static char *caja_history_sidebar_get_tab_label(CajaSidebar *sidebar) {
  return g_strdup(_("History"));
}

static char *caja_history_sidebar_get_tab_tooltip(CajaSidebar *sidebar) {
  return g_strdup(_("Show History"));
}

static GdkPixbuf *caja_history_sidebar_get_tab_icon(CajaSidebar *sidebar) {
  return NULL;
}

static void caja_history_sidebar_is_visible_changed(CajaSidebar *sidebar,
                                                    gboolean is_visible) {
  /* Do nothing */
}
...
static void caja_history_sidebar_set_parent_window(CajaHistorySidebar *sidebar,
...
static CajaSidebar *caja_history_sidebar_create(CajaSidebarProvider *provider,
                                                CajaWindowInfo *window) {
  CajaHistorySidebar *sidebar;
...
static void caja_history_sidebar_provider_init(
    CajaHistorySidebarProvider *sidebar) {}

static void caja_history_sidebar_provider_class_init(
    CajaHistorySidebarProviderClass *class) {}
