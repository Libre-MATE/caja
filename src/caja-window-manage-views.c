...
static void begin_location_change(CajaWindowSlot *slot, GFile *location,
                                  GFile *previous_location,
                                  GList *new_selection,
                                  CajaLocationChangeType type, guint distance,
...
static void create_content_view(CajaWindowSlot *slot, const char *view_id);
static void display_view_selection_failure(CajaWindow *window, CajaFile *file,
                                           GFile *location, GError *error);
static void load_new_location(CajaWindowSlot *slot, GFile *location,
                              GList *selection,
 ...
 */
static void set_displayed_location(CajaWindowSlot *slot, GFile *location) {
  gboolean recreate;
  ...
  if (slot->current_location_bookmark == NULL || location == NULL) {
    ...
    bookmark_location =
        caja_bookmark_get_location(slot->current_location_bookmark);
...
static void check_bookmark_location_matches(CajaBookmark *bookmark,
                                            GFile *location) {
  GFile *bookmark_location;
 ...
 */
static void check_last_bookmark_location_matches_slot(CajaWindowSlot *slot) {
  check_bookmark_location_matches(slot->last_location_bookmark, slot->location);
}

static void handle_go_back(CajaNavigationWindowSlot *navigation_slot,
                           GFile *location) {
  CajaWindowSlot *slot;
  guint i;
  GList *link;
  ...
  g_assert(g_list_length(navigation_slot->back_list) >
           slot->location_change_distance);
  check_bookmark_location_matches(
      CAJA_BOOKMARK(g_list_nth_data(navigation_slot->back_list,
                                    slot->location_change_distance)),
      location);
  ...
  navigation_slot->back_list =
      g_list_remove_link(navigation_slot->back_list, link);
  g_object_unref(link->data);
  g_list_free_1(link);
}

static void handle_go_forward(CajaNavigationWindowSlot *navigation_slot,
                              GFile *location) {
  CajaWindowSlot *slot;
  guint i;
  GList *link;
  ...
  g_assert(g_list_length(navigation_slot->forward_list) >
           slot->location_change_distance);
  check_bookmark_location_matches(
      CAJA_BOOKMARK(g_list_nth_data(navigation_slot->forward_list,
                                    slot->location_change_distance)),
      location);
  g_assert(slot->location != NULL);
   ...
   */
  navigation_slot->back_list =
      g_list_prepend(navigation_slot->back_list, slot->last_location_bookmark);
  ...
  navigation_slot->forward_list =
      g_list_remove_link(navigation_slot->forward_list, link);
  g_object_unref(link->data);
  g_list_free_1(link);
}

static void handle_go_elsewhere(CajaWindowSlot *slot, GFile *location) {
  if (CAJA_IS_NAVIGATION_WINDOW_SLOT(slot)) {
    ...
    if (slot->location != NULL) {
      ...
      if (!g_file_equal(slot->location, location)) {
         ...
         * one. */
        navigation_slot->back_list = g_list_prepend(
            navigation_slot->back_list, slot->last_location_bookmark);
        g_object_ref(navigation_slot->back_list->data);
      }
    }
  }
...
static void viewed_file_changed_callback(CajaFile *file, CajaWindowSlot *slot) {
  CajaWindow *window;
  GFile *new_location;
      ...
      if (CAJA_IS_NAVIGATION_WINDOW(window)) {
        ...
        if (go_to_file != NULL) {
          ...
          if (slot == slot->pane->active_slot) {
             ...
             */
            caja_path_bar_clear_buttons(CAJA_PATH_BAR(
                CAJA_NAVIGATION_WINDOW_PANE(slot->pane)->path_bar));
          }
...
static void update_history(CajaWindowSlot *slot, CajaLocationChangeType type,
                           GFile *new_location) {
  switch (type) {
...
static void cancel_viewed_file_changed_callback(CajaWindowSlot *slot) {
  CajaFile *file;

  file = slot->viewed_file;
  if (file != NULL) {
    g_signal_handlers_disconnect_by_func(
        G_OBJECT(file), G_CALLBACK(viewed_file_changed_callback), slot);
    caja_file_monitor_remove(file, &slot->viewed_file);
  }
}

static void new_window_show_callback(GtkWidget *widget, gpointer user_data) {
  CajaWindow *window;
  ...
  g_signal_handlers_disconnect_by_func(
      widget, G_CALLBACK(new_window_show_callback), user_data);
}

void caja_window_slot_open_location_full(CajaWindowSlot *slot, GFile *location,
  ...
  caja_debug_log(FALSE, CAJA_DEBUG_LOG_DOMAIN_USER,
                 "window %p open location: old=\"%s\", new=\"%s\"", window,
                 old_uri, new_uri);
  g_free(old_uri);
  g_free(new_uri);
  ...
  target_same =
      is_desktop && !caja_desktop_window_loaded(CAJA_DESKTOP_WINDOW(window));
  ...
  switch (mode) {
    case CAJA_WINDOW_OPEN_ACCORDING_TO_MODE:
      if (g_settings_get_boolean(caja_preferences,
        ...
        /* always use browser: if we're on the desktop the target is a new
         * navigation window, otherwise it's the same window.
         */
        if (is_desktop) {
          new_uri = g_file_get_uri(location);
  ...
  if (target_same) {
    ...
    target_window = caja_application_get_spatial_window(
        window->application, window, NULL, location,
        gtk_window_get_screen(GTK_WINDOW(window)), &existing);
  }
  ...
  if ((flags & CAJA_WINDOW_OPEN_FLAG_NEW_TAB) != 0 &&
    ...
    target_slot =
        caja_window_open_slot(window->details->active_pane, slot_flags);
  }
    ...
    if (CAJA_IS_SPATIAL_WINDOW(window) && !CAJA_IS_DESKTOP_WINDOW(window)) {
      if (gtk_widget_get_visible(GTK_WIDGET(target_window))) {
        ...
        g_signal_connect_object(target_window, "show",
                                G_CALLBACK(new_window_show_callback), window,
                                G_CONNECT_AFTER);
      }
    }
  }
  ...
  if (!(is_desktop && target_same) &&
      (target_window == window && target_slot == slot && old_location &&
...
void caja_window_slot_open_location(CajaWindowSlot *slot, GFile *location,
...
void caja_window_slot_go_home(CajaWindowSlot *slot, gboolean new_tab) {
  GFile *home;
  CajaWindowOpenFlags flags;
  ...
  caja_window_slot_open_location_full(
      slot, home, CAJA_WINDOW_OPEN_ACCORDING_TO_MODE, flags, NULL, NULL, NULL);
  g_object_unref(home);
}

static char *caja_window_slot_get_view_error_label(CajaWindowSlot *slot) {
...
static void report_current_content_view_failure_to_user(CajaWindowSlot *slot) {
  CajaWindow *window;
  char *message;
  ...
  eel_show_error_dialog(
      message, _("You can choose another view or go to a different location."),
      GTK_WINDOW(window));
  g_free(message);
}

static void report_nascent_content_view_failure_to_user(CajaWindowSlot *slot,
                                                        CajaView *view) {
  CajaWindow *window;
  char *message;
  ...
  /* TODO? why are we using the current view's error label here, instead of the
   * next view's? This behavior has already been present in pre-slot days.
   */
...
static gboolean report_callback(CajaWindowSlot *slot, GError *error) {
  if (slot->open_callback != NULL) {
    slot->open_callback(slot->pane->window, error,
                        slot->open_callback_user_data);
...
static gpointer copy_object(gconstpointer obj, gpointer user_data) {
  (void)user_data;
...
static void begin_location_change(CajaWindowSlot *slot, GFile *location,
  ...
  g_assert(type == CAJA_LOCATION_CHANGE_BACK ||
           type == CAJA_LOCATION_CHANGE_FORWARD || distance == 0);
  ...
  if (slot->current_location_bookmark != NULL && slot->content_view != NULL) {
    char *current_pos;
  ...
  caja_file_call_when_ready(
      ...
      CAJA_FILE_ATTRIBUTE_INFO | CAJA_FILE_ATTRIBUTE_MOUNT,
      got_file_info_for_view_selection_callback, slot);

  g_object_unref(window);
}

static void setup_new_spatial_window(CajaWindowSlot *slot, CajaFile *file) {
  CajaWindow *window;
  char *scroll_string;
  ...
  if (CAJA_IS_SPATIAL_WINDOW(window) && !CAJA_IS_DESKTOP_WINDOW(window)) {
    ...
    show_hidden_file_setting = caja_file_get_metadata(
        file, CAJA_METADATA_KEY_WINDOW_SHOW_HIDDEN_FILES, NULL);
    ...
    geometry_string =
        caja_file_get_metadata(file, CAJA_METADATA_KEY_WINDOW_GEOMETRY, NULL);
    if (geometry_string != NULL) {
      eel_gtk_window_set_initial_geometry_from_string(
          GTK_WINDOW(window), geometry_string, CAJA_SPATIAL_WINDOW_MIN_WIDTH,
          CAJA_SPATIAL_WINDOW_MIN_HEIGHT, FALSE);
    }
    ...
    if (slot->pending_selection == NULL) {
      ...
      scroll_string = caja_file_get_metadata(
          file, CAJA_METADATA_KEY_WINDOW_SCROLL_POSITION, NULL);
    } else {
...
static void got_file_info_for_view_selection_callback(CajaFile *file,
  ...
  if (error && error->domain == G_IO_ERROR &&
      error->code == G_IO_ERROR_NOT_MOUNTED && !slot->tried_mount) {
  ...
  if (error == NULL || (error->domain == G_IO_ERROR &&
    ...
    if (slot->location_change_type != CAJA_LOCATION_CHANGE_FALLBACK) {
      ...
      view_id =
          caja_file_get_metadata(file, CAJA_METADATA_KEY_DEFAULT_VIEW, NULL);
      if (view_id != NULL &&
          !caja_view_factory_view_supports_uri(
              view_id, location, caja_file_get_file_type(file), mimetype)) {
        g_free(view_id);
        view_id = NULL;
      }
    }
    ...
    if (view_id == NULL) {
      ...
      if (eel_uri_is_search(uri)) {
        ...
        view_id =
            caja_global_preferences_get_default_folder_viewer_preference_as_iid();
      }
  ...
  if (view_id != NULL) {
    ...
    if (!gtk_widget_get_visible(GTK_WIDGET(window))) {
      ...
      if (g_list_length(gtk_application_get_windows(GTK_APPLICATION(app))) ==
          1) {
      ...
      if (slot->location == NULL) {
        ...
        g_signal_connect_object(viewed_file, "changed",
                                G_CALLBACK(viewed_file_changed_callback), slot,
                                0);
...
static void create_content_view(CajaWindowSlot *slot, const char *view_id) {
  ...
  if (slot->content_view != NULL &&
      g_strcmp0(caja_view_get_view_id(slot->content_view), view_id) == 0) {
    ...
    view = caja_view_factory_create(view_id, CAJA_WINDOW_SLOT_INFO(slot));
  ...
  if (slot->pending_location != NULL) {
    load_new_location(slot, slot->pending_location, slot->pending_selection,
                      FALSE, TRUE);
    ...
    load_new_location(slot, slot->location, selection, FALSE, TRUE);
...
static void caja_window_emit_location_change(CajaWindow *window,
                                             GFile *location) {
  char *uri;
...
static void slot_add_extension_extra_widgets(CajaWindowSlot *slot) {
  GList *providers, *l;
  char *uri;
  ...
  for (l = providers; l != NULL; l = l->next) {
    ...
    widget = caja_location_widget_provider_get_widget(
        provider, uri, GTK_WIDGET(slot->pane->window));
    if (widget != NULL) {
...
static void caja_window_slot_show_x_content_bar(CajaWindowSlot *slot,
                                                GMount *mount,
...
static void caja_window_slot_show_trash_bar(CajaWindowSlot *slot,
                                            CajaWindow *window) {
  GtkWidget *bar;
...
static void found_mount_cb(GObject *source_object, GAsyncResult *res,
  ...
  if (mount != NULL) {
    ...
    caja_autorun_get_x_content_types_for_mount_async(
        mount, (CajaAutorunGetContent)found_content_type_cb, data->cancellable,
        data);
    return;
  }
...
static void update_for_new_location(CajaWindowSlot *slot) {
  ...
  location_really_changed =
      slot->location == NULL || !g_file_equal(slot->location, new_location);
  ...
  if (location_really_changed) {
    ...
    g_file_find_enclosing_mount_async(slot->location, G_PRIORITY_DEFAULT,
  ...
  if (slot == slot->pane->active_slot) {
    ...
    if (CAJA_IS_NAVIGATION_WINDOW(window) &&
      ...
      caja_navigation_window_load_extension_toolbar_items(
          CAJA_NAVIGATION_WINDOW(window));
    }
  }
}
...
void caja_window_report_load_complete(CajaWindow *window, CajaView *view) {
  ...
  if (view == slot->content_view) {
    if (slot->pending_scroll_to != NULL) {
      caja_view_scroll_to_file(slot->content_view, slot->pending_scroll_to);
    }
    end_location_change(slot);
  }
}
...
static void free_location_change(CajaWindowSlot *slot) {
  ...
  if (slot->determine_view_file != NULL) {
    caja_file_cancel_call_when_ready(slot->determine_view_file,
...
static void cancel_location_change(CajaWindowSlot *slot) {
  if (slot->pending_location != NULL && slot->location != NULL &&
     ...
     */
    selection = caja_view_get_selection(slot->content_view);
    load_new_location(slot, slot->location, selection, TRUE, FALSE);
    g_list_free_full(selection, g_object_unref);
  }
...
void caja_window_report_view_failed(CajaWindow *window, CajaView *view) {
  ...
  g_warning(
      "A view failed. The UI will handle this with a dialog but this should be "
      "debugged.");
...
static void display_view_selection_failure(CajaWindow *window, CajaFile *file,
   ...
   */
  uri_for_display =
      eel_str_middle_truncate(full_uri_for_display, MAX_URI_IN_DIALOG_LENGTH);
  ...
  if (error == NULL) {
    if (caja_file_is_directory(file)) {
      error_message =
          g_strdup_printf(_("Could not display \"%s\"."), uri_for_display);
      detail_message = g_strdup(
          _("Caja has no installed viewer capable of displaying the folder."));
    } else {
      error_message =
          g_strdup_printf(_("Could not display \"%s\"."), uri_for_display);
      detail_message = g_strdup(_("The location is not a folder."));
    }
  } else if (error->domain == G_IO_ERROR) {
    switch (error->code) {
      case G_IO_ERROR_NOT_FOUND:
        error_message =
            g_strdup_printf(_("Could not find \"%s\"."), uri_for_display);
        detail_message =
            g_strdup(_("Please check the spelling and try again."));
        break;
      case G_IO_ERROR_NOT_SUPPORTED:
        ...
        if (scheme_string != NULL) {
          detail_message = g_strdup_printf(
              _("Caja cannot handle \"%s\" locations."), scheme_string);
        } else {
          detail_message =
              g_strdup(_("Caja cannot handle this kind of location."));
        }
      ...
      case G_IO_ERROR_HOST_NOT_FOUND:
        ...
        error_message = g_strdup_printf(
            _("Could not display \"%s\", because the host could not be found."),
            uri_for_display);
        detail_message =
            g_strdup(_("Check that the spelling is correct and that your proxy "
...
void caja_window_slot_set_content_view(CajaWindowSlot *slot, const char *id) {
  CajaWindow *window;
  CajaFile *file;
  char *uri;
  ...
  uri = caja_window_slot_get_location_uri(slot);
  caja_debug_log(FALSE, CAJA_DEBUG_LOG_DOMAIN_USER,
                 "change view of window %p: \"%s\" to \"%s\"", window, uri, id);
  g_free(uri);
  ...
  file = caja_file_get(slot->location);
  caja_file_set_metadata(file, CAJA_METADATA_KEY_DEFAULT_VIEW, NULL, id);
  caja_file_unref(file);
  ...
  if (caja_view_get_selection_count(slot->content_view) == 0) {
    ...
    slot->pending_scroll_to =
        caja_view_get_first_visible_file(slot->content_view);
  }
...
void caja_navigation_window_back_or_forward(CajaNavigationWindow *window,
  ...
  if (new_tab) {
    ...
    begin_location_change(
        slot, location, old_location, NULL,
        back ? CAJA_LOCATION_CHANGE_BACK : CAJA_LOCATION_CHANGE_FORWARD,
        distance, scroll_pos, NULL, NULL);
...
void caja_window_slot_reload(CajaWindowSlot *slot) {
  ...
  begin_location_change(slot, location, location, selection,
                        CAJA_LOCATION_CHANGE_RELOAD, 0, current_pos, NULL,
                        NULL);
  g_free(current_pos);
  g_object_unref(location);
  g_list_free_full(selection, g_object_unref);
