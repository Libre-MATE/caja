...
enum { CONTENTS_CHANGED, LAST_SIGNAL };
...
static CajaBookmark *new_bookmark_from_uri(const char *uri, const char *label) {
  ...
  if (uri) {
    ...
    if (caja_file_check_if_ready(file, CAJA_FILE_ATTRIBUTES_FOR_ICON)) {
...
static GFile *caja_bookmark_list_get_legacy_file(void) {
  char *filename;
  GFile *file;
...
static GFile *caja_bookmark_list_get_file(void) {
  char *filename;
  GFile *file;
...
static void bookmark_in_list_changed_callback(CajaBookmark *bookmark,
...
static void stop_monitoring_bookmark(CajaBookmarkList *bookmarks,
                                     CajaBookmark *bookmark) {
  g_signal_handlers_disconnect_by_func(
      bookmark, bookmark_in_list_changed_callback, bookmarks);
}

static void stop_monitoring_one(gpointer data, gpointer user_data) {
  g_assert(CAJA_IS_BOOKMARK(data));
  g_assert(CAJA_IS_BOOKMARK_LIST(user_data));

  stop_monitoring_bookmark(CAJA_BOOKMARK_LIST(user_data), CAJA_BOOKMARK(data));
}

static void clear(CajaBookmarkList *bookmarks) {
  g_list_foreach(bookmarks->list, stop_monitoring_one, bookmarks);
  g_list_free_full(bookmarks->list, g_object_unref);
  bookmarks->list = NULL;
}

static void do_finalize(GObject *object) {
...
static GObject *do_constructor(GType type, guint n_construct_params,
  ...
  retval = G_OBJECT_CLASS(caja_bookmark_list_parent_class)
...
static void caja_bookmark_list_class_init(CajaBookmarkListClass *class) {
  GObjectClass *object_class = G_OBJECT_CLASS(class);

  object_class->finalize = do_finalize;
  object_class->constructor = do_constructor;

  signals[CONTENTS_CHANGED] = g_signal_new(
      "contents_changed", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaBookmarkListClass, contents_changed), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
}

static void bookmark_monitor_changed_cb(GFileMonitor *monitor, GFile *child,
                                        GFile *other_file,
...
static void caja_bookmark_list_init(CajaBookmarkList *bookmarks) {
  GFile *file;
...
static void insert_bookmark_internal(CajaBookmarkList *bookmarks,
                                     CajaBookmark *bookmark, int index) {
...
void caja_bookmark_list_append(CajaBookmarkList *bookmarks,
  ...
  insert_bookmark_internal(bookmarks, caja_bookmark_copy(bookmark), -1);
...
gboolean caja_bookmark_list_contains(CajaBookmarkList *bookmarks,
  ...
  return g_list_find_custom(bookmarks->list, (gpointer)bookmark,
                            caja_bookmark_compare_with) != NULL;
}
 ...
 **/
void caja_bookmark_list_move_item(CajaBookmarkList *bookmarks, guint index,
                                  guint destination) {
  GList *bookmark_item;
  ...
  bookmarks->list = g_list_remove_link(bookmarks->list, bookmark_item);

  if (index < destination) {
    bookmarks->list =
        g_list_insert(bookmarks->list, bookmark_item->data, destination - 1);
  } else {
    bookmarks->list =
        g_list_insert(bookmarks->list, bookmark_item->data, destination);
  }
 ...
 **/
const char *caja_bookmark_list_get_window_geometry(
    CajaBookmarkList *bookmarks) {
...
void caja_bookmark_list_insert_item(CajaBookmarkList *bookmarks,
                                    CajaBookmark *new_bookmark, guint index) {
  g_return_if_fail(CAJA_IS_BOOKMARK_LIST(bookmarks));
  g_return_if_fail(index <= g_list_length(bookmarks->list));

  insert_bookmark_internal(bookmarks, caja_bookmark_copy(new_bookmark), index);
...
static void load_file_finish(CajaBookmarkList *bookmarks, GObject *source,
                             GAsyncResult *res) {
  GError *error = NULL;
  gchar *contents = NULL;

  g_file_load_contents_finish(G_FILE(source), res, &contents, NULL, NULL,
                              &error);
  ...
  if (error == NULL) {
    ...
    for (i = 0; lines[i]; i++) {
      ...
      if (lines[i][0] != '\0' && lines[i][0] != ' ') {
        /* gtk 2.7/2.8 might have labels appended to bookmarks which are
        ...
        insert_bookmark_internal(bookmarks,
                                 new_bookmark_from_uri(lines[i], label), -1);
...
static void load_file_async(CajaBookmarkList *self,
...
static void save_file_finish(CajaBookmarkList *bookmarks, GObject *source,
                             GAsyncResult *res) {
  GError *error = NULL;
  GFile *file;
...
static void save_file_async(CajaBookmarkList *bookmarks,
                            GAsyncReadyCallback callback) {
  GFile *file;
  GList *l;
  ...
  for (l = bookmarks->list; l; l = l->next) {
    ...
    /* make sure we save label if it has one for compatibility with GTK 2.7
     * and 2.8 */
  ...
  g_file_replace_contents_async(file, bookmark_string->str,
                                bookmark_string->len, NULL, FALSE, 0, NULL,
                                callback, bookmarks);

  g_object_unref(file);
}

static void process_next_op(CajaBookmarkList *bookmarks);

static void op_processed_cb(GObject *source, GAsyncResult *res,
                            gpointer user_data) {
