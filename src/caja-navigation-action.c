...
static void activate_back_or_forward_menu_item(GtkMenuItem *menu_item,
  ...
  caja_navigation_window_back_or_forward(window, back, index,
...
static void fill_menu(CajaNavigationWindow *window, GtkWidget *menu,
                      gboolean back) {
...
static void show_menu_callback(GtkMenuToolButton *button,
...
static void connect_proxy(GtkAction *action, GtkWidget *proxy) {
  if (GTK_IS_MENU_TOOL_BUTTON(proxy)) {
    ...
    g_signal_connect(proxy, "show-menu", G_CALLBACK(show_menu_callback),
                     action);

    /* Make sure that middle click works. Note that there is some code
     * duplication between here and caja-window-menus.c */
    child = eel_gtk_menu_tool_button_get_button(button);
    g_signal_connect(child, "button-press-event",
                     G_CALLBACK(proxy_button_press_event_cb), NULL);
...
static void disconnect_proxy(GtkAction *action, GtkWidget *proxy) {
  if (GTK_IS_MENU_TOOL_BUTTON(proxy)) {
    ...
    g_signal_handlers_disconnect_by_func(proxy, G_CALLBACK(show_menu_callback),
                                         action);

    child = eel_gtk_menu_tool_button_get_button(GTK_MENU_TOOL_BUTTON(proxy));
    g_signal_handlers_disconnect_by_func(
        child, G_CALLBACK(proxy_button_press_event_cb), NULL);
    g_signal_handlers_disconnect_by_func(
...
static void caja_navigation_action_set_property(GObject *object, guint prop_id,
                                                const GValue *value,
                                                GParamSpec *pspec) {
...
static void caja_navigation_action_get_property(GObject *object, guint prop_id,
                                                GValue *value,
                                                GParamSpec *pspec) {
  CajaNavigationAction *nav;
...
static void caja_navigation_action_class_init(
  ...
  g_object_class_install_property(
      object_class, PROP_ARROW_TOOLTIP,
      g_param_spec_string("arrow-tooltip", "Arrow Tooltip", "Arrow Tooltip",
                          NULL, G_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_DIRECTION,
      g_param_spec_int("direction", "Direction", "Direction", 0, G_MAXINT, 0,
                       G_PARAM_READWRITE));
  g_object_class_install_property(
      object_class, PROP_WINDOW,
      g_param_spec_object("window", "Window", "The navigation window",
