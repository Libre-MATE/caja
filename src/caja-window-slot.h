...
#define CAJA_WINDOW_SLOT_CLASS(k) \
  (G_TYPE_CHECK_CLASS_CAST((k), CAJA_TYPE_WINDOW_SLOT, CajaWindowSlotClass))
#define CAJA_WINDOW_SLOT(obj) \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), CAJA_TYPE_WINDOW_SLOT, CajaWindowSlot))
#define CAJA_IS_WINDOW_SLOT(obj) \
  (G_TYPE_CHECK_INSTANCE_TYPE((obj), CAJA_TYPE_WINDOW_SLOT))
#define CAJA_IS_WINDOW_SLOT_CLASS(k) \
  (G_TYPE_CHECK_CLASS_TYPE((k), CAJA_TYPE_WINDOW_SLOT))
#define CAJA_WINDOW_SLOT_GET_CLASS(o) \
  (G_TYPE_INSTANCE_GET_CLASS((o), CAJA_TYPE_WINDOW_SLOT, CajaWindowSlotClass))
...
#define caja_window_slot_go_to(slot, location, new_tab)   \
  caja_window_slot_open_location_full(                    \
      ...
      (new_tab ? CAJA_WINDOW_OPEN_FLAG_NEW_TAB : 0), NULL, NULL, NULL)

#define caja_window_slot_go_to_full(slot, location, new_tab, callback, \
                                    user_data)                         \
  caja_window_slot_open_location_full(                                 \
      slot, location, CAJA_WINDOW_OPEN_ACCORDING_TO_MODE,              \
      (new_tab ? CAJA_WINDOW_OPEN_FLAG_NEW_TAB : 0), NULL, callback,   \
      user_data)

#define caja_window_slot_go_to_with_selection(slot, location, new_selection)   \
  caja_window_slot_open_location_with_selection(slot, location, new_selection, \
                                                FALSE)

void caja_window_slot_go_home(CajaWindowSlot *slot, gboolean new_tab);
void caja_window_slot_go_up(CajaWindowSlot *slot, gboolean close_behind);
...
void caja_window_slot_set_allow_stop(CajaWindowSlot *slot, gboolean allow_stop);
