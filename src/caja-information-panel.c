...
static void caja_information_panel_drag_data_received(
    GtkWidget *widget, GdkDragContext *context, int x, int y,
    GtkSelectionData *selection_data, guint info, guint time);
static void caja_information_panel_read_defaults(
...
static void caja_information_panel_theme_changed(GSettings *settings,
                                                 const gchar *key,
                                                 gpointer user_data);
static void caja_information_panel_update_appearance(
    CajaInformationPanel *information_panel);
static void caja_information_panel_update_buttons(
    CajaInformationPanel *information_panel);
static void background_metadata_changed_callback(
    CajaInformationPanel *information_panel);
...
typedef enum { NO_PART, BACKGROUND_PART, ICON_PART } InformationPanelPart;
...
G_DEFINE_TYPE_WITH_CODE(
    CajaInformationPanel, caja_information_panel, EEL_TYPE_BACKGROUND_BOX,
...
static const char *caja_information_panel_get_sidebar_id(CajaSidebar *sidebar) {
  return CAJA_INFORMATION_PANEL_ID;
}

static char *caja_information_panel_get_tab_label(CajaSidebar *sidebar) {
  return g_strdup(_("Information"));
}

static char *caja_information_panel_get_tab_tooltip(CajaSidebar *sidebar) {
  return g_strdup(_("Show Information"));
}

static GdkPixbuf *caja_information_panel_get_tab_icon(CajaSidebar *sidebar) {
  return NULL;
}

static void caja_information_panel_is_visible_changed(CajaSidebar *sidebar,
                                                      gboolean is_visible) {
  /* Do nothing */
}
...
static void caja_information_panel_class_init(
    CajaInformationPanelClass *klass) {
  ...
  signals[LOCATION_CHANGED] = g_signal_new(
      "location_changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaInformationPanelClass, location_changed), NULL, NULL,
      g_cclosure_marshal_VOID__STRING, G_TYPE_NONE, 1, G_TYPE_STRING);
}

/* utility routine to allocate the box the holds the command buttons */
static void make_button_box(CajaInformationPanel *information_panel) {
  information_panel->details->button_box_centerer =
      gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0);

  gtk_box_pack_start(GTK_BOX(information_panel->details->container),
                     information_panel->details->button_box_centerer, TRUE,
                     TRUE, 0);

  information_panel->details->button_box = caja_keep_last_vertical_box_new(4);
  gtk_container_set_border_width(
      GTK_CONTAINER(information_panel->details->button_box), 8);
  ...
  gtk_box_pack_start(GTK_BOX(information_panel->details->button_box_centerer),
                     information_panel->details->button_box, TRUE, TRUE, 0);
  information_panel->details->has_buttons = FALSE;
}

/* initialize the instance's fields, create the necessary subviews, etc. */

static void caja_information_panel_init(
    CajaInformationPanel *information_panel) {
  information_panel->details =
      caja_information_panel_get_instance_private(information_panel);
  ...
  gtk_container_set_border_width(
      GTK_CONTAINER(information_panel->details->container), 0);
  ...
  information_panel->details->title =
      CAJA_SIDEBAR_TITLE(caja_sidebar_title_new());
  gtk_widget_show(GTK_WIDGET(information_panel->details->title));
  gtk_box_pack_start(GTK_BOX(information_panel->details->container),
                     GTK_WIDGET(information_panel->details->title), FALSE,
                     FALSE, 8);
  ...
  g_signal_connect(
      caja_preferences, "changed::" CAJA_PREFERENCES_SIDE_PANE_BACKGROUND_SET,
      G_CALLBACK(caja_information_panel_theme_changed), information_panel);
  g_signal_connect(
      caja_preferences, "changed::" CAJA_PREFERENCES_SIDE_PANE_BACKGROUND_COLOR,
      G_CALLBACK(caja_information_panel_theme_changed), information_panel);
  g_signal_connect(
      caja_preferences, "changed::" CAJA_PREFERENCES_SIDE_PANE_BACKGROUND_URI,
      G_CALLBACK(caja_information_panel_theme_changed), information_panel);
...
static void caja_information_panel_finalize(GObject *object) {
  ...
  if (information_panel->details->file != NULL) {
    caja_file_monitor_remove(information_panel->details->file,
                             information_panel);
...
static void reset_background_callback(GtkWidget *menu_item,
                                      GtkWidget *information_panel) {
...
static gboolean information_panel_has_background(
    CajaInformationPanel *information_panel) {
  EelBackground *background;
...
static GtkWidget *caja_information_panel_create_context_menu(
    CajaInformationPanel *information_panel) {
  ...
  g_signal_connect_object(menu_item, "activate",
                          G_CALLBACK(reset_background_callback),
                          information_panel, 0);
...
static void caja_information_panel_read_defaults(
  ...
  background_set = g_settings_get_boolean(
      caja_preferences, CAJA_PREFERENCES_SIDE_PANE_BACKGROUND_SET);

  background_color = NULL;
  background_image = NULL;
  if (background_set) {
    background_color = g_settings_get_string(
        caja_preferences, CAJA_PREFERENCES_SIDE_PANE_BACKGROUND_COLOR);
    background_image = g_settings_get_string(
        caja_preferences, CAJA_PREFERENCES_SIDE_PANE_BACKGROUND_URI);
  }
  ...
  if (background_color && strlen(background_color)) {
    information_panel->details->default_background_color =
        g_strdup(background_color);
  }

  /* set up the default background image */

  if (background_image && strlen(background_image)) {
    information_panel->details->default_background_image =
        g_strdup(background_image);
  }
...
static InformationPanelPart hit_test(CajaInformationPanel *information_panel,
                                     int x, int y) {
  GtkAllocation *allocation;
  gboolean bg_hit;

  if (caja_sidebar_title_hit_test_icon(information_panel->details->title, x,
                                       y)) {
    return ICON_PART;
  }
  ...
  bg_hit = allocation != NULL && x >= allocation->x && y >= allocation->y &&
...
static void receive_dropped_uri_list(CajaInformationPanel *information_panel,
  ...
  switch (hit_test(information_panel, x, y)) {
    ...
    case BACKGROUND_PART:
      /* FIXME bugzilla.gnome.org 42507: Does this work for all images, or only
       * background images? Other views handle background images differently
       * from other URIs.
       */
      ...
      if (uri_is_local_image(uris[0])) {
        if (information_panel->details->file != NULL) {
          caja_file_set_metadata(information_panel->details->file,
                                 CAJA_METADATA_KEY_CUSTOM_ICON, NULL, uris[0]);
          caja_file_set_metadata(information_panel->details->file,
                                 CAJA_METADATA_KEY_ICON_SCALE, NULL, NULL);
        }
      } else {
        GFile *f;
        ...
        if (!g_file_is_native(f)) {
          ...
          eel_show_error_dialog(_("The file that you dropped is not an image."),
...
static void receive_dropped_color(CajaInformationPanel *information_panel,
                                  GdkDragAction action, int x, int y,
                                  GtkSelectionData *selection_data) {
  guint16 *channels;
  char color_spec[8];
  ...
  g_snprintf(color_spec, sizeof(color_spec), "#%02X%02X%02X", channels[0] >> 8,
             channels[1] >> 8, channels[2] >> 8);
  ...
  switch (hit_test(information_panel, x, y)) {
    ...
    case BACKGROUND_PART:
      ...
      if (action > 0) {
        ...
        background = eel_get_widget_background(GTK_WIDGET(information_panel));
        eel_background_set_dropped_color(background,
                                         GTK_WIDGET(information_panel), action,
                                         x, y, selection_data);
      }
...
static void receive_dropped_keyword(CajaInformationPanel *information_panel,
                                    int x, int y,
                                    GtkSelectionData *selection_data) {
  caja_drag_file_receive_dropped_keyword(
  ...
  switch (info) {
    ...
    case TARGET_URI_LIST:
      receive_dropped_uri_list(information_panel,
                               gdk_drag_context_get_selected_action(context), x,
                               y, selection_data);
      break;
    case TARGET_COLOR:
...
static gboolean caja_information_panel_press_event(GtkWidget *widget,
  ...
  if (event->button == CONTEXTUAL_MENU_BUTTON) {
    ...
    eel_pop_up_context_menu(GTK_MENU(menu), event);
  }
  return TRUE;
}

static GtkWindow *caja_information_panel_get_window(
    CajaInformationPanel *information_panel) {
  GtkWidget *result;

  result =
      gtk_widget_get_ancestor(GTK_WIDGET(information_panel), GTK_TYPE_WINDOW);

  return result == NULL ? NULL : GTK_WINDOW(result);
}

static void command_button_callback(GtkWidget *button, GAppInfo *application) {
  CajaInformationPanel *information_panel;
  GList files;

  information_panel =
      CAJA_INFORMATION_PANEL(g_object_get_data(G_OBJECT(button), "user_data"));
...
/* interpret commands for buttons specified by metadata. Handle some built-in
   ones explicitly, or fork a shell to handle general ones */
/* for now, we don't have any of these */
static void metadata_button_callback(GtkWidget *button,
                                     const char *command_str) {
  // CajaInformationPanel *self = CAJA_INFORMATION_PANEL (g_object_get_data
  // (G_OBJECT (button), "user_data"));
}

/* utility routine that allocates the command buttons from the command list */

static void add_command_button(CajaInformationPanel *information_panel,
  ...
  temp_str = g_strdup_printf(_("Open With %s"),
                             g_app_info_get_display_name(application));
  ...
  g_signal_connect_data(
      temp_button, "clicked", G_CALLBACK(command_button_callback),
      g_object_ref(application), (GClosureNotify)g_object_unref, 0);
...
/* utility to construct command buttons for the information_panel from the
 * passed in metadata string */

static void add_buttons_from_metadata(CajaInformationPanel *information_panel,
    ...
    if (temp_str) {
      *temp_str = '\0';
      if (!g_ascii_strcasecmp(current_term, "button")) {
        if (button_name) g_free(button_name);
        ...
        if (button_name != NULL) {
          ...
          g_signal_connect_data(temp_button, "clicked",
                                G_CALLBACK(metadata_button_callback),
                                command_string, (GClosureNotify)g_free, 0);

          g_object_set_data(G_OBJECT(temp_button), "user_data",
                            information_panel);
...
static void caja_information_panel_update_buttons(
  ...
  button_data = caja_file_get_metadata(information_panel->details->file,
                                       CAJA_METADATA_KEY_SIDEBAR_BUTTONS, NULL);
  if (button_data) {
  ...
  if (caja_mime_has_any_applications_for_file(
          information_panel->details->file) &&
      !caja_file_is_directory(information_panel->details->file)) {
    GAppInfo *default_app;

    default_app = caja_mime_get_default_application_for_file(
        information_panel->details->file);
...
static void caja_information_panel_update_appearance(
  ...
  gtk_style_context_add_class(
      gtk_widget_get_style_context(GTK_WIDGET(information_panel)),
...
static void background_metadata_changed_callback(
  ...
  ready =
      caja_file_check_if_ready(information_panel->details->file, attributes);
...
/* here is the key routine that populates the information_panel with the
 * appropriate information when the uri changes */

static void caja_information_panel_set_uri(
    CajaInformationPanel *information_panel, const char *new_uri,
  ...
  attributes = caja_mime_actions_get_required_file_attributes();
  caja_file_monitor_add(information_panel->details->file, information_panel,
                        attributes);
  ...
  caja_sidebar_title_set_file(information_panel->details->title,
                              information_panel->details->file, initial_title);
}

static void title_changed_callback(CajaWindowInfo *window, char *new_title,
                                   CajaInformationPanel *panel) {
  caja_sidebar_title_set_text(panel->details->title, new_title);
}
...
static void loading_uri_callback(CajaWindowInfo *window, char *uri,
                                 CajaInformationPanel *panel) {
  ...
  caja_information_panel_set_uri(panel, uri, title);
  g_free(title);
}

static void selection_changed_callback(CajaWindowInfo *window,
                                       CajaInformationPanel *panel) {
  int selection_count;
  GList *selection;
  char *uri, *name;
...
static void caja_information_panel_set_parent_window(
    CajaInformationPanel *panel, CajaWindowInfo *window) {
  gpointer slot;
  char *title, *location;
  ...
  caja_information_panel_set_uri(panel, location, title);
  g_free(location);
  g_free(title);
}

static CajaSidebar *caja_information_panel_create(CajaSidebarProvider *provider,
                                                  CajaWindowInfo *window) {
  CajaInformationPanel *panel;
...
static void caja_information_panel_provider_init(
    CajaInformationPanelProvider *sidebar) {}

static void caja_information_panel_provider_class_init(
    CajaInformationPanelProviderClass *class) {}
