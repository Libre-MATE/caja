...
G_DEFINE_TYPE_WITH_PRIVATE(CajaViewAsAction, caja_view_as_action,
                           GTK_TYPE_ACTION)
G_GNUC_END_IGNORE_DEPRECATIONS;

enum { PROP_0, PROP_WINDOW };

static void activate_nth_short_list_item(CajaWindow *window, guint index) {
  CajaWindowSlot *slot;
  ...
  caja_window_slot_set_content_view(
      slot, g_list_nth_data(window->details->short_list_viewers, index));
}

static void activate_extra_viewer(CajaWindow *window) {
  CajaWindowSlot *slot;
...
static void view_as_menu_switch_views_callback(GtkComboBox *combo_box,
  ...
  if (active < 0) {
    return;
  } else if (active < GPOINTER_TO_INT(g_object_get_data(G_OBJECT(combo_box),
                                                        "num viewers"))) {
...
static void view_as_changed_callback(CajaWindow *window,
  ...
  for (node = window->details->short_list_viewers, index = 0; node != NULL;
...
static void connect_proxy(GtkAction *action, GtkWidget *proxy) {
  if (GTK_IS_TOOL_ITEM(proxy)) {
    ...
    /* Option menu for content view types; it's empty here, filled in when a uri
     * is set. Pack it into vbox so it doesn't grow vertically when location bar
     * does.
     */
    ...
    gtk_box_pack_end(GTK_BOX(view_as_menu_vbox), view_as_combo_box, TRUE, FALSE,
                     0);
    ...
    g_signal_connect(window, "view-as-changed",
                     G_CALLBACK(view_as_changed_callback), view_as_combo_box);
  }
...
static void disconnect_proxy(GtkAction *action, GtkWidget *proxy) {
  if (GTK_IS_TOOL_ITEM(proxy)) {
    ...
    g_signal_handlers_disconnect_matched(
        ...
        G_CALLBACK(view_as_changed_callback), NULL);
  }
...
static void caja_view_as_action_finalize(GObject *object) {
  (*G_OBJECT_CLASS(parent_class)->finalize)(object);
}

static void caja_view_as_action_set_property(GObject *object, guint prop_id,
                                             const GValue *value,
                                             GParamSpec *pspec) {
...
static void caja_view_as_action_get_property(GObject *object, guint prop_id,
