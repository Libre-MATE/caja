...
G_DEFINE_TYPE(CajaNavigationWindowPane, caja_navigation_window_pane,
              CAJA_TYPE_WINDOW_PANE)
#define parent_class caja_navigation_window_pane_parent_class

static void real_set_active(CajaWindowPane *pane, gboolean is_active) {
  ...
  for (l = CAJA_PATH_BAR(nav_pane->path_bar)->button_list; l; l = l->next) {
    gtk_widget_set_sensitive(
        gtk_bin_get_child(
            GTK_BIN(caja_path_bar_get_button_from_button_list_entry(l->data))),
        is_active);
  }

  /* navigation bar (manual entry) */
  caja_location_bar_set_active(CAJA_LOCATION_BAR(nav_pane->navigation_bar),
                               is_active);

  /* location button */
  gtk_widget_set_sensitive(
      gtk_bin_get_child(GTK_BIN(nav_pane->location_button)), is_active);
}

static gboolean navigation_bar_focus_in_callback(GtkWidget *widget,
                                                 GdkEventFocus *event,
                                                 gpointer user_data) {
  CajaWindowPane *pane;
...
static int bookmark_list_get_uri_index(GList *list, GFile *location) {
  GList *l;
  int i;
...
static void search_bar_focus_in_callback(CajaSearchBar *bar,
                                         CajaWindowPane *pane) {
  caja_window_set_active_pane(pane->window, pane);
}

static void search_bar_activate_callback(CajaSearchBar *bar,
...
static void search_bar_cancel_callback(GtkWidget *widget,
  ...
  if (caja_navigation_window_pane_hide_temporary_bars(pane)) {
    caja_navigation_window_restore_focus_widget(
...
static void navigation_bar_location_changed_callback(
    GtkWidget *widget, const char *uri, CajaNavigationWindowPane *pane) {
  GFile *location;
...
static void path_bar_location_changed_callback(GtkWidget *widget,
  ...
  caja_window_set_active_pane(CAJA_WINDOW_PANE(pane)->window,
                              CAJA_WINDOW_PANE(pane));

  /* check whether we already visited the target location */
  i = bookmark_list_get_uri_index(slot->back_list, location);
  if (i >= 0) {
    caja_navigation_window_back_or_forward(
        CAJA_NAVIGATION_WINDOW(win_pane->window), TRUE, i, FALSE);
  } else {
    caja_window_slot_go_to(win_pane->active_slot, location, FALSE);
  }
}

static gboolean location_button_should_be_active(
    CajaNavigationWindowPane *pane) {
  return g_settings_get_boolean(caja_preferences,
...
static void location_button_toggled_cb(GtkToggleButton *toggle,
  ...
  g_settings_set_boolean(caja_preferences,
                         CAJA_PREFERENCES_ALWAYS_USE_LOCATION_ENTRY, is_active);
...
static GtkWidget *location_button_create(CajaNavigationWindowPane *pane) {
  GtkWidget *image;
  GtkWidget *button;
  ...
  button = g_object_new(GTK_TYPE_TOGGLE_BUTTON, "image", image,
                        "focus-on-click", FALSE, "active",
                        location_button_should_be_active(pane), NULL);

  gtk_widget_set_tooltip_text(
      button, _("Toggle between button and text-based location bar"));

  g_signal_connect(button, "toggled", G_CALLBACK(location_button_toggled_cb),
                   pane);
  return button;
}

static gboolean path_bar_path_event_callback(CajaPathBar *path_bar,
                                             GFile *location,
...
static void notebook_popup_menu_show(CajaNavigationWindowPane *pane,
                                     GdkEventButton *event) {
  GtkWidget *popup;
  GtkWidget *item;
  ...
  can_move_left =
      caja_notebook_can_reorder_current_child_relative(notebook, -1);
  can_move_right =
      caja_notebook_can_reorder_current_child_relative(notebook, 1);
  ...
  gtk_menu_attach_to_widget(GTK_MENU(popup), pane->notebook, NULL);

  gtk_menu_popup_at_pointer(GTK_MENU(popup), (const GdkEvent *)event);
}

/* emitted when the user clicks the "close" button of tabs */
static void notebook_tab_close_requested(CajaNotebook *notebook,
                                         CajaWindowSlot *slot,
                                         CajaWindowPane *pane) {
...
static gboolean notebook_switch_page_cb(GtkNotebook *notebook, GtkWidget *page,
  ...
  slot =
      caja_window_pane_get_slot_for_content_box(CAJA_WINDOW_PANE(pane), widget);
  g_assert(slot != NULL);
...
void caja_navigation_window_pane_remove_page(CajaNavigationWindowPane *pane,
                                             int page_num) {
  GtkNotebook *notebook;
  notebook = GTK_NOTEBOOK(pane->notebook);

  g_signal_handlers_block_by_func(notebook, G_CALLBACK(notebook_switch_page_cb),
                                  pane);
...
void caja_navigation_window_pane_add_slot_in_tab(
    CajaNavigationWindowPane *pane, CajaWindowSlot *slot,
...
static void real_sync_location_widgets(CajaWindowPane *pane) {
  ...
  if (slot->location != NULL) {
    ...
    caja_location_bar_set_location(
        CAJA_LOCATION_BAR(navigation_pane->navigation_bar), uri);
    g_free(uri);
    caja_path_bar_set_path(CAJA_PATH_BAR(navigation_pane->path_bar),
                           slot->location);
  }
  ...
  if (pane == pane->window->details->active_pane) {
    ...
    navigation_slot = CAJA_NAVIGATION_WINDOW_SLOT(
        pane->window->details->active_pane->active_slot);
...
gboolean caja_navigation_window_pane_hide_temporary_bars(
    CajaNavigationWindowPane *pane) {
  CajaWindowSlot *slot;
  gboolean success;
...
void caja_navigation_window_pane_always_use_location_entry(
    CajaNavigationWindowPane *pane, gboolean use_entry) {
  if (use_entry) {
    caja_navigation_window_pane_set_bar_mode(pane, CAJA_BAR_NAVIGATION);
  } else {
    caja_navigation_window_pane_set_bar_mode(pane, CAJA_BAR_PATH);
  ...
  g_signal_handlers_unblock_by_func(
      pane->location_button, G_CALLBACK(location_button_toggled_cb), pane);
}
...
void caja_navigation_window_pane_setup(CajaNavigationWindowPane *pane) {
  ...
  gtk_box_pack_start(GTK_BOX(pane->widget), hbox, FALSE, FALSE, 0);
  gtk_widget_show(hbox);

  /* the header size group ensures that the location bar has the same height as
   * the sidebar header */
  header_size_group = CAJA_NAVIGATION_WINDOW(CAJA_WINDOW_PANE(pane)->window)
                          ->details->header_size_group;
  ...
  gtk_box_pack_start(GTK_BOX(hbox), pane->navigation_bar, TRUE, TRUE, 0);
  ...
  gtk_box_pack_start(GTK_BOX(hbox), pane->search_bar, TRUE, TRUE, 0);

  pane->notebook = g_object_new(CAJA_TYPE_NOTEBOOK, NULL);
  gtk_box_pack_start(GTK_BOX(pane->widget), pane->notebook, TRUE, TRUE, 0);
...
void caja_navigation_window_pane_show_location_bar_temporarily(
    CajaNavigationWindowPane *pane) {
  if (!caja_navigation_window_pane_location_bar_showing(pane)) {
...
void caja_navigation_window_pane_show_navigation_bar_temporarily(
    CajaNavigationWindowPane *pane) {
...
gboolean caja_navigation_window_pane_path_bar_showing(
    CajaNavigationWindowPane *pane) {
...
void caja_navigation_window_pane_set_bar_mode(CajaNavigationWindowPane *pane,
                                              CajaBarMode mode) {
  gboolean use_entry;
  ...
  if (focus_widget != NULL &&
      !caja_navigation_window_is_in_temporary_navigation_bar(focus_widget,
                                                             window) &&
...
gboolean caja_navigation_window_pane_search_bar_showing(
    CajaNavigationWindowPane *pane) {
...
void caja_navigation_window_pane_hide_location_bar(
    CajaNavigationWindowPane *pane, gboolean save_preference) {
  pane->temporary_location_bar = FALSE;
...
gboolean caja_navigation_window_pane_location_bar_showing(
    CajaNavigationWindowPane *pane) {
...
static void caja_navigation_window_pane_show(CajaWindowPane *pane) {
  CajaNavigationWindowPane *npane = CAJA_NAVIGATION_WINDOW_PANE(pane);

  gtk_widget_show(npane->widget);
}

/* either called due to slot change, or due to location change in the current
 * slot. */
static void real_sync_search_widgets(CajaWindowPane *window_pane) {
  CajaWindowSlot *slot;
  CajaDirectory *directory;
  CajaSearchDirectory *search_directory;
  CajaNavigationWindowPane *pane;
...
static void caja_navigation_window_pane_class_init(
    CajaNavigationWindowPaneClass *class) {
  ...
  CAJA_WINDOW_PANE_CLASS(class)->sync_location_widgets =
      real_sync_location_widgets;
}
