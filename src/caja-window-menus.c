...
#define POPUP_PATH_EXTENSION_ACTIONS \
  "/background/Before Zoom Items/Extension Actions"
...
static BookmarkHolder *bookmark_holder_new(
    CajaBookmark *bookmark, CajaWindow *window, GCallback refresh_callback,
    CajaBookmarkFailedCallback failed_callback) {
  ...
  new_bookmark_holder->changed_handler_id =
      g_signal_connect_object(bookmark, "appearance_changed", refresh_callback,
                              window, G_CONNECT_SWAPPED);

  return new_bookmark_holder;
}

static void bookmark_holder_free(BookmarkHolder *bookmark_holder) {
...
static void bookmark_holder_free_cover(gpointer callback_data,
                                       GClosure *closure) {
...
static void activate_bookmark_in_menu_item(GtkAction *action,
                                           gpointer user_data) {
  ...
  if (caja_bookmark_uri_known_not_to_exist(holder->bookmark)) {
    ...
    caja_window_slot_go_to(slot, location, should_open_in_new_tab());
    g_object_unref(location);
  }
}

void caja_menus_append_bookmark_to_menu(
    CajaWindow *window, CajaBookmark *bookmark, const char *parent_path,
    const char *parent_id, guint index_in_parent, GtkActionGroup *action_group,
    guint merge_id, GCallback refresh_callback,
    CajaBookmarkFailedCallback failed_callback) {
  ...
  bookmark_holder =
      bookmark_holder_new(bookmark, window, refresh_callback, failed_callback);
  ...
  g_snprintf(action_name, sizeof(action_name), "%s%d", parent_id,
             index_in_parent);

  G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
  action =
      gtk_action_new(action_name, name,
                     _("Go to the location specified by this bookmark"), NULL);
  ...
  g_signal_connect_data(action, "activate",
                        ...
                        bookmark_holder, bookmark_holder_free_cover, 0);
  ...
  gtk_ui_manager_add_ui(window->details->ui_manager, merge_id, parent_path,
                        action_name, action_name, GTK_UI_MANAGER_MENUITEM,
                        FALSE);

  path = g_strdup_printf("%s/%s", parent_path, action_name);
  menuitem = gtk_ui_manager_get_widget(window->details->ui_manager, path);
...
static void action_close_window_slot_callback(GtkAction *action,
                                              gpointer user_data) {
  CajaWindow *window;
  CajaWindowSlot *slot;
...
static void action_connect_to_server_callback(GtkAction *action,
                                              gpointer user_data) {
...
static void action_stop_callback(GtkAction *action, gpointer user_data) {
  CajaWindow *window;
  CajaWindowSlot *slot;
...
static void action_home_callback(GtkAction *action, gpointer user_data) {
  CajaWindow *window;
  CajaWindowSlot *slot;
  ...
  caja_window_slot_go_home(slot, should_open_in_new_tab());
}

static void action_go_to_computer_callback(GtkAction *action,
                                           gpointer user_data) {
  CajaWindow *window;
  CajaWindowSlot *slot;
  GFile *computer;
  ...
  caja_window_slot_go_to(slot, computer, should_open_in_new_tab());
  g_object_unref(computer);
}

static void action_go_to_network_callback(GtkAction *action,
                                          gpointer user_data) {
  CajaWindow *window;
  CajaWindowSlot *slot;
  GFile *network;
  ...
  caja_window_slot_go_to(slot, network, should_open_in_new_tab());
  g_object_unref(network);
}

static void action_go_to_templates_callback(GtkAction *action,
                                            gpointer user_data) {
  CajaWindow *window;
  CajaWindowSlot *slot;
  char *path;
...
static void action_go_to_trash_callback(GtkAction *action, gpointer user_data) {
  CajaWindow *window;
  CajaWindowSlot *slot;
  GFile *trash;
  ...
  caja_window_slot_go_to(slot, trash, should_open_in_new_tab());
  g_object_unref(trash);
}

static void action_reload_callback(GtkAction *action, gpointer user_data) {
  caja_window_reload(CAJA_WINDOW(user_data), should_open_in_new_tab());
}

static void action_zoom_in_callback(GtkAction *action, gpointer user_data) {
  caja_window_zoom_in(CAJA_WINDOW(user_data));
}

static void action_zoom_out_callback(GtkAction *action, gpointer user_data) {
  caja_window_zoom_out(CAJA_WINDOW(user_data));
}

static void action_zoom_normal_callback(GtkAction *action, gpointer user_data) {
...
static void action_show_backup_files_callback(GtkAction *action,
                                              gpointer callback_data) {
  CajaWindow *window;
...
static void show_hidden_files_preference_callback(gpointer callback_data) {
  CajaWindow *window;

  window = CAJA_WINDOW(callback_data);

  if (window->details->show_hidden_files_mode ==
    ...
    /* update button */
    g_signal_handlers_block_by_func(action, action_show_hidden_files_callback,
                                    window);
    ...
    caja_window_info_set_hidden_files_mode(
        window, CAJA_WINDOW_SHOW_HIDDEN_FILES_DEFAULT);
  }
}

static void show_backup_files_preference_callback(gpointer callback_data) {
  CajaWindow *window;

  window = CAJA_WINDOW(callback_data);

  if (window->details->show_backup_files_mode ==
      CAJA_WINDOW_SHOW_BACKUP_FILES_DEFAULT) {
    GtkAction *action;

    G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
    action = gtk_action_group_get_action(window->details->main_action_group,
                                         CAJA_ACTION_SHOW_BACKUP_FILES);
    g_assert(GTK_IS_ACTION(action));

    /* update button */
    g_signal_handlers_block_by_func(action, action_show_backup_files_callback,
                                    window);
    gtk_toggle_action_set_active(
        GTK_TOGGLE_ACTION(action),
        g_settings_get_boolean(caja_preferences,
    ...
    G_GNUC_END_IGNORE_DEPRECATIONS;
    g_signal_handlers_unblock_by_func(action, action_show_backup_files_callback,
                                      window);

    /* inform views */
    caja_window_info_set_backup_files_mode(
        window, CAJA_WINDOW_SHOW_BACKUP_FILES_DEFAULT);
  }
}

static void preferences_respond_callback(GtkDialog *dialog, gint response_id) {
  if (response_id == GTK_RESPONSE_CLOSE) {
    gtk_widget_destroy(GTK_WIDGET(dialog));
  }
}

static void action_preferences_callback(GtkAction *action, gpointer user_data) {
  GtkWindow *window;

  window = GTK_WINDOW(user_data);

  caja_file_management_properties_dialog_show(
      G_CALLBACK(preferences_respond_callback), window);
}

static void action_backgrounds_and_emblems_callback(GtkAction *action,
                                                    gpointer user_data) {
  GtkWindow *window;
...
static void action_about_caja_callback(GtkAction *action, gpointer user_data) {
  const gchar *license[] = {
      ...
      N_("You should have received a copy of the GNU General Public License "
         "along with Caja; if not, write to the Free Software Foundation, "
         "Inc., "
  ...
  if (!g_key_file_load_from_file(
          key_file, CAJA_DATADIR G_DIR_SEPARATOR_S "caja.about", 0, &error)) {
  ...
  gtk_show_about_dialog(
      GTK_WINDOW(user_data), "program-name", _("Caja"), "title",
      _("About Caja"), "version", VERSION, "comments",
      _("Caja lets you organize "
...
static void action_up_callback(GtkAction *action, gpointer user_data) {
  caja_window_go_up(CAJA_WINDOW(user_data), FALSE, should_open_in_new_tab());
}

static void action_caja_manual_callback(GtkAction *action, gpointer user_data) {
  CajaWindow *window;
  GError *error;
...
static void menu_item_select_cb(GtkMenuItem *proxy, CajaWindow *window) {
  GtkAction *action;
  char *message;
...
static void menu_item_deselect_cb(GtkMenuItem *proxy, CajaWindow *window) {
  gtk_statusbar_pop(GTK_STATUSBAR(window->details->statusbar),
                    window->details->help_message_cid);
}

static GtkWidget *get_event_widget(GtkWidget *proxy) {
  GtkWidget *widget;
...
static gboolean proxy_button_press_event_cb(GtkButton *button,
...
static gboolean proxy_button_release_event_cb(GtkButton *button,
                                              GdkEventButton *event,
                                              gpointer user_data) {
  if (event->button == 2) {
...
static void disconnect_proxy_cb(GtkUIManager *manager, GtkAction *action,
                                GtkWidget *proxy, CajaWindow *window) {
  GtkWidget *widget;
  ...
  if (widget) {
    g_signal_handlers_disconnect_by_func(
        widget, G_CALLBACK(proxy_button_press_event_cb), action);
    g_signal_handlers_disconnect_by_func(
...
static void connect_proxy_cb(GtkUIManager *manager, GtkAction *action,
  ...
  if (GTK_IS_TOOL_BUTTON(proxy)) {
    ...
    if (icon != NULL) {
      ...
      gtk_tool_button_set_icon_widget(GTK_TOOL_BUTTON(proxy), widget);
    }
  }
  ...
  if (widget) {
    g_signal_connect(widget, "button-press-event",
                     G_CALLBACK(proxy_button_press_event_cb), action);
    g_signal_connect(widget, "button-release-event",
                     G_CALLBACK(proxy_button_release_event_cb), action);
  }
}

static void trash_state_changed_cb(CajaTrashMonitor *monitor, gboolean state,
                                   CajaWindow *window) {
...
static void caja_window_initialize_trash_icon_monitor(CajaWindow *window) {
...
static const GtkActionEntry main_entries[] = {
    /* name, icon name, label */ {"File", NULL, N_("_File"), NULL, NULL, NULL},
    /* name, icon name, label */ {"Edit", NULL, N_("_Edit"), NULL, NULL, NULL},
    /* name, icon name, label */ {"View", NULL, N_("_View"), NULL, NULL, NULL},
    /* name, icon name, label */ {"Help", NULL, N_("_Help"), NULL, NULL, NULL},
    /* name, icon name */
    {"Close", "window-close",
     /* label, accelerator */ N_("_Close"), "<control>W",
     /* tooltip */ N_("Close this folder"),
     G_CALLBACK(action_close_window_slot_callback)},
    {"Backgrounds and Emblems", NULL, N_("_Backgrounds and Emblems..."), NULL,
     N_("Display patterns, colors, and emblems that can be used to customize "
        "appearance"),
     G_CALLBACK(action_backgrounds_and_emblems_callback)},
    {"Preferences", "preferences-desktop", N_("Prefere_nces"), NULL,
     N_("Edit Caja preferences"), G_CALLBACK(action_preferences_callback)},
    /* name, icon name, label */
    {"Up", "go-up", N_("Open _Parent"), "<alt>Up", N_("Open the parent folder"),
     G_CALLBACK(action_up_callback)},
    /* name, icon name, label */
    {"UpAccel", NULL, "UpAccel", "", NULL, G_CALLBACK(action_up_callback)},
    /* name, icon name */
    {"Stop", "process-stop",
    ...
    {"ZoomInAccel", NULL,
     ...
     /* tooltip */ NULL, G_CALLBACK(action_zoom_in_callback)},
    /* name, icon name */
    {"ZoomInAccel2", NULL,
    ...
    {"ZoomOutAccel", NULL,
     ...
     /* tooltip */ NULL, G_CALLBACK(action_zoom_out_callback)},
    /* name, icon name */
    {"Zoom Normal", "zoom-original",
    ...
    /* name, icon name */
    {"Go to Computer", CAJA_ICON_COMPUTER,
     /* label, accelerator */ N_("_Computer"), NULL,
     /* tooltip */
     N_("Browse all local and remote disks and folders accessible from this "
        "computer"),
     G_CALLBACK(action_go_to_computer_callback)},
    /* name, icon name */
    {"Go to Network", CAJA_ICON_NETWORK,
    ...
    /* name, icon name */
    {"Go to Templates", CAJA_ICON_TEMPLATE,
...
static const GtkToggleActionEntry main_toggle_entries[] = {
    /* name, icon name */ {
        "Show Hidden Files", NULL,
        /* label, accelerator */ N_("Show _Hidden Files"), "<control>H",
        /* tooltip */
        N_("Toggle the display of hidden files in the current window"),
        G_CALLBACK(action_show_hidden_files_callback), TRUE},
    /* name, stock id */
    {"Show Backup Files", NULL,
     /* label, accelerator */ N_("Show Bac_kup Files"), "<control>K",
     /* tooltip */
     N_("Toggle the display of backup files in the current window"),
     G_CALLBACK(action_show_backup_files_callback), TRUE},
...
void caja_window_initialize_menus(CajaWindow *window) {
  ...
  gtk_action_group_add_actions(action_group, main_entries,
                               G_N_ELEMENTS(main_entries), window);
  gtk_action_group_add_toggle_actions(action_group, main_toggle_entries,
                                      G_N_ELEMENTS(main_toggle_entries),
                                      window);
  ...
  action =
      gtk_action_group_get_action(action_group, CAJA_ACTION_SHOW_HIDDEN_FILES);
  g_signal_handlers_block_by_func(action, action_show_hidden_files_callback,
                                  window);
  gtk_toggle_action_set_active(
      GTK_TOGGLE_ACTION(action),
  ...
  g_signal_connect_swapped(
      ...
      G_CALLBACK(show_hidden_files_preference_callback), window);

  action =
      gtk_action_group_get_action(action_group, CAJA_ACTION_SHOW_BACKUP_FILES);
  g_signal_handlers_block_by_func(action, action_show_backup_files_callback,
                                  window);
  gtk_toggle_action_set_active(
      GTK_TOGGLE_ACTION(action),
  ...
  g_signal_connect_swapped(
      caja_preferences, "changed::" CAJA_PREFERENCES_SHOW_BACKUP_FILES,
      G_CALLBACK(show_backup_files_preference_callback), window);
  ...
  g_signal_connect(ui_manager, "connect_proxy", G_CALLBACK(connect_proxy_cb),
                   window);
...
void caja_window_finalize_menus(CajaWindow *window) {
  ...
  g_signal_handlers_disconnect_by_func(monitor, trash_state_changed_cb, window);

  g_signal_handlers_disconnect_by_func(
      caja_preferences, show_hidden_files_preference_callback, window);

  g_signal_handlers_disconnect_by_func(
      caja_preferences, show_backup_files_preference_callback, window);
}

static GList *get_extension_menus(CajaWindow *window) {
  CajaWindowSlot *slot;
  GList *providers;
  GList *items;
  GList *l;
  ...
  for (l = providers; l != NULL; l = l->next) {
    ...
    file_items = caja_menu_provider_get_background_items(
        provider, GTK_WIDGET(window), slot->viewed_file);
...
static void add_extension_menu_items(CajaWindow *window, guint merge_id,
                                     GtkActionGroup *action_group,
                                     GList *menu_items,
  ...
  for (l = menu_items; l; l = l->next) {
    ...
    gtk_ui_manager_add_ui(
        ui_manager, merge_id, path, action_name, action_name,
    ...
    if (menu != NULL) {
      ...
      G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
      subdir = g_build_path("/", subdirectory, "/", gtk_action_get_name(action),
                            NULL);
      G_GNUC_END_IGNORE_DEPRECATIONS;
      add_extension_menu_items(window, merge_id, action_group, children,
                               subdir);
...
#endif /* ENABLE_NLS */
  G_GNUC_END_IGNORE_DEPRECATIONS;
  gtk_ui_manager_insert_action_group(window->details->ui_manager, action_group,
                                     0);
  g_object_unref(action_group); /* owned by ui manager */
