...
static void on_changed(GtkEditable *editable, CajaNotesViewer *notes);
...
G_DEFINE_TYPE_WITH_CODE(
    CajaNotesViewer, caja_notes_viewer, GTK_TYPE_SCROLLED_WINDOW,
...
G_DEFINE_TYPE_WITH_CODE(
    CajaNotesViewerProvider, caja_notes_viewer_provider, G_TYPE_OBJECT,
...
static gboolean schedule_save_callback(gpointer data) {
  CajaNotesViewer *notes;
...
static void cancel_pending_save(CajaNotesViewer *notes) {
...
static void schedule_save(CajaNotesViewer *notes) {
...
static void set_saved_text(CajaNotesViewer *notes, char *new_notes) {
  char *old_text;
  ...
  if (g_strcmp0(old_text, new_notes) != 0) {
    g_signal_emit_by_name(CAJA_SIDEBAR(notes), "tab_icon_changed");
  }

  g_free(old_text);
}

/* save the metainfo corresponding to the current uri, if any, into the text
 * field */
static void notes_save_metainfo(CajaNotesViewer *notes) {
  char *notes_text;
  ...
  g_signal_handlers_block_matched(
      notes->details->file, G_SIGNAL_MATCH_FUNC | G_SIGNAL_MATCH_DATA, 0, 0,
      NULL, G_CALLBACK(load_note_text_from_metadata), notes);
  ...
  caja_file_set_metadata(notes->details->file, CAJA_METADATA_KEY_ANNOTATION,
                         NULL, notes_text);
...
static void load_note_text_from_metadata(CajaFile *file,
  ...
  if (g_strcmp0(saved_text, notes->details->previous_saved_text) != 0) {
    ...
    g_signal_handlers_block_matched(notes->details->text_buffer,
                                    ...
                                    0, 0, NULL, G_CALLBACK(on_changed), notes);
    gtk_text_buffer_set_text(notes->details->text_buffer, saved_text, -1);
    g_signal_handlers_unblock_matched(
        notes->details->text_buffer, G_SIGNAL_MATCH_FUNC | G_SIGNAL_MATCH_DATA,
        0, 0, NULL, G_CALLBACK(on_changed), notes);
  } else {
    g_free(saved_text);
  }
...
static void done_with_file(CajaNotesViewer *notes) {
  cancel_pending_save(notes);
...
static void notes_load_metainfo(CajaNotesViewer *notes) {
   ...
   */
  g_signal_handlers_block_matched(notes->details->text_buffer,
                                  G_SIGNAL_MATCH_FUNC | G_SIGNAL_MATCH_DATA, 0,
                                  0, NULL, G_CALLBACK(on_changed), notes);
...
static void loading_uri_callback(CajaWindowInfo *window, const char *location,
                                 CajaNotesViewer *notes) {
...
static void caja_notes_viewer_init(CajaNotesViewer *sidebar) {
  ...
  details->note_text_field =
      gtk_text_view_new_with_buffer(details->text_buffer);
  ...
  g_signal_connect(details->text_buffer, "changed", G_CALLBACK(on_changed),
                   sidebar);

  gtk_widget_show_all(GTK_WIDGET(sidebar));
}

static void caja_notes_viewer_finalize(GObject *object) {
...
static void caja_notes_viewer_class_init(CajaNotesViewerClass *class) {
  G_OBJECT_CLASS(class)->finalize = caja_notes_viewer_finalize;
}

static const char *caja_notes_viewer_get_sidebar_id(CajaSidebar *sidebar) {
  return CAJA_NOTES_SIDEBAR_ID;
}

static char *caja_notes_viewer_get_tab_label(CajaSidebar *sidebar) {
  return g_strdup(_("Notes"));
}

static char *caja_notes_viewer_get_tab_tooltip(CajaSidebar *sidebar) {
  return g_strdup(_("Show Notes"));
}

static GdkPixbuf *caja_notes_viewer_get_tab_icon(CajaSidebar *sidebar) {
  CajaNotesViewer *notes;
...
static void caja_notes_viewer_is_visible_changed(CajaSidebar *sidebar,
                                                 gboolean is_visible) {
  /* Do nothing */
}
...
static void caja_notes_viewer_set_parent_window(CajaNotesViewer *sidebar,
...
static CajaSidebar *caja_notes_viewer_create_sidebar(
    CajaSidebarProvider *provider, CajaWindowInfo *window) {
  CajaNotesViewer *sidebar;
...
static GList *get_property_pages(CajaPropertyPageProvider *provider,
  ...
  page = caja_property_page_new("CajaNotesViewer::property_page",
                                gtk_label_new(_("Notes")), GTK_WIDGET(viewer));
...
static void caja_notes_viewer_provider_init(CajaNotesViewerProvider *sidebar) {}

static void caja_notes_viewer_provider_class_init(
    CajaNotesViewerProviderClass *class) {}
