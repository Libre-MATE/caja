...
static gboolean skeleton_handle_show_items_cb(
    CajaFreedesktopFileManager1 *object, GDBusMethodInvocation *invocation,
    const gchar *const *uris, const gchar *startup_id,
...
static gboolean skeleton_handle_show_folders_cb(
    CajaFreedesktopFileManager1 *object, GDBusMethodInvocation *invocation,
    const gchar *const *uris, const gchar *startup_id,
    CajaFreedesktopDBus *fdb) {
  CajaApplication *application;
...
static gboolean skeleton_handle_show_item_properties_cb(
    CajaFreedesktopFileManager1 *object, GDBusMethodInvocation *invocation,
    const gchar *const *uris, const gchar *startup_id,
    CajaFreedesktopDBus *fdb) {
  CajaApplication *application;
  ...
  if (uris[0] != NULL) {
    ...
    window = caja_application_get_spatial_window(
        application, NULL, startup_id, file, gdk_screen_get_default(), NULL);
  ...
  caja_freedesktop_file_manager1_complete_show_item_properties(object,
                                                               invocation);
  return TRUE;
}

static void bus_acquired_cb(GDBusConnection *conn, const gchar *name,
                            gpointer user_data) {
...
static void name_lost_cb(GDBusConnection *connection, const gchar *name,
  ...
  caja_debug_log(
      ...
      "Lost (or failed to acquire) the name %s on the session message bus\n",
      name);
}

static void caja_freedesktop_dbus_dispose(GObject *object) {
  ...
  if (fdb->skeleton != NULL) {
    g_dbus_interface_skeleton_unexport(
        G_DBUS_INTERFACE_SKELETON(fdb->skeleton));
...
static void caja_freedesktop_dbus_class_init(CajaFreedesktopDBusClass *klass) {
  GObjectClass *object_class = G_OBJECT_CLASS(klass);

  object_class->dispose = caja_freedesktop_dbus_dispose;
}

static void caja_freedesktop_dbus_init(CajaFreedesktopDBus *fdb) {
  fdb->owner_id = g_bus_own_name(G_BUS_TYPE_SESSION, CAJA_FDO_DBUS_NAME,
                                 G_BUS_NAME_OWNER_FLAGS_NONE, bus_acquired_cb,
                                 name_acquired_cb, name_lost_cb, fdb, NULL);
}

/* Tries to own the org.freedesktop.FileManager1 service name */
CajaFreedesktopDBus *caja_freedesktop_dbus_new(CajaApplication *application) {
  CajaFreedesktopDBus *fdb =
      g_object_new(caja_freedesktop_dbus_get_type(), NULL);
