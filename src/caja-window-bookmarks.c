...
static void remove_bookmarks_for_uri_if_yes(GtkDialog *dialog, int response,
  ...
  if (response == GTK_RESPONSE_YES) {
    ...
    caja_bookmark_list_delete_items_with_uri(window->details->bookmark_list,
                                             uri);
  }
...
static void show_bogus_bookmark_window(CajaWindow *window,
  ...
  prompt =
      _("Do you want to remove any bookmarks with the "
  ...
  dialog = eel_show_yes_no_dialog(prompt, detail,
                                  ...
                                  "process-stop", GTK_WINDOW(window));

  g_signal_connect(dialog, "response",
                   G_CALLBACK(remove_bookmarks_for_uri_if_yes), window);
  g_object_set_data_full(G_OBJECT(dialog), "uri", g_file_get_uri(location),
                         g_free);
...
static GtkWindow *get_or_create_bookmarks_window(CajaWindow *window) {
  if (bookmarks_window == NULL) {
    bookmarks_window =
        create_bookmarks_window(window->details->bookmark_list, window);
  } else {
...
void caja_window_edit_bookmarks(CajaWindow *window) {
  ...
  gtk_window_set_screen(dialog, gtk_window_get_screen(GTK_WINDOW(window)));
  gtk_window_present(dialog);
}

static void remove_bookmarks_menu_items(CajaWindow *window) {
  GtkUIManager *ui_manager;

  ui_manager = caja_window_get_ui_manager(window);
  if (window->details->bookmarks_merge_id != 0) {
    gtk_ui_manager_remove_ui(ui_manager, window->details->bookmarks_merge_id);
...
static void update_bookmarks(CajaWindow *window) {
  ...
  for (index = 0; index < bookmark_count; ++index) {
    ...
    caja_menus_append_bookmark_to_menu(
        CAJA_WINDOW(window), bookmark,
        CAJA_WINDOW_GET_CLASS(window)->bookmarks_placeholder, "dynamic", index,
        window->details->bookmarks_action_group,
        window->details->bookmarks_merge_id, G_CALLBACK(refresh_bookmarks_menu),
...
void caja_window_initialize_bookmarks_menu(CajaWindow *window) {
  ...
  g_signal_connect_object(window->details->bookmark_list, "contents_changed",
                          G_CALLBACK(refresh_bookmarks_menu), window,
                          G_CONNECT_SWAPPED);
}
