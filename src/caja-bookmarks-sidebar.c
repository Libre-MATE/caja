...
G_DEFINE_TYPE_WITH_CODE(
    CajaBookmarksSidebar, caja_bookmarks_sidebar, GTK_TYPE_SCROLLED_WINDOW,
    G_IMPLEMENT_INTERFACE(CAJA_TYPE_SIDEBAR,
                          caja_bookmarks_sidebar_iface_init));

G_DEFINE_TYPE_WITH_CODE(CajaBookmarksSidebarProvider,
                        caja_bookmarks_sidebar_provider, G_TYPE_OBJECT,
...
static void update_bookmarks(CajaBookmarksSidebar *sidebar) {
  GtkListStore *store;
  ...
  selection =
      GTK_TREE_SELECTION(gtk_tree_view_get_selection(sidebar->tree_view));
  ...
  for (index = 0; index < bookmark_count; index++) {
    ...
    gtk_list_store_set(store, &iter, BOOKMARKS_SIDEBAR_COLUMN_ICON, surface,
                       ...
                       BOOKMARKS_SIDEBAR_COLUMN_BOOKMARK, bookmark, -1);
  ...
  if (bookmarks_list_empty) {
    gtk_list_store_append(store, &iter);
    gtk_list_store_set(store, &iter, BOOKMARKS_SIDEBAR_COLUMN_ICON, NULL,
                       ...
                       BOOKMARKS_SIDEBAR_COLUMN_BOOKMARK, NULL, -1);
    gtk_tree_selection_select_iter(selection, &iter);
  }
}

static void open_selected_item(CajaBookmarksSidebar *sidebar, GtkTreePath *path,
  ...
  gtk_tree_model_get(model, &iter, BOOKMARKS_SIDEBAR_COLUMN_BOOKMARK, &bookmark,
                     -1);
...
static void row_activated_callback(GtkTreeView *tree_view, GtkTreePath *path,
...
static gboolean button_press_event_callback(GtkWidget *widget,
  ...
  if (gtk_tree_view_get_path_at_pos(sidebar->tree_view, event->x, event->y,
                                    &path, NULL, NULL, NULL)) {
    open_selected_item(sidebar, path, open_flags);
 ...
 * allow selection.
 */
static gboolean is_row_selectable(GtkTreeSelection *selection,
...
static void caja_bookmarks_sidebar_init(CajaBookmarksSidebar *sidebar) {
  ...
  gtk_tree_view_column_set_attributes(
      col, cell, "surface", BOOKMARKS_SIDEBAR_COLUMN_ICON, "visible",
      BOOKMARKS_SIDEBAR_COLUMN_ICON_VISIBLE, NULL);

  cell = gtk_cell_renderer_text_new();
  gtk_tree_view_column_pack_start(col, cell, TRUE);
  gtk_tree_view_column_set_attributes(col, cell, "text",
                                      BOOKMARKS_SIDEBAR_COLUMN_NAME, "style",
                                      BOOKMARKS_SIDEBAR_COLUMN_STYLE, NULL);

  gtk_tree_view_column_set_fixed_width(col, CAJA_ICON_SIZE_SMALLER);
  gtk_tree_view_append_column(tree_view, col);

  store = gtk_list_store_new(
      BOOKMARKS_SIDEBAR_COLUMN_COUNT, CAIRO_GOBJECT_TYPE_SURFACE, G_TYPE_STRING,
      PANGO_TYPE_STYLE, G_TYPE_BOOLEAN, CAJA_TYPE_BOOKMARK);
   ...
   * (above).
   */
  gtk_tree_selection_set_select_function(selection, is_row_selectable, NULL,
                                         NULL);
...
static void caja_bookmarks_sidebar_finalize(GObject *object) {
  CajaBookmarksSidebar *sidebar;
...
static void caja_bookmarks_sidebar_class_init(
    CajaBookmarksSidebarClass *class) {
...
static const char *caja_bookmarks_sidebar_get_sidebar_id(CajaSidebar *sidebar) {
  return CAJA_BOOKMARKS_SIDEBAR_ID;
}

static char *caja_bookmarks_sidebar_get_tab_label(CajaSidebar *sidebar) {
  return g_strdup(_("Bookmarks"));
}

static char *caja_bookmarks_sidebar_get_tab_tooltip(CajaSidebar *sidebar) {
  return g_strdup(_("Show a list of your personal bookmarks"));
}

static GdkPixbuf *caja_bookmarks_sidebar_get_tab_icon(CajaSidebar *sidebar) {
  return NULL;
}

static void caja_bookmarks_sidebar_is_visible_changed(CajaSidebar *sidebar,
                                                      gboolean is_visible) {
  /* Do nothing */
}
...
static void caja_bookmarks_sidebar_set_parent_window(
    CajaBookmarksSidebar *sidebar, CajaWindowInfo *window) {
  CajaWindowSlotInfo *slot;
  ...
  g_signal_connect_object(sidebar->bookmarks, "contents_changed",
                          G_CALLBACK(update_bookmarks), sidebar,
                          G_CONNECT_SWAPPED);
...
static CajaSidebar *caja_bookmarks_sidebar_create(CajaSidebarProvider *provider,
...
static void caja_bookmarks_sidebar_provider_init(
    CajaBookmarksSidebarProvider *sidebar) {}

static void caja_bookmarks_sidebar_provider_class_init(
    CajaBookmarksSidebarProviderClass *class) {}
