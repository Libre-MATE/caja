...
G_DEFINE_TYPE_WITH_CODE(CajaWindowSlot, caja_window_slot, G_TYPE_OBJECT,
...
static void query_editor_changed_callback(CajaSearchBar *bar, CajaQuery *query,
                                          gboolean reload,
                                          CajaWindowSlot *slot) {
...
static void real_update_query_editor(CajaWindowSlot *slot) {
  CajaDirectory *directory;
  ...
  if (CAJA_IS_SEARCH_DIRECTORY(directory)) {
    ...
    query_editor = caja_query_editor_new(
    ...
    if (query != NULL) {
      caja_query_editor_set_query(CAJA_QUERY_EDITOR(query_editor), query);
...
static void real_active(CajaWindowSlot *slot) {
  CajaWindow *window;
...
static void caja_window_slot_active(CajaWindowSlot *slot) {
  CajaWindow *window;
  CajaWindowPane *pane;
  ...
  EEL_CALL_METHOD(CAJA_WINDOW_SLOT_CLASS, slot, active, (slot));
}

static void real_inactive(CajaWindowSlot *slot) {
  CajaWindow *window;
...
static void caja_window_slot_inactive(CajaWindowSlot *slot) {
  CajaWindow *window;
  CajaWindowPane *pane;
  ...
  EEL_CALL_METHOD(CAJA_WINDOW_SLOT_CLASS, slot, inactive, (slot));
}
...
static int caja_window_slot_get_selection_count(CajaWindowSlot *slot) {
  g_assert(CAJA_IS_WINDOW_SLOT(slot));
...
char *caja_window_slot_get_location_uri(CajaWindowSlotInfo *slot) {
  g_assert(CAJA_IS_WINDOW_SLOT(slot));
...
static void caja_window_slot_make_hosting_pane_active(CajaWindowSlot *slot) {
  g_assert(CAJA_IS_WINDOW_SLOT(slot));
...
static CajaWindow *caja_window_slot_get_window(CajaWindowSlot *slot) {
  g_assert(CAJA_IS_WINDOW_SLOT(slot));
 ...
 */
static void caja_window_slot_set_title(CajaWindowSlot *slot,
                                       const char *title) {
  CajaWindow *window;
  gboolean changed;
  ...
  if (eel_strlen(slot->title) > 0 && slot->current_location_bookmark &&
      caja_bookmark_set_name(slot->current_location_bookmark, slot->title)) {
    changed = TRUE;
...
void caja_window_slot_update_icon(CajaWindowSlot *slot) {
  ...
  info = EEL_CALL_METHOD_WITH_RETURN_VALUE(CAJA_WINDOW_CLASS, window, get_icon,
                                           (window, slot));

  if (slot != slot->pane->active_slot) return;
  ...
  if (info) {
    ...
    if (icon_name != NULL) {
      ...
      if ((current_icon_name == NULL) ||
          (strcmp(icon_name, current_icon_name) != 0)) {
...
void caja_window_slot_is_in_active_pane(CajaWindowSlot *slot,
                                        gboolean is_active) {
...
void caja_window_slot_disconnect_content_view(CajaWindowSlot *slot,
  ...
  window = slot->pane->window;
  if (window != NULL && window->details->active_pane &&
      window->details->active_pane->active_slot == slot) {
...
void caja_window_slot_set_content_view_widget(CajaWindowSlot *slot,
  ...
  if (new_view != NULL) {
    ...
    gtk_box_pack_start(GTK_BOX(slot->view_box), widget, TRUE, TRUE, 0);
...
void caja_window_slot_set_allow_stop(CajaWindowSlot *slot, gboolean allow) {
  CajaWindow *window;
...
void caja_window_slot_set_status(CajaWindowSlot *slot, const char *status) {
  CajaWindow *window;
...
void caja_window_slot_update_query_editor(CajaWindowSlot *slot) {
  ...
  EEL_CALL_METHOD(CAJA_WINDOW_SLOT_CLASS, slot, update_query_editor, (slot));
...
void caja_window_slot_remove_extra_location_widgets(CajaWindowSlot *slot) {
  gtk_container_foreach(GTK_CONTAINER(slot->extra_location_widgets), remove_all,
...
void caja_window_slot_add_extra_location_widget(CajaWindowSlot *slot,
                                                GtkWidget *widget) {
  gtk_box_pack_start(GTK_BOX(slot->extra_location_widgets), widget, TRUE, TRUE,
                     0);
  gtk_widget_show(slot->extra_location_frame);
}

void caja_window_slot_add_current_location_to_history_list(
    CajaWindowSlot *slot) {
  if ((slot->pane->window == NULL ||
       !CAJA_IS_DESKTOP_WINDOW(slot->pane->window)) &&
      caja_add_bookmark_to_history_list(slot->current_location_bookmark)) {
    caja_send_history_list_changed();
  }
}

/* returns either the pending or the actual current location - used by side
 * panes. */
static char *real_slot_info_get_current_location(CajaWindowSlotInfo *info) {
  CajaWindowSlot *slot;
...
static CajaView *real_slot_info_get_current_view(CajaWindowSlotInfo *info) {
  CajaWindowSlot *slot;
