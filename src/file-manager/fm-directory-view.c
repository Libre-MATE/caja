...
#define FM_DIRECTORY_VIEW_MENU_PATH_APPLICATIONS_SUBMENU_PLACEHOLDER \
  "/MenuBar/File/Open Placeholder/Open With/Applications Placeholder"
#define FM_DIRECTORY_VIEW_MENU_PATH_APPLICATIONS_PLACEHOLDER \
  "/MenuBar/File/Open Placeholder/Applications Placeholder"
#define FM_DIRECTORY_VIEW_MENU_PATH_SCRIPTS_PLACEHOLDER \
  "/MenuBar/File/Open Placeholder/Scripts/Scripts Placeholder"
#define FM_DIRECTORY_VIEW_MENU_PATH_EXTENSION_ACTIONS_PLACEHOLDER \
  "/MenuBar/Edit/Extension Actions"
#define FM_DIRECTORY_VIEW_MENU_PATH_NEW_DOCUMENTS_PLACEHOLDER        \
...
#define FM_DIRECTORY_VIEW_POPUP_PATH_APPLICATIONS_SUBMENU_PLACEHOLDER \
  "/selection/Open Placeholder/Open With/Applications Placeholder"
#define FM_DIRECTORY_VIEW_POPUP_PATH_APPLICATIONS_PLACEHOLDER \
  "/selection/Open Placeholder/Applications Placeholder"
#define FM_DIRECTORY_VIEW_POPUP_PATH_SCRIPTS_PLACEHOLDER \
  "/selection/Open Placeholder/Scripts/Scripts Placeholder"
#define FM_DIRECTORY_VIEW_POPUP_PATH_EXTENSION_ACTIONS \
...
#define FM_DIRECTORY_VIEW_POPUP_PATH_BACKGROUND_SCRIPTS_PLACEHOLDER \
  "/background/Before Zoom Items/New Object Items/Scripts/Scripts Placeholder"
#define FM_DIRECTORY_VIEW_POPUP_PATH_BACKGROUND_NEW_DOCUMENTS_PLACEHOLDER \
  "/background/Before Zoom Items/New Object Items/New Documents/New "     \
...
struct FMDirectoryViewDetails {
   ...
   * This flag should need not be set inside subclasses. FMDirectoryView
...
static void load_directory(FMDirectoryView *view, CajaDirectory *directory);
...
static void fm_directory_view_drop_proxy_received_uris(
    FMDirectoryView *view, const GList *source_uri_list, const char *target_uri,
    GdkDragAction action);
static void fm_directory_view_drop_proxy_received_netscape_url(
    FMDirectoryView *view, const char *netscape_url, const char *target_uri,
    GdkDragAction action);
static void clipboard_changed_callback(CajaClipboardMonitor *monitor,
                                       FMDirectoryView *view);
static void open_one_in_new_window(gpointer data, gpointer callback_data);
static void open_one_in_folder_window(gpointer data, gpointer callback_data);
...
static void metadata_for_directory_as_file_ready_callback(
    CajaFile *file, gpointer callback_data);
static void metadata_for_files_in_directory_ready_callback(
    CajaDirectory *directory, GList *files, gpointer callback_data);
static void fm_directory_view_trash_state_changed_callback(
    CajaTrashMonitor *trash, gboolean state, gpointer callback_data);
...
static void action_move_to_next_pane_callback(GtkAction *action,
                                              gpointer callback_data);
static void action_rename_callback(GtkAction *action, gpointer callback_data);
...
static void action_connect_to_server_link_callback(GtkAction *action,
                                                   gpointer data);
static void action_mount_volume_callback(GtkAction *action, gpointer data);
static void action_unmount_volume_callback(GtkAction *action, gpointer data);
static void action_format_volume_callback(GtkAction *action, gpointer data);
static void action_start_volume_callback(GtkAction *action, gpointer data);
static void action_stop_volume_callback(GtkAction *action, gpointer data);
static void action_detect_media_callback(GtkAction *action, gpointer data);
...
static inline void fm_directory_view_widget_to_file_operation_position(
    FMDirectoryView *view, GdkPoint *position);
static void fm_directory_view_widget_to_file_operation_position_xy(
    FMDirectoryView *view, int *x, int *y);

/* undo-related actions */

static void undo_redo_menu_update_callback(CajaUndoStackManager *manager,
...
EEL_CLASS_BOILERPLATE(FMDirectoryView, fm_directory_view,
...
static ApplicationLaunchParameters *application_launch_parameters_new(
    GAppInfo *application, GList *files, FMDirectoryView *directory_view) {
...
static void application_launch_parameters_free(
...
static GList *file_and_directory_list_to_files(GList *fad_list) {
  GList *res, *l;
...
static GList *file_and_directory_list_from_files(CajaDirectory *directory,
                                                 GList *files) {
  GList *res, *l;
  FileAndDirectory *fad = NULL;
...
static void file_and_directory_free(FileAndDirectory *fad) {
  caja_directory_unref(fad->directory);
  caja_file_unref(fad->file);
  g_free(fad);
}

static void file_and_directory_list_free(GList *list) {
  GList *l;
...
static ScriptLaunchParameters *script_launch_parameters_new(
...
static CreateTemplateParameters *create_template_parameters_new(
    CajaFile *file, FMDirectoryView *directory_view) {
...
static void create_templates_parameters_free(
...
CajaWindowSlotInfo *fm_directory_view_get_caja_window_slot(
    FMDirectoryView *view) {
...
static gboolean fm_directory_view_confirm_multiple(GtkWindow *parent_window,
                                                   int count, gboolean tabs) {
  GtkDialog *dialog;
  char *prompt;
  char *detail;
  int response;
  ...
  if (tabs) {
    detail =
        g_strdup_printf(ngettext("This will open %'d separate tab.",
                                 "This will open %'d separate tabs.", count),
                        count);
  } else {
    detail =
  ...
  dialog = eel_show_yes_no_dialog(prompt, detail, "gtk-ok", "process-stop",
                                  parent_window);
  g_free(detail);
...
static gboolean selection_contains_one_item_in_menu_callback(
  ...
  if (!view->details->menu_states_untrustworthy) {
    g_warning(
        "Expected one selected item, found %'d. No action will be performed.",
...
static gboolean selection_not_empty_in_menu_callback(FMDirectoryView *view,
                                                     GList *selection) {
  if (selection != NULL) {
    return TRUE;
  }
  ...
  if (!view->details->menu_states_untrustworthy) {
    g_warning(
        "Empty selection found when selection was expected. No action will be "
        "performed.");
  }
...
static char *get_view_directory(FMDirectoryView *view) {
  char *uri, *path;
  GFile *f;
...
void fm_directory_view_activate_files(FMDirectoryView *view, GList *files,
  ...
  caja_mime_activate_files(fm_directory_view_get_containing_window(view),
                           view->details->slot, files, path, mode, flags,
                           confirm_multiple);

  g_free(path);
}

void fm_directory_view_activate_file(FMDirectoryView *view, CajaFile *file,
                                     CajaWindowOpenMode mode,
                                     CajaWindowOpenFlags flags) {
  char *path;
  ...
  caja_mime_activate_file(fm_directory_view_get_containing_window(view),
                          view->details->slot, file, path, mode, flags);

  g_free(path);
}

static void action_open_callback(GtkAction *action, gpointer callback_data) {
  GList *selection;
  FMDirectoryView *view;
  ...
  fm_directory_view_activate_files(view, selection,
                                   CAJA_WINDOW_OPEN_ACCORDING_TO_MODE, 0, TRUE);
  caja_file_list_free(selection);
}

static void action_open_close_parent_callback(GtkAction *action,
                                              gpointer callback_data) {
  GList *selection;
  FMDirectoryView *view;
  ...
  fm_directory_view_activate_files(view, selection,
                                   ...
                                   CAJA_WINDOW_OPEN_FLAG_CLOSE_BEHIND, TRUE);
  caja_file_list_free(selection);
}

static void action_open_alternate_callback(GtkAction *action,
                                           gpointer callback_data) {
  FMDirectoryView *view;
  GList *selection;
  GtkWindow *window;
...
static void action_open_new_tab_callback(GtkAction *action,
                                         gpointer callback_data) {
  FMDirectoryView *view;
  GList *selection;
  GtkWindow *window;
  ...
  if (fm_directory_view_confirm_multiple(window, g_list_length(selection),
    ...
    fm_directory_view_activate_files(view, selection,
                                     CAJA_WINDOW_OPEN_ACCORDING_TO_MODE,
                                     CAJA_WINDOW_OPEN_FLAG_NEW_TAB, FALSE);
  }

  caja_file_list_free(selection);
}

static void action_open_folder_window_callback(GtkAction *action,
                                               gpointer callback_data) {
  FMDirectoryView *view;
  GList *selection;
  GtkWindow *window;
...
static void open_location(FMDirectoryView *directory_view, const char *new_uri,
                          CajaWindowOpenMode mode, CajaWindowOpenFlags flags) {
  GtkWindow *window;
  GFile *location;
  ...
  caja_window_slot_info_open_location(directory_view->details->slot, location,
                                      mode, flags, NULL);
  g_object_unref(location);
}

static void application_selected_cb(CajaOpenWithDialog *dialog, GAppInfo *app,
                                    gpointer user_data) {
...
static void choose_program(FMDirectoryView *view, CajaFile *file) {
  GtkWidget *dialog;
  char *uri;
  char *mime_type;
  ...
  g_object_set_data_full(G_OBJECT(dialog), "directory-view:file",
  ...
  g_signal_connect_object(dialog, "application_selected",
                          G_CALLBACK(application_selected_cb),
                          fm_directory_view_get_containing_window(view), 0);
...
static void open_with_other_program(FMDirectoryView *view) {
  GList *selection;
...
static void action_other_application_callback(GtkAction *action,
                                              gpointer callback_data) {
...
static void trash_or_delete_selected_files(FMDirectoryView *view) {
...
static gboolean real_trash(FMDirectoryView *view) {
...
static void action_trash_callback(GtkAction *action, gpointer callback_data) {
  trash_or_delete_selected_files(FM_DIRECTORY_VIEW(callback_data));
}

static void delete_selected_files(FMDirectoryView *view) {
  GList *selection;
  GList *node;
  GList *locations;
  ...
  caja_file_operations_delete(
      locations, fm_directory_view_get_containing_window(view), NULL, NULL);

  g_list_free_full(locations, g_object_unref);
  caja_file_list_free(selection);
}

static void action_delete_callback(GtkAction *action, gpointer callback_data) {
  delete_selected_files(FM_DIRECTORY_VIEW(callback_data));
}

static void action_restore_from_trash_callback(GtkAction *action,
                                               gpointer callback_data) {
  FMDirectoryView *view;
  GList *selection;
...
static gboolean real_delete(FMDirectoryView *view) {
  G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
  GtkAction *action;
...
static void action_duplicate_callback(GtkAction *action,
                                      gpointer callback_data) {
  FMDirectoryView *view;
  GList *selection;
  ...
  if (selection_not_empty_in_menu_callback(view, selection)) {
     ...
     * should change things here so that we use a get_icon_locations (view,
     * selection). Not a problem in this case but in other places the selection
     * may change by the time we go and retrieve the icon positions, relying on
     * the selection staying intact to ensure the right sequence and count of
     * positions is fragile.
     */
    selected_item_locations =
        fm_directory_view_get_selected_icon_locations(view);
...
static void action_create_link_callback(GtkAction *action,
                                        gpointer callback_data) {
  FMDirectoryView *view;
  GList *selection;
...
static void action_invert_selection_callback(GtkAction *action,
                                             gpointer callback_data) {
  g_assert(FM_IS_DIRECTORY_VIEW(callback_data));
...
static void pattern_select_response_cb(GtkWidget *dialog, int response,
  ...
  switch (response) {
    case GTK_RESPONSE_OK:
      ...
      selection = caja_directory_match_pattern(
    ...
    case GTK_RESPONSE_HELP:
      ...
      if (error) {
        eel_show_error_dialog(_("There was an error displaying help."),
                              error->message, GTK_WINDOW(dialog));
        g_error_free(error);
      }
      break;
    default:
...
static void select_pattern(FMDirectoryView *view) {
  GtkWidget *dialog;
  GtkWidget *label;
  GtkWidget *example;
  GtkWidget *grid;
  ...
  eel_dialog_add_button(GTK_DIALOG(dialog), _("_Help"), "help-browser",
                        GTK_RESPONSE_HELP);

  eel_dialog_add_button(GTK_DIALOG(dialog), _("_Cancel"), "process-stop",
                        GTK_RESPONSE_CANCEL);
  ...
  example_pattern = g_strdup_printf("<b>%s</b><i>%s</i>", _("Examples: "),
                                    "*.png, file\?\?.txt, pict*.\?\?\?");
  ...
  grid = gtk_grid_new();
  g_object_set(grid, "orientation", GTK_ORIENTATION_VERTICAL, "border-width", 6,
               "row-spacing", 6, "column-spacing", 12, NULL);

  gtk_container_add(GTK_CONTAINER(grid), label);
  gtk_grid_attach_next_to(GTK_GRID(grid), entry, label, GTK_POS_RIGHT, 1, 1);
  gtk_grid_attach_next_to(GTK_GRID(grid), example, entry, GTK_POS_BOTTOM, 1, 1);
...
static void action_select_pattern_callback(GtkAction *action,
                                           gpointer callback_data) {
  g_assert(FM_IS_DIRECTORY_VIEW(callback_data));
...
static void action_reset_to_defaults_callback(GtkAction *action,
                                              gpointer callback_data) {
  g_assert(FM_IS_DIRECTORY_VIEW(callback_data));
...
static void hidden_files_mode_changed(CajaWindow *window,
                                      gpointer callback_data) {
  FMDirectoryView *directory_view;
...
static void backup_files_mode_changed(CajaWindow *window,
                                      gpointer callback_data) {
  FMDirectoryView *directory_view;
...
static void action_save_search_callback(GtkAction *action,
                                        gpointer callback_data) {
  FMDirectoryView *directory_view;
...
static void query_name_entry_changed_cb(GtkWidget *entry, GtkWidget *button) {
  const char *text;
  gboolean sensitive;
...
static void action_save_search_as_callback(GtkAction *action,
                                           gpointer callback_data) {
  FMDirectoryView *directory_view;
  ...
  if (directory_view->details->model &&
    ...
    gtk_container_set_border_width(GTK_CONTAINER(dialog), 5);
    gtk_box_set_spacing(
        GTK_BOX(gtk_dialog_get_content_area(GTK_DIALOG(dialog))), 2);
    gtk_window_set_resizable(GTK_WINDOW(dialog), FALSE);

    grid = gtk_grid_new();
    g_object_set(grid, "orientation", GTK_ORIENTATION_VERTICAL, "border-width",
                 5, "row-spacing", 6, "column-spacing", 12, NULL);
    ...
    g_signal_connect(entry, "changed", G_CALLBACK(query_name_entry_changed_cb),
                     save_button);
    ...
    chooser =
        gtk_file_chooser_button_new(_("Select Folder to Save Search In"),
                                    GTK_FILE_CHOOSER_ACTION_SELECT_FOLDER);
    gtk_widget_set_hexpand(chooser, TRUE);
    gtk_grid_attach_next_to(GTK_GRID(grid), chooser, label, GTK_POS_RIGHT, 1,
                            1);
    ...
    if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_OK) {
      ...
      if (g_str_has_suffix(entry_text, CAJA_SAVED_SEARCH_EXTENSION)) {
        ...
        filename_utf8 =
            g_strconcat(entry_text, CAJA_SAVED_SEARCH_EXTENSION, NULL);
      }
...
static void action_empty_trash_callback(GtkAction *action,
                                        gpointer callback_data) {
  g_assert(FM_IS_DIRECTORY_VIEW(callback_data));
...
static void action_new_folder_callback(GtkAction *action,
                                       gpointer callback_data) {
  g_assert(FM_IS_DIRECTORY_VIEW(callback_data));
...
static void action_new_empty_file_callback(GtkAction *action,
                                           gpointer callback_data) {
  g_assert(FM_IS_DIRECTORY_VIEW(callback_data));
...
static void action_new_launcher_callback(GtkAction *action,
                                         gpointer callback_data) {
  char *parent_uri;
  FMDirectoryView *view;
  GtkWindow *window;
  ...
  window = fm_directory_view_get_containing_window(view);
  caja_debug_log(FALSE, CAJA_DEBUG_LOG_DOMAIN_USER,
                 "directory view create new launcher in window=%p: %s", window,
                 parent_uri);
  caja_launch_application_from_command(
      gtk_widget_get_screen(GTK_WIDGET(view)), "mate-desktop-item-edit",
      "mate-desktop-item-edit", FALSE, "--create-new", parent_uri, NULL);

  g_free(parent_uri);
}

static void action_properties_callback(GtkAction *action,
                                       gpointer callback_data) {
  FMDirectoryView *view;
  GList *selection;
  ...
  if (g_list_length(selection) == 0) {
    if (view->details->directory_as_file != NULL) {
      ...
      files =
          g_list_append(NULL, caja_file_ref(view->details->directory_as_file));
...
static void action_location_properties_callback(GtkAction *action,
  ...
  files = g_list_append(
      NULL, caja_file_ref(view->details->location_popup_directory_as_file));
...
static gboolean all_files_in_trash(GList *files) {
  GList *node;
...
static gboolean all_selected_items_in_trash(FMDirectoryView *view) {
  GList *selection;
  gboolean result;
...
static gboolean we_are_in_vfolder_desktop_dir(FMDirectoryView *view) {
  CajaFile *file;
  char *mime_type;
...
static void text_attribute_names_changed_callback(gpointer callback_data) {
  FMDirectoryView *view;

  view = FM_DIRECTORY_VIEW(callback_data);

  EEL_CALL_METHOD(FM_DIRECTORY_VIEW_CLASS, view, text_attribute_names_changed,
                  (view));
}

static void image_display_policy_changed_callback(gpointer callback_data) {
  FMDirectoryView *view;

  view = FM_DIRECTORY_VIEW(callback_data);

  EEL_CALL_METHOD(FM_DIRECTORY_VIEW_CLASS, view, image_display_policy_changed,
                  (view));
}
...
static void click_policy_changed_callback(gpointer callback_data) {
  ...
  EEL_CALL_METHOD(FM_DIRECTORY_VIEW_CLASS, view, click_policy_changed, (view));
}
...
static void sort_directories_first_changed_callback(gpointer callback_data) {
  ...
  preference_value = g_settings_get_boolean(
      caja_preferences, CAJA_PREFERENCES_SORT_DIRECTORIES_FIRST);
...
static void set_up_scripts_directory_global(void) {
  ...
  scripts_directory_path =
      g_build_filename(g_get_user_config_dir(), "caja", "scripts", NULL);

  if (g_mkdir_with_parents(scripts_directory_path, 0755) == 0) {
    scripts_directory_uri =
        g_filename_to_uri(scripts_directory_path, NULL, NULL);
    scripts_directory_uri_length = strlen(scripts_directory_uri);

    /* Support for GNOME Nautilus scripts
     */
    char *nautilus_scripts_path =
        g_build_filename(g_get_home_dir(), ".gnome2", "nautilus-scripts", NULL);

    if (g_file_test(nautilus_scripts_path, G_FILE_TEST_IS_DIR) == TRUE) {
      char *nautilus_syslink = g_build_filename(g_get_user_config_dir(), "caja",
                                                "scripts", "nautilus", NULL);
      /* If link already exists, or also any other kind of file/dir with same
       * name, ignore it */
...
static void scripts_added_or_changed_callback(CajaDirectory *directory,
                                              GList *files,
                                              gpointer callback_data) {
  FMDirectoryView *view;
...
static void templates_added_or_changed_callback(CajaDirectory *directory,
                                                GList *files,
                                                gpointer callback_data) {
  FMDirectoryView *view;
...
static void add_directory_to_directory_list(FMDirectoryView *view,
                                            CajaDirectory *directory,
...
static void remove_directory_from_directory_list(FMDirectoryView *view,
                                                 CajaDirectory *directory,
                                                 GList **directory_list,
                                                 GCallback changed_callback) {
...
static void add_directory_to_scripts_directory_list(FMDirectoryView *view,
  ...
  add_directory_to_directory_list(
      view, directory, &view->details->scripts_directory_list,
      G_CALLBACK(scripts_added_or_changed_callback));
}

static void remove_directory_from_scripts_directory_list(
    FMDirectoryView *view, CajaDirectory *directory) {
  remove_directory_from_directory_list(
      view, directory, &view->details->scripts_directory_list,
      G_CALLBACK(scripts_added_or_changed_callback));
}

static void add_directory_to_templates_directory_list(
    FMDirectoryView *view, CajaDirectory *directory) {
  add_directory_to_directory_list(
      view, directory, &view->details->templates_directory_list,
      G_CALLBACK(templates_added_or_changed_callback));
}

static void remove_directory_from_templates_directory_list(
    FMDirectoryView *view, CajaDirectory *directory) {
  remove_directory_from_directory_list(
      view, directory, &view->details->templates_directory_list,
      G_CALLBACK(templates_added_or_changed_callback));
...
static void slot_active(CajaWindowSlot *slot, FMDirectoryView *view) {
...
static void slot_inactive(CajaWindowSlot *slot, FMDirectoryView *view) {
  g_assert(view->details->active ||
...
static void fm_directory_view_grab_focus(CajaView *view) {
...
static void view_iface_update_menus(CajaView *view) {
  fm_directory_view_update_menus(FM_DIRECTORY_VIEW(view));
}

static GtkWidget *fm_directory_view_get_widget(CajaView *view) {
  return GTK_WIDGET(view);
}

static int fm_directory_view_get_selection_count(CajaView *view) {
...
static GList *fm_directory_view_get_selection_locations(CajaView *view) {
  GList *files;
  GList *locations;
  GList *l;
  GFile *location = NULL;
...
static GList *file_list_from_location_list(const GList *uri_list) {
  GList *file_list;
  const GList *node;

  file_list = NULL;
  for (node = uri_list; node != NULL; node = node->next) {
    file_list = g_list_prepend(file_list, caja_file_get(node->data));
  }
  return g_list_reverse(file_list);
}

static void fm_directory_view_set_selection_locations(
...
void fm_directory_view_init_view_iface(CajaViewIface *iface) {
  ...
  iface->restore_default_zoom_level =
      (gpointer)fm_directory_view_restore_default_zoom_level;
  ...
  iface->drop_proxy_received_uris =
      (gpointer)fm_directory_view_drop_proxy_received_uris;
  iface->drop_proxy_received_netscape_url =
      (gpointer)fm_directory_view_drop_proxy_received_netscape_url;
}
...
static void fm_directory_view_init(FMDirectoryView *view) {
  ...
  view->details->non_ready_files =
      g_hash_table_new_full(file_and_directory_hash, file_and_directory_equal,
                            (GDestroyNotify)file_and_directory_free, NULL);
  ...
  g_signal_connect_object(caja_signaller_get_current(), "user_dirs_changed",
                          G_CALLBACK(user_dirs_changed), view,
                          G_CONNECT_SWAPPED);

  view->details->sort_directories_first = g_settings_get_boolean(
      caja_preferences, CAJA_PREFERENCES_SORT_DIRECTORIES_FIRST);

  g_signal_connect_object(
      caja_trash_monitor_get(), "trash_state_changed",
  ...
  g_signal_connect_object(caja_signaller_get_current(), "popup_menu_changed",
                          G_CALLBACK(fm_directory_view_update_menus), view,
                          G_CONNECT_SWAPPED);
  ...
  g_signal_connect_swapped(
      caja_preferences, "changed::" CAJA_PREFERENCES_SHOW_IMAGE_FILE_THUMBNAILS,
...
static void real_unmerge_menus(FMDirectoryView *view) {
  ...
  caja_ui_unmerge_ui(ui_manager, &view->details->dir_merge_id,
                     &view->details->dir_action_group);
  caja_ui_unmerge_ui(ui_manager, &view->details->extensions_menu_merge_id,
                     &view->details->extensions_menu_action_group);
  caja_ui_unmerge_ui(ui_manager, &view->details->open_with_merge_id,
                     &view->details->open_with_action_group);
  caja_ui_unmerge_ui(ui_manager, &view->details->scripts_merge_id,
                     &view->details->scripts_action_group);
  caja_ui_unmerge_ui(ui_manager, &view->details->templates_merge_id,
                     &view->details->templates_action_group);
}

static void fm_directory_view_destroy(GtkWidget *object) {
  FMDirectoryView *view;
  GList *node, *next;
  ...
  for (node = view->details->scripts_directory_list; node != NULL;
       node = next) {
    next = node->next;
    remove_directory_from_scripts_directory_list(view, node->data);
  }

  for (node = view->details->templates_directory_list; node != NULL;
       node = next) {
    next = node->next;
  ...
  while (view->details->subdirectory_list != NULL) {
    fm_directory_view_remove_subdirectory(
        view, view->details->subdirectory_list->data);
  }
...
static void fm_directory_view_finalize(GObject *object) {
  FMDirectoryView *view;
  ...
  g_signal_handlers_disconnect_by_func(
      caja_icon_view_preferences, text_attribute_names_changed_callback, view);
  g_signal_handlers_disconnect_by_func(
      caja_preferences, image_display_policy_changed_callback, view);
  g_signal_handlers_disconnect_by_func(caja_preferences,
                                       click_policy_changed_callback, view);
  g_signal_handlers_disconnect_by_func(
      caja_preferences, sort_directories_first_changed_callback, view);
...
void fm_directory_view_display_selection_info(FMDirectoryView *view) {
  ...
  if (non_folder_count != 0) {
    ...
    if (folder_count == 0) {
      if (non_folder_count == 1) {
        items_string = g_strdup_printf(_("\"%s\" selected"), first_item_name);
      } else {
        items_string =
    ...
    if (non_folder_size_known) {
      ...
      if (g_settings_get_boolean(caja_preferences,
        ...
        size_string = g_format_size_full((guint64)non_folder_size,
       ...
       */
      non_folder_str = g_strdup_printf(_("%s (%s)"), items_string, size_string);
  ...
  free_space_str =
      caja_file_get_volume_free_space(view->details->directory_as_file);
  if (free_space_str != NULL) {
    obj_selected_free_space_str =
        g_strdup_printf(_("Free space: %s"), free_space_str);
  }
  if (folder_count == 0 && non_folder_count == 0) {
    ...
    item_count_str = g_strdup_printf(
        ngettext("%'u item", "%'u items", item_count), item_count);

    if (free_space_str != NULL) {
      status_string = g_strdup_printf(_("%s, Free space: %s"), item_count_str,
                                      free_space_str);
    ...
    if (free_space_str == NULL) {
       ...
       * be shown.
       */
      status_string = g_strdup_printf(_("%s, %s"), non_folder_str,
                                      obj_selected_free_space_str);
    }
    ...
    if (free_space_str == NULL) {
       ...
       */
      status_string =
          g_strdup_printf("%s%s", folder_count_str, folder_item_count_str);
    } else {
      /* Marking this for translation, since you
       * might want to change "," to something else.
       * After the comma the amount of free space will
       ...
       */
      status_string =
          g_strdup_printf(_("%s%s, %s"), folder_count_str,
                          folder_item_count_str, obj_selected_free_space_str);
    }
  } else {
    if (obj_selected_free_space_str == NULL) {
       ...
       */
      status_string = g_strdup_printf(_("%s%s, %s"), folder_count_str,
                                      folder_item_count_str, non_folder_str);
    } else {
      /* This is marked for translation in case a localizer
       ...
       */
      status_string = g_strdup_printf(_("%s%s, %s, %s"), folder_count_str,
                                      folder_item_count_str, non_folder_str,
                                      obj_selected_free_space_str);
    }
  }
  ...
  caja_window_slot_info_set_status(view->details->slot, status_string);
...
static void done_loading(FMDirectoryView *view, gboolean all_files_seen) {
  if (!view->details->loading) {
    return;
  }
  ...
  if (view->details->window != NULL) {
    ...
    if (all_files_seen) {
      caja_window_info_report_load_complete(view->details->window,
                                            CAJA_VIEW(view));
    }
 ...
 */
static void debuting_files_add_file_callback(FMDirectoryView *view,
                                             CajaFile *new_file,
                                             CajaDirectory *directory,
  ...
  if (g_hash_table_remove(data->debuting_files, location)) {
    ...
    if (g_hash_table_size(data->debuting_files) == 0) {
      ...
      fm_directory_view_reveal_selection(view);
      g_signal_handlers_disconnect_by_func(
          view, G_CALLBACK(debuting_files_add_file_callback), data);
    }
  }
...
static void pre_copy_move_add_file_callback(FMDirectoryView *view,
                                            CajaFile *new_file,
                                            CajaDirectory *directory,
 ...
 */
static CopyMoveDoneData *pre_copy_move(FMDirectoryView *directory_view) {
 ...
 */
static gboolean copy_move_done_partition_func(gpointer data,
                                              gpointer callback_data) {
  GFile *location;
  gboolean result;
...
static gboolean remove_not_really_moved_files(gpointer key, gpointer value,
  ...
  *added_files = g_list_prepend(*added_files, caja_file_get(loc));
  return TRUE;
}
 ...
 */
static void copy_move_done_callback(GHashTable *debuting_files, gpointer data) {
  FMDirectoryView *directory_view;
  CopyMoveDoneData *copy_move_done_data;
  ...
  if (directory_view != NULL) {
    ...
    debuting_files_data->added_files = eel_g_list_partition(
        copy_move_done_data->added_files, copy_move_done_partition_func,
        debuting_files, &copy_move_done_data->added_files);

    /* We're passed the same data used by pre_copy_move_add_file_callback, so
     * disconnecting it will free data. We've already siphoned off the
     * added_files we need, and stashed the directory_view pointer.
     */
    g_signal_handlers_disconnect_by_func(
        directory_view, G_CALLBACK(pre_copy_move_add_file_callback), data);
    ...
    if (g_hash_table_size(debuting_files) == 0) {
      ...
      /* We need to run after the default handler adds the folder we want to
       * operate on. The ADD_FILE signal is registered as G_SIGNAL_RUN_LAST, so
       * we must use connect_after.
       */
      g_signal_connect_data(
          directory_view, "add_file",
          G_CALLBACK(debuting_files_add_file_callback), debuting_files_data,
...
static gboolean real_file_still_belongs(FMDirectoryView *view, CajaFile *file,
                                        CajaDirectory *directory) {
...
static gboolean still_should_show_file(FMDirectoryView *view, CajaFile *file,
                                       CajaDirectory *directory) {
  return fm_directory_view_should_show_file(view, file) &&
         EEL_INVOKE_METHOD(FM_DIRECTORY_VIEW_CLASS, view, file_still_belongs,
                           (view, file, directory));
}

static gboolean ready_to_load(CajaFile *file) {
  return caja_file_check_if_ready(file, CAJA_FILE_ATTRIBUTES_FOR_ICON);
}
...
static void sort_files(FMDirectoryView *view, GList **list) {
 ...
 */
static void process_new_files(FMDirectoryView *view) {
  GList *new_added_files, *new_changed_files, *old_added_files,
      *old_changed_files;
  GHashTable *non_ready_files;
  GList *node, *next;
...
static void process_old_files(FMDirectoryView *view) {
  ...
  if (files_added != NULL || files_changed != NULL) {
    ...
    for (node = files_added; node != NULL; node = node->next) {
      pending = node->data;
      g_signal_emit(view, signals[ADD_FILE], 0, pending->file,
                    pending->directory);
    }
    ...
    for (node = files_changed; node != NULL; node = node->next) {
      pending = node->data;
      g_signal_emit(view,
                    signals[still_should_show_file(view, pending->file,
                                                   pending->directory)
                                ? FILE_CHANGED
                                : REMOVE_FILE],
    ...
    if (files_changed != NULL) {
      ...
      send_selection_change =
          eel_g_lists_sort_and_check_for_intersection(&files, &selection);
      caja_file_list_free(files);
      caja_file_list_free(selection);
    }
...
static void display_pending_files(FMDirectoryView *view) {
...
static void update_menus_if_pending(FMDirectoryView *view) {
  if (!view->details->menu_states_untrustworthy) {
    return;
  }
...
static gboolean update_menus_timeout_callback(gpointer data) {
  FMDirectoryView *view;
...
static void schedule_idle_display_of_pending_files(FMDirectoryView *view) {
  ...
  view->details->display_pending_source_id = g_idle_add_full(
...
static void queue_pending_files(FMDirectoryView *view, CajaDirectory *directory,
                                GList *files, GList **pending_list) {
  if (files == NULL) {
    return;
  }
  ...
  *pending_list = g_list_concat(
      file_and_directory_list_from_files(directory, files), *pending_list);
...
static void remove_changes_timeout_callback(FMDirectoryView *view) {
...
static void schedule_changes(FMDirectoryView *view) {
   ...
   */
  if (view->details->changes_timeout_id != 0 || view->details->loading) {
    return;
  }

  view->details->changes_timeout_id = g_timeout_add(
      UPDATE_INTERVAL_TIMEOUT_INTERVAL, changes_timeout_callback, view);
}
...
static void files_added_callback(CajaDirectory *directory, GList *files,
                                 gpointer callback_data) {
  FMDirectoryView *view;
  GtkWindow *window;
  char *uri;
  ...
  caja_debug_log_with_file_list(FALSE, CAJA_DEBUG_LOG_DOMAIN_ASYNC, files,
                                "files added in window %p: %s", window,
...
static void files_changed_callback(CajaDirectory *directory, GList *files,
                                   gpointer callback_data) {
  FMDirectoryView *view;
  GtkWindow *window;
  char *uri;
  ...
  uri = fm_directory_view_get_uri(view);
  caja_debug_log_with_file_list(FALSE, CAJA_DEBUG_LOG_DOMAIN_ASYNC, files,
                                "files changed in window %p: %s", window,
                                uri ? uri : "(no directory)");
  g_free(uri);

  schedule_changes(view);

  queue_pending_files(view, directory, files,
                      &view->details->new_changed_files);
...
static void done_loading_callback(CajaDirectory *directory,
                                  gpointer callback_data) {
  FMDirectoryView *view;
...
static void load_error_callback(CajaDirectory *directory, GError *error,
                                gpointer callback_data) {
  FMDirectoryView *view;
   ...
   */
  g_signal_emit(view, signals[LOAD_ERROR], 0, error);
}

static void real_load_error(FMDirectoryView *view, GError *error) {
  ...
  if (!view->details->reported_load_error) {
    fm_report_error_loading_directory(
        fm_directory_view_get_directory_as_file(view), error,
        fm_directory_view_get_containing_window(view));
  }
...
void fm_directory_view_add_subdirectory(FMDirectoryView *view,
  ...
  attributes = CAJA_FILE_ATTRIBUTES_FOR_ICON |
               CAJA_FILE_ATTRIBUTE_DIRECTORY_ITEM_COUNT |
               CAJA_FILE_ATTRIBUTE_INFO | CAJA_FILE_ATTRIBUTE_LINK_INFO |
               CAJA_FILE_ATTRIBUTE_MOUNT | CAJA_FILE_ATTRIBUTE_EXTENSION_INFO;

  caja_directory_file_monitor_add(directory, &view->details->model,
                                  view->details->show_hidden_files, attributes,
                                  files_added_callback, view);

  g_signal_connect(directory, "files_added", G_CALLBACK(files_added_callback),
                   view);
  g_signal_connect(directory, "files_changed",
                   G_CALLBACK(files_changed_callback), view);

  view->details->subdirectory_list =
      g_list_prepend(view->details->subdirectory_list, directory);
}
...
void fm_directory_view_remove_subdirectory(FMDirectoryView *view,
  ...
  view->details->subdirectory_list =
      g_list_remove(view->details->subdirectory_list, directory);
...
void fm_directory_view_bump_zoom_level(FMDirectoryView *view,
  ...
  EEL_CALL_METHOD(FM_DIRECTORY_VIEW_CLASS, view, bump_zoom_level,
                  (view, zoom_increment));
}
...
void fm_directory_view_zoom_to_level(FMDirectoryView *view,
  ...
  EEL_CALL_METHOD(FM_DIRECTORY_VIEW_CLASS, view, zoom_to_level,
                  (view, zoom_level));
}
...
CajaZoomLevel fm_directory_view_get_zoom_level(FMDirectoryView *view) {
  ...
  return EEL_CALL_METHOD_WITH_RETURN_VALUE(FM_DIRECTORY_VIEW_CLASS, view,
                                           get_zoom_level, (view));
}
...
void fm_directory_view_restore_default_zoom_level(FMDirectoryView *view) {
  ...
  EEL_CALL_METHOD(FM_DIRECTORY_VIEW_CLASS, view, restore_default_zoom_level,
                  (view));
}
 ...
 *
 **/
static gboolean fm_directory_view_can_rename_file(FMDirectoryView *view,
                                                  CajaFile *file) {
  return EEL_CALL_METHOD_WITH_RETURN_VALUE(FM_DIRECTORY_VIEW_CLASS, view,
...
EelBackground *fm_directory_view_get_background(FMDirectoryView *view) {
  return eel_get_widget_background(
      fm_directory_view_get_background_widget(view));
}

static void real_set_is_active(FMDirectoryView *view, gboolean is_active) {
  EelBackground *bg;
...
static void fm_directory_view_set_is_active(FMDirectoryView *view,
  ...
  EEL_CALL_METHOD(FM_DIRECTORY_VIEW_CLASS, view, set_is_active,
                  (view, is_active));
}
...
void fm_directory_view_invert_selection(FMDirectoryView *view) {
  ...
  EEL_CALL_METHOD(FM_DIRECTORY_VIEW_CLASS, view, invert_selection, (view));
}

GList *fm_directory_view_get_selection_for_file_transfer(
    FMDirectoryView *view) {
  g_return_val_if_fail(FM_IS_DIRECTORY_VIEW(view), NULL);

  return EEL_CALL_METHOD_WITH_RETURN_VALUE(
      FM_DIRECTORY_VIEW_CLASS, view, get_selection_for_file_transfer, (view));
}
...
static void prepend_uri_one(gpointer data, gpointer callback_data) {
  CajaFile *file;
  GList **result;
...
static void offset_drop_points(GArray *relative_item_points, int x_offset,
                               int y_offset) {
  guint index;
...
static void fm_directory_view_create_links_for_files(
    FMDirectoryView *view, GList *files, GArray *relative_item_points) {
  GList *uris;
  char *dir_uri;
  CopyMoveDoneData *copy_move_done_data;
  ...
  caja_file_operations_copy_move(uris, relative_item_points, dir_uri,
                                 GDK_ACTION_LINK, GTK_WIDGET(view),
                                 copy_move_done_callback, copy_move_done_data);
  g_free(dir_uri);
  g_list_free_full(uris, g_free);
}

static void fm_directory_view_duplicate_selection(
    FMDirectoryView *view, GList *files, GArray *relative_item_points) {
  GList *uris;
  CopyMoveDoneData *copy_move_done_data;
  ...
  copy_move_done_data = pre_copy_move(view);
  caja_file_operations_copy_move(uris, relative_item_points, NULL,
                                 GDK_ACTION_COPY, GTK_WIDGET(view),
                                 copy_move_done_callback, copy_move_done_data);
  g_list_free_full(uris, g_free);
}
 ...
 *	 CAJA_DESKTOP_LINK_TRASH, CAJA_DESKTOP_LINK_HOME,
 *CAJA_DESKTOP_LINK_MOUNT
 */

static gboolean special_link_in_selection(FMDirectoryView *view) {
  gboolean saw_link;
...
static gboolean desktop_or_home_dir_in_selection(FMDirectoryView *view) {
  ...
  for (node = selection; node != NULL; node = node->next) {
    ...
    saw_desktop_or_home_dir =
        caja_file_is_home(file) || caja_file_is_desktop_directory(file);
...
static void trash_or_delete_files(GtkWindow *parent_window, const GList *files,
  ...
  caja_file_operations_trash_or_delete(
      locations, parent_window, (CajaDeleteCallback)trash_or_delete_done_cb,
      view);
  g_list_free_full(locations, g_object_unref);
}

static gboolean can_rename_file(FMDirectoryView *view, CajaFile *file) {
  return caja_file_can_rename(file);
}

static void start_renaming_file(FMDirectoryView *view, CajaFile *file,
                                gboolean select_all) {
  if (file != NULL) {
...
static gboolean delayed_rename_file_hack_callback(RenameData *data) {
  FMDirectoryView *view;
  CajaFile *new_file;
...
static void delayed_rename_file_hack_removed(RenameData *data) {
  g_object_unref(data->view);
  caja_file_unref(data->new_file);
  g_free(data);
}

static void rename_file(FMDirectoryView *view, CajaFile *new_file) {
  /* HACK!!!!
  ...
  */
  if (FM_IS_LIST_VIEW(view)) {
    ...
    view->details->delayed_rename_file_id = g_timeout_add_full(
        G_PRIORITY_DEFAULT, 100, (GSourceFunc)delayed_rename_file_hack_callback,
...
static void reveal_newly_added_folder(FMDirectoryView *view, CajaFile *new_file,
                                      CajaDirectory *directory,
  ...
  if (g_file_equal(location, target_location)) {
    g_signal_handlers_disconnect_by_func(
        view, G_CALLBACK(reveal_newly_added_folder), (void *)target_location);
...
static void track_newly_added_locations(FMDirectoryView *view,
                                        CajaFile *new_file,
                                        CajaDirectory *directory,
                                        gpointer user_data) {
  NewFolderData *data;
  ...
  g_hash_table_insert(data->added_locations, caja_file_get_location(new_file),
                      NULL);
}

static void new_folder_done(GFile *new_folder, gpointer user_data) {
  FMDirectoryView *directory_view;
  CajaFile *file;
  ...
  g_signal_handlers_disconnect_by_func(
      directory_view, G_CALLBACK(track_newly_added_locations), (void *)data);
  ...
  caja_file_set_metadata(file, CAJA_METADATA_KEY_SCREEN, NULL, screen_string);

  if (g_hash_table_lookup_extended(data->added_locations, new_folder, NULL,
                                   NULL)) {
...
static NewFolderData *new_folder_data_new(FMDirectoryView *directory_view) {
  NewFolderData *data;
...
static GdkPoint *context_menu_to_file_operation_position(
    FMDirectoryView *directory_view) {
  ...
  if (fm_directory_view_using_manual_layout(directory_view) &&
    ...
    EEL_CALL_METHOD(
        FM_DIRECTORY_VIEW_CLASS, directory_view,
...
static void update_context_menu_position_from_event(FMDirectoryView *view,
...
void fm_directory_view_new_folder(FMDirectoryView *directory_view) {
  char *parent_uri;
  NewFolderData *data;
  GdkPoint *pos;
  ...
  caja_file_operations_new_folder(GTK_WIDGET(directory_view), pos, parent_uri,
                                  new_folder_done, data);

  g_free(parent_uri);
}

static NewFolderData *setup_new_folder_data(FMDirectoryView *directory_view) {
  NewFolderData *data;
...
static void fm_directory_view_new_file_with_initial_contents(
    FMDirectoryView *directory_view, const char *parent_uri,
    const char *filename, const char *initial_contents, int length,
    GdkPoint *pos) {
  NewFolderData *data;
  ...
  caja_file_operations_new_file(GTK_WIDGET(directory_view), pos, parent_uri,
                                filename, initial_contents, length,
                                new_folder_done, data);
}

void fm_directory_view_new_file(FMDirectoryView *directory_view,
                                const char *parent_uri, CajaFile *source) {
  GdkPoint *pos;
  NewFolderData *data;
  char *source_uri;
  char *container_uri;
  ...
  caja_file_operations_new_file_from_template(
      GTK_WIDGET(directory_view), pos,
...
static void open_one_in_new_window(gpointer data, gpointer callback_data) {
  ...
  fm_directory_view_activate_file(FM_DIRECTORY_VIEW(callback_data),
                                  CAJA_FILE(data),
                                  CAJA_WINDOW_OPEN_IN_NAVIGATION, 0);
}
...
static void open_one_in_folder_window(gpointer data, gpointer callback_data) {
  ...
  fm_directory_view_activate_file(FM_DIRECTORY_VIEW(callback_data),
                                  CAJA_FILE(data), CAJA_WINDOW_OPEN_IN_SPATIAL,
                                  0);
}
...
static char *escape_action_name(const char *action_name, const char *prefix) {
  GString *s;
...
static char *escape_action_path(const char *action_path) {
  GString *s;
...
static void add_submenu(GtkUIManager *ui_manager, GtkActionGroup *action_group,
                        guint merge_id, const char *parent_path,
                        const char *uri, const char *label,
                        cairo_surface_t *surface, gboolean add_action) {
  if (parent_path != NULL) {
    char *escaped_label;
    char *action_name;
    char *submenu_name;
    ...
    if (add_action) {
      ...
      G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
      action = gtk_action_new(action_name, escaped_label, NULL, NULL);
      G_GNUC_END_IGNORE_DEPRECATIONS;
      if (surface != NULL) {
    ...
    gtk_ui_manager_add_ui(ui_manager, merge_id, parent_path,
                          escaped_submenu_name, action_name,
...
static void add_application_to_open_with_menu(FMDirectoryView *view,
                                              GAppInfo *application,
                                              GList *files, int index,
  ...
  launch_parameters =
      application_launch_parameters_new(application, files, view);
  escaped_app =
      eel_str_double_underscores(g_app_info_get_display_name(application));
  if (submenu)
  ...
  tip = g_strdup_printf(
      ngettext("Use \"%s\" to open the selected item",
  ...
  g_signal_connect_data(
      action, "activate", G_CALLBACK(open_with_launch_application_callback),
      launch_parameters, (GClosureNotify)application_launch_parameters_free, 0);

  G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
  gtk_action_group_add_action(view->details->open_with_action_group, action);
  G_GNUC_END_IGNORE_DEPRECATIONS;
  g_object_unref(action);

  gtk_ui_manager_add_ui(caja_window_info_get_ui_manager(view->details->window),
                        view->details->open_with_merge_id, menu_placeholder,
                        action_name, action_name, GTK_UI_MANAGER_MENUITEM,
                        FALSE);

  path = g_strdup_printf("%s/%s", menu_placeholder, action_name);
  menuitem = gtk_ui_manager_get_widget(
      caja_window_info_get_ui_manager(view->details->window), path);
  gtk_image_menu_item_set_always_show_image(GTK_IMAGE_MENU_ITEM(menuitem),
                                            TRUE);
  g_free(path);

  gtk_ui_manager_add_ui(caja_window_info_get_ui_manager(view->details->window),
                        view->details->open_with_merge_id, popup_placeholder,
                        action_name, action_name, GTK_UI_MANAGER_MENUITEM,
                        FALSE);
...
static void add_parent_folder_to_open_menu(FMDirectoryView *view, GList *files,
                                           const char *menu_placeholder,
                                           const char *popup_placeholder) {
  CajaFile *file;
  gchar *uri;
  ...
  if (caja_file_is_directory(file)) return;
  ...
  g_signal_connect_data(action, "activate",
                        G_CALLBACK(open_parent_folder_callback), uri,
...
static void get_x_content_async_callback(char **content, gpointer user_data) {
  FMDirectoryView *view;
...
static void add_x_content_apps(FMDirectoryView *view, CajaFile *file,
                               GList **applications) {
  GMount *mount;
  ...
  if (x_content_types != NULL) {
    ...
    caja_autorun_get_x_content_types_for_mount_async(
        mount, get_x_content_async_callback, NULL, g_object_ref(view));
  }

  g_object_unref(mount);
}

static void reset_open_with_menu(FMDirectoryView *view, GList *selection) {
  ...
  /* Clear any previous inserted items in the applications and viewers
   * placeholders */
  ...
  caja_ui_prepare_merge_ui(ui_manager, "OpenWithGroup",
  ...
  for (node = applications, index = 0; node != NULL;
    ...
    gtk_ui_manager_add_ui(
        ...
        view->details->open_with_merge_id, menu_path, "separator", NULL,
        GTK_UI_MANAGER_SEPARATOR, FALSE);

    add_application_to_open_with_menu(view, node->data, selection, index,
  ...
  if (eel_uri_is_search(uri) && g_list_length(selection) == 1)
    add_parent_folder_to_open_menu(view, selection,
                                   FM_DIRECTORY_VIEW_MENU_PATH_OPEN,
                                   FM_DIRECTORY_VIEW_POPUP_PATH_OPEN);

  g_free(uri);

  open_with_chooser_visible =
      other_applications_visible && g_list_length(selection) == 1;
...
static GList *get_all_extension_menu_items(GtkWidget *window,
                                           GList *selection) {
  GList *items;
  GList *providers;
  GList *l;
  ...
  for (l = providers; l != NULL; l = l->next) {
    ...
    file_items = caja_menu_provider_get_file_items(provider, window, selection);
...
static void extension_action_callback_data_free(
...
static gboolean search_in_menu_items(GList *items, const char *item_name) {
  GList *list;
...
static void extension_action_callback(GtkAction *action,
                                      gpointer callback_data) {
  ExtensionActionCallbackData *data;
  char *item_name;
  gboolean is_valid;
  ...
  items = get_all_extension_menu_items(
      gtk_widget_get_toplevel(GTK_WIDGET(data->view)), data->selection);
...
static cairo_surface_t *get_menu_icon(const char *icon_name,
                                      GtkWidget *widget) {
  CajaIconInfo *info;
...
static cairo_surface_t *get_menu_icon_for_file(CajaFile *file,
                                               GtkWidget *widget) {
  CajaIconInfo *info;
  cairo_surface_t *surface;
  int size, scale;
...
static GtkAction *add_extension_action_for_files(FMDirectoryView *view,
                                                 CajaMenuItem *item,
                                                 GList *files) {
  ...
  g_object_get(G_OBJECT(item), "name", &name, "label", &label, "tip", &tip,
               "icon", &icon, "sensitive", &sensitive, "priority", &priority,
               NULL);

  G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
  action = gtk_action_new(name, label, tip, icon);
  G_GNUC_END_IGNORE_DEPRECATIONS;
  ...
  if (icon != NULL) {
    ...
    if (surface != NULL) {
      g_object_set_data_full(G_OBJECT(action), "menu-icon", surface,
                             (GDestroyNotify)cairo_surface_destroy);
    }
  }
  ...
  g_signal_connect_data(action, "activate",
                        G_CALLBACK(extension_action_callback), data,
...
static void add_extension_menu_items(FMDirectoryView *view, GList *files,
                                     GList *menu_items,
  ...
  for (l = menu_items; l; l = l->next) {
    ...
    path = g_build_path("/", FM_DIRECTORY_VIEW_POPUP_PATH_EXTENSION_ACTIONS,
                        subdirectory, NULL);
    G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
    ...
    gtk_ui_manager_add_ui(
        ui_manager, view->details->extensions_menu_merge_id, path, action_name,
        action_name,
    ...
    if (menu != NULL) {
      ...
      G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
      subdir =
          g_build_path("/", subdirectory, gtk_action_get_name(action), NULL);
      G_GNUC_END_IGNORE_DEPRECATIONS;
      add_extension_menu_items(view, files, children, subdir);
...
static void reset_extension_actions_menu(FMDirectoryView *view,
                                         GList *selection) {
  GList *items;
  GtkUIManager *ui_manager;
  ...
  caja_ui_prepare_merge_ui(ui_manager, "DirExtensionsMenuGroup",
                           &view->details->extensions_menu_merge_id,
                           &view->details->extensions_menu_action_group);

  items = get_all_extension_menu_items(
      gtk_widget_get_toplevel(GTK_WIDGET(view)), selection);
  if (items != NULL) {
...
static char *change_to_view_directory(FMDirectoryView *view) {
  char *path;
  char *old_path;
...
static char *get_file_paths_or_uris_as_newline_delimited_string(
    GList *selection, gboolean get_paths) {
  char *path;
  char *result;
  ...
  for (node = selection; node != NULL; node = node->next) {
    ...
    if (CAJA_IS_DESKTOP_ICON_FILE(node->data)) {
      link =
          caja_desktop_icon_file_get_link(CAJA_DESKTOP_ICON_FILE(node->data));
      if (link != NULL) {
...
static void get_strings_for_environment_variables(FMDirectoryView *view,
  ...
  if (eel_str_has_prefix(directory_uri, "file:") ||
      eel_uri_is_desktop(directory_uri) || eel_uri_is_trash(directory_uri)) {
...
static FMDirectoryView *get_directory_view_of_extra_pane(
    FMDirectoryView *view) {
  CajaWindowSlotInfo *slot;

  slot =
      caja_window_info_get_extra_slot(fm_directory_view_get_caja_window(view));
  if (slot != NULL) {
    CajaView *next_view;
 ...
 */
static void set_script_environment_variables(FMDirectoryView *view,
                                             GList *selected_files) {
  char *file_paths;
  char *uris;
  char *uri;
  ...
  get_strings_for_environment_variables(view, selected_files, &file_paths,
                                        &uris, &uri);

  g_setenv("CAJA_SCRIPT_SELECTED_FILE_PATHS", file_paths, TRUE);
  g_setenv("NAUTILUS_SCRIPT_SELECTED_FILE_PATHS", file_paths,
           TRUE);  // compatibilidad GNOME

  g_free(file_paths);

  g_setenv("CAJA_SCRIPT_SELECTED_URIS", uris, TRUE);
  g_setenv("NAUTILUS_SCRIPT_SELECTED_URIS", uris,
           TRUE);  // compatibilidad GNOME
  ...
  g_setenv("NAUTILUS_SCRIPT_WINDOW_GEOMETRY", geometry_string,
           TRUE);  // compatibilidad GNOME
  ...
  if (next_view) {
    GList *next_pane_selected_files =
        fm_directory_view_get_selection(next_view);
  ...
  g_setenv("NAUTILUS_SCRIPT_NEXT_PANE_SELECTED_FILE_PATHS", file_paths,
           TRUE);  // compatibilidad GNOME
  g_free(file_paths);

  g_setenv("CAJA_SCRIPT_NEXT_PANE_SELECTED_URIS", uris, TRUE);
  g_setenv("NAUTILUS_SCRIPT_NEXT_PANE_SELECTED_URIS", uris,
           TRUE);  // compatibilidad GNOME
  g_free(uris);

  g_setenv("CAJA_SCRIPT_NEXT_PANE_CURRENT_URI", uri, TRUE);
  g_setenv("NAUTILUS_SCRIPT_NEXT_PANE_CURRENT_URI", uri,
           TRUE);  // compatibilidad GNOME
  g_free(uri);
}
...
static void run_script_callback(GtkAction *action, gpointer callback_data) {
  ...
  selected_files =
      fm_directory_view_get_selection(launch_parameters->directory_view);
  set_script_environment_variables(launch_parameters->directory_view,
                                   selected_files);

  parameters = get_file_names_as_parameter_array(
      selected_files, launch_parameters->directory_view->details->model);
  ...
  window = fm_directory_view_get_containing_window(
      launch_parameters->directory_view);
  caja_debug_log(FALSE, CAJA_DEBUG_LOG_DOMAIN_USER,
                 "directory view run_script_callback, window=%p, name=\"%s\", "
                 "script_path=\"%s\" (omitting script parameters)",
...
static void add_script_to_scripts_menus(FMDirectoryView *directory_view,
                                        CajaFile *file, const char *menu_path,
  ...
  G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
  action = gtk_action_new(action_name, escaped_label, tip, NULL);
  G_GNUC_END_IGNORE_DEPRECATIONS;
  ...
  g_signal_connect_data(action, "activate", G_CALLBACK(run_script_callback),
                        launch_parameters,
                        (GClosureNotify)script_launch_parameters_free, 0);

  G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
  gtk_action_group_add_action_with_accel(
      directory_view->details->scripts_action_group, action, NULL);
  G_GNUC_END_IGNORE_DEPRECATIONS;
  g_object_unref(action);

  ui_manager = caja_window_info_get_ui_manager(directory_view->details->window);

  gtk_ui_manager_add_ui(ui_manager, directory_view->details->scripts_merge_id,
                        menu_path, action_name, action_name,
                        GTK_UI_MANAGER_MENUITEM, FALSE);
  gtk_ui_manager_add_ui(ui_manager, directory_view->details->scripts_merge_id,
                        popup_path, action_name, action_name,
                        GTK_UI_MANAGER_MENUITEM, FALSE);
  gtk_ui_manager_add_ui(ui_manager, directory_view->details->scripts_merge_id,
                        popup_bg_path, action_name, action_name,
                        GTK_UI_MANAGER_MENUITEM, FALSE);
...
static void add_submenu_to_directory_menus(FMDirectoryView *directory_view,
                                           GtkActionGroup *action_group,
                                           guint merge_id, CajaFile *file,
  ...
  name = caja_file_get_display_name(file);
  surface = get_menu_icon_for_file(file, GTK_WIDGET(directory_view));
  add_submenu(ui_manager, action_group, merge_id, menu_path, uri, name, surface,
              TRUE);
  add_submenu(ui_manager, action_group, merge_id, popup_path, uri, name,
              surface, FALSE);
  add_submenu(ui_manager, action_group, merge_id, popup_bg_path, uri, name,
              surface, FALSE);
  if (surface) {
...
static gboolean update_directory_in_scripts_menu(FMDirectoryView *view,
                                                 CajaDirectory *directory) {
  ...
  popup_path = g_strconcat(FM_DIRECTORY_VIEW_POPUP_PATH_SCRIPTS_PLACEHOLDER,
                           escaped_path, NULL);
  popup_bg_path =
      g_strconcat(FM_DIRECTORY_VIEW_POPUP_PATH_BACKGROUND_SCRIPTS_PLACEHOLDER,
                  escaped_path, NULL);
  g_free(escaped_path);
  ...
  for (node = file_list; node != NULL; node = node->next) {
    ...
    if (caja_file_is_launchable(file)) {
      add_script_to_scripts_menus(view, file, menu_path, popup_path,
                                  popup_bg_path);
      any_scripts = TRUE;
      ...
      if (directory_belongs_in_scripts_menu(uri)) {
        ...
        add_submenu_to_directory_menus(view,
                                       view->details->scripts_action_group,
                                       view->details->scripts_merge_id, file,
                                       menu_path, popup_path, popup_bg_path);
...
static void update_scripts_menu(FMDirectoryView *view) {
  gboolean any_scripts;
  ...
  caja_ui_prepare_merge_ui(ui_manager, "ScriptsGroup",
...
static void create_template_callback(GtkAction *action,
                                     gpointer callback_data) {
  CreateTemplateParameters *parameters;

  parameters = callback_data;

  fm_directory_view_new_file(parameters->directory_view, NULL,
                             parameters->file);
}

static void add_template_to_templates_menus(FMDirectoryView *directory_view,
                                            CajaFile *file,
                                            const char *menu_path,
                                            const char *popup_bg_path) {
  ...
  g_signal_connect_data(action, "activate",
                        G_CALLBACK(create_template_callback), parameters,
  ...
  gtk_ui_manager_add_ui(ui_manager, directory_view->details->templates_merge_id,
                        menu_path, action_name, action_name,
                        GTK_UI_MANAGER_MENUITEM, FALSE);
...
static gboolean directory_belongs_in_templates_menu(
...
static gboolean update_directory_in_templates_menu(
    FMDirectoryView *view, const char *templates_directory_uri,
    CajaDirectory *directory) {
  ...
  menu_path = g_strconcat(FM_DIRECTORY_VIEW_MENU_PATH_NEW_DOCUMENTS_PLACEHOLDER,
                          escaped_path, NULL);
  popup_bg_path = g_strconcat(
      FM_DIRECTORY_VIEW_POPUP_PATH_BACKGROUND_NEW_DOCUMENTS_PLACEHOLDER,
      escaped_path, NULL);
  g_free(escaped_path);
  ...
  num = 0;
  any_templates = FALSE;
  for (node = file_list; num < TEMPLATE_LIMIT && node != NULL;
       node = node->next, num++) {
    file = node->data;
    ...
    if (caja_file_is_directory(file)) {
      ...
      if (directory_belongs_in_templates_menu(templates_directory_uri, uri)) {
        ...
        add_submenu_to_directory_menus(view,
                                       view->details->templates_action_group,
                                       view->details->templates_merge_id, file,
                                       menu_path, NULL, popup_bg_path);
...
static void update_templates_menu(FMDirectoryView *view) {
  gboolean any_templates;
  GList *sorted_copy, *node;
  GtkUIManager *ui_manager;
  GtkAction *action;
  ...
  caja_ui_prepare_merge_ui(ui_manager, "TemplatesGroup",
  ...
  for (node = sorted_copy; node != NULL; node = node->next) {
    ...
    } else if (update_directory_in_templates_menu(view, templates_directory_uri,
                                                  directory)) {
      any_templates = TRUE;
    }
    g_free(uri);
...
static void action_open_scripts_folder_callback(GtkAction *action,
  ...
  open_location(view, scripts_directory_uri, CAJA_WINDOW_OPEN_ACCORDING_TO_MODE,
                0);
  ...
  eel_show_info_dialog_with_details(
      ...
      _("All executable files in this folder will appear in the "
        ...
        "files (only if local)\n\n"
        "CAJA_SCRIPT_SELECTED_URIS: newline-delimited URIs for selected "
        "files\n\n"
        "CAJA_SCRIPT_CURRENT_URI: URI for current location\n\n"
        "CAJA_SCRIPT_WINDOW_GEOMETRY: position and size of current window\n\n"
        "CAJA_SCRIPT_NEXT_PANE_SELECTED_FILE_PATHS: newline-delimited paths "
        "for selected files in the inactive pane of a split-view window (only "
        "if local)\n\n"
        "CAJA_SCRIPT_NEXT_PANE_SELECTED_URIS: newline-delimited URIs for "
...
static GtkMenu *create_popup_menu(FMDirectoryView *view,
  ...
  menu = gtk_ui_manager_get_widget(
      caja_window_info_get_ui_manager(view->details->window), popup_path);
...
static void copy_or_cut_files(FMDirectoryView *view, GList *clipboard_contents,
                              gboolean cut) {
  int count;
  char *status_string;
  ...
  gtk_clipboard_set_with_data(caja_clipboard_get(GTK_WIDGET(view)), targets,
                              n_targets, caja_get_clipboard_callback,
                              caja_clear_clipboard_callback, NULL);
  ...
  if (count == 1) {
    ...
    if (cut) {
      status_string =
          g_strdup_printf(ngettext("The %'d selected item will be moved "
                                   "if you select the Paste command",
      ...
      status_string =
          g_strdup_printf(ngettext("The %'d selected item will be copied "
                                   "if you select the Paste command",
...
static void move_copy_selection_to_location(FMDirectoryView *view,
  ...
  for (l = selection; l != NULL; l = l->next) {
    uris = g_list_prepend(uris, caja_file_get_uri((CajaFile *)l->data));
  }
  uris = g_list_reverse(uris);

  fm_directory_view_move_copy_items(uris, NULL, target_uri, copy_action, 0, 0,
                                    view);
...
static void move_copy_selection_to_next_pane(FMDirectoryView *view,
                                             int copy_action) {
  CajaWindowSlotInfo *slot;
  char *dest_location;
...
static void action_copy_to_next_pane_callback(GtkAction *action,
  ...
  view = FM_DIRECTORY_VIEW(callback_data);
  move_copy_selection_to_next_pane(view, GDK_ACTION_COPY);
}
...
static void action_copy_to_home_callback(GtkAction *action,
                                         gpointer callback_data) {
  FMDirectoryView *view;
  char *dest_location;
...
static void action_move_to_home_callback(GtkAction *action,
                                         gpointer callback_data) {
  FMDirectoryView *view;
  char *dest_location;
...
static void action_copy_to_desktop_callback(GtkAction *action,
                                            gpointer callback_data) {
  FMDirectoryView *view;
  char *dest_location;
...
static void action_move_to_desktop_callback(GtkAction *action,
                                            gpointer callback_data) {
  FMDirectoryView *view;
  char *dest_location;
...
static void paste_clipboard_data(FMDirectoryView *view,
  ...
  cut = FALSE;
  item_uris = caja_clipboard_get_uri_list_from_selection_data(
      selection_data, &cut, copied_files_atom);

  if (item_uris == NULL || destination_uri == NULL) {
    caja_window_slot_info_set_status(
        view->details->slot, _("There is nothing on the clipboard to paste."));
  } else {
...
static void paste_into_clipboard_received_callback(
...
static void paste_into(FMDirectoryView *view, CajaFile *target) {
  PasteIntoData *data;
...
static void real_action_undo(FMDirectoryView *view) {
  ...
  caja_undostack_manager_undo(manager, GTK_WIDGET(view),
...
static void real_action_redo(FMDirectoryView *view) {
  ...
  caja_undostack_manager_redo(manager, GTK_WIDGET(view),
                              finish_undoredo_callback);
}

static void real_action_rename(FMDirectoryView *view, gboolean select_all) {
  GList *selection;
...
static void file_mount_callback(CajaFile *file, GFile *result_location,
                                GError *error, gpointer callback_data) {
  if (error != NULL && (error->domain != G_IO_ERROR ||
                        (error->code != G_IO_ERROR_CANCELLED &&
                         error->code != G_IO_ERROR_FAILED_HANDLED &&
                         error->code != G_IO_ERROR_ALREADY_MOUNTED))) {
    eel_show_error_dialog(_("Unable to mount location"), error->message, NULL);
  }
}

static void file_unmount_callback(CajaFile *file, GFile *result_location,
                                  GError *error, gpointer callback_data) {
  FMDirectoryView *view;
  ...
  if (error != NULL && (error->domain != G_IO_ERROR ||
                        (error->code != G_IO_ERROR_CANCELLED &&
                         error->code != G_IO_ERROR_FAILED_HANDLED))) {
    eel_show_error_dialog(_("Unable to unmount location"), error->message,
                          NULL);
  }
}

static void file_eject_callback(CajaFile *file, GFile *result_location,
                                GError *error, gpointer callback_data) {
  FMDirectoryView *view;
  ...
  if (error != NULL && (error->domain != G_IO_ERROR ||
                        (error->code != G_IO_ERROR_CANCELLED &&
                         error->code != G_IO_ERROR_FAILED_HANDLED))) {
    eel_show_error_dialog(_("Unable to eject location"), error->message, NULL);
  }
}

static void file_stop_callback(CajaFile *file, GFile *result_location,
                               GError *error, gpointer callback_data) {
  if (error != NULL && (error->domain != G_IO_ERROR ||
                        (error->code != G_IO_ERROR_CANCELLED &&
                         error->code != G_IO_ERROR_FAILED_HANDLED))) {
    eel_show_error_dialog(_("Unable to stop drive"), error->message, NULL);
  }
}
...
static void action_unmount_volume_callback(GtkAction *action, gpointer data) {
  ...
  for (l = selection; l != NULL; l = l->next) {
    ...
    if (caja_file_can_unmount(file)) {
      ...
      caja_file_unmount(file, mount_op, NULL, file_unmount_callback,
                        g_object_ref(view));
      g_object_unref(mount_op);
    }
  }
...
static void action_eject_volume_callback(GtkAction *action, gpointer data) {
  GList *selection, *l;
  FMDirectoryView *view;
  CajaFile *file = NULL;
  ...
  for (l = selection; l != NULL; l = l->next) {
    ...
    if (caja_file_can_eject(file)) {
      ...
      mount_op = gtk_mount_operation_new(
          fm_directory_view_get_containing_window(view));
      fm_directory_view_set_initiated_unmount(view, TRUE);
      caja_file_eject(file, mount_op, NULL, file_eject_callback,
                      g_object_ref(view));
      g_object_unref(mount_op);
    }
  }
...
static void file_start_callback(CajaFile *file, GFile *result_location,
                                GError *error, gpointer callback_data) {
  if (error != NULL && (error->domain != G_IO_ERROR ||
                        (error->code != G_IO_ERROR_CANCELLED &&
                         error->code != G_IO_ERROR_FAILED_HANDLED &&
...
static void action_stop_volume_callback(GtkAction *action, gpointer data) {
  ...
  for (l = selection; l != NULL; l = l->next) {
    ...
    if (caja_file_can_stop(file)) {
      ...
      mount_op = gtk_mount_operation_new(
          fm_directory_view_get_containing_window(view));
      caja_file_stop(file, mount_op, NULL, file_stop_callback, NULL);
      g_object_unref(mount_op);
    }
  }
  caja_file_list_free(selection);
...
static void action_detect_media_callback(GtkAction *action, gpointer data) {
  ...
  for (l = selection; l != NULL; l = l->next) {
    ...
    if (caja_file_can_poll_for_media(file) &&
        !caja_file_is_media_check_automatic(file)) {
...
static void action_self_mount_volume_callback(GtkAction *action,
                                              gpointer data) {
  CajaFile *file;
  FMDirectoryView *view;
  GMountOperation *mount_op;
  ...
  mount_op =
      gtk_mount_operation_new(fm_directory_view_get_containing_window(view));
  g_mount_operation_set_password_save(mount_op, G_PASSWORD_SAVE_FOR_SESSION);
  caja_file_mount(file, mount_op, NULL, file_mount_callback, NULL);
  g_object_unref(mount_op);
}

static void action_self_unmount_volume_callback(GtkAction *action,
                                                gpointer data) {
  CajaFile *file;
  FMDirectoryView *view;
  GMountOperation *mount_op;
...
static void action_self_eject_volume_callback(GtkAction *action,
                                              gpointer data) {
  CajaFile *file;
  FMDirectoryView *view;
  GMountOperation *mount_op;
...
static void action_self_format_volume_callback(GtkAction *action,
                                               gpointer data) {
  CajaFile *file;
  FMDirectoryView *view;
...
static void action_self_start_volume_callback(GtkAction *action,
                                              gpointer data) {
  CajaFile *file;
  FMDirectoryView *view;
  GMountOperation *mount_op;
...
static void action_self_stop_volume_callback(GtkAction *action, gpointer data) {
  CajaFile *file;
  FMDirectoryView *view;
  GMountOperation *mount_op;
...
static void action_self_detect_media_callback(GtkAction *action,
                                              gpointer data) {
  CajaFile *file;
  FMDirectoryView *view;
...
static void action_location_mount_volume_callback(GtkAction *action,
                                                  gpointer data) {
  CajaFile *file;
  FMDirectoryView *view;
  GMountOperation *mount_op;
...
static void action_location_unmount_volume_callback(GtkAction *action,
                                                    gpointer data) {
  CajaFile *file;
  FMDirectoryView *view;
  GMountOperation *mount_op;
...
static void action_location_eject_volume_callback(GtkAction *action,
                                                  gpointer data) {
  CajaFile *file;
  FMDirectoryView *view;
  GMountOperation *mount_op;
...
static void action_location_format_volume_callback(GtkAction *action,
                                                   gpointer data) {
  CajaFile *file;
  FMDirectoryView *view;
...
static void action_location_start_volume_callback(GtkAction *action,
                                                  gpointer data) {
  CajaFile *file;
  FMDirectoryView *view;
  GMountOperation *mount_op;
...
static void action_location_stop_volume_callback(GtkAction *action,
                                                 gpointer data) {
  CajaFile *file;
  FMDirectoryView *view;
  GMountOperation *mount_op;
...
static void action_location_detect_media_callback(GtkAction *action,
                                                  gpointer data) {
  CajaFile *file;
  FMDirectoryView *view;
...
static void connect_to_server_response_callback(GtkDialog *dialog,
                                                int response_id,
                                                gpointer data) {
...
static void action_connect_to_server_link_callback(GtkAction *action,
  ...
  if (uri != NULL) {
    ...
    gtk_dialog_add_button(GTK_DIALOG(dialog), _("_Connect"), GTK_RESPONSE_OK);

    g_object_set_data_full(G_OBJECT(dialog), "link-uri", g_strdup(uri), g_free);
    g_object_set_data_full(G_OBJECT(dialog), "link-icon", g_strdup(icon_name),
                           g_free);
    ...
    g_signal_connect(dialog, "response",
                     G_CALLBACK(connect_to_server_response_callback), entry);
    gtk_widget_show(dialog);
  }
...
static void action_location_open_alternate_callback(GtkAction *action,
  ...
  fm_directory_view_activate_file(view, file, CAJA_WINDOW_OPEN_IN_NAVIGATION,
                                  0);
}

static void action_location_open_in_new_tab_callback(GtkAction *action,
                                                     gpointer callback_data) {
  FMDirectoryView *view;
  CajaFile *file;
  ...
  fm_directory_view_activate_file(view, file,
                                  CAJA_WINDOW_OPEN_ACCORDING_TO_MODE,
                                  CAJA_WINDOW_OPEN_FLAG_NEW_TAB);
}

static void action_location_open_folder_window_callback(
  ...
  fm_directory_view_activate_file(view, file, CAJA_WINDOW_OPEN_IN_SPATIAL, 0);
}
...
static void action_location_paste_files_into_callback(GtkAction *action,
                                                      gpointer callback_data) {
  FMDirectoryView *view;
  CajaFile *file;
...
static void action_location_delete_callback(GtkAction *action,
  ...
  caja_file_operations_delete(
      files, fm_directory_view_get_containing_window(view), NULL, NULL);

  g_list_free_full(files, g_object_unref);
}

static void action_location_restore_from_trash_callback(
    GtkAction *action, gpointer callback_data) {
  FMDirectoryView *view;
  CajaFile *file;
  GList l;
...
static void fm_directory_view_init_show_hidden_files(FMDirectoryView *view) {
  ...
  if (mode == CAJA_WINDOW_SHOW_HIDDEN_FILES_DEFAULT) {
    show_hidden_default_setting = g_settings_get_boolean(
...
static void fm_directory_view_init_show_backup_files(FMDirectoryView *view) {
  ...
  if (mode == CAJA_WINDOW_SHOW_BACKUP_FILES_DEFAULT) {
    show_backup_default_setting = g_settings_get_boolean(
        caja_preferences, CAJA_PREFERENCES_SHOW_BACKUP_FILES);
...
static const GtkActionEntry directory_view_entries[] = {
    /* name, icon name, label */ {"New Documents", "document-new",
                                  N_("Create _Document"), NULL, NULL, NULL},
    /* name, icon name, label */
    {"Open With", NULL, N_("Open Wit_h"), NULL,
    ...
    {"PropertiesAccel", NULL,
     ...
     /* tooltip */ NULL, G_CALLBACK(action_properties_callback)},
    /* name, icon name */
    ...
    /* name, icon name, label */
    {"No Templates", NULL, N_("No templates installed"), NULL, NULL, NULL},
    /* name, icon name */
    {"New Empty File", NULL,
     /* Translators: this is used to indicate that a file doesn't contain
        anything */
    ...
    {"OpenAccel", NULL,
     ...
     /* tooltip */ NULL, G_CALLBACK(action_open_callback)},
    /* name, icon name */
    {"OpenAlternate", NULL,
    ...
    /* name, icon name, label */
    {"CopyToMenu", NULL, N_("Cop_y to"), NULL, NULL, NULL},
    /* name, icon name, label */
    {"MoveToMenu", NULL, N_("M_ove to"), NULL, NULL, NULL},
    /* name, icon name */
    {"Select All", NULL,
    ...
    {"Invert Selection", NULL,
     ...
     /* tooltip */
     N_("Select all and only the items that are not currently selected"),
    ...
    /* name, icon name */
    {"RenameSelectAll", NULL,
     /* label, accelerator */ "RenameSelectAll", "<shift>F2",
     /* tooltip */ NULL, G_CALLBACK(action_rename_select_all_callback)},
    /* name, icon name */
    {"Trash", NULL,
    ...
    {"Reset to Defaults", NULL,
     ...
     /* tooltip */
     N_("Reset sorting order and zoom level to match preferences for this "
        "view"),
     G_CALLBACK(action_reset_to_defaults_callback)},
    /* name, icon name */
    {"Connect To Server Link", NULL,
    ...
    /* name, icon name */
    {"OpenCloseParent", NULL,
     /* label, accelerator */ N_("Open File and Close window"),
     "<alt><shift>Down",
    ...
    /* name, icon name */
    {FM_ACTION_LOCATION_CUT, "edit-cut",
    ...
    /* name, icon name */
    {FM_ACTION_LOCATION_COPY, "edit-copy",
    ...
    /* name, icon name */
    {FM_ACTION_LOCATION_PASTE_FILES_INTO, "edit-paste",
     /* label, accelerator */ N_("_Paste Into Folder"), "",
     /* tooltip */
     N_("Move or copy files previously selected by a Cut or Copy command into "
        "this folder"),
    ...
    /* name, icon name */
    {"LocationProperties", "document-properties",
    ...
    /* name, icon name, label */
    {FM_ACTION_COPY_TO_NEXT_PANE, NULL, N_("_Other pane"), NULL,
     N_("Copy the current selection to the other pane in the window"),
     G_CALLBACK(action_copy_to_next_pane_callback)},
    /* name, icon name, label */
    {FM_ACTION_MOVE_TO_NEXT_PANE, NULL, N_("_Other pane"), NULL,
     N_("Move the current selection to the other pane in the window"),
     G_CALLBACK(action_move_to_next_pane_callback)},
    /* name, icon name, label */
    {FM_ACTION_COPY_TO_HOME, CAJA_ICON_HOME, N_("_Home Folder"), NULL,
     N_("Copy the current selection to the home folder"),
     G_CALLBACK(action_copy_to_home_callback)},
    /* name, icon name, label */
    {FM_ACTION_MOVE_TO_HOME, CAJA_ICON_HOME, N_("_Home Folder"), NULL,
     N_("Move the current selection to the home folder"),
     G_CALLBACK(action_move_to_home_callback)},
    /* name, icon name, label */
    {FM_ACTION_COPY_TO_DESKTOP, CAJA_ICON_DESKTOP, N_("_Desktop"), NULL,
     N_("Copy the current selection to the desktop"),
     G_CALLBACK(action_copy_to_desktop_callback)},
    /* name, icon name, label */
    {FM_ACTION_MOVE_TO_DESKTOP, CAJA_ICON_DESKTOP, N_("_Desktop"), NULL,
     N_("Move the current selection to the desktop"),
     G_CALLBACK(action_move_to_desktop_callback)},
};

static void connect_proxy(FMDirectoryView *view, GtkAction *action,
                          GtkWidget *proxy, GtkActionGroup *action_group) {
  const gchar *action_name;
...
static void pre_activate(FMDirectoryView *view, GtkAction *action,
                         GtkActionGroup *action_group) {
  GdkEvent *event;
  GtkWidget *proxy;
...
static void real_merge_menus(FMDirectoryView *view) {
  ...
  gtk_action_group_add_actions(action_group, directory_view_entries,
                               G_N_ELEMENTS(directory_view_entries), view);
  G_GNUC_END_IGNORE_DEPRECATIONS;

  /* Translators: %s is a directory */
  tooltip = g_strdup_printf(_("Run or manage scripts from %s"),
                            "~/.config/caja/scripts");
  G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
  ...
  g_signal_connect_object(fm_directory_view_get_background(view),
                          "settings_changed", G_CALLBACK(schedule_update_menus),
                          G_OBJECT(view), G_CONNECT_SWAPPED);
  view->details->scripts_invalid = TRUE;
  view->details->templates_invalid = TRUE;
}

static gboolean can_paste_into_file(CajaFile *file) {
  if (caja_file_is_directory(file) && caja_file_can_write(file)) {
    return TRUE;
  }
...
static void clipboard_targets_received(GtkClipboard *clipboard,
  ...
  gtk_action_set_sensitive(
      action, GPOINTER_TO_INT(g_object_get_data(
                  G_OBJECT(action), "can-paste-according-to-clipboard")) &&
                  GPOINTER_TO_INT(g_object_get_data(
                      G_OBJECT(action), "can-paste-according-to-destination")));
  G_GNUC_END_IGNORE_DEPRECATIONS;
...
static gboolean showing_trash_directory(FMDirectoryView *view) {
  CajaFile *file;
...
static gboolean should_show_empty_trash(FMDirectoryView *view) {
  return (showing_trash_directory(view) ||
          caja_window_info_get_window_type(view->details->window) ==
...
static gboolean file_list_all_are_folders(GList *file_list) {
  ...
  for (l = file_list; l != NULL; l = l->next) {
    file = CAJA_FILE(l->data);
    if (caja_file_is_caja_link(file) && !CAJA_IS_DESKTOP_ICON_FILE(file)) {
       ...
       */
      is_dir = (linked_file != NULL && caja_file_is_directory(linked_file)) ||
               (activation_uri != NULL &&
...
static void file_should_show_foreach(
    CajaFile *file, gboolean *show_mount, gboolean *show_unmount,
    gboolean *show_eject, gboolean *show_connect, gboolean *show_format,
    gboolean *show_start, gboolean *show_stop, gboolean *show_poll,
  ...
  *start_stop_type = caja_file_get_start_stop_type(file);
  ...
  if (caja_file_is_caja_link(file)) {
    ...
    uri = caja_file_get_activation_uri(file);
    if (uri != NULL &&
        (eel_istr_has_prefix(uri, "ftp:") || eel_istr_has_prefix(uri, "ssh:") ||
...
static void file_should_show_self(CajaFile *file, gboolean *show_mount,
                                  gboolean *show_unmount, gboolean *show_eject,
                                  gboolean *show_format, gboolean *show_start,
...
static gboolean files_are_all_directories(GList *files) {
  GList *l;
...
static gboolean files_is_none_directory(GList *files) {
  GList *l;
...
static void update_restore_from_trash_action(GtkAction *action, GList *files,
                                             gboolean is_self) {
  ...
  if (files != NULL) {
    if (g_list_length(files) == 1) {
      ...
      original_dirs_hash =
          caja_trashed_files_get_original_directories(files, NULL);
  ...
  if (original_file != NULL || original_dirs != NULL) {
    ...
    if (is_self) {
      ...
      tooltip = g_strdup_printf(
          _("Move the open folder out of the trash to \"%s\""), original_name);
    } else if (files_are_all_directories(files)) {
      if (original_name != NULL) {
        tooltip = g_strdup_printf(
            ngettext("Move the selected folder out of the trash to \"%s\"",
                     "Move the selected folders out of the trash to \"%s\"",
                     g_list_length(files)),
            original_name);
      } else {
        tooltip = g_strdup_printf(
            ngettext("Move the selected folder out of the trash",
      ...
      if (original_name != NULL) {
        tooltip = g_strdup_printf(
            ngettext("Move the selected file out of the trash to \"%s\"",
                     "Move the selected files out of the trash to \"%s\"",
                     g_list_length(files)),
            original_name);
      } else {
        tooltip = g_strdup_printf(ngettext(
            "Move the selected file out of the trash",
            "Move the selected files out of the trash", g_list_length(files)));
      }
    } else {
      if (original_name != NULL) {
        tooltip = g_strdup_printf(
            ngettext("Move the selected item out of the trash to \"%s\"",
        ...
        tooltip = g_strdup_printf(ngettext(
            ...
            "Move the selected items out of the trash", g_list_length(files)));
      }
    }
...
static void real_update_menus_volumes(FMDirectoryView *view, GList *selection,
  ...
  for (l = selection;
       l != NULL && (show_mount || show_unmount || show_eject || show_connect ||
                     show_format || show_start || show_stop || show_poll);
       l = l->next) {
    ...
    file_should_show_foreach(file, &show_mount_one, &show_unmount_one,
                             &show_eject_one, &show_connect_one,
                             &show_format_one, &show_start_one, &show_stop_one,
                             &show_poll_one, &start_stop_type);
    ...
    switch (start_stop_type) {
      ...
      case G_DRIVE_START_STOP_TYPE_MULTIDISK:
        ...
        gtk_action_set_tooltip(action,
                               _("Start the selected multi-disk drive"));
        break;
  ...
  show_self_mount = show_self_unmount = show_self_eject = show_self_format =
      show_self_start = show_self_stop = show_self_poll = FALSE;

  file = fm_directory_view_get_directory_as_file(view);
  file_should_show_self(file, &show_self_mount, &show_self_unmount,
                        &show_self_eject, &show_self_format, &show_self_start,
                        &show_self_stop, &show_self_poll,
    ...
    switch (self_start_stop_type) {
      ...
      case G_DRIVE_START_STOP_TYPE_UNKNOWN:
        ...
        gtk_action_set_tooltip(
            action, _("Start the drive associated with the open folder"));
        break;
      case G_DRIVE_START_STOP_TYPE_SHUTDOWN:
        gtk_action_set_label(action, _("_Start"));
        gtk_action_set_tooltip(
      ...
      case G_DRIVE_START_STOP_TYPE_NETWORK:
        gtk_action_set_label(action, _("_Connect"));
        gtk_action_set_tooltip(
            action, _("Connect to the drive associated with the open folder"));
        break;
      case G_DRIVE_START_STOP_TYPE_MULTIDISK:
        gtk_action_set_label(action, _("_Start Multi-disk Drive"));
        gtk_action_set_tooltip(
            action,
            _("Start the multi-disk drive associated with the open folder"));
        break;
      case G_DRIVE_START_STOP_TYPE_PASSWORD:
        gtk_action_set_label(action, _("_Unlock Drive"));
        gtk_action_set_tooltip(
            action, _("Unlock the drive associated with the open folder"));
        break;
    }
  }
  ...
  if (show_self_stop) {
    switch (self_start_stop_type) {
      ...
      case G_DRIVE_START_STOP_TYPE_UNKNOWN:
        gtk_action_set_label(action, _("_Stop"));
        gtk_action_set_tooltip(
            action, _("_Stop the drive associated with the open folder"));
        break;
      case G_DRIVE_START_STOP_TYPE_SHUTDOWN:
        gtk_action_set_label(action, _("_Safely Remove Drive"));
        gtk_action_set_tooltip(
            action,
            _("Safely remove the drive associated with the open folder"));
        break;
      case G_DRIVE_START_STOP_TYPE_NETWORK:
        gtk_action_set_label(action, _("_Disconnect"));
        gtk_action_set_tooltip(
            action, _("Disconnect the drive associated with the open folder"));
        break;
      case G_DRIVE_START_STOP_TYPE_MULTIDISK:
        gtk_action_set_label(action, _("_Stop Multi-disk Drive"));
        gtk_action_set_tooltip(
            action,
            _("Stop the multi-disk drive associated with the open folder"));
        break;
      case G_DRIVE_START_STOP_TYPE_PASSWORD:
        gtk_action_set_label(action, _("_Lock Drive"));
        gtk_action_set_tooltip(
            action, _("Lock the drive associated with the open folder"));
        break;
    }
  }
...
static void real_update_location_menu_volumes(FMDirectoryView *view) {
  GtkAction *action;
  CajaFile *file;
  gboolean show_mount;
  gboolean show_unmount;
  ...
  file_should_show_foreach(file, &show_mount, &show_unmount, &show_eject,
                           &show_connect, &show_format, &show_start, &show_stop,
                           &show_poll, &start_stop_type);
 ...
 */
static void real_update_paste_menu(FMDirectoryView *view, GList *selection,
                                   gint selection_count) {
  ...
  can_paste_files_into =
      (selection_count == 1 && can_paste_into_file(CAJA_FILE(selection->data)));
  ...
  gtk_clipboard_request_targets(caja_clipboard_get(GTK_WIDGET(view)),
                                clipboard_targets_received, view);
}

static void real_update_location_menu(FMDirectoryView *view) {
  GtkAction *action;
  CajaFile *file;
  ...
  if (caja_window_info_get_window_type(view->details->window) ==
      CAJA_WINDOW_NAVIGATION) {
    if (g_settings_get_boolean(caja_preferences,
    ...
    label = g_strdup(ngettext("_Browse Folder", "_Browse Folders", 1));
  }
  G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
  action = gtk_action_group_get_action(view->details->dir_action_group,
  ...
  file = view->details->location_popup_directory_as_file;
  g_assert(CAJA_IS_FILE(file));
  g_assert(caja_file_check_if_ready(
      file, CAJA_FILE_ATTRIBUTE_INFO | CAJA_FILE_ATTRIBUTE_MOUNT |
  ...
  can_delete_file =
      caja_file_can_delete(file) && !is_special_link && !is_desktop_or_home_dir;
  ...
  action = gtk_action_group_get_action(view->details->dir_action_group,
                                       FM_ACTION_LOCATION_PASTE_FILES_INTO);
  G_GNUC_END_IGNORE_DEPRECATIONS;
  g_object_set_data(G_OBJECT(action), "can-paste-according-to-destination",
  ...
  if (file != NULL && caja_file_is_in_trash(file)) {
    ...
    show_separate_delete_command = g_settings_get_boolean(
        caja_preferences, CAJA_PREFERENCES_ENABLE_DELETE);
  }

  G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
  action = gtk_action_group_get_action(view->details->dir_action_group,
                                       FM_ACTION_LOCATION_TRASH);
  g_object_set(action, "label", label, "tooltip", tip, "icon-name",
               (file != NULL && caja_file_is_in_trash(file))
                   ? CAJA_ICON_DELETE
                   : CAJA_ICON_TRASH_FULL,
               NULL);
  gtk_action_set_sensitive(action, can_delete_file);
  ...
  if (show_separate_delete_command) {
    gtk_action_set_sensitive(action, can_delete_file);
    g_object_set(action, "icon-name", CAJA_ICON_DELETE, "sensitive",
  ...
  /* we silently assume that fm_directory_view_supports_properties always
   * returns the same value. Therefore, we don't update the sensitivity of
   * FM_ACTION_LOCATION_PROPERTIES */
}
...
static gboolean has_writable_extra_pane(FMDirectoryView *view) {
...
static void real_update_menus(FMDirectoryView *view) {
  ...
  selection_contains_desktop_or_home_dir =
      desktop_or_home_dir_in_selection(view);
  ...
  can_delete_files = can_delete_all(selection) && selection_count != 0 &&
                     ...
                     !selection_contains_desktop_or_home_dir;
  can_copy_files = selection_count != 0 && !selection_contains_special_link;
  ...
  show_open_alternate =
      file_list_all_are_folders(selection) && selection_count > 0 &&
      !(caja_window_info_get_window_type(view->details->window) ==
            CAJA_WINDOW_DESKTOP &&
  ...
  if (caja_window_info_get_window_type(view->details->window) ==
    ...
    if (g_settings_get_boolean(caja_preferences,
      ...
      if (selection_count == 0 || selection_count == 1) {
        ...
        label_with_underscore = g_strdup_printf(
            ngettext("Open in %'d New _Window", "Open in %'d New _Windows",
                     selection_count),
            selection_count);
      }
    } else {
      if (selection_count == 0 || selection_count == 1) {
        ...
        label_with_underscore = g_strdup_printf(
            ngettext("Browse in %'d New _Window", "Browse in %'d New _Windows",
                     selection_count),
            selection_count);
      }
    ...
    label_with_underscore = g_strdup(
        ngettext("_Browse Folder", "_Browse Folders", selection_count));
  }
    ...
    if (g_settings_get_boolean(caja_preferences,
      ...
      if (selection_count == 0 || selection_count == 1) {
        ...
        label_with_underscore =
            g_strdup_printf(ngettext("Open in %'d New _Tab",
                                     "Open in %'d New _Tabs", selection_count),
                            selection_count);
      }
    } else {
      if (selection_count == 0 || selection_count == 1) {
        ...
        label_with_underscore = g_strdup_printf(
            ngettext("Browse in %'d New _Tab", "Browse in %'d New _Tabs",
                     selection_count),
            selection_count);
      }
    }
  ...
  action = gtk_action_group_get_action(view->details->dir_action_group,
                                       FM_ACTION_TRASH);
  g_object_set(action, "label", label, "tooltip", tip, "icon-name",
               all_selected_items_in_trash(view) ? CAJA_ICON_DELETE
                                                 : CAJA_ICON_TRASH_FULL,
               NULL);
  ...
  if (show_separate_delete_command) {
    g_object_set(action, "label", _("_Delete"), "icon-name", CAJA_ICON_DELETE,
                 NULL);
  }
  gtk_action_set_sensitive(action, can_delete_files);
  ...
  G_GNUC_END_IGNORE_DEPRECATIONS;
  g_object_set(action, "label",
               ngettext("Ma_ke Link", "Ma_ke Links", selection_count), NULL);
  ...
  if (selection_count == 0) {
    gtk_action_set_tooltip(
        action, _("View or modify the properties of the open folder"));
  } else {
    gtk_action_set_tooltip(
        action, _("View or modify the properties of each selected item"));
  }
  ...
  g_object_set(action, "label", _("E_mpty Trash"), NULL);
  ...
  if (view->details->model && CAJA_IS_SEARCH_DIRECTORY(view->details->model)) {
    CajaSearchDirectory *search;
  ...
  disable_command_line = g_settings_get_boolean(
      mate_lockdown_preferences, CAJA_PREFERENCES_LOCKDOWN_COMMAND_LINE);
  G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
  action = gtk_action_group_get_action(view->details->dir_action_group,
...
void fm_directory_view_pop_up_selection_context_menu(FMDirectoryView *view,
  ...
  eel_pop_up_context_menu(
      create_popup_menu(view, FM_DIRECTORY_VIEW_POPUP_PATH_SELECTION), NULL);
}

/**
 * fm_directory_view_pop_up_background_context_menu
 *
 * Pop up a context menu appropriate to the view globally at the last right
 *click location.
 * @view: FMDirectoryView of interest.
 *
 * Return value: CajaDirectory for this view.
...
void fm_directory_view_pop_up_background_context_menu(FMDirectoryView *view,
  ...
  eel_pop_up_context_menu(
      create_popup_menu(view, FM_DIRECTORY_VIEW_POPUP_PATH_BACKGROUND), event);
}

static void real_pop_up_location_context_menu(FMDirectoryView *view) {
  /* always update the menu before showing it. Shouldn't be too expensive. */
  real_update_location_menu(view);

  update_context_menu_position_from_event(view,
...
static void location_popup_file_attributes_ready(CajaFile *file,
                                                 gpointer data) {
  FMDirectoryView *view;
...
static void unschedule_pop_up_location_context_menu(FMDirectoryView *view) {
  if (view->details->location_popup_directory_as_file != NULL) {
    ...
    caja_file_cancel_call_when_ready(
        ...
        location_popup_file_attributes_ready, view);
    caja_file_unref(view->details->location_popup_directory_as_file);
    view->details->location_popup_directory_as_file = NULL;
  }
}

static void schedule_pop_up_location_context_menu(FMDirectoryView *view,
                                                  GdkEventButton *event,
                                                  CajaFile *file) {
  g_assert(CAJA_IS_FILE(file));
...
static void fm_directory_view_drop_proxy_received_uris(
  ...
  fm_directory_view_move_copy_items(
      source_uri_list, NULL, target_uri != NULL ? target_uri : container_uri,
      action, 0, 0, view);
...
static void fm_directory_view_drop_proxy_received_netscape_url(
    FMDirectoryView *view, const char *netscape_url, const char *target_uri,
    GdkDragAction action) {
  fm_directory_view_handle_netscape_url_drop(view, netscape_url, target_uri,
                                             action, 0, 0);
}
...
static void schedule_update_menus(FMDirectoryView *view) {
  ...
  if (view->details->update_menus_timeout_id == 0) {
    view->details->update_menus_timeout_id = g_timeout_add(
        view->details->update_interval, update_menus_timeout_callback, view);
  }
}
...
static void schedule_update_status(FMDirectoryView *view) {
  ...
  if (view->details->update_status_idle_id == 0) {
    view->details->update_status_idle_id = g_idle_add_full(
...
void fm_directory_view_notify_selection_changed(FMDirectoryView *view) {
  ...
  if (caja_debug_log_is_domain_enabled(CAJA_DEBUG_LOG_DOMAIN_USER)) {
    ...
    caja_debug_log_with_file_list(FALSE, CAJA_DEBUG_LOG_DOMAIN_USER, selection,
                                  "selection changed in window %p", window);
    caja_file_list_free(selection);
  }
...
static void file_changed_callback(CajaFile *file, gpointer callback_data) {
  ...
  if (!caja_file_is_in_trash(file)) {
    EEL_CALL_METHOD(FM_DIRECTORY_VIEW_CLASS, view, emblems_changed, (view));
  }
}
...
static void load_directory(FMDirectoryView *view, CajaDirectory *directory) {
   ...
   */
  attributes = CAJA_FILE_ATTRIBUTE_INFO | CAJA_FILE_ATTRIBUTE_MOUNT |
               CAJA_FILE_ATTRIBUTE_FILESYSTEM_INFO;
  view->details->metadata_for_directory_as_file_pending = TRUE;
  view->details->metadata_for_files_in_directory_pending = TRUE;
  caja_file_call_when_ready(view->details->directory_as_file, attributes,
                            metadata_for_directory_as_file_ready_callback,
                            view);
  caja_directory_call_when_ready(view->details->model, attributes, FALSE,
   ...
   */
  attributes = CAJA_FILE_ATTRIBUTE_INFO | CAJA_FILE_ATTRIBUTE_FILESYSTEM_INFO;
  caja_file_monitor_add(view->details->directory_as_file,
                        &view->details->directory_as_file, attributes);
...
static void finish_loading(FMDirectoryView *view) {
  CajaFileAttributes attributes;

  caja_window_info_report_load_underway(view->details->window, CAJA_VIEW(view));
  ...
  caja_directory_file_monitor_add(view->details->model, &view->details->model,
                                  view->details->show_hidden_files, attributes,
                                  files_added_callback, view);
...
static void finish_loading_if_all_metadata_loaded(FMDirectoryView *view) {
...
static char **real_get_emblem_names_to_exclude(FMDirectoryView *view) {
  char **excludes;
  int i;
...
static void fm_directory_view_merge_menus(FMDirectoryView *view) {
  ...
  EEL_CALL_METHOD(FM_DIRECTORY_VIEW_CLASS, view, merge_menus, (view));
}

static void fm_directory_view_unmerge_menus(FMDirectoryView *view) {
  g_return_if_fail(FM_IS_DIRECTORY_VIEW(view));

  EEL_CALL_METHOD(FM_DIRECTORY_VIEW_CLASS, view, unmerge_menus, (view));
}

static void disconnect_handler(GObject *object, guint *id) {
  if (*id != 0) {
...
static void disconnect_directory_handler(FMDirectoryView *view, guint *id) {
  disconnect_handler(G_OBJECT(view->details->model), id);
}

static void disconnect_directory_as_file_handler(FMDirectoryView *view,
                                                 guint *id) {
...
static void disconnect_model_handlers(FMDirectoryView *view) {
  ...
  disconnect_directory_as_file_handler(view,
                                       &view->details->file_changed_handler_id);
  caja_file_cancel_call_when_ready(
      view->details->directory_as_file,
...
void fm_directory_view_reset_to_defaults(FMDirectoryView *view) {
  ...
  EEL_CALL_METHOD(FM_DIRECTORY_VIEW_CLASS, view, reset_to_defaults, (view));
  mode = caja_window_info_get_hidden_files_mode(view->details->window);
  if (mode != CAJA_WINDOW_SHOW_HIDDEN_FILES_DEFAULT) {
    caja_window_info_set_hidden_files_mode(
        view->details->window, CAJA_WINDOW_SHOW_HIDDEN_FILES_DEFAULT);
  }
}
...
void fm_directory_view_select_all(FMDirectoryView *view) {
  ...
  EEL_CALL_METHOD(FM_DIRECTORY_VIEW_CLASS, view, select_all, (view));
}
...
void fm_directory_view_set_selection(FMDirectoryView *view, GList *selection) {
  ...
  EEL_CALL_METHOD(FM_DIRECTORY_VIEW_CLASS, view, set_selection,
                  (view, selection));
}
...
void fm_directory_view_reveal_selection(FMDirectoryView *view) {
  ...
  EEL_CALL_METHOD(FM_DIRECTORY_VIEW_CLASS, view, reveal_selection, (view));
}
...
gboolean fm_directory_view_should_show_file(FMDirectoryView *view,
                                            CajaFile *file) {
  return caja_file_should_show(file, view->details->show_hidden_files,
...
static gboolean real_using_manual_layout(FMDirectoryView *view) {
  g_return_val_if_fail(FM_IS_DIRECTORY_VIEW(view), FALSE);
...
void fm_directory_view_update_menus(FMDirectoryView *view) {
  ...
  EEL_CALL_METHOD(FM_DIRECTORY_VIEW_CLASS, view, update_menus, (view));
...
char *fm_directory_view_get_backing_uri(FMDirectoryView *view) {
  ...
  if (CAJA_IS_DESKTOP_DIRECTORY(directory)) {
    directory = caja_desktop_directory_get_real_directory(
        CAJA_DESKTOP_DIRECTORY(directory));
  } else {
    caja_directory_ref(directory);
  }
...
void fm_directory_view_move_copy_items(const GList *item_uris,
                                       ...
                                       const char *target_uri, int copy_action,
                                       int x, int y, FMDirectoryView *view) {
  CajaFile *target_file;

  g_assert(relative_item_points == NULL || relative_item_points->len == 0 ||
  ...
  if (target_file != NULL && caja_file_is_launcher(target_file)) {
    ...
    caja_launch_desktop_file(gtk_widget_get_screen(GTK_WIDGET(view)),
                             target_uri, item_uris,
                             fm_directory_view_get_containing_window(view));
    return;
  } else if (copy_action == GDK_ACTION_COPY && caja_is_engrampa_installed() &&
             target_file != NULL && caja_file_is_archive(target_file)) {
    char *command, *quoted_uri, *tmp;
    const GList *l;
    GdkScreen *screen;

    /* Handle dropping onto a engrampa archiver file, instead of starting a
     * move/copy */
  ...
  caja_file_operations_copy_move(item_uris, relative_item_points, target_uri,
                                 copy_action, GTK_WIDGET(view),
...
static void fm_directory_view_trash_state_changed_callback(
    CajaTrashMonitor *trash_monitor, gboolean state, gpointer callback_data) {
  FMDirectoryView *view;
...
static void revert_slashes(char *string) {
  while (*string != 0) {
    if (*string == '/') {
      *string = '\\';
    }
...
static GdkDragAction ask_link_action(FMDirectoryView *view) {
  ...
  gtk_dialog_add_button(GTK_DIALOG(dialog), _("Make a _Link"), 0);

  eel_dialog_add_button(GTK_DIALOG(dialog), _("_Cancel"), "process-stop", 1);

  gtk_dialog_add_button(GTK_DIALOG(dialog), _("_Download"), 2);
...
static void handle_netscape_url_drop_ask_cb(GObject *source_object,
                                            GAsyncResult *res,
                                            gpointer user_data) {
  ...
  if (action != 0) {
    fm_directory_view_handle_netscape_url_drop(data->view, data->encoded_url,
                                               data->target_uri, action,
                                               data->x, data->y);
  }
...
static gboolean handle_netscape_url_drop_timeout(gpointer user_data) {
  NetscapeUrlDropAsk *data;
...
void fm_directory_view_handle_netscape_url_drop(FMDirectoryView *view,
  ...
  if (!g_file_is_native(f)) {
    eel_show_warning_dialog(
        _("Drag and drop is not supported."),
  ...
  if (action == GDK_ACTION_ASK) {
    ...
    data->timeout = g_timeout_add(1000, handle_netscape_url_drop_timeout, data);
    g_file_query_info_async(f, G_FILE_ATTRIBUTE_STANDARD_CONTENT_TYPE, 0, 0,
                            data->cancellable, handle_netscape_url_drop_ask_cb,
                            data);
  ...
  if ((action != GDK_ACTION_DEFAULT) && (action != GDK_ACTION_COPY) &&
      (action != GDK_ACTION_MOVE) && (action != GDK_ACTION_LINK)) {
  ...
  if (action == GDK_ACTION_LINK) {
    ...
    if (!eel_str_is_empty(link_name)) {
      ...
      success = caja_link_local_create(
          target_uri != NULL ? target_uri : container_uri, link_name,
          link_display_name, "mate-fs-bookmark", url, &point, screen_num, TRUE,
          &error);
      if (!success) {
        if (error) {
          eel_show_error_dialog(_("Link Creation Failed"), error->message,
                                NULL);
          g_error_free(error);
        } else {
          gchar *error_message =
              g_strdup_printf(_("Cannot create the link to %s"), url);
          eel_show_error_dialog(_("Link Creation Failed"), error_message, NULL);
    ...
    fm_directory_view_move_copy_items(
        uri_list, points, target_uri != NULL ? target_uri : container_uri,
        action, x, y, view);
...
void fm_directory_view_handle_uri_list_drop(FMDirectoryView *view,
  ...
  fm_directory_view_move_copy_items(
      real_uri_list, points, target_uri != NULL ? target_uri : container_uri,
      action, x, y, view);

  g_list_free_full(real_uri_list, g_free);

  if (points != NULL) g_array_free(points, TRUE);

  g_free(container_uri);
}

void fm_directory_view_handle_text_drop(FMDirectoryView *view, const char *text,
                                        const char *target_uri,
                                        GdkDragAction action, int x, int y) {
  int length;
  char *container_uri;
  GdkPoint pos;
  ...
  fm_directory_view_new_file_with_initial_contents(
      ...
      /* Translators: This is the filename used for when you dnd text to a
         directory */
      _("dropped text.txt"), text, length, &pos);
...
static GArray *real_get_selected_icon_locations(FMDirectoryView *view) {
  /* By default, just return an empty list. */
  return g_array_new(FALSE, TRUE, sizeof(GdkPoint));
}

static void fm_directory_view_set_property(GObject *object, guint prop_id,
                                           const GValue *value,
                                           GParamSpec *pspec) {
  FMDirectoryView *directory_view;
  CajaWindowSlotInfo *slot;
  ...
  switch (prop_id) {
    case PROP_WINDOW_SLOT:
      ...
      g_signal_connect_object(
          directory_view->details->window, "hidden-files-mode-changed",
          G_CALLBACK(hidden_files_mode_changed), directory_view, 0);
      fm_directory_view_init_show_hidden_files(directory_view);

      g_signal_connect_object(
          directory_view->details->window, "backup-files-mode-changed",
          G_CALLBACK(backup_files_mode_changed), directory_view, 0);
      fm_directory_view_init_show_backup_files(directory_view);
...
gboolean fm_directory_view_handle_scroll_event(FMDirectoryView *directory_view,
  ...
  if (event->state & GDK_CONTROL_MASK) {
    switch (event->direction) {
      ...
      case GDK_SCROLL_SMOOTH:
        gdk_event_get_scroll_deltas((const GdkEvent *)event, &delta_x,
                                    &delta_y);
...
static gboolean fm_directory_view_scroll_event(GtkWidget *widget,
                                               GdkEventScroll *event) {
  FMDirectoryView *directory_view;
...
static void fm_directory_view_parent_set(GtkWidget *widget,
...
static void fm_directory_view_class_init(FMDirectoryViewClass *klass) {
  ...
  signals[ADD_FILE] =
      g_signal_new("add_file", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(FMDirectoryViewClass, add_file), NULL, NULL,
                   fm_marshal_VOID__OBJECT_OBJECT, G_TYPE_NONE, 2,
                   CAJA_TYPE_FILE, CAJA_TYPE_DIRECTORY);
  signals[BEGIN_FILE_CHANGES] = g_signal_new(
      "begin_file_changes", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(FMDirectoryViewClass, begin_file_changes), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
  signals[BEGIN_LOADING] =
      g_signal_new("begin_loading", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(FMDirectoryViewClass, begin_loading), NULL,
                   NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
  signals[CLEAR] =
      g_signal_new("clear", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(FMDirectoryViewClass, clear), NULL, NULL,
                   g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
  signals[END_FILE_CHANGES] = g_signal_new(
      "end_file_changes", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(FMDirectoryViewClass, end_file_changes), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
  signals[FLUSH_ADDED_FILES] = g_signal_new(
      "flush_added_files", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(FMDirectoryViewClass, flush_added_files), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
  signals[END_LOADING] = g_signal_new(
      "end_loading", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(FMDirectoryViewClass, end_loading), NULL, NULL,
      g_cclosure_marshal_VOID__BOOLEAN, G_TYPE_NONE, 1, G_TYPE_BOOLEAN);
  signals[FILE_CHANGED] =
      g_signal_new("file_changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(FMDirectoryViewClass, file_changed), NULL,
                   NULL, fm_marshal_VOID__OBJECT_OBJECT, G_TYPE_NONE, 2,
                   CAJA_TYPE_FILE, CAJA_TYPE_DIRECTORY);
  signals[LOAD_ERROR] = g_signal_new(
      "load_error", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(FMDirectoryViewClass, load_error), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);
  signals[REMOVE_FILE] =
      g_signal_new("remove_file", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(FMDirectoryViewClass, remove_file), NULL,
                   NULL, fm_marshal_VOID__OBJECT_OBJECT, G_TYPE_NONE, 2,
                   CAJA_TYPE_FILE, CAJA_TYPE_DIRECTORY);
  ...
  g_object_class_install_property(
      G_OBJECT_CLASS(klass), PROP_WINDOW_SLOT,
      g_param_spec_object("window-slot", "Window Slot",
                          "The parent window slot reference",
                          CAJA_TYPE_WINDOW_SLOT_INFO,
                          G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY));

  signals[TRASH] = g_signal_new("trash", G_TYPE_FROM_CLASS(klass),
                                G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                                G_STRUCT_OFFSET(FMDirectoryViewClass, trash),
                                g_signal_accumulator_true_handled, NULL,
                                fm_marshal_BOOLEAN__VOID, G_TYPE_BOOLEAN, 0);
  signals[DELETE] = g_signal_new("delete", G_TYPE_FROM_CLASS(klass),
                                 G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
  ...
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_Delete, 0, "trash", 0);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_KP_Delete, 0, "trash", 0);
...
static void undo_update_menu(FMDirectoryView *view) {
  ...
  g_object_set(action, "label", label, "tooltip", tooltip, NULL);
  G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
