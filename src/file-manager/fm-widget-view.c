...
static GList *fm_widget_view_get_selection_for_file_transfer(
...
G_DEFINE_TYPE_WITH_CODE(FMWidgetView, fm_widget_view, FM_TYPE_DIRECTORY_VIEW,
                        G_IMPLEMENT_INTERFACE(CAJA_TYPE_VIEW,
                                              fm_widget_view_iface_init));

static void fm_widget_view_add_file(FMDirectoryView *view, CajaFile *file,
...
static void fm_widget_view_begin_loading(FMDirectoryView *view) {
  GtkWindow *window;
  CajaFile *file;
  gchar *uri;
  GList *providers, *l;
  ...
  for (l = providers; l != NULL; l = l->next) {
    ...
    if (caja_widget_view_provider_supports_uri(
            provider, uri, caja_file_get_file_type(file), mimetype)) {
...
static void fm_widget_view_clear(FMDirectoryView *view) {
  FMWidgetView *widget_view;
...
static void fm_widget_view_file_changed(FMDirectoryView *view, CajaFile *file,
                                        CajaDirectory *directory) {}

static GtkWidget *fm_widget_view_get_background_widget(FMDirectoryView *view) {
...
static guint fm_widget_view_get_item_count(FMDirectoryView *view) {
  FMWidgetView *widget_view;
...
static gboolean fm_widget_view_is_empty(FMDirectoryView *view) {
  FMWidgetView *widget_view;

  widget_view = FM_WIDGET_VIEW(view);
  g_return_val_if_fail(FM_IS_WIDGET_VIEW(view), TRUE);
  g_return_val_if_fail(CAJA_IS_WIDGET_VIEW_PROVIDER(widget_view->provider),
                       TRUE);

  return caja_widget_view_provider_get_item_count(widget_view->provider) == 0;
}

static void fm_widget_view_end_file_changes(FMDirectoryView *view) {}

static void fm_widget_view_remove_file(FMDirectoryView *view, CajaFile *file,
                                       CajaDirectory *directory) {}

static void fm_widget_view_set_selection(FMDirectoryView *view,
                                         GList *selection) {
  fm_directory_view_notify_selection_changed(view);
}

static void fm_widget_view_select_all(FMDirectoryView *view) {}

static void fm_widget_view_reveal_selection(FMDirectoryView *view) {}

static void fm_widget_view_merge_menus(FMDirectoryView *view) {
  FM_DIRECTORY_VIEW_CLASS(fm_widget_view_parent_class)->merge_menus(view);
}

static void fm_widget_view_update_menus(FMDirectoryView *view) {
  FM_DIRECTORY_VIEW_CLASS(fm_widget_view_parent_class)->update_menus(view);
}

/* Reset sort criteria and zoom level to match defaults */
static void fm_widget_view_reset_to_defaults(FMDirectoryView *view) {}

static void fm_widget_view_bump_zoom_level(FMDirectoryView *view,
                                           int zoom_increment) {}

static CajaZoomLevel fm_widget_view_get_zoom_level(FMDirectoryView *view) {
  return CAJA_ZOOM_LEVEL_STANDARD;
}

static void fm_widget_view_zoom_to_level(FMDirectoryView *view,
                                         CajaZoomLevel zoom_level) {}

static void fm_widget_view_restore_default_zoom_level(FMDirectoryView *view) {}

static gboolean fm_widget_view_can_zoom_in(FMDirectoryView *view) {
  return FALSE;
}

static gboolean fm_widget_view_can_zoom_out(FMDirectoryView *view) {
  return FALSE;
}

static void fm_widget_view_start_renaming_file(FMDirectoryView *view,
                                               CajaFile *file,
                                               gboolean select_all) {}

static void fm_widget_view_click_policy_changed(
    FMDirectoryView *directory_view) {}

static int fm_widget_view_compare_files(FMDirectoryView *view, CajaFile *file1,
                                        CajaFile *file2) {
  if (file1 < file2) {
    return -1;
  }
...
static gboolean fm_widget_view_using_manual_layout(FMDirectoryView *view) {
  return FALSE;
}

static void fm_widget_view_end_loading(FMDirectoryView *view,
                                       gboolean all_files_seen) {}

static void fm_widget_view_finalize(GObject *object) {
  G_OBJECT_CLASS(fm_widget_view_parent_class)->finalize(object);
}

static void fm_widget_view_emblems_changed(FMDirectoryView *directory_view) {}

static char *fm_widget_view_get_first_visible_file(CajaView *view) {
  FMWidgetView *widget_view;
  ...
  return caja_widget_view_provider_get_first_visible_file(
      widget_view->provider);
}

static void fm_widget_view_scroll_to_file(CajaView *view, const char *uri) {}

static void fm_widget_view_grab_focus(CajaView *view) {
  gtk_widget_grab_focus(GTK_WIDGET(view));
}

static void fm_widget_view_sort_directories_first_changed(
    FMDirectoryView *view) {}

static void fm_widget_view_class_init(FMWidgetViewClass *class) {
  ...
  fm_directory_view_class->click_policy_changed =
      fm_widget_view_click_policy_changed;
  ...
  fm_directory_view_class->get_background_widget =
      fm_widget_view_get_background_widget;
  ...
  fm_directory_view_class->get_selection_for_file_transfer =
      fm_widget_view_get_selection_for_file_transfer;
  ...
  fm_directory_view_class->restore_default_zoom_level =
      fm_widget_view_restore_default_zoom_level;
  ...
  fm_directory_view_class->sort_directories_first_changed =
      fm_widget_view_sort_directories_first_changed;
  fm_directory_view_class->start_renaming_file =
      fm_widget_view_start_renaming_file;
  ...
  fm_directory_view_class->using_manual_layout =
      fm_widget_view_using_manual_layout;
  fm_directory_view_class->end_loading = fm_widget_view_end_loading;
}

static const char *fm_widget_view_get_id(CajaView *view) {
...
static void fm_widget_view_init(FMWidgetView *widget_view) {
  widget_view->provider = NULL;
}

static CajaView *fm_widget_view_create(CajaWindowSlotInfo *slot) {
  FMWidgetView *view;
  g_assert(CAJA_IS_WINDOW_SLOT_INFO(slot));

  view = g_object_new(FM_TYPE_WIDGET_VIEW, "window-slot", slot, NULL);

  return CAJA_VIEW(view);
}

static gboolean fm_widget_view_supports_uri(const char *uri,
                                            GFileType file_type,
  ...
  for (l = providers; l != NULL; l = l->next) {
    ...
    provider = CAJA_WIDGET_VIEW_PROVIDER(l->data);
    if (caja_widget_view_provider_supports_uri(provider, uri, file_type,
                                               mime_type)) {
      result = TRUE;
    }
  }
...
void fm_widget_view_register(void) {
  ...
  fm_widget_view.view_menu_label_with_mnemonic =
      _(fm_widget_view.view_menu_label_with_mnemonic);
  ...
  fm_widget_view.display_location_label =
