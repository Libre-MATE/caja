...
static FMIconView *get_icon_view(CajaIconContainer *container) {
  /* Type unsafe comparison for performance */
  return ((FMIconContainer *)container)->view;
}

static CajaIconInfo *fm_icon_container_get_icon_images(
  ...
  use_embedding = FALSE;
  if (embedded_text) {
    *embedded_text = caja_file_peek_top_left_text(
        file, need_large_embeddded_text, embedded_text_needs_loading);
  ...
  if (emblem_pixbufs != NULL) {
    ...
    if (emblem_size > 0) {
      ...
      *emblem_pixbufs = caja_file_get_emblem_pixbufs(file, emblem_size, FALSE,
                                                     emblems_to_ignore);
  ...
  *has_window_open = caja_file_has_open_window(file);
  ...
  if (!fm_icon_view_is_compact(icon_view) ||
      caja_icon_container_get_zoom_level(container) >
...
static char *fm_icon_container_get_icon_description(
    CajaIconContainer *container, CajaIconData *data) {
  CajaFile *file;
  char *mime_type;
...
static void fm_icon_container_start_monitor_top_left(
...
static void fm_icon_container_stop_monitor_top_left(
    CajaIconContainer *container, CajaIconData *data, gconstpointer client) {
  CajaFile *file;
...
static void fm_icon_container_prioritize_thumbnailing(
    CajaIconContainer *container, CajaIconData *data) {
  CajaFile *file;
 ...
 * beneath icons.
 */
static GQuark *fm_icon_container_get_icon_text_attributes_from_preferences(
    void) {
...
static int quarkv_length(GQuark *attributes) {
  int i;
  i = 0;
 ...
 *
 **/
static GQuark *fm_icon_container_get_icon_text_attribute_names(
    CajaIconContainer *container, int *len) {
  GQuark *attributes;
  int piece_count;
 ...
 */
static void fm_icon_container_get_icon_text(CajaIconContainer *container,
                                            CajaIconData *data,
                                            char **editable_text,
  ...
  if (caja_icon_container_get_zoom_level(container) ==
...
static SortCategory get_sort_category(CajaFile *file) {
...
static int fm_desktop_icon_container_icons_compare(CajaIconContainer *container,
                                                   CajaIconData *data_a,
                                                   CajaIconData *data_b) {
  CajaFile *file_a;
  CajaFile *file_b;
  ...
  if (category_a == category_b) {
    return caja_file_compare_for_sort(
        ...
        fm_directory_view_should_sort_directories_first(directory_view), FALSE);
  }
...
static int fm_icon_container_compare_icons(CajaIconContainer *container,
                                           CajaIconData *icon_a,
                                           CajaIconData *icon_b) {
  FMIconView *icon_view;
  ...
  if (FM_ICON_CONTAINER(container)->sort_for_desktop) {
    return fm_desktop_icon_container_icons_compare(container, icon_a, icon_b);
  }

  /* Type unsafe comparisons for performance */
  return fm_icon_view_compare_files(icon_view, (CajaFile *)icon_a,
                                    (CajaFile *)icon_b);
}

static int fm_icon_container_compare_icons_by_name(CajaIconContainer *container,
                                                   CajaIconData *icon_a,
                                                   CajaIconData *icon_b) {
  return caja_file_compare_for_sort(CAJA_FILE(icon_a), CAJA_FILE(icon_b),
                                    CAJA_FILE_SORT_BY_DISPLAY_NAME, FALSE,
                                    FALSE);
}

static void fm_icon_container_freeze_updates(CajaIconContainer *container) {
  FMIconView *icon_view;
  icon_view = get_icon_view(container);
  g_return_if_fail(icon_view != NULL);
...
static void fm_icon_container_unfreeze_updates(CajaIconContainer *container) {
  FMIconView *icon_view;
  icon_view = get_icon_view(container);
  g_return_if_fail(icon_view != NULL);
...
static void fm_icon_container_dispose(GObject *object) {
...
static void fm_icon_container_class_init(FMIconContainerClass *klass) {
...
static void fm_icon_container_init(FMIconContainer *icon_container) {
  gtk_style_context_add_class(
      gtk_widget_get_style_context(GTK_WIDGET(icon_container)),
...
CajaIconContainer *fm_icon_container_new(FMIconView *view) {
  return fm_icon_container_construct(g_object_new(FM_TYPE_ICON_CONTAINER, NULL),
                                     view);
}
