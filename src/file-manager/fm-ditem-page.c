...
static gboolean _g_key_file_load_from_gfile(GKeyFile *key_file, GFile *file,
                                            GKeyFileFlags flags,
                                            GError **error) {
  char *data;
  gsize len;
...
static gboolean _g_key_file_save_to_uri(GKeyFile *key_file, const char *uri,
                                        GError **error) {
  GFile *file;
  char *data;
  gsize len;
  ...
  if (!g_file_replace_contents(file, data, len, NULL, FALSE, G_FILE_CREATE_NONE,
                               NULL, NULL, error)) {
    g_object_unref(file);
    g_free(data);
    return FALSE;
...
static GKeyFile *_g_key_file_new_from_file(GFile *file, GKeyFileFlags flags,
                                           GError **error) {
  GKeyFile *key_file;
...
static GKeyFile *_g_key_file_new_from_uri(const char *uri, GKeyFileFlags flags,
                                          GError **error) {
  GKeyFile *key_file;
  GFile *file;
...
static ItemEntry *item_entry_new(const char *field, const char *description,
                                 gboolean localized, gboolean filename) {
  ItemEntry *entry;
...
static void item_entry_free(ItemEntry *entry) {
  g_free(entry->current_value);
  g_free(entry);
}

static void fm_ditem_page_url_drag_data_received(
    GtkWidget *widget, GdkDragContext *context, int x, int y,
    GtkSelectionData *selection_data, guint info, guint time, GtkEntry *entry) {
  char **uris;
...
static void fm_ditem_page_exec_drag_data_received(
    GtkWidget *widget, GdkDragContext *context, int x, int y,
    GtkSelectionData *selection_data, guint info, guint time, GtkEntry *entry) {
  char **uris;
  gboolean exactly_one;
  ...
  gtk_entry_set_text(entry, uri ? uri : "");
...
static void save_entry(GtkEntry *entry, GKeyFile *key_file, const char *uri) {
  GError *error;
  ...
  if (item_entry->localized) {
    ...
    g_key_file_set_locale_string(key_file, MAIN_GROUP, item_entry->field,
                                 languages[0], val);
  } else {
...
static void entry_activate_cb(GtkWidget *entry, GtkWidget *container) {
  const char *uri;
  GKeyFile *key_file;
...
static gboolean entry_focus_out_cb(GtkWidget *entry, GdkEventFocus *event,
                                   GtkWidget *container) {
  const char *uri;
  GKeyFile *key_file;
...
static GtkWidget *build_grid(GtkWidget *container, GKeyFile *key_file,
                             GtkSizeGroup *label_size_group, GList *entries) {
  GList *l;
  char *val;
  GtkWidget *grid;
  GtkWidget *label;
  ...
  for (l = entries; l; l = l->next) {
    ...
    if (item_entry->localized) {
      val = g_key_file_get_locale_string(key_file, MAIN_GROUP,
                                         item_entry->field, NULL, NULL);
    } else {
      val =
          g_key_file_get_string(key_file, MAIN_GROUP, item_entry->field, NULL);
    }
    ...
    g_signal_connect(entry, "activate", G_CALLBACK(entry_activate_cb),
                     container);
    g_signal_connect(entry, "focus_out_event", G_CALLBACK(entry_focus_out_cb),
                     container);
    ...
    if (item_entry->filename) {
      ...
      g_signal_connect(entry, "drag_data_received",
                       G_CALLBACK(fm_ditem_page_url_drag_data_received), entry);
...
static void create_page(GKeyFile *key_file, GtkWidget *box) {
  GtkWidget *grid;
  GList *entries;
  ...
  if (g_strcmp0(type, "Link") == 0) {
    entries = g_list_prepend(
        entries, item_entry_new("Comment", _("Comment"), TRUE, FALSE));
    entries =
        g_list_prepend(entries, item_entry_new("URL", _("URL"), FALSE, TRUE));
    entries = g_list_prepend(
        entries, item_entry_new("GenericName", _("Description"), TRUE, FALSE));
  } else if (g_strcmp0(type, "Application") == 0) {
    entries = g_list_prepend(
        entries, item_entry_new("Comment", _("Comment"), TRUE, FALSE));
    entries = g_list_prepend(
        entries, item_entry_new("Exec", _("Command"), FALSE, FALSE));
    entries = g_list_prepend(
        entries, item_entry_new("GenericName", _("Description"), TRUE, FALSE));
  } else {
...
static void ditem_read_cb(GObject *source_object, GAsyncResult *res,
                          gpointer user_data) {
  GtkWidget *box;
  gsize file_size;
  char *file_contents;
  ...
  if (g_file_load_contents_finish(G_FILE(source_object), res, &file_contents,
                                  &file_size, NULL, NULL)) {
    GKeyFile *key_file;

    key_file = g_key_file_new();
    g_object_set_data_full(G_OBJECT(box), "keyfile", key_file,
                           (GDestroyNotify)g_key_file_free);
    if (g_key_file_load_from_data(key_file, file_contents, file_size, 0,
                                  NULL)) {
...
static void fm_ditem_page_create_begin(const char *uri, GtkWidget *box) {
  GFile *location;
...
GtkWidget *fm_ditem_page_make_box(GtkSizeGroup *label_size_group,
  ...
  g_object_set_data_full(G_OBJECT(box), "label-size-group", label_size_group,
