...
static GList *fm_empty_view_get_selection_for_file_transfer(
    FMDirectoryView *view);
static void fm_empty_view_scroll_to_file(CajaView *view, const char *uri);
...
static void fm_empty_view_add_file(FMDirectoryView *view, CajaFile *file,
  ...
  g_timer_start(timer);
  icon = caja_file_get_icon_surface(
      file, caja_get_icon_size_for_zoom_level(CAJA_ZOOM_LEVEL_STANDARD), TRUE,
      gtk_widget_get_scale_factor(GTK_WIDGET(view)), 0);
...
static void fm_empty_view_begin_loading(FMDirectoryView *view) {}

static void fm_empty_view_clear(FMDirectoryView *view) {}

static void fm_empty_view_file_changed(FMDirectoryView *view, CajaFile *file,
                                       CajaDirectory *directory) {}

static GtkWidget *fm_empty_view_get_background_widget(FMDirectoryView *view) {
...
static guint fm_empty_view_get_item_count(FMDirectoryView *view) {
  return FM_EMPTY_VIEW(view)->details->number_of_files;
}

static gboolean fm_empty_view_is_empty(FMDirectoryView *view) {
  return FM_EMPTY_VIEW(view)->details->number_of_files == 0;
}

static void fm_empty_view_end_file_changes(FMDirectoryView *view) {}

static void fm_empty_view_remove_file(FMDirectoryView *view, CajaFile *file,
                                      CajaDirectory *directory) {
  FM_EMPTY_VIEW(view)->details->number_of_files--;
  g_assert(FM_EMPTY_VIEW(view)->details->number_of_files >= 0);
}

static void fm_empty_view_set_selection(FMDirectoryView *view,
                                        GList *selection) {
  fm_directory_view_notify_selection_changed(view);
}

static void fm_empty_view_select_all(FMDirectoryView *view) {}

static void fm_empty_view_reveal_selection(FMDirectoryView *view) {}

static void fm_empty_view_merge_menus(FMDirectoryView *view) {
  EEL_CALL_PARENT(FM_DIRECTORY_VIEW_CLASS, merge_menus, (view));
}

static void fm_empty_view_update_menus(FMDirectoryView *view) {
  EEL_CALL_PARENT(FM_DIRECTORY_VIEW_CLASS, update_menus, (view));
}

/* Reset sort criteria and zoom level to match defaults */
static void fm_empty_view_reset_to_defaults(FMDirectoryView *view) {}

static void fm_empty_view_bump_zoom_level(FMDirectoryView *view,
                                          int zoom_increment) {}

static CajaZoomLevel fm_empty_view_get_zoom_level(FMDirectoryView *view) {
  return CAJA_ZOOM_LEVEL_STANDARD;
}

static void fm_empty_view_zoom_to_level(FMDirectoryView *view,
                                        CajaZoomLevel zoom_level) {}

static void fm_empty_view_restore_default_zoom_level(FMDirectoryView *view) {}

static gboolean fm_empty_view_can_zoom_in(FMDirectoryView *view) {
  return FALSE;
}

static gboolean fm_empty_view_can_zoom_out(FMDirectoryView *view) {
  return FALSE;
}

static void fm_empty_view_start_renaming_file(FMDirectoryView *view,
                                              CajaFile *file,
                                              gboolean select_all) {}

static void fm_empty_view_click_policy_changed(
    FMDirectoryView *directory_view) {}

static int fm_empty_view_compare_files(FMDirectoryView *view, CajaFile *file1,
                                       CajaFile *file2) {
  if (file1 < file2) {
    return -1;
  }
...
static gboolean fm_empty_view_using_manual_layout(FMDirectoryView *view) {
  return FALSE;
}

static void fm_empty_view_end_loading(FMDirectoryView *view,
                                      gboolean all_files_seen) {}

static void fm_empty_view_finalize(GObject *object) {
...
static void fm_empty_view_emblems_changed(FMDirectoryView *directory_view) {}

static char *fm_empty_view_get_first_visible_file(CajaView *view) {
  return NULL;
}

static void fm_empty_view_scroll_to_file(CajaView *view, const char *uri) {}

static void fm_empty_view_grab_focus(CajaView *view) {
  gtk_widget_grab_focus(GTK_WIDGET(view));
}

static void fm_empty_view_sort_directories_first_changed(
    FMDirectoryView *view) {}

static void fm_empty_view_class_init(FMEmptyViewClass *class) {
  ...
  fm_directory_view_class->click_policy_changed =
      fm_empty_view_click_policy_changed;
  ...
  fm_directory_view_class->sort_directories_first_changed =
      fm_empty_view_sort_directories_first_changed;
  fm_directory_view_class->start_renaming_file =
      fm_empty_view_start_renaming_file;
...
static const char *fm_empty_view_get_id(CajaView *view) {
...
static void fm_empty_view_init(FMEmptyView *empty_view) {
  empty_view->details = g_new0(FMEmptyViewDetails, 1);
}

static CajaView *fm_empty_view_create(CajaWindowSlotInfo *slot) {
  FMEmptyView *view;

  g_assert(CAJA_IS_WINDOW_SLOT_INFO(slot));

  view = g_object_new(FM_TYPE_EMPTY_VIEW, "window-slot", slot, NULL);

  return CAJA_VIEW(view);
}

static gboolean fm_empty_view_supports_uri(const char *uri, GFileType file_type,
