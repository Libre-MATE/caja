...
static void fm_list_model_multi_drag_source_init(
...
G_DEFINE_TYPE_WITH_CODE(
    FMListModel, fm_list_model, G_TYPE_OBJECT,
...
static GtkTreeModelFlags fm_list_model_get_flags(GtkTreeModel *tree_model) {
  return GTK_TREE_MODEL_ITERS_PERSIST;
}

static int fm_list_model_get_n_columns(GtkTreeModel *tree_model) {
  return FM_LIST_MODEL_NUM_COLUMNS +
         FM_LIST_MODEL(tree_model)->details->columns->len;
}
...
static void fm_list_model_ptr_to_iter(FMListModel *model, GSequenceIter *ptr,
                                      GtkTreeIter *iter) {
...
static gint fm_list_model_get_icon_scale(FMListModel *model) {
  gint retval = -1;
...
static void fm_list_model_get_value(GtkTreeModel *tree_model, GtkTreeIter *iter,
  ...
  switch (column) {
    ...
    case FM_LIST_MODEL_LARGEST_ICON_COLUMN:
      if (!g_settings_get_boolean(caja_preferences,
        ...
        surface = cairo_image_surface_create(CAIRO_FORMAT_ARGB32, icon_size,
                                             icon_size);
      ...
      if (file != NULL) {
        ...
        if (model->details->drag_view != NULL) {
          ...
          gtk_tree_view_get_drag_dest_row(model->details->drag_view, &path_a,
                                          NULL);
          if (path_a != NULL) {
            GtkTreePath *path_b;
        ...
        emblem = NULL;
        emblem_icons = caja_file_get_emblem_icons(file, emblems_to_ignore);
        ...
        if (model->details->highlight_files != NULL &&
            g_list_find_custom(model->details->highlight_files, file,
                               (GCompareFunc)caja_file_compare_location)) {
    ...
    case FM_LIST_MODEL_FILE_NAME_IS_EDITABLE_COLUMN:
      ...
      break;
    default:
      if (column >= FM_LIST_MODEL_NUM_COLUMNS ||
          column < FM_LIST_MODEL_NUM_COLUMNS + model->details->columns->len) {
        CajaColumn *caja_column;
        GQuark attribute;
        caja_column =
            model->details->columns->pdata[column - FM_LIST_MODEL_NUM_COLUMNS];

        g_value_init(value, G_TYPE_STRING);
        g_object_get(caja_column, "attribute_q", &attribute, NULL);
        if (file != NULL) {
          char *str;

          str = caja_file_get_string_attribute_with_default_q(file, attribute);
...
static GSequenceIter *lookup_file(FMListModel *model, CajaFile *file,
  ...
  if (directory) {
    parent_ptr =
        g_hash_table_lookup(model->details->directory_reverse_map, directory);
  }
...
GList *fm_list_model_get_all_iters_for_file(FMListModel *model,
  ...
  g_hash_table_foreach(model->details->directory_reverse_map, file_to_iter_cb,
                       &data);
...
static int fm_list_model_file_entry_compare_func(gconstpointer a,
  ...
  if (file_entry1->file != NULL && file_entry2->file != NULL) {
    result = caja_file_compare_for_sort_by_attribute_q(
        file_entry1->file, file_entry2->file, model->details->sort_attribute,
...
int fm_list_model_compare_func(FMListModel *model, CajaFile *file1,
                               CajaFile *file2) {
  int result;

  result = caja_file_compare_for_sort_by_attribute_q(
      file1, file2, model->details->sort_attribute,
      model->details->sort_directories_first,
      (model->details->order == GTK_SORT_DESCENDING));
...
static void fm_list_model_sort_file_entries(FMListModel *model,
                                            GSequence *files,
                                            GtkTreePath *path) {
...
static void fm_list_model_sort(FMListModel *model) {
  GtkTreePath *path;
...
static void fm_list_model_set_sort_column_id(GtkTreeSortable *sortable,
                                             gint sort_column_id,
                                             GtkSortType order) {
  FMListModel *model;
  ...
  model->details->sort_attribute =
      fm_list_model_get_attribute_from_sort_column_id(model, sort_column_id);
...
static gboolean fm_list_model_has_default_sort_func(GtkTreeSortable *sortable) {
  return FALSE;
}

static gboolean fm_list_model_multi_row_draggable(
    EggTreeMultiDragSource *drag_source, GList *path_list) {
  return TRUE;
}

static void each_path_get_data_binder(CajaDragEachSelectedItemDataGet data_get,
                                      gpointer context, gpointer data) {
  ...
  for (l = info->path_list; l != NULL; l = l->next) {
    ...
    if (file) {
      gtk_tree_view_get_cell_area(info->model->details->drag_view, path, column,
                                  &cell_area);

      uri = caja_file_get_uri(file);

      (*data_get)(uri, 0, cell_area.y - info->model->details->drag_begin_y,
                  cell_area.width, cell_area.height, data);
...
static gboolean fm_list_model_multi_drag_data_get(
    EggTreeMultiDragSource *drag_source, GList *path_list,
  ...
  if (gtk_target_list_find(drag_target_list,
                           ...
                           &target_info)) {
    caja_drag_drag_data_get(NULL, NULL, selection_data, target_info,
                            GDK_CURRENT_TIME, &context,
...
static gboolean fm_list_model_multi_drag_data_delete(
    EggTreeMultiDragSource *drag_source, GList *path_list) {
  return TRUE;
}

static void add_dummy_row(FMListModel *model, FileEntry *parent_entry) {
  ...
  dummy_file_entry->ptr =
      g_sequence_insert_sorted(parent_entry->files, dummy_file_entry,
...
gboolean fm_list_model_add_file(FMListModel *model, CajaFile *file,
                                CajaDirectory *directory) {
  GtkTreeIter iter;
  GtkTreePath *path;
  FileEntry *file_entry;
  ...
  file_entry->ptr = g_sequence_insert_sorted(
      files, file_entry, fm_list_model_file_entry_compare_func, model);
...
void fm_list_model_file_changed(FMListModel *model, CajaFile *file,
                                CajaDirectory *directory) {
...
static void fm_list_model_remove(FMListModel *model, GtkTreeIter *iter) {
  GSequenceIter *ptr;
...
void fm_list_model_remove_file(FMListModel *model, CajaFile *file,
                               CajaDirectory *directory) {
  GtkTreeIter iter;
...
static void fm_list_model_clear_directory(FMListModel *model,
                                          GSequence *files) {
  GtkTreeIter iter;
  FileEntry *file_entry = NULL;
...
CajaFile *fm_list_model_file_for_path(FMListModel *model, GtkTreePath *path) {
  ...
  if (gtk_tree_model_get_iter(GTK_TREE_MODEL(model), &iter, path)) {
    gtk_tree_model_get(GTK_TREE_MODEL(model), &iter, FM_LIST_MODEL_FILE_COLUMN,
                       &file, -1);
  }
  return file;
}

gboolean fm_list_model_load_subdirectory(FMListModel *model, GtkTreePath *path,
                                         CajaDirectory **directory) {
  GtkTreeIter iter;
  FileEntry *file_entry;
  CajaDirectory *subdirectory;
  ...
  g_hash_table_insert(model->details->directory_reverse_map, subdirectory,
                      file_entry->ptr);
...
void fm_list_model_set_should_sort_directories_first(
    FMListModel *model, gboolean sort_directories_first) {
...
int fm_list_model_get_sort_column_id_from_attribute(FMListModel *model,
  ...
  for (i = 0; i < model->details->columns->len; i++) {
    ...
    column = CAJA_COLUMN(model->details->columns->pdata[i]);
...
void fm_list_model_set_drag_view(FMListModel *model, GtkTreeView *view,
                                 int drag_begin_x, int drag_begin_y) {
  g_return_if_fail(model != NULL);
...
int fm_list_model_add_column(FMListModel *model, CajaColumn *column) {
...
static void fm_list_model_dispose(GObject *object) {
  FMListModel *model;
...
static void fm_list_model_finalize(GObject *object) {
  FMListModel *model;
...
static void fm_list_model_class_init(FMListModelClass *klass) {
  ...
  list_model_signals[SUBDIRECTORY_UNLOADED] = g_signal_new(
      "subdirectory_unloaded", FM_TYPE_LIST_MODEL, G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(FMListModelClass, subdirectory_unloaded), NULL, NULL,
      g_cclosure_marshal_VOID__OBJECT, G_TYPE_NONE, 1, CAJA_TYPE_DIRECTORY);

  list_model_signals[GET_ICON_SCALE] =
      g_signal_new("get-icon-scale", FM_TYPE_LIST_MODEL,
...
void fm_list_model_set_highlight_for_files(FMListModel *model, GList *files) {
  if (model->details->highlight_files != NULL) {
    g_list_foreach(model->details->highlight_files, refresh_row, model);
    caja_file_list_free(model->details->highlight_files);
    model->details->highlight_files = NULL;
  }
