...
static const SortCriterion sort_criteria[] = {
    {CAJA_FILE_SORT_BY_DISPLAY_NAME, "name", "Sort by Name", N_("by _Name"),
     N_("Keep icons sorted by name in rows")},
    {CAJA_FILE_SORT_BY_SIZE, "size", "Sort by Size", N_("by _Size"),
     N_("Keep icons sorted by size in rows")},
    {CAJA_FILE_SORT_BY_SIZE_ON_DISK, "size_on_disk", "Sort by Size on Disk",
     N_("by S_ize on Disk"), N_("Keep icons sorted by disk usage in rows")},
    {CAJA_FILE_SORT_BY_TYPE, "type", "Sort by Type", N_("by _Type"),
     N_("Keep icons sorted by type in rows")},
    {CAJA_FILE_SORT_BY_MTIME, "modification date", "Sort by Modification Date",
     N_("by Modification _Date"),
     N_("Keep icons sorted by modification date in rows")},
    {CAJA_FILE_SORT_BY_BTIME, "creation date", "Sort by Creation Date",
     N_("by _Creation Date"), N_("Keep icons sorted by creation date in rows")},
    {CAJA_FILE_SORT_BY_EMBLEMS, "emblems", "Sort by Emblems", N_("by _Emblems"),
     N_("Keep icons sorted by emblems in rows")},
    {CAJA_FILE_SORT_BY_TRASHED_TIME, "trashed", "Sort by Trash Time",
     N_("by T_rash Time"), N_("Keep icons sorted by trash time in rows")},
    {CAJA_FILE_SORT_BY_EXTENSION, "extension", "Sort by Extension",
     N_("by E_xtension"),
...
static void set_sort_criterion_by_sort_type(FMIconView *icon_view,
                                            CajaFileSortType sort_type);
static gboolean set_sort_reversed(FMIconView *icon_view, gboolean new_value);
static void switch_to_manual_layout(FMIconView *view);
static void preview_audio(FMIconView *icon_view, CajaFile *file,
                          gboolean start_flag);
...
static void default_sort_in_reverse_order_changed_callback(
...
static void fm_icon_view_destroy(GtkWidget *object) {
...
static void fm_icon_view_finalize(GObject *object) {
  FMIconView *icon_view;
  ...
  g_signal_handlers_disconnect_by_func(
      caja_preferences, default_sort_order_changed_callback, icon_view);
  g_signal_handlers_disconnect_by_func(
      caja_preferences, default_sort_in_reverse_order_changed_callback,
      icon_view);
  g_signal_handlers_disconnect_by_func(
      caja_icon_view_preferences, default_use_tighter_layout_changed_callback,
      icon_view);
...
static CajaIconContainer *get_icon_container(FMIconView *icon_view) {
  return CAJA_ICON_CONTAINER(gtk_bin_get_child(GTK_BIN(icon_view)));
}

static gboolean get_stored_icon_position_callback(CajaIconContainer *container,
                                                  CajaFile *file,
  ...
  position_string =
      caja_file_get_metadata(file, CAJA_METADATA_KEY_ICON_POSITION, "");
  position_good = sscanf(position_string, " %d , %d %c", &position->x,
                         &position->y, &c) == 2;
  g_free(position_string);

  /* If it is the desktop directory, maybe the mate-libs metadata has
  ...
  if (fm_icon_view_supports_scaling(icon_view)) {
    ...
    scale_string =
        caja_file_get_metadata(file, CAJA_METADATA_KEY_ICON_SCALE, "1");
...
static void real_set_sort_criterion(FMIconView *icon_view,
  ...
  if (clear) {
    ...
    caja_file_set_metadata(file, CAJA_METADATA_KEY_ICON_VIEW_SORT_REVERSED,
                           NULL, NULL);
    icon_view->details->sort = get_sort_criterion_by_sort_type(
        get_default_sort_order(file, &icon_view->details->sort_reversed));
  } else {
    /* Store the new sort setting. */
    fm_icon_view_set_directory_sort_by(icon_view, file, sort->metadata_text);
  }
...
static void set_sort_criterion(FMIconView *icon_view,
                               const SortCriterion *sort) {
  if (sort == NULL || icon_view->details->sort == sort) {
    return;
  }
...
static void action_unstretch_callback(GtkAction *action,
                                      gpointer callback_data) {
  g_assert(FM_IS_ICON_VIEW(callback_data));
...
static void fm_icon_view_clean_up(FMIconView *icon_view) {
  EEL_CALL_METHOD(FM_ICON_VIEW_CLASS, icon_view, clean_up, (icon_view));
}

static void fm_icon_view_real_clean_up(FMIconView *icon_view) {
...
static void action_clean_up_callback(GtkAction *action,
                                     gpointer callback_data) {
  fm_icon_view_clean_up(FM_ICON_VIEW(callback_data));
}

static void set_tighter_layout(FMIconView *icon_view, gboolean new_value) {
...
static void action_tighter_layout_callback(GtkAction *action,
                                           gpointer user_data) {
...
static gboolean fm_icon_view_using_auto_layout(FMIconView *icon_view) {
  return caja_icon_container_is_auto_layout(get_icon_container(icon_view));
}

static gboolean fm_icon_view_using_tighter_layout(FMIconView *icon_view) {
  return caja_icon_container_is_tighter_layout(get_icon_container(icon_view));
}

static void action_sort_radio_callback(GtkAction *action,
...
static void list_covers(CajaIconData *data, gpointer callback_data) {
  GSList **file_list;
...
static void unref_cover(CajaIconData *data, gpointer callback_data) {
...
static gboolean should_show_file_on_screen(FMDirectoryView *view,
                                           CajaFile *file) {
...
static void fm_icon_view_remove_file(FMDirectoryView *view, CajaFile *file,
                                     CajaDirectory *directory) {
  FMIconView *icon_view;

  /* This used to assert that 'directory == fm_directory_view_get_model (view)',
   * but that resulted in a lot of crash reports (bug #352592). I don't see how
   * that trace happens. It seems that somehow we get a files_changed event sent
   * to the view from a directory that isn't the model, but the code disables
   * the monitor and signal callback handlers when changing directories. Maybe
   * we can get some more information when this happens. Further discussion in
   * bug #368178.
   */
  if (directory != fm_directory_view_get_model(view)) {
    ...
    g_warning(
        "fm_icon_view_remove_file() - directory not icon view model, shouldn't "
        "happen.\n"
        "file: %p:%s, dir: %p:%s, model: %p:%s, view loading: %d\n"
        "If you see this, please add this info to "
        "https://bugzilla.gnome.org/show_bug.cgi?id=368178",
        file, file_uri, directory, dir_uri, fm_directory_view_get_model(view),
        model_uri, fm_directory_view_get_loading(view));
    g_free(file_uri);
    g_free(dir_uri);
    g_free(model_uri);
  }
...
static void fm_icon_view_add_file(FMDirectoryView *view, CajaFile *file,
                                  CajaDirectory *directory) {
  FMIconView *icon_view;
  CajaIconContainer *icon_container;
  ...
  if (fm_directory_view_get_loading(view) &&
      caja_icon_container_is_empty(icon_container)) {
...
static void fm_icon_view_flush_added_files(FMDirectoryView *view) {
  caja_icon_container_layout_now(get_icon_container(FM_ICON_VIEW(view)));
}

static void fm_icon_view_file_changed(FMDirectoryView *view, CajaFile *file,
                                      CajaDirectory *directory) {
  FMIconView *icon_view;
  ...
  if (!icon_view->details->filter_by_screen) {
    caja_icon_container_request_update(get_icon_container(icon_view),
                                       CAJA_ICON_CONTAINER_ICON_DATA(file));
    return;
  }
...
static gboolean fm_icon_view_supports_auto_layout(FMIconView *view) {
  g_return_val_if_fail(FM_IS_ICON_VIEW(view), FALSE);

  return EEL_CALL_METHOD_WITH_RETURN_VALUE(FM_ICON_VIEW_CLASS, view,
                                           supports_auto_layout, (view));
}

static gboolean fm_icon_view_supports_scaling(FMIconView *view) {
  g_return_val_if_fail(FM_IS_ICON_VIEW(view), FALSE);
...
static gboolean fm_icon_view_supports_manual_layout(FMIconView *view) {
  g_return_val_if_fail(FM_IS_ICON_VIEW(view), FALSE);
...
static gboolean fm_icon_view_supports_keep_aligned(FMIconView *view) {
  g_return_val_if_fail(FM_IS_ICON_VIEW(view), FALSE);
...
static gboolean fm_icon_view_supports_labels_beside_icons(FMIconView *view) {
  g_return_val_if_fail(FM_IS_ICON_VIEW(view), FALSE);

  return EEL_CALL_METHOD_WITH_RETURN_VALUE(
      FM_ICON_VIEW_CLASS, view, supports_labels_beside_icons, (view));
}

static gboolean fm_icon_view_supports_tighter_layout(FMIconView *view) {
...
static void update_layout_menus(FMIconView *view) {
  ...
  if (fm_icon_view_supports_auto_layout(view)) {
    ...
    /* Mark sort criterion. */
    action_name =
        is_auto_layout ? view->details->sort->action : FM_ACTION_MANUAL_LAYOUT;
    ...
    gtk_action_set_sensitive(action,
                             fm_icon_view_supports_tighter_layout(view));
  ...
  gtk_action_set_visible(action, fm_icon_view_supports_manual_layout(view));
  ...
  gtk_action_set_visible(action, fm_icon_view_supports_keep_aligned(view));
...
static char *fm_icon_view_get_directory_sort_by(FMIconView *icon_view,
                                                CajaFile *file) {
  ...
  return EEL_CALL_METHOD_WITH_RETURN_VALUE(
      FM_ICON_VIEW_CLASS, icon_view, get_directory_sort_by, (icon_view, file));
}
...
static CajaFileSortType get_default_sort_order(CajaFile *file,
  ...
  if (auto_storaged_added == FALSE) {
    ...
    eel_g_settings_add_auto_boolean(
        caja_preferences, CAJA_PREFERENCES_DEFAULT_SORT_IN_REVERSE_ORDER,
...
static char *fm_icon_view_real_get_directory_sort_by(FMIconView *icon_view,
                                                     CajaFile *file) {
  const SortCriterion *default_sort_criterion;
  default_sort_criterion =
      get_sort_criterion_by_sort_type(get_default_sort_order(file, NULL));
  g_return_val_if_fail(default_sort_criterion != NULL, NULL);

  return caja_file_get_metadata(file, CAJA_METADATA_KEY_ICON_VIEW_SORT_BY,
...
static void fm_icon_view_set_directory_sort_by(FMIconView *icon_view,
  ...
  EEL_CALL_METHOD(FM_ICON_VIEW_CLASS, icon_view, set_directory_sort_by,
                  (icon_view, file, sort_by));
}

static void fm_icon_view_real_set_directory_sort_by(FMIconView *icon_view,
                                                    CajaFile *file,
                                                    const char *sort_by) {
  const SortCriterion *default_sort_criterion;
  default_sort_criterion =
  ...
  caja_file_set_metadata(file, CAJA_METADATA_KEY_ICON_VIEW_SORT_BY,
                         default_sort_criterion->metadata_text, sort_by);
}

static gboolean fm_icon_view_get_directory_sort_reversed(FMIconView *icon_view,
                                                         CajaFile *file) {
  if (!fm_icon_view_supports_auto_layout(icon_view)) {
    return FALSE;
  }

  return EEL_CALL_METHOD_WITH_RETURN_VALUE(FM_ICON_VIEW_CLASS, icon_view,
                                           get_directory_sort_reversed,
                                           (icon_view, file));
}

static gboolean fm_icon_view_real_get_directory_sort_reversed(
    FMIconView *icon_view, CajaFile *file) {
  gboolean reversed;

  get_default_sort_order(file, &reversed);
  return caja_file_get_boolean_metadata(
      file, CAJA_METADATA_KEY_ICON_VIEW_SORT_REVERSED, reversed);
}

static void fm_icon_view_set_directory_sort_reversed(FMIconView *icon_view,
                                                     CajaFile *file,
                                                     gboolean sort_reversed) {
  if (!fm_icon_view_supports_auto_layout(icon_view)) {
    return;
  ...
  EEL_CALL_METHOD(FM_ICON_VIEW_CLASS, icon_view, set_directory_sort_reversed,
                  (icon_view, file, sort_reversed));
}

static void fm_icon_view_real_set_directory_sort_reversed(
    FMIconView *icon_view, CajaFile *file, gboolean sort_reversed) {
  gboolean reversed;

  get_default_sort_order(file, &reversed);
  caja_file_set_boolean_metadata(
      file, CAJA_METADATA_KEY_ICON_VIEW_SORT_REVERSED, reversed, sort_reversed);
}

static gboolean get_default_directory_keep_aligned(void) { return TRUE; }

static gboolean fm_icon_view_get_directory_keep_aligned(FMIconView *icon_view,
                                                        CajaFile *file) {
...
static void fm_icon_view_set_directory_keep_aligned(FMIconView *icon_view,
                                                    CajaFile *file,
  ...
  caja_file_set_boolean_metadata(file, CAJA_METADATA_KEY_ICON_VIEW_KEEP_ALIGNED,
                                 get_default_directory_keep_aligned(),
                                 keep_aligned);
}

static gboolean get_default_directory_lock_icons_position(void) {
  return FALSE;
}

static gboolean fm_icon_view_get_directory_lock_icons_position(
    FMIconView *icon_view, CajaFile *file) {
  if (!fm_icon_view_supports_manual_layout(icon_view)) {
    return FALSE;
  }
...
static void fm_icon_view_set_directory_lock_icons_position(
    FMIconView *icon_view, CajaFile *file, gboolean lock_icons_position) {
  if (!fm_icon_view_supports_manual_layout(icon_view)) {
    return;
  }
  ...
  caja_file_set_boolean_metadata(
      file, CAJA_METADATA_KEY_ICON_VIEW_LOCK_ICONS_POSITION,
      get_default_directory_lock_icons_position(), lock_icons_position);
}

static gboolean fm_icon_view_get_directory_auto_layout(FMIconView *icon_view,
                                                       CajaFile *file) {
  if (!fm_icon_view_supports_auto_layout(icon_view)) {
    return FALSE;
  }
  ...
  return EEL_CALL_METHOD_WITH_RETURN_VALUE(FM_ICON_VIEW_CLASS, icon_view,
                                           get_directory_auto_layout,
                                           (icon_view, file));
}

static gboolean fm_icon_view_real_get_directory_auto_layout(
    FMIconView *icon_view, CajaFile *file) {
  return caja_file_get_boolean_metadata(
      file, CAJA_METADATA_KEY_ICON_VIEW_AUTO_LAYOUT, TRUE);
}

static void fm_icon_view_set_directory_auto_layout(FMIconView *icon_view,
                                                   CajaFile *file,
  ...
  EEL_CALL_METHOD(FM_ICON_VIEW_CLASS, icon_view, set_directory_auto_layout,
                  (icon_view, file, auto_layout));
}

static void fm_icon_view_real_set_directory_auto_layout(FMIconView *icon_view,
                                                        CajaFile *file,
                                                        gboolean auto_layout) {
  if (!fm_icon_view_supports_manual_layout(icon_view)) {
    return;
  ...
  caja_file_set_boolean_metadata(file, CAJA_METADATA_KEY_ICON_VIEW_AUTO_LAYOUT,
                                 TRUE, auto_layout);
}
...
static gboolean fm_icon_view_get_directory_tighter_layout(FMIconView *icon_view,
                                                          CajaFile *file) {
  return EEL_CALL_METHOD_WITH_RETURN_VALUE(FM_ICON_VIEW_CLASS, icon_view,
                                           get_directory_tighter_layout,
                                           (icon_view, file));
}

static gboolean default_directory_tighter_layout = FALSE;

static gboolean get_default_directory_tighter_layout(void) {
  static gboolean auto_storaged_added = FALSE;
...
static gboolean fm_icon_view_real_get_directory_tighter_layout(
    FMIconView *icon_view, CajaFile *file) {
...
static void fm_icon_view_set_directory_tighter_layout(FMIconView *icon_view,
                                                      CajaFile *file,
                                                      gboolean tighter_layout) {
  EEL_CALL_METHOD(FM_ICON_VIEW_CLASS, icon_view, set_directory_tighter_layout,
                  (icon_view, file, tighter_layout));
}

static void fm_icon_view_real_set_directory_tighter_layout(
    FMIconView *icon_view, CajaFile *file, gboolean tighter_layout) {
  if (!fm_icon_view_supports_tighter_layout(icon_view)) {
    return;
  }
  ...
  caja_file_set_boolean_metadata(
      file, CAJA_METADATA_KEY_ICON_VIEW_TIGHTER_LAYOUT,
      get_default_directory_tighter_layout(), tighter_layout);
}
...
static gboolean real_supports_manual_layout(FMIconView *view) {
  g_return_val_if_fail(FM_IS_ICON_VIEW(view), FALSE);
...
static const SortCriterion *get_sort_criterion_by_metadata_text(
    const char *metadata_text) {
  guint i;
...
#define DEFAULT_ZOOM_LEVEL(icon_view) \
  icon_view->details->compact ? default_compact_zoom_level : default_zoom_level

static CajaZoomLevel get_default_zoom_level(FMIconView *icon_view) {
...
static void set_labels_beside_icons(FMIconView *icon_view) {
  gboolean labels_beside;

  if (fm_icon_view_supports_labels_beside_icons(icon_view)) {
    labels_beside =
        fm_icon_view_is_compact(icon_view) ||
        g_settings_get_boolean(caja_icon_view_preferences,
                               CAJA_PREFERENCES_ICON_VIEW_LABELS_BESIDE_ICONS);

    if (labels_beside) {
      caja_icon_container_set_label_position(get_icon_container(icon_view),
...
static void set_columns_same_width(FMIconView *icon_view) {
  gboolean all_columns_same_width;

  if (fm_icon_view_is_compact(icon_view)) {
    all_columns_same_width = g_settings_get_boolean(
        caja_compact_view_preferences,
        CAJA_PREFERENCES_COMPACT_VIEW_ALL_COLUMNS_SAME_WIDTH);
    caja_icon_container_set_all_columns_same_width(
        get_icon_container(icon_view), all_columns_same_width);
  }
}
...
static void fm_icon_view_begin_loading(FMDirectoryView *view) {
   ...
   */
  if (FM_IS_DESKTOP_ICON_VIEW(view)) {
    caja_connect_desktop_background_to_settings(
        CAJA_ICON_CONTAINER(icon_container));
  } else {
    GdkDragAction default_action;

    if (caja_window_info_get_window_type(fm_directory_view_get_caja_window(
            view)) == CAJA_WINDOW_NAVIGATION) {
      default_action = CAJA_DND_ACTION_SET_AS_GLOBAL_BACKGROUND;
    } else {
      default_action = CAJA_DND_ACTION_SET_AS_FOLDER_BACKGROUND;
    }

    caja_connect_background_to_file_metadata(icon_container, file,
                                             default_action);
  }
  ...
  if (fm_directory_view_supports_zooming(FM_DIRECTORY_VIEW(icon_view))) {
    ...
    if (icon_view->details->compact) {
      level = caja_file_get_integer_metadata(
          file, CAJA_METADATA_KEY_COMPACT_VIEW_ZOOM_LEVEL,
          get_default_zoom_level(icon_view));
    } else {
      level = caja_file_get_integer_metadata(
          file, CAJA_METADATA_KEY_ICON_VIEW_ZOOM_LEVEL,
          get_default_zoom_level(icon_view));
    }
  ...
  set_sort_reversed(icon_view,
                    fm_icon_view_get_directory_sort_reversed(icon_view, file));
...
static void icon_view_notify_clipboard_info(CajaClipboardMonitor *monitor,
...
static void fm_icon_view_end_loading(FMDirectoryView *view,
  ...
  icon_container = GTK_WIDGET(get_icon_container(icon_view));
  caja_icon_container_end_loading(CAJA_ICON_CONTAINER(icon_container),
                                  all_files_seen);
...
static CajaZoomLevel fm_icon_view_get_zoom_level(FMDirectoryView *view) {
  g_return_val_if_fail(FM_IS_ICON_VIEW(view), CAJA_ZOOM_LEVEL_STANDARD);

  return caja_icon_container_get_zoom_level(
...
static void fm_icon_view_set_zoom_level(FMIconView *view,
  ...
  if (view->details->compact) {
    caja_file_set_integer_metadata(
        ...
        CAJA_METADATA_KEY_COMPACT_VIEW_ZOOM_LEVEL, get_default_zoom_level(view),
        new_level);
  } else {
    caja_file_set_integer_metadata(
        fm_directory_view_get_directory_as_file(FM_DIRECTORY_VIEW(view)),
        CAJA_METADATA_KEY_ICON_VIEW_ZOOM_LEVEL, get_default_zoom_level(view),
        new_level);
  }
...
static void fm_icon_view_zoom_to_level(FMDirectoryView *view,
...
static void fm_icon_view_restore_default_zoom_level(FMDirectoryView *view) {
  FMIconView *icon_view;

  g_return_if_fail(FM_IS_ICON_VIEW(view));

  icon_view = FM_ICON_VIEW(view);
  fm_directory_view_zoom_to_level(view, get_default_zoom_level(icon_view));
}

static gboolean fm_icon_view_can_zoom_in(FMDirectoryView *view) {
  g_return_val_if_fail(FM_IS_ICON_VIEW(view), FALSE);

  return fm_icon_view_get_zoom_level(view) < CAJA_ZOOM_LEVEL_LARGEST;
}

static gboolean fm_icon_view_can_zoom_out(FMDirectoryView *view) {
  g_return_val_if_fail(FM_IS_ICON_VIEW(view), FALSE);

  return fm_icon_view_get_zoom_level(view) > CAJA_ZOOM_LEVEL_SMALLEST;
}

static GtkWidget *fm_icon_view_get_background_widget(FMDirectoryView *view) {
...
static gboolean fm_icon_view_is_empty(FMDirectoryView *view) {
  ...
  return caja_icon_container_is_empty(get_icon_container(FM_ICON_VIEW(view)));
}
...
static GList *fm_icon_view_get_selection(FMDirectoryView *view) {
  ...
  list =
      caja_icon_container_get_selection(get_icon_container(FM_ICON_VIEW(view)));
  caja_file_list_ref(list);
  return list;
}

static void count_item(CajaIconData *icon_data, gpointer callback_data) {
  guint *count;
...
static guint fm_icon_view_get_item_count(FMDirectoryView *view) {
  guint count;
  ...
  caja_icon_container_for_each(get_icon_container(FM_ICON_VIEW(view)),
                               count_item, &count);
...
static void action_reversed_order_callback(GtkAction *action,
                                           gpointer user_data) {
  FMIconView *icon_view;

  icon_view = FM_ICON_VIEW(user_data);

  if (set_sort_reversed(
          icon_view, gtk_toggle_action_get_active(GTK_TOGGLE_ACTION(action)))) {
    caja_icon_container_sort(get_icon_container(icon_view));
    fm_icon_view_reveal_selection(FM_DIRECTORY_VIEW(icon_view));
  }
}

static void action_keep_aligned_callback(GtkAction *action,
                                         gpointer user_data) {
  FMIconView *icon_view;
  CajaFile *file;
  gboolean keep_aligned;
  ...
  file = fm_directory_view_get_directory_as_file(FM_DIRECTORY_VIEW(icon_view));
  fm_icon_view_set_directory_keep_aligned(icon_view, file, keep_aligned);

  caja_icon_container_set_keep_aligned(get_icon_container(icon_view),
                                       keep_aligned);
}

static void action_lock_icons_position_callback(GtkAction *action,
                                                gpointer user_data) {
  FMIconView *icon_view;
  CajaFile *file;
  ...
  file = fm_directory_view_get_directory_as_file(FM_DIRECTORY_VIEW(icon_view));
  fm_icon_view_set_directory_lock_icons_position(icon_view, file,
                                                 lock_icons_position);
...
static void switch_to_manual_layout(FMIconView *icon_view) {
  ...
  caja_icon_container_set_auto_layout(get_icon_container(icon_view), FALSE);
}

static void layout_changed_callback(CajaIconContainer *container,
                                    FMIconView *icon_view) {
  CajaFile *file;
  ...
  if (file != NULL) {
    fm_icon_view_set_directory_auto_layout(
        icon_view, file, fm_icon_view_using_auto_layout(icon_view));
    fm_icon_view_set_directory_tighter_layout(
        icon_view, file, fm_icon_view_using_tighter_layout(icon_view));
  }

  update_layout_menus(icon_view);
}

static gboolean fm_icon_view_can_rename_file(FMDirectoryView *view,
                                             CajaFile *file) {
  if (!(fm_icon_view_get_zoom_level(view) > CAJA_ZOOM_LEVEL_SMALLEST)) {
    return FALSE;
  }

  return FM_DIRECTORY_VIEW_CLASS(fm_icon_view_parent_class)
      ->can_rename_file(view, file);
}

static void fm_icon_view_start_renaming_file(FMDirectoryView *view,
                                             CajaFile *file,
                                             gboolean select_all) {
  /* call parent class to make sure the right icon is selected */
  FM_DIRECTORY_VIEW_CLASS(fm_icon_view_parent_class)
...
static const GtkToggleActionEntry icon_view_toggle_entries[] = {
    /* name, stock id */ {
        ...
        G_CALLBACK(action_tighter_layout_callback), 0},
    /* name, stock id */
    {"Reversed Order", NULL,
     /* label, accelerator */ N_("Re_versed Order"), NULL,
     /* tooltip */ N_("Display icons in the opposite order"),
     G_CALLBACK(action_reversed_order_callback), 0},
    /* name, stock id */
    {"Keep Aligned", NULL,
     /* label, accelerator */ N_("_Keep Aligned"), NULL,
     /* tooltip */ N_("Keep icons lined up on a grid"),
     G_CALLBACK(action_keep_aligned_callback), 0},
    /* name, stock id */
    {"Lock Icons Position", NULL,
     /* label, accelerator */ N_("_Lock Icons Position"), NULL,
     /* tooltip */ N_("Prevent repositioning icons"),
     G_CALLBACK(action_lock_icons_position_callback), 0},
};

static const GtkRadioActionEntry arrange_radio_entries[] = {
    {"Manual Layout", NULL, N_("_Manually"), NULL,
     N_("Leave icons wherever they are dropped"), CAJA_FILE_SORT_NONE},
    {"Sort by Name", NULL, N_("By _Name"), NULL,
     N_("Keep icons sorted by name in rows"), CAJA_FILE_SORT_BY_DISPLAY_NAME},
    {"Sort by Size", NULL, N_("By _Size"), NULL,
     N_("Keep icons sorted by size in rows"), CAJA_FILE_SORT_BY_SIZE},
    {"Sort by Size on Disk", NULL, N_("By S_ize on Disk"), NULL,
     N_("Keep icons sorted by disk usage in rows"),
     CAJA_FILE_SORT_BY_SIZE_ON_DISK},
    {"Sort by Type", NULL, N_("By _Type"), NULL,
     N_("Keep icons sorted by type in rows"), CAJA_FILE_SORT_BY_TYPE},
    {"Sort by Modification Date", NULL, N_("By Modification _Date"), NULL,
     N_("Keep icons sorted by modification date in rows"),
     CAJA_FILE_SORT_BY_MTIME},
    {"Sort by Creation Date", NULL, N_("By _Creation Date"), NULL,
     N_("Keep icons sorted by creation date in rows"), CAJA_FILE_SORT_BY_BTIME},
    {"Sort by Emblems", NULL, N_("By _Emblems"), NULL,
     N_("Keep icons sorted by emblems in rows"), CAJA_FILE_SORT_BY_EMBLEMS},
    {"Sort by Trash Time", NULL, N_("By T_rash Time"), NULL,
     N_("Keep icons sorted by trash time in rows"),
     CAJA_FILE_SORT_BY_TRASHED_TIME},
    {"Sort by Extension", NULL, N_("By E_xtension"), NULL,
...
static void fm_icon_view_merge_menus(FMDirectoryView *view) {
  ...
  gtk_action_group_add_actions(action_group, icon_view_entries,
                               G_N_ELEMENTS(icon_view_entries), icon_view);
  gtk_action_group_add_toggle_actions(action_group, icon_view_toggle_entries,
                                      G_N_ELEMENTS(icon_view_toggle_entries),
                                      icon_view);
  gtk_action_group_add_radio_actions(
      action_group, arrange_radio_entries, G_N_ELEMENTS(arrange_radio_entries),
      -1, G_CALLBACK(action_sort_radio_callback), icon_view);
  ...
  if (!fm_icon_view_supports_auto_layout(icon_view)) {
    ...
    action = gtk_action_group_get_action(action_group, FM_ACTION_ARRANGE_ITEMS);
    gtk_action_set_visible(action, FALSE);
  }

  if (fm_icon_view_supports_scaling(icon_view)) {
    gtk_ui_manager_add_ui(ui_manager, icon_view->details->icon_merge_id,
                          POPUP_PATH_ICON_APPEARANCE, FM_ACTION_STRETCH,
                          FM_ACTION_STRETCH, GTK_UI_MANAGER_MENUITEM, FALSE);
    gtk_ui_manager_add_ui(ui_manager, icon_view->details->icon_merge_id,
                          POPUP_PATH_ICON_APPEARANCE, FM_ACTION_UNSTRETCH,
                          FM_ACTION_UNSTRETCH, GTK_UI_MANAGER_MENUITEM, FALSE);
  }

  update_layout_menus(icon_view);
}

static void fm_icon_view_unmerge_menus(FMDirectoryView *view) {
  FMIconView *icon_view;
  GtkUIManager *ui_manager;
  ...
  if (ui_manager != NULL) {
    caja_ui_unmerge_ui(ui_manager, &icon_view->details->icon_merge_id,
...
static void fm_icon_view_update_menus(FMDirectoryView *view) {
  ...
  gtk_action_set_sensitive(
      action, selection_count == 1 && icon_container != NULL &&
                  !caja_icon_container_has_stretch_handles(icon_container));

  gtk_action_set_visible(action, fm_icon_view_supports_scaling(icon_view));
...
static void fm_icon_view_reset_to_defaults(FMDirectoryView *view) {
  CajaIconContainer *icon_container;
  FMIconView *icon_view;
  ...
  caja_icon_container_set_keep_aligned(icon_container,
                                       get_default_directory_keep_aligned());
  caja_icon_container_set_lock_icons_position(
...
static void fm_icon_view_reveal_selection(FMDirectoryView *view) {
  ...
  if (selection != NULL) {
    caja_icon_container_reveal(get_icon_container(FM_ICON_VIEW(view)),
                               selection->data);
  }
...
static GArray *fm_icon_view_get_selected_icon_locations(FMDirectoryView *view) {
  g_return_val_if_fail(FM_IS_ICON_VIEW(view), NULL);
...
static void fm_icon_view_set_selection(FMDirectoryView *view,
  ...
  caja_icon_container_set_selection(get_icon_container(FM_ICON_VIEW(view)),
                                    selection);
}

static void fm_icon_view_invert_selection(FMDirectoryView *view) {
  g_return_if_fail(FM_IS_ICON_VIEW(view));

  caja_icon_container_invert_selection(get_icon_container(FM_ICON_VIEW(view)));
}

static gboolean fm_icon_view_using_manual_layout(FMDirectoryView *view) {
  g_return_val_if_fail(FM_IS_ICON_VIEW(view), FALSE);

  return !fm_icon_view_using_auto_layout(FM_ICON_VIEW(view));
}

static void fm_icon_view_widget_to_file_operation_position(
...
static void icon_container_activate_callback(CajaIconContainer *container,
                                             GList *file_list,
                                             FMIconView *icon_view) {
  g_assert(FM_IS_ICON_VIEW(icon_view));
  g_assert(container == get_icon_container(icon_view));
...
static void icon_container_activate_alternate_callback(
    CajaIconContainer *container, GList *file_list, FMIconView *icon_view) {
  ...
  fm_directory_view_activate_files(FM_DIRECTORY_VIEW(icon_view), file_list,
                                   CAJA_WINDOW_OPEN_ACCORDING_TO_MODE, flags,
                                   TRUE);
}

static void band_select_started_callback(CajaIconContainer *container,
                                         FMIconView *icon_view) {
  g_assert(FM_IS_ICON_VIEW(icon_view));
  g_assert(container == get_icon_container(icon_view));

  fm_directory_view_start_batching_selection_changes(
      FM_DIRECTORY_VIEW(icon_view));
}

static void band_select_ended_callback(CajaIconContainer *container,
                                       FMIconView *icon_view) {
  g_assert(FM_IS_ICON_VIEW(icon_view));
  g_assert(container == get_icon_container(icon_view));

  fm_directory_view_stop_batching_selection_changes(
      FM_DIRECTORY_VIEW(icon_view));
}

/* handle the preview signal by inspecting the mime type.  For now, we only
 * preview local sound files. */
...
/* here's the timer task that actually plays the file using mpg123, ogg123 or
 * play. */
/* FIXME bugzilla.gnome.org 41258: we should get the application from our
 * mime-type stuff */
static gboolean play_file(gpointer callback_data) {
  CajaFile *file;
  FMIconView *icon_view;
  GPid child_pid;
  char **argv;
  ...
  if (!g_spawn_async_with_pipes(
          ...
          NULL /* user_data */, &child_pid, NULL, NULL, NULL, &error)) {
    g_strfreev(argv);
  ...
  icon_view->details->audio_preview_child_watch =
      g_child_watch_add(child_pid, audio_child_died, NULL);
  icon_view->details->audio_preview_child_pid = child_pid;

  return FALSE;
}

/* FIXME bugzilla.gnome.org 42530: Hardcoding this here sucks. We should be
 * using components for open ended things like this.
 */

/* this routine is invoked from the preview signal handler to preview a sound
   file.  We want to wait a suitable delay until we actually do it, so set up a
   timer task to actually start playing.  If we move out before the task files,
   we remove it. */
...
static void preview_audio(FMIconView *icon_view, CajaFile *file,
  ...
  if (start_flag) {
    ...
    icon_view->details->audio_preview_timeout =
        g_timeout_add_seconds(1, play_file, icon_view);
  }
}

static gboolean sound_preview_type_supported(CajaFile *file) {
  char *mime_type;
  guint i;
...
static gboolean should_preview_sound(CajaFile *file) {
  GFile *location;
...
static int icon_container_preview_callback(CajaIconContainer *container,
                                           CajaFile *file, gboolean start_flag,
                                           FMIconView *icon_view) {
  int result;
   ...
   */
  if (fm_icon_view_get_zoom_level(FM_DIRECTORY_VIEW(icon_view)) <=
...
static void renaming_icon_callback(CajaIconContainer *container,
                                   GtkWidget *widget, gpointer callback_data) {
  FMDirectoryView *directory_view;

  directory_view = FM_DIRECTORY_VIEW(callback_data);
  caja_clipboard_set_up_editable(
      GTK_EDITABLE(widget), fm_directory_view_get_ui_manager(directory_view),
      FALSE);
}

int fm_icon_view_compare_files(FMIconView *icon_view, CajaFile *a,
                               CajaFile *b) {
  return caja_file_compare_for_sort(
      a, b, icon_view->details->sort->sort_type,
      /* Use type-unsafe cast for performance */
      fm_directory_view_should_sort_directories_first(
          (FMDirectoryView *)icon_view),
      icon_view->details->sort_reversed);
}

static int compare_files(FMDirectoryView *icon_view, CajaFile *a, CajaFile *b) {
...
static void fm_icon_view_screen_changed(GtkWidget *widget,
                                        GdkScreen *previous_screen) {
  FMDirectoryView *view;
  GList *files, *l;

  if (GTK_WIDGET_CLASS(fm_icon_view_parent_class)->screen_changed) {
    GTK_WIDGET_CLASS(fm_icon_view_parent_class)
...
static gboolean fm_icon_view_scroll_event(GtkWidget *widget,
  ...
  if (icon_view->details->compact) {
    if ((ret = fm_directory_view_handle_scroll_event(
             FM_DIRECTORY_VIEW(icon_view), scroll_event)))
      return ret;
  ...
  return GTK_WIDGET_CLASS(fm_icon_view_parent_class)
      ->scroll_event(widget, scroll_event);
}

static void selection_changed_callback(CajaIconContainer *container,
                                       FMIconView *icon_view) {
  g_assert(FM_IS_ICON_VIEW(icon_view));
  g_assert(container == get_icon_container(icon_view));
...
static void icon_container_context_click_selection_callback(
    CajaIconContainer *container, GdkEventButton *event,
    FMIconView *icon_view) {
  g_assert(CAJA_IS_ICON_CONTAINER(container));
  g_assert(FM_IS_ICON_VIEW(icon_view));
...
static void icon_container_context_click_background_callback(
    CajaIconContainer *container, GdkEventButton *event,
    FMIconView *icon_view) {
  g_assert(CAJA_IS_ICON_CONTAINER(container));
  g_assert(FM_IS_ICON_VIEW(icon_view));
...
static void icon_position_changed_callback(CajaIconContainer *container,
                                           CajaFile *file,
  ...
  if (fm_directory_view_get_active(FM_DIRECTORY_VIEW(icon_view)) &&
    ...
    icon_view->details->react_to_icon_change_idle_id =
        g_idle_add(fm_icon_view_react_to_icon_change_idle_callback, icon_view);
  }
  ...
  if (!fm_icon_view_using_auto_layout(icon_view)) {
    ...
    position_string = g_strdup_printf("%d,%d", position->x, position->y);
    caja_file_set_metadata(file, CAJA_METADATA_KEY_ICON_POSITION, NULL,
                           position_string);
    g_free(position_string);
  }

  g_ascii_dtostr(scale_string, sizeof(scale_string), position->scale);
  caja_file_set_metadata(file, CAJA_METADATA_KEY_ICON_SCALE, "1.0",
                         scale_string);
}

/* Attempt to change the filename to the new text.  Notify user if operation
 * fails. */
static void fm_icon_view_icon_text_changed_callback(
    CajaIconContainer *container, CajaFile *file, char *new_name,
    FMIconView *icon_view) {
  g_assert(CAJA_IS_FILE(file));
...
static char *get_icon_uri_callback(CajaIconContainer *container, CajaFile *file,
                                   FMIconView *icon_view) {
  g_assert(CAJA_IS_ICON_CONTAINER(container));
  g_assert(CAJA_IS_FILE(file));
  g_assert(FM_IS_ICON_VIEW(icon_view));
...
static char *get_icon_drop_target_uri_callback(CajaIconContainer *container,
                                               CajaFile *file,
                                               FMIconView *icon_view) {
...
static void fm_icon_view_text_attribute_names_changed(
    FMDirectoryView *directory_view) {
  g_assert(FM_IS_ICON_VIEW(directory_view));

  caja_icon_container_request_update_all(
      get_icon_container(FM_ICON_VIEW(directory_view)));
}

static void fm_icon_view_embedded_text_policy_changed(
    FMDirectoryView *directory_view) {
  g_assert(FM_IS_ICON_VIEW(directory_view));
...
static void fm_icon_view_image_display_policy_changed(
    FMDirectoryView *directory_view) {
  g_assert(FM_IS_ICON_VIEW(directory_view));
...
static void fm_icon_view_click_policy_changed(FMDirectoryView *directory_view) {
  g_assert(FM_IS_ICON_VIEW(directory_view));

  fm_icon_view_update_click_mode(FM_ICON_VIEW(directory_view));
}

static void fm_icon_view_emblems_changed(FMDirectoryView *directory_view) {
  g_assert(FM_IS_ICON_VIEW(directory_view));
...
static void default_use_tighter_layout_changed_callback(
    gpointer callback_data) {
  FMIconView *icon_view;
  CajaFile *file;
  CajaIconContainer *icon_container;
...
static void fm_icon_view_sort_directories_first_changed(
    FMDirectoryView *directory_view) {
  FMIconView *icon_view;
...
static gboolean icon_view_can_accept_item(CajaIconContainer *container,
...
static char *icon_view_get_container_uri(CajaIconContainer *container,
                                         FMDirectoryView *view) {
  return fm_directory_view_get_uri(view);
}

static void icon_view_move_copy_items(CajaIconContainer *container,
                                      const GList *item_uris,
                                      GArray *relative_item_points,
                                      const char *target_dir, int copy_action,
                                      int x, int y, FMDirectoryView *view) {
  caja_clipboard_clear_if_colliding_uris(
      GTK_WIDGET(view), item_uris,
...
static void fm_icon_view_update_click_mode(FMIconView *icon_view) {
  ...
  click_mode =
      g_settings_get_enum(caja_preferences, CAJA_PREFERENCES_CLICK_POLICY);

  caja_icon_container_set_single_click_mode(
      icon_container, click_mode == CAJA_CLICK_POLICY_SINGLE);
}

static gboolean get_stored_layout_timestamp(CajaIconContainer *container,
  ...
  if (icon_data == NULL) {
    ...
    *timestamp = caja_file_get_time_metadata(
        file, CAJA_METADATA_KEY_ICON_VIEW_LAYOUT_TIMESTAMP);
    caja_file_unref(file);
  } else {
    *timestamp = caja_file_get_time_metadata(
        CAJA_FILE(icon_data), CAJA_METADATA_KEY_ICON_POSITION_TIMESTAMP);
  }

  return TRUE;
}

static gboolean store_layout_timestamp(CajaIconContainer *container,
                                       CajaIconData *icon_data,
  ...
  if (icon_data == NULL) {
    ...
    caja_file_set_time_metadata(
        file, CAJA_METADATA_KEY_ICON_VIEW_LAYOUT_TIMESTAMP, (time_t)*timestamp);
    caja_file_unref(file);
  } else {
...
static gboolean focus_in_event_callback(GtkWidget *widget, GdkEventFocus *event,
                                        gpointer user_data) {
...
static CajaIconContainer *create_icon_container(FMIconView *icon_view) {
  CajaIconContainer *icon_container;
  ...
  g_signal_connect_object(
      icon_container, "activate_alternate",
  ...
  g_signal_connect_object(icon_container, "band_select_started",
                          G_CALLBACK(band_select_started_callback), icon_view,
                          0);
  g_signal_connect_object(icon_container, "band_select_ended",
                          G_CALLBACK(band_select_ended_callback), icon_view, 0);
  g_signal_connect_object(
      icon_container, "context_click_selection",
      G_CALLBACK(icon_container_context_click_selection_callback), icon_view,
      0);
  g_signal_connect_object(
      icon_container, "context_click_background",
      G_CALLBACK(icon_container_context_click_background_callback), icon_view,
      0);
  g_signal_connect_object(icon_container, "icon_position_changed",
                          G_CALLBACK(icon_position_changed_callback), icon_view,
                          0);
  ...
  g_signal_connect_object(icon_container, "get_container_uri",
                          G_CALLBACK(icon_view_get_container_uri), icon_view,
                          0);
  ...
  g_signal_connect_object(icon_container, "get_stored_layout_timestamp",
                          G_CALLBACK(get_stored_layout_timestamp), icon_view,
                          0);
...
static void icon_view_handle_netscape_url(CajaIconContainer *container,
                                          const char *encoded_url,
                                          const char *target_uri,
                                          GdkDragAction action, int x, int y,
                                          FMIconView *view) {
  fm_directory_view_handle_netscape_url_drop(
      FM_DIRECTORY_VIEW(view), encoded_url, target_uri, action, x, y);
}

static void icon_view_handle_uri_list(CajaIconContainer *container,
                                      const char *item_uris,
                                      const char *target_uri,
                                      GdkDragAction action, int x, int y,
                                      FMIconView *view) {
  fm_directory_view_handle_uri_list_drop(FM_DIRECTORY_VIEW(view), item_uris,
                                         target_uri, action, x, y);
}

static void icon_view_handle_text(CajaIconContainer *container,
                                  const char *text, const char *target_uri,
                                  GdkDragAction action, int x, int y,
                                  FMIconView *view) {
  fm_directory_view_handle_text_drop(FM_DIRECTORY_VIEW(view), text, target_uri,
                                     action, x, y);
}

static void icon_view_handle_raw(CajaIconContainer *container,
                                 const char *raw_data, int length,
                                 const char *target_uri,
                                 const char *direct_save_uri,
                                 GdkDragAction action, int x, int y,
                                 FMIconView *view) {
  fm_directory_view_handle_raw_drop(FM_DIRECTORY_VIEW(view), raw_data, length,
                                    target_uri, direct_save_uri, action, x, y);
}

static char *icon_view_get_first_visible_file(CajaView *view) {
  CajaFile *file;
  FMIconView *icon_view;

  icon_view = FM_ICON_VIEW(view);

  file = CAJA_FILE(caja_icon_container_get_first_visible_icon(
      get_icon_container(icon_view)));
...
static void icon_view_scroll_to_file(CajaView *view, const char *uri) {
  FMIconView *icon_view;
...
static void fm_icon_view_set_property(GObject *object, guint prop_id,
  ...
  switch (prop_id) {
    case PROP_COMPACT:
      ...
      if (icon_view->details->compact) {
        caja_icon_container_set_layout_mode(
            get_icon_container(icon_view),
...
static void fm_icon_view_class_init(FMIconViewClass *klass) {
  ...
  fm_directory_view_class->get_background_widget =
      fm_icon_view_get_background_widget;
  fm_directory_view_class->get_selected_icon_locations =
      fm_icon_view_get_selected_icon_locations;
  fm_directory_view_class->get_selection = fm_icon_view_get_selection;
  fm_directory_view_class->get_selection_for_file_transfer =
      fm_icon_view_get_selection;
  ...
  fm_directory_view_class->restore_default_zoom_level =
  ...
  fm_directory_view_class->click_policy_changed =
      fm_icon_view_click_policy_changed;
  fm_directory_view_class->embedded_text_policy_changed =
      fm_icon_view_embedded_text_policy_changed;
  fm_directory_view_class->emblems_changed = fm_icon_view_emblems_changed;
  fm_directory_view_class->image_display_policy_changed =
      fm_icon_view_image_display_policy_changed;
  ...
  fm_directory_view_class->sort_directories_first_changed =
      fm_icon_view_sort_directories_first_changed;
  fm_directory_view_class->start_renaming_file =
  ...
  fm_directory_view_class->widget_to_file_operation_position =
      fm_icon_view_widget_to_file_operation_position;
...
static const char *fm_icon_view_get_id(CajaView *view) {
  if (FM_IS_DESKTOP_ICON_VIEW(view)) {
...
static void fm_icon_view_init(FMIconView *icon_view) {
  ...
  g_signal_connect_swapped(
      caja_preferences, "changed::" CAJA_PREFERENCES_DEFAULT_SORT_ORDER,
      G_CALLBACK(default_sort_order_changed_callback), icon_view);
  g_signal_connect_swapped(
      caja_preferences,
      "changed::" CAJA_PREFERENCES_DEFAULT_SORT_IN_REVERSE_ORDER,
      G_CALLBACK(default_sort_in_reverse_order_changed_callback), icon_view);
  g_signal_connect_swapped(
      caja_icon_view_preferences,
      "changed::" CAJA_PREFERENCES_ICON_VIEW_DEFAULT_USE_TIGHTER_LAYOUT,
      G_CALLBACK(default_use_tighter_layout_changed_callback), icon_view);
  g_signal_connect_swapped(
      caja_icon_view_preferences,
      "changed::" CAJA_PREFERENCES_ICON_VIEW_DEFAULT_ZOOM_LEVEL,
      G_CALLBACK(default_zoom_level_changed_callback), icon_view);
  g_signal_connect_swapped(
      caja_icon_view_preferences,
      "changed::" CAJA_PREFERENCES_ICON_VIEW_LABELS_BESIDE_ICONS,
      G_CALLBACK(labels_beside_icons_changed_callback), icon_view);
  ...
  g_signal_connect_swapped(
      ...
      G_CALLBACK(all_columns_same_width_changed_callback), icon_view);

  g_signal_connect_object(get_icon_container(icon_view), "handle_netscape_url",
                          G_CALLBACK(icon_view_handle_netscape_url), icon_view,
                          0);
  ...
  icon_view->details->clipboard_handler_id =
      g_signal_connect(caja_clipboard_monitor_get(), "clipboard_info",
                       G_CALLBACK(icon_view_notify_clipboard_info), icon_view);
}

static CajaView *fm_icon_view_create(CajaWindowSlotInfo *slot) {
  FMIconView *view;

  view = g_object_new(FM_TYPE_ICON_VIEW, "window-slot", slot, "compact", FALSE,
                      NULL);
...
static CajaView *fm_compact_view_create(CajaWindowSlotInfo *slot) {
  FMIconView *view;

  view = g_object_new(FM_TYPE_ICON_VIEW, "window-slot", slot, "compact", TRUE,
                      NULL);
...
static gboolean fm_icon_view_supports_uri(const char *uri, GFileType file_type,
...
#define TRANSLATE_VIEW_INFO(view_info)                              \
  ...
  view_info.view_menu_label_with_mnemonic =                         \
      _(view_info.view_menu_label_with_mnemonic);                   \
...
gboolean fm_icon_view_is_compact(FMIconView *view) {
