...
static void destroy_node_without_reporting(FMTreeModel *model, TreeNode *node);
static void report_node_contents_changed(FMTreeModel *model, TreeNode *node);

G_DEFINE_TYPE_WITH_CODE(FMTreeModel, fm_tree_model, G_TYPE_OBJECT,
                        G_IMPLEMENT_INTERFACE(GTK_TYPE_TREE_MODEL,
                                              fm_tree_model_tree_model_init));

static GtkTreeModelFlags fm_tree_model_get_flags(GtkTreeModel *tree_model) {
  return GTK_TREE_MODEL_ITERS_PERSIST;
}

static void object_unref_if_not_NULL(gpointer object) {
  if (object == NULL) {
    return;
  }
...
static FMTreeModelRoot *tree_model_root_new(FMTreeModel *model) {
  FMTreeModelRoot *root;
...
static TreeNode *tree_node_new(CajaFile *file, FMTreeModelRoot *root) {
  TreeNode *node;
...
static void tree_node_unparent(FMTreeModel *model, TreeNode *node) {
  ...
  if (parent == NULL && node == model->details->root_node) {
    /* it's the first root node -> if there is a next then let it be the first
     * root node */
...
static void tree_node_destroy(FMTreeModel *model, TreeNode *node) {
  ...
  if (node->closed_surface != NULL) cairo_surface_destroy(node->closed_surface);
  if (node->open_surface) cairo_surface_destroy(node->open_surface);
...
static void tree_node_parent(TreeNode *node, TreeNode *parent) {
  TreeNode *first_child;
...
static cairo_surface_t *get_menu_icon(GIcon *icon) {
  CajaIconInfo *info;
...
static cairo_surface_t *get_menu_icon_for_file(TreeNode *node, CajaFile *file,
  ...
  emblem = NULL;
  emblem_icons = caja_file_get_emblem_icons(node->file, emblems_to_ignore);
  ...
  highlight =
      (g_list_find_custom(model->details->highlighted_files, file,
                          (GCompareFunc)caja_file_compare_location) != NULL);
...
static cairo_surface_t *tree_node_get_surface(TreeNode *node,
                                              CajaFileIconFlags flags) {
  if (node->parent == NULL) {
...
static gboolean tree_node_update_surface(TreeNode *node,
...
static gboolean tree_node_update_closed_surface(TreeNode *node) {
  return tree_node_update_surface(node, &node->closed_surface, 0);
}

static gboolean tree_node_update_open_surface(TreeNode *node) {
  return tree_node_update_surface(node, &node->open_surface,
                                  CAJA_FILE_ICON_FLAGS_FOR_OPEN_FOLDER);
}

static gboolean tree_node_update_display_name(TreeNode *node) {
  char *display_name;
...
static cairo_surface_t *tree_node_get_closed_surface(TreeNode *node) {
...
static cairo_surface_t *tree_node_get_open_surface(TreeNode *node) {
  if (node->open_surface == NULL) {
    node->open_surface =
        tree_node_get_surface(node, CAJA_FILE_ICON_FLAGS_FOR_OPEN_FOLDER);
  }
  return node->open_surface;
}

static const char *tree_node_get_display_name(TreeNode *node) {
  if (node->display_name == NULL) {
...
static gboolean tree_node_has_dummy_child(TreeNode *node) {
...
static int tree_node_get_child_index(TreeNode *parent, TreeNode *child) {
  int i;
  TreeNode *node;
...
static gboolean make_iter_invalid(GtkTreeIter *iter) {
  iter->stamp = 0;
  iter->user_data = NULL;
...
static gboolean make_iter_for_node(TreeNode *node, GtkTreeIter *iter,
                                   int stamp) {
  if (node == NULL) {
...
static gboolean make_iter_for_dummy_row(TreeNode *parent, GtkTreeIter *iter,
                                        int stamp) {
  g_assert(tree_node_has_dummy_child(parent));
  g_assert(parent != NULL);
  iter->stamp = stamp;
...
static TreeNode *get_node_from_file(FMTreeModelRoot *root, CajaFile *file) {
  return g_hash_table_lookup(root->file_to_node_map, file);
}

static TreeNode *get_parent_node_from_file(FMTreeModelRoot *root,
                                           CajaFile *file) {
...
static TreeNode *create_node_for_file(FMTreeModelRoot *root, CajaFile *file) {
  TreeNode *node;
...
static char *get_node_uri(GtkTreeIter *iter) {
...
static void decrement_ref_count(FMTreeModel *model, TreeNode *node, int count) {
...
static void abandon_node_ref_count(FMTreeModel *model, TreeNode *node) {
  if (node->parent != NULL) {
    ...
    if (node->ref_count != 0) {
      ...
      g_message("abandoning %d ref of %s, count is now %d", node->ref_count,
                uri, node->parent->all_children_ref_count);
      g_free(uri);
    }
#endif
  }
...
static void abandon_dummy_row_ref_count(FMTreeModel *model, TreeNode *node) {
...
static void report_row_inserted(FMTreeModel *model, GtkTreeIter *iter) {
  GtkTreePath *path;
...
static void report_row_contents_changed(FMTreeModel *model, GtkTreeIter *iter) {
  GtkTreePath *path;
...
static void report_row_has_child_toggled(FMTreeModel *model,
                                         GtkTreeIter *iter) {
  GtkTreePath *path;
...
static GtkTreePath *get_node_path(FMTreeModel *model, TreeNode *node) {
  GtkTreeIter iter;
...
static void report_dummy_row_inserted(FMTreeModel *model, TreeNode *parent) {
  GtkTreeIter iter;
...
static void report_dummy_row_deleted(FMTreeModel *model, TreeNode *parent) {
  GtkTreeIter iter;
...
static void report_node_inserted(FMTreeModel *model, TreeNode *node) {
  GtkTreeIter iter;
  ...
  if (node->directory != NULL || node->parent == NULL) {
...
static void report_node_has_child_toggled(FMTreeModel *model, TreeNode *node) {
  GtkTreeIter iter;
...
static void report_dummy_row_contents_changed(FMTreeModel *model,
                                              TreeNode *parent) {
  GtkTreeIter iter;
...
static void stop_monitoring_directory(FMTreeModel *model, TreeNode *node) {
...
static void destroy_children_without_reporting(FMTreeModel *model,
                                               TreeNode *parent) {
...
static void destroy_node(FMTreeModel *model, TreeNode *node) {
  TreeNode *parent;
...
static void destroy_children(FMTreeModel *model, TreeNode *parent) {
  while (parent->first_child != NULL) {
    destroy_node(model, parent->first_child);
  }
}

static void destroy_children_by_function(FMTreeModel *model, TreeNode *parent,
                                         FilePredicate f) {
  TreeNode *child, *next;
...
static void destroy_by_function(FMTreeModel *model, FilePredicate f) {
  TreeNode *node;
...
static gboolean update_node_without_reporting(FMTreeModel *model,
                                              TreeNode *node) {
  gboolean changed;
...
static void insert_node(FMTreeModel *model, TreeNode *parent, TreeNode *node) {
...
static void reparent_node(FMTreeModel *model, TreeNode *node) {
  GtkTreePath *path;
  TreeNode *new_parent;
...
static gboolean should_show_file(FMTreeModel *model, CajaFile *file) {
  gboolean should;
  TreeNode *node;

  should = caja_file_should_show(file, model->details->show_hidden_files, TRUE,
                                 model->details->show_backup_files);

  if (should && model->details->show_only_directories &&
...
static void update_node(FMTreeModel *model, TreeNode *node) {
...
static void process_file_change(FMTreeModelRoot *root, CajaFile *file) {
  TreeNode *node, *parent;
...
static void files_changed_callback(CajaDirectory *directory,
                                   GList *changed_files,
...
static void set_done_loading(FMTreeModel *model, TreeNode *node,
...
static void done_loading_callback(CajaDirectory *directory,
  ...
  g_signal_emit(root->model, tree_model_signals[ROW_LOADED], 0, &iter);
}

static CajaFileAttributes get_tree_monitor_attributes(void) {
...
static void start_monitoring_directory(FMTreeModel *model, TreeNode *node) {
  CajaDirectory *directory;
  CajaFileAttributes attributes;
  ...
  node->done_loading_id = g_signal_connect(
      directory, "done_loading", G_CALLBACK(done_loading_callback), node->root);
  node->files_added_id = g_signal_connect(
      directory, "files_added", G_CALLBACK(files_changed_callback), node->root);
  node->files_changed_id =
  ...
  caja_directory_file_monitor_add(directory, model,
                                  model->details->show_hidden_files, attributes,
                                  files_changed_callback, node->root);
}

static int fm_tree_model_get_n_columns(GtkTreeModel *model) {
  return FM_TREE_MODEL_NUM_COLUMNS;
}

static GType fm_tree_model_get_column_type(GtkTreeModel *model, int index) {
  switch (index) {
...
static gboolean iter_is_valid(FMTreeModel *model, const GtkTreeIter *iter) {
  TreeNode *node, *parent;
...
static gboolean fm_tree_model_get_iter(GtkTreeModel *model, GtkTreeIter *iter,
                                       GtkTreePath *path) {
  int *indices;
  GtkTreeIter parent;
  int depth, i;
...
static GtkTreePath *fm_tree_model_get_path(GtkTreeModel *model,
                                           GtkTreeIter *iter) {
  ...
  if (node == NULL) {
    ...
    if (parent == NULL) {
      ...
      i = 0;
      for (cnode = tree_model->details->root_node; cnode != node;
           cnode = cnode->next) {
        i++;
      }
...
static void fm_tree_model_get_value(GtkTreeModel *model, GtkTreeIter *iter,
  ...
  switch (column) {
    case FM_TREE_MODEL_DISPLAY_NAME_COLUMN:
      ...
      if (node == NULL) {
        ...
        g_value_set_static_string(
            value, parent->done_loading ? _("(Empty)") : _("Loading..."));
      } else {
    ...
    case FM_TREE_MODEL_CLOSED_SURFACE_COLUMN:
      ...
      g_value_set_boxed(
          value, node == NULL ? NULL : tree_node_get_closed_surface(node));
      break;
    case FM_TREE_MODEL_OPEN_SURFACE_COLUMN:
      g_value_init(value, CAIRO_GOBJECT_TYPE_SURFACE);
      g_value_set_boxed(value,
                        node == NULL ? NULL : tree_node_get_open_surface(node));
      break;
    case FM_TREE_MODEL_FONT_STYLE_COLUMN:
...
static gboolean fm_tree_model_iter_next(GtkTreeModel *model,
                                        GtkTreeIter *iter) {
...
static gboolean fm_tree_model_iter_children(GtkTreeModel *model,
                                            GtkTreeIter *iter,
                                            GtkTreeIter *parent_iter) {
  TreeNode *parent;
...
static gboolean fm_tree_model_iter_parent(GtkTreeModel *model,
                                          GtkTreeIter *iter,
...
static gboolean fm_tree_model_iter_has_child(GtkTreeModel *model,
                                             GtkTreeIter *iter) {
  gboolean has_child;
  TreeNode *node;
...
static int fm_tree_model_iter_n_children(GtkTreeModel *model,
                                         GtkTreeIter *iter) {
  ...
  g_return_val_if_fail(
      iter == NULL || iter_is_valid(FM_TREE_MODEL(model), iter), FALSE);
...
static gboolean fm_tree_model_iter_nth_child(GtkTreeModel *model,
                                             GtkTreeIter *iter,
  ...
  g_return_val_if_fail(FM_IS_TREE_MODEL(model), FALSE);
  g_return_val_if_fail(
      parent_iter == NULL || iter_is_valid(FM_TREE_MODEL(model), parent_iter),
      FALSE);
...
static void update_monitoring(FMTreeModel *model, TreeNode *node) {
  TreeNode *child;
...
static gboolean update_monitoring_idle_callback(gpointer callback_data) {
  FMTreeModel *model;
  TreeNode *node;
...
static void stop_monitoring_directory_and_children(FMTreeModel *model,
                                                   TreeNode *node) {
  TreeNode *child;
...
static void stop_monitoring(FMTreeModel *model) {
  TreeNode *node;
...
static void fm_tree_model_ref_node(GtkTreeModel *model, GtkTreeIter *iter) {
  TreeNode *node, *parent;
  ...
  if (parent != NULL) {
    ...
    g_message("ref of %s, count is now %d", uri,
              parent->all_children_ref_count);
    g_free(uri);
#endif
  }
}

static void fm_tree_model_unref_node(GtkTreeModel *model, GtkTreeIter *iter) {
  TreeNode *node, *parent;
  ...
  if (parent != NULL) {
    ...
    uri = get_node_uri(iter);
    g_message("unref of %s, count is now %d", uri,
              parent->all_children_ref_count - 1);
    g_free(uri);
#endif
...
void fm_tree_model_add_root_uri(FMTreeModel *model, const char *root_uri,
                                const char *display_name, GIcon *icon,
                                GMount *mount) {
  CajaFile *file;
  TreeNode *node, *cnode;
  ...
  if (model->details->root_node == NULL) {
    ...
    /* append it */
    for (cnode = model->details->root_node; cnode->next != NULL;
         cnode = cnode->next)
      ;
    cnode->next = node;
    node->prev = cnode;
...
void fm_tree_model_remove_root_uri(FMTreeModel *model, const char *uri) {
  TreeNode *node;
  CajaFile *file;
...
static gboolean file_is_not_directory(CajaFile *file) {
...
void fm_tree_model_set_show_only_directories(FMTreeModel *model,
                                             gboolean show_only_directories) {
  g_return_if_fail(FM_IS_TREE_MODEL(model));
  g_return_if_fail(show_only_directories == FALSE ||
...
CajaFile *fm_tree_model_iter_get_file(FMTreeModel *model, GtkTreeIter *iter) {
  TreeNode *node;
...
int fm_tree_model_iter_compare_roots(FMTreeModel *model, GtkTreeIter *iter_a,
                                     GtkTreeIter *iter_b) {
  TreeNode *a, *b, *n;
...
gboolean fm_tree_model_iter_is_root(FMTreeModel *model, GtkTreeIter *iter) {
  TreeNode *node;
...
gboolean fm_tree_model_file_get_iter(FMTreeModel *model, GtkTreeIter *iter,
                                     CajaFile *file,
  ...
  if (current_iter != NULL && current_iter->user_data != NULL) {
    node =
        get_node_from_file(((TreeNode *)current_iter->user_data)->root, file);
    return make_iter_for_node(node, iter, model->details->stamp);
  }

  for (root_node = model->details->root_node; root_node != NULL;
...
static void do_update_node(CajaFile *file, FMTreeModel *model) {
...
void fm_tree_model_set_highlight_for_files(FMTreeModel *model, GList *files) {
  if (model->details->highlighted_files != NULL) {
    ...
    g_list_foreach(old_files, (GFunc)do_update_node, model);

    caja_file_list_free(old_files);
  }

  if (files != NULL) {
    model->details->highlighted_files = caja_file_list_copy(files);
    g_list_foreach(model->details->highlighted_files, (GFunc)do_update_node,
                   model);
  }
}

static void fm_tree_model_init(FMTreeModel *model) {
...
static void fm_tree_model_finalize(GObject *object) {
  FMTreeModel *model;
...
static void fm_tree_model_class_init(FMTreeModelClass *class) {
  G_OBJECT_CLASS(class)->finalize = fm_tree_model_finalize;

  tree_model_signals[ROW_LOADED] = g_signal_new(
      "row_loaded", FM_TYPE_TREE_MODEL, G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(FMTreeModelClass, row_loaded), NULL, NULL,
