...
/* fm-desktop-icon-view.c - implementation of icon view for managing the
   desktop.
...
static void fm_desktop_icon_view_update_icon_container_fonts(
...
static void desktop_directory_changed_callback(gpointer callback_data) {
  g_free(desktop_directory);
  desktop_directory = caja_get_desktop_directory();
}

static CajaIconContainer *get_icon_container(FMDesktopIconView *icon_view) {
  g_return_val_if_fail(FM_IS_DESKTOP_ICON_VIEW(icon_view), NULL);
  g_return_val_if_fail(
      CAJA_IS_ICON_CONTAINER(gtk_bin_get_child(GTK_BIN(icon_view))), NULL);

  return CAJA_ICON_CONTAINER(gtk_bin_get_child(GTK_BIN(icon_view)));
}

static void icon_container_set_workarea(CajaIconContainer *icon_container,
                                        GdkScreen *screen, long *workareas,
                                        int n_items) {
  ...
  for (i = 0; i < n_items; i += 4) {
    ...
    if ((x + width) > screen_width || (y + height) > screen_height) continue;
...
static void net_workarea_changed(FMDesktopIconView *icon_view,
                                 GdkWindow *window) {
  GdkDisplay *display;
  ...
  if (!gdk_property_get(
          window, gdk_atom_intern("_NET_NUMBER_OF_DESKTOPS", FALSE),
          gdk_x11_xatom_to_atom(XA_CARDINAL), 0, 4, FALSE, &type_returned,
          &format_returned, &length_returned, (guchar **)&nworkareas)) {
    g_warning("Can not calculate _NET_NUMBER_OF_DESKTOPS");
  }
  if (gdk_x11_display_error_trap_pop(display) || nworkareas == NULL ||
  ...
  if (nworkareas == NULL || (*nworkareas < 1) ||
      !gdk_property_get(window, gdk_atom_intern("_NET_WORKAREA", FALSE),
                        gdk_x11_xatom_to_atom(XA_CARDINAL), 0,
                        ((*nworkareas) * 4 * 4), FALSE, &type_returned,
                        &format_returned, &length_returned,
                        (guchar **)&workareas)) {
    g_warning("Can not get _NET_WORKAREA");
    workareas = NULL;
  }

  if (gdk_x11_display_error_trap_pop(display) || workareas == NULL ||
      type_returned != gdk_x11_xatom_to_atom(XA_CARDINAL) ||
      ((*nworkareas) * 4 * sizeof(long)) != length_returned ||
      format_returned != 32) {
    g_warning("Can not determine workarea, guessing at layout");
    caja_icon_container_set_margins(icon_container, 0, 0, 0, 0);
  } else {
    GdkScreen *screen;

    screen = gdk_window_get_screen(window);

    icon_container_set_workarea(icon_container, screen, workareas,
                                length_returned / sizeof(long));
  }

  if (nworkareas != NULL) g_free(nworkareas);

  if (workareas != NULL) g_free(workareas);
}

static GdkFilterReturn desktop_icon_view_property_filter(GdkXEvent *gdk_xevent,
                                                         GdkEvent *event,
                                                         gpointer data) {
...
static void fm_desktop_icon_view_dispose(GObject *object) {
  FMDesktopIconView *icon_view;
  ...
  if (ui_manager != NULL) {
    caja_ui_unmerge_ui(ui_manager, &icon_view->priv->desktop_merge_id,
  ...
  g_signal_handlers_disconnect_by_func(caja_desktop_preferences,
                                       font_changed_callback, icon_view);

  g_signal_handlers_disconnect_by_func(
      mate_lockdown_preferences, fm_directory_view_update_menus, icon_view);
  g_signal_handlers_disconnect_by_func(
      caja_preferences, desktop_directory_changed_callback, NULL);

  G_OBJECT_CLASS(fm_desktop_icon_view_parent_class)->dispose(object);
}

static void fm_desktop_icon_view_class_init(FMDesktopIconViewClass *class) {
  ...
  FM_ICON_VIEW_CLASS(class)->supports_labels_beside_icons =
      real_supports_labels_beside_icons;
}

static void fm_desktop_icon_view_handle_middle_click(
    CajaIconContainer *icon_container, GdkEventButton *event,
  ...
  seat = gdk_display_get_default_seat(
      gtk_widget_get_display(GTK_WIDGET(icon_container)));
  ...
  XSendEvent(GDK_DISPLAY_XDISPLAY(gdk_display_get_default()), GDK_ROOT_WINDOW(),
             True, ButtonPressMask, (XEvent *)&x_event);
}

static void unrealized_callback(GtkWidget *widget,
                                FMDesktopIconView *desktop_icon_view) {
...
static void realized_callback(GtkWidget *widget,
                              FMDesktopIconView *desktop_icon_view) {
  GdkWindow *root_window;
  GdkScreen *screen;
...
static CajaZoomLevel get_default_zoom_level(void) {
  ...
  return CLAMP(default_zoom_level, CAJA_ZOOM_LEVEL_SMALLEST,
...
static gboolean do_desktop_rescan(gpointer data) {
  FMDesktopIconView *desktop_icon_view;
  struct stat buf;
  ...
  caja_directory_force_reload(
      fm_directory_view_get_model(FM_DIRECTORY_VIEW(desktop_icon_view)));
  return TRUE;
}

static void done_loading(CajaDirectory *model,
                         FMDesktopIconView *desktop_icon_view) {
  struct stat buf;
 ...
 * instantiated.
 */
static void delayed_init(FMDesktopIconView *desktop_icon_view) {
  ...
  desktop_icon_view->priv->reload_desktop_timeout = g_timeout_add_seconds(
      RESCAN_TIMEOUT, do_desktop_rescan, desktop_icon_view);
...
static void font_changed_callback(gpointer callback_data) {
  ...
  fm_desktop_icon_view_update_icon_container_fonts(
      FM_DESKTOP_ICON_VIEW(callback_data));
}
...
static void fm_desktop_icon_view_update_icon_container_fonts(
  ...
  font = g_settings_get_string(caja_desktop_preferences,
...
static void fm_desktop_icon_view_init(FMDesktopIconView *desktop_icon_view) {
  ...
  desktop_icon_view->priv =
      fm_desktop_icon_view_get_instance_private(desktop_icon_view);

  if (desktop_directory == NULL) {
    g_signal_connect_swapped(
        caja_preferences, "changed::" CAJA_PREFERENCES_DESKTOP_IS_HOME_DIR,
        G_CALLBACK(desktop_directory_changed_callback), NULL);
  ...
  if (!caja_monitor_active()) {
    desktop_icon_view->priv->delayed_init_signal =
        g_signal_connect_object(desktop_icon_view, "begin_loading",
  ...
  gtk_scrolled_window_set_overlay_scrolling(
      GTK_SCROLLED_WINDOW(desktop_icon_view), FALSE);
  ...
  g_signal_connect_object(desktop_icon_view, "unrealize",
                          G_CALLBACK(unrealized_callback), desktop_icon_view,
                          0);
  ...
  g_signal_connect_swapped(
      ...
      G_CALLBACK(default_zoom_level_changed), desktop_icon_view);

  g_signal_connect_swapped(
      caja_desktop_preferences, "changed::" CAJA_PREFERENCES_DESKTOP_FONT,
      G_CALLBACK(font_changed_callback), desktop_icon_view);
...
static void action_new_launcher_callback(GtkAction *action, gpointer data) {
  ...
  caja_launch_application_from_command(
      gtk_widget_get_screen(GTK_WIDGET(data)), "mate-desktop-item-edit",
      "mate-desktop-item-edit", FALSE, "--create-new", desktop_directory, NULL);
  g_free(desktop_directory);
}

static void action_change_background_callback(GtkAction *action,
                                              gpointer data) {
  g_assert(FM_DIRECTORY_VIEW(data));

  caja_launch_application_from_command(
      gtk_widget_get_screen(GTK_WIDGET(data)), _("Background"),
      "mate-appearance-properties", FALSE, "--show-page=background", NULL);
}

static void action_empty_trash_conditional_callback(GtkAction *action,
                                                    gpointer data) {
...
static gboolean trash_link_is_selection(FMDirectoryView *view) {
  GList *selection;
  gboolean result;
  ...
  if (eel_g_list_exactly_one_item(selection) &&
    ...
    link = caja_desktop_icon_file_get_link(
        CAJA_DESKTOP_ICON_FILE(selection->data));
...
static void real_update_menus(FMDirectoryView *view) {
  ...
  FM_DIRECTORY_VIEW_CLASS(fm_desktop_icon_view_parent_class)
      ->update_menus(view);
  ...
  gtk_action_set_visible(action, !disable_command_line);
...
static const GtkActionEntry desktop_view_entries[] = {
    ...
    {"New Launcher Desktop", NULL,
     ...
     /* tooltip */
     N_("Create a new launcher"), G_CALLBACK(action_new_launcher_callback)},
    /* name, stock id */
    {"Change Background", NULL,
     /* label, accelerator */
     N_("Change Desktop _Background"), NULL,
     /* tooltip */
     N_("Show a window that lets you set your desktop background's pattern or "
        "color"),
...
static void real_merge_menus(FMDirectoryView *view) {
  ...
  gtk_action_group_add_actions(action_group, desktop_view_entries,
                               G_N_ELEMENTS(desktop_view_entries), view);
  G_GNUC_END_IGNORE_DEPRECATIONS;
...
static gboolean real_supports_scaling(FMIconView *view) { return TRUE; }

static gboolean real_supports_keep_aligned(FMIconView *view) { return TRUE; }
...
static CajaView *fm_desktop_icon_view_create(CajaWindowSlotInfo *slot) {
  FMIconView *view;

  view = g_object_new(FM_TYPE_DESKTOP_ICON_VIEW, "window-slot", slot, NULL);
  return CAJA_VIEW(view);
}

static gboolean fm_desktop_icon_view_supports_uri(const char *uri,
                                                  GFileType file_type,
...
void fm_desktop_icon_view_register(void) {
  ...
  fm_desktop_icon_view.startup_error_label =
      _(fm_desktop_icon_view.startup_error_label);
