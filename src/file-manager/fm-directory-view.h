...
#define FM_DIRECTORY_VIEW_CLASS(klass)                      \
  (G_TYPE_CHECK_CLASS_CAST((klass), FM_TYPE_DIRECTORY_VIEW, \
...
#define FM_DIRECTORY_VIEW_GET_CLASS(obj)                    \
  (G_TYPE_INSTANCE_GET_CLASS((obj), FM_TYPE_DIRECTORY_VIEW, \
                             FMDirectoryViewClass))
...
struct FMDirectoryViewClass {
   ...
   * It must be replaced by each subclass.
   */
  void (*add_file)(FMDirectoryView *view, CajaFile *file,
                   CajaDirectory *directory);
  void (*remove_file)(FMDirectoryView *view, CajaFile *file,
                      CajaDirectory *directory);
   ...
   * It must be replaced by each subclass.
   */
  void (*file_changed)(FMDirectoryView *view, CajaFile *file,
                       CajaDirectory *directory);
   ...
   */
  void (*end_loading)(FMDirectoryView *view, gboolean all_files_seen);
   ...
   */
  void (*load_error)(FMDirectoryView *view, GError *error);
  ...
  void (*bump_zoom_level)(FMDirectoryView *view, int zoom_increment);
  ...
  void (*zoom_to_level)(FMDirectoryView *view, CajaZoomLevel level);

  CajaZoomLevel (*get_zoom_level)(FMDirectoryView *view);

  /* restore_default_zoom_level is a function pointer that subclasses must
   * override to restore the zoom level of an object to a default setting. */
   ...
   */
  int (*compare_files)(FMDirectoryView *view, CajaFile *a, CajaFile *b);
  ...
  gboolean (*can_rename_file)(FMDirectoryView *view, CajaFile *file);
   ...
   * */
  void (*start_renaming_file)(FMDirectoryView *view, CajaFile *file,
                              gboolean select_all);

  gboolean (*file_still_belongs)(FMDirectoryView *view, CajaFile *file,
                                 CajaDirectory *directory);

  /* convert *point from widget's coordinate system to a coordinate
   * system used for specifying file operation positions, which is
   * view-specific.
   *
   * This is used by the the icon view, which converts the screen position to a
   * zoom level-independent coordinate system.
   */
  ...
  void (*set_is_active)(FMDirectoryView *view, gboolean is_active);
...
void fm_directory_view_queue_file_change(FMDirectoryView *view, CajaFile *file);
...
void fm_directory_view_new_file(FMDirectoryView *view, const char *parent_uri,
                                CajaFile *source);
...
void fm_directory_view_move_copy_items_between_views(FMDirectoryView *source,
                                                     FMDirectoryView *target,
                                                     gboolean copy);
