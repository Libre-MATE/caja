...
void fm_report_error_loading_directory(CajaFile *file, GError *error,
  ...
  if (error->domain == G_IO_ERROR) {
    switch (error->code) {
      case G_IO_ERROR_PERMISSION_DENIED:
        message = g_strdup_printf(_("You do not have the permissions necessary "
                                    "to view the contents of \"%s\"."),
                                  file_name);
        break;
      case G_IO_ERROR_NOT_FOUND:
        message = g_strdup_printf(_("\"%s\" could not be found. Perhaps it has "
                                    "recently been deleted."),
                                  file_name);
        break;
      default:
        message = g_strdup_printf(
            _("Sorry, could not display all the contents of \"%s\": %s"),
  ...
  eel_show_error_dialog(_("The folder contents could not be displayed."),
...
void fm_report_error_renaming_file(CajaFile *file, const char *new_name,
  ...
  original_name_truncated = eel_str_middle_truncate(
      original_name, MAXIMUM_DISPLAYED_FILE_NAME_LENGTH);
  g_free(original_name);

  new_name_truncated =
      eel_str_middle_truncate(new_name, MAXIMUM_DISPLAYED_FILE_NAME_LENGTH);
  ...
  if (error->domain == G_IO_ERROR) {
    switch (error->code) {
      case G_IO_ERROR_EXISTS:
        message =
            g_strdup_printf(_("The name \"%s\" is already used in this folder. "
      ...
      case G_IO_ERROR_PERMISSION_DENIED:
        message = g_strdup_printf(
            _("You do not have the permissions necessary to rename \"%s\"."),
            original_name_truncated);
        break;
      case G_IO_ERROR_INVALID_FILENAME:
        if (strchr(new_name, '/') != NULL) {
          message = g_strdup_printf(_("The name \"%s\" is not valid because it "
  ...
  if (message == NULL) {
    /* We should invent decent error messages for every case we actually
     * experience. */
...
void fm_report_error_setting_group(CajaFile *file, GError *error,
  ...
  if (message == NULL) {
    ...
    /* fall through */
    message =
        g_strdup_printf(_("Sorry, could not change the group of \"%s\": %s"),
                        file_name, error->message);
  }

  eel_show_error_dialog(_("The group could not be changed."), message,
                        parent_window);
...
void fm_report_error_setting_owner(CajaFile *file, GError *error,
  ...
  message =
      g_strdup_printf(_("Sorry, could not change the owner of \"%s\": %s"),
                      file_name, error->message);

  eel_show_error_dialog(_("The owner could not be changed."), message,
                        parent_window);
...
void fm_report_error_setting_permissions(CajaFile *file, GError *error,
  ...
  message = g_strdup_printf(
      _("Sorry, could not change the permissions of \"%s\": %s"), file_name,
      error->message);
...
static void fm_rename_data_free(FMRenameData *data) {
  g_free(data->name);
  g_free(data);
}

static void rename_callback(CajaFile *file, GFile *result_location,
...
void fm_rename_file(CajaFile *file, const char *new_name,
  ...
  g_object_set_data_full(G_OBJECT(file), NEW_NAME_TAG, data,
                         (GDestroyNotify)fm_rename_data_free);

  /* Start the timed wait to cancel the rename. */
  old_name = caja_file_get_display_name(file);
  wait_message =
      g_strdup_printf(_("Renaming \"%s\" to \"%s\"."), old_name, new_name);
  g_free(old_name);
  ...
  caja_debug_log(FALSE, CAJA_DEBUG_LOG_DOMAIN_USER,
                 "rename file old=\"%s\", new=\"%s\"", uri, new_name);
  g_free(uri);

  /* Start the rename. */
  caja_file_rename(file, new_name, rename_callback, NULL);
}
