...
enum { TITLE_COLUMN, VALUE_COLUMN, COLUMN_COUNT };
...
enum { TARGET_URI_LIST, TARGET_MATE_URI_LIST, TARGET_RESET_BACKGROUND };
...
static void file_changed_callback(CajaFile *file, gpointer user_data);
...
static void permission_combo_update(FMPropertiesWindow *window,
                                    GtkComboBox *combo);
static void value_field_update(FMPropertiesWindow *window, GtkLabel *field);
static void properties_window_update(FMPropertiesWindow *window, GList *files);
static void is_directory_ready_callback(CajaFile *file, gpointer data);
...
static gboolean name_field_focus_out(CajaEntry *name_field,
                                     GdkEventFocus *event,
                                     gpointer callback_data);
static void name_field_activate(CajaEntry *name_field, gpointer callback_data);
...
G_DEFINE_TYPE_WITH_PRIVATE(FMPropertiesWindow, fm_properties_window,
                           GTK_TYPE_DIALOG);

static gboolean is_multi_file_window(FMPropertiesWindow *window) {
  GList *l;
  int count;
...
static CajaFile *get_original_file(FMPropertiesWindow *window) {
...
static CajaFile *get_target_file_for_original_file(CajaFile *file) {
...
static CajaFile *get_target_file(FMPropertiesWindow *window) {
  return CAJA_FILE(window->details->target_files->data);
}

static void add_prompt(GtkWidget *vbox, const char *prompt_text,
...
static void add_prompt_and_separator(GtkWidget *vbox, const char *prompt_text) {
...
static void get_image_for_properties_window(FMPropertiesWindow *window,
                                            char **icon_name,
  ...
  icon = NULL;
  icon_scale =
      gtk_widget_get_scale_factor(GTK_WIDGET(window->details->notebook));
  ...
  if (!icon) {
    icon = caja_icon_info_lookup_from_name("text-x-generic",
                                           CAJA_ICON_SIZE_STANDARD, icon_scale);
  }
  ...
  if (icon_pixbuf != NULL) {
    *icon_pixbuf =
        caja_icon_info_get_pixbuf_at_size(icon, CAJA_ICON_SIZE_STANDARD);
  }

  g_object_unref(icon);
}

static void update_properties_window_icon(FMPropertiesWindow *window) {
  GdkPixbuf *pixbuf;
...
static void reset_icon(FMPropertiesWindow *properties_window) {
  GList *l;
  ...
  for (l = properties_window->details->original_files; l != NULL; l = l->next) {
    ...
    caja_file_set_metadata(file, CAJA_METADATA_KEY_CUSTOM_ICON, NULL, NULL);
  }
}

static void fm_properties_window_drag_data_received(
  ...
  if (!exactly_one) {
    eel_show_error_dialog(
        ...
        _("Please drag just one image to set a custom icon."), window);
  } else {
...
static GtkWidget *create_image_widget(FMPropertiesWindow *window,
...
static void set_name_field(FMPropertiesWindow *window,
   ...
   */
  use_label = is_multi_file_window(window) ||
              !caja_file_can_rename(get_original_file(window));
  new_widget = !window->details->name_field ||
               (use_label ? CAJA_IS_ENTRY(window->details->name_field)
  ...
  if (new_widget) {
    ...
    if (use_label) {
      window->details->name_field = GTK_WIDGET(attach_ellipsizing_value_label(
          window->details->basic_grid, GTK_WIDGET(window->details->name_label),
          name));
      ...
      gtk_grid_attach_next_to(
          window->details->basic_grid, window->details->name_field,
          GTK_WIDGET(window->details->name_label), GTK_POS_RIGHT, 1, 1);
  ...
  /* Only replace text if the file's name has changed. */
    ...
    if (use_label) {
       ...
       * selection change).
       */
      gchar *displayed_name = gtk_editable_get_chars(
          GTK_EDITABLE(window->details->name_field), 0, -1);
...
static void update_name_field(FMPropertiesWindow *window) {
  CajaFile *file;

  gtk_label_set_text_with_mnemonic(
      window->details->name_label,
      ngettext("_Name:", "_Names:", get_not_gone_original_file_count(window)));
  ...
  if (is_multi_file_window(window)) {
     ...
     */
    if (window->details->name_field) {
      original_name = (const char *)g_object_get_data(
          G_OBJECT(window->details->name_field), "original_name");
    }
    ...
    if (original_name == NULL || g_strcmp0(original_name, current_name) != 0) {
      g_object_set_data_full(G_OBJECT(window->details->name_field),
                             "original_name", current_name, g_free);
    } else {
      g_free(current_name);
    }
  }
...
static void name_field_restore_original_name(CajaEntry *name_field) {
...
static void rename_callback(CajaFile *file, GFile *res_loc, GError *error,
                            gpointer callback_data) {
  ...
  if (error != NULL) {
    fm_report_error_renaming_file(file, window->details->pending_name, error,
                                  GTK_WINDOW(window));
...
static void set_pending_name(FMPropertiesWindow *window, const char *name) {
  g_free(window->details->pending_name);
  window->details->pending_name = g_strdup(name);
}

static void name_field_done_editing(CajaEntry *name_field,
                                    FMPropertiesWindow *window) {
  CajaFile *file;
  char *new_name;
  ...
  if (strlen(new_name) == 0) {
    ...
    if (strcmp(new_name, original_name) != 0) {
      ...
      g_object_ref(window);
      caja_file_rename(file, new_name, rename_callback, window);
    }
  }
...
static gboolean file_has_keyword(CajaFile *file, const char *keyword) {
  GList *keywords, *word;
...
static void get_initial_emblem_state(FMPropertiesWindow *window,
                                     const char *name, GList **on,
                                     GList **off) {
  GList *l;
  ...
  *off = NULL;
  ...
  for (l = window->details->original_files; l != NULL; l = l->next) {
    ...
    initial_emblems =
        g_hash_table_lookup(window->details->initial_emblems, l->data);
...
static void emblem_button_toggled(GtkToggleButton *button,
                                  FMPropertiesWindow *window) {
  GList *l;
  GList *keywords;
  GList *word;
  ...
  if (gtk_toggle_button_get_active(button) &&
    ...
    /* Go to the initial state unless the initial state was
       consistent */
    get_initial_emblem_state(window, name, &files_on, &files_off);
  ...
  g_signal_handlers_block_by_func(G_OBJECT(button),
                                  G_CALLBACK(emblem_button_toggled), window);
...
static void emblem_button_update(FMPropertiesWindow *window,
                                 GtkToggleButton *button) {
  GList *l;
  char *name;
  gboolean all_set;
...
static void update_properties_window_title(FMPropertiesWindow *window) {
  char *title;
...
static void clear_extension_pages(FMPropertiesWindow *window) {
  int i;
  int num_pages;
  GtkWidget *page = NULL;

  num_pages = gtk_notebook_get_n_pages(GTK_NOTEBOOK(window->details->notebook));

  for (i = 0; i < num_pages; i++) {
    page =
        gtk_notebook_get_nth_page(GTK_NOTEBOOK(window->details->notebook), i);

    if (g_object_get_data(G_OBJECT(page), "is-extension-page")) {
      gtk_notebook_remove_page(GTK_NOTEBOOK(window->details->notebook), i);
      num_pages--;
      i--;
    }
  }
...
static void refresh_extension_pages(FMPropertiesWindow *window) {
  clear_extension_pages(window);
  append_extension_pages(window);
}

static void remove_from_dialog(FMPropertiesWindow *window, CajaFile *file) {
  int index;
  GList *original_link;
  GList *target_link;
  ...
  window->details->original_files =
      g_list_remove_link(window->details->original_files, original_link);
  g_list_free(original_link);

  window->details->target_files =
      g_list_remove_link(window->details->target_files, target_link);
  ...
  g_signal_handlers_disconnect_by_func(
      original_file, G_CALLBACK(file_changed_callback), window);
  g_signal_handlers_disconnect_by_func(
      target_file, G_CALLBACK(file_changed_callback), window);
...
static GList *get_mime_list(FMPropertiesWindow *window) {
  GList *ret;
  GList *l;
...
static void schedule_files_update(FMPropertiesWindow *window) {
  g_assert(FM_IS_PROPERTIES_WINDOW(window));

  if (window->details->update_files_timeout_id == 0) {
    window->details->update_files_timeout_id =
        g_timeout_add(FILES_UPDATE_INTERVAL, update_files_callback, window);
  }
}
...
static gboolean file_list_attributes_identical(GList *file_list,
  ...
  for (l = file_list; l != NULL; l = l->next) {
    ...
    if (first_attr == NULL) {
      first_attr =
          caja_file_get_string_attribute_with_default(file, attribute_name);
    } else {
      char *attr;
...
static char *file_list_get_string_attribute(GList *file_list,
  ...
  if (file_list_attributes_identical(file_list, attribute_name)) {
    ...
    for (l = file_list; l != NULL; l = l->next) {
      ...
      file = CAJA_FILE(l->data);
      if (!caja_file_is_gone(file)) {
        return caja_file_get_string_attribute_with_default(file,
                                                           attribute_name);
      }
    }
...
static gboolean file_list_all_directories(GList *file_list) {
  GList *l;
  for (l = file_list; l != NULL; l = l->next) {
...
static void value_field_update_internal(GtkLabel *label, GList *file_list) {
  const char *attribute_name;
  ...
  inconsistent_string =
      g_object_get_data(G_OBJECT(label), "inconsistent_string");
  attribute_value = file_list_get_string_attribute(file_list, attribute_name,
                                                   inconsistent_string);
  if (!strcmp(attribute_name, "type") &&
    ...
    if (strcmp(mime_type, inconsistent_string)) {
      ...
      tmp = attribute_value;
      attribute_value =
          g_strdup_printf(C_("MIME type description (MIME type)", "%s (%s)"),
...
static void value_field_update(FMPropertiesWindow *window, GtkLabel *label) {
  gboolean use_original;

  use_original =
      GPOINTER_TO_INT(g_object_get_data(G_OBJECT(label), "show_original"));

  value_field_update_internal(label,
                              (use_original ? window->details->original_files
                                            : window->details->target_files));
}

static GtkLabel *attach_label(GtkGrid *grid, GtkWidget *sibling,
                              const char *initial_text, gboolean ellipsize_text,
                              gboolean selectable, gboolean mnemonic) {
  GtkWidget *label_field;

  if (ellipsize_text) {
    label_field = gtk_label_new(initial_text);
    gtk_label_set_ellipsize(GTK_LABEL(label_field), PANGO_ELLIPSIZE_END);
  } else if (mnemonic) {
  ...
  if (sibling != NULL) {
    gtk_grid_attach_next_to(grid, label_field, sibling, GTK_POS_RIGHT, 1, 1);
  } else {
...
static GtkLabel *attach_value_label(GtkGrid *grid, GtkWidget *sibling,
                                    const char *initial_text) {
...
static GtkWidget *attach_value_field_internal(FMPropertiesWindow *window,
                                              GtkGrid *grid, GtkWidget *sibling,
  ...
  g_object_set_data(G_OBJECT(value_field), "show_original",
                    GINT_TO_POINTER(show_original));

  window->details->value_fields =
      g_list_prepend(window->details->value_fields, value_field);
  return GTK_WIDGET(value_field);
}

static GtkWidget *attach_value_field(FMPropertiesWindow *window, GtkGrid *grid,
                                     GtkWidget *sibling,
                                     const char *file_attribute_name,
                                     const char *inconsistent_string,
                                     gboolean show_original) {
  return attach_value_field_internal(window, grid, sibling, file_attribute_name,
                                     inconsistent_string, show_original, FALSE);
}

static GtkWidget *attach_ellipsizing_value_field(
    FMPropertiesWindow *window, GtkGrid *grid, GtkWidget *sibling,
    const char *file_attribute_name, const char *inconsistent_string,
    gboolean show_original) {
  return attach_value_field_internal(window, grid, sibling, file_attribute_name,
                                     inconsistent_string, show_original, TRUE);
}

static void group_change_callback(CajaFile *file, GFile *res_loc, GError *error,
                                  FMPropertiesWindow *window) {
  char *group;
...
static void cancel_group_change_callback(FMPropertiesWindow *window) {
  ...
  caja_file_cancel(file, (CajaFileOperationCallback)group_change_callback,
                   window);
...
static gboolean schedule_group_change_timeout(FMPropertiesWindow *window) {
  CajaFile *file;
  char *group;
  ...
  eel_timed_wait_start((EelCancelCallback)cancel_group_change_callback, window,
                       _("Cancel Group Change?"), GTK_WINDOW(window));

  caja_file_set_group(file, group,
...
static void schedule_group_change(FMPropertiesWindow *window, CajaFile *file,
                                  const char *group) {
  g_assert(FM_IS_PROPERTIES_WINDOW(window));
  ...
  window->details->group_change_timeout = g_timeout_add(
      CHOWN_CHGRP_TIMEOUT, (GSourceFunc)schedule_group_change_timeout, window);
}

static void unschedule_or_cancel_group_change(FMPropertiesWindow *window) {
  CajaFile *file;
  char *group;
  ...
  g_assert((file == NULL && group == NULL) || (file != NULL && group != NULL));
...
static void changed_group_callback(GtkComboBox *combo_box, CajaFile *file) {
  char *group;
  char *cur_group;
...
static void synch_groups_combo_box(GtkComboBox *combo_box, CajaFile *file) {
  GList *groups;
  GList *node;
  GtkTreeModel *model;
  GtkListStore *store;
...
static GtkComboBox *attach_combo_box(GtkGrid *grid, GtkWidget *sibling,
  ...
  gtk_grid_attach_next_to(grid, combo_box, sibling, GTK_POS_RIGHT, 1, 1);

  return GTK_COMBO_BOX(combo_box);
}

static GtkComboBox *attach_group_combo_box(GtkGrid *grid, GtkWidget *sibling,
                                           CajaFile *file) {
  ...
  g_signal_connect_data(combo_box, "changed",
                        G_CALLBACK(changed_group_callback), caja_file_ref(file),
                        (GClosureNotify)caja_file_unref, 0);

  return combo_box;
}

static void owner_change_callback(CajaFile *file, GFile *result_location,
                                  GError *error, FMPropertiesWindow *window) {
  char *owner;
...
static void cancel_owner_change_callback(FMPropertiesWindow *window) {
  ...
  caja_file_cancel(file, (CajaFileOperationCallback)owner_change_callback,
                   window);
...
static gboolean schedule_owner_change_timeout(FMPropertiesWindow *window) {
  CajaFile *file;
  char *owner;
  ...
  eel_timed_wait_start((EelCancelCallback)cancel_owner_change_callback, window,
                       _("Cancel Owner Change?"), GTK_WINDOW(window));

  caja_file_set_owner(file, owner,
...
static void schedule_owner_change(FMPropertiesWindow *window, CajaFile *file,
                                  const char *owner) {
  g_assert(FM_IS_PROPERTIES_WINDOW(window));
  ...
  g_object_ref(G_OBJECT(window));
  window->details->owner_change_timeout = g_timeout_add(
      CHOWN_CHGRP_TIMEOUT, (GSourceFunc)schedule_owner_change_timeout, window);
}

static void unschedule_or_cancel_owner_change(FMPropertiesWindow *window) {
  CajaFile *file;
  char *owner;
  ...
  g_assert((file == NULL && owner == NULL) || (file != NULL && owner != NULL));
...
static void changed_owner_callback(GtkComboBox *combo_box, CajaFile *file) {
  char *owner_text;
  char **name_array;
  char *new_owner;
  char *cur_owner;
...
static void synch_user_menu(GtkComboBox *combo_box, CajaFile *file) {
  GList *users;
  GList *node;
  GtkTreeModel *model;
  GtkListStore *store;
  ...
  if (!tree_model_entries_equal(model, 1, users)) {
    ...
    for (node = users, user_index = 0; node != NULL;
      ...
      gtk_list_store_set(store, &iter, 0, combo_text, 1, user_name, -1);
  ...
  if (owner_index < 0 && owner_name != NULL) {
    if (users != NULL) {
      ...
      gtk_list_store_set(store, &iter, 0, "-", 1, NULL, -1);
    }
    ...
    gtk_list_store_prepend(store, &iter);
    gtk_list_store_set(store, &iter, 0, owner_name, 1, user_name, -1);
...
static GtkComboBox *attach_owner_combo_box(GtkGrid *grid, GtkWidget *sibling,
                                           CajaFile *file) {
  GtkComboBox *combo_box;
  ...
  g_signal_connect_object(file, "changed", G_CALLBACK(synch_user_menu),
                          combo_box, G_CONNECT_SWAPPED);
  g_signal_connect_data(combo_box, "changed",
                        G_CALLBACK(changed_owner_callback), caja_file_ref(file),
                        (GClosureNotify)caja_file_unref, 0);

  return combo_box;
}

static gboolean file_has_prefix(CajaFile *file, GList *prefix_candidates) {
  GList *p;
...
static void directory_contents_value_field_update(FMPropertiesWindow *window) {
  ...
  for (l = window->details->target_files; l; l = l->next) {
    ...
    if (caja_file_is_directory(file)) {
      file_status = caja_file_get_deep_counts(
          file, &directory_count, &file_count, &file_unreadable, &file_size,
   ...
   */
  if (window->details->deep_count_finished && status != CAJA_REQUEST_DONE) {
    return;
  }
  ...
  if (total_count == 0) {
    ...
    if (g_settings_get_boolean(caja_preferences,
      ...
      size_on_disk_str =
          g_format_size_full(total_size_on_disk, G_FORMAT_SIZE_IEC_UNITS);
    } else {
    ...
    text = g_strdup_printf(
        ngettext("%'d item, with size %s (%s on disk)",
                 "%'d items, totalling %s (%s on disk)", total_count),
...
static void schedule_directory_contents_update(FMPropertiesWindow *window) {
  g_assert(FM_IS_PROPERTIES_WINDOW(window));
  ...
  if (window->details->update_directory_contents_timeout_id == 0) {
    window->details->update_directory_contents_timeout_id =
        g_timeout_add(DIRECTORY_CONTENTS_UPDATE_INTERVAL,
                      update_directory_contents_callback, window);
  }
}

static GtkLabel *attach_directory_contents_value_field(
    FMPropertiesWindow *window, GtkGrid *grid, GtkWidget *sibling) {
  GtkLabel *value_field;
  GList *l;
  CajaFile *file = NULL;
  ...
  for (l = window->details->target_files; l; l = l->next) {
    ...
    g_signal_connect_object(file, "updated_deep_count_in_progress",
...
static GtkLabel *attach_title_field(GtkGrid *grid, const char *title) {
  return attach_label(grid, NULL, title, FALSE, FALSE, TRUE);
}

#define INCONSISTENT_STATE_STRING "\xE2\x80\x92"

static void append_title_value_pair(FMPropertiesWindow *window, GtkGrid *grid,
                                    const char *title,
                                    const char *file_attribute_name,
  ...
  value = attach_value_field(window, grid, GTK_WIDGET(title_label),
                             file_attribute_name, inconsistent_state,
                             show_original);
  gtk_label_set_mnemonic_widget(title_label, value);
}

static void append_title_and_ellipsizing_value(FMPropertiesWindow *window,
                                               GtkGrid *grid, const char *title,
                                               const char *file_attribute_name,
                                               const char *inconsistent_state,
                                               gboolean show_original) {
  GtkLabel *title_label;
  ...
  title_label = attach_title_field(grid, title);
  value = attach_ellipsizing_value_field(window, grid, GTK_WIDGET(title_label),
                                         file_attribute_name,
                                         inconsistent_state, show_original);
  gtk_label_set_mnemonic_widget(title_label, value);
}

static void append_directory_contents_fields(FMPropertiesWindow *window,
                                             GtkGrid *grid) {
  ...
  value_field = attach_directory_contents_value_field(window, grid,
                                                      GTK_WIDGET(title_field));

  gtk_label_set_mnemonic_widget(title_field, GTK_WIDGET(value_field));
}

static GtkWidget *create_page_with_hbox(GtkNotebook *notebook,
                                        const char *title) {
  GtkWidget *hbox;
...
static GtkWidget *create_page_with_vbox(GtkNotebook *notebook,
                                        const char *title) {
  GtkWidget *vbox;
...
static GtkWidget *append_blank_row(GtkGrid *grid) {
  return GTK_WIDGET(attach_title_field(grid, ""));
}

static void append_blank_slim_row(GtkGrid *grid) {
  GtkWidget *w;
...
static GtkWidget *create_grid_with_standard_properties(void) {
  GtkWidget *grid;
...
static gboolean is_merged_trash_directory(CajaFile *file) {
  char *file_uri;
  gboolean result;
...
static gboolean is_computer_directory(CajaFile *file) {
  char *file_uri;
  gboolean result;
...
static gboolean is_network_directory(CajaFile *file) {
  char *file_uri;
  gboolean result;
...
static gboolean is_burn_directory(CajaFile *file) {
  char *file_uri;
  gboolean result;
...
static gboolean should_show_custom_icon_buttons(FMPropertiesWindow *window) {
  if (is_multi_file_window(window)) {
    return FALSE;
  }
...
static gboolean should_show_file_type(FMPropertiesWindow *window) {
  if (!is_multi_file_window(window) &&
...
static gboolean should_show_location_info(FMPropertiesWindow *window) {
  if (!is_multi_file_window(window) &&
      (is_merged_trash_directory(get_target_file(window)) ||
       is_computer_directory(get_target_file(window)) ||
       is_network_directory(get_target_file(window)) ||
...
static gboolean should_show_accessed_date(FMPropertiesWindow *window) {
...
static gboolean should_show_modified_date(FMPropertiesWindow *window) {
  CajaFile *file;

  if (is_multi_file_window(window)) return FALSE;

  file = get_original_file(window);
  if ((file != NULL) && caja_file_can_unmount(file)) return FALSE;

  return TRUE;
}

static gboolean should_show_link_target(FMPropertiesWindow *window) {
  if (!is_multi_file_window(window) &&
...
static gboolean should_show_free_space(FMPropertiesWindow *window) {
  if (!is_multi_file_window(window) &&
      (is_merged_trash_directory(get_target_file(window)) ||
       is_computer_directory(get_target_file(window)) ||
       is_network_directory(get_target_file(window)) ||
...
static gboolean should_show_volume_usage(FMPropertiesWindow *window) {
  CajaFile *file;
...
static void paint_used_legend(GtkWidget *widget, cairo_t *cr, gpointer data) {
  FMPropertiesWindow *window;
  gint width, height;
...
static void paint_free_legend(GtkWidget *widget, cairo_t *cr, gpointer data) {
  FMPropertiesWindow *window;
  gint width, height;
  GtkAllocation allocation;
...
static void paint_pie_chart(GtkWidget *widget, cairo_t *cr, gpointer data) {
  ...
  free = (double)window->details->volume_free /
         (double)window->details->volume_capacity;
  used = 1.0 - free;
...
static void _pie_style_shade(GdkRGBA *a, GdkRGBA *b, gdouble k) {
  gdouble red;
  gdouble green;
  gdouble blue;
...
static GtkWidget *create_pie_widget(FMPropertiesWindow *window) {
  ...
  if (g_settings_get_boolean(caja_preferences,
    ...
    free = g_format_size_full(window->details->volume_free,
                              G_FORMAT_SIZE_IEC_UNITS);
    used = g_format_size_full(
        window->details->volume_capacity - window->details->volume_free,
        G_FORMAT_SIZE_IEC_UNITS);
  } else {
    capacity = g_format_size(window->details->volume_capacity);
    free = g_format_size(window->details->volume_free);
    used = g_format_size(window->details->volume_capacity -
  ...
  if (!gtk_style_context_lookup_color(style, "chart_rgba_1",
                                      &window->details->used_color)) {
  ...
  if (!gtk_style_context_lookup_color(style, "chart_rgba_2",
                                      &window->details->free_color)) {
  ...
  _pie_style_shade(&window->details->used_color,
                   &window->details->used_stroke_color, 0.7);
  _pie_style_shade(&window->details->free_color,
                   &window->details->free_stroke_color, 0.7);
  ...
  gtk_container_add_with_properties(GTK_CONTAINER(grid), pie_canvas, "height",
                                    4, NULL);
  gtk_grid_attach_next_to(grid, used_canvas, pie_canvas, GTK_POS_RIGHT, 1, 1);
  gtk_grid_attach_next_to(grid, used_label, used_canvas, GTK_POS_RIGHT, 1, 1);

  gtk_grid_attach_next_to(grid, free_canvas, used_canvas, GTK_POS_BOTTOM, 1, 1);
  gtk_grid_attach_next_to(grid, free_label, free_canvas, GTK_POS_RIGHT, 1, 1);

  gtk_grid_attach_next_to(grid, capacity_label, free_canvas, GTK_POS_BOTTOM, 2,
                          1);
  gtk_grid_attach_next_to(grid, fstype_label, capacity_label, GTK_POS_BOTTOM, 2,
                          1);

  g_signal_connect(pie_canvas, "draw", G_CALLBACK(paint_pie_chart), window);
  g_signal_connect(used_canvas, "draw", G_CALLBACK(paint_used_legend), window);
  g_signal_connect(free_canvas, "draw", G_CALLBACK(paint_free_legend), window);

  return GTK_WIDGET(grid);
}

static GtkWidget *create_volume_usage_widget(FMPropertiesWindow *window) {
  GtkWidget *piewidget;
  gchar *uri;
  CajaFile *file;
  GFile *location;
  ...
  if (info) {
    window->details->volume_capacity = g_file_info_get_attribute_uint64(
...
static void create_basic_page(FMPropertiesWindow *window) {
  GtkGrid *grid;
  ...
  icon_pixmap_widget =
      create_image_widget(window, should_show_custom_icon_buttons(window));
  ...
  if (fm_ditem_page_should_show(window->details->target_files)) {
    ...
    box =
        fm_ditem_page_make_box(label_size_group, window->details->target_files);
  ...
  if (should_show_file_type(window)) {
    append_title_value_pair(window, grid, _("Type:"), "type",
                            INCONSISTENT_STATE_STRING, FALSE);
  }

  if (should_show_link_target(window)) {
    append_title_and_ellipsizing_value(window, grid, _("Link target:"),
                                       "link_target", INCONSISTENT_STATE_STRING,
                                       FALSE);
  }
  ...
  if (is_multi_file_window(window) ||
    ...
    append_title_value_pair(window, grid, _("Size:"), "size_detail",
                            INCONSISTENT_STATE_STRING, FALSE);
    append_title_value_pair(window, grid, _("Size on Disk:"),
                            "size_on_disk_detail", INCONSISTENT_STATE_STRING,
                            FALSE);
  }
  ...
  if (should_show_location_info(window)) {
    append_title_and_ellipsizing_value(window, grid, _("Location:"), "where",
                                       INCONSISTENT_STATE_STRING, TRUE);

    append_title_and_ellipsizing_value(window, grid, _("Volume:"), "volume",
                                       INCONSISTENT_STATE_STRING, FALSE);
  }

  if (should_show_accessed_date(window) || should_show_modified_date(window)) {
    append_blank_row(grid);
  }

  if (should_show_accessed_date(window)) {
    append_title_value_pair(window, grid, _("Accessed:"), "date_accessed",
                            INCONSISTENT_STATE_STRING, FALSE);
  }

  if (should_show_modified_date(window)) {
    append_title_value_pair(window, grid, _("Modified:"), "date_modified",
                            INCONSISTENT_STATE_STRING, FALSE);
    append_title_value_pair(window, grid, _("Created:"), "date_created",
                            INCONSISTENT_STATE_STRING, FALSE);
  }
  ...
  if (should_show_free_space(window)) {
    ...
    append_title_value_pair(window, grid, _("Free space:"), "free_space",
                            INCONSISTENT_STATE_STRING, FALSE);
  }
...
static GHashTable *get_initial_emblems(GList *files) {
  GHashTable *ret;
  GList *l;
...
static gboolean files_has_directory(FMPropertiesWindow *window) {
  GList *l;
...
static gboolean files_has_changable_permissions_directory(
    FMPropertiesWindow *window) {
  GList *l;
  ...
  for (l = window->details->target_files; l != NULL; l = l->next) {
    CajaFile *file;
    file = CAJA_FILE(l->data);
    if (caja_file_is_directory(file) && caja_file_can_get_permissions(file) &&
...
static gboolean files_has_file(FMPropertiesWindow *window) {
  GList *l;
...
static void create_emblems_page(FMPropertiesWindow *window) {
  ...
  /* stop GTK 3.22 builds from ballooning the properties dialog to full screen
   * height */
  ...
  gtk_notebook_append_page(window->details->notebook, scroller,
                           gtk_label_new(_("Emblems")));

  icons = caja_emblem_list_available();
  scale = gtk_widget_get_scale_factor(scroller);

  window->details->initial_emblems =
      get_initial_emblems(window->details->original_files);
  ...
  while (l != NULL) {
    ...
    info = caja_icon_info_lookup_from_name(emblem_name, CAJA_ICON_SIZE_SMALL,
                                           scale);
    pixbuf =
        caja_icon_info_get_pixbuf_nodefault_at_size(info, CAJA_ICON_SIZE_SMALL);
    ...
    eel_labeled_image_set_fixed_image_height(
        EEL_LABELED_IMAGE(gtk_bin_get_child(GTK_BIN(button))),
    ...
    g_object_set_data_full(G_OBJECT(button), "caja_emblem_name",
                           caja_emblem_get_keyword_from_icon_name(emblem_name),
                           g_free);

    window->details->emblem_buttons =
        g_list_append(window->details->emblem_buttons, button);

    g_signal_connect_object(button, "toggled",
                            G_CALLBACK(emblem_button_toggled), G_OBJECT(window),
                            0);
...
static void start_long_operation(FMPropertiesWindow *window) {
...
static void end_long_operation(FMPropertiesWindow *window) {
...
static void permission_change_callback(CajaFile *file, GFile *res_loc,
...
static void update_permissions(FMPropertiesWindow *window, guint32 vfs_new_perm,
                               guint32 vfs_mask, gboolean is_folder,
  ...
  for (l = window->details->target_files; l != NULL; l = l->next) {
    ...
    g_object_ref(window);
    caja_file_set_permissions(file, permissions, permission_change_callback,
                              window);
  }
}

static gboolean initial_permission_state_consistent(
    FMPropertiesWindow *window, guint32 mask, gboolean is_folder,
  ...
  for (l = window->details->target_files; l != NULL; l = l->next) {
    ...
    permissions = GPOINTER_TO_INT(
        g_hash_table_lookup(window->details->initial_permissions, file));
...
static void permission_button_toggled(GtkToggleButton *button,
  ...
  if (gtk_toggle_button_get_active(button) &&
    ...
    if (!window->details->has_recursive_apply &&
        initial_permission_state_consistent(window, permission_mask, is_folder,
                                            is_special)) {
      inconsistent = FALSE;
      on = TRUE;
    }
  ...
  g_signal_handlers_block_by_func(
      G_OBJECT(button), G_CALLBACK(permission_button_toggled), window);
  ...
  update_permissions(window, on ? permission_mask : 0, permission_mask,
                     is_folder, is_special, inconsistent);
}
...
static void permission_button_update(FMPropertiesWindow *window,
  ...
  for (l = window->details->target_files; l != NULL; l = l->next) {
    ...
    if (!is_special && ((caja_file_is_directory(file) && !is_folder) ||
                        (!caja_file_is_directory(file) && is_folder))) {
      continue;
    }
  ...
  gtk_toggle_button_set_inconsistent(
      button, (!all_unset && !all_set) || (!is_folder && no_match));
...
static void set_up_permissions_checkbox(FMPropertiesWindow *window,
  ...
  g_object_set_data(G_OBJECT(check_button), "properties_window", window);
  g_object_set_data(G_OBJECT(check_button), "is-folder",
                    GINT_TO_POINTER(is_folder));

  window->details->permission_buttons =
      g_list_prepend(window->details->permission_buttons, check_button);

  g_signal_connect_object(check_button, "toggled",
                          G_CALLBACK(permission_button_toggled), window, 0);
}

static GtkWidget *add_permissions_checkbox_with_label(
    FMPropertiesWindow *window, GtkGrid *grid, GtkWidget *sibling,
    const char *label, guint32 permission_to_check, GtkLabel *label_for,
    gboolean is_folder) {
  GtkWidget *check_button;
  ...
  if (sibling) {
    gtk_grid_attach_next_to(grid, check_button, sibling, GTK_POS_RIGHT, 1, 1);
  } else {
    gtk_container_add(GTK_CONTAINER(grid), check_button);
  }

  set_up_permissions_checkbox(window, check_button, permission_to_check,
                              is_folder);
...
static GtkWidget *add_permissions_checkbox(FMPropertiesWindow *window,
                                           GtkGrid *grid, GtkWidget *sibling,
                                           CheckboxType type,
  ...
  return add_permissions_checkbox_with_label(
      window, grid, sibling, label, permission_to_check, label_for, is_folder);
}
...
static void permission_combo_changed(GtkWidget *combo,
  ...
  update_permissions(window, vfs_new_perm, vfs_mask, is_folder, FALSE,
                     use_original);
}
...
static void permission_combo_update(FMPropertiesWindow *window,
  ...
  g_signal_handlers_block_by_func(G_OBJECT(combo),
                                  G_CALLBACK(permission_combo_changed), window);
  ...
  if (is_folder) {
    ...
    sensitive = !all_file_cannot_set || window->details->has_recursive_apply;
  }
  gtk_widget_set_sensitive(GTK_WIDGET(combo), sensitive);

  g_signal_handlers_unblock_by_func(
      G_OBJECT(combo), G_CALLBACK(permission_combo_changed), window);
}

static void add_permissions_combo_box(FMPropertiesWindow *window, GtkGrid *grid,
  ...
  if (is_folder) {
    ...
    gtk_list_store_append(store, &iter);
    gtk_list_store_set(store, &iter, 0, _("List files only"), 1,
                       PERMISSION_READ, -1);
    gtk_list_store_append(store, &iter);
    gtk_list_store_set(store, &iter, 0, _("Access files"), 1,
                       PERMISSION_READ | PERMISSION_EXEC, -1);
    gtk_list_store_append(store, &iter);
    gtk_list_store_set(store, &iter, 0, _("Create and delete files"), 1,
    ...
    gtk_list_store_append(store, &iter);
    gtk_list_store_set(store, &iter, 0, _("Read and write"), 1,
                       PERMISSION_READ | PERMISSION_WRITE, -1);
  }
  if (window->details->has_recursive_apply) {
  ...
  window->details->permission_combos =
      g_list_prepend(window->details->permission_combos, combo);

  g_signal_connect(combo, "changed", G_CALLBACK(permission_combo_changed),
                   window);
  ...
  gtk_grid_attach_next_to(grid, combo, GTK_WIDGET(label), GTK_POS_RIGHT, 1, 1);
}

static GtkWidget *append_special_execution_checkbox(
    FMPropertiesWindow *window, GtkGrid *grid, GtkWidget *sibling,
    const char *label_text, guint32 permission_to_check) {
  GtkWidget *check_button;
  ...
  set_up_permissions_checkbox(window, check_button, permission_to_check, FALSE);
...
static void append_special_execution_flags(FMPropertiesWindow *window,
                                           GtkGrid *grid) {
  GtkWidget *title;
  ...
  append_special_execution_checkbox(window, grid, title, _("Set _user ID"),
                                    UNIX_PERM_SUID);
  append_special_execution_checkbox(window, grid, NULL, _("Set gro_up ID"),
                                    UNIX_PERM_SGID);
  append_special_execution_checkbox(window, grid, NULL, _("_Sticky"),
                                    UNIX_PERM_STICKY);
}
...
static void create_simple_permissions(FMPropertiesWindow *window,
                                      GtkGrid *page_grid) {
  ...
  if (!is_multi_file_window(window) &&
      caja_file_can_set_owner(get_target_file(window))) {
    GtkComboBox *owner_combo_box;

    owner_label = attach_title_field(page_grid, _("_Owner:"));
    /* Combo box in this case. */
    owner_combo_box = attach_owner_combo_box(page_grid, GTK_WIDGET(owner_label),
                                             get_target_file(window));
    gtk_label_set_mnemonic_widget(owner_label, GTK_WIDGET(owner_combo_box));
  } else {
    owner_label = attach_title_field(page_grid, _("Owner:"));
    /* Static text in this case. */
    value = attach_value_field(window, page_grid, GTK_WIDGET(owner_label),
                               "owner", INCONSISTENT_STATE_STRING, FALSE);
    gtk_label_set_mnemonic_widget(owner_label, value);
  }

  if (has_directory) {
    add_permissions_combo_box(window, page_grid, PERMISSION_USER, TRUE, FALSE);
  }
  if (has_file || window->details->has_recursive_apply) {
    add_permissions_combo_box(window, page_grid, PERMISSION_USER, FALSE,
                              !has_directory);
  }
  ...
  if (!is_multi_file_window(window) &&
      caja_file_can_set_group(get_target_file(window))) {
    ...
    group_combo_box = attach_group_combo_box(page_grid, GTK_WIDGET(group_label),
                                             get_target_file(window));
    gtk_label_set_mnemonic_widget(group_label, GTK_WIDGET(group_combo_box));
  } else {
    group_label = attach_title_field(page_grid, _("Group:"));

    /* Static text in this case. */
    value = attach_value_field(window, page_grid, GTK_WIDGET(group_label),
                               "group", INCONSISTENT_STATE_STRING, FALSE);
    gtk_label_set_mnemonic_widget(group_label, value);
  }

  if (has_directory) {
    add_permissions_combo_box(window, page_grid, PERMISSION_GROUP, TRUE, FALSE);
  }
  if (has_file || window->details->has_recursive_apply) {
    add_permissions_combo_box(window, page_grid, PERMISSION_GROUP, FALSE,
                              !has_directory);
  }
  ...
  if (has_directory) {
    add_permissions_combo_box(window, page_grid, PERMISSION_OTHER, TRUE, FALSE);
  }
  if (has_file || window->details->has_recursive_apply) {
    add_permissions_combo_box(window, page_grid, PERMISSION_OTHER, FALSE,
                              !has_directory);
  }
  ...
  add_permissions_checkbox_with_label(
      window, page_grid, GTK_WIDGET(execute_label),
      _("Allow _executing file as program"),
      UNIX_PERM_USER_EXEC | UNIX_PERM_GROUP_EXEC | UNIX_PERM_OTHER_EXEC,
      execute_label, FALSE);
}

static void create_permission_checkboxes(FMPropertiesWindow *window,
                                         GtkGrid *page_grid,
                                         gboolean is_folder) {
  ...
  gtk_grid_attach_next_to(page_grid, GTK_WIDGET(check_button_grid),
                          GTK_WIDGET(owner_perm_label), GTK_POS_RIGHT, 1, 3);

  w = add_permissions_checkbox(window, check_button_grid, NULL,
                               PERMISSIONS_CHECKBOXES_READ, UNIX_PERM_USER_READ,
                               owner_perm_label, is_folder);

  w = add_permissions_checkbox(
      window, check_button_grid, w, PERMISSIONS_CHECKBOXES_WRITE,
      UNIX_PERM_USER_WRITE, owner_perm_label, is_folder);

  w = add_permissions_checkbox(
      window, check_button_grid, w, PERMISSIONS_CHECKBOXES_EXECUTE,
      UNIX_PERM_USER_EXEC, owner_perm_label, is_folder);

  w = add_permissions_checkbox(
      window, check_button_grid, NULL, PERMISSIONS_CHECKBOXES_READ,
      UNIX_PERM_GROUP_READ, group_perm_label, is_folder);

  w = add_permissions_checkbox(
      window, check_button_grid, w, PERMISSIONS_CHECKBOXES_WRITE,
      UNIX_PERM_GROUP_WRITE, group_perm_label, is_folder);

  w = add_permissions_checkbox(
      window, check_button_grid, w, PERMISSIONS_CHECKBOXES_EXECUTE,
      UNIX_PERM_GROUP_EXEC, group_perm_label, is_folder);

  w = add_permissions_checkbox(
      window, check_button_grid, NULL, PERMISSIONS_CHECKBOXES_READ,
      UNIX_PERM_OTHER_READ, other_perm_label, is_folder);

  w = add_permissions_checkbox(
      window, check_button_grid, w, PERMISSIONS_CHECKBOXES_WRITE,
      UNIX_PERM_OTHER_WRITE, other_perm_label, is_folder);

  add_permissions_checkbox(window, check_button_grid, w,
                           PERMISSIONS_CHECKBOXES_EXECUTE, UNIX_PERM_OTHER_EXEC,
                           other_perm_label, is_folder);
}

static void create_advanced_permissions(FMPropertiesWindow *window,
                                        GtkGrid *page_grid) {
  GtkLabel *group_label;
  GtkLabel *owner_label;
  ...
  if (!is_multi_file_window(window) &&
    ...
    /* Static text in this case. */
    attach_value_field(window, page_grid, GTK_WIDGET(group_label), "group",
                       INCONSISTENT_STATE_STRING, FALSE);
  }
  ...
  append_title_value_pair(window, page_grid, _("Text view:"), "permissions",
                          INCONSISTENT_STATE_STRING, FALSE);
}

static void set_recursive_permissions_done(gpointer callback_data) {
  FMPropertiesWindow *window;
...
static void apply_recursive_clicked(GtkWidget *recursive_button,
                                    FMPropertiesWindow *window) {
  ...
  for (l = window->details->permission_buttons; l != NULL; l = l->next) {
    ...
    p = GPOINTER_TO_INT(g_object_get_data(G_OBJECT(button), "permission"));
    is_folder =
        GPOINTER_TO_INT(g_object_get_data(G_OBJECT(button), "is-folder"));
    is_special =
        GPOINTER_TO_INT(g_object_get_data(G_OBJECT(button), "is-special"));
  ...
  for (l = window->details->target_files; l != NULL; l = l->next) {
    ...
    if (caja_file_is_directory(file) && caja_file_can_set_permissions(file)) {
      char *uri;

      uri = caja_file_get_uri(file);
      start_long_operation(window);
      g_object_ref(window);
      caja_file_set_permissions_recursive(
          uri, file_permission, file_permission_mask, dir_permission,
          dir_permission_mask, set_recursive_permissions_done, window);
      g_free(uri);
    }
  }
}

static void create_permissions_page(FMPropertiesWindow *window) {
  GtkWidget *vbox;
  GList *file_list;
  ...
  if (all_can_get_permissions(file_list) &&
      all_can_get_permissions(window->details->target_files)) {
    GtkGrid *page_grid;

    window->details->initial_permissions =
        get_initial_permissions(window->details->target_files);
    window->details->has_recursive_apply =
        files_has_changable_permissions_directory(window);
    ...
    gtk_box_pack_start(GTK_BOX(vbox), GTK_WIDGET(page_grid), TRUE, TRUE, 0);
...
#ifdef HAVE_SELINUX
    append_title_value_pair(window, page_grid, _("SELinux context:"),
                            "selinux_context", INCONSISTENT_STATE_STRING,
                            FALSE);
#endif
    append_title_value_pair(window, page_grid, _("Last changed:"),
    ...
    if (window->details->has_recursive_apply) {
      ...
      hbox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0);
      gtk_widget_show(hbox);
      gtk_container_add_with_properties(GTK_CONTAINER(page_grid), hbox, "width",
                                        2, NULL);

      button = gtk_button_new_with_mnemonic(
          _("Apply Permissions to Enclosed Files"));
      gtk_widget_show(button);
      gtk_box_pack_start(GTK_BOX(hbox), button, FALSE, FALSE, 0);
      g_signal_connect(button, "clicked", G_CALLBACK(apply_recursive_clicked),
                       window);
    }
  } else {
    char *prompt_text;
    ...
    if (!is_multi_file_window(window)) {
      ...
      prompt_text = g_strdup(
          _("The permissions of the selected file could not be determined."));
    }
...
static void append_extension_pages(FMPropertiesWindow *window) {
   ...
   */
  module_providers =
      caja_module_get_extensions_for_type(CAJA_TYPE_PROPERTY_PAGE_PROVIDER);
  ...
  for (p = providers; p != NULL; p = p->next) {
    ...
    for (l = pages; l != NULL; l = l->next) {
      ...
      gtk_notebook_append_page(window->details->notebook, page_widget, label);

      g_object_set_data(G_OBJECT(page_widget), "is-extension-page", page);
...
static gboolean should_show_emblems(FMPropertiesWindow *window) {
...
static gboolean should_show_permissions(FMPropertiesWindow *window) {
  CajaFile *file;
   ...
   */
  if (!is_multi_file_window(window) &&
      (is_merged_trash_directory(file) || is_computer_directory(file))) {
    return FALSE;
  }
...
static StartupData *startup_data_new(GList *original_files, GList *target_files,
...
static void file_changed_callback(CajaFile *file, gpointer user_data) {
  ...
  if (!g_list_find(window->details->changed_files, file)) {
    caja_file_ref(file);
    window->details->changed_files =
        g_list_prepend(window->details->changed_files, file);

    schedule_files_update(window);
  }
}

static gboolean is_a_special_file(CajaFile *file) {
  if (file == NULL || CAJA_IS_DESKTOP_ICON_FILE(file) ||
      is_merged_trash_directory(file) || is_computer_directory(file)) {
    return TRUE;
  }
  return FALSE;
...
static gboolean should_show_open_with(FMPropertiesWindow *window) {
  CajaFile *file;
  ...
  if (is_multi_file_window(window)) {
    if (!file_list_attributes_identical(window->details->original_files,
      ...
      for (l = window->details->original_files; l; l = l->next) {
        file = CAJA_FILE(l->data);
        if (caja_file_is_directory(file) || is_a_special_file(file)) {
          return FALSE;
        }
      }
    }
...
static void create_open_with_page(FMPropertiesWindow *window) {
  GtkWidget *vbox;
  char *mime_type;
  ...
  if (!is_multi_file_window(window)) {
    ...
    vbox =
        caja_mime_application_chooser_new_for_multiple_files(uris, mime_type);
  }

  gtk_widget_show(vbox);
  g_free(mime_type);

  gtk_notebook_append_page(window->details->notebook, vbox,
                           gtk_label_new(_("Open With")));
}

static FMPropertiesWindow *create_properties_window(StartupData *startup_data) {
  FMPropertiesWindow *window;
  GList *l;
  GtkWidget *action_area;

  window = FM_PROPERTIES_WINDOW(
      gtk_widget_new(fm_properties_window_get_type(), NULL));

  window->details->original_files =
      caja_file_list_copy(startup_data->original_files);

  window->details->target_files =
      caja_file_list_copy(startup_data->target_files);
  ...
  for (l = window->details->original_files; l != NULL; l = l->next) {
    ...
    caja_file_monitor_add(file, &window->details->original_files, attributes);
  }
  ...
  for (l = window->details->target_files; l != NULL; l = l->next) {
    g_signal_connect_object(CAJA_FILE(l->data), "changed",
  ...
  g_signal_connect(window->details->notebook, "scroll-event",
                   G_CALLBACK(eel_notebook_scroll_event_cb), NULL);

  gtk_widget_show(GTK_WIDGET(window->details->notebook));
  gtk_box_pack_start(GTK_BOX(gtk_dialog_get_content_area(GTK_DIALOG(window))),
                     GTK_WIDGET(window->details->notebook), TRUE, TRUE, 0);
  ...
  eel_dialog_add_button(GTK_DIALOG(window), _("_Help"), "help-browser",
                        GTK_RESPONSE_HELP);

  action_area = gtk_widget_get_parent(eel_dialog_add_button(
      GTK_DIALOG(window), _("_Close"), "window-close", GTK_RESPONSE_CLOSE));

  /* FIXME - HIGificiation, should be done inside GTK+ */
  gtk_container_set_border_width(
      GTK_CONTAINER(gtk_dialog_get_content_area(GTK_DIALOG(window))), 12);
...
static void add_window(FMPropertiesWindow *window) {
  if (!is_multi_file_window(window)) {
    g_hash_table_insert(windows, get_original_file(window), window);
    g_object_set_data(G_OBJECT(window), "window_key",
                      get_original_file(window));
  }
}

static void remove_window(FMPropertiesWindow *window) {
  gpointer key;
...
static GtkWindow *get_existing_window(GList *file_list) {
  if (!file_list->next) {
...
static void cancel_call_when_ready_callback(gpointer key, gpointer value,
                                            gpointer user_data) {
  caja_file_cancel_call_when_ready(CAJA_FILE(key), is_directory_ready_callback,
                                   user_data);
}
...
void fm_properties_window_present(GList *original_files,
                                  GtkWidget *parent_widget) {
  GList *l, *next;
  ...
  if (windows == NULL) {
    windows = eel_g_hash_table_new_free_at_exit(NULL, NULL, "property windows");
  }
  ...
  for (l = startup_data->target_files; l != NULL; l = next) {
    next = l->next;
    caja_file_call_when_ready(CAJA_FILE(l->data), CAJA_FILE_ATTRIBUTE_INFO,
                              is_directory_ready_callback, startup_data);
  }
}

static void real_response(GtkDialog *dialog, int response) {
  GError *error = NULL;
...
static void real_destroy(GtkWidget *object) {
  FMPropertiesWindow *window;
  GList *l;
  ...
  for (l = window->details->original_files; l != NULL; l = l->next) {
    caja_file_monitor_remove(CAJA_FILE(l->data),
                             &window->details->original_files);
  }
 ...
 * */
static char *make_relative_uri_from_full(const char *uri,
                                         const char *base_uri) {
  g_assert(uri != NULL);
 ...
 * file */
static void set_icon(const char *icon_uri,
                     FMPropertiesWindow *properties_window) {
  char *icon_path;
  ...
  if (icon_path != NULL) {
    ...
    for (l = properties_window->details->original_files; l != NULL;
         l = l->next) {
      char *file_uri;
      ...
      if (caja_file_is_mime_type(file, "application/x-desktop")) {
        if (caja_link_local_set_icon(file_uri, icon_path)) {
          caja_file_invalidate_attributes(
              file, CAJA_FILE_ATTRIBUTE_INFO | CAJA_FILE_ATTRIBUTE_LINK_INFO);
        }
      } else {
        char *real_icon_uri;
        ...
        caja_file_set_metadata(file, CAJA_METADATA_KEY_CUSTOM_ICON, NULL,
                               real_icon_uri);
        caja_file_set_metadata(file, CAJA_METADATA_KEY_ICON_SCALE, NULL, NULL);
...
static void update_preview_callback(GtkFileChooser *icon_chooser,
                                    FMPropertiesWindow *window) {
  ...
  if (pixbuf != NULL) {
    ...
    if (gdk_pixbuf_get_width(pixbuf) > PREVIEW_IMAGE_WIDTH) {
      ...
      scaled_pixbuf = gdk_pixbuf_scale_simple(
          pixbuf, PREVIEW_IMAGE_WIDTH,
          (int)(scale * (double)PREVIEW_IMAGE_WIDTH), GDK_INTERP_HYPER);
      g_object_unref(pixbuf);
...
static void custom_icon_file_chooser_response_cb(GtkDialog *dialog,
                                                 gint response,
                                                 FMPropertiesWindow *window) {
  char *uri;
...
static void select_image_button_callback(GtkWidget *widget,
  ...
  if (dialog == NULL) {
    ...
    dialog = eel_file_chooser_dialog_new(
        _("Select Custom Icon"), GTK_WINDOW(window),
        GTK_FILE_CHOOSER_ACTION_OPEN, "document-revert", GTK_RESPONSE_NO,
        "process-stop", GTK_RESPONSE_CANCEL, "document-open", GTK_RESPONSE_OK,
        NULL);
    gtk_file_chooser_add_shortcut_folder(GTK_FILE_CHOOSER(dialog), ICONDIR,
                                         NULL);
    gtk_file_chooser_add_shortcut_folder(GTK_FILE_CHOOSER(dialog), PIXMAPDIR,
                                         NULL);
  ...
  for (l = window->details->original_files; l != NULL; l = l->next) {
    file = CAJA_FILE(l->data);
    image_path =
        caja_file_get_metadata(file, CAJA_METADATA_KEY_CUSTOM_ICON, NULL);
