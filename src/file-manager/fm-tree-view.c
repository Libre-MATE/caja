...
static void fm_tree_view_activate_file(FMTreeView *view, CajaFile *file,
...
G_DEFINE_TYPE_WITH_CODE(FMTreeViewProvider, fm_tree_view_provider,
                        G_TYPE_OBJECT,
                        G_IMPLEMENT_INTERFACE(CAJA_TYPE_SIDEBAR_PROVIDER,
                                              sidebar_provider_iface_init));

static void notify_clipboard_info(CajaClipboardMonitor *monitor,
                                  CajaClipboardInfo *info, FMTreeView *view) {
  if (info != NULL && info->cut) {
    fm_tree_model_set_highlight_for_files(view->details->child_model,
                                          info->files);
  } else {
...
static gboolean show_iter_for_file(FMTreeView *view, CajaFile *file,
                                   GtkTreeIter *iter) {
  GtkTreeModel *model;
  ...
  /* check if file is visible in the same root as the currently selected folder
   * is */
  gtk_tree_view_get_cursor(view->details->tree_widget, &path, NULL);
  if (path != NULL) {
    if (gtk_tree_model_get_iter(model, &cur_iter, path) &&
        fm_tree_model_file_get_iter(view->details->child_model, iter, file,
                                    &cur_iter)) {
  ...
  if (fm_tree_model_file_get_iter(view->details->child_model, iter, file,
                                  NULL)) {
    return TRUE;
  }
...
static void refresh_highlight(FMTreeView *view) {
...
static gboolean show_selection_idle_callback(gpointer callback_data) {
  FMTreeView *view;
  CajaFile *file;
  GtkTreeIter iter;
  GtkTreePath *path, *sort_path;
  ...
  path = gtk_tree_model_get_path(GTK_TREE_MODEL(view->details->child_model),
                                 &iter);
  sort_path = gtk_tree_model_sort_convert_child_path_to_path(
      view->details->sort_model, path);
  gtk_tree_path_free(path);
  ...
  if (gtk_widget_get_realized(GTK_WIDGET(view->details->tree_widget))) {
    gtk_tree_view_scroll_to_cell(view->details->tree_widget, sort_path, NULL,
                                 FALSE, 0, 0);
  }
  gtk_tree_path_free(sort_path);
...
static void schedule_show_selection(FMTreeView *view) {
  if (view->details->show_selection_idle_id == 0) {
    view->details->show_selection_idle_id =
        g_idle_add(show_selection_idle_callback, view);
  }
}

static void schedule_select_and_show_location(FMTreeView *view,
                                              char *location) {
...
static void row_loaded_callback(GtkTreeModel *tree_model, GtkTreeIter *iter,
                                FMTreeView *view) {
  CajaFile *file, *tmp_file, *selection_file;

  if (view->details->selection_location == NULL || !view->details->selecting ||
...
static CajaFile *sort_model_iter_to_file(FMTreeView *view, GtkTreeIter *iter) {
  GtkTreeIter child_iter;

  gtk_tree_model_sort_convert_iter_to_child_iter(view->details->sort_model,
                                                 &child_iter, iter);
  return fm_tree_model_iter_get_file(view->details->child_model, &child_iter);
}

static CajaFile *sort_model_path_to_file(FMTreeView *view, GtkTreePath *path) {
  GtkTreeIter iter;

  if (!gtk_tree_model_get_iter(GTK_TREE_MODEL(view->details->sort_model), &iter,
                               path)) {
    return NULL;
  }
...
static void got_activation_uri_callback(CajaFile *file,
  ...
  open_in_same_slot =
      (view->details->activation_flags &
       (CAJA_WINDOW_OPEN_FLAG_NEW_WINDOW | CAJA_WINDOW_OPEN_FLAG_NEW_TAB)) == 0;
  ...
  if (caja_file_is_launcher(file)) {
    ...
    g_free(file_uri);
  } else if (uri != NULL && caja_file_is_executable(file) &&
             caja_file_can_execute(file) && !caja_file_is_directory(file)) {
    ...
    if (file_uri == NULL) {
      ...
      caja_window_slot_info_open_location(
          slot, location, CAJA_WINDOW_OPEN_ACCORDING_TO_MODE,
          view->details->activation_flags, NULL);
    ...
    if (!open_in_same_slot || view->details->selection_location == NULL ||
...
static void cancel_activation(FMTreeView *view) {
  if (view->details->activation_file == NULL) {
    return;
  }

  caja_file_cancel_call_when_ready(view->details->activation_file,
...
static void row_activated_callback(GtkTreeView *treeview, GtkTreePath *path,
  ...
  if (gtk_tree_view_row_expanded(view->details->tree_widget, path)) {
    ...
    gtk_tree_view_expand_row(view->details->tree_widget, path, FALSE);
  }
}

static gboolean selection_changed_timer_callback(FMTreeView *view) {
...
static void selection_changed_callback(GtkTreeSelection *selection,
  ...
  if (event) {
    ...
    if (is_keyboard) {
       ...
       */
      view->details->selection_changed_timer = g_timeout_add(
          300, (GSourceFunc)selection_changed_timer_callback, view);
    } else {
...
static int compare_rows(GtkTreeModel *model, GtkTreeIter *a, GtkTreeIter *b,
                        gpointer callback_data) {
  ...
  if (file_a == file_b) {
    ...
    result = caja_file_compare_for_sort(
        file_a, file_b, CAJA_FILE_SORT_BY_DISPLAY_NAME, FALSE, FALSE);
  }
...
static void move_copy_items_callback(CajaTreeViewDragDest *dest,
  ...
  caja_clipboard_clear_if_colliding_uris(GTK_WIDGET(view), item_uris,
                                         copied_files_atom);
  caja_file_operations_copy_move(item_uris, NULL, target_uri, action,
                                 GTK_WIDGET(view->details->tree_widget), NULL,
                                 NULL);
}

static void add_root_for_mount(FMTreeView *view, GMount *mount) {
  char *mount_uri, *name;
  GFile *root;
  GIcon *icon;

  if (g_mount_is_shadowed(mount)) return;
  ...
  fm_tree_model_add_root_uri(view->details->child_model, mount_uri, name, icon,
                             mount);
...
static void mount_removed_callback(GVolumeMonitor *volume_monitor,
  ...
  mount_uri = g_file_get_uri(root);
  g_object_unref(root);
  fm_tree_model_remove_root_uri(view->details->child_model, mount_uri);
  g_free(mount_uri);
}

static void clipboard_contents_received_callback(
...
static gboolean is_parent_writable(CajaFile *file) {
  CajaFile *parent;
  gboolean result;
...
static gboolean button_pressed_callback(GtkTreeView *treeview,
  ...
  if (event->button == 3) {
    ...
    gtk_widget_set_sensitive(
        view->details->popup_create_folder,
        caja_file_is_directory(view->details->popup_file) &&
    ...
    if (caja_file_is_directory(view->details->popup_file) &&
      ...
      gtk_clipboard_request_contents(
          caja_clipboard_get(GTK_WIDGET(view->details->tree_widget)),
          copied_files_atom, clipboard_contents_received_callback,
          g_object_ref(view));
    }
    can_move_file_to_trash = caja_file_can_trash(view->details->popup_file);
    gtk_widget_set_sensitive(view->details->popup_trash,
                             can_move_file_to_trash);
    ...
    if (g_settings_get_boolean(caja_preferences,
      ...
      file_is_home_or_desktop =
          caja_file_is_home(view->details->popup_file) ||
          caja_file_is_desktop_directory(view->details->popup_file);
      file_is_special_link =
          CAJA_IS_DESKTOP_ICON_FILE(view->details->popup_file);

      can_delete_file = parent_file_is_writable && !file_is_home_or_desktop &&
                        !file_is_special_link;
    ...
    mount = fm_tree_model_get_mount_for_root_node_file(
        view->details->child_model, view->details->popup_file);
    if (mount) {
    ...
    gtk_tree_view_set_cursor(view->details->tree_widget, cursor_path, NULL,
                             FALSE);
...
static void fm_tree_view_open_cb(GtkWidget *menu_item, FMTreeView *view) {
  fm_tree_view_activate_file(view, view->details->popup_file, 0);
}

static void fm_tree_view_open_in_new_tab_cb(GtkWidget *menu_item,
                                            FMTreeView *view) {
  fm_tree_view_activate_file(view, view->details->popup_file,
                             CAJA_WINDOW_OPEN_FLAG_NEW_TAB);
}

static void fm_tree_view_open_in_new_window_cb(GtkWidget *menu_item,
                                               FMTreeView *view) {
  /* fm_tree_view_activate_file (view, view->details->popup_file,
   * CAJA_WINDOW_OPEN_FLAG_NEW_WINDOW); */
...
static void fm_tree_view_create_folder_cb(GtkWidget *menu_item,
                                          FMTreeView *view) {
  char *parent_uri;

  parent_uri = caja_file_get_uri(view->details->popup_file);
  caja_file_operations_new_folder(GTK_WIDGET(view->details->tree_widget), NULL,
                                  parent_uri, new_folder_done,
                                  view->details->tree_widget);

  g_free(parent_uri);
}

static void copy_or_cut_files(FMTreeView *view, gboolean cut) {
  ...
  gtk_clipboard_set_with_data(
      caja_clipboard_get(GTK_WIDGET(view->details->tree_widget)), targets,
      n_targets, caja_get_clipboard_callback, caja_clear_clipboard_callback,
      NULL);
  ...
  caja_window_info_push_status(view->details->window, status_string);
  g_free(status_string);
}

static void fm_tree_view_cut_cb(GtkWidget *menu_item, FMTreeView *view) {
  copy_or_cut_files(view, TRUE);
}

static void fm_tree_view_copy_cb(GtkWidget *menu_item, FMTreeView *view) {
  copy_or_cut_files(view, FALSE);
}

static void paste_clipboard_data(FMTreeView *view,
  ...
  if (item_uris == NULL || destination_uri == NULL) {
    caja_window_info_push_status(
        view->details->window,
        _("There is nothing on the clipboard to paste."));
  } else {
    caja_file_operations_copy_move(item_uris, NULL, destination_uri,
...
static void fm_tree_view_paste_cb(GtkWidget *menu_item, FMTreeView *view) {
  gtk_clipboard_request_contents(
      caja_clipboard_get(GTK_WIDGET(view->details->tree_widget)),
      copied_files_atom, paste_into_clipboard_received_callback, view);
}
...
static void fm_tree_view_trash_cb(GtkWidget *menu_item, FMTreeView *view) {
  GList *list;
  ...
  list =
      g_list_prepend(NULL, caja_file_get_location(view->details->popup_file));

  caja_file_operations_trash_or_delete(
      list, fm_tree_view_get_containing_window(view), NULL, NULL);
  g_list_free_full(list, g_object_unref);
}

static void fm_tree_view_delete_cb(GtkWidget *menu_item, FMTreeView *view) {
  GList *location_list;
  ...
  caja_file_operations_delete(
      location_list, fm_tree_view_get_containing_window(view), NULL, NULL);
  g_list_free_full(location_list, g_object_unref);
}

static void fm_tree_view_properties_cb(GtkWidget *menu_item, FMTreeView *view) {
  GList *list;
...
static void fm_tree_view_unmount_cb(GtkWidget *menu_item, FMTreeView *view) {
  ...
  mount = fm_tree_model_get_mount_for_root_node_file(view->details->child_model,
                                                     file);
...
static void fm_tree_view_eject_cb(GtkWidget *menu_item, FMTreeView *view) {
  CajaFile *file = view->details->popup_file;
  GMount *mount;
...
static gboolean free_popup_file_in_idle_cb(gpointer data) {
  FMTreeView *view;
...
static void popup_menu_deactivated(GtkMenuShell *menu_shell, gpointer data) {
  FMTreeView *view;

  view = FM_TREE_VIEW(data);

  /* The popup menu is deactivated. (I.E. hidden)
     We want to free popup_file, but can't right away as it might immediately
     get used if we're deactivation due to activating a menu item. So, we free
     it in idle */

  if (view->details->popup_file != NULL &&
      view->details->popup_file_idle_handler == 0) {
    view->details->popup_file_idle_handler =
        g_idle_add(free_popup_file_in_idle_cb, view);
  }
}
...
static void create_popup_menu(FMTreeView *view) {
  ...
  g_signal_connect(popup, "deactivate", G_CALLBACK(popup_menu_deactivated),
                   view);

  /* add the "open" menu item */
  menu_item = eel_image_menu_item_new_from_icon("document-open", _("_Open"));
  g_signal_connect(menu_item, "activate", G_CALLBACK(fm_tree_view_open_cb),
                   view);
  ...
  g_signal_connect(menu_item, "activate",
                   G_CALLBACK(fm_tree_view_open_in_new_tab_cb), view);
  gtk_widget_show(menu_item);
  gtk_menu_shell_append(GTK_MENU_SHELL(popup), menu_item);
  ...
  g_signal_connect(menu_item, "activate",
                   G_CALLBACK(fm_tree_view_open_in_new_window_cb), view);
  gtk_widget_show(menu_item);
  gtk_menu_shell_append(GTK_MENU_SHELL(popup), menu_item);
  view->details->popup_open_in_new_window = menu_item;
  ...
  g_signal_connect(menu_item, "activate",
                   G_CALLBACK(fm_tree_view_create_folder_cb), view);
  gtk_widget_show(menu_item);
  gtk_menu_shell_append(GTK_MENU_SHELL(popup), menu_item);
  ...
  g_signal_connect(menu_item, "activate", G_CALLBACK(fm_tree_view_cut_cb),
                   view);
  gtk_widget_show(menu_item);
  gtk_menu_shell_append(GTK_MENU_SHELL(popup), menu_item);
  ...
  g_signal_connect(menu_item, "activate", G_CALLBACK(fm_tree_view_copy_cb),
                   view);
  gtk_widget_show(menu_item);
  gtk_menu_shell_append(GTK_MENU_SHELL(popup), menu_item);
  ...
  menu_item =
      eel_image_menu_item_new_from_icon("edit-paste", _("_Paste Into Folder"));
  g_signal_connect(menu_item, "activate", G_CALLBACK(fm_tree_view_paste_cb),
                   view);
  gtk_widget_show(menu_item);
  gtk_menu_shell_append(GTK_MENU_SHELL(popup), menu_item);
  ...
  menu_item = eel_image_menu_item_new_from_icon(CAJA_ICON_TRASH_FULL,
                                                _("Mo_ve to Trash"));
  g_signal_connect(menu_item, "activate", G_CALLBACK(fm_tree_view_trash_cb),
                   view);
  gtk_widget_show(menu_item);
  gtk_menu_shell_append(GTK_MENU_SHELL(popup), menu_item);
  ...
  g_signal_connect(menu_item, "activate", G_CALLBACK(fm_tree_view_delete_cb),
                   view);
  gtk_widget_show(menu_item);
  gtk_menu_shell_append(GTK_MENU_SHELL(popup), menu_item);
  ...
  g_signal_connect(menu_item, "activate", G_CALLBACK(fm_tree_view_unmount_cb),
                   view);
  gtk_widget_show(menu_item);
  gtk_menu_shell_append(GTK_MENU_SHELL(popup), menu_item);
  ...
  g_signal_connect(menu_item, "activate", G_CALLBACK(fm_tree_view_eject_cb),
                   view);
  gtk_widget_show(menu_item);
  gtk_menu_shell_append(GTK_MENU_SHELL(popup), menu_item);
  ...
  menu_item = eel_image_menu_item_new_from_icon("document-properties",
                                                _("_Properties"));
  g_signal_connect(menu_item, "activate",
                   G_CALLBACK(fm_tree_view_properties_cb), view);
  gtk_widget_show(menu_item);
  gtk_menu_shell_append(GTK_MENU_SHELL(popup), menu_item);
...
static void create_tree(FMTreeView *view) {
  ...
  gtk_tree_view_column_set_attributes(
      column, cell, "surface", FM_TREE_MODEL_CLOSED_SURFACE_COLUMN, NULL);

  cell = gtk_cell_renderer_text_new();
  gtk_tree_view_column_pack_start(column, cell, TRUE);
  gtk_tree_view_column_set_attributes(
      column, cell, "text", FM_TREE_MODEL_DISPLAY_NAME_COLUMN, "style",
      FM_TREE_MODEL_FONT_STYLE_COLUMN, NULL);
...
static void update_filtering_from_preferences(FMTreeView *view) {
  ...
  fm_tree_model_set_show_only_directories(
      view->details->child_model,
      g_settings_get_boolean(caja_tree_sidebar_preferences,
                             CAJA_PREFERENCES_TREE_SHOW_ONLY_DIRECTORIES));
}

static void parent_set_callback(GtkWidget *widget, GtkWidget *previous_parent,
                                gpointer callback_data) {
  FMTreeView *view;
...
static void fm_tree_view_init(FMTreeView *view) {
  ...
  g_signal_connect_object(view, "parent_set", G_CALLBACK(parent_set_callback),
                          view, 0);
...
static void fm_tree_view_dispose(GObject *object) {
  FMTreeView *view;
  ...
  g_signal_handlers_disconnect_by_func(
      caja_preferences, G_CALLBACK(filtering_changed_callback), view);
...
static void fm_tree_view_finalize(GObject *object) {
  FMTreeView *view;
...
static void fm_tree_view_class_init(FMTreeViewClass *class) {
...
static const char *fm_tree_view_get_sidebar_id(CajaSidebar *sidebar) {
  return TREE_SIDEBAR_ID;
}

static char *fm_tree_view_get_tab_label(CajaSidebar *sidebar) {
  return g_strdup(_("Tree"));
}

static char *fm_tree_view_get_tab_tooltip(CajaSidebar *sidebar) {
  return g_strdup(_("Show Tree"));
}

static GdkPixbuf *fm_tree_view_get_tab_icon(CajaSidebar *sidebar) {
  return NULL;
}

static void fm_tree_view_is_visible_changed(CajaSidebar *sidebar,
                                            gboolean is_visible) {
  /* Do nothing */
}

static void hidden_files_mode_changed_callback(CajaWindowInfo *window,
                                               FMTreeView *view) {
  update_filtering_from_preferences(view);
}
...
static void fm_tree_view_set_parent_window(FMTreeView *sidebar,
                                           CajaWindowInfo *window) {
  char *location;
  CajaWindowSlotInfo *slot;
  ...
  g_signal_connect_object(window, "hidden_files_mode_changed",
                          G_CALLBACK(hidden_files_mode_changed_callback),
                          sidebar, 0);
}

static CajaSidebar *fm_tree_view_create(CajaSidebarProvider *provider,
                                        CajaWindowInfo *window) {
  FMTreeView *sidebar;
...
static void fm_tree_view_provider_init(FMTreeViewProvider *sidebar) {}

static void fm_tree_view_provider_class_init(FMTreeViewProviderClass *class) {}
