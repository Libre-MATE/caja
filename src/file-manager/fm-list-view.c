...
static GList *fm_list_view_get_selection_for_file_transfer(
...
static void fm_list_view_scale_font_size(FMListView *view,
                                         CajaZoomLevel new_level);
static void fm_list_view_scroll_to_file(FMListView *view, CajaFile *file);
static void fm_list_view_iface_init(CajaViewIface *iface);
static void fm_list_view_rename_callback(CajaFile *file, GFile *result_location,
...
static const gchar *get_default_sort_order(CajaFile *file, gboolean *reversed) {
  const gchar *retval;
  const char *attributes[] = {
      "name", /* is really "manually" which doesn't apply to lists */
      "name",          "uri",          "size",          "type",
      "date_modified", "date_created", "date_accessed", "emblems",
      "trashed_on",    "size_on_disk", "extension",     NULL};
...
static void tree_selection_foreach_set_boolean(GtkTreeModel *model,
                                               GtkTreePath *path,
                                               GtkTreeIter *iter,
...
static gboolean tree_selection_not_empty(GtkTreeSelection *selection) {
  gboolean not_empty;

  not_empty = FALSE;
  gtk_tree_selection_selected_foreach(
      selection, tree_selection_foreach_set_boolean, &not_empty);
  return not_empty;
}

static gboolean tree_view_has_selection(GtkTreeView *view) {
  return tree_selection_not_empty(gtk_tree_view_get_selection(view));
}

static void activate_selected_items(FMListView *view) {
  GList *file_list;
  ...
  if (view->details->renaming_file) {
    ...
    if (view->details->renaming_file_activate_timeout == 0) {
      view->details->renaming_file_activate_timeout =
          g_timeout_add(WAIT_FOR_RENAME_ON_ACTIVATE,
                        (GSourceFunc)activate_selected_items, view);
    }
    return;
  }
  ...
  fm_directory_view_activate_files(FM_DIRECTORY_VIEW(view), file_list,
                                   CAJA_WINDOW_OPEN_ACCORDING_TO_MODE, 0, TRUE);
  caja_file_list_free(file_list);
}

static void activate_selected_items_alternate(FMListView *view, CajaFile *file,
...
static void fm_list_view_did_not_drag(FMListView *view, GdkEventButton *event) {
  GtkTreeView *tree_view;
  GtkTreeSelection *selection;
  GtkTreePath *path;
...
static void drag_data_get_callback(GtkWidget *widget, GdkDragContext *context,
  ...
  if (EGG_IS_TREE_MULTI_DRAG_SOURCE(model)) {
    egg_tree_multi_drag_source_drag_data_get(EGG_TREE_MULTI_DRAG_SOURCE(model),
                                             ref_list, selection_data);
  }
}

static void filtered_selection_foreach(GtkTreeModel *model, GtkTreePath *path,
  ...
  selection_data->list = g_list_prepend(
      selection_data->list, gtk_tree_row_reference_new(model, path));
}

static GList *get_filtered_selection_refs(GtkTreeView *tree_view) {
  struct SelectionForeachData selection_data;
  ...
  gtk_tree_selection_selected_foreach(
      selection_data.selection, filtered_selection_foreach, &selection_data);
  return g_list_reverse(selection_data.list);
}

static void ref_list_free(GList *ref_list) {
  g_list_free_full(ref_list, (GDestroyNotify)gtk_tree_row_reference_free);
}

static void stop_drag_check(FMListView *view) {
  view->details->drag_button = 0;
}

static cairo_surface_t *get_drag_surface(FMListView *view) {
  GtkTreePath *path;
  GtkTreeIter iter;
  cairo_surface_t *ret;
  ...
  if (gtk_tree_view_get_path_at_pos(
          view->details->tree_view, view->details->drag_x,
          view->details->drag_y, &path, NULL, NULL, NULL)) {
    GtkTreeModel *model;
    ...
    gtk_tree_view_get_cell_area(view->details->tree_view, path,
                                view->details->file_name_column, &cell_area);
...
static void drag_begin_callback(GtkWidget *widget, GdkDragContext *context,
  ...
  g_object_set_data_full(G_OBJECT(context), "drag-info", ref_list,
                         (GDestroyNotify)ref_list_free);
}

static gboolean motion_notify_callback(GtkWidget *widget, GdkEventMotion *event,
                                       gpointer callback_data) {
  FMListView *view;
  ...
  if (view->details->drag_button != 0) {
    ...
    if (gtk_drag_check_threshold(widget, view->details->drag_x,
                                 view->details->drag_y, event->x, event->y)) {
      gtk_drag_begin_with_coordinates(
          widget, source_target_list,
          GDK_ACTION_MOVE | GDK_ACTION_COPY | GDK_ACTION_LINK | GDK_ACTION_ASK,
          view->details->drag_button, (GdkEvent *)event, event->x, event->y);
    }
    return TRUE;
  }
...
static void do_popup_menu(GtkWidget *widget, FMListView *view,
                          GdkEventButton *event) {
  if (tree_view_has_selection(GTK_TREE_VIEW(widget))) {
    fm_directory_view_pop_up_selection_context_menu(FM_DIRECTORY_VIEW(view),
                                                    event);
  } else {
    fm_directory_view_pop_up_background_context_menu(FM_DIRECTORY_VIEW(view),
                                                     event);
  }
}
...
static gboolean button_press_callback(GtkWidget *widget, GdkEventButton *event,
  ...
  fm_list_model_set_drag_view(FM_LIST_MODEL(gtk_tree_view_get_model(tree_view)),
...
static gboolean popup_menu_callback(GtkWidget *widget, gpointer callback_data) {
  FMListView *view;
...
static void subdirectory_done_loading_callback(CajaDirectory *directory,
                                               FMListView *view) {
...
static void row_expanded_callback(GtkTreeView *treeview, GtkTreeIter *iter,
  ...
  if (fm_list_model_load_subdirectory(view->details->model, path, &directory)) {
    ...
    caja_debug_log(
        FALSE, CAJA_DEBUG_LOG_DOMAIN_USER,
        "list view row expanded window=%p: %s",
        fm_directory_view_get_containing_window(FM_DIRECTORY_VIEW(view)), uri);
    g_free(uri);
...
static gboolean unload_file_timeout(gpointer data) {
  ...
  if (unload_data->view != NULL) {
    ...
    if (fm_list_model_get_tree_iter_from_file(model, unload_data->file,
                                              unload_data->directory, &iter)) {
      GtkTreePath *path;
...
static void row_collapsed_callback(GtkTreeView *treeview, GtkTreeIter *iter,
  ...
  if (gtk_tree_model_iter_parent(model, &parent, iter)) {
    gtk_tree_model_get(model, &parent, FM_LIST_MODEL_SUBDIRECTORY_COLUMN,
                       &directory, -1);
  }
...
static void subdirectory_unloaded_callback(FMListModel *model,
                                           CajaDirectory *directory,
                                           gpointer callback_data) {
  FMListView *view;
  ...
  switch (event->keyval) {
    ...
    case GDK_KEY_space:
      ...
      if (!gtk_widget_has_focus(
              GTK_WIDGET(FM_LIST_VIEW(view)->details->tree_view))) {
        handled = FALSE;
        break;
      }
      if ((event->state & GDK_SHIFT_MASK) != 0) {
...
static void fm_list_view_reveal_selection(FMDirectoryView *view) {
  ...
  if (selection != NULL) {
    ...
    if (fm_list_model_get_first_iter_for_file(list_view->details->model, file,
                                              &iter)) {
      GtkTreePath *path;
...
static void sort_column_changed_callback(GtkTreeSortable *sortable,
  ...
  sort_attr = fm_list_model_get_attribute_from_sort_column_id(
  ...
  if (view->details->last_sort_attr != sort_attr &&
     ...
     * with a reversed preference, or if it makes sense for the attribute (i.e.
     * date). */
    if (sort_attr == default_sort_attr) {
      /* use value from preferences */
      reversed = (g_settings_get_boolean(
                      caja_preferences,
                      CAJA_PREFERENCES_DEFAULT_SORT_IN_REVERSE_ORDER) != FALSE);
    } else {
...
static void cell_renderer_editing_started_cb(GtkCellRenderer *renderer,
  ...
  caja_clipboard_set_up_editable(
      GTK_EDITABLE(entry),
      fm_directory_view_get_ui_manager(FM_DIRECTORY_VIEW(list_view)), FALSE);
}

static void cell_renderer_editing_canceled(GtkCellRendererText *cell,
                                           FMListView *view) {
...
static void cell_renderer_edited(GtkCellRendererText *cell,
                                 const char *path_str, const char *new_text,
                                 FMListView *view) {
  GtkTreePath *path;
  CajaFile *file;
  GtkTreeIter iter;
   ...
   */
  if (new_text[0] == '\0') {
    g_object_set(G_OBJECT(view->details->file_name_cell), "editable", FALSE,
                 NULL);
    fm_directory_view_unfreeze_updates(FM_DIRECTORY_VIEW(view));
    return;
  }
  ...
  if (strcmp(new_text, view->details->original_name) != 0) {
    ...
    fm_rename_file(file, new_text, fm_list_view_rename_callback,
                   g_object_ref(view));
...
static char *get_root_uri_callback(CajaTreeViewDragDest *dest,
                                   gpointer user_data) {
  FMListView *view;
...
static CajaFile *get_file_for_path_callback(CajaTreeViewDragDest *dest,
                                            GtkTreePath *path,
                                            gpointer user_data) {
  FMListView *view;
...
static void list_view_handle_netscape_url(CajaTreeViewDragDest *dest,
...
static void list_view_handle_uri_list(CajaTreeViewDragDest *dest,
...
static void list_view_handle_text(CajaTreeViewDragDest *dest, const char *text,
                                  const char *target_uri, GdkDragAction action,
                                  int x, int y, FMListView *view) {
  fm_directory_view_handle_text_drop(FM_DIRECTORY_VIEW(view), text, target_uri,
                                     action, x, y);
}

static void list_view_handle_raw(CajaTreeViewDragDest *dest,
...
static void move_copy_items_callback(CajaTreeViewDragDest *dest,
...

{
  ...
  fm_directory_view_move_copy_items(item_uris, NULL, target_uri, action, x, y,
                                    view);
}

static void apply_columns_settings(FMListView *list_view, char **column_order,
...
static gboolean focus_in_event_callback(GtkWidget *widget, GdkEventFocus *event,
  ...
  slot_info =
      fm_directory_view_get_caja_window_slot(FM_DIRECTORY_VIEW(list_view));
  caja_window_slot_info_make_hosting_pane_active(slot_info);

  return FALSE;
}

static gint get_icon_scale_callback(FMListModel *model, FMListView *view) {
...
static void create_and_set_up_tree_view(FMListView *view) {
  ...
  binding_set =
      gtk_binding_set_by_class(GTK_WIDGET_GET_CLASS(view->details->tree_view));
  ...
  for (l = caja_columns; l != NULL; l = l->next) {
    ...
    g_object_get(caja_column, "name", &name, "label", &label, "xalign", &xalign,
                 NULL);

    column_num = fm_list_model_add_column(view->details->model, caja_column);
    ...
    if (!strcmp(name, "name")) {
      ...
      gtk_tree_view_column_set_min_width(view->details->file_name_column,
                                         20 * font_size);
      ...
      gtk_tree_view_column_set_attributes(
          ...
          FM_LIST_MODEL_SMALLEST_ICON_COLUMN, NULL);

      cell = gtk_cell_renderer_text_new();
      g_object_set(cell, "ellipsize", PANGO_ELLIPSIZE_END, "ellipsize-set",
                   TRUE, NULL);
      view->details->file_name_cell = (GtkCellRendererText *)cell;
      g_signal_connect(cell, "edited", G_CALLBACK(cell_renderer_edited), view);
      g_signal_connect(cell, "editing-canceled",
                       G_CALLBACK(cell_renderer_editing_canceled), view);
   ...
   * folder is loaded */
  apply_columns_settings(view, default_column_order_auto_value,
...
static void fm_list_view_add_file(FMDirectoryView *view, CajaFile *file,
                                  CajaDirectory *directory) {
  FMListModel *model;
...
static char **get_visible_columns(FMListView *list_view) {
  CajaFile *file;
  ...
  visible_columns = caja_file_get_metadata_list(
      file, CAJA_METADATA_KEY_LIST_VIEW_VISIBLE_COLUMNS);
...
static char **get_column_order(FMListView *list_view) {
  CajaFile *file;
  GList *column_order;
  char **ret;
  ...
  column_order = caja_file_get_metadata_list(
      file, CAJA_METADATA_KEY_LIST_VIEW_COLUMN_ORDER);
...
static void set_sort_order_from_metadata_and_preferences(
    FMListView *list_view) {
  char *sort_attribute;
  int sort_column_id;
  CajaFile *file;
  ...
  file = fm_directory_view_get_directory_as_file(FM_DIRECTORY_VIEW(list_view));
  sort_attribute = caja_file_get_metadata(
      file, CAJA_METADATA_KEY_LIST_VIEW_SORT_COLUMN, NULL);
  ...
  sort_reversed = caja_file_get_boolean_metadata(
      file, CAJA_METADATA_KEY_LIST_VIEW_SORT_REVERSED, default_sort_reversed);
...
static gboolean list_view_changed_foreach(GtkTreeModel *model,
...
static CajaZoomLevel get_default_zoom_level(void) {
...
static void set_zoom_level_from_metadata_and_preferences(
    FMListView *list_view) {
  if (fm_directory_view_supports_zooming(FM_DIRECTORY_VIEW(list_view))) {
    ...
    level = caja_file_get_integer_metadata(
        file, CAJA_METADATA_KEY_LIST_VIEW_ZOOM_LEVEL, get_default_zoom_level());
...
static void stop_cell_editing(FMListView *list_view) {
  GtkTreeViewColumn *column;
...
static void fm_list_view_file_changed(FMDirectoryView *view, CajaFile *file,
                                      CajaDirectory *directory) {
  FMListView *listview;
  GtkTreeIter iter;
  ...
  if (listview->details->renaming_file != NULL &&
     ...
     */
    if (fm_list_model_get_tree_iter_from_file(listview->details->model, file,
                                              directory, &iter)) {
      GtkTreePath *file_path;

      file_path = gtk_tree_model_get_path(
          GTK_TREE_MODEL(listview->details->model), &iter);
      gtk_tree_view_scroll_to_cell(listview->details->tree_view, file_path,
                                   NULL, FALSE, 0.0, 0.0);
...
static void fm_list_view_get_selection_foreach_func(GtkTreeModel *model,
                                                    GtkTreePath *path,
                                                    GtkTreeIter *iter,
                                                    gpointer data) {
  GList **list;
...
static GList *fm_list_view_get_selection(FMDirectoryView *view) {
  ...
  gtk_tree_selection_selected_foreach(
      gtk_tree_view_get_selection(FM_LIST_VIEW(view)->details->tree_view),
      fm_list_view_get_selection_foreach_func, &list);

  return g_list_reverse(list);
}

static void fm_list_view_get_selection_for_file_transfer_foreach_func(
...
static GList *fm_list_view_get_selection_for_file_transfer(
  ...
  gtk_tree_selection_selected_foreach(
      selection_data.selection,
      fm_list_view_get_selection_for_file_transfer_foreach_func,
      &selection_data);

  return g_list_reverse(selection_data.list);
}

static guint fm_list_view_get_item_count(FMDirectoryView *view) {
...
static void fm_list_view_remove_file(FMDirectoryView *view, CajaFile *file,
                                     CajaDirectory *directory) {
  GtkTreePath *path;
  GtkTreeIter iter;
  ...
  if (fm_list_model_get_tree_iter_from_file(list_view->details->model, file,
                                            directory, &iter)) {
    GtkTreePath *file_path;
    GtkTreeSelection *selection;
...
static void fm_list_view_set_selection(FMDirectoryView *view,
  ...
  for (node = selection; node != NULL; node = node->next) {
    file = node->data;
    iters =
        fm_list_model_get_all_iters_for_file(list_view->details->model, file);
...
static void fm_list_view_invert_selection(FMDirectoryView *view) {
  ...
  gtk_tree_selection_selected_foreach(
      tree_selection, fm_list_view_get_selection_foreach_func, &selection);
...
static void column_editor_response_callback(GtkWidget *dialog, int response_id,
                                            gpointer user_data) {
  gtk_widget_destroy(GTK_WIDGET(dialog));
}

static void column_chooser_changed_callback(CajaColumnChooser *chooser,
                                            FMListView *view) {
  CajaFile *file;
  char **visible_columns;
  char **column_order;
  ...
  caja_column_chooser_get_settings(chooser, &visible_columns, &column_order);
  ...
  caja_file_set_metadata_list(file, CAJA_METADATA_KEY_LIST_VIEW_VISIBLE_COLUMNS,
                              list);
  g_list_free(list);
  ...
  list = g_list_reverse(list);
  caja_file_set_metadata_list(file, CAJA_METADATA_KEY_LIST_VIEW_COLUMN_ORDER,
                              list);
  g_list_free(list);
...
static void column_chooser_set_from_arrays(CajaColumnChooser *chooser,
                                           FMListView *view,
                                           char **visible_columns,
                                           char **column_order) {
  ...
  caja_column_chooser_set_settings(chooser, visible_columns, column_order);

  g_signal_handlers_unblock_by_func(
      chooser, G_CALLBACK(column_chooser_changed_callback), view);
}

static void column_chooser_set_from_settings(CajaColumnChooser *chooser,
                                             FMListView *view) {
  char **visible_columns;
  char **column_order;
...
static void column_chooser_use_default_callback(CajaColumnChooser *chooser,
                                                FMListView *view) {
  CajaFile *file;
...
static GtkWidget *create_column_editor(FMListView *view) {
  GtkWidget *window;
  GtkWidget *label;
  GtkWidget *box;
  ...
  eel_dialog_add_button(GTK_DIALOG(window), _("_Close"), "window-close",
                        GTK_RESPONSE_CLOSE);
  ...
  column_chooser_set_from_settings(CAJA_COLUMN_CHOOSER(column_chooser), view);

  return window;
}

static void action_visible_columns_callback(GtkAction *action,
                                            gpointer callback_data) {
  FMListView *list_view;
...
static void fm_list_view_merge_menus(FMDirectoryView *view) {
  ...
  gtk_action_group_add_actions(action_group, list_view_entries,
...
static void fm_list_view_unmerge_menus(FMDirectoryView *view) {
  FMListView *list_view;
  GtkUIManager *ui_manager;
  ...
  ui_manager = fm_directory_view_get_ui_manager(view);
  if (ui_manager != NULL) {
    caja_ui_unmerge_ui(ui_manager, &list_view->details->list_merge_id,
...
static void fm_list_view_reset_to_defaults(FMDirectoryView *view) {
  ...
  caja_file_set_metadata(file, CAJA_METADATA_KEY_LIST_VIEW_SORT_COLUMN, NULL,
                         NULL);
  caja_file_set_metadata(file, CAJA_METADATA_KEY_LIST_VIEW_SORT_REVERSED, NULL,
                         NULL);
  caja_file_set_metadata(file, CAJA_METADATA_KEY_LIST_VIEW_ZOOM_LEVEL, NULL,
                         NULL);
  caja_file_set_metadata_list(file, CAJA_METADATA_KEY_LIST_VIEW_COLUMN_ORDER,
                              NULL);
  caja_file_set_metadata_list(file, CAJA_METADATA_KEY_LIST_VIEW_VISIBLE_COLUMNS,
  ...
  gtk_tree_sortable_set_sort_column_id(
      ...
      fm_list_model_get_sort_column_id_from_attribute(
          FM_LIST_VIEW(view)->details->model,
          g_quark_from_string(default_sort_order)),
      default_sort_reversed ? GTK_SORT_DESCENDING : GTK_SORT_ASCENDING);

  fm_list_view_set_zoom_level(FM_LIST_VIEW(view), get_default_zoom_level(),
                              FALSE);
...
static void fm_list_view_scale_font_size(FMListView *view,
  ...
  g_object_set(G_OBJECT(view->details->file_name_cell), "scale",
               pango_scale[new_level], NULL);
  for (l = view->details->cells; l != NULL; l = l->next) {
    g_object_set(G_OBJECT(l->data), "scale", pango_scale[new_level], NULL);
  }
}
...
static void fm_list_view_set_zoom_level(FMListView *view,
  ...
  caja_file_set_integer_metadata(
      ...
      CAJA_METADATA_KEY_LIST_VIEW_ZOOM_LEVEL, get_default_zoom_level(),
      new_level);
  ...
  gtk_cell_renderer_set_fixed_size(
      GTK_CELL_RENDERER(view->details->pixbuf_cell), -1, icon_size);
...
static CajaZoomLevel fm_list_view_get_zoom_level(FMDirectoryView *view) {
  FMListView *list_view;
...
static void fm_list_view_start_renaming_file(FMDirectoryView *view,
                                             CajaFile *file,
                                             gboolean select_all) {
  FMListView *list_view;
  GtkTreeIter iter;
  ...
  if (list_view->details->file_name_column &&
      list_view->details->editable_widget) {
    gtk_editable_select_region(
        GTK_EDITABLE(list_view->details->editable_widget), 0, -1);
    return;
  }
  ...
  /* Freeze updates to the view to prevent losing rename focus when the tree
   * view updates */
  fm_directory_view_freeze_updates(FM_DIRECTORY_VIEW(view));

  path =
      gtk_tree_model_get_path(GTK_TREE_MODEL(list_view->details->model), &iter);

  /* Make filename-cells editable. */
  g_object_set(G_OBJECT(list_view->details->file_name_cell), "editable", TRUE,
               NULL);

  gtk_tree_view_scroll_to_cell(list_view->details->tree_view, NULL,
                               list_view->details->file_name_column, TRUE, 0.0,
                               0.0);
  gtk_tree_view_set_cursor_on_cell(
      list_view->details->tree_view, path, list_view->details->file_name_column,
      GTK_CELL_RENDERER(list_view->details->file_name_cell), TRUE);
  ...
  if (list_view->details->editable_widget != NULL) {
    ...
    gtk_editable_select_region(
        GTK_EDITABLE(list_view->details->editable_widget), start_offset,
        end_offset);
  }
...
static void fm_list_view_click_policy_changed(FMDirectoryView *directory_view) {
  GdkDisplay *display;
  FMListView *view;
  GtkTreeIter iter;
...
static void fm_list_view_sort_directories_first_changed(FMDirectoryView *view) {
  ...
  fm_list_model_set_should_sort_directories_first(
      list_view->details->model,
      fm_directory_view_should_sort_directories_first(view));
}

static int fm_list_view_compare_files(FMDirectoryView *view, CajaFile *file1,
                                      CajaFile *file2) {
  FMListView *list_view;
...
static gboolean fm_list_view_using_manual_layout(FMDirectoryView *view) {
  g_return_val_if_fail(FM_IS_LIST_VIEW(view), FALSE);

  return FALSE;
}

static void fm_list_view_dispose(GObject *object) {
  FMListView *list_view;
...
static void fm_list_view_finalize(GObject *object) {
  ...
  g_signal_handlers_disconnect_by_func(
      caja_preferences, default_sort_order_changed_callback, list_view);
...
static char *fm_list_view_get_first_visible_file(CajaView *view) {
  CajaFile *file;
  GtkTreePath *path;
  GtkTreeIter iter;
  FMListView *list_view;
...
static void fm_list_view_scroll_to_file(FMListView *view, CajaFile *file) {
  ...
  if (!fm_list_model_get_first_iter_for_file(view->details->model, file,
                                             &iter)) {
    return;
  }
...
static void list_view_scroll_to_file(CajaView *view, const char *uri) {
  if (uri != NULL) {
    CajaFile *file;
...
static void list_view_notify_clipboard_info(CajaClipboardMonitor *monitor,
...
static void fm_list_view_end_loading(FMDirectoryView *view,
...
static void real_set_is_active(FMDirectoryView *view, gboolean is_active) {
  ...
  if (is_active) {
    gtk_widget_override_background_color(tree_view, GTK_STATE_FLAG_NORMAL,
                                         NULL);
  } else {
  ...
  EEL_CALL_PARENT(FM_DIRECTORY_VIEW_CLASS, set_is_active, (view, is_active));
}

static void fm_list_view_class_init(FMListViewClass *class) {
  ...
  fm_directory_view_class->get_background_widget =
      fm_list_view_get_background_widget;
  fm_directory_view_class->get_selection = fm_list_view_get_selection;
  fm_directory_view_class->get_selection_for_file_transfer =
      fm_list_view_get_selection_for_file_transfer;
  ...
  fm_directory_view_class->sort_directories_first_changed =
      fm_list_view_sort_directories_first_changed;
  ...
  eel_g_settings_add_auto_enum(caja_preferences, CAJA_PREFERENCES_CLICK_POLICY,
...
static const char *fm_list_view_get_id(CajaView *view) {
...
static void fm_list_view_init(FMListView *list_view) {
  ...
  g_signal_connect_swapped(
      ...
      G_CALLBACK(default_zoom_level_changed_callback), list_view);
  g_signal_connect_swapped(
      caja_list_view_preferences,
...
static CajaView *fm_list_view_create(CajaWindowSlotInfo *slot) {
  FMListView *view;

  view = g_object_new(FM_TYPE_LIST_VIEW, "window-slot", slot, NULL);
  return CAJA_VIEW(view);
}

static gboolean fm_list_view_supports_uri(const char *uri, GFileType file_type,
...
void fm_list_view_register(void) {
  ...
  fm_list_view.view_menu_label_with_mnemonic =
      _(fm_list_view.view_menu_label_with_mnemonic);
