...
enum { PATH_CLICKED, PATH_EVENT, LAST_SIGNAL };
...
G_DEFINE_TYPE(CajaPathBar, caja_path_bar, GTK_TYPE_CONTAINER);

static void caja_path_bar_finalize(GObject *object);
static void caja_path_bar_dispose(GObject *object);

static void caja_path_bar_get_preferred_width(GtkWidget *widget, gint *minimum,
                                              gint *natural);
static void caja_path_bar_get_preferred_height(GtkWidget *widget, gint *minimum,
                                               gint *natural);

static void caja_path_bar_unmap(GtkWidget *widget);
static void caja_path_bar_size_allocate(GtkWidget *widget,
                                        GtkAllocation *allocation);
static void caja_path_bar_add(GtkContainer *container, GtkWidget *widget);
static void caja_path_bar_remove(GtkContainer *container, GtkWidget *widget);
...
static gboolean caja_path_bar_slider_button_release(GtkWidget *widget,
                                                    GdkEventButton *event,
                                                    CajaPathBar *path_bar);
static void caja_path_bar_grab_notify(GtkWidget *widget, gboolean was_grabbed);
...
static void update_button_types(CajaPathBar *path_bar) {
  GList *list;
  GFile *path = NULL;
...
static void trash_state_changed_cb(CajaTrashMonitor *monitor, gboolean state,
  ...
  for (list = path_bar->button_list; list; list = list->next) {
    ...
    if (g_file_equal(file, button_data->path)) {
      ...
      surface = caja_icon_info_get_surface_at_size(icon_info,
...
static void caja_path_bar_slider_drag_motion(GtkWidget *widget,
  ...
  if (path_bar->drag_slider_timeout == 0) {
    ...
    path_bar->drag_slider_timeout =
        g_timeout_add(timeout, slider_timeout, path_bar);
...
static void caja_path_bar_slider_drag_leave(GtkWidget *widget,
...
static void caja_path_bar_init(CajaPathBar *path_bar) {
  char *p;
  ...
  path_bar->up_slider_button =
      get_slider_button(path_bar, "pan-start-symbolic");
  path_bar->down_slider_button =
      get_slider_button(path_bar, "pan-end-symbolic");
  gtk_style_context_add_class(
      gtk_widget_get_style_context(GTK_WIDGET(path_bar->up_slider_button)),
      "slider-button");
  gtk_style_context_add_class(
      gtk_widget_get_style_context(GTK_WIDGET(path_bar->down_slider_button)),
      "slider-button");
  ...
  g_signal_connect_swapped(
      ...
      G_CALLBACK(desktop_location_changed_callback), path_bar);

  g_signal_connect_swapped(path_bar->up_slider_button, "clicked",
                           G_CALLBACK(caja_path_bar_scroll_up), path_bar);
  g_signal_connect_swapped(path_bar->down_slider_button, "clicked",
                           G_CALLBACK(caja_path_bar_scroll_down), path_bar);

  g_signal_connect(path_bar->up_slider_button, "button_press_event",
                   G_CALLBACK(caja_path_bar_slider_button_press), path_bar);
  g_signal_connect(path_bar->up_slider_button, "button_release_event",
                   G_CALLBACK(caja_path_bar_slider_button_release), path_bar);
  g_signal_connect(path_bar->down_slider_button, "button_press_event",
                   G_CALLBACK(caja_path_bar_slider_button_press), path_bar);
  g_signal_connect(path_bar->down_slider_button, "button_release_event",
                   G_CALLBACK(caja_path_bar_slider_button_release), path_bar);

  gtk_drag_dest_set(GTK_WIDGET(path_bar->up_slider_button), 0, NULL, 0, 0);
  gtk_drag_dest_set_track_motion(GTK_WIDGET(path_bar->up_slider_button), TRUE);
  g_signal_connect(path_bar->up_slider_button, "drag-motion",
                   G_CALLBACK(caja_path_bar_slider_drag_motion), path_bar);
  g_signal_connect(path_bar->up_slider_button, "drag-leave",
                   G_CALLBACK(caja_path_bar_slider_drag_leave), path_bar);

  gtk_drag_dest_set(GTK_WIDGET(path_bar->down_slider_button), 0, NULL, 0, 0);
  gtk_drag_dest_set_track_motion(GTK_WIDGET(path_bar->up_slider_button), TRUE);
  g_signal_connect(path_bar->down_slider_button, "drag-motion",
                   G_CALLBACK(caja_path_bar_slider_drag_motion), path_bar);
  g_signal_connect(path_bar->down_slider_button, "drag-leave",
                   G_CALLBACK(caja_path_bar_slider_drag_leave), path_bar);

  g_signal_connect(caja_trash_monitor_get(), "trash_state_changed",
                   G_CALLBACK(trash_state_changed_cb), path_bar);
}

static void caja_path_bar_class_init(CajaPathBarClass *path_bar_class) {
  ...
  path_bar_signals[PATH_CLICKED] = g_signal_new(
      "path-clicked", G_OBJECT_CLASS_TYPE(path_bar_class), G_SIGNAL_RUN_FIRST,
      G_STRUCT_OFFSET(CajaPathBarClass, path_clicked), NULL, NULL,
      g_cclosure_marshal_VOID__OBJECT, G_TYPE_NONE, 1, G_TYPE_FILE);

  path_bar_signals[PATH_EVENT] =
      g_signal_new("path-event", G_OBJECT_CLASS_TYPE(path_bar_class),
                   G_SIGNAL_RUN_FIRST | G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(CajaPathBarClass, path_event), NULL, NULL,
                   NULL, G_TYPE_BOOLEAN, 2, G_TYPE_FILE, GDK_TYPE_EVENT);
...
static void caja_path_bar_finalize(GObject *object) {
  ...
  g_signal_handlers_disconnect_by_func(
      caja_preferences, desktop_location_changed_callback, path_bar);

  G_OBJECT_CLASS(caja_path_bar_parent_class)->finalize(object);
}

/* Removes the settings signal handler.  It's safe to call multiple times */
static void remove_settings_signal(CajaPathBar *path_bar, GdkScreen *screen) {
  if (path_bar->settings_signal_id) {
    GtkSettings *settings;

    settings = gtk_settings_get_for_screen(screen);
    g_signal_handler_disconnect(settings, path_bar->settings_signal_id);
    path_bar->settings_signal_id = 0;
  }
}

static void caja_path_bar_dispose(GObject *object) {
  remove_settings_signal(CAJA_PATH_BAR(object),
...
static void caja_path_bar_get_preferred_width(GtkWidget *widget, gint *minimum,
  ...
  *minimum = *natural = 0;
   ...
   */
  gtk_widget_get_preferred_width(path_bar->down_slider_button, &slider_width,
                                 NULL);
  gtk_widget_get_preferred_width(path_bar->up_slider_button, &slider_width,
                                 NULL);
...
static void caja_path_bar_get_preferred_height(GtkWidget *widget, gint *minimum,
  ...
  *minimum = *natural = 0;
  ...
  for (list = path_bar->button_list; list; list = list->next) {
    button_data = BUTTON_DATA(list->data);
    gtk_widget_get_preferred_height(button_data->button, &child_min,
                                    &child_nat);
...
static void caja_path_bar_update_slider_buttons(CajaPathBar *path_bar) {
  if (path_bar->button_list) {
    GtkWidget *button;
...
static void caja_path_bar_size_allocate(GtkWidget *widget,
  ...
  gtk_widget_get_preferred_size(
      BUTTON_DATA(path_bar->button_list->data)->button, &child_requisition,
      NULL);
  ...
  if (width <= allocation_width) {
    ...
    while (list && !reached_end) {
      ...
      if (width + child_requisition.width + path_bar->spacing + slider_space >
          allocation_width) {
        reached_end = TRUE;
      } else {
        if (list == path_bar->fake_root) {
  ...
  for (list = first_button; list; list = list->prev) {
    ...
    if (need_sliders && direction == GTK_TEXT_DIR_RTL) {
      if (child_allocation.x - path_bar->spacing - path_bar->slider_width <
          widget_allocation.x) {
        break;
      }
    } else {
      if (need_sliders && direction == GTK_TEXT_DIR_LTR) {
        if (child_allocation.x + child_allocation.width + path_bar->spacing +
                path_bar->slider_width >
    ...
    if (direction == GTK_TEXT_DIR_RTL) {
      ...
      down_slider_offset =
          child_allocation.x - allocation->x - path_bar->slider_width;
    } else {
...
static void caja_path_bar_screen_changed(GtkWidget *widget,
  ...
  if (GTK_WIDGET_CLASS(caja_path_bar_parent_class)->screen_changed) {
    GTK_WIDGET_CLASS(caja_path_bar_parent_class)
...
static gboolean caja_path_bar_scroll(GtkWidget *widget, GdkEventScroll *event) {
  CajaPathBar *path_bar;
...
static void caja_path_bar_remove_1(GtkContainer *container, GtkWidget *widget) {
...
static void caja_path_bar_remove(GtkContainer *container, GtkWidget *widget) {
  ...
  while (children) {
    if (widget == BUTTON_DATA(children->data)->button) {
      caja_path_bar_remove_1(container, widget);
      path_bar->button_list =
          g_list_remove_link(path_bar->button_list, children);
...
static void caja_path_bar_scroll_down(CajaPathBar *path_bar) {
  ...
  for (list = path_bar->button_list; list; list = list->next) {
    if (list->next &&
        gtk_widget_get_child_visible(BUTTON_DATA(list->next->data)->button)) {
      down_button = list;
      break;
    }
  }
  ...
  gtk_widget_get_allocation(BUTTON_DATA(down_button->data)->button,
                            &button_allocation);
   ...
   * need. */
  gtk_widget_get_allocation(BUTTON_DATA(up_button->data)->button,
                            &button_allocation);
...
static void caja_path_bar_scroll_up(CajaPathBar *path_bar) {
  ...
  for (list = g_list_last(path_bar->button_list); list; list = list->prev) {
    if (list->prev &&
        gtk_widget_get_child_visible(BUTTON_DATA(list->prev->data)->button)) {
...
static gboolean caja_path_bar_scroll_timeout(CajaPathBar *path_bar) {
  ...
  if (path_bar->timer) {
    ...
    if (path_bar->need_timer) {
      ...
      path_bar->timer = g_timeout_add(
          SCROLL_TIMEOUT, (GSourceFunc)caja_path_bar_scroll_timeout, path_bar);
  ...
  if (!path_bar->timer) {
    ...
    path_bar->timer =
        g_timeout_add(INITIAL_SCROLL_TIMEOUT,
                      (GSourceFunc)caja_path_bar_scroll_timeout, path_bar);
  }
...
static void reload_icons(CajaPathBar *path_bar) {
  GList *list;
...
static void change_icon_theme(CajaPathBar *path_bar) {
  path_bar->icon_size = CAJA_PATH_BAR_ICON_SIZE;
...
static void caja_path_bar_check_icon_theme(CajaPathBar *path_bar) {
  ...
  path_bar->settings_signal_id = g_signal_connect(
      settings, "notify", G_CALLBACK(settings_notify_cb), path_bar);
...
static gboolean button_event_cb(GtkWidget *button, GdkEventButton *event,
                                gpointer data) {
  ButtonData *button_data;
  CajaPathBar *path_bar;
  GList *button_list;
  ...
  if (event->type == GDK_BUTTON_RELEASE &&
      !GPOINTER_TO_UINT(
          g_object_get_data(G_OBJECT(button), "handle-button-release"))) {
    return FALSE;
  }
  ...
  g_signal_emit(path_bar, path_bar_signals[PATH_EVENT], 0, button_data->path,
                event, &retval);

  return retval;
}

static void button_drag_begin_cb(GtkWidget *widget,
...
static CajaIconInfo *get_type_icon_info(ButtonData *button_data) {
  gint icon_scale =
      gtk_widget_get_scale_factor(GTK_WIDGET(button_data->button));

  switch (button_data->type) {
    case ROOT_BUTTON:
      return caja_icon_info_lookup_from_name(
          CAJA_ICON_FILESYSTEM, CAJA_PATH_BAR_ICON_SIZE, icon_scale);

    case HOME_BUTTON:
      return caja_icon_info_lookup_from_name(
          CAJA_ICON_HOME, CAJA_PATH_BAR_ICON_SIZE, icon_scale);

    case DESKTOP_BUTTON:
      return caja_icon_info_lookup_from_name(
          CAJA_ICON_DESKTOP, CAJA_PATH_BAR_ICON_SIZE, icon_scale);

    case NORMAL_BUTTON:
      if (button_data->is_base_dir) {
        return caja_file_get_icon(button_data->file, CAJA_PATH_BAR_ICON_SIZE,
                                  icon_scale, CAJA_FILE_ICON_FLAGS_NONE);
      }
 ...
 */
static void set_label_padding_size(ButtonData *button_data) {
...
static void caja_path_bar_update_button_appearance(ButtonData *button_data) {
  ...
  if (button_data->image != NULL) {
    if (button_data->custom_icon) {
      gtk_image_set_from_surface(GTK_IMAGE(button_data->image),
      ...
      if (surface != NULL) {
        gtk_image_set_from_surface(GTK_IMAGE(button_data->image), surface);
        gtk_style_context_add_class(
            gtk_widget_get_style_context(button_data->button), "image-button");
        gtk_widget_show(GTK_WIDGET(button_data->image));
...
static gboolean setup_file_path_mounted_mount(GFile *location,
                                              ButtonData *button_data) {
  ...
  if (!button_data->button) return FALSE;
  result = FALSE;
  ...
  for (l = mounts; l != NULL; l = l->next) {
    ...
    if (g_file_equal(location, root)) {
      ...
      g_object_unref(icon);
      button_data->custom_icon =
          caja_icon_info_get_surface_at_size(info, CAJA_PATH_BAR_ICON_SIZE);
      g_object_unref(info);
...
static void setup_button_type(ButtonData *button_data, CajaPathBar *path_bar,
                              GFile *location) {
  if (path_bar->root_path != NULL &&
      g_file_equal(location, path_bar->root_path)) {
    button_data->type = ROOT_BUTTON;
  } else if (path_bar->home_path != NULL &&
             g_file_equal(location, path_bar->home_path)) {
    button_data->type = HOME_BUTTON;
    button_data->fake_root = TRUE;
  } else if (path_bar->desktop_path != NULL &&
             g_file_equal(location, path_bar->desktop_path)) {
    if (!desktop_is_home) {
...
static void button_drag_data_get_cb(GtkWidget *widget, GdkDragContext *context,
...
static void setup_button_drag_source(ButtonData *button_data) {
  GtkTargetList *target_list;
  const GtkTargetEntry targets[] = {
      {CAJA_ICON_DND_MATE_ICON_LIST_TYPE, 0, CAJA_ICON_DND_MATE_ICON_LIST}};

  gtk_drag_source_set(
      button_data->button, GDK_BUTTON1_MASK | GDK_BUTTON2_MASK, NULL, 0,
...
static void button_data_file_changed(CajaFile *file, ButtonData *button_data) {
  ...
  if (!g_file_equal(button_data->path, location)) {
    ...
    if (renamed) {
       ...
       * bar
       */
      child = g_file_has_prefix(button_data->path, path_bar->current_path);
    ...
    /* if the current or a parent location are gone, don't do anything, as the
     * view will get the event too and call us back.
     */
    current_location = caja_file_get_location(current_button_data->file);
    ...
    if (g_file_has_prefix(location, current_location)) {
      ...
      position =
          g_list_position(path_bar->button_list,
...
static ButtonData *make_directory_button(CajaPathBar *path_bar, CajaFile *file,
                                         gboolean current_dir,
                                         gboolean base_dir,
  ...
  g_signal_connect(button_data->button, "button-press-event",
                   G_CALLBACK(button_event_cb), button_data);
  g_signal_connect(button_data->button, "button-release-event",
                   G_CALLBACK(button_event_cb), button_data);
  g_signal_connect(button_data->button, "drag-begin",
                   G_CALLBACK(button_drag_begin_cb), button_data);
  ...
  caja_drag_slot_proxy_init(button_data->button, &(button_data->drag_info));
...
static gboolean caja_path_bar_check_parent_path(
    CajaPathBar *path_bar, GFile *location, ButtonData **current_button_data) {
  GList *list;
  GList *current_path;
...
gboolean caja_path_bar_set_path(CajaPathBar *path_bar, GFile *file_path) {
  ButtonData *button_data;
