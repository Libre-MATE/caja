...
void caja_x_content_bar_set_x_content_type(CajaXContentBar *bar,
  ...
  if (strcmp(x_content_type, "x-content/audio-cdda") == 0) {
    ...
    message = g_strdup_printf(_("The media has been detected as \"%s\"."),
                              description);
  }
  ...
  /* TODO: We really need a GtkBrowserBackButton-ish widget here.. until then,
   * we only show the default application. */
...
static void caja_x_content_bar_set_property(GObject *object, guint prop_id,
                                            const GValue *value,
                                            GParamSpec *pspec) {
  CajaXContentBar *bar;
...
static void caja_x_content_bar_get_property(GObject *object, guint prop_id,
...
static void caja_x_content_bar_finalize(GObject *object) {
  CajaXContentBar *bar = CAJA_X_CONTENT_BAR(object);

  g_free(bar->priv->x_content_type);
  if (bar->priv->mount != NULL) g_object_unref(bar->priv->mount);

  G_OBJECT_CLASS(caja_x_content_bar_parent_class)->finalize(object);
}

static void caja_x_content_bar_class_init(CajaXContentBarClass *klass) {
  ...
  g_object_class_install_property(
      object_class, PROP_X_CONTENT_TYPE,
      g_param_spec_string("x-content-type",
                          "The x-content type for the cluebar",
                          "The x-content type for the cluebar", NULL,
                          G_PARAM_READWRITE | G_PARAM_CONSTRUCT));
}

static void button_clicked_callback(GtkWidget *button, CajaXContentBar *bar) {
  GAppInfo *default_app;

  if (bar->priv->x_content_type == NULL || bar->priv->mount == NULL) return;

  default_app =
      g_app_info_get_default_for_type(bar->priv->x_content_type, FALSE);
  if (default_app != NULL) {
...
static void caja_x_content_bar_init(CajaXContentBar *bar) {
  GtkWidget *hbox;
  ...
  g_signal_connect(bar->priv->button, "clicked",
                   G_CALLBACK(button_clicked_callback), bar);
}

GtkWidget *caja_x_content_bar_new(GMount *mount, const char *x_content_type) {
  GObject *bar;

  bar = g_object_new(CAJA_TYPE_X_CONTENT_BAR, "mount", mount, "x-content-type",
                     x_content_type, NULL);
