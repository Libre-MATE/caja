...
static GtkTreeView *bookmark_list_widget =
    NULL; /* awkward name to distinguish from CajaBookmarkList */
...
static void on_bookmark_list_changed(CajaBookmarkList *list,
                                     gpointer user_data);
static void on_name_field_changed(GtkEditable *editable, gpointer user_data);
static void on_remove_button_clicked(GtkButton *button, gpointer user_data);
static void on_jump_button_clicked(GtkButton *button, gpointer user_data);
static void on_row_changed(GtkListStore *store, GtkTreePath *path,
                           GtkTreeIter *iter, gpointer user_data);
static void on_row_deleted(GtkListStore *store, GtkTreePath *path,
                           gpointer user_data);
static void on_row_activated(GtkTreeView *view, GtkTreePath *path,
                             GtkTreeViewColumn *column, gpointer user_data);
static gboolean on_button_pressed(GtkTreeView *view, GdkEventButton *event,
                                  gpointer user_data);
static gboolean on_key_pressed(GtkTreeView *view, GdkEventKey *event,
                               gpointer user_data);
...
static gboolean on_text_field_focus_out_event(GtkWidget *widget,
                                              GdkEventFocus *event,
                                              gpointer user_data);
static void on_uri_field_changed(GtkEditable *editable, gpointer user_data);
static gboolean on_window_delete_event(GtkWidget *widget, GdkEvent *event,
                                       gpointer user_data);
static void on_window_hide_event(GtkWidget *widget, gpointer user_data);
static void on_window_destroy_event(GtkWidget *widget, gpointer user_data);
...
static void caja_bookmarks_window_response_callback(GtkDialog *dialog,
                                                    int response_id,
...
static GtkListStore *create_bookmark_store(void) {
  return gtk_list_store_new(BOOKMARK_LIST_COLUMN_COUNT,
                            CAIRO_GOBJECT_TYPE_SURFACE, G_TYPE_STRING,
                            G_TYPE_OBJECT, PANGO_TYPE_STYLE);
}
...
static void setup_empty_list(void) {
  ...
  gtk_list_store_set(bookmark_empty_list_store, &iter,
                     ...
                     BOOKMARK_LIST_COLUMN_STYLE, PANGO_STYLE_ITALIC, -1);
}

static void bookmarks_set_empty(gboolean empty) {
  GtkTreeIter iter;
  ...
  if (empty) {
    ...
    if (caja_bookmark_list_length(bookmarks) > 0 && !get_selection_exists()) {
      gtk_tree_model_iter_nth_child(GTK_TREE_MODEL(bookmark_list_store), &iter,
                                    NULL, 0);
...
static void edit_bookmarks_dialog_reset_signals(gpointer data, GObject *obj) {
  g_signal_handler_disconnect(jump_button, jump_button_signal_id);
  g_signal_handler_disconnect(bookmark_list_widget, row_activated_signal_id);
  jump_button_signal_id = g_signal_connect(
      jump_button, "clicked", G_CALLBACK(on_jump_button_clicked), NULL);
 ...
 **/
GtkWindow *create_bookmarks_window(CajaBookmarkList *list,
  ...
  builder =
      gtk_builder_new_from_resource("/org/mate/caja/caja-bookmarks-window.ui");

  window = (GtkWidget *)gtk_builder_get_object(builder, "bookmarks_dialog");
  bookmark_list_widget =
      (GtkTreeView *)gtk_builder_get_object(builder, "bookmark_tree_view");
  remove_button =
      (GtkWidget *)gtk_builder_get_object(builder, "bookmark_delete_button");
  jump_button =
      (GtkWidget *)gtk_builder_get_object(builder, "bookmark_jump_button");
  ...
  g_object_weak_ref(G_OBJECT(window_source),
                    edit_bookmarks_dialog_reset_signals, window_source);

  bookmark_list_widget =
      GTK_TREE_VIEW(gtk_builder_get_object(builder, "bookmark_tree_view"));

  rend = gtk_cell_renderer_pixbuf_new();
  col = gtk_tree_view_column_new_with_attributes(
      "Icon", rend, "surface", BOOKMARK_LIST_COLUMN_ICON, NULL);
  gtk_tree_view_append_column(bookmark_list_widget, GTK_TREE_VIEW_COLUMN(col));
  gtk_tree_view_column_set_fixed_width(GTK_TREE_VIEW_COLUMN(col),
                                       CAJA_ICON_SIZE_SMALLER);

  rend = gtk_cell_renderer_text_new();
  g_object_set(rend, "ellipsize", PANGO_ELLIPSIZE_END, "ellipsize-set", TRUE,
               NULL);

  col = gtk_tree_view_column_new_with_attributes(
      "Icon", rend, "text", BOOKMARK_LIST_COLUMN_NAME, "style",
      BOOKMARK_LIST_COLUMN_STYLE, NULL);
  gtk_tree_view_append_column(bookmark_list_widget, GTK_TREE_VIEW_COLUMN(col));
  ...
  gtk_box_pack_start(
      GTK_BOX(gtk_builder_get_object(builder, "bookmark_name_placeholder")),
  ...
  gtk_box_pack_start(
      GTK_BOX(gtk_builder_get_object(builder, "bookmark_location_placeholder")),
  ...
  row_changed_signal_id = g_signal_connect(bookmark_list_store, "row_changed",
                                           G_CALLBACK(on_row_changed), NULL);
  row_deleted_signal_id = g_signal_connect(bookmark_list_store, "row_deleted",
  ...
  selection_changed_id = g_signal_connect(
      bookmark_selection, "changed", G_CALLBACK(on_selection_changed), NULL);

  g_signal_connect(window, "delete_event", G_CALLBACK(on_window_delete_event),
                   NULL);
  g_signal_connect(window, "hide", G_CALLBACK(on_window_hide_event), NULL);
  g_signal_connect(window, "destroy", G_CALLBACK(on_window_destroy_event),
                   NULL);
  g_signal_connect(window, "response",
                   G_CALLBACK(caja_bookmarks_window_response_callback), NULL);

  name_field_changed_signal_id = g_signal_connect(
      name_field, "changed", G_CALLBACK(on_name_field_changed), NULL);
  ...
  uri_field_changed_signal_id = g_signal_connect(
      uri_field, "changed", G_CALLBACK(on_uri_field_changed), NULL);
...
void edit_bookmarks_dialog_set_signals(CajaWindow *window) {
  ...
  jump_button_signal_id = g_signal_connect(
      jump_button, "clicked", G_CALLBACK(on_jump_button_clicked), window);
  row_activated_signal_id =
      g_signal_connect(bookmark_list_widget, "row_activated",
...
static CajaBookmark *get_selected_bookmark(void) {
  g_return_val_if_fail(CAJA_IS_BOOKMARK_LIST(bookmarks), NULL);

  if (!get_selection_exists()) return NULL;

  if (caja_bookmark_list_length(bookmarks) < 1) return NULL;
...
static guint get_selected_row(void) {
  ...
  gtk_tree_selection_get_selected(bookmark_selection, &model, &iter);
...
static void caja_bookmarks_window_restore_geometry(GtkWidget *window) {
  ...
  if (window_geometry != NULL) {
    eel_gtk_window_set_initial_geometry_from_string(
        GTK_WINDOW(window), window_geometry, BOOKMARKS_WINDOW_MIN_WIDTH,
        BOOKMARKS_WINDOW_MIN_HEIGHT, FALSE);
...
static void on_bookmark_list_changed(CajaBookmarkList *bookmarks,
                                     gpointer data) {
  g_return_if_fail(CAJA_IS_BOOKMARK_LIST(bookmarks));
...
static void on_name_field_changed(GtkEditable *editable, gpointer user_data) {
  ...
  if (!get_selection_exists()) return;

  /* Update text displayed in list instantly. Also remember that
   * user has changed text so we update real bookmark later.
   */
  gtk_tree_selection_get_selected(bookmark_selection, NULL, &iter);

  gtk_list_store_set(bookmark_list_store, &iter, BOOKMARK_LIST_COLUMN_NAME,
                     gtk_entry_get_text(GTK_ENTRY(name_field)), -1);
  text_changed = TRUE;
...
static void bookmarks_delete_bookmark(void) {
  GtkTreeIter iter;
  GtkTreePath *path;
  ...
  if (!gtk_tree_selection_get_selected(bookmark_selection, NULL, &iter)) return;

  /* Remove the selected item from the list store. on_row_deleted() will
     remove it from the bookmark list. */
  path = gtk_tree_model_get_path(GTK_TREE_MODEL(bookmark_list_store), &iter);
  indices = gtk_tree_path_get_indices(path);
  row = indices[0];
  gtk_tree_path_free(path);
  ...
  rows =
      gtk_tree_model_iter_n_children(GTK_TREE_MODEL(bookmark_list_store), NULL);
...
static void on_row_changed(GtkListStore *store, GtkTreePath *path,
  ...
  indices = gtk_tree_path_get_indices(path);
  row = indices[0];
  gtk_tree_model_get(GTK_TREE_MODEL(store), iter, BOOKMARK_LIST_COLUMN_BOOKMARK,
                     &bookmark, -1);
  ...
  if (row < (gint)caja_bookmark_list_length(bookmarks)) {
    ...
    bookmark_in_list = caja_bookmark_list_item_at(bookmarks, row);

    if (bookmark_in_list == bookmark) insert_bookmark = FALSE;
  }
  ...
  if (insert_bookmark) {
    ...
    gtk_list_store_set(store, iter, BOOKMARK_LIST_COLUMN_BOOKMARK, bookmark,
                       -1);
...
static void on_selection_changed(GtkTreeSelection *treeselection,
  ...
  caja_entry_set_text(CAJA_ENTRY(name_field), name ? name : "");
...
static void update_bookmark_from_text(void) {
  if (text_changed) {
    ...
    location = g_file_parse_name(gtk_entry_get_text(GTK_ENTRY(uri_field)));

    bookmark =
        caja_bookmark_new(location, gtk_entry_get_text(GTK_ENTRY(name_field)),
    ...
    /* We also have to update the bookmark pointer in the list
       store. */
    gtk_tree_selection_get_selected(bookmark_selection, NULL, &iter);
    g_signal_handler_block(bookmark_list_store, row_changed_signal_id);

    bookmark_in_list = caja_bookmark_list_item_at(bookmarks, selected_row);
    ...
    gtk_list_store_set(bookmark_list_store, &iter,
                       ...
                       BOOKMARK_LIST_COLUMN_ICON, surface, -1);
...
static gboolean restore_geometry(gpointer data) {
...
static void repopulate(void) {
  ...
  g_signal_handler_block(bookmark_list_store, row_deleted_signal_id);
  g_signal_handler_block(bookmark_list_widget, row_activated_signal_id);
  g_signal_handler_block(bookmark_list_widget, key_pressed_signal_id);
  g_signal_handler_block(bookmark_list_widget, button_pressed_signal_id);

  gtk_list_store_clear(store);

  g_signal_handler_unblock(bookmark_list_widget, row_activated_signal_id);
  g_signal_handler_unblock(bookmark_list_widget, key_pressed_signal_id);
  g_signal_handler_unblock(bookmark_list_widget, button_pressed_signal_id);
  g_signal_handler_unblock(bookmark_list_store, row_deleted_signal_id);
  ...
  for (index = 0; index < caja_bookmark_list_length(bookmarks); ++index) {
    ...
    gtk_list_store_set(store, &iter, BOOKMARK_LIST_COLUMN_ICON,
                       bookmark_surface, BOOKMARK_LIST_COLUMN_NAME,
                       bookmark_name, BOOKMARK_LIST_COLUMN_BOOKMARK, bookmark,
                       BOOKMARK_LIST_COLUMN_STYLE, PANGO_STYLE_NORMAL, -1);
...
static int handle_close_accelerator(GtkWindow *window, GdkEventKey *event,
                                    gpointer user_data) {
