...
#define MENU_PATH_BOOKMARKS_PLACEHOLDER \
  "/MenuBar/Other Menus/Bookmarks/Bookmarks Placeholder"
...
G_DEFINE_TYPE_WITH_PRIVATE(CajaNavigationWindow, caja_navigation_window,
                           CAJA_TYPE_WINDOW)
...
#ifdef HAVE_X11_XF86KEYSYM_H
    {XF86XK_Back, CAJA_ACTION_BACK}, {XF86XK_Forward, CAJA_ACTION_FORWARD}
#endif
};

static void caja_navigation_window_init(CajaNavigationWindow *window) {
  ...
  window->details->header_size_group =
  ...
  gtk_grid_attach(GTK_GRID(CAJA_WINDOW(window)->details->grid),
                  window->details->content_paned, 0, 3, 1, 1);
  gtk_widget_show(window->details->content_paned);

  vbox = gtk_box_new(GTK_ORIENTATION_VERTICAL, 0);
  gtk_paned_pack2(GTK_PANED(window->details->content_paned), vbox, TRUE, FALSE);
  ...
  gtk_grid_attach(GTK_GRID(CAJA_WINDOW(window)->details->grid), toolbar, 0, 1,
                  1, 1);
  ...
  g_signal_connect_swapped(
      ...
      G_CALLBACK(always_use_location_entry_changed), window);

  g_signal_connect_swapped(caja_preferences,
                           "changed::" CAJA_PREFERENCES_ALWAYS_USE_BROWSER,
                           G_CALLBACK(always_use_browser_changed), window);
}
...
static void mouse_back_button_changed(gpointer callback_data) {
  ...
  new_back_button =
      g_settings_get_int(caja_preferences, CAJA_PREFERENCES_MOUSE_BACK_BUTTON);

  /* Bounds checking */
  if (new_back_button < 6 || new_back_button > UPPER_MOUSE_LIMIT) return;
...
static void mouse_forward_button_changed(gpointer callback_data) {
  ...
  new_forward_button = g_settings_get_int(
      caja_preferences, CAJA_PREFERENCES_MOUSE_FORWARD_BUTTON);

  /* Bounds checking */
  if (new_forward_button < 6 || new_forward_button > UPPER_MOUSE_LIMIT) return;
...
static void remember_focus_widget(CajaNavigationWindow *window) {
  ...
  if (focus_widget != NULL &&
      !caja_navigation_window_is_in_temporary_navigation_bar(
          focus_widget, navigation_window) &&
      !caja_navigation_window_is_in_temporary_search_bar(focus_widget,
                                                         navigation_window)) {
    caja_navigation_window_unset_focus_widget(navigation_window);

    navigation_window->details->last_focus_widget = focus_widget;
    g_object_add_weak_pointer(
        G_OBJECT(focus_widget),
        (gpointer *)&(
            CAJA_NAVIGATION_WINDOW(window)->details->last_focus_widget));
  }
}
...
static void side_pane_close_requested_callback(GtkWidget *widget,
                                               gpointer user_data) {
  CajaNavigationWindow *window;
...
static void side_pane_size_allocate_callback(GtkWidget *widget,
  ...
  if (allocation->width != window->details->side_pane_width) {
    ...
    g_settings_set_int(caja_window_state, CAJA_WINDOW_STATE_SIDEBAR_WIDTH,
                       allocation->width <= 1 ? 0 : allocation->width);
  }
}

static void setup_side_pane_width(CajaNavigationWindow *window) {
  gint scale;
  ...
  window->details->side_pane_width =
      g_settings_get_int(caja_window_state, CAJA_WINDOW_STATE_SIDEBAR_WIDTH) *
      scale;
...
static void set_current_side_panel(CajaNavigationWindow *window,
                                   CajaSidebar *panel) {
  if (window->details->current_side_panel) {
    caja_sidebar_is_visible_changed(window->details->current_side_panel, FALSE);
...
static void side_pane_switch_page_callback(CajaSidePane *side_pane,
                                           GtkWidget *widget,
                                           CajaNavigationWindow *window) {
  const char *id;
  CajaSidebar *sidebar;
  ...
  g_settings_set_string(caja_window_state, CAJA_WINDOW_STATE_SIDE_PANE_VIEW,
                        id);
}

static void caja_navigation_window_set_up_sidebar(
    CajaNavigationWindow *window) {
  GtkWidget *title;
  ...
  gtk_size_group_add_widget(window->details->header_size_group, title);

  gtk_paned_pack1(GTK_PANED(window->details->content_paned),
                  GTK_WIDGET(window->sidebar), FALSE, FALSE);

  setup_side_pane_width(window);
  g_signal_connect(window->sidebar, "size_allocate",
                   G_CALLBACK(side_pane_size_allocate_callback), window);

  add_sidebar_panels(window);

  g_signal_connect(window->sidebar, "close_requested",
                   G_CALLBACK(side_pane_close_requested_callback), window);

  g_signal_connect(window->sidebar, "switch_page",
                   G_CALLBACK(side_pane_switch_page_callback), window);

  gtk_widget_show(GTK_WIDGET(window->sidebar));
}

static void caja_navigation_window_tear_down_sidebar(
    CajaNavigationWindow *window) {
  GList *node, *next;
  CajaSidebar *sidebar_panel = NULL;

  g_signal_handlers_disconnect_by_func(window->sidebar,
                                       side_pane_switch_page_callback, window);
  ...
  for (node = window->sidebar_panels; node != NULL; node = next) {
    ...
    caja_navigation_window_remove_sidebar_panel(window, sidebar_panel);
  }
...
static gboolean caja_navigation_window_state_event(GtkWidget *widget,
...
static gboolean caja_navigation_window_key_press_event(GtkWidget *widget,
                                                       GdkEventKey *event) {
  CajaNavigationWindow *window;
  gsize i;
  ...
  for (i = 0; i < G_N_ELEMENTS(extra_navigation_window_keybindings); i++) {
    if (extra_navigation_window_keybindings[i].keyval == event->keyval) {
      ...
      action = gtk_action_group_get_action(
          window->details->navigation_action_group,
  ...
  return GTK_WIDGET_CLASS(caja_navigation_window_parent_class)
      ->key_press_event(widget, event);
}

static gboolean caja_navigation_window_button_press_event(
  ...
  if (mouse_extra_buttons && (((int)event->button) == mouse_back_button)) {
    ...
    handled = TRUE;
  } else if (mouse_extra_buttons &&
             (((int)event->button) == mouse_forward_button)) {
    caja_navigation_window_go_forward(window);
    handled = TRUE;
  } else if (GTK_WIDGET_CLASS(caja_navigation_window_parent_class)
                 ->button_press_event) {
    handled = GTK_WIDGET_CLASS(caja_navigation_window_parent_class)
...
static void caja_navigation_window_destroy(GtkWidget *object) {
  CajaNavigationWindow *window;
...
static void caja_navigation_window_finalize(GObject *object) {
  CajaNavigationWindow *window;
  ...
  g_signal_handlers_disconnect_by_func(caja_preferences,
                                       always_use_browser_changed, window);
  g_signal_handlers_disconnect_by_func(
      caja_preferences, always_use_location_entry_changed, window);
...
void caja_navigation_window_add_sidebar_panel(CajaNavigationWindow *window,
  ...
  caja_side_pane_add_panel(window->sidebar, GTK_WIDGET(sidebar_panel), label,
                           tooltip);
  g_free(tooltip);
  g_free(label);
  ...
  window->sidebar_panels =
      g_list_prepend(window->sidebar_panels, g_object_ref(sidebar_panel));

  /* Show if default */
  sidebar_id = caja_sidebar_get_sidebar_id(sidebar_panel);
  default_id = g_settings_get_string(caja_window_state,
                                     CAJA_WINDOW_STATE_SIDE_PANE_VIEW);
  if (sidebar_id && default_id && !strcmp(sidebar_id, default_id)) {
    caja_side_pane_show_panel(window->sidebar, GTK_WIDGET(sidebar_panel));
  }
  g_free(default_id);
}
...
void caja_navigation_window_remove_sidebar_panel(CajaNavigationWindow *window,
  ...
  g_signal_handlers_disconnect_by_func(
      sidebar_panel, side_panel_image_changed_callback, window);

  caja_side_pane_remove_panel(window->sidebar, GTK_WIDGET(sidebar_panel));
...
static void real_sync_title(CajaWindow *window, CajaWindowSlot *slot) {
  ...
  EEL_CALL_PARENT(CAJA_WINDOW_CLASS, sync_title, (window, slot));

  if (slot == window->details->active_pane->active_slot) {
    char *window_title;

    /* if spatial mode is default, we keep "File Browser" in the window title
     * to recognize browser windows. Otherwise, we default to the directory
     * name.
     */
...
static CajaIconInfo *real_get_icon(CajaWindow *window, CajaWindowSlot *slot) {
...
static void real_sync_allow_stop(CajaWindow *window, CajaWindowSlot *slot) {
  CajaNavigationWindow *navigation_window;
  CajaNotebook *notebook;

  navigation_window = CAJA_NAVIGATION_WINDOW(window);
  caja_navigation_window_set_spinner_active(navigation_window,
                                            slot->allow_stop);
...
static void real_prompt_for_location(CajaWindow *window, const char *initial) {
  CajaNavigationWindowPane *pane;
...
void caja_navigation_window_hide_search(CajaNavigationWindow *window) {
  CajaNavigationWindowPane *pane =
      CAJA_NAVIGATION_WINDOW_PANE(CAJA_WINDOW(window)->details->active_pane);
  if (caja_navigation_window_pane_search_bar_showing(pane)) {
...
static void add_sidebar_panels(CajaNavigationWindow *window) {
  ...
  for (p = providers; p != NULL; p = p->next) {
    ...
    sidebar_panel =
        caja_sidebar_provider_create(provider, CAJA_WINDOW_INFO(window));
    caja_navigation_window_add_sidebar_panel(window, sidebar_panel);
  ...
  set_current_side_panel(window, CAJA_SIDEBAR(current));
}
...
void caja_navigation_window_hide_status_bar(CajaNavigationWindow *window) {
  ...
  g_settings_set_boolean(caja_window_state,
                         CAJA_WINDOW_STATE_START_WITH_STATUS_BAR, FALSE);
}
...
void caja_navigation_window_show_status_bar(CajaNavigationWindow *window) {
  ...
  g_settings_set_boolean(caja_window_state,
                         CAJA_WINDOW_STATE_START_WITH_STATUS_BAR, TRUE);
}
...
void caja_navigation_window_hide_toolbar(CajaNavigationWindow *window) {
  ...
  caja_navigation_window_update_show_hide_menu_items(window);
  g_settings_set_boolean(caja_window_state,
                         CAJA_WINDOW_STATE_START_WITH_TOOLBAR, FALSE);
}
...
void caja_navigation_window_show_toolbar(CajaNavigationWindow *window) {
  ...
  caja_navigation_window_update_show_hide_menu_items(window);
  g_settings_set_boolean(caja_window_state,
                         CAJA_WINDOW_STATE_START_WITH_TOOLBAR, TRUE);
}
...
void caja_navigation_window_show_sidebar(CajaNavigationWindow *window) {
  ...
  caja_navigation_window_update_show_hide_menu_items(window);
  g_settings_set_boolean(caja_window_state,
                         CAJA_WINDOW_STATE_START_WITH_SIDEBAR, TRUE);
}
...
gboolean caja_navigation_window_sidebar_showing(CajaNavigationWindow *window) {
  ...
  return (window->sidebar != NULL) &&
         gtk_widget_get_visible(
             gtk_paned_get_child1(GTK_PANED(window->details->content_paned)));
}
...
gint caja_navigation_window_get_base_page_index(CajaNavigationWindow *window) {
  ...
  slot = CAJA_NAVIGATION_WINDOW_SLOT(
      CAJA_WINDOW(window)->details->active_pane->active_slot);
 ...
 * base on user prefs.
 */
static void caja_navigation_window_show(GtkWidget *widget) {
  CajaNavigationWindow *window;
  ...
  /* Initially show or hide views based on preferences; once the window is
   * displayed these can be controlled on a per-window basis from View menu
   * items.
   */

  if (g_settings_get_boolean(caja_window_state,
                             CAJA_WINDOW_STATE_START_WITH_TOOLBAR)) {
    caja_navigation_window_show_toolbar(window);
  } else {
    caja_navigation_window_hide_toolbar(window);
  }

  show_location_bar = g_settings_get_boolean(
      caja_window_state, CAJA_WINDOW_STATE_START_WITH_LOCATION_BAR);
  always_use_location_entry = g_settings_get_boolean(
      caja_preferences, CAJA_PREFERENCES_ALWAYS_USE_LOCATION_ENTRY);
  for (walk = CAJA_WINDOW(window)->details->panes; walk; walk = walk->next) {
    CajaNavigationWindowPane *pane = walk->data;
  ...
  if (g_settings_get_boolean(caja_window_state,
                             CAJA_WINDOW_STATE_START_WITH_SIDEBAR)) {
  ...
  if (g_settings_get_boolean(caja_window_state,
                             CAJA_WINDOW_STATE_START_WITH_STATUS_BAR)) {
...
static void caja_navigation_window_save_geometry(CajaNavigationWindow *window) {
  ...
  if (gtk_widget_get_window(GTK_WIDGET(window))) {
    ...
    if (!is_maximized) {
      g_settings_set_string(caja_window_state, CAJA_WINDOW_STATE_GEOMETRY,
                            geometry_string);
    }
...
static void real_window_close(CajaWindow *window) {
  caja_navigation_window_save_geometry(CAJA_NAVIGATION_WINDOW(window));
}

static void real_get_min_size(CajaWindow *window, guint *min_width,
                              guint *min_height) {
  if (min_width) {
...
static void real_get_default_size(CajaWindow *window, guint *default_width,
...
static CajaWindowSlot *real_open_slot(CajaWindowPane *pane,
  ...
  caja_navigation_window_pane_add_slot_in_tab(CAJA_NAVIGATION_WINDOW_PANE(pane),
                                              slot, flags);
...
static void real_close_slot(CajaWindowPane *pane, CajaWindowSlot *slot) {
  ...
  caja_navigation_window_pane_remove_page(CAJA_NAVIGATION_WINDOW_PANE(pane),
                                          page_num);

  gtk_notebook_set_show_tabs(notebook, gtk_notebook_get_n_pages(notebook) > 1);

  EEL_CALL_PARENT(CAJA_WINDOW_CLASS, close_slot, (pane, slot));
}

static void caja_navigation_window_class_init(
    CajaNavigationWindowClass *class) {
  CAJA_WINDOW_CLASS(class)->window_type = CAJA_WINDOW_NAVIGATION;
  CAJA_WINDOW_CLASS(class)->bookmarks_placeholder =
  ...
  GTK_WIDGET_CLASS(class)->window_state_event =
      caja_navigation_window_state_event;
  GTK_WIDGET_CLASS(class)->key_press_event =
      caja_navigation_window_key_press_event;
  GTK_WIDGET_CLASS(class)->button_press_event =
