...
G_DEFINE_TYPE(CajaWindowPane, caja_window_pane, G_TYPE_OBJECT)
#define parent_class caja_window_pane_parent_class

static inline CajaWindowSlot *get_first_inactive_slot(CajaWindowPane *pane) {
  GList *l;
...
void caja_window_pane_show(CajaWindowPane *pane) {
  pane->visible = TRUE;
  EEL_CALL_METHOD(CAJA_WINDOW_PANE_CLASS, pane, show, (pane));
}
...
void caja_window_pane_slot_close(CajaWindowPane *pane, CajaWindowSlot *slot) {
  if (pane->window) {
    ...
    /* If that was the last slot in the active pane, close the pane or even the
     * whole window. */
...
void caja_window_pane_sync_location_widgets(CajaWindowPane *pane) {
  EEL_CALL_METHOD(CAJA_WINDOW_PANE_CLASS, pane, sync_location_widgets, (pane));
}

void caja_window_pane_sync_search_widgets(CajaWindowPane *pane) {
  g_assert(CAJA_IS_WINDOW_PANE(pane));

  EEL_CALL_METHOD(CAJA_WINDOW_PANE_CLASS, pane, sync_search_widgets, (pane));
}
...
void caja_window_pane_set_active(CajaWindowPane *pane, gboolean is_active) {
  ...
  /* notify the current slot about its activity state (so that it can e.g.
   * modify the bg color) */
  caja_window_slot_is_in_active_pane(pane->active_slot, is_active);

  EEL_CALL_METHOD(CAJA_WINDOW_PANE_CLASS, pane, set_active, (pane, is_active));
}
