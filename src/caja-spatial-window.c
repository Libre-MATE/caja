...
#define MENU_PATH_SPATIAL_BOOKMARKS_PLACEHOLDER \
  "/MenuBar/Other Menus/Places/Bookmarks Placeholder"
...
G_DEFINE_TYPE_WITH_PRIVATE(CajaSpatialWindow, caja_spatial_window,
                           CAJA_TYPE_WINDOW)

static void caja_spatial_window_save_geometry(CajaSpatialWindow *window,
                                              CajaFile *viewed_file);

static gboolean save_window_geometry_timeout(gpointer callback_data) {
...
static gboolean caja_spatial_window_configure_event(GtkWidget *widget,
                                                    GdkEventConfigure *event) {
  CajaSpatialWindow *window;

  window = CAJA_SPATIAL_WINDOW(widget);

  GTK_WIDGET_CLASS(caja_spatial_window_parent_class)
...
static void caja_spatial_window_unrealize(GtkWidget *widget) {
  CajaSpatialWindow *window;
  CajaWindowSlot *slot;
...
static gboolean caja_spatial_window_state_event(GtkWidget *widget,
  ...
  if (!CAJA_IS_DESKTOP_WINDOW(widget)) {
    if (event->changed_mask & GDK_WINDOW_STATE_MAXIMIZED &&
      ...
      caja_file_set_boolean_metadata(
          viewed_file, CAJA_METADATA_KEY_WINDOW_MAXIMIZED, FALSE,
          event->new_window_state & GDK_WINDOW_STATE_MAXIMIZED);
    }

    if (event->changed_mask & GDK_WINDOW_STATE_STICKY && viewed_file != NULL) {
      caja_file_set_boolean_metadata(
          viewed_file, CAJA_METADATA_KEY_WINDOW_STICKY, FALSE,
          event->new_window_state & GDK_WINDOW_STATE_STICKY);
    }

    if (event->changed_mask & GDK_WINDOW_STATE_ABOVE && viewed_file != NULL) {
      caja_file_set_boolean_metadata(
          viewed_file, CAJA_METADATA_KEY_WINDOW_KEEP_ABOVE, FALSE,
          event->new_window_state & GDK_WINDOW_STATE_ABOVE);
    }
  }

  if (GTK_WIDGET_CLASS(caja_spatial_window_parent_class)->window_state_event !=
      NULL) {
    return GTK_WIDGET_CLASS(caja_spatial_window_parent_class)
...
static void caja_spatial_window_finalize(GObject *object) {
  CajaSpatialWindow *window;
...
static void caja_spatial_window_save_geometry(CajaSpatialWindow *window,
  ...
  if (gtk_widget_get_window(GTK_WIDGET(window)) &&
    ...
    caja_file_set_metadata(viewed_file, CAJA_METADATA_KEY_WINDOW_GEOMETRY, NULL,
                           geometry_string);
  }
}

static void caja_spatial_window_save_scroll_position(CajaSpatialWindow *window,
                                                     CajaWindowSlot *slot) {
  char *scroll_string;

  if (slot->content_view == NULL || slot->viewed_file == NULL) {
    return;
  }

  scroll_string = caja_view_get_first_visible_file(slot->content_view);
  caja_file_set_metadata(slot->viewed_file,
                         CAJA_METADATA_KEY_WINDOW_SCROLL_POSITION, NULL,
                         scroll_string);
  g_free(scroll_string);
}

static void caja_spatial_window_save_show_hidden_files_mode(
    CajaSpatialWindow *window, CajaFile *viewed_file) {
  ...
  if (mode != CAJA_WINDOW_SHOW_HIDDEN_FILES_DEFAULT) {
    ...
    caja_file_set_metadata(viewed_file,
                           CAJA_METADATA_KEY_WINDOW_SHOW_HIDDEN_FILES, NULL,
                           show_hidden_file_setting);
  }
}

static void caja_spatial_window_show(GtkWidget *widget) {
  CajaWindow *window;
  CajaWindowSlot *slot;
  GFile *location;
  ...
  while (location != NULL) {
    ...
    if (!caja_file_check_if_ready(file, CAJA_FILE_ATTRIBUTE_INFO)) {
      caja_file_call_when_ready(file, CAJA_FILE_ATTRIBUTE_INFO, NULL, NULL);
    }
...
static void action_close_parent_folders_callback(GtkAction *action,
                                                 gpointer user_data) {
  caja_application_close_parent_windows(CAJA_SPATIAL_WINDOW(user_data));
}

static void action_close_all_folders_callback(GtkAction *action,
                                              gpointer user_data) {
...
static void real_prompt_for_location(CajaWindow *window, const char *initial) {
  ...
  if (initial != NULL) {
    caja_location_dialog_set_location(CAJA_LOCATION_DIALOG(dialog), initial);
  }
...
static void sync_window_title(CajaWindow *window) {
  CajaWindowSlot *slot;

  slot = caja_window_get_active_slot(window);

  /* Don't change desktop's title, it would override the one already defined */
  if (CAJA_IS_DESKTOP_WINDOW(window)) return;
...
static void real_set_allow_up(CajaWindow *window, gboolean allow) {
  ...
  CAJA_WINDOW_CLASS(caja_spatial_window_parent_class)
...
static CajaWindowSlot *real_open_slot(CajaWindowPane *pane,
  ...
  slot->pane = pane;
  gtk_container_add(
      GTK_CONTAINER(CAJA_SPATIAL_WINDOW(pane->window)->details->content_box),
      slot->content_box);
  gtk_widget_show(slot->content_box);
  return slot;
}

static void save_spatial_data(CajaSpatialWindow *window, CajaWindowSlot *slot) {
  caja_spatial_window_save_geometry(window, slot->viewed_file);
...
static void location_menu_item_activated_callback(GtkWidget *menu_item,
                                                  CajaWindow *window) {
  CajaWindowSlot *slot;
  GFile *current;
  GFile *dest;
  ...
  if (!g_file_equal(current, dest)) {
    ...
    caja_window_slot_open_location_with_selection(slot, dest, selection,
                                                  close_behind);
...
static gboolean location_button_pressed_callback(GtkWidget *widget,
...
static void location_button_clicked_callback(GtkWidget *widget,
                                             CajaSpatialWindow *window) {
  CajaWindowSlot *slot;
  ...
  while (location != NULL) {
    ...
    surface = caja_file_get_icon_surface(
        file, caja_get_icon_size_for_stock_size(GTK_ICON_SIZE_MENU), TRUE,
    ...
    g_signal_connect(menu_item, "activate",
                     G_CALLBACK(location_menu_item_activated_callback), window);

    g_object_set_data_full(G_OBJECT(menu_item), "location",
                           g_object_ref(location),
                           (GDestroyNotify)g_object_unref);

    if (child_location) {
      g_object_set_data_full(G_OBJECT(menu_item), "child_location",
  ...
  g_signal_connect(popup, "deactivate", G_CALLBACK(menu_deactivate_callback),
                   loop);

  gtk_grab_add(popup);
  gtk_menu_popup_at_widget(GTK_MENU(popup), widget, GDK_GRAVITY_SOUTH_WEST,
...
static int get_dnd_icon_size(CajaSpatialWindow *window) {
...
static void location_button_drag_begin_callback(GtkWidget *widget,
  ...
  surface =
      caja_file_get_icon_surface(slot->viewed_file, get_dnd_icon_size(window),
                                 FALSE, gtk_widget_get_scale_factor(widget),
                                 CAJA_FILE_ICON_FLAGS_IGNORE_VISITING |
                                     CAJA_FILE_ICON_FLAGS_FOR_DRAG_ACCEPT);
 ...
 * wouldn't work */
static void get_data_binder(CajaDragEachSelectedItemDataGet iteratee,
  ...
  iteratee(location, 0, 0, icon_size, icon_size, data);

  g_free(location);
}

static void location_button_drag_data_get_callback(
    GtkWidget *widget, GdkDragContext *context,
    GtkSelectionData *selection_data, guint info, guint time,
    CajaSpatialWindow *window) {
  caja_drag_drag_data_get(widget, context, selection_data, info, time, window,
                          get_data_binder);
}
...
void caja_spatial_window_set_location_button(CajaSpatialWindow *window,
  ...
  if (location != NULL) {
    ...
    name = caja_file_get_display_name(file);
    gtk_label_set_label(GTK_LABEL(window->details->location_label), name);
    g_free(name);
    ...
    if (error == NULL) {
      ...
      if (surface != NULL) {
        gtk_image_set_from_surface(GTK_IMAGE(window->details->location_icon),
                                   surface);
        cairo_surface_destroy(surface);
      } else {
...
static void action_search_callback(GtkAction *action, gpointer user_data) {
  CajaWindow *window;
  char *uri;
  GFile *f;
...
static const GtkActionEntry spatial_entries[] = {
    ...
    {SPATIAL_ACTION_GO_TO_LOCATION, NULL, N_("Open _Location..."), "<control>L",
    ...
    /* name, icon name, label */
    {"Search", "edit-find", N_("_Search for Files..."), "<control>F",
     N_("Locate documents and folders on this computer by name or content"),
     G_CALLBACK(action_search_callback)},
};

static void caja_spatial_window_init(CajaSpatialWindow *window) {
  GtkWidget *arrow;
  GtkWidget *hbox, *vbox;
  ...
  gtk_grid_attach(GTK_GRID(win->details->grid), win->details->statusbar, 0, 5,
                  1, 1);
  ...
  gtk_grid_attach(GTK_GRID(CAJA_WINDOW(window)->details->grid), vbox, 0, 1, 1,
                  3);
  ...
  g_signal_connect(window->details->location_button, "button-press-event",
                   G_CALLBACK(location_button_pressed_callback), window);
  gtk_button_set_relief(GTK_BUTTON(window->details->location_button),
                        GTK_RELIEF_NORMAL);

  gtk_widget_show(window->details->location_button);
  hbox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 3);
  gtk_container_add(GTK_CONTAINER(window->details->location_button), hbox);
  gtk_widget_show(hbox);

  window->details->location_icon =
      gtk_image_new_from_icon_name("document-open", GTK_ICON_SIZE_MENU);
  gtk_box_pack_start(GTK_BOX(hbox), window->details->location_icon, FALSE,
                     FALSE, 0);
  gtk_widget_show(window->details->location_icon);

  window->details->location_label = gtk_label_new("");
  gtk_label_set_ellipsize(GTK_LABEL(window->details->location_label),
                          PANGO_ELLIPSIZE_END);
  gtk_label_set_max_width_chars(GTK_LABEL(window->details->location_label),
                                MAX_SHORTNAME_PATH);
  gtk_box_pack_start(GTK_BOX(hbox), window->details->location_label, FALSE,
                     FALSE, 0);
  ...
  gtk_drag_source_set(
      window->details->location_button, GDK_BUTTON1_MASK | GDK_BUTTON2_MASK,
      location_button_drag_types, G_N_ELEMENTS(location_button_drag_types),
      GDK_ACTION_MOVE | GDK_ACTION_COPY | GDK_ACTION_LINK | GDK_ACTION_ASK);
  g_signal_connect(window->details->location_button, "drag_begin",
                   G_CALLBACK(location_button_drag_begin_callback), window);
  g_signal_connect(window->details->location_button, "drag_data_get",
                   G_CALLBACK(location_button_drag_data_get_callback), window);
  ...
  gtk_action_group_add_actions(action_group, spatial_entries,
                               G_N_ELEMENTS(spatial_entries), window);
...
static void caja_spatial_window_class_init(CajaSpatialWindowClass *klass) {
  ...
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_BackSpace, GDK_SHIFT_MASK,
                               "go_up", 1, G_TYPE_BOOLEAN, TRUE);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_Up,
                               GDK_SHIFT_MASK | GDK_MOD1_MASK, "go_up", 1,
                               G_TYPE_BOOLEAN, TRUE);
}
