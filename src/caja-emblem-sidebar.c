...
static const GtkTargetEntry drag_types[] = {{"property/keyword", 0, 0}};

enum { TARGET_URI_LIST, TARGET_URI, TARGET_NETSCAPE_URL };
...
G_DEFINE_TYPE_WITH_CODE(CajaEmblemSidebarProvider, caja_emblem_sidebar_provider,
                        G_TYPE_OBJECT,
                        G_IMPLEMENT_INTERFACE(CAJA_TYPE_SIDEBAR_PROVIDER,
                                              sidebar_provider_iface_init));

static void caja_emblem_sidebar_drag_data_get_cb(
    GtkWidget *widget, GdkDragContext *context, GtkSelectionData *data,
    guint info, guint time, CajaEmblemSidebar *emblem_sidebar) {
  char *keyword;
  ...
  gtk_selection_data_set(data, gtk_selection_data_get_target(data), 8, keyword,
                         strlen(keyword));
}

static void caja_emblem_sidebar_enter_notify_cb(
    GtkWidget *widget, CajaEmblemSidebar *emblem_sidebar) {
  GdkPixbuf *pixbuf;
...
static void caja_emblem_sidebar_leave_notify_cb(
    GtkWidget *widget, CajaEmblemSidebar *emblem_sidebar) {
  GdkPixbuf *pixbuf;
  EelLabeledImage *image;
...
static gboolean caja_emblem_sidebar_button_press_cb(
...
static void send_emblems_changed(void) {
...
static void caja_emblem_sidebar_delete_cb(GtkWidget *menu_item,
                                          CajaEmblemSidebar *emblem_sidebar) {
  if (caja_emblem_remove_emblem(
          emblem_sidebar->details->popup_emblem_keyword)) {
    send_emblems_changed();
  } else {
    char *error;

    error = g_strdup_printf(_("Could not remove emblem with name '%s'."),
                            emblem_sidebar->details->popup_emblem_display_name);
    eel_show_error_dialog(
        error,
        _("This is probably because the emblem is a permanent one, and not one "
...
static void rename_dialog_response_cb(GtkWidget *dialog, int response,
                                      CajaEmblemSidebar *emblem_sidebar) {
  GtkWidget *entry;
  char *keyword, *name;
...
static GtkWidget *create_rename_emblem_dialog(CajaEmblemSidebar *emblem_sidebar,
                                              const char *keyword,
  ...
  g_object_set_data(G_OBJECT(dialog), "emblem-keyword", g_strdup(keyword));
...
static void caja_emblem_sidebar_rename_cb(GtkWidget *menu_item,
                                          CajaEmblemSidebar *emblem_sidebar) {
  GtkWidget *dialog;

  dialog = create_rename_emblem_dialog(
      emblem_sidebar, emblem_sidebar->details->popup_emblem_keyword,
      emblem_sidebar->details->popup_emblem_display_name,
      emblem_sidebar->details->popup_emblem_pixbuf);
  g_signal_connect(dialog, "response", G_CALLBACK(rename_dialog_response_cb),
                   emblem_sidebar);
  gtk_widget_show(dialog);
}

static void create_popup_menu(CajaEmblemSidebar *emblem_sidebar) {
  ...
  menu_item =
      eel_image_menu_item_new_from_icon("document-properties", _("Rename"));

  g_signal_connect(menu_item, "activate",
                   G_CALLBACK(caja_emblem_sidebar_rename_cb), emblem_sidebar);
  ...
  g_signal_connect(menu_item, "activate",
                   G_CALLBACK(caja_emblem_sidebar_delete_cb), emblem_sidebar);
  gtk_widget_show(menu_item);
  gtk_menu_shell_append(GTK_MENU_SHELL(popup), menu_item);
...
static GtkWidget *create_emblem_widget_with_pixbuf(
    CajaEmblemSidebar *emblem_sidebar, const char *keyword,
    const char *display_name, GdkPixbuf *pixbuf) {
  ...
  g_object_set_data_full(G_OBJECT(event_box), "emblem-keyword",
                         g_strdup(keyword), g_free);
  g_object_set_data_full(G_OBJECT(event_box), "emblem-display-name",
                         g_strdup(display_name), g_free);
  g_object_set_data_full(G_OBJECT(event_box), "original-pixbuf", pixbuf,
                         g_object_unref);
  g_object_set_data_full(G_OBJECT(event_box), "prelight-pixbuf",
                         prelight_pixbuf, g_object_unref);
  g_object_set_data(G_OBJECT(event_box), "labeled-image", image);

  return event_box;
}

static GtkWidget *create_emblem_widget(CajaEmblemSidebar *emblem_sidebar,
                                       const char *name) {
  GtkWidget *ret;
  ...
  ret = create_emblem_widget_with_pixbuf(emblem_sidebar, keyword, display_name,
                                         pixbuf);
  g_free(keyword);
  g_object_unref(info);
  return ret;
...
static void emblem_name_entry_changed_cb(GtkWidget *entry, Emblem *emblem) {
  char *text;
...
static void destroy_emblem(Emblem *emblem, gpointer user_data) {
...
static void destroy_emblem_list(GSList *list) {
  g_slist_foreach(list, (GFunc)destroy_emblem, NULL);
  g_slist_free(list);
}

static GtkWidget *create_add_emblems_dialog(CajaEmblemSidebar *emblem_sidebar,
                                            GSList *emblems) {
  ...
  if (g_slist_length(emblems) > 1) {
    label = gtk_label_new(
        _("Enter a descriptive name next to each emblem.  This name will be "
          "used in other places to identify the emblem."));
  } else {
    label = gtk_label_new(
        _("Enter a descriptive name next to the emblem.  This name will be "
          "used in other places to identify the emblem."));
  }
  ...
  while (list != NULL) {
    ...
    entry = gtk_entry_new();
    gtk_entry_set_activates_default(GTK_ENTRY(entry), TRUE);
    g_signal_connect(entry, "changed", G_CALLBACK(emblem_name_entry_changed_cb),
                     emblem);
  ...
  g_object_set_data_full(G_OBJECT(dialog), "emblems-to-add", emblems,
                         (GDestroyNotify)destroy_emblem_list);

  return dialog;
}

static void remove_widget(GtkWidget *widget, GtkContainer *container) {
...
static void add_emblems_dialog_response_cb(GtkWidget *dialog, int response,
                                           CajaEmblemSidebar *emblem_sidebar) {
  Emblem *emblem;
  GSList *emblems;
  GSList *l;
  ...
  switch (response) {
    ...
    case GTK_RESPONSE_OK:
      emblems = g_object_get_data(G_OBJECT(dialog), "emblems-to-add");
      ...
      for (l = emblems; l; l = l->next) {
        ...
        if (!caja_emblem_verify_keyword(GTK_WINDOW(dialog), keyword,
                                        emblem->name)) {
          g_free(keyword);
          return;
        } else {
      ...
      for (l = emblems; l; l = l->next) {
        ...
        caja_emblem_install_custom_emblem(emblem->pixbuf, emblem->keyword,
                                          emblem->name, GTK_WINDOW(dialog));
      }
...
static void show_add_emblems_dialog(CajaEmblemSidebar *emblem_sidebar,
                                    GSList *emblems) {
  GtkWidget *dialog;
  ...
  g_signal_connect(dialog, "response",
                   G_CALLBACK(add_emblems_dialog_response_cb), emblem_sidebar);
...
static void caja_emblem_sidebar_drag_received_cb(
  ...
  switch (info) {
    case TARGET_URI_LIST:
      if (data_format != 8 || data_length == 0) {
      ...
      if (had_failure && emblems != NULL) {
        eel_show_error_dialog(
            _("Some of the files could not be added as emblems."),
            _("The emblems do not appear to be valid images."), NULL);
      } else if (had_failure && emblems == NULL) {
        eel_show_error_dialog(
            _("None of the files could be added as emblems."),
            _("The emblems do not appear to be valid images."), NULL);
      }
      ...
      if (pixbuf != NULL) {
        ...
        if (uri_utf8) {
          error = g_strdup_printf(
              _("The file '%s' does not appear to be a valid image."),
              uri_utf8);
        } else {
          error = g_strdup(
              _("The dragged file does not appear to be a valid image."));
        }
    ...
    case TARGET_NETSCAPE_URL:
      ...
      if (pixbuf != NULL) {
        ...
        error = g_strdup_printf(
            _("The file '%s' does not appear to be a valid image."), uri);
        eel_show_error_dialog(_("The emblem cannot be added."), error, NULL);
        g_free(error);
      }
...
static GtkWidget *caja_emblem_sidebar_create_container(
    CajaEmblemSidebar *emblem_sidebar) {
  ...
  gtk_drag_dest_set(scroller, GTK_DEST_DEFAULT_ALL, dest_types,
                    G_N_ELEMENTS(dest_types),
                    GDK_ACTION_COPY | GDK_ACTION_MOVE);
...
static gint emblem_widget_sort_func(gconstpointer a, gconstpointer b) {
  GObject *obj_a, *obj_b;
...
static void caja_emblem_sidebar_populate(CajaEmblemSidebar *emblem_sidebar) {
  ...
  if (erase_pixbuf != NULL) {
    emblem_widget = create_emblem_widget_with_pixbuf(
        emblem_sidebar, ERASE_EMBLEM_KEYWORD, _("Erase"), erase_pixbuf);
    gtk_container_add(GTK_CONTAINER(emblem_sidebar->details->emblems_table),
                      emblem_widget);
  }
...
static void caja_emblem_sidebar_init(CajaEmblemSidebar *emblem_sidebar) {
  GtkWidget *widget;
  ...
  g_signal_connect_object(caja_signaller_get_current(), "emblems_changed",
                          G_CALLBACK(emblems_changed_callback), emblem_sidebar,
                          0);

  gtk_orientable_set_orientation(GTK_ORIENTABLE(emblem_sidebar),
                                 GTK_ORIENTATION_VERTICAL);
  gtk_box_pack_start(GTK_BOX(emblem_sidebar), widget, TRUE, TRUE, 0);
}
...
static void caja_emblem_sidebar_class_init(
    CajaEmblemSidebarClass *object_klass) {
...
static const char *caja_emblem_sidebar_get_sidebar_id(CajaSidebar *sidebar) {
  return CAJA_EMBLEM_SIDEBAR_ID;
}

static char *caja_emblem_sidebar_get_tab_label(CajaSidebar *sidebar) {
  return g_strdup(_("Emblems"));
}

static char *caja_emblem_sidebar_get_tab_tooltip(CajaSidebar *sidebar) {
  return g_strdup(_("Show Emblems"));
}

static GdkPixbuf *caja_emblem_sidebar_get_tab_icon(CajaSidebar *sidebar) {
  return NULL;
}

static void caja_emblem_sidebar_is_visible_changed(CajaSidebar *sidebar,
                                                   gboolean is_visible) {
  /* Do nothing */
}
...
static void caja_emblem_sidebar_set_parent_window(CajaEmblemSidebar *sidebar,
                                                  CajaWindowInfo *window) {
  sidebar->details->window = window;
}

static CajaSidebar *caja_emblem_sidebar_create(CajaSidebarProvider *provider,
                                               CajaWindowInfo *window) {
...
static void caja_emblem_sidebar_provider_init(
    CajaEmblemSidebarProvider *sidebar) {}

static void caja_emblem_sidebar_provider_class_init(
    CajaEmblemSidebarProviderClass *class) {}
