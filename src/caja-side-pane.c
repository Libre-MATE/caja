...
enum { CLOSE_REQUESTED, SWITCH_PAGE, LAST_SIGNAL };

static guint signals[LAST_SIGNAL] = {0};

G_DEFINE_TYPE_WITH_PRIVATE(CajaSidePane, caja_side_pane, GTK_TYPE_BOX)

static SidePanel *panel_for_widget(CajaSidePane *side_pane, GtkWidget *widget) {
  GList *l;
...
static void side_panel_free(SidePanel *panel) {
  g_free(panel->title);
  g_free(panel->tooltip);
  g_slice_free(SidePanel, panel);
}

static void switch_page_callback(GtkWidget *notebook, GtkWidget *page,
  ...
  panel = panel_for_widget(
      side_pane, gtk_notebook_get_nth_page(
...
static void select_panel(CajaSidePane *side_pane, SidePanel *panel) {
  int page_num;
...
static void caja_side_pane_class_init(CajaSidePaneClass *klass) {
  ...
  signals[CLOSE_REQUESTED] = g_signal_new(
      "close_requested", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaSidePaneClass, close_requested), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
  signals[SWITCH_PAGE] = g_signal_new(
      "switch_page", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaSidePaneClass, switch_page), NULL, NULL,
      g_cclosure_marshal_VOID__OBJECT, G_TYPE_NONE, 1, GTK_TYPE_WIDGET);
}

static void panel_item_activate_callback(GtkMenuItem *item,
                                         gpointer user_data) {
  CajaSidePane *side_pane;
  SidePanel *panel;
...
static gboolean select_button_press_callback(GtkWidget *widget,
  ...
  if ((event->type == GDK_BUTTON_PRESS) && event->button == 1) {
    ...
    gtk_menu_popup_at_widget(GTK_MENU(side_pane->details->menu), widget,
...
static gboolean select_button_key_press_callback(GtkWidget *widget,
                                                 GdkEventKey *event,
                                                 gpointer user_data) {
  CajaSidePane *side_pane;
...
static void close_clicked_callback(GtkWidget *widget, gpointer user_data) {
  CajaSidePane *side_pane;
...
static void caja_side_pane_init(CajaSidePane *side_pane) {
  ...
  gtk_orientable_set_orientation(GTK_ORIENTABLE(side_pane),
  ...
  g_signal_connect(select_button, "button_press_event",
                   G_CALLBACK(select_button_press_callback), side_pane);
  g_signal_connect(select_button, "key_press_event",
                   G_CALLBACK(select_button_key_press_callback), side_pane);
  ...
  g_signal_connect(close_button, "clicked", G_CALLBACK(close_clicked_callback),
                   side_pane);
  ...
  gtk_box_pack_end(GTK_BOX(hbox), side_pane->details->shortcut_box, FALSE,
                   FALSE, 0);

  side_pane->details->notebook = gtk_notebook_new();
  gtk_notebook_set_show_tabs(GTK_NOTEBOOK(side_pane->details->notebook), FALSE);
  gtk_notebook_set_show_border(GTK_NOTEBOOK(side_pane->details->notebook),
                               FALSE);
  g_signal_connect_object(side_pane->details->notebook, "switch_page",
                          G_CALLBACK(switch_page_callback), side_pane, 0);

  gtk_widget_show(side_pane->details->notebook);

  gtk_box_pack_start(GTK_BOX(side_pane), side_pane->details->notebook, TRUE,
                     TRUE, 0);
  ...
  g_signal_connect(side_pane->details->menu, "deactivate",
                   G_CALLBACK(menu_deactivate_callback), select_button);
  gtk_menu_attach_to_widget(GTK_MENU(side_pane->details->menu),
                            GTK_WIDGET(side_pane), menu_detach_callback);

  gtk_widget_show(side_pane->details->menu);

  gtk_widget_set_tooltip_text(close_button, _("Close the side pane"));
}
...
void caja_side_pane_add_panel(CajaSidePane *side_pane, GtkWidget *widget,
  ...
  g_signal_connect(panel->menu_item, "activate",
                   G_CALLBACK(panel_item_activate_callback), side_pane);

  side_pane->details->panels = g_list_append(side_pane->details->panels, panel);

  gtk_notebook_append_page(GTK_NOTEBOOK(side_pane->details->notebook), widget,
                           NULL);
}

void caja_side_pane_remove_panel(CajaSidePane *side_pane, GtkWidget *widget) {
  SidePanel *panel;
  ...
  if (panel) {
    ...
    side_pane->details->panels =
        g_list_remove(side_pane->details->panels, panel);

    side_panel_free(panel);
  }
}

void caja_side_pane_show_panel(CajaSidePane *side_pane, GtkWidget *widget) {
  SidePanel *panel;
  int page_num;
  ...
  page_num =
      gtk_notebook_page_num(GTK_NOTEBOOK(side_pane->details->notebook), widget);
  gtk_notebook_set_current_page(GTK_NOTEBOOK(side_pane->details->notebook),
                                page_num);
}

static void shortcut_clicked_callback(GtkWidget *button, gpointer user_data) {
  CajaSidePane *side_pane;
  GtkWidget *page;
...
static GtkWidget *create_shortcut(CajaSidePane *side_pane, SidePanel *panel,
                                  GdkPixbuf *pixbuf) {
  GtkWidget *button;
  GtkWidget *image;
  ...
  g_signal_connect(button, "clicked", G_CALLBACK(shortcut_clicked_callback),
                   side_pane);
...
void caja_side_pane_set_panel_image(CajaSidePane *side_pane, GtkWidget *widget,
                                    GdkPixbuf *pixbuf) {
  SidePanel *panel;
  GtkWidget *image;
...
GtkWidget *caja_side_pane_get_current_panel(CajaSidePane *side_pane) {
  ...
  index =
      gtk_notebook_get_current_page(GTK_NOTEBOOK(side_pane->details->notebook));
  return gtk_notebook_get_nth_page(GTK_NOTEBOOK(side_pane->details->notebook),
                                   index);
}
