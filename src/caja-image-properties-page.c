...
G_DEFINE_TYPE_WITH_PRIVATE(CajaImagePropertiesPage, caja_image_properties_page,
                           GTK_TYPE_BOX);

G_DEFINE_TYPE_WITH_CODE(
    CajaImagePropertiesPageProvider, caja_image_properties_page_provider,
    G_TYPE_OBJECT,
    G_IMPLEMENT_INTERFACE(CAJA_TYPE_PROPERTY_PAGE_PROVIDER,
                          property_page_provider_iface_init));

static void caja_image_properties_page_finalize(GObject *object) {
...
static void file_close_callback(GObject *object, GAsyncResult *res,
                                gpointer data) {
  CajaImagePropertiesPage *page;
  GInputStream *stream;
...
static GtkWidget *append_label(GtkWidget *vbox, const char *str) {
  GtkWidget *label;
...
static void add_row(CajaImagePropertiesPage *page, const char *col_name_str,
  ...
  gtk_grid_attach(GTK_GRID(page->details->grid), label, 0, page->details->row,
                  1, 1);
  ...
  gtk_grid_attach(GTK_GRID(page->details->grid), label, 1, page->details->row,
                  1, 1);
...
#ifdef HAVE_EXIF
static char *exif_string_to_utf8(const char *exif_str) {
  char *utf8_str;
...
static void exif_content_callback(ExifContent *content, gpointer data) {
  ...
  attribute->value =
      g_strdup(exif_content_get_value(content, attribute->tag, b, sizeof(b)));
  if (attribute->value != NULL) {
    attribute->found = TRUE;
  }
}

static char *exifdata_get_tag_name_utf8(ExifTag tag) {
  return exif_string_to_utf8(exif_tag_get_name(tag));
}

static char *exifdata_get_tag_value_utf8(ExifData *data, ExifTag tag) {
  struct ExifAttribute attribute;
  char *utf8_value;
...
static gboolean append_tag_value_pair(CajaImagePropertiesPage *page,
                                      ExifData *data, ExifTag tag,
                                      char *description) {
  char *utf_attribute;
  char *utf_value;
...
static void append_exifdata_string(ExifData *exifdata,
                                   CajaImagePropertiesPage *page) {
  if (exifdata && exifdata->ifd[0] && exifdata->ifd[0]->count) {
    ...
    if (!append_tag_value_pair(page, exifdata, EXIF_TAG_DATE_TIME_ORIGINAL,
                               _("Date Taken"))) {
      if (!append_tag_value_pair(page, exifdata, EXIF_TAG_DATE_TIME_DIGITIZED,
                                 _("Date Digitized"))) {
        append_tag_value_pair(page, exifdata, EXIF_TAG_DATE_TIME,
                              _("Date Modified"));
      }
    }

    append_tag_value_pair(page, exifdata, EXIF_TAG_EXPOSURE_TIME,
                          _("Exposure Time"));
    append_tag_value_pair(page, exifdata, EXIF_TAG_APERTURE_VALUE,
                          _("Aperture Value"));
    append_tag_value_pair(page, exifdata, EXIF_TAG_ISO_SPEED_RATINGS,
                          _("ISO Speed Rating"));
    append_tag_value_pair(page, exifdata, EXIF_TAG_FLASH, _("Flash Fired"));
    append_tag_value_pair(page, exifdata, EXIF_TAG_METERING_MODE,
                          _("Metering Mode"));
    append_tag_value_pair(page, exifdata, EXIF_TAG_EXPOSURE_PROGRAM,
                          _("Exposure Program"));
    append_tag_value_pair(page, exifdata, EXIF_TAG_FOCAL_LENGTH,
                          _("Focal Length"));
...
#ifdef HAVE_EXEMPI
static void append_xmp_value_pair(CajaImagePropertiesPage *page, XmpPtr xmp,
                                  const char *ns, const char *propname,
                                  char *descr) {
  uint32_t options;
  XmpStringPtr value;
    ...
    if (XMP_IS_PROP_SIMPLE(options)) {
      ...
      if (iter) {
        ...
        while (xmp_iterator_next(iter, NULL, NULL, value, &options) &&
          ...
          g_string_append_printf(str, "%s", xmp_string_cstr(value));
        }
...
static void append_xmpdata_string(XmpPtr xmp, CajaImagePropertiesPage *page) {
  if (xmp != NULL) {
...
static void load_finished(CajaImagePropertiesPage *page) {
  ...
  if (page->details->got_size) {
    ...
    if (!exif_data) goto no_exifdata;

    exif_entry = exif_content_get_entry(exif_data->ifd[EXIF_IFD_0],
                                        EXIF_TAG_ORIENTATION);
    if (!exif_entry) goto no_exifdata;

    orientation =
        exif_get_short(exif_entry->data, exif_data_get_byte_order(exif_data));
    if (orientation > 4) {
  ...
  no_exifdata:
    ...
    gtk_box_pack_start(GTK_BOX(page->details->vbox), page->details->grid, FALSE,
                       FALSE, 0);
    page->details->row = 0;
    ...
    value = g_strdup_printf(ngettext("%d pixel", "%d pixels", width), width);
    add_row(page, _("Width"), value);

    value = g_strdup_printf(ngettext("%d pixel", "%d pixels", height), height);
    ...
    append_label(page->details->vbox, _("Failed to load image information"));
  }
...
static void file_read_callback(GObject *object, GAsyncResult *res,
                               gpointer data) {
  CajaImagePropertiesPage *page;
  GInputStream *stream;
  gssize count_read;
  ...
  if (count_read > 0) {
    ...
    if (page->details->pixbuf_still_loading) {
      if (!gdk_pixbuf_loader_write(page->details->loader, page->details->buffer,
                                   count_read, NULL)) {
  ...
  if (done_reading) {
    load_finished(page);
    g_input_stream_close_async(stream, 0, page->details->cancellable,
                               file_close_callback, page);
  }
}

static void size_prepared_callback(GdkPixbufLoader *loader, int width,
                                   int height, gpointer callback_data) {
  CajaImagePropertiesPage *page;
...
static void file_open_callback(GObject *object, GAsyncResult *res,
                               gpointer data) {
  CajaImagePropertiesPage *page;
  GFile *file;
...
static void load_location(CajaImagePropertiesPage *page, const char *location) {
  GFile *file;
...
#ifdef HAVE_EXEMPI
  {
    /* Current Exempi does not support setting custom IO to be able to use
     * Mate-vfs */
    /* So it will only work with local files. Future version might remove this
     * limitation */
    XmpFilePtr xf;
    char *localname;
    ...
    if (localname) {
      ...
      page->details->xmp =
          xmp_files_get_new_xmp(xf); /* only load when loading */
  ...
  g_file_read_async(file, 0, page->details->cancellable, file_open_callback,
                    page);

  g_object_unref(file);
}

static void caja_image_properties_page_class_init(
    CajaImagePropertiesPageClass *class) {
...
static void caja_image_properties_page_init(CajaImagePropertiesPage *page) {
...
static gboolean is_mime_type_supported(const char *mime_type) {
...
static GList *get_property_pages(CajaPropertyPageProvider *provider,
                                 GList *files) {
  GList *pages;
  ...
  real_page =
      caja_property_page_new("CajaImagePropertiesPage::property_page",
                             gtk_label_new(_("Image")), GTK_WIDGET(page));
...
static void caja_image_properties_page_provider_init(
    CajaImagePropertiesPageProvider *sidebar) {}

static void caja_image_properties_page_provider_class_init(
    CajaImagePropertiesPageProviderClass *class) {}
