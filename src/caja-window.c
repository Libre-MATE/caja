...
#define CAJA_MENU_PATH_EXTRA_VIEWER_PLACEHOLDER \
  "/MenuBar/View/View Choices/Extra Viewer"
#define CAJA_MENU_PATH_SHORT_LIST_PLACEHOLDER \
  "/MenuBar/View/View Choices/Short List"
...
static void action_view_as_callback(GtkAction *action, ActivateViewData *data);
...
static void caja_window_init(CajaWindow *window) {
  GtkWidget *grid;
  GtkWidget *menu;
  GtkWidget *statusbar;
  ...
  window->details->help_message_cid =
      gtk_statusbar_get_context_id(GTK_STATUSBAR(statusbar), "help_message");
  ...
  g_signal_connect_object(caja_signaller_get_current(), "popup_menu_changed",
                          G_CALLBACK(caja_window_load_extension_menus), window,
                          G_CONNECT_SWAPPED);
}

/* Unconditionally synchronize the GtkUIManager of WINDOW. */
static void caja_window_ui_update(CajaWindow *window) {
  g_assert(CAJA_IS_WINDOW(window));

  gtk_ui_manager_ensure_update(window->details->ui_manager);
}

static void caja_window_push_status(CajaWindow *window, const char *text) {
...
void caja_window_go_to(CajaWindow *window, GFile *location) {
  ...
  caja_window_slot_go_to(window->details->active_pane->active_slot, location,
                         FALSE);
}
...
void caja_window_go_to_full(CajaWindow *window, GFile *location,
  ...
  caja_window_slot_go_to_full(window->details->active_pane->active_slot,
...
void caja_window_go_to_with_selection(CajaWindow *window, GFile *location,
  ...
  caja_window_slot_go_to_with_selection(
      window->details->active_pane->active_slot, location, new_selection);
}

static gboolean caja_window_go_up_signal(CajaWindow *window,
                                         gboolean close_behind) {
  caja_window_go_up(window, close_behind, FALSE);
  return TRUE;
}

static void caja_window_reload_signal(CajaWindow *window) {
...
void caja_window_new_tab(CajaWindow *window) {
  ...
  if (location != NULL) {
    ...
    new_slot_position = g_settings_get_enum(caja_preferences,
...
/*Opens a new window when called from an existing window and goes to the same
 * location that's in the existing window.*/
void caja_window_new_window(CajaWindow *window) {
  ...
  if (location != NULL) {
    ...
    new_window = caja_application_create_navigation_window(
        window->application, gtk_window_get_screen(GTK_WINDOW(window)));
...
static void real_set_allow_up(CajaWindow *window, gboolean allow) {
  GtkAction *action;
...
void caja_window_allow_up(CajaWindow *window, gboolean allow) {
  ...
  EEL_CALL_METHOD(CAJA_WINDOW_CLASS, window, set_allow_up, (window, allow));
}

static void update_cursor(CajaWindow *window) {
  CajaWindowSlot *slot;
...
void caja_window_sync_allow_stop(CajaWindow *window, CajaWindowSlot *slot) {
  GtkAction *action;
  gboolean allow_stop;
  ...
  if (slot != window->details->active_pane->active_slot ||
    ...
    EEL_CALL_METHOD(CAJA_WINDOW_CLASS, window, sync_allow_stop, (window, slot));
  }
}
...
void caja_window_reload(CajaWindow *window, gboolean new_tab) {
  ...
  if (new_tab && slot->location != NULL) {
    caja_window_slot_open_location_full(
        slot, slot->location, CAJA_WINDOW_OPEN_ACCORDING_TO_MODE,
        CAJA_WINDOW_OPEN_FLAG_NEW_TAB, NULL, NULL, NULL);
  } else {
...
void caja_window_prompt_for_location(CajaWindow *window, const char *initial) {
  ...
  EEL_CALL_METHOD(CAJA_WINDOW_CLASS, window, prompt_for_location,
                  (window, initial));
}

static char *caja_window_get_location_uri(CajaWindow *window) {
  CajaWindowSlot *slot;
...
void caja_window_zoom_to_level(CajaWindow *window, CajaZoomLevel level) {
  g_assert(window != NULL);
 ...
 */
static guint get_max_forced_height(GdkScreen *screen) {
  gint scale = gdk_window_get_scale_factor(gdk_screen_get_root_window(screen));
  return (HeightOfScreen(gdk_x11_screen_get_xscreen(screen)) / scale * 90) /
         100;
}
 ...
 */
static guint get_max_forced_width(GdkScreen *screen) {
  gint scale = gdk_window_get_scale_factor(gdk_screen_get_root_window(screen));
 ...
 * construction time.
 */
static void caja_window_set_initial_window_geometry(CajaWindow *window) {
  GdkScreen *screen;
  ...
  EEL_CALL_METHOD(CAJA_WINDOW_CLASS, window, get_default_size,
                  (window, &default_width, &default_height));

  gtk_window_set_default_size(GTK_WINDOW(window),
                              MIN(default_width, max_width_for_screen),
                              MIN(default_height, max_height_for_screen));
}

static void caja_window_constructed(GObject *self) {
  CajaWindow *window;
...
static void caja_window_set_property(GObject *object, guint arg_id,
...
static void caja_window_get_property(GObject *object, guint arg_id,
...
static void free_stored_viewers(CajaWindow *window) {
...
static void caja_window_destroy(GtkWidget *object) {
  CajaWindow *window;
  GList *panes_copy;
...
static void caja_window_finalize(GObject *object) {
  CajaWindow *window;
...
static GObject *caja_window_constructor(
  ...
  object = (*G_OBJECT_CLASS(caja_window_parent_class)->constructor)(
...
static void caja_window_view_visible(CajaWindow *window, CajaView *view) {
  CajaWindowSlot *slot;
  CajaWindowPane *pane;
  GList *l, *walk;
...
void caja_window_close(CajaWindow *window) {
  ...
  EEL_CALL_METHOD(CAJA_WINDOW_CLASS, window, close, (window));
...
static void real_close_slot(CajaWindowPane *pane, CajaWindowSlot *slot) {
...
void caja_window_close_slot(CajaWindowSlot *slot) {
  ...
  EEL_CALL_METHOD(CAJA_WINDOW_CLASS, slot->pane->window, close_slot,
                  (slot->pane, slot));
...
static void real_set_active_pane(CajaWindow *window, CajaWindowPane *new_pane) {
 ...
 * the window. */
void caja_window_set_active_pane(CajaWindow *window, CajaWindowPane *new_pane) {
...
void caja_window_set_active_slot(CajaWindow *window, CajaWindowSlot *new_slot) {
  ...
  if (old_slot != NULL) {
    ...
    if (old_slot->content_view != NULL) {
      caja_window_slot_disconnect_content_view(old_slot,
                                               old_slot->content_view);
    }
  ...
  /* deal with panes */
  if (new_slot && new_slot->pane != window->details->active_pane) {
...
static void caja_window_realize(GtkWidget *widget) {
  GTK_WIDGET_CLASS(caja_window_parent_class)->realize(widget);
  update_cursor(CAJA_WINDOW(widget));
}

static gboolean caja_window_key_press_event(GtkWidget *widget,
                                            GdkEventKey *event) {
  ...
  for (i = 0; i < G_N_ELEMENTS(extra_window_keybindings); i++) {
    if (extra_window_keybindings[i].keyval == event->keyval) {
      ...
      action_groups =
          gtk_ui_manager_get_action_groups(window->details->ui_manager);
      G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
      while (action_groups != NULL && action == NULL) {
        action = gtk_action_group_get_action(
            action_groups->data, extra_window_keybindings[i].action);
  ...
  return GTK_WIDGET_CLASS(caja_window_parent_class)
...
static void free_activate_view_data(gpointer data, GClosure *closure) {
...
static void action_view_as_callback(GtkAction *action, ActivateViewData *data) {
  ...
  if (gtk_toggle_action_get_active(GTK_TOGGLE_ACTION(action))) {
    ...
    slot = window->details->active_pane->active_slot;
    caja_window_slot_set_content_view(slot, data->id);
  }
  G_GNUC_END_IGNORE_DEPRECATIONS;
}

static GtkRadioAction *add_view_as_menu_item(
    CajaWindow *window, const char *placeholder_path, const char *identifier,
  ...
  G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
  action =
      gtk_radio_action_new(action_name, _(info->view_menu_label_with_mnemonic),
                           _(info->display_location_label), NULL, 0);
  G_GNUC_END_IGNORE_DEPRECATIONS;
  ...
  g_signal_connect_data(action, "activate", G_CALLBACK(action_view_as_callback),
  ...
  gtk_ui_manager_add_ui(window->details->ui_manager, merge_id, placeholder_path,
 ...
 */
static void update_extra_viewer_in_view_as_menus(CajaWindow *window,
                                                 const char *id) {
  ...
  if (id == NULL) {
    ...
    if (had_extra_viewer && strcmp(window->details->extra_viewer, id) == 0) {
      return;
    }
  }
  g_free(window->details->extra_viewer);
  ...
  if (id != NULL) {
    window->details->extra_viewer_merge_id =
        gtk_ui_manager_new_merge_id(window->details->ui_manager);
    window->details->extra_viewer_radio_action =
        add_view_as_menu_item(window, CAJA_MENU_PATH_EXTRA_VIEWER_PLACEHOLDER,
                              window->details->extra_viewer, 0,
                              window->details->extra_viewer_merge_id);
  }
}

static void remove_extra_viewer_in_view_as_menus(CajaWindow *window) {
  update_extra_viewer_in_view_as_menus(window, NULL);
}

static void replace_extra_viewer_in_view_as_menus(CajaWindow *window) {
  CajaWindowSlot *slot;
  const char *id;
 ...
 */
static void caja_window_synch_view_as_menus(CajaWindow *window) {
  CajaWindowSlot *slot;
  int index;
  char action_name[32];
  GList *node;
  ...
  for (node = window->details->short_list_viewers, index = 1; node != NULL;
  ...
  g_signal_handlers_block_matched(action, G_SIGNAL_MATCH_FUNC, 0, 0, NULL,
                                  action_view_as_callback, NULL);
  G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
  gtk_toggle_action_set_active(GTK_TOGGLE_ACTION(action), TRUE);
  G_GNUC_END_IGNORE_DEPRECATIONS;
  g_signal_handlers_unblock_matched(action, G_SIGNAL_MATCH_FUNC, 0, 0, NULL,
                                    action_view_as_callback, NULL);
}

static void refresh_stored_viewers(CajaWindow *window) {
  CajaWindowSlot *slot;
  GList *viewers;
  char *uri, *mimetype;
  ...
  viewers = caja_view_factory_get_views_for_uri(
      uri, caja_file_get_file_type(slot->viewed_file), mimetype);
  g_free(uri);
  g_free(mimetype);
...
static void load_view_as_menu(CajaWindow *window) {
  GList *node;
  guint merge_id;
...
#ifdef ENABLE_NLS
  gtk_action_group_set_translation_domain(window->details->view_as_action_group,
                                          GETTEXT_PACKAGE);
#endif /* ENABLE_NLS */
  G_GNUC_END_IGNORE_DEPRECATIONS;
  ...
  if (g_list_length(window->details->short_list_viewers) > 1) {
    ...
    for (node = window->details->short_list_viewers, index = 1; node != NULL;
      ...
      add_view_as_menu_item(window, CAJA_MENU_PATH_SHORT_LIST_PLACEHOLDER,
                            node->data, index, merge_id);
    }
  }
  gtk_ui_manager_insert_action_group(window->details->ui_manager,
                                     window->details->view_as_action_group, -1);
  g_object_unref(
      window->details->view_as_action_group); /* owned by ui_manager */
...
static void load_view_as_menus_callback(CajaFile *file,
...
void caja_window_load_view_as_menus(CajaWindow *window) {
  ...
  cancel_view_as_callback(slot);
  caja_file_call_when_ready(slot->viewed_file, attributes,
                            load_view_as_menus_callback, slot);
}
...
static char *real_get_title(CajaWindow *window) {
  g_assert(CAJA_IS_WINDOW(window));

  return caja_window_slot_get_title(window->details->active_pane->active_slot);
}

static void real_sync_title(CajaWindow *window, CajaWindowSlot *slot) {
  if (slot == window->details->active_pane->active_slot) {
    g_signal_emit_by_name(window, "title_changed", slot->title);
  }
}

void caja_window_sync_title(CajaWindow *window, CajaWindowSlot *slot) {
  EEL_CALL_METHOD(CAJA_WINDOW_CLASS, window, sync_title, (window, slot));
}
...
void caja_window_sync_zoom_widgets(CajaWindow *window) {
  ...
  if (view != NULL) {
    ...
    can_zoom = supports_zooming && zoom_level >= CAJA_ZOOM_LEVEL_SMALLEST &&
  ...
  g_signal_emit(window, signals[ZOOM_CHANGED], 0, zoom_level, supports_zooming,
                can_zoom, can_zoom_in, can_zoom_out);
}

static void zoom_level_changed_callback(CajaView *view, CajaWindow *window) {
  g_assert(CAJA_IS_WINDOW(window));
...
void caja_window_disconnect_content_view(CajaWindow *window, CajaView *view) {
  ...
  g_signal_handlers_disconnect_by_func(
      view, G_CALLBACK(zoom_level_changed_callback), window);
}
 ...
 * base on user prefs.
 */
static void caja_window_show(GtkWidget *widget) {
  CajaWindow *window;
...
void caja_window_slot_set_viewed_file(CajaWindowSlot *slot, CajaFile *file) {
  CajaFileAttributes attributes;
  ...
  if (slot->viewed_file != NULL) {
    ...
    if (CAJA_IS_SPATIAL_WINDOW(window)) {
      caja_file_set_has_open_window(slot->viewed_file, FALSE);
    }
    caja_file_monitor_remove(slot->viewed_file, slot);
  }
...
void caja_send_history_list_changed(void) {
  g_signal_emit_by_name(caja_signaller_get_current(), "history_list_changed");
}
 ...
 */
static void remove_from_history_list(CajaBookmark *bookmark) {
  GList *node;
   ...
   */
  node = g_list_find_custom(history_list, bookmark, caja_bookmark_compare_uris);
...
gboolean caja_add_to_history_list_no_notify(GFile *location, const char *name,
...
CajaWindowSlot *caja_window_get_slot_for_view(CajaWindow *window,
  ...
  for (walk = window->details->panes; walk; walk = walk->next) {
    ...
    for (l = pane->slots; l != NULL; l = l->next) {
      slot = l->data;
      if (slot->content_view == view || slot->new_content_view == view) {
        return slot;
      }
    }
  }
...
void caja_forget_history(void) {
  ...
  for (window_node = gtk_application_get_windows(GTK_APPLICATION(app));
       window_node != NULL; window_node = window_node->next) {
    if (CAJA_IS_NAVIGATION_WINDOW(window_node->data)) {
      CajaNavigationWindow *window;

      window = CAJA_NAVIGATION_WINDOW(window_node->data);

      for (walk = CAJA_WINDOW(window_node->data)->details->panes; walk;
           walk = walk->next) {
        CajaWindowPane *pane = walk->data;
        for (l = pane->slots; l != NULL; l = l->next) {
    ...
    for (walk = CAJA_WINDOW(window_node->data)->details->panes; walk;
      ...
      for (l = pane->slots; l != NULL; l = l->next) {
        slot = l->data;
        history_list =
            g_list_remove(history_list, slot->current_location_bookmark);
      }
    }
  }
...
GList *caja_get_history_list(void) { return history_list; }

static gpointer caja_window_copy_history_item(gconstpointer src,
                                              gpointer data) {
  (void)data;
  return g_object_ref(G_OBJECT(src));
}

static GList *caja_window_get_history(CajaWindow *window) {
  return g_list_copy_deep(history_list, caja_window_copy_history_item, NULL);
}

static CajaWindowType caja_window_get_window_type(CajaWindow *window) {
  g_assert(CAJA_IS_WINDOW(window));

  return CAJA_WINDOW_GET_CLASS(window)->window_type;
}

static int caja_window_get_selection_count(CajaWindow *window) {
  CajaWindowSlot *slot;
...
static GList *caja_window_get_selection(CajaWindow *window) {
  CajaWindowSlot *slot;
...
static CajaWindowShowHiddenFilesMode caja_window_get_hidden_files_mode(
    CajaWindowInfo *window) {
  return window->details->show_hidden_files_mode;
}

static void caja_window_set_hidden_files_mode(
    CajaWindowInfo *window, CajaWindowShowHiddenFilesMode mode) {
  window->details->show_hidden_files_mode = mode;

  g_signal_emit_by_name(window, "hidden_files_mode_changed");
}

static CajaWindowShowBackupFilesMode caja_window_get_backup_files_mode(
    CajaWindowInfo *window) {
  return window->details->show_backup_files_mode;
}

static void caja_window_set_backup_files_mode(
...
static gboolean caja_window_get_initiated_unmount(CajaWindowInfo *window) {
  return window->details->initiated_unmount;
}

static void caja_window_set_initiated_unmount(CajaWindowInfo *window,
                                              gboolean initiated_unmount) {
  window->details->initiated_unmount = initiated_unmount;
}

static char *caja_window_get_cached_title(CajaWindow *window) {
  CajaWindowSlot *slot;
...
CajaWindowSlot *caja_window_get_extra_slot(CajaWindow *window) {
  ...
  /* return NULL if there is only one pane */
  if (window->details->panes == NULL || window->details->panes->next == NULL) {
    return NULL;
  }
...
static void caja_window_class_init(CajaWindowClass *class) {
  ...
  g_object_class_install_property(
      G_OBJECT_CLASS(class), ARG_APP,
      g_param_spec_object("app", "Application",
                          "The CajaApplication associated with this window.",
                          CAJA_TYPE_APPLICATION,
                          G_PARAM_READWRITE | G_PARAM_CONSTRUCT));

  signals[GO_UP] = g_signal_new(
      "go_up", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
      G_STRUCT_OFFSET(CajaWindowClass, go_up),
      g_signal_accumulator_true_handled, NULL,
      caja_src_marshal_BOOLEAN__BOOLEAN, G_TYPE_BOOLEAN, 1, G_TYPE_BOOLEAN);
  signals[RELOAD] = g_signal_new(
      "reload", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
      G_STRUCT_OFFSET(CajaWindowClass, reload), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
  signals[PROMPT_FOR_LOCATION] = g_signal_new(
      "prompt-for-location", G_TYPE_FROM_CLASS(class),
      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
      G_STRUCT_OFFSET(CajaWindowClass, prompt_for_location), NULL, NULL,
      g_cclosure_marshal_VOID__STRING, G_TYPE_NONE, 1, G_TYPE_STRING);
  signals[ZOOM_CHANGED] = g_signal_new(
      "zoom-changed", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST, 0, NULL,
      NULL, caja_src_marshal_VOID__INT_BOOLEAN_BOOLEAN_BOOLEAN_BOOLEAN,
      G_TYPE_NONE, 5, G_TYPE_INT, G_TYPE_BOOLEAN, G_TYPE_BOOLEAN,
      G_TYPE_BOOLEAN, G_TYPE_BOOLEAN);
  signals[VIEW_AS_CHANGED] = g_signal_new(
      "view-as-changed", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST, 0, NULL,
      NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  binding_set = gtk_binding_set_by_class(class);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_BackSpace, 0, "go_up", 1,
                               G_TYPE_BOOLEAN, FALSE);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_F5, 0, "reload", 0);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_slash, 0,
                               "prompt-for-location", 1, G_TYPE_STRING, "/");
