...
#define TOOLBAR_PATH_EXTENSION_ACTIONS \
  "/Toolbar/Extra Buttons Placeholder/Extension Actions"
...
void caja_navigation_window_activate_spinner(CajaNavigationWindow *window) {
  ...
  gtk_toolbar_insert(GTK_TOOLBAR(window->details->toolbar), item, -1);
...
static GList *get_extension_toolbar_items(CajaNavigationWindow *window) {
  CajaWindowSlot *slot;
  GList *items;
  GList *providers;
  GList *l;
...
void caja_navigation_window_load_extension_toolbar_items(
  ...
  if (window->details->extensions_toolbar_action_group != NULL) {
    gtk_ui_manager_remove_action_group(
        ui_manager, window->details->extensions_toolbar_action_group);
