...
static void caja_property_browser_update_contents(
    CajaPropertyBrowser *property_browser);
static void caja_property_browser_set_category(
    CajaPropertyBrowser *property_browser, const char *new_category);
static void caja_property_browser_set_dragged_file(
    CajaPropertyBrowser *property_browser, const char *dragged_file_name);
static void caja_property_browser_set_drag_type(
    CajaPropertyBrowser *property_browser, const char *new_drag_type);
...
static void caja_property_browser_drag_begin(GtkWidget *widget,
                                             GdkDragContext *context);
static void caja_property_browser_drag_data_get(
...
/* misc utilities */
static void element_clicked_callback(GtkWidget *image_table, GtkWidget *child,
...
static GtkTargetEntry drag_types[] = {{"text/uri-list", 0, PROPERTY_TYPE}};

G_DEFINE_TYPE_WITH_PRIVATE(CajaPropertyBrowser, caja_property_browser,
                           GTK_TYPE_WINDOW)

/* Destroy the three dialogs for adding patterns/colors/emblems if any of them
   exist. */
static void caja_property_browser_destroy_dialogs(
    CajaPropertyBrowser *property_browser) {
...
static void caja_property_browser_dispose(GObject *object) {
  CajaPropertyBrowser *property_browser;
...
static void caja_property_browser_class_init(CajaPropertyBrowserClass *klass) {
...
static void caja_property_browser_init(CajaPropertyBrowser *property_browser) {
  ...
  property_browser->details =
      caja_property_browser_get_instance_private(property_browser);
  ...
  gtk_box_pack_start(GTK_BOX(vbox), property_browser->details->container, TRUE,
                     TRUE, 0);

  /* make the category container */
  property_browser->details->category_container =
  ...
  gtk_box_pack_start(GTK_BOX(property_browser->details->container),
                     property_browser->details->category_container, FALSE,
                     FALSE, 0);
  gtk_widget_show(property_browser->details->category_container);
  gtk_scrolled_window_set_policy(
      GTK_SCROLLED_WINDOW(property_browser->details->category_container),
  ...
  property_browser->details->category_box =
      gtk_box_new(GTK_ORIENTATION_VERTICAL, 6);
  gtk_container_add(GTK_CONTAINER(viewport),
                    property_browser->details->category_box);
  gtk_container_add(
      GTK_CONTAINER(property_browser->details->category_container), viewport);
  ...
  gtk_box_pack_start(GTK_BOX(property_browser->details->container),
                     property_browser->details->content_container, TRUE, TRUE,
                     0);
  ...
  g_signal_connect_object(temp_button, "clicked",
                          G_CALLBACK(help_button_callback), property_browser,
                          0);
  ...
  gtk_widget_show(temp_button);
  gtk_box_pack_end(GTK_BOX(property_browser->details->bottom_box), temp_button,
                   FALSE, FALSE, 0);
  gtk_widget_grab_default(temp_button);
  gtk_widget_grab_focus(temp_button);
  g_signal_connect_object(temp_button, "clicked",
                          G_CALLBACK(done_button_callback), property_browser,
                          0);
  ...
  g_signal_connect_object(property_browser->details->remove_button, "clicked",
                          G_CALLBACK(remove_button_callback), property_browser,
                          0);

  /* now create the "add new" button */
  property_browser->details->add_button =
      gtk_button_new_with_mnemonic(_("Add new..."));
  ...
  g_signal_connect(property_browser, "delete_event",
                   G_CALLBACK(caja_property_browser_delete_event_callback),
                   NULL);
  g_signal_connect(property_browser, "hide",
                   G_CALLBACK(caja_property_browser_hide_callback), NULL);

  g_signal_connect_object(caja_signaller_get_current(), "emblems_changed",
                          G_CALLBACK(emblems_changed_callback),
                          property_browser, 0);

  /* initially, display the top level */
  caja_property_browser_set_path(property_browser,
                                 BROWSER_CATEGORIES_FILE_NAME);
}
...
void caja_property_browser_show(GdkScreen *screen) {
  ...
  if (browser == NULL) {
    ...
    g_object_add_weak_pointer(G_OBJECT(browser), (gpointer *)&browser);
  } else {
...
static gboolean caja_property_browser_delete_event_callback(
...
static void caja_property_browser_drag_begin(GtkWidget *widget,
  ...
  if (pixbuf != NULL) {
    ...
    gtk_drag_set_icon_pixbuf(context, pixbuf, x_delta, y_delta);
  ...
  switch (info) {
    case PROPERTY_TYPE:
      /* formulate the drag data based on the drag type.  Eventually, we will
         probably select the behavior from properties in the category xml
         definition, but for now we hardwire it to the drag_type */

      is_reset = FALSE;
      if (strcmp(property_browser->details->drag_type, "property/keyword") ==
          0) {
        char *keyword_str = eel_filename_strip_extension(
            property_browser->details->dragged_file);
        gtk_selection_data_set(selection_data, target, 8, keyword_str,
                               strlen(keyword_str));
        g_free(keyword_str);
        return;
        ...
        if (g_strcmp0(property_browser->details->dragged_file,
                      RESET_IMAGE_NAME) != 0) {
          GdkRGBA color;
          ...
          gtk_selection_data_set(selection_data, target, 16,
                                 (const char *)&colorArray[0], 8);
          return;
        } else {
          is_reset = TRUE;
        }
      ...
      image_file_name = g_strdup_printf(
          "%s/%s/%s", CAJA_DATADIR,
      ...
      if (!g_file_test(image_file_name, G_FILE_TEST_EXISTS)) {
        ...
        image_file_name = g_strdup_printf(
            "%s/%s/%s", user_directory, property_browser->details->category,
            property_browser->details->dragged_file);
      ...
      gtk_selection_data_set(selection_data, target, 8, image_file_uri,
                             strlen(image_file_uri));
      g_free(image_file_name);
...
/* drag and drop end handler, where we destroy ourselves, since the transaction
 * is complete */
...
static gboolean ensure_file_is_image(GFile *file) {
  GFileInfo *info;
...
static GdkPixbuf *make_drag_image(CajaPropertyBrowser *property_browser,
  ...
  image_file_name = g_strdup_printf(
      "%s/%s/%s", CAJA_DATADIR, property_browser->details->category, file_name);
  ...
  orig_pixbuf = gdk_pixbuf_new_from_file_at_scale(
      image_file_name, MAX_ICON_WIDTH, MAX_ICON_HEIGHT, TRUE, NULL);
  ...
  if (strcmp(property_browser->details->category, "patterns") == 0 &&
    ...
    pixbuf = caja_customization_make_pattern_chit(
        orig_pixbuf, property_browser->details->property_chit, TRUE, is_reset);
  } else {
    pixbuf = eel_gdk_pixbuf_scale_down_to_fit(orig_pixbuf, MAX_ICON_WIDTH,
                                              MAX_ICON_HEIGHT);
  }
...
static GdkPixbuf *make_color_drag_image(CajaPropertyBrowser *property_browser,
  ...
  color_square = gdk_pixbuf_new(GDK_COLORSPACE_RGB, TRUE, 8, COLOR_SQUARE_SIZE,
                                COLOR_SQUARE_SIZE);
  ...
  if (property_browser->details->property_chit != NULL) {
    ret = caja_customization_make_pattern_chit(
...
/* this callback handles button presses on the category widget. It maintains the
 * active state */

static void category_toggled_callback(GtkWidget *widget, char *category_name) {
  CajaPropertyBrowser *property_browser;

  property_browser =
      CAJA_PROPERTY_BROWSER(g_object_get_data(G_OBJECT(widget), "user_data"));

  /* exit remove mode when the user switches categories, since there might be
     nothing to remove in the new category */
  property_browser->details->remove_mode = FALSE;

  caja_property_browser_set_category(property_browser, category_name);
}

static xmlDocPtr read_browser_xml(CajaPropertyBrowser *property_browser) {
  char *path;
  xmlDocPtr document;
...
static void write_browser_xml(CajaPropertyBrowser *property_browser,
                              xmlDocPtr document) {
  char *user_directory, *path;

  user_directory = caja_get_user_directory();
  path =
      g_build_filename(user_directory, property_browser->details->path, NULL);
  g_free(user_directory);
  xmlSaveFile(path, document);
  g_free(path);
}

static xmlNodePtr get_color_category(xmlDocPtr document) {
  return eel_xml_get_root_child_by_name_and_property(
      document, (const xmlChar *)"category", (const xmlChar *)"name",
      (const xmlChar *)"colors");
}

/* routines to remove specific category types.  First, handle colors */

static void remove_color(CajaPropertyBrowser *property_browser,
  ...
  if (cur_node != NULL) {
    ...
    for (color_node = eel_xml_get_children(cur_node); color_node != NULL;
...
static void remove_pattern(CajaPropertyBrowser *property_browser,
  ...
  pattern_path =
      g_build_filename(user_directory, "patterns", pattern_name, NULL);
  g_free(user_directory);

  /* delete the pattern from the pattern directory */
  if (g_unlink(pattern_path) != 0) {
    char *message = g_strdup_printf(
        _("Sorry, but pattern %s could not be deleted."), pattern_name);
...
static void remove_emblem(CajaPropertyBrowser *property_browser,
                          const char *emblem_name) {
  /* delete the emblem from the emblem directory */
  if (caja_emblem_remove_emblem(emblem_name) == FALSE) {
    char *message = g_strdup_printf(
        _("Sorry, but emblem %s could not be deleted."), emblem_name);
...
static void caja_property_browser_remove_element(
    CajaPropertyBrowser *property_browser, EelLabeledImage *child) {
...
static void update_preview_cb(GtkFileChooser *fc, GtkImage *preview) {
  char *filename;
...
static void icon_button_clicked_cb(GtkButton *b, CajaPropertyBrowser *browser) {
  GtkWidget *dialog;
  ...
  dialog = eel_file_chooser_dialog_new(
      _("Select an Image File for the New Emblem"), GTK_WINDOW(browser),
      GTK_FILE_CHOOSER_ACTION_OPEN, "process-stop", GTK_RESPONSE_CANCEL,
  ...
  g_signal_connect(dialog, "update-preview", G_CALLBACK(update_preview_cb),
                   preview);
  ...
  if (res == GTK_RESPONSE_ACCEPT) {
    ...
    browser->details->filename =
        gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(dialog));
    gtk_image_set_from_file(GTK_IMAGE(browser->details->image_button),
                            browser->details->filename);
  }
...
static GtkWidget *caja_emblem_dialog_new(
    CajaPropertyBrowser *property_browser) {
  GtkWidget *widget;
  GtkWidget *button;
  GtkWidget *dialog;
  ...
  gtk_entry_set_activates_default(GTK_ENTRY(property_browser->details->keyword),
                                  TRUE);
  gtk_entry_set_max_length(GTK_ENTRY(property_browser->details->keyword), 24);
  gtk_widget_show(property_browser->details->keyword);
  gtk_grid_attach(GTK_GRID(grid), property_browser->details->keyword, 1, 0, 1,
                  1);
  ...
  property_browser->details->image_path =
      g_build_filename(CAJA_PIXMAPDIR, "emblems.png", NULL);
  ...
  property_browser->details->image_button =
      gtk_image_new_from_file(property_browser->details->image_path);
...
static GtkWidget *caja_color_selection_dialog_new(
    CajaPropertyBrowser *property_browser) {
  GtkWidget *widget;
  GtkWidget *dialog;
  ...
  gtk_entry_set_activates_default(
      GTK_ENTRY(property_browser->details->color_name), TRUE);
  gtk_entry_set_max_length(GTK_ENTRY(property_browser->details->color_name),
                           24);
  gtk_widget_grab_focus(property_browser->details->color_name);
  gtk_label_set_mnemonic_widget(GTK_LABEL(widget),
                                property_browser->details->color_name);
  gtk_widget_show(property_browser->details->color_name);
  gtk_grid_attach(GTK_GRID(grid), property_browser->details->color_name, 1, 0,
                  1, 1);
  gtk_widget_grab_focus(property_browser->details->color_name);
  ...
  gtk_label_set_mnemonic_widget(GTK_LABEL(widget),
                                property_browser->details->color_picker);

  gtk_grid_attach(GTK_GRID(grid), property_browser->details->color_picker, 1, 1,
                  1, 1);
...
static void add_pattern_to_browser(GtkDialog *dialog, gint response_id,
                                   gpointer data) {
  ...
  if (!g_file_copy(selected, dest, 0, NULL, NULL, NULL, NULL)) {
    char *message = g_strdup_printf(
        _("Sorry, but the pattern %s could not be installed."), basename);
...
static void add_new_pattern(CajaPropertyBrowser *property_browser) {
  GtkWidget *dialog;
  ...
  if (property_browser->details->patterns_dialog) {
    ...
    property_browser->details->patterns_dialog = dialog =
        eel_file_chooser_dialog_new(
            _("Select an Image File to Add as a Pattern"),
            GTK_WINDOW(property_browser), GTK_FILE_CHOOSER_ACTION_OPEN,
            "process-stop", GTK_RESPONSE_CANCEL, "document-open",
    ...
    g_signal_connect(dialog, "response", G_CALLBACK(add_pattern_to_browser),
                     property_browser);
...
static void add_color_to_file(CajaPropertyBrowser *property_browser,
  ...
  if (cur_node != NULL) {
    ...
    if (!color_name_exists) {
      new_color_node =
          xmlNewChild(cur_node, NULL, (const xmlChar *)"color", NULL);
      xmlNodeSetContent(new_color_node, color_spec);
      xmlSetProp(new_color_node, (const xmlChar *)"local",
                 (const xmlChar *)"1");
      ...
      eel_show_error_dialog(_("The color cannot be installed."),
                            _("Sorry, but you must specify an unused color "
...
static void add_color_to_browser(GtkWidget *widget, gint which_button,
                                 gpointer data) {
  CajaPropertyBrowser *property_browser = CAJA_PROPERTY_BROWSER(data);
  ...
  if (which_button == GTK_RESPONSE_OK) {
    ...
    gtk_color_button_get_color(
        GTK_COLOR_BUTTON(property_browser->details->color_picker), &color);
    color_spec = gdk_color_to_string(&color);

    color_name =
        gtk_entry_get_text(GTK_ENTRY(property_browser->details->color_name));
    ...
    if (strlen(stripped_color_name) == 0) {
      ...
      add_color_to_file(property_browser, (const xmlChar *)color_spec,
                        (const xmlChar *)stripped_color_name);
      caja_property_browser_update_contents(property_browser);
    }
...
/* create the color selection dialog, pre-set with the color that was just
 * selected */
static void show_color_selection_window(GtkWidget *widget, gpointer data) {
  GdkColor color;
  CajaPropertyBrowser *property_browser = CAJA_PROPERTY_BROWSER(data);
  ...
  gtk_color_selection_get_current_color(
      GTK_COLOR_SELECTION(gtk_color_selection_dialog_get_color_selection(
          GTK_COLOR_SELECTION_DIALOG(
              property_browser->details->colors_dialog))),
      &color);
  gtk_widget_destroy(property_browser->details->colors_dialog);

  /* allocate a new color selection dialog */
  property_browser->details->colors_dialog =
      caja_color_selection_dialog_new(property_browser);
  ...
  g_signal_connect_object(property_browser->details->colors_dialog, "response",
                          ...
                          0);
  gtk_window_set_position(GTK_WINDOW(property_browser->details->colors_dialog),
                          GTK_WIN_POS_MOUSE);
...
static void add_new_color(CajaPropertyBrowser *property_browser) {
  if (property_browser->details->colors_dialog) {
    ...
    property_browser->details->colors_dialog =
        gtk_color_selection_dialog_new(_("Select a Color to Add"));
    color_dialog =
        GTK_COLOR_SELECTION_DIALOG(property_browser->details->colors_dialog);

    eel_add_weak_pointer(&property_browser->details->colors_dialog);

    g_object_get(color_dialog, "ok-button", &ok_button, "cancel-button",
                 &cancel_button, "help-button", &help_button, NULL);

    g_signal_connect_object(ok_button, "clicked",
                            G_CALLBACK(show_color_selection_window),
                            property_browser, 0);
...
static void emblem_dialog_clicked(GtkWidget *dialog, int which_button,
                                  CajaPropertyBrowser *property_browser) {
  char *emblem_path;
  ...
  if (which_button == GTK_RESPONSE_OK) {
    ...
    if (property_browser->details->filename) {
      ...
      if (ensure_file_is_image(emblem_file)) {
        ...
        eel_show_error_dialog(_("The file is not an image."), message,
                              GTK_WINDOW(property_browser));
        g_free(message);
        g_free(emblem_path);
        emblem_path = NULL;
    ...
    if (pixbuf == NULL) {
      char *message =
          g_strdup_printf(_("Sorry, but \"%s\" is not a usable image file."),
                          property_browser->details->image_path);
      eel_show_error_dialog(_("The file is not an image."), message,
                            GTK_WINDOW(property_browser));
      g_free(message);
    ...
    new_keyword =
        gtk_entry_get_text(GTK_ENTRY(property_browser->details->keyword));
    if (new_keyword == NULL) {
    ...
    caja_emblem_install_custom_emblem(pixbuf, stripped_keyword,
                                      stripped_keyword,
                                      GTK_WINDOW(property_browser));
...
static void add_new_emblem(CajaPropertyBrowser *property_browser) {
  if (property_browser->details->emblems_dialog) {
    gtk_window_present(GTK_WINDOW(property_browser->details->emblems_dialog));
  } else {
    property_browser->details->emblems_dialog =
        caja_emblem_dialog_new(property_browser);

    eel_add_weak_pointer(&property_browser->details->emblems_dialog);

    g_signal_connect_object(property_browser->details->emblems_dialog,
                            "response", G_CALLBACK(emblem_dialog_clicked),
                            property_browser, 0);
    gtk_window_set_position(
        GTK_WINDOW(property_browser->details->emblems_dialog),
        GTK_WIN_POS_MOUSE);
...
static void help_button_callback(GtkWidget *widget,
  ...
  if (error) {
    ...
    dialog = gtk_message_dialog_new(
        GTK_WINDOW(property_browser), GTK_DIALOG_DESTROY_WITH_PARENT,
...
/* this callback handles clicks on the image or color based content content
 * elements */
...
static void element_clicked_callback(GtkWidget *image_table, GtkWidget *child,
  ...
  if (property_browser->details->remove_mode) {
    caja_property_browser_remove_element(property_browser,
                                         EEL_LABELED_IMAGE(child));
    property_browser->details->remove_mode = FALSE;
    caja_property_browser_update_contents(property_browser);
    gtk_widget_show(property_browser->details->help_label);
    return;
  ...
  gtk_drag_begin_with_coordinates(
      GTK_WIDGET(property_browser), target_list,
      GDK_ACTION_ASK | GDK_ACTION_MOVE | GDK_ACTION_COPY, event->button,
      event->event, event->x, event->y);

  gtk_target_list_unref(target_list);

  /* optionally (if the shift key is down) hide the property browser - it will
   * later be destroyed when the drag ends */
...
static void labeled_image_configure(EelLabeledImage *labeled_image) {
...
static GtkWidget *labeled_image_new(const char *text, GdkPixbuf *pixbuf,
  ...
  if (property_name != NULL) {
    g_object_set_data_full(G_OBJECT(labeled_image), "property-name",
                           g_strdup(property_name), g_free);
  }

  return labeled_image;
}

static void make_properties_from_directories(
    CajaPropertyBrowser *property_browser) {
  char *object_name;
  char *object_label;
  ...
  g_return_if_fail(
      EEL_IS_IMAGE_TABLE(property_browser->details->content_table));
  ...
  if (property_browser->details->category_type == CAJA_PROPERTY_EMBLEM) {
    ...
    while (l != NULL) {
      ...
      info = caja_icon_info_lookup_from_name(icon_name, CAJA_ICON_SIZE_STANDARD,
                                             1);
      object_pixbuf =
          caja_icon_info_get_pixbuf_at_size(info, CAJA_ICON_SIZE_STANDARD);
      ...
      property_image = labeled_image_new(object_label, object_pixbuf,
                                         object_name, PANGO_SCALE_LARGE);
      ...
      property_browser->details->keywords =
          g_list_prepend(property_browser->details->keywords, keyword);
    ...
    customization_data =
        caja_customization_data_new(property_browser->details->category,
    ...
    while (caja_customization_data_get_next_element_for_display(
        customization_data, &object_name, &object_pixbuf, &object_label)) {
      property_image = labeled_image_new(object_label, object_pixbuf,
                                         object_name, PANGO_SCALE_LARGE);
    ...
    property_browser->details->has_local =
        caja_customization_data_private_data_was_displayed(customization_data);
  ...
  if (property_browser->details->category_type == CAJA_PROPERTY_EMBLEM) {
    ...
    eel_wrap_table_reorder_child(EEL_WRAP_TABLE(image_table), blank,
                                 num_images - 1);
    ...
    g_free(path);
    property_image = labeled_image_new(
        _("Erase"), object_pixbuf, (const char *)"erase", PANGO_SCALE_LARGE);
    eel_labeled_image_set_fixed_image_height(EEL_LABELED_IMAGE(property_image),
                                             MAX_EMBLEM_HEIGHT);

    gtk_container_add(GTK_CONTAINER(image_table), property_image);
    gtk_widget_show(property_image);

    eel_wrap_table_reorder_child(EEL_WRAP_TABLE(image_table), property_image,
                                 -1);
  ...
  if (reset_object != NULL) {
    ...
    eel_wrap_table_reorder_child(EEL_WRAP_TABLE(image_table), reset_object, 0);
  }
}

/* utility routine to add a reset property in the first position */
static void add_reset_property(CajaPropertyBrowser *property_browser) {
  ...
  reset_image_file_name =
      g_strdup_printf("%s/%s/%s", CAJA_DATADIR, "patterns", RESET_IMAGE_NAME);
  reset_pixbuf = gdk_pixbuf_new_from_file(reset_image_file_name, NULL);
  if (reset_pixbuf != NULL &&
      property_browser->details->property_chit != NULL) {
    reset_chit = caja_customization_make_pattern_chit(
        reset_pixbuf, property_browser->details->property_chit, FALSE, TRUE);
  }

  g_free(reset_image_file_name);

  reset_image = labeled_image_new(
      _("Reset"), reset_chit != NULL ? reset_chit : reset_pixbuf,
      RESET_IMAGE_NAME, PANGO_SCALE_MEDIUM);
  gtk_container_add(GTK_CONTAINER(property_browser->details->content_table),
                    reset_image);
  eel_wrap_table_reorder_child(
      EEL_WRAP_TABLE(property_browser->details->content_table), reset_image, 0);
...
static void make_properties_from_xml_node(CajaPropertyBrowser *property_browser,
                                          xmlNodePtr node) {
  ...
  for (child_node = eel_xml_get_children(node); child_node != NULL;
     ...
     * To prevent these colors from suddenly showing up now, this legacy
     * remains.
     */
    ...
    if (deleted == NULL && (!local_only || local != NULL)) {
      ...
      pixbuf =
          make_color_drag_image(property_browser, (const char *)color, FALSE);

      /* make the tile from the pixmap and name */
      new_property = labeled_image_new(_((const char *)name), pixbuf,
                                       (const char *)color, PANGO_SCALE_LARGE);
...
/* make_category generates widgets corresponding all of the objects in the
 * passed in directory */
static void make_category(CajaPropertyBrowser *property_browser,
                          const char *path, const char *mode, xmlNodePtr node,
...
static GtkWidget *property_browser_category_button_new(const char *display_name,
                                                       const char *image) {
  GtkWidget *button;
  char *file_name;
  ...
  if (file_name != NULL) {
    button = eel_labeled_image_radio_button_new_from_file_name(display_name,
                                                               file_name);
  } else {
...
static void free_category_link_name(gpointer data, GClosure *closure) {
  (void)closure;
  g_free(data);
}

/* this is a utility routine to generate a category link widget and install it
 * in the browser */
static void make_category_link(CajaPropertyBrowser *property_browser,
                               const char *name, const char *display_name,
                               const char *image, GtkRadioButton **group) {
  GtkWidget *button;
  ...
  gtk_box_pack_start(GTK_BOX(property_browser->details->category_box), button,
                     FALSE, FALSE, 0);
  ...
  g_signal_connect_data(button, "toggled",
                        G_CALLBACK(category_toggled_callback), g_strdup(name),
                        free_category_link_name, 0);
}

/* update_contents populates the property browser with information specified by
 * the path and other state variables */
void caja_property_browser_update_contents(
    CajaPropertyBrowser *property_browser) {
  xmlNodePtr cur_node;
  xmlDocPtr document;
  GtkWidget *viewport;
  ...
  property_browser->details->content_frame =
      gtk_scrolled_window_new(NULL, NULL);
  gtk_widget_set_vexpand(property_browser->details->content_frame, TRUE);
  gtk_scrolled_window_set_shadow_type(
      GTK_SCROLLED_WINDOW(property_browser->details->content_frame),
      GTK_SHADOW_IN);
  viewport = gtk_viewport_new(NULL, NULL);
  gtk_widget_show(viewport);
  gtk_viewport_set_shadow_type(GTK_VIEWPORT(viewport), GTK_SHADOW_IN);
  gtk_container_add(GTK_CONTAINER(property_browser->details->content_container),
                    property_browser->details->content_frame);
  ...
  gtk_scrolled_window_set_policy(
      ...
      GTK_POLICY_NEVER, GTK_POLICY_AUTOMATIC);
  gtk_scrolled_window_set_overlay_scrolling(
      GTK_SCROLLED_WINDOW(property_browser->details->content_frame), FALSE);
  ...
  eel_wrap_table_set_x_spacing(
      EEL_WRAP_TABLE(property_browser->details->content_table),
      IMAGE_TABLE_X_SPACING);
  eel_wrap_table_set_y_spacing(
      EEL_WRAP_TABLE(property_browser->details->content_table),
      IMAGE_TABLE_Y_SPACING);

  g_signal_connect_object(property_browser->details->content_table,
                          "child_pressed", G_CALLBACK(element_clicked_callback),
                          property_browser, 0);

  gtk_container_add(GTK_CONTAINER(viewport),
                    property_browser->details->content_table);
  gtk_container_add(GTK_CONTAINER(property_browser->details->content_frame),
                    viewport);
  ...
  for (cur_node = eel_xml_get_children(xmlDocGetRootElement(document));
    ...
    if (cur_node->name &&
        xmlStrcmp(cur_node->name, (const xmlChar *)"category") == 0) {
      name = xmlGetProp(cur_node, (const xmlChar *)"name");

      if (property_browser->details->category != NULL &&
          strcmp(property_browser->details->category, (const char *)name) ==
              0) {
        ...
        make_category(property_browser, (const char *)path, (const char *)mode,
                      cur_node, _((const char *)description));
        caja_property_browser_set_drag_type(property_browser,
                                            (const char *)type);
      ...
      if (!got_categories) {
        ...
        make_category_link(property_browser, (const char *)name,
                           _((const char *)display_name), (const char *)image,
                           &group);
  ...
  if (property_browser->details->category == NULL) {
    gtk_label_set_text(GTK_LABEL(property_browser->details->title_label),
                       _("Select a Category:"));
    ...
    gtk_image_set_from_icon_name(
        GTK_IMAGE(property_browser->details->add_button_image), icon_name,
        GTK_ICON_SIZE_BUTTON);

    if (text != NULL) {
      gtk_button_set_label(GTK_BUTTON(property_browser->details->add_button),
                           text);
    }
    ...
    /* case out instead of substituting to provide flexibilty for other
     * languages */
    /* FIXME: Using spaces to add padding is not good design. */
    switch (property_browser->details->category_type) {
      case CAJA_PROPERTY_PATTERN:
    ...
    if (text != NULL) {
      gtk_button_set_label(GTK_BUTTON(property_browser->details->remove_button),
                           text);
    }
  }
}
...
/* here is the routine that populates the property browser with the appropriate
   information when the path changes */

void caja_property_browser_set_path(CajaPropertyBrowser *property_browser,
                                    const char *new_path) {
