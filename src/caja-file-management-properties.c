...
/* caja-file-management-properties.c - Functions to create and show the caja
   preference dialog.
...
#define CAJA_FILE_MANAGEMENT_PROPERTIES_DEFAULT_VIEW_WIDGET \
  "default_view_combobox"
#define CAJA_FILE_MANAGEMENT_PROPERTIES_ICON_VIEW_ZOOM_WIDGET \
  "icon_view_zoom_combobox"
#define CAJA_FILE_MANAGEMENT_PROPERTIES_COMPACT_VIEW_ZOOM_WIDGET \
  "compact_view_zoom_combobox"
#define CAJA_FILE_MANAGEMENT_PROPERTIES_LIST_VIEW_ZOOM_WIDGET \
  "list_view_zoom_combobox"
#define CAJA_FILE_MANAGEMENT_PROPERTIES_SORT_ORDER_WIDGET "sort_order_combobox"
#define CAJA_FILE_MANAGEMENT_PROPERTIES_DATE_FORMAT_WIDGET \
  "date_format_combobox"
#define CAJA_FILE_MANAGEMENT_PROPERTIES_PREVIEW_TEXT_WIDGET \
  "preview_text_combobox"
#define CAJA_FILE_MANAGEMENT_PROPERTIES_PREVIEW_IMAGE_WIDGET \
  "preview_image_combobox"
#define CAJA_FILE_MANAGEMENT_PROPERTIES_PREVIEW_SOUND_WIDGET \
  "preview_sound_combobox"
#define CAJA_FILE_MANAGEMENT_PROPERTIES_PREVIEW_FOLDER_WIDGET \
  "preview_folder_combobox"

/* bool preferences */
#define CAJA_FILE_MANAGEMENT_PROPERTIES_FOLDERS_FIRST_WIDGET \
  "sort_folders_first_checkbutton"
#define CAJA_FILE_MANAGEMENT_PROPERTIES_COMPACT_LAYOUT_WIDGET \
  "compact_layout_checkbutton"
#define CAJA_FILE_MANAGEMENT_PROPERTIES_LABELS_BESIDE_ICONS_WIDGET \
  "labels_beside_icons_checkbutton"
#define CAJA_FILE_MANAGEMENT_PROPERTIES_ALL_COLUMNS_SAME_WIDTH \
  "all_columns_same_width_checkbutton"
#define CAJA_FILE_MANAGEMENT_PROPERTIES_ALWAYS_USE_BROWSER_WIDGET \
  "always_use_browser_checkbutton"
#define CAJA_FILE_MANAGEMENT_PROPERTIES_TRASH_CONFIRM_WIDGET \
  "trash_confirm_checkbutton"
#define CAJA_FILE_MANAGEMENT_PROPERTIES_TRASH_CONFIRM_TRASH_WIDGET \
  "trash_confirm_trash_checkbutton"
#define CAJA_FILE_MANAGEMENT_PROPERTIES_TRASH_DELETE_WIDGET \
  "trash_delete_checkbutton"
#define CAJA_FILE_MANAGEMENT_PROPERTIES_SHOW_HIDDEN_WIDGET \
  "hidden_files_checkbutton"
#define CAJA_FILE_MANAGEMENT_PROPERTIES_SHOW_BACKUP_WIDGET \
  "backup_files_checkbutton"
#define CAJA_FILE_MANAGEMENT_PROPERTIES_TREE_VIEW_FOLDERS_WIDGET \
  "treeview_folders_checkbutton"
#define CAJA_FILE_MANAGEMENT_PROPERTIES_MEDIA_AUTOMOUNT_OPEN \
  "media_automount_open_checkbutton"
...
#define CAJA_FILE_MANAGEMENT_PROPERTIES_SHOW_ICONS_IN_LIST_VIEW \
  "show_icons_in_list_view"

/* int enums */
#define CAJA_FILE_MANAGEMENT_PROPERTIES_THUMBNAIL_LIMIT_WIDGET \
  "preview_image_size_combobox"

static const char *const default_view_values[] = {"icon-view", "list-view",
                                                  "compact-view", NULL};

static const char *const zoom_values[] = {"smallest", "smaller", "small",
...
static const char *const sort_order_values[] = {
    "name",  "directory", "size",    "size_on_disk", "type",       "mtime",
    "btime", "atime",     "emblems", "extension",    "trash-time", NULL};

static const char *const date_format_values[] = {"locale", "iso", "informal",
                                                 NULL};
...
enum { EXT_STATE_COLUMN, EXT_ICON_COLUMN, EXT_INFO_COLUMN, EXT_STRUCT_COLUMN };

static void caja_file_management_properties_dialog_update_media_sensitivity(
    GtkBuilder *builder);

static void preferences_show_help(GtkWindow *parent, char const *helpfile,
                                  char const *sect_id) {
  GError *error = NULL;
  char *help_string;
  ...
  gtk_show_uri_on_window(parent, help_string, gtk_get_current_event_time(),
                         &error);
  g_free(help_string);
...
static void caja_file_management_properties_dialog_response_cb(
    GtkDialog *parent, int response_id, GtkBuilder *builder) {
  if (response_id == GTK_RESPONSE_HELP) {
    ...
    g_signal_handlers_disconnect_by_func(
        caja_media_preferences,
        caja_file_management_properties_dialog_update_media_sensitivity,
        builder);
  }
}

static void columns_changed_callback(CajaColumnChooser *chooser,
...
static void update_icon_captions_from_settings(GtkBuilder *builder) {
  ...
  captions = g_settings_get_strv(caja_icon_view_preferences,
                                 CAJA_PREFERENCES_ICON_VIEW_CAPTIONS);
...
static void caja_file_management_properties_dialog_setup_icon_caption_page(
    GtkBuilder *builder) {
  GList *columns;
  int i;
  gboolean writable;

  writable = g_settings_is_writable(caja_icon_view_preferences,
                                    CAJA_PREFERENCES_ICON_VIEW_CAPTIONS);
  ...
  for (i = 0; icon_captions_components[i] != NULL; i++) {
    ...
    g_signal_connect(combo_box, "changed",
                     G_CALLBACK(icon_captions_changed_callback), builder);
  }
...
static void create_date_format_menu(GtkBuilder *builder) {
  GtkComboBoxText *combo_box;
  gchar *date_string;
  GDateTime *now;

  combo_box = GTK_COMBO_BOX_TEXT(gtk_builder_get_object(
      builder, CAJA_FILE_MANAGEMENT_PROPERTIES_DATE_FORMAT_WIDGET));
...
static void set_columns_from_settings(CajaColumnChooser *chooser) {
  char **visible_columns;
  char **column_order;

  visible_columns =
      g_settings_get_strv(caja_list_view_preferences,
                          CAJA_PREFERENCES_LIST_VIEW_DEFAULT_VISIBLE_COLUMNS);
  column_order =
      g_settings_get_strv(caja_list_view_preferences,
                          CAJA_PREFERENCES_LIST_VIEW_DEFAULT_COLUMN_ORDER);
...
static void use_default_callback(CajaColumnChooser *chooser,
                                 gpointer user_data) {
...
static void caja_file_management_properties_dialog_setup_list_column_page(
    GtkBuilder *builder) {
  GtkWidget *chooser;
  GtkWidget *box;
  ...
  g_signal_connect(chooser, "changed", G_CALLBACK(columns_changed_callback),
                   chooser);
  g_signal_connect(chooser, "use_default", G_CALLBACK(use_default_callback),
                   chooser);
...
static void caja_file_management_properties_dialog_update_media_sensitivity(
  ...
  gtk_widget_set_sensitive(
      GTK_WIDGET(gtk_builder_get_object(builder, "media_handling_vbox")),
      !g_settings_get_boolean(caja_media_preferences,
                              CAJA_PREFERENCES_MEDIA_AUTORUN_NEVER));
}

static void other_type_combo_box_changed(GtkComboBox *combo_box,
                                         GtkComboBox *action_combo_box) {
  GtkTreeIter iter;
  GtkTreeModel *model;
  char *x_content_type;
  ...
  gtk_tree_model_get(model, &iter, 2, &x_content_type, -1);

  caja_autorun_prepare_combo_box(GTK_WIDGET(action_combo_box), x_content_type,
...
static void extension_about_clicked(GtkButton *button, Extension *ext) {
  GtkAboutDialog *extension_about_dialog;

  extension_about_dialog = (GtkAboutDialog *)gtk_about_dialog_new();
  gtk_about_dialog_set_program_name(
      extension_about_dialog, ext->name != NULL ? ext->name : ext->filename);
  gtk_about_dialog_set_comments(extension_about_dialog, ext->description);
  gtk_about_dialog_set_logo_icon_name(
      extension_about_dialog, ext->icon != NULL ? ext->icon : "system-run");
  gtk_about_dialog_set_copyright(extension_about_dialog, ext->copyright);
  gtk_about_dialog_set_authors(extension_about_dialog,
                               (const gchar **)ext->author);
  ...
  gtk_window_set_title(GTK_WINDOW(extension_about_dialog),
                       _("About Extension"));
  gtk_window_set_icon_name(GTK_WINDOW(extension_about_dialog),
                           ext->icon != NULL ? ext->icon : "system-run");
...
static void extension_configure_clicked(GtkButton *button, Extension *ext) {
  if (extension_configure_check(ext)) {
    caja_configurable_run_config(CAJA_CONFIGURABLE(ext->module));
  }
}

static void extension_list_selection_changed_about(GtkTreeSelection *selection,
  ...
  if (ext != NULL) {
    ...
    extension_about_id = g_signal_connect(
        about_button, "clicked", G_CALLBACK(extension_about_clicked), ext);
  }
}

static void extension_list_selection_changed_configure(
    GtkTreeSelection *selection, GtkButton *configure_button) {
  GtkTreeModel *model;
  GtkTreeIter iter;
  Extension *ext;
  ...
  if (ext != NULL) {
    ...
    if (extension_configure_check(ext)) {
      ...
      extension_configure_id =
          g_signal_connect(configure_button, "clicked",
                           G_CALLBACK(extension_configure_clicked), ext);
    }
  }
}

static void extension_state_toggled(GtkCellRendererToggle *cell,
  ...
  if (gtk_tree_model_get_iter_from_string(model, &iter, path_str)) {
    ...
    if (caja_extension_set_state(ext, new_state)) {
      gtk_list_store_set(GTK_LIST_STORE(model), &iter, EXT_STATE_COLUMN,
                         new_state, -1);
    }
  }
...
static void caja_file_management_properties_dialog_setup_media_page(
    GtkBuilder *builder) {
  unsigned int n;
  GList *l;
  GList *content_types;
  ...
  GtkTreeIter iter;
  const char *s[] = {"media_audio_cdda_combobox",
  ...
  for (n = 0; s[n * 2] != NULL; n++) {
    caja_autorun_prepare_combo_box(
        GTK_WIDGET(gtk_builder_get_object(builder, s[n * 2])), s[n * 2 + 1],
        TRUE, TRUE, TRUE, NULL, NULL);
  }

  other_type_combo_box =
      GTK_WIDGET(gtk_builder_get_object(builder, "media_other_type_combobox"));

  other_type_list_store = gtk_list_store_new(3, CAIRO_GOBJECT_TYPE_SURFACE,
  ...
  for (l = content_types; l != NULL; l = l->next) {
    ...
    if (!g_str_has_prefix(content_type, "x-content/")) continue;
    for (n = 0; s[n * 2] != NULL; n++) {
    ...
    if (icon != NULL) {
      ...
      g_object_unref(icon);
      surface =
          caja_icon_info_get_surface_nodefault_at_size(icon_info, icon_size);
      g_object_unref(icon_info);
    } else {
      surface = NULL;
    }

    gtk_list_store_set(other_type_list_store, &iter, 0, surface, 1, description,
                       2, content_type, -1);
  ...
  gtk_cell_layout_pack_start(GTK_CELL_LAYOUT(other_type_combo_box), renderer,
                             FALSE);
  gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(other_type_combo_box),
                                 renderer, "surface", 0, NULL);
  ...
  g_signal_connect(
      other_type_combo_box, "changed", G_CALLBACK(other_type_combo_box_changed),
...
static void caja_file_management_properties_dialog_setup_extension_page(
    GtkBuilder *builder) {
  ...
  view = GTK_TREE_VIEW(gtk_builder_get_object(builder, "extension_view"));
  store = GTK_LIST_STORE(gtk_builder_get_object(builder, "extension_store"));

  toggle = GTK_CELL_RENDERER_TOGGLE(
      gtk_builder_get_object(builder, "extension_toggle"));
  g_object_set(toggle, "xpad", 6, NULL);

  g_signal_connect(toggle, "toggled", G_CALLBACK(extension_state_toggled),
                   view);
  ...
  for (i = 0; i < g_list_length(extensions); i++) {
    ...
    if (ext->icon != NULL) {
      ext_surface_icon = gtk_icon_theme_load_surface(
          icon_theme, ext->icon, 24,
          gtk_widget_get_scale_factor(GTK_WIDGET(view)), NULL,
          GTK_ICON_LOOKUP_USE_BUILTIN, NULL);
    } else {
      ext_surface_icon = gtk_icon_theme_load_surface(
          icon_theme, "system-run", 24,
          gtk_widget_get_scale_factor(GTK_WIDGET(view)), NULL,
          GTK_ICON_LOOKUP_USE_BUILTIN, NULL);
    }

    if (ext->description != NULL) {
      ext_text_info = g_markup_printf_escaped(
          "<b>%s</b>\n%s", ext->name ? ext->name : ext->filename,
          ext->description);
    } else {
      ext_text_info = g_markup_printf_escaped(
          "<b>%s</b>", ext->name ? ext->name : ext->filename);
    }

    gtk_list_store_append(store, &iter);
    gtk_list_store_set(store, &iter, EXT_STATE_COLUMN, ext->state,
                       EXT_ICON_COLUMN, ext_surface_icon, EXT_INFO_COLUMN,
                       ext_text_info, EXT_STRUCT_COLUMN, ext, -1);

    g_free(ext_text_info);
    if (ext_surface_icon) cairo_surface_destroy(ext_surface_icon);
  }

  about_button =
      GTK_BUTTON(gtk_builder_get_object(builder, "about_extension_button"));
  configure_button =
      GTK_BUTTON(gtk_builder_get_object(builder, "configure_extension_button"));
...
static void bind_builder_bool_inverted(GtkBuilder *builder, GSettings *settings,
...
static void caja_file_management_properties_dialog_setup(GtkBuilder *builder,
                                                         GtkWindow *window) {
  GtkWidget *dialog;
  ...
  bind_builder_bool_inverted(
      builder, caja_preferences,
  ...
  bind_builder_radio(
      builder, caja_preferences, (const char **)click_behavior_components,
      CAJA_PREFERENCES_CLICK_POLICY, (const char **)click_behavior_values);
  ...
  g_signal_connect_swapped(
      caja_media_preferences, "changed::" CAJA_PREFERENCES_MEDIA_AUTORUN_NEVER,
      G_CALLBACK(
          caja_file_management_properties_dialog_update_media_sensitivity),
      builder);
...
static gboolean delete_event_callback(GtkWidget *widget, GdkEventAny *event,
                                      gpointer data) {
  void (*response_callback)(GtkDialog * dialog, gint response_id);
...
void caja_file_management_properties_dialog_show(GCallback close_callback,
  ...
  builder = gtk_builder_new_from_resource(
      "/org/mate/caja/caja-file-management-properties.ui");

  g_signal_connect(gtk_builder_get_object(builder, "file_management_dialog"),
                   "response", close_callback, NULL);
  g_signal_connect(gtk_builder_get_object(builder, "file_management_dialog"),
                   "delete_event", G_CALLBACK(delete_event_callback),
                   close_callback);
