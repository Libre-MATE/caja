...
enum { PROP_WINDOW = 1, NUM_PROPERTIES };
...
static void restore_button_clicked_cb(GtkWidget *button, CajaTrashBar *bar) {
  GList *locations, *files, *l;

  locations =
      caja_window_info_get_selection(CAJA_WINDOW_INFO(bar->priv->window));
  files = NULL;
  ...
  caja_restore_files_from_trash(files,
...
static void selection_changed_cb(CajaWindow *window, CajaTrashBar *bar) {
  int count;
...
static void connect_window_and_update_button(CajaTrashBar *bar) {
...
static void caja_trash_bar_set_property(GObject *object, guint prop_id,
                                        const GValue *value,
                                        GParamSpec *pspec) {
  CajaTrashBar *bar;
...
static void caja_trash_bar_finalize(GObject *obj) {
  CajaTrashBar *bar;

  bar = CAJA_TRASH_BAR(obj);

  if (bar->priv->selection_handler_id) {
    g_signal_handler_disconnect(bar->priv->window,
                                bar->priv->selection_handler_id);
  }
...
static void caja_trash_bar_trash_state_changed(CajaTrashMonitor *trash_monitor,
...
static void caja_trash_bar_class_init(CajaTrashBarClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object(
          "window", "window", "the CajaWindow", CAJA_TYPE_WINDOW,
          G_PARAM_WRITABLE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS));
}

static void empty_trash_callback(GtkWidget *button, gpointer data) {
  GtkWidget *window;
...
static void caja_trash_bar_init(CajaTrashBar *bar) {
  GtkWidget *label;
  GtkWidget *hbox;
  ...
  g_signal_connect(bar->priv->empty_button, "clicked",
                   G_CALLBACK(empty_trash_callback), bar);

  bar->priv->restore_button =
      gtk_button_new_with_mnemonic(_("Restore Selected Items"));
  ...
  g_signal_connect(bar->priv->restore_button, "clicked",
                   G_CALLBACK(restore_button_clicked_cb), bar);

  g_signal_connect_object(caja_trash_monitor_get(), "trash_state_changed",
                          G_CALLBACK(caja_trash_bar_trash_state_changed), bar,
                          0);
}
