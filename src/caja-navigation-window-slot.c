...
G_DEFINE_TYPE(CajaNavigationWindowSlot, caja_navigation_window_slot,
              CAJA_TYPE_WINDOW_SLOT)

#define parent_class caja_navigation_window_slot_parent_class

gboolean caja_navigation_window_slot_should_close_with_mount(
    CajaNavigationWindowSlot *slot, GMount *mount) {
  ...
  close_with_mount &=
      g_file_has_prefix(CAJA_WINDOW_SLOT(slot)->location, mount_location) ||
      g_file_equal(CAJA_WINDOW_SLOT(slot)->location, mount_location);

  /* we could also consider the forward list here, but since the “go home”
   * request in caja-window-manager-views.c:mount_removed_callback() would
   * discard those anyway, we don't consider them.
   */
...
void caja_navigation_window_slot_clear_forward_list(
...
void caja_navigation_window_slot_clear_back_list(
    CajaNavigationWindowSlot *slot) {
  g_assert(CAJA_IS_NAVIGATION_WINDOW_SLOT(slot));
...
static void caja_navigation_window_slot_update_query_editor(
    CajaWindowSlot *slot) {
  CajaDirectory *directory;
  ...
  if (CAJA_IS_SEARCH_DIRECTORY(directory)) {
    ...
    if (caja_search_directory_is_saved_search(search_directory)) {
      query_editor = caja_query_editor_new(
          TRUE, caja_search_directory_is_indexed(search_directory));
    } else {
      query_editor = caja_query_editor_new_with_bar(
          FALSE, caja_search_directory_is_indexed(search_directory),
...
static void caja_navigation_window_slot_active(CajaWindowSlot *slot) {
  ...
  page_num =
      gtk_notebook_page_num(GTK_NOTEBOOK(pane->notebook), slot->content_box);
  g_assert(page_num >= 0);
...
static void caja_navigation_window_slot_dispose(GObject *object) {
  CajaNavigationWindowSlot *slot;
...
static void caja_navigation_window_slot_class_init(
    CajaNavigationWindowSlotClass *class) {
  CAJA_WINDOW_SLOT_CLASS(class)->active = caja_navigation_window_slot_active;
  CAJA_WINDOW_SLOT_CLASS(class)->update_query_editor =
      caja_navigation_window_slot_update_query_editor;
