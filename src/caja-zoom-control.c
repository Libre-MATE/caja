...
static const char *const caja_zoom_control_accessible_action_descriptions[] = {
    N_("Increase the view size"), N_("Decrease the view size"),
...
#define NUM_ACTIONS \
  ((int)G_N_ELEMENTS(caja_zoom_control_accessible_action_names))

G_DEFINE_TYPE_WITH_PRIVATE(CajaZoomControl, caja_zoom_control, GTK_TYPE_BOX);

static void caja_zoom_control_finalize(GObject *object) {
...
static void zoom_button_clicked(GtkButton *button,
                                CajaZoomControl *zoom_control) {
  g_signal_emit(zoom_control, signals[ZOOM_TO_DEFAULT], 0);
}

static void zoom_popup_menu_show(GtkWidget *widget, GdkEventButton *event,
                                 CajaZoomControl *zoom_control) {
  GtkMenu *menu;
...
static void zoom_popup_menu(GtkWidget *widget, CajaZoomControl *zoom_control) {
  GtkMenu *menu;
...
static gboolean caja_zoom_control_button_press_event(
    GtkWidget *widget, GdkEventButton *event, CajaZoomControl *zoom_control) {
...
static void zoom_out_clicked(GtkButton *button, CajaZoomControl *zoom_control) {
  if (caja_zoom_control_can_zoom_out(zoom_control)) {
    g_signal_emit(G_OBJECT(zoom_control), signals[ZOOM_OUT], 0);
  }
}

static void zoom_in_clicked(GtkButton *button, CajaZoomControl *zoom_control) {
  if (caja_zoom_control_can_zoom_in(zoom_control)) {
    g_signal_emit(G_OBJECT(zoom_control), signals[ZOOM_IN], 0);
  }
}

static void set_label_size(CajaZoomControl *zoom_control) {
  const char *text;
  PangoLayout *layout;
  int width;
  int height;
  ...
  gtk_label_set_text(GTK_LABEL(zoom_control->details->zoom_label), text);
}

static void label_style_set_callback(GtkWidget *label, GtkStyleContext *style,
                                     gpointer user_data) {
  set_label_size(CAJA_ZOOM_CONTROL(user_data));
}

static void caja_zoom_control_init(CajaZoomControl *zoom_control) {
  GtkWidget *image;
  int i;
  ...
  for (i = CAJA_ZOOM_LEVEL_LARGEST; i >= CAJA_ZOOM_LEVEL_SMALLEST; i--) {
    zoom_control->details->preferred_zoom_levels = g_list_prepend(
        zoom_control->details->preferred_zoom_levels, GINT_TO_POINTER(i));
  }
  ...
  gtk_box_pack_start(GTK_BOX(zoom_control), zoom_control->details->zoom_out,
                     FALSE, FALSE, 0);
  ...
  g_signal_connect(zoom_control->details->zoom_label, "style_set",
                   G_CALLBACK(label_style_set_callback), zoom_control);
  set_label_size(zoom_control);

  gtk_container_add(GTK_CONTAINER(zoom_control->details->zoom_button),
                    zoom_control->details->zoom_label);

  gtk_box_pack_start(GTK_BOX(zoom_control), zoom_control->details->zoom_button,
                     TRUE, TRUE, 0);
  ...
  gtk_box_pack_start(GTK_BOX(zoom_control), zoom_control->details->zoom_in,
                     FALSE, FALSE, 0);
...
static void caja_zoom_control_redraw(CajaZoomControl *zoom_control) {
  int percent;
  char *num_str;
  ...
  percent = floor((100.0 * caja_get_relative_icon_size_for_zoom_level(
...
static void zoom_menu_callback(GtkMenuItem *item, gpointer callback_data) {
  CajaZoomLevel zoom_level;
  CajaZoomControl *zoom_control;
  gboolean can_zoom;
  ...
  zoom_level = (CajaZoomLevel)GPOINTER_TO_INT(
      g_object_get_data(G_OBJECT(item), "zoom_level"));
...
static GtkRadioMenuItem *create_zoom_menu_item(
    CajaZoomControl *zoom_control, GtkMenu *menu, CajaZoomLevel zoom_level,
  ...
  percent = floor(
      (100.0 * caja_get_relative_icon_size_for_zoom_level(zoom_level)) + .5);
  ...
  g_object_set_data(G_OBJECT(menu_item), "zoom_level",
                    GINT_TO_POINTER(zoom_level));
  g_signal_connect_object(menu_item, "activate", G_CALLBACK(zoom_menu_callback),
                          zoom_control, 0);
...
static GtkMenu *create_zoom_menu(CajaZoomControl *zoom_control) {
  ...
  previous_item = NULL;
  for (node = zoom_control->details->preferred_zoom_levels; node != NULL;
       node = node->next) {
...
static void caja_zoom_control_change_value(CajaZoomControl *zoom_control,
  ...
  switch (scroll) {
    ...
    case GTK_SCROLL_STEP_UP:
      ...
      break;
    default:
      g_warning("Invalid scroll type %d for CajaZoomControl:change_value",
                scroll);
  }
}
...
CajaZoomLevel caja_zoom_control_get_min_zoom_level(
    CajaZoomControl *zoom_control) {
  return zoom_control->details->min_zoom_level;
}

CajaZoomLevel caja_zoom_control_get_max_zoom_level(
    CajaZoomControl *zoom_control) {
...
static gboolean caja_zoom_control_scroll_event(GtkWidget *widget,
...
static void caja_zoom_control_class_init(CajaZoomControlClass *class) {
  ...
  signals[ZOOM_IN] =
      g_signal_new("zoom_in", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(CajaZoomControlClass, zoom_in), NULL, NULL,
                   g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  signals[ZOOM_OUT] =
      g_signal_new("zoom_out", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(CajaZoomControlClass, zoom_out), NULL, NULL,
                   g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  signals[ZOOM_TO_LEVEL] = g_signal_new(
      "zoom_to_level", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaZoomControlClass, zoom_to_level), NULL, NULL,
      g_cclosure_marshal_VOID__INT, G_TYPE_NONE, 1, G_TYPE_INT);

  signals[ZOOM_TO_DEFAULT] =
      g_signal_new("zoom_to_default", G_TYPE_FROM_CLASS(class),
                   G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                   G_STRUCT_OFFSET(CajaZoomControlClass, zoom_to_default), NULL,
                   NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  signals[CHANGE_VALUE] = g_signal_new(
      "change_value", G_TYPE_FROM_CLASS(class),
      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
      G_STRUCT_OFFSET(CajaZoomControlClass, change_value), NULL, NULL,
  ...
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_KP_Subtract, 0,
                               "change_value", 1, GTK_TYPE_SCROLL_TYPE,
                               GTK_SCROLL_STEP_DOWN);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_minus, 0, "change_value", 1,
                               GTK_TYPE_SCROLL_TYPE, GTK_SCROLL_STEP_DOWN);

  gtk_binding_entry_add_signal(binding_set, GDK_KEY_KP_Equal, 0,
                               "zoom_to_default", 0);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_KP_Equal, 0,
                               "zoom_to_default", 0);

  gtk_binding_entry_add_signal(binding_set, GDK_KEY_KP_Add, 0, "change_value",
                               1, GTK_TYPE_SCROLL_TYPE, GTK_SCROLL_STEP_UP);
  gtk_binding_entry_add_signal(binding_set, GDK_KEY_plus, 0, "change_value", 1,
                               GTK_TYPE_SCROLL_TYPE, GTK_SCROLL_STEP_UP);
}

static gboolean caja_zoom_control_accessible_do_action(AtkAction *accessible,
                                                       int i) {
  GtkWidget *widget;
  ...
  g_signal_emit(widget, signals[caja_zoom_control_accessible_action_signals[i]],
                0);

  return TRUE;
}

static int caja_zoom_control_accessible_get_n_actions(AtkAction *accessible) {
  return NUM_ACTIONS;
}

static const char *caja_zoom_control_accessible_action_get_description(
...
static const char *caja_zoom_control_accessible_action_get_name(
    AtkAction *accessible, int i) {
  g_assert(i >= 0 && i < NUM_ACTIONS);
...
static void caja_zoom_control_accessible_action_interface_init(
...
static void caja_zoom_control_accessible_get_current_value(AtkValue *accessible,
                                                           GValue *value) {
  ...
  control =
      CAJA_ZOOM_CONTROL(gtk_accessible_get_widget(GTK_ACCESSIBLE(accessible)));
  if (!control) {
...
static void caja_zoom_control_accessible_get_maximum_value(AtkValue *accessible,
                                                           GValue *value) {
  CajaZoomControl *control;
...
static void caja_zoom_control_accessible_get_minimum_value(AtkValue *accessible,
                                                           GValue *value) {
  CajaZoomControl *control;
...
static CajaZoomLevel nearest_preferred(CajaZoomControl *zoom_control,
                                       CajaZoomLevel value) {
  ...
  last_value =
      GPOINTER_TO_INT(zoom_control->details->preferred_zoom_levels->data);
...
static gboolean caja_zoom_control_accessible_set_current_value(
    AtkValue *accessible, const GValue *value) {
  CajaZoomControl *control;
  CajaZoomLevel zoom;
...
static void caja_zoom_control_accessible_value_interface_init(
    AtkValueIface *iface) {
...
static const char *caja_zoom_control_accessible_get_name(
    AtkObject *accessible) {
  return _("Zoom");
}

static const char *caja_zoom_control_accessible_get_description(
    AtkObject *accessible) {
  return _("Set the zoom level of the current view");
}

static void caja_zoom_control_accessible_initialize(AtkObject *accessible,
                                                    gpointer data) {
...
G_DEFINE_TYPE_WITH_CODE(
    CajaZoomControlAccessible, caja_zoom_control_accessible,
    GTK_TYPE_CONTAINER_ACCESSIBLE,
    G_IMPLEMENT_INTERFACE(ATK_TYPE_ACTION,
                          caja_zoom_control_accessible_action_interface_init)
        G_IMPLEMENT_INTERFACE(
            ATK_TYPE_VALUE, caja_zoom_control_accessible_value_interface_init));
static void caja_zoom_control_accessible_class_init(
    CajaZoomControlAccessibleClass *klass) {
...
static void caja_zoom_control_accessible_init(
    CajaZoomControlAccessible *accessible) {}

void caja_zoom_control_set_active_appearance(CajaZoomControl *zoom_control,
                                             gboolean is_active) {
  gtk_widget_set_sensitive(
      gtk_bin_get_child(GTK_BIN(zoom_control->details->zoom_in)), is_active);
  gtk_widget_set_sensitive(
      gtk_bin_get_child(GTK_BIN(zoom_control->details->zoom_out)), is_active);
