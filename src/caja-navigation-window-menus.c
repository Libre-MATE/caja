...
#define MENU_PATH_HISTORY_PLACEHOLDER \
  "/MenuBar/Other Menus/Go/History Placeholder"
...
static void action_close_all_windows_callback(GtkAction *action,
                                              gpointer user_data) {
  CajaApplication *app;
...
static void action_back_callback(GtkAction *action, gpointer user_data) {
  caja_navigation_window_back_or_forward(CAJA_NAVIGATION_WINDOW(user_data),
                                         TRUE, 0, should_open_in_new_tab());
}

static void action_forward_callback(GtkAction *action, gpointer user_data) {
  caja_navigation_window_back_or_forward(CAJA_NAVIGATION_WINDOW(user_data),
                                         FALSE, 0, should_open_in_new_tab());
}

static void forget_history_if_yes(GtkDialog *dialog, int response,
...
static void forget_history_if_confirmed(CajaWindow *window) {
  GtkDialog *dialog;

  dialog = eel_create_question_dialog(
      _("Are you sure you want to clear the list "
        "of locations you have visited?"),
      NULL, "process-stop", GTK_RESPONSE_CANCEL, "edit-clear", RESPONSE_FORGET,
      GTK_WINDOW(window));

  gtk_widget_show(GTK_WIDGET(dialog));

  g_signal_connect(dialog, "response", G_CALLBACK(forget_history_if_yes), NULL);

  gtk_dialog_set_default_response(dialog, GTK_RESPONSE_CANCEL);
}

static void action_clear_history_callback(GtkAction *action,
                                          gpointer user_data) {
  forget_history_if_confirmed(CAJA_WINDOW(user_data));
}

static void action_split_view_switch_next_pane_callback(GtkAction *action,
                                                        gpointer user_data) {
  caja_window_pane_switch_to(caja_window_get_next_pane(CAJA_WINDOW(user_data)));
}

static void action_split_view_same_location_callback(GtkAction *action,
                                                     gpointer user_data) {
  CajaWindow *window;
...
static void action_show_hide_toolbar_callback(GtkAction *action,
                                              gpointer user_data) {
...
static void action_show_hide_sidebar_callback(GtkAction *action,
                                              gpointer user_data) {
  CajaNavigationWindow *window;
...
static void pane_show_hide_location_bar(CajaNavigationWindowPane *pane,
                                        gboolean is_active) {
...
static void action_show_hide_location_bar_callback(GtkAction *action,
                                                   gpointer user_data) {
  CajaWindow *window;
  GList *walk;
  gboolean is_active;
  ...
  /* Do the active pane first, because this will trigger an update of the menu
   * items, which in turn relies on the active pane. */
  pane_show_hide_location_bar(
      CAJA_NAVIGATION_WINDOW_PANE(window->details->active_pane), is_active);

  for (walk = window->details->panes; walk; walk = walk->next) {
    pane_show_hide_location_bar(CAJA_NAVIGATION_WINDOW_PANE(walk->data),
                                is_active);
  }
}

static void action_show_hide_statusbar_callback(GtkAction *action,
                                                gpointer user_data) {
  CajaNavigationWindow *window;
...
static void action_split_view_callback(GtkAction *action, gpointer user_data) {
  CajaNavigationWindow *window;
  gboolean is_active;
  ...
  if (is_active != caja_navigation_window_split_view_showing(window)) {
    ...
    if (caja_window->details->active_pane &&
        caja_window->details->active_pane->active_slot) {
      caja_view_update_menus(
          caja_window->details->active_pane->active_slot->content_view);
    }
  }
}

void caja_navigation_window_update_show_hide_menu_items(
    CajaNavigationWindow *window) {
  GtkAction *action;
  ...
  gtk_toggle_action_set_active(
      ...
      caja_navigation_window_pane_location_bar_showing(
          CAJA_NAVIGATION_WINDOW_PANE(
              CAJA_WINDOW(window)->details->active_pane)));
...
void caja_navigation_window_update_spatial_menu_item(
    CajaNavigationWindow *window) {
  GtkAction *action;
  ...
  gtk_action_set_visible(
      action, !g_settings_get_boolean(caja_preferences,
                                      CAJA_PREFERENCES_ALWAYS_USE_BROWSER));
  G_GNUC_END_IGNORE_DEPRECATIONS;
}

static void action_add_bookmark_callback(GtkAction *action,
                                         gpointer user_data) {
  caja_window_add_bookmark_for_current_location(CAJA_WINDOW(user_data));
}

static void action_edit_bookmarks_callback(GtkAction *action,
                                           gpointer user_data) {
...
void caja_navigation_window_remove_go_menu_items(CajaNavigationWindow *window) {
  ...
  if (window->details->go_menu_merge_id != 0) {
    gtk_ui_manager_remove_ui(ui_manager, window->details->go_menu_merge_id);
...
static void show_bogus_history_window(CajaWindow *window,
  ...
  detail = g_strdup_printf(_("The location \"%s\" does not exist."),
                           uri_for_display);

  eel_show_warning_dialog(_("The history location doesn't exist."), detail,
                          GTK_WINDOW(window));
 ...
 * refresh_go_menu:
 *
 * Refresh list of bookmarks at end of Go menu to match centralized history
 *list.
 * @window: The CajaWindow whose Go menu will be refreshed.
 **/
static void refresh_go_menu(CajaNavigationWindow *window) {
  GtkUIManager *ui_manager;
  GList *node;
  int index;
  ...
  gtk_ui_manager_insert_action_group(ui_manager,
                                     window->details->go_menu_action_group, -1);
  g_object_unref(window->details->go_menu_action_group);

  /* Add in a new set of history items. */
  for (node = caja_get_history_list(), index = 0; node != NULL && index < 10;
       node = node->next, index++) {
    caja_menus_append_bookmark_to_menu(
        CAJA_WINDOW(window), CAJA_BOOKMARK(node->data),
        MENU_PATH_HISTORY_PLACEHOLDER, "history", index,
        window->details->go_menu_action_group,
        window->details->go_menu_merge_id, G_CALLBACK(schedule_refresh_go_menu),
        show_bogus_history_window);
  }
}

static gboolean refresh_go_menu_idle_callback(gpointer data) {
...
static void schedule_refresh_go_menu(CajaNavigationWindow *window) {
  ...
  if (window->details->refresh_go_menu_idle_id == 0) {
    window->details->refresh_go_menu_idle_id =
        g_idle_add(refresh_go_menu_idle_callback, window);
  }
}
 ...
 */
static void caja_navigation_window_initialize_go_menu(
    CajaNavigationWindow *window) {
  ...
  g_signal_connect_object(caja_signaller_get_current(), "history_list_changed",
                          G_CALLBACK(schedule_refresh_go_menu), window,
                          G_CONNECT_SWAPPED);
}

void caja_navigation_window_update_split_view_actions_sensitivity(
    CajaNavigationWindow *window) {
  CajaWindow *win;
  ...
  if (win->details->active_pane->active_slot) {
    active_pane_location =
        caja_window_slot_get_location(win->details->active_pane->active_slot);
  } else {
  ...
  if (next_pane && next_pane->active_slot) {
    ...
    next_pane_is_in_same_location =
        (active_pane_location && next_pane_location &&
  ...
  gtk_action_set_sensitive(
      action, have_multiple_panes && !next_pane_is_in_same_location);
  G_GNUC_END_IGNORE_DEPRECATIONS;
...
static void action_new_window_callback(GtkAction *action, gpointer user_data) {
...
static void action_new_tab_callback(GtkAction *action, gpointer user_data) {
  CajaWindow *window;
...
static void action_folder_window_callback(GtkAction *action,
                                          gpointer user_data) {
  ...
  window = caja_application_get_spatial_window(
      current_window->application, current_window, NULL, current_location,
      gtk_window_get_screen(GTK_WINDOW(current_window)), NULL);
...
static void action_go_to_location_callback(GtkAction *action,
                                           gpointer user_data) {
  CajaWindow *window;
...
/* The ctrl-f Keyboard shortcut always enables, rather than toggles
   the search mode */
static void action_show_search_callback(GtkAction *action, gpointer user_data) {
  ...
  G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
  search_action = gtk_action_group_get_action(
      window->details->navigation_action_group, CAJA_ACTION_SEARCH);
  ...
  if (gtk_toggle_action_get_active(GTK_TOGGLE_ACTION(search_action))) {
    ...
    /* Otherwise, enable */
    gtk_toggle_action_set_active(GTK_TOGGLE_ACTION(search_action), TRUE);
  }
  G_GNUC_END_IGNORE_DEPRECATIONS;
}

static void action_show_hide_search_callback(GtkAction *action,
                                             gpointer user_data) {
  gboolean var_action;
  CajaNavigationWindow *window;
...
static void action_tabs_previous_callback(GtkAction *action,
                                          gpointer user_data) {
  CajaNavigationWindowPane *pane;

  pane =
      CAJA_NAVIGATION_WINDOW_PANE(CAJA_WINDOW(user_data)->details->active_pane);
  caja_notebook_set_current_page_relative(CAJA_NOTEBOOK(pane->notebook), -1);
}

static void action_tabs_next_callback(GtkAction *action, gpointer user_data) {
  CajaNavigationWindowPane *pane;
...
static void action_tabs_move_left_callback(GtkAction *action,
                                           gpointer user_data) {
  CajaNavigationWindowPane *pane;

  pane =
      CAJA_NAVIGATION_WINDOW_PANE(CAJA_WINDOW(user_data)->details->active_pane);
  caja_notebook_reorder_current_child_relative(CAJA_NOTEBOOK(pane->notebook),
                                               -1);
}

static void action_tabs_move_right_callback(GtkAction *action,
                                            gpointer user_data) {
  CajaNavigationWindowPane *pane;
...
static void action_tab_change_action_activate_callback(GtkAction *action,
                                                       gpointer user_data) {
  CajaWindow *window;
  ...
  if (window && window->details->active_pane) {
    ...
    notebook = GTK_NOTEBOOK(
        CAJA_NAVIGATION_WINDOW_PANE(window->details->active_pane)->notebook);
    if (notebook) {
      int num;
...
static const GtkActionEntry navigation_entries[] = {
    /* name, icon name, label */ {"Go", NULL, N_("_Go"), NULL, NULL, NULL},
    /* name, icon name, label */
    {"Bookmarks", NULL, N_("_Bookmarks"), NULL, NULL, NULL},
    /* name, icon name, label */ {"Tabs", NULL, N_("_Tabs"), NULL, NULL, NULL},
    /* name, icon name, label */
    {"New Window", "window-new", N_("New _Window"), "<control>N",
     N_("Open another Caja window for the displayed location"),
     G_CALLBACK(action_new_window_callback)},
    /* name, icon name, label */
    {"New Tab", "tab-new", N_("New _Tab"), "<control>T",
     N_("Open another tab for the displayed location"),
     G_CALLBACK(action_new_tab_callback)},
    /* name, icon name, label */
    {"Folder Window", "folder", N_("Open Folder W_indow"), NULL,
     N_("Open a folder window for the displayed location"),
     G_CALLBACK(action_folder_window_callback)},
    /* name, icon name, label */
    {"Close All Windows", NULL, N_("Close _All Windows"), "<control>Q",
     N_("Close all Navigation windows"),
     G_CALLBACK(action_close_all_windows_callback)},
    /* name, icon name, label */
    {"Go to Location", NULL, N_("_Location..."), "<control>L",
     N_("Specify a location to open"),
     G_CALLBACK(action_go_to_location_callback)},
    /* name, icon name, label */
    {"Clear History", NULL, N_("Clea_r History"), NULL,
     N_("Clear contents of Go menu and Back/Forward lists"),
     G_CALLBACK(action_clear_history_callback)},
    /* name, icon name, label */
    {"SplitViewNextPane", NULL, N_("S_witch to Other Pane"), "F6",
     N_("Move focus to the other pane in a split view window"),
     G_CALLBACK(action_split_view_switch_next_pane_callback)},
    /* name, icon name, label */
    {"SplitViewSameLocation", NULL, N_("Sa_me Location as Other Pane"), NULL,
     N_("Go to the same location as in the extra pane"),
     G_CALLBACK(action_split_view_same_location_callback)},
    /* name, icon name, label */
    {"Add Bookmark", "list-add", N_("_Add Bookmark"), "<control>d",
     N_("Add a bookmark for the current location to this menu"),
     G_CALLBACK(action_add_bookmark_callback)},
    /* name, icon name, label */
    {"Edit Bookmarks", NULL, N_("_Edit Bookmarks..."), "<control>b",
     N_("Display a window that allows editing the bookmarks in this menu"),
     G_CALLBACK(action_edit_bookmarks_callback)},
    {"TabsPrevious", NULL, N_("_Previous Tab"), "<control>Page_Up",
     N_("Activate previous tab"), G_CALLBACK(action_tabs_previous_callback)},
    {"TabsNext", NULL, N_("_Next Tab"), "<control>Page_Down",
     N_("Activate next tab"), G_CALLBACK(action_tabs_next_callback)},
...
void caja_navigation_window_initialize_actions(CajaNavigationWindow *window) {
  ...
  gtk_action_group_add_actions(action_group, navigation_entries,
                               G_N_ELEMENTS(navigation_entries), window);
  gtk_action_group_add_toggle_actions(action_group, navigation_toggle_entries,
                                      G_N_ELEMENTS(navigation_toggle_entries),
                                      window);

  action =
      g_object_new(CAJA_TYPE_NAVIGATION_ACTION, "name", "Back", "label",
                   _("_Back"), "icon-name", "go-previous", "tooltip",
                   _("Go to the previous visited location"), "arrow-tooltip",
                   _("Back history"), "window", window, "direction",
                   CAJA_NAVIGATION_DIRECTION_BACK, "is_important", TRUE, NULL);
  g_signal_connect(action, "activate", G_CALLBACK(action_back_callback),
                   window);
  gtk_action_group_add_action_with_accel(action_group, action, "<alt>Left");
  g_object_unref(action);

  action = g_object_new(
      CAJA_TYPE_NAVIGATION_ACTION, "name", "Forward", "label", _("_Forward"),
      "icon-name", "go-next", "tooltip", _("Go to the next visited location"),
      "arrow-tooltip", _("Forward history"), "window", window, "direction",
      CAJA_NAVIGATION_DIRECTION_FORWARD, "is_important", TRUE, NULL);
  g_signal_connect(action, "activate", G_CALLBACK(action_forward_callback),
                   window);
  gtk_action_group_add_action_with_accel(action_group, action, "<alt>Right");

  g_object_unref(action);

  action =
      g_object_new(CAJA_TYPE_ZOOM_ACTION, "name", "Zoom", "label", _("_Zoom"),
                   "window", window, "is_important", FALSE, NULL);
  gtk_action_group_add_action(action_group, action);
  g_object_unref(action);

  action = g_object_new(CAJA_TYPE_VIEW_AS_ACTION, "name", "ViewAs", "label",
                        _("_View As"), "window", window, "is_important", FALSE,
                        NULL);
  gtk_action_group_add_action(action_group, action);
  g_object_unref(action);
  G_GNUC_END_IGNORE_DEPRECATIONS;
  ...
  for (i = 0; i < 10; ++i) {
    ...
    g_signal_connect(action, "activate",
                     G_CALLBACK(action_tab_change_action_activate_callback),
                     window);
    ...
    gtk_ui_manager_add_ui(ui_manager, gtk_ui_manager_new_merge_id(ui_manager),
                          ...
                          GTK_UI_MANAGER_ACCELERATOR, FALSE);
  }
