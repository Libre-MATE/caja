...
static void autorun_software_dialog_mount_unmounted(
    GMount *mount, AutorunSoftwareDialogData *data);
...
static void autorun(GMount *mount) {
  ...
  if (path_to_spawn != NULL && cwd_for_program != NULL) {
    if (chdir(cwd_for_program) == 0) {
      ...
      error_string = g_strdup_printf(_("Error starting autorun program: %s"),
                                     strerror(errno));
      goto out;
    }
    error_string = g_strdup_printf(_("Error starting autorun program: %s"),
...
static void present_autorun_for_software_dialog(GMount *mount) {
  ...
  dialog = gtk_message_dialog_new_with_markup(
      ...
      _("<big><b>This medium contains software intended to be automatically "
  ...
  icon_info = caja_icon_info_lookup(
      icon, icon_size, gtk_widget_get_scale_factor(GTK_WIDGET(dialog)));
