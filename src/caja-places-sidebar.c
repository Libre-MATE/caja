...
enum { GTK_TREE_MODEL_ROW, TEXT_URI_LIST };
...
#define CAJA_SHORTCUTS_MODEL_FILTER_TYPE \
  (_caja_shortcuts_model_filter_get_type())

GType _caja_shortcuts_model_filter_get_type(void);
static void caja_shortcuts_model_filter_drag_source_iface_init(
    GtkTreeDragSourceIface *iface);

G_DEFINE_TYPE_WITH_CODE(
    CajaShortcutsModelFilter, _caja_shortcuts_model_filter,
...
G_DEFINE_TYPE_WITH_CODE(CajaPlacesSidebar, caja_places_sidebar,
                        GTK_TYPE_SCROLLED_WINDOW,
                        G_IMPLEMENT_INTERFACE(CAJA_TYPE_SIDEBAR,
                                              caja_places_sidebar_iface_init));

G_DEFINE_TYPE_WITH_CODE(CajaPlacesSidebarProvider, caja_places_sidebar_provider,
                        G_TYPE_OBJECT,
                        G_IMPLEMENT_INTERFACE(CAJA_TYPE_SIDEBAR_PROVIDER,
                                              sidebar_provider_iface_init));

static cairo_surface_t *get_eject_icon(gboolean highlighted) {
  GdkPixbuf *eject;
  ...
  eject_icon_info =
      caja_icon_info_lookup_from_name("media-eject", icon_size, icon_scale);
...
static gboolean is_built_in_bookmark(CajaFile *file) {
  gboolean built_in;
  gint idx;
  ...
  for (idx = 0; idx < G_USER_N_DIRECTORIES; idx++) {
    ...
    if (caja_file_is_user_special_directory(file, idx)) {
      if (idx != G_USER_DIRECTORY_PUBLIC_SHARE &&
          idx != G_USER_DIRECTORY_TEMPLATES) {
        built_in = TRUE;
      }
...
static GtkTreeIter add_heading(CajaPlacesSidebar *sidebar,
                               SectionType section_type, const gchar *title) {
  GtkTreeIter iter, child_iter;

  gtk_list_store_append(sidebar->store, &iter);
  gtk_list_store_set(sidebar->store, &iter, PLACES_SIDEBAR_COLUMN_ROW_TYPE,
                     PLACES_HEADING, PLACES_SIDEBAR_COLUMN_SECTION_TYPE,
                     section_type, PLACES_SIDEBAR_COLUMN_HEADING_TEXT, title,
                     PLACES_SIDEBAR_COLUMN_EJECT, FALSE,
                     PLACES_SIDEBAR_COLUMN_NO_EJECT, TRUE, -1);

  gtk_tree_model_filter_refilter(GTK_TREE_MODEL_FILTER(sidebar->filter_model));
  gtk_tree_model_filter_convert_child_iter_to_iter(
      GTK_TREE_MODEL_FILTER(sidebar->filter_model), &child_iter, &iter);

  return child_iter;
}

static void check_heading_for_section(CajaPlacesSidebar *sidebar,
...
static GtkTreeIter add_place(CajaPlacesSidebar *sidebar, PlaceType place_type,
                             SectionType section_type, const char *name,
                             GIcon *icon, const char *uri, GDrive *drive,
                             GVolume *volume, GMount *mount, const int index,
                             const char *tooltip) {
  GdkPixbuf *pixbuf;
  ...
  gtk_list_store_append(sidebar->store, &iter);
  gtk_list_store_set(
      sidebar->store, &iter, PLACES_SIDEBAR_COLUMN_ICON, surface,
      PLACES_SIDEBAR_COLUMN_NAME, name, PLACES_SIDEBAR_COLUMN_URI, uri,
      PLACES_SIDEBAR_COLUMN_DRIVE, drive, PLACES_SIDEBAR_COLUMN_VOLUME, volume,
      PLACES_SIDEBAR_COLUMN_MOUNT, mount, PLACES_SIDEBAR_COLUMN_ROW_TYPE,
      place_type, PLACES_SIDEBAR_COLUMN_INDEX, index,
      PLACES_SIDEBAR_COLUMN_EJECT, show_eject_button,
      PLACES_SIDEBAR_COLUMN_NO_EJECT, !show_eject_button,
      PLACES_SIDEBAR_COLUMN_BOOKMARK, place_type != PLACES_BOOKMARK,
      PLACES_SIDEBAR_COLUMN_TOOLTIP, tooltip, PLACES_SIDEBAR_COLUMN_EJECT_ICON,
      eject, PLACES_SIDEBAR_COLUMN_SECTION_TYPE, section_type, -1);
...
static void compare_for_selection(CajaPlacesSidebar *sidebar,
                                  const gchar *location, const gchar *added_uri,
                                  const gchar *last_uri, GtkTreeIter *iter,
                                  GtkTreePath **path) {
  int res;
  ...
  if (res == 0) {
    ...
    *path = gtk_tree_model_get_path(sidebar->filter_model, iter);
...
static void update_places(CajaPlacesSidebar *sidebar) {
  ...
  if (gtk_tree_selection_get_selected(selection, &model, &last_iter)) {
    gtk_tree_model_get(model, &last_iter, PLACES_SIDEBAR_COLUMN_URI, &last_uri,
                       -1);
  }
  ...
  last_iter =
      add_place(sidebar, PLACES_BUILT_IN, SECTION_COMPUTER, display_name, icon,
                mount_uri, NULL, NULL, NULL, 0, _("Open your personal folder"));
  g_object_unref(icon);
  g_free(display_name);
  compare_for_selection(sidebar, location, mount_uri, last_uri, &last_iter,
                        &select_path);
  g_free(mount_uri);
  ...
  if (strcmp(g_get_home_dir(), desktop_path) != 0) {
    ...
    last_iter = add_place(sidebar, PLACES_BUILT_IN, SECTION_COMPUTER,
                          _("Desktop"), icon, mount_uri, NULL, NULL, NULL, 0,
  ...
  last_iter = add_place(sidebar, PLACES_BUILT_IN, SECTION_COMPUTER,
                        _("File System"), icon, mount_uri, NULL, NULL, NULL, 0,
  ...
  for (index = 0; index < G_USER_N_DIRECTORIES; index++) {
    ...
    last_iter = add_place(sidebar, PLACES_BUILT_IN, SECTION_COMPUTER, name,
                          icon, mount_uri, NULL, NULL, NULL, 0, tooltip);
    compare_for_selection(sidebar, location, mount_uri, last_uri, &last_iter,
                          &select_path);
    g_free(name);
    g_object_unref(root);
  ...
  last_iter =
      add_place(sidebar, PLACES_BUILT_IN, SECTION_COMPUTER, _("Trash"), icon,
                mount_uri, NULL, NULL, NULL, 0, _("Open the trash"));
  compare_for_selection(sidebar, location, mount_uri, last_uri, &last_iter,
                        &select_path);
  g_object_unref(icon);
    ...
    if (volumes != NULL) {
      for (ll = volumes; ll != NULL; ll = ll->next) {
        ...
        if (mount != NULL) {
          ...
          last_iter =
              add_place(sidebar, PLACES_MOUNTED_VOLUME, SECTION_DEVICES, name,
                        icon, mount_uri, drive, volume, mount, 0, tooltip);
          compare_for_selection(sidebar, location, mount_uri, last_uri,
          ...
          last_iter =
              add_place(sidebar, PLACES_MOUNTED_VOLUME, SECTION_DEVICES, name,
                        icon, NULL, drive, volume, NULL, 0, tooltip);
          g_object_unref(icon);
          g_free(name);
          g_free(tooltip);
        }
      ...
      if (g_drive_is_media_removable(drive) &&
        ...
        last_iter = add_place(sidebar, PLACES_BUILT_IN, SECTION_DEVICES, name,
                              icon, NULL, drive, NULL, NULL, 0, tooltip);
        g_object_unref(icon);
        g_free(tooltip);
        g_free(name);
      }
  ...
  for (l = volumes; l != NULL; l = l->next) {
    ...
    if (mount != NULL) {
      ...
      name = g_mount_get_name(mount);
      last_iter =
          add_place(sidebar, PLACES_MOUNTED_VOLUME, SECTION_DEVICES, name, icon,
                    mount_uri, NULL, volume, mount, 0, tooltip);
      compare_for_selection(sidebar, location, mount_uri, last_uri, &last_iter,
                            &select_path);
      g_object_unref(mount);
      g_object_unref(icon);
      ...
      icon = g_volume_get_icon(volume);
      name = g_volume_get_name(volume);
      last_iter = add_place(sidebar, PLACES_MOUNTED_VOLUME, SECTION_DEVICES,
                            name, icon, NULL, NULL, volume, NULL, 0, name);
      g_object_unref(icon);
      g_free(name);
    }
    g_object_unref(volume);
  ...
  for (l = mounts; l != NULL; l = l->next) {
    ...
    name = g_mount_get_name(mount);
    tooltip = g_file_get_parse_name(root);
    last_iter = add_place(sidebar, PLACES_MOUNTED_VOLUME, SECTION_COMPUTER,
                          name, icon, mount_uri, NULL, NULL, mount, 0, tooltip);
    compare_for_selection(sidebar, location, mount_uri, last_uri, &last_iter,
                          &select_path);
    g_object_unref(root);
    g_object_unref(mount);
  ...
  for (index = 0; index < bookmark_count; ++index) {
    ...
    last_iter = add_place(sidebar, PLACES_BOOKMARK, SECTION_BOOKMARKS, name,
                          icon, mount_uri, NULL, NULL, NULL, index, tooltip);
    compare_for_selection(sidebar, location, mount_uri, last_uri, &last_iter,
                          &select_path);
    g_free(name);
    g_object_unref(root);
  ...
  for (l = network_mounts; l != NULL; l = l->next) {
    ...
    name = g_mount_get_name(mount);
    tooltip = g_file_get_parse_name(root);
    last_iter = add_place(sidebar, PLACES_MOUNTED_VOLUME, SECTION_NETWORK, name,
                          icon, mount_uri, NULL, NULL, mount, 0, tooltip);
    compare_for_selection(sidebar, location, mount_uri, last_uri, &last_iter,
                          &select_path);
    g_object_unref(root);
    g_object_unref(mount);
  ...
  last_iter = add_place(sidebar, PLACES_BUILT_IN, SECTION_NETWORK,
                        _("Browse Network"), icon, mount_uri, NULL, NULL, NULL,
                        0, _("Browse the contents of the network"));
  g_object_unref(icon);
  compare_for_selection(sidebar, location, mount_uri, last_uri, &last_iter,
                        &select_path);
...
static void mount_added_callback(GVolumeMonitor *volume_monitor, GMount *mount,
                                 CajaPlacesSidebar *sidebar) {
  update_places(sidebar);
}

static void mount_removed_callback(GVolumeMonitor *volume_monitor,
                                   GMount *mount, CajaPlacesSidebar *sidebar) {
  update_places(sidebar);
}

static void mount_changed_callback(GVolumeMonitor *volume_monitor,
                                   GMount *mount, CajaPlacesSidebar *sidebar) {
  update_places(sidebar);
}

static void volume_added_callback(GVolumeMonitor *volume_monitor,
                                  GVolume *volume, CajaPlacesSidebar *sidebar) {
  update_places(sidebar);
}

static void volume_removed_callback(GVolumeMonitor *volume_monitor,
                                    GVolume *volume,
                                    CajaPlacesSidebar *sidebar) {
  update_places(sidebar);
}

static void volume_changed_callback(GVolumeMonitor *volume_monitor,
                                    GVolume *volume,
                                    CajaPlacesSidebar *sidebar) {
  update_places(sidebar);
}

static void drive_disconnected_callback(GVolumeMonitor *volume_monitor,
                                        GDrive *drive,
                                        CajaPlacesSidebar *sidebar) {
  update_places(sidebar);
}

static void drive_connected_callback(GVolumeMonitor *volume_monitor,
                                     GDrive *drive,
                                     CajaPlacesSidebar *sidebar) {
  update_places(sidebar);
}

static void drive_changed_callback(GVolumeMonitor *volume_monitor,
                                   GDrive *drive, CajaPlacesSidebar *sidebar) {
  update_places(sidebar);
}

static gboolean over_eject_button(CajaPlacesSidebar *sidebar, gint x, gint y,
                                  GtkTreePath **path) {
  ...
  *path = NULL;
  ...
  if (gtk_tree_view_get_path_at_pos(sidebar->tree_view, x, y, path, &column,
                                    NULL, NULL)) {
    gtk_tree_model_get_iter(model, &iter, *path);
    gtk_tree_model_get(model, &iter, PLACES_SIDEBAR_COLUMN_EJECT, &show_eject,
                       -1);
    ...
    gtk_widget_style_get(GTK_WIDGET(sidebar->tree_view), "horizontal-separator",
                         &hseparator, NULL);
    ...
    gtk_tree_view_column_cell_get_position(
        column, sidebar->eject_icon_cell_renderer, &x_offset, &width);
...
static gboolean clicked_eject_button(CajaPlacesSidebar *sidebar,
                                     GtkTreePath **path) {
...
static void loading_uri_callback(CajaWindowInfo *window, char *location,
                                 CajaPlacesSidebar *sidebar) {
  GtkTreeIter iter;
  gboolean valid;
  char *uri;
  ...
  if (strcmp(sidebar->uri, location) != 0) {
    ...
    while (valid) {
      gtk_tree_model_get(sidebar->filter_model, &iter,
                         PLACES_SIDEBAR_COLUMN_URI, &uri, -1);
      if (uri != NULL) {
...
static gboolean compute_drop_position(GtkTreeView *tree_view, int x, int y,
                                      GtkTreePath **path,
  ...
  gtk_tree_model_get_iter(model, &iter, *path);
  gtk_tree_model_get(model, &iter, PLACES_SIDEBAR_COLUMN_ROW_TYPE, &place_type,
                     PLACES_SIDEBAR_COLUMN_SECTION_TYPE, &section_type, -1);
  ...
  if (section_type != SECTION_BOOKMARKS && sidebar->drag_data_received &&
  ...
  if (*pos != GTK_TREE_VIEW_DROP_BEFORE && sidebar->drag_data_received &&
      sidebar->drag_data_info == GTK_TREE_MODEL_ROW) {
...
static gboolean get_drag_data(GtkTreeView *tree_view, GdkDragContext *context,
                              unsigned int time) {
  GdkAtom target;
...
static void free_drag_data(CajaPlacesSidebar *sidebar) {
...
static gboolean can_accept_file_as_bookmark(CajaFile *file) {
  return (caja_file_is_directory(file) && !is_built_in_bookmark(file));
}

static gboolean can_accept_items_as_bookmarks(const GList *items) {
  int max;
  CajaFile *file = NULL;
...
static gboolean drag_motion_callback(GtkTreeView *tree_view,
  ...
  if (pos == GTK_TREE_VIEW_DROP_BEFORE || pos == GTK_TREE_VIEW_DROP_AFTER) {
    ...
    if (sidebar->drag_list == NULL) {
      ...
      gtk_tree_model_get_iter(sidebar->filter_model, &iter, path);
      gtk_tree_model_get(GTK_TREE_MODEL(sidebar->filter_model), &iter,
                         PLACES_SIDEBAR_COLUMN_URI, &uri, -1);
      caja_drag_default_drop_action_for_icons(context, uri, sidebar->drag_list,
                                              &action);
      g_free(uri);
    }
  }
...
static void drag_leave_callback(GtkTreeView *tree_view, GdkDragContext *context,
                                unsigned int time, CajaPlacesSidebar *sidebar) {
...
static void bookmarks_drop_uris(CajaPlacesSidebar *sidebar,
...
static GList *uri_list_from_selection(GList *selection) {
  GList *ret;
  GList *l;
...
static GList *build_selection_list(const char *data) {
  GList *result;
  char **uris;
  int i;
  CajaDragSelectionItem *item = NULL;
...
static gboolean get_selected_iter(CajaPlacesSidebar *sidebar,
                                  GtkTreeIter *iter) {
  GtkTreeSelection *selection;
...
static void reorder_bookmarks(CajaPlacesSidebar *sidebar, int new_position) {
  GtkTreeIter iter;
  PlaceType type;
  int old_position;
  guint list_length;
  ...
  if (!get_selected_iter(sidebar, &iter)) return;

  gtk_tree_model_get(GTK_TREE_MODEL(sidebar->filter_model), &iter,
                     PLACES_SIDEBAR_COLUMN_ROW_TYPE, &type,
                     PLACES_SIDEBAR_COLUMN_INDEX, &old_position, -1);

  if (type != PLACES_BOOKMARK || old_position < 0) return;

  list_length = caja_bookmark_list_length(sidebar->bookmarks);
  if (((guint)old_position) >= list_length) return;

  caja_bookmark_list_move_item(sidebar->bookmarks, old_position, new_position);
}
...
static void drag_data_received_callback(GtkWidget *widget,
  ...
  if (!sidebar->drag_data_received) {
    if (gtk_selection_data_get_target(selection_data) != GDK_NONE &&
        info == TEXT_URI_LIST) {
      sidebar->drag_list =
          build_selection_list(gtk_selection_data_get_data(selection_data));
    } else {
      sidebar->drag_list = NULL;
    }
  ...
  if (tree_pos == GTK_TREE_VIEW_DROP_BEFORE ||
    ...
    gtk_tree_model_get(model, &iter, PLACES_SIDEBAR_COLUMN_SECTION_TYPE,
                       &section_type, PLACES_SIDEBAR_COLUMN_ROW_TYPE,
                       &place_type, PLACES_SIDEBAR_COLUMN_INDEX, &position, -1);
    ...
    if (real_action == GDK_ACTION_ASK) {
      real_action = caja_drag_drop_action_ask(
          GTK_WIDGET(tree_view), gdk_drag_context_get_actions(context));
    }
    ...
    if (real_action > 0) {
      ...
      gtk_tree_model_get(model, &iter, PLACES_SIDEBAR_COLUMN_URI, &drop_uri,
                         -1);
      ...
      switch (info) {
        case TEXT_URI_LIST:
          ...
          caja_file_operations_copy_move(uris, NULL, drop_uri, real_action,
                                         GTK_WIDGET(tree_view), NULL, NULL);
...
static gboolean drag_drop_callback(GtkTreeView *tree_view,
                                   GdkDragContext *context, int x, int y,
                                   unsigned int time,
                                   CajaPlacesSidebar *sidebar) {
...
static void bookmarks_popup_menu_detach_cb(GtkWidget *attach_widget,
                                           GtkMenu *menu) {
  CajaPlacesSidebar *sidebar;
  ...
  *show_stop = FALSE;
  ...
  if (drive != NULL) {
    ...
    *show_stop = g_drive_can_stop(drive);
    ...
    if (*show_stop) *show_unmount = FALSE;
  ...
  if (volume != NULL) {
    if (mount == NULL) *show_mount = g_volume_can_mount(volume);
  }
}
...
static void bookmarks_check_popup_sensitivity(CajaPlacesSidebar *sidebar) {
  ...
  gtk_widget_set_sensitive(sidebar->popup_menu_remove_item,
                           (type == PLACES_BOOKMARK));
  gtk_widget_set_sensitive(sidebar->popup_menu_rename_item,
                           (type == PLACES_BOOKMARK));
  gtk_widget_set_sensitive(sidebar->popup_menu_empty_trash_item,
                           !caja_trash_monitor_is_empty());

  check_visibility(mount, volume, drive, &show_mount, &show_unmount,
                   &show_eject, &show_rescan, &show_format, &show_start,
                   &show_stop);

  /* We actually want both eject and unmount since eject will unmount all
   * volumes.
   * TODO: hide unmount if the drive only has a single mountable volume
   */

  show_empty_trash = (uri != NULL) && (!strcmp(uri, "trash:///"));

  gtk_widget_set_visible(sidebar->popup_menu_separator_item,
                         show_mount || show_unmount || show_eject ||
                             show_format || show_empty_trash);
  ...
  gtk_widget_set_visible(sidebar->popup_menu_empty_trash_item,
                         show_empty_trash);

  /* Adjust start/stop items to reflect the type of the drive */
  gtk_menu_item_set_label(GTK_MENU_ITEM(sidebar->popup_menu_start_item),
                          _("_Start"));
  gtk_menu_item_set_label(GTK_MENU_ITEM(sidebar->popup_menu_stop_item),
                          _("_Stop"));
  if ((show_start || show_stop) && drive != NULL) {
    switch (g_drive_get_start_stop_type(drive)) {
      case G_DRIVE_START_STOP_TYPE_SHUTDOWN:
        /* start() for type G_DRIVE_START_STOP_TYPE_SHUTDOWN is normally not
         * used */
        gtk_menu_item_set_label(GTK_MENU_ITEM(sidebar->popup_menu_start_item),
                                _("_Power On"));
        gtk_menu_item_set_label(GTK_MENU_ITEM(sidebar->popup_menu_stop_item),
                                _("_Safely Remove Drive"));
        break;
      ...
      case G_DRIVE_START_STOP_TYPE_MULTIDISK:
        gtk_menu_item_set_label(GTK_MENU_ITEM(sidebar->popup_menu_start_item),
                                _("_Start Multi-disk Device"));
        gtk_menu_item_set_label(GTK_MENU_ITEM(sidebar->popup_menu_stop_item),
                                _("_Stop Multi-disk Device"));
        break;
...
static void bookmarks_selection_changed_cb(GtkTreeSelection *selection,
                                           CajaPlacesSidebar *sidebar) {
  bookmarks_check_popup_sensitivity(sidebar);
}

static void volume_mounted_cb(GVolume *volume, GObject *user_data) {
  GMount *mount;
  CajaPlacesSidebar *sidebar;
  ...
  if (mount != NULL) {
    ...
    if (sidebar->go_to_after_mount_slot != NULL) {
      if ((sidebar->go_to_after_mount_flags &
           CAJA_WINDOW_OPEN_FLAG_NEW_WINDOW) == 0) {
        caja_window_slot_info_open_location(
            sidebar->go_to_after_mount_slot, location,
        ...
        new = caja_application_create_navigation_window(
            cur->application, gtk_window_get_screen(GTK_WINDOW(cur)));
...
static void open_selected_bookmark(CajaPlacesSidebar *sidebar,
  ...
  if (uri != NULL) {
    ...
    if ((flags & CAJA_WINDOW_OPEN_FLAG_NEW_WINDOW) == 0) {
      slot = caja_window_info_get_active_slot(sidebar->window);
      caja_window_slot_info_open_location(
          slot, location, CAJA_WINDOW_OPEN_ACCORDING_TO_MODE, flags, NULL);
    } else {
      CajaWindow *cur, *new;
    ...
    gtk_tree_model_get(model, &iter, PLACES_SIDEBAR_COLUMN_DRIVE, &drive,
                       PLACES_SIDEBAR_COLUMN_VOLUME, &volume, -1);
    ...
    if (volume != NULL && !sidebar->mounting) {
      ...
      caja_file_operations_mount_volume_full(
          NULL, volume, FALSE, volume_mounted_cb, G_OBJECT(sidebar));
...
static void open_shortcut_from_menu(CajaPlacesSidebar *sidebar,
                                    CajaWindowOpenFlags flags) {
  GtkTreeModel *model;
  GtkTreePath *path;
...
static void open_shortcut_cb(GtkMenuItem *item, CajaPlacesSidebar *sidebar) {
  open_shortcut_from_menu(sidebar, 0);
}

static void open_shortcut_in_new_window_cb(GtkMenuItem *item,
                                           CajaPlacesSidebar *sidebar) {
  open_shortcut_from_menu(sidebar, CAJA_WINDOW_OPEN_FLAG_NEW_WINDOW);
}

static void open_shortcut_in_new_tab_cb(GtkMenuItem *item,
                                        CajaPlacesSidebar *sidebar) {
  open_shortcut_from_menu(sidebar, CAJA_WINDOW_OPEN_FLAG_NEW_TAB);
}

/* Rename the selected bookmark */
static void rename_selected_bookmark(CajaPlacesSidebar *sidebar) {
  GtkTreeIter iter;
  ...
  if (get_selected_iter(sidebar, &iter)) {
    ...
    path =
        gtk_tree_model_get_path(GTK_TREE_MODEL(sidebar->filter_model), &iter);
    column = gtk_tree_view_get_column(GTK_TREE_VIEW(sidebar->tree_view), 0);
    renderers = gtk_cell_layout_get_cells(GTK_CELL_LAYOUT(column));
    cell = g_list_nth_data(renderers, 6);
    g_list_free(renderers);
    g_object_set(cell, "editable", TRUE, NULL);
    gtk_tree_view_set_cursor_on_cell(GTK_TREE_VIEW(sidebar->tree_view), path,
                                     column, cell, TRUE);
    gtk_tree_path_free(path);
  }
}

static void rename_shortcut_cb(GtkMenuItem *item, CajaPlacesSidebar *sidebar) {
  rename_selected_bookmark(sidebar);
}

/* Removes the selected bookmarks */
static void remove_selected_bookmarks(CajaPlacesSidebar *sidebar) {
  GtkTreeIter iter;
  PlaceType type;
  int index;
  ...
  gtk_tree_model_get(GTK_TREE_MODEL(sidebar->filter_model), &iter,
                     PLACES_SIDEBAR_COLUMN_ROW_TYPE, &type, -1);
  ...
  gtk_tree_model_get(GTK_TREE_MODEL(sidebar->filter_model), &iter,
                     PLACES_SIDEBAR_COLUMN_INDEX, &index, -1);

  caja_bookmark_list_delete_item_at(sidebar->bookmarks, index);
}

static void remove_shortcut_cb(GtkMenuItem *item, CajaPlacesSidebar *sidebar) {
  remove_selected_bookmarks(sidebar);
}

static void mount_shortcut_cb(GtkMenuItem *item, CajaPlacesSidebar *sidebar) {
  GtkTreeIter iter;
  GVolume *volume;
...
static void unmount_done(gpointer data) {
  CajaWindow *window;
...
static void do_unmount(GMount *mount, CajaPlacesSidebar *sidebar) {
  if (mount != NULL) {
    caja_window_info_set_initiated_unmount(sidebar->window, TRUE);
    caja_file_operations_unmount_mount_full(
        NULL, mount, FALSE, TRUE, unmount_done, g_object_ref(sidebar->window));
  }
}

static void do_unmount_selection(CajaPlacesSidebar *sidebar) {
  GtkTreeIter iter;
  GMount *mount;
  ...
  gtk_tree_model_get(GTK_TREE_MODEL(sidebar->filter_model), &iter,
                     PLACES_SIDEBAR_COLUMN_MOUNT, &mount, -1);
...
static void unmount_shortcut_cb(GtkMenuItem *item, CajaPlacesSidebar *sidebar) {
...
static void volume_eject_cb(GObject *source_object, GAsyncResult *res,
  ...
  if (!g_volume_eject_with_operation_finish(G_VOLUME(source_object), res,
    ...
    caja_application_notify_unmount_show(
        _("It is now safe to remove the drive"));
  }
}
...
static void do_eject(GMount *mount, GVolume *volume, GDrive *drive,
  ...
  caja_application_notify_unmount_show(
      _("Writing data to the drive -- do not unplug"));
  g_object_unref(mount_op);
}

static void eject_shortcut_cb(GtkMenuItem *item, CajaPlacesSidebar *sidebar) {
  GtkTreeIter iter;
  GMount *mount;
  GVolume *volume;
  GDrive *drive;
  ...
  gtk_tree_model_get(GTK_TREE_MODEL(sidebar->filter_model), &iter,
                     PLACES_SIDEBAR_COLUMN_MOUNT, &mount,
                     PLACES_SIDEBAR_COLUMN_VOLUME, &volume,
                     PLACES_SIDEBAR_COLUMN_DRIVE, &drive, -1);
...
static gboolean eject_or_unmount_bookmark(CajaPlacesSidebar *sidebar,
  ...
  gtk_tree_model_get(model, &iter, PLACES_SIDEBAR_COLUMN_MOUNT, &mount,
                     PLACES_SIDEBAR_COLUMN_VOLUME, &volume,
                     PLACES_SIDEBAR_COLUMN_DRIVE, &drive, -1);
...
static void rescan_shortcut_cb(GtkMenuItem *item, CajaPlacesSidebar *sidebar) {
  GtkTreeIter iter;
  GDrive *drive;
...
static void format_shortcut_cb(GtkMenuItem *item, CajaPlacesSidebar *sidebar) {
  GAppInfo *app_info;
  char *cmdline;
  ...
  cmdline = g_strjoin(" ", "gnome-disks", "--format-device", "--block-device",
...
static void start_shortcut_cb(GtkMenuItem *item, CajaPlacesSidebar *sidebar) {
  GtkTreeIter iter;
  GDrive *drive;
...
static void stop_shortcut_cb(GtkMenuItem *item, CajaPlacesSidebar *sidebar) {
  GtkTreeIter iter;
  GDrive *drive;
...
static void empty_trash_cb(GtkMenuItem *item, CajaPlacesSidebar *sidebar) {
  caja_file_operations_empty_trash(GTK_WIDGET(sidebar->window));
}

/* Handler for GtkWidget::key-press-event on the shortcuts list */
static gboolean bookmarks_key_press_event_cb(GtkWidget *widget,
                                             GdkEventKey *event,
                                             CajaPlacesSidebar *sidebar) {
  guint modifiers;
  GtkTreePath *path;
...
static void bookmarks_build_popup_menu(CajaPlacesSidebar *sidebar) {
  GtkWidget *item;
  ...
  gtk_menu_attach_to_widget(GTK_MENU(sidebar->popup_menu), GTK_WIDGET(sidebar),
                            bookmarks_popup_menu_detach_cb);

  item = eel_image_menu_item_new_from_icon("document-open", _("_Open"));

  g_signal_connect(item, "activate", G_CALLBACK(open_shortcut_cb), sidebar);
  ...
  g_signal_connect(item, "activate", G_CALLBACK(open_shortcut_in_new_tab_cb),
                   sidebar);
  gtk_widget_show(item);
  gtk_menu_shell_append(GTK_MENU_SHELL(sidebar->popup_menu), item);

  item = eel_image_menu_item_new_from_icon(NULL, _("Open in New _Window"));
  g_signal_connect(item, "activate", G_CALLBACK(open_shortcut_in_new_window_cb),
                   sidebar);
  gtk_widget_show(item);
  gtk_menu_shell_append(GTK_MENU_SHELL(sidebar->popup_menu), item);
  ...
  g_signal_connect(item, "activate", G_CALLBACK(remove_shortcut_cb), sidebar);
  gtk_widget_show(item);
  gtk_menu_shell_append(GTK_MENU_SHELL(sidebar->popup_menu), item);

  item = eel_image_menu_item_new_from_icon(NULL, _("Rename..."));
  sidebar->popup_menu_rename_item = item;
  g_signal_connect(item, "activate", G_CALLBACK(rename_shortcut_cb), sidebar);
  gtk_widget_show(item);
  gtk_menu_shell_append(GTK_MENU_SHELL(sidebar->popup_menu), item);
  ...
  g_signal_connect(item, "activate", G_CALLBACK(empty_trash_cb), sidebar);
  gtk_widget_show(item);
  gtk_menu_shell_append(GTK_MENU_SHELL(sidebar->popup_menu), item);
...
static void bookmarks_update_popup_menu(CajaPlacesSidebar *sidebar) {
  bookmarks_build_popup_menu(sidebar);
}

static void bookmarks_popup_menu(CajaPlacesSidebar *sidebar,
                                 GdkEventButton *event) {
  bookmarks_update_popup_menu(sidebar);
  eel_pop_up_context_menu(GTK_MENU(sidebar->popup_menu), event);
}

/* Callback used for the GtkWidget::popup-menu signal of the shortcuts list */
static gboolean bookmarks_popup_menu_cb(GtkWidget *widget,
                                        CajaPlacesSidebar *sidebar) {
  bookmarks_popup_menu(sidebar, NULL);
  return TRUE;
}

static gboolean bookmarks_button_release_event_cb(GtkWidget *widget,
                                                  GdkEventButton *event,
                                                  CajaPlacesSidebar *sidebar) {
  GtkTreePath *path;
  GtkTreeView *tree_view;
...
static void update_eject_buttons(CajaPlacesSidebar *sidebar,
                                 GtkTreePath *path) {
  GtkTreeIter iter;
  ...
  path_same = (path != NULL) && (sidebar->eject_highlight_path != NULL) &&
  ...
  if (path) {
    ...
    gtk_tree_model_get(GTK_TREE_MODEL(sidebar->filter_model), &iter,
                       PLACES_SIDEBAR_COLUMN_EJECT, &icon_visible, -1);
  }
  ...
  if (!icon_visible || path == NULL || !path_same) {
    ...
    if (sidebar->eject_highlight_path) {
      gtk_tree_model_get_iter(GTK_TREE_MODEL(sidebar->store), &iter,
                              sidebar->eject_highlight_path);

      gtk_list_store_set(sidebar->store, &iter,
                         PLACES_SIDEBAR_COLUMN_EJECT_ICON,
                         get_eject_icon(FALSE), -1);
  ...
  if (path != NULL) {
    ...
    gtk_list_store_set(sidebar->store, &iter, PLACES_SIDEBAR_COLUMN_EJECT_ICON,
                       get_eject_icon(TRUE), -1);
    gtk_tree_model_filter_refilter(
        GTK_TREE_MODEL_FILTER(sidebar->filter_model));
...
static gboolean bookmarks_motion_event_cb(GtkWidget *widget,
 ...
 * open in a new tab.
 */
static gboolean bookmarks_button_press_event_cb(GtkWidget *widget,
                                                GdkEventButton *event,
                                                CajaPlacesSidebar *sidebar) {
...
static void bookmarks_edited(GtkCellRenderer *cell, gchar *path_string,
                             gchar *new_text, CajaPlacesSidebar *sidebar) {
  GtkTreePath *path;
  GtkTreeIter iter;
  CajaBookmark *bookmark;
  int index;
...
static void bookmarks_editing_canceled(GtkCellRenderer *cell,
                                       CajaPlacesSidebar *sidebar) {
  g_object_set(cell, "editable", FALSE, NULL);
}

static void trash_state_changed_cb(CajaTrashMonitor *trash_monitor,
...
static gboolean tree_selection_func(GtkTreeSelection *selection,
  ...
  gtk_tree_model_get_iter(model, &iter, path);
  gtk_tree_model_get(model, &iter, PLACES_SIDEBAR_COLUMN_ROW_TYPE, &row_type,
                     -1);
...
static void icon_cell_renderer_func(GtkTreeViewColumn *column,
...
static void padding_cell_renderer_func(GtkTreeViewColumn *column,
                                       GtkCellRenderer *cell,
  ...
  if (type == PLACES_HEADING) {
    g_object_set(cell, "visible", FALSE, "xpad", 0, "ypad", 0, NULL);
  } else {
    g_object_set(cell, "visible", TRUE, "xpad", 3, "ypad", 0, NULL);
  }
}

static void heading_cell_renderer_func(GtkTreeViewColumn *column,
                                       GtkCellRenderer *cell,
                                       GtkTreeModel *model, GtkTreeIter *iter,
                                       gpointer user_data) {
  PlaceType type;
...
static void caja_places_sidebar_init(CajaPlacesSidebar *sidebar) {
  GtkTreeView *tree_view;
  ...
  gtk_scrolled_window_set_shadow_type(GTK_SCROLLED_WINDOW(sidebar),
                                      GTK_SHADOW_IN);
  gtk_scrolled_window_set_overlay_scrolling(GTK_SCROLLED_WINDOW(sidebar),
                                            FALSE);
  ...
  cell = gtk_cell_renderer_text_new();
  gtk_tree_view_column_pack_start(col, cell, FALSE);
  gtk_tree_view_column_set_attributes(col, cell, "text",
                                      PLACES_SIDEBAR_COLUMN_HEADING_TEXT, NULL);
  g_object_set(cell, "weight", PANGO_WEIGHT_BOLD, "weight-set", TRUE, "ypad", 1,
               "xpad", 0, NULL);
  gtk_tree_view_column_set_cell_data_func(col, cell, heading_cell_renderer_func,
                                          sidebar, NULL);

  /* icon padding */
  cell = gtk_cell_renderer_text_new();
  gtk_tree_view_column_pack_start(col, cell, FALSE);
  gtk_tree_view_column_set_cell_data_func(col, cell, padding_cell_renderer_func,
                                          sidebar, NULL);

  /* icon renderer */
  cell = gtk_cell_renderer_pixbuf_new();
  gtk_tree_view_column_pack_start(col, cell, FALSE);
  gtk_tree_view_column_set_attributes(col, cell, "surface",
                                      PLACES_SIDEBAR_COLUMN_ICON, NULL);
  gtk_tree_view_column_set_cell_data_func(col, cell, icon_cell_renderer_func,
                                          sidebar, NULL);
  ...
  gtk_tree_view_column_set_attributes(col, cell, "text",
                                      PLACES_SIDEBAR_COLUMN_NAME, "visible",
                                      PLACES_SIDEBAR_COLUMN_EJECT, NULL);
  g_object_set(cell, "ellipsize", PANGO_ELLIPSIZE_END, "ellipsize-set", TRUE,
               NULL);

  /* eject icon renderer */
  cell = gtk_cell_renderer_pixbuf_new();
  sidebar->eject_icon_cell_renderer = cell;
  g_object_set(cell, "mode", GTK_CELL_RENDERER_MODE_ACTIVATABLE, "stock-size",
               GTK_ICON_SIZE_MENU, "xpad", EJECT_BUTTON_XPAD,
               /* align right, because for some reason gtk+ expands
               this even though we tell it not to. */
               "xalign", 1.0, NULL);
  gtk_tree_view_column_pack_start(col, cell, FALSE);
  gtk_tree_view_column_set_attributes(col, cell, "visible",
                                      PLACES_SIDEBAR_COLUMN_EJECT, "surface",
                                      PLACES_SIDEBAR_COLUMN_EJECT_ICON, NULL);
  ...
  gtk_tree_view_column_set_attributes(
      col, cell, "text", PLACES_SIDEBAR_COLUMN_NAME, "visible",
      PLACES_SIDEBAR_COLUMN_NO_EJECT, "editable-set",
      PLACES_SIDEBAR_COLUMN_BOOKMARK, NULL);
  g_object_set(cell, "ellipsize", PANGO_ELLIPSIZE_END, "ellipsize-set", TRUE,
               NULL);
  ...
  sidebar->store = gtk_list_store_new(
      PLACES_SIDEBAR_COLUMN_COUNT, G_TYPE_INT, G_TYPE_STRING, G_TYPE_DRIVE,
      G_TYPE_VOLUME, G_TYPE_MOUNT, G_TYPE_STRING, CAIRO_GOBJECT_TYPE_SURFACE,
      G_TYPE_INT, G_TYPE_BOOLEAN, G_TYPE_BOOLEAN, G_TYPE_BOOLEAN, G_TYPE_STRING,
      CAIRO_GOBJECT_TYPE_SURFACE, G_TYPE_INT, G_TYPE_STRING);

  gtk_tree_view_set_tooltip_column(tree_view, PLACES_SIDEBAR_COLUMN_TOOLTIP);

  sidebar->filter_model = caja_shortcuts_model_filter_new(
      sidebar, GTK_TREE_MODEL(sidebar->store), NULL);
  ...
  gtk_tree_selection_set_select_function(selection, tree_selection_func,
                                         sidebar, NULL);

  gtk_tree_view_enable_model_drag_source(
      GTK_TREE_VIEW(tree_view), GDK_BUTTON1_MASK, caja_shortcuts_source_targets,
      G_N_ELEMENTS(caja_shortcuts_source_targets), GDK_ACTION_MOVE);
  gtk_drag_dest_set(GTK_WIDGET(tree_view), 0, caja_shortcuts_drop_targets,
                    G_N_ELEMENTS(caja_shortcuts_drop_targets),
                    GDK_ACTION_MOVE | GDK_ACTION_COPY | GDK_ACTION_LINK);

  g_signal_connect(tree_view, "key-press-event",
                   G_CALLBACK(bookmarks_key_press_event_cb), sidebar);

  g_signal_connect(tree_view, "drag-motion", G_CALLBACK(drag_motion_callback),
                   sidebar);
  g_signal_connect(tree_view, "drag-leave", G_CALLBACK(drag_leave_callback),
                   sidebar);
  g_signal_connect(tree_view, "drag-data-received",
                   G_CALLBACK(drag_data_received_callback), sidebar);
  g_signal_connect(tree_view, "drag-drop", G_CALLBACK(drag_drop_callback),
                   sidebar);

  g_signal_connect(selection, "changed",
                   G_CALLBACK(bookmarks_selection_changed_cb), sidebar);
  g_signal_connect(tree_view, "popup-menu", G_CALLBACK(bookmarks_popup_menu_cb),
                   sidebar);
  ...
  g_signal_connect_swapped(
      ...
      G_CALLBACK(desktop_location_changed_callback), sidebar);

  g_signal_connect_object(caja_trash_monitor_get(), "trash_state_changed",
                          G_CALLBACK(trash_state_changed_cb), sidebar, 0);
}

static void caja_places_sidebar_dispose(GObject *object) {
  CajaPlacesSidebar *sidebar;
  ...
  g_signal_handlers_disconnect_by_func(
      caja_preferences, desktop_location_changed_callback, sidebar);

  G_OBJECT_CLASS(caja_places_sidebar_parent_class)->dispose(object);
}

static void caja_places_sidebar_class_init(CajaPlacesSidebarClass *class) {
...
static const char *caja_places_sidebar_get_sidebar_id(CajaSidebar *sidebar) {
  return CAJA_PLACES_SIDEBAR_ID;
}

static char *caja_places_sidebar_get_tab_label(CajaSidebar *sidebar) {
  return g_strdup(_("Places"));
}

static char *caja_places_sidebar_get_tab_tooltip(CajaSidebar *sidebar) {
  return g_strdup(_("Show Places"));
}

static GdkPixbuf *caja_places_sidebar_get_tab_icon(CajaSidebar *sidebar) {
  return NULL;
}

static void caja_places_sidebar_is_visible_changed(CajaSidebar *sidebar,
                                                   gboolean is_visible) {
  /* Do nothing */
}
...
static void caja_places_sidebar_set_parent_window(CajaPlacesSidebar *sidebar,
                                                  CajaWindowInfo *window) {
  CajaWindowSlotInfo *slot;
...
static CajaSidebar *caja_places_sidebar_create(CajaSidebarProvider *provider,
                                               CajaWindowInfo *window) {
  CajaPlacesSidebar *sidebar;
...
static void caja_places_sidebar_provider_init(
    CajaPlacesSidebarProvider *sidebar) {}

static void caja_places_sidebar_provider_class_init(
    CajaPlacesSidebarProviderClass *class) {}
...
static void _caja_shortcuts_model_filter_class_init(
    CajaShortcutsModelFilterClass *class) {}

static void _caja_shortcuts_model_filter_init(CajaShortcutsModelFilter *model) {
  model->sidebar = NULL;
}

/* GtkTreeDragSource::row_draggable implementation for the shortcuts filter
 * model */
static gboolean caja_shortcuts_model_filter_row_draggable(
...
static GtkTreeModel *caja_shortcuts_model_filter_new(CajaPlacesSidebar *sidebar,
  ...
  model = g_object_new(CAJA_SHORTCUTS_MODEL_FILTER_TYPE, "child-model",
