...
/* Keep window from shrinking down ridiculously small; numbers are somewhat
 * arbitrary */
...
G_DEFINE_TYPE_WITH_PRIVATE(CajaApplication, caja_application,
                           GTK_TYPE_APPLICATION);

static GList *caja_application_get_spatial_window_list(void) {
  return caja_application_spatial_window_list;
}

static void startup_volume_mount_cb(GObject *source_object, GAsyncResult *res,
                                    gpointer user_data) {
  g_volume_mount_finish(G_VOLUME(source_object), res, NULL);
}

static void automount_all_volumes(CajaApplication *application) {
  GList *volumes, *l;

  if (g_settings_get_boolean(caja_media_preferences,
...
static void smclient_save_state_cb(EggSMClient *client, GKeyFile *state_file,
  ...
  if (data != NULL) {
    g_key_file_set_string(state_file, "Caja", "documents", data);
  }
  g_free(data);
}

static void smclient_quit_cb(EggSMClient *client,
                             CajaApplication *application) {
  caja_application_quit(application);
}

static void caja_application_smclient_initialize(CajaApplication *self) {
  g_signal_connect(self->smclient, "save_state",
                   G_CALLBACK(smclient_save_state_cb), self);
  g_signal_connect(self->smclient, "quit", G_CALLBACK(smclient_quit_cb), self);

  /* TODO: Should connect to quit_requested and block logout on active transfer?
   */
}

static void caja_application_smclient_startup(CajaApplication *self) {
  g_assert(self->smclient == NULL);
  self->smclient = egg_sm_client_get();
}

static void caja_empty_callback_to_ensure_read(CajaApplication *self) {
  /*do nothing, just exist to suppress runtime error*/
  (void)self;
}

static void open_window(CajaApplication *application, GFile *location,
                        GdkScreen *screen, const char *geometry,
  ...
  /*connect before trying to read or this preference won't be read by root or
   * after change*/
  g_signal_connect_swapped(
      caja_preferences, "changed::" CAJA_PREFERENCES_ALWAYS_USE_BROWSER,
      G_CALLBACK(caja_empty_callback_to_ensure_read), self);
  ...
  if (geometry != NULL && !gtk_widget_get_visible(GTK_WIDGET(window))) {
    ...
    eel_gtk_window_set_initial_geometry_from_string(
        GTK_WINDOW(window), geometry, APPLICATION_WINDOW_MIN_WIDTH,
        APPLICATION_WINDOW_MIN_HEIGHT, FALSE);
  }

  g_free(uri);
}

static void open_tabs(CajaApplication *application, GFile **locations,
                      guint n_files, GdkScreen *screen, const char *geometry,
                      gboolean browser_window) {
  CajaApplication *self = CAJA_APPLICATION(application);
  CajaWindow *window;
  gchar *uri = NULL;
...
static void open_windows(CajaApplication *application, GFile **files,
                         GdkScreen *screen, const char *geometry, guint n_files,
                         gboolean browser_window, gboolean open_in_tabs) {
  guint i;
...
static void caja_application_open(GApplication *app, GFile **files,
                                  gint n_files, const gchar *options) {
  ...
  if (strlen(options) > 0) {
    ...
    open_windows(self, files, gdk_screen_get_default(), geometry, n_files,
                 browser_window, open_in_tabs);
...
void caja_application_open_location(CajaApplication *application,
  ...
  caja_window_slot_open_location_full(
      caja_window_get_active_slot(window), location, open_in_tabs,
      CAJA_WINDOW_OPEN_FLAG_NEW_WINDOW, sel_list, NULL, NULL);
...
static void caja_application_init(CajaApplication *application) {
...
static void caja_application_finalize(GObject *object) {
  CajaApplication *application;
...
static gboolean check_required_directories(CajaApplication *application) {
  char *user_directory;
  ...
  if (directories != NULL) {
    ...
    if (failed_count == 1) {
      error_string = g_strdup_printf(
          _("Caja could not create the required folder \"%s\"."),
          directories_as_string->str);
      detail_string =
          _("Before running Caja, please create the following folder, or "
            "set permissions such that Caja can create it.");
    } else {
      error_string = g_strdup_printf(
          _("Caja could not create the following required folders: "
            "%s."),
          directories_as_string->str);
      detail_string =
          _("Before running Caja, please create these folders, or "
...
static void menu_provider_items_updated_handler(CajaMenuProvider *provider,
                                                GtkWidget *parent_window,
                                                gpointer data) {
  g_signal_emit_by_name(caja_signaller_get_current(), "popup_menu_changed");
}
...
static gboolean automount_all_volumes_idle_cb(gpointer data) {
...
static GtkWidget *get_desktop_manager_selection(GdkDisplay *display) {
  ...
  selection_owner = XGetSelectionOwner(
      GDK_DISPLAY_XDISPLAY(display),
      gdk_x11_atom_to_xatom_for_display(display, selection_atom));
  if (selection_owner != None) {
    return NULL;
  }

  selection_widget =
      gtk_invisible_new_for_screen(gdk_display_get_default_screen(display));
  /* We need this for gdk_x11_get_server_time() */
  gtk_widget_add_events(selection_widget, GDK_PROPERTY_CHANGE_MASK);

  if (gtk_selection_owner_set_for_display(
          display, selection_widget, selection_atom,
...
static void desktop_unrealize_cb(GtkWidget *widget,
                                 GtkWidget *selection_widget) {
  gtk_widget_destroy(selection_widget);
}

static gboolean selection_clear_event_cb(GtkWidget *widget,
...
static void caja_application_create_desktop_windows(
    CajaApplication *application) {
  GdkDisplay *display;
  GtkWidget *selection_widget;
  ...
  if (selection_widget != NULL) {
    ...
    g_signal_connect(window, "unrealize", G_CALLBACK(desktop_unrealize_cb),
                     selection_widget);
...
static void caja_application_open_desktop(CajaApplication *application) {
  if (caja_application_desktop_windows == NULL) {
    caja_application_create_desktop_windows(application);
  }
}
static void caja_application_close_desktop(void) {
  if (caja_application_desktop_windows != NULL) {
    g_list_free_full(caja_application_desktop_windows,
...
static CajaSpatialWindow *caja_application_get_existing_spatial_window(
    GFile *location) {
  GList *l;
  CajaWindowSlot *slot;
  GFile *window_location = NULL;

  for (l = caja_application_get_spatial_window_list(); l != NULL; l = l->next) {
...
static CajaSpatialWindow *find_parent_spatial_window(
...
static gboolean caja_window_delete_event_callback(GtkWidget *widget,
                                                  GdkEvent *event,
                                                  gpointer user_data) {
  CajaWindow *window;
...
static CajaWindow *create_window(CajaApplication *application,
                                 GType window_type, GdkScreen *screen) {
  CajaWindow *window;

  g_return_val_if_fail(CAJA_IS_APPLICATION(application), NULL);

  window = CAJA_WINDOW(
      gtk_widget_new(window_type, "app", application, "screen", screen, NULL));
  g_signal_connect_data(window, "delete_event",
                        G_CALLBACK(caja_window_delete_event_callback), NULL,
...
static void spatial_window_destroyed_callback(void *user_data,
                                              GObject *window) {
  caja_application_spatial_window_list =
      g_list_remove(caja_application_spatial_window_list, window);
}
...
CajaWindow *caja_application_get_spatial_window(CajaApplication *application,
  ...
  g_return_val_if_fail(CAJA_IS_APPLICATION(application), NULL);
  window = CAJA_WINDOW(caja_application_get_existing_spatial_window(location));
  ...
  if (requesting_window) {
    ...
    gtk_window_get_position(GTK_WINDOW(requesting_window), &orig_x, &orig_y);
    gtk_window_get_size(GTK_WINDOW(requesting_window), &orig_width,
                        &orig_height);
    gtk_window_get_default_size(GTK_WINDOW(window), &new_width, &new_height);
  ...
  caja_application_spatial_window_list =
      g_list_prepend(caja_application_spatial_window_list, window);
  g_object_weak_ref(G_OBJECT(window), spatial_window_destroyed_callback, NULL);

  uri = g_file_get_uri(location);
  caja_debug_log(FALSE, CAJA_DEBUG_LOG_DOMAIN_USER,
                 "present NEW spatial window=%p: %s", window, uri);
  g_free(uri);
...
CajaWindow *caja_application_create_navigation_window(
    CajaApplication *application, GdkScreen *screen) {
  CajaWindow *window;
  ...
  maximized =
      g_settings_get_boolean(caja_window_state, CAJA_WINDOW_STATE_MAXIMIZED);
  if (maximized) {
  ...
  geometry_string =
      g_settings_get_string(caja_window_state, CAJA_WINDOW_STATE_GEOMETRY);
  if (geometry_string != NULL && geometry_string[0] != 0) {
    eel_gtk_window_set_initial_geometry_from_string(
        GTK_WINDOW(window), geometry_string, CAJA_NAVIGATION_WINDOW_MIN_WIDTH,
        CAJA_NAVIGATION_WINDOW_MIN_HEIGHT, TRUE);
  }
  g_free(geometry_string);

  caja_debug_log(FALSE, CAJA_DEBUG_LOG_DOMAIN_USER,
                 "create new navigation window=%p", window);
...
static void desktop_changed_callback(gpointer user_data) {
  ...
  if (g_settings_get_boolean(mate_background_preferences,
...
static gboolean window_can_be_closed(CajaWindow *window) {
...
static void check_screen_lock_and_mount(CajaApplication *application,
                                        GVolume *volume) {
  if (application->screensaver_active) {
    ...
    application->volume_queue =
        g_list_prepend(application->volume_queue, g_object_ref(volume));
  } else {
...
static void volume_removed_callback(GVolumeMonitor *monitor, GVolume *volume,
                                    CajaApplication *application) {
  g_debug("Volume %p removed, removing from the queue", volume);

  /* clear it from the queue, if present */
  application->volume_queue = g_list_remove(application->volume_queue, volume);
}

static void volume_added_callback(GVolumeMonitor *monitor, GVolume *volume,
                                  CajaApplication *application) {
  if (g_settings_get_boolean(caja_media_preferences,
                             CAJA_PREFERENCES_MEDIA_AUTOMOUNT) &&
...
static void drive_eject_button_pressed(GDrive *drive,
                                       CajaApplication *application) {
...
static void drive_listen_for_eject_button(GDrive *drive,
                                          CajaApplication *application) {
  g_signal_connect(drive, "eject-button",
                   G_CALLBACK(drive_eject_button_pressed), application);
}

static void drive_connected_callback(GVolumeMonitor *monitor, GDrive *drive,
                                     CajaApplication *application) {
  drive_listen_for_eject_button(drive, application);
}

static void autorun_show_window(GMount *mount, gpointer user_data) {
  GFile *location;
  ...
  if (g_settings_get_boolean(caja_preferences,
    ...
    window = caja_application_create_navigation_window(
        application, gdk_screen_get_default());
  } else {
    window = caja_application_get_spatial_window(
        application, NULL, NULL, location, gdk_screen_get_default(), NULL);
  }
...
static void mount_added_callback(GVolumeMonitor *monitor, GMount *mount,
                                 CajaApplication *application) {
...
static CajaWindowSlot *get_first_navigation_slot(GList *slot_list) {
  GList *l;
...
static gboolean should_close_slot_with_mount(CajaWindow *window,
                                             CajaWindowSlot *slot,
                                             GMount *mount) {
  if (CAJA_IS_SPATIAL_WINDOW(window)) {
    return TRUE;
  }
  return caja_navigation_window_slot_should_close_with_mount(
      CAJA_NAVIGATION_WINDOW_SLOT(slot), mount);
}
 ...
 */
static void mount_removed_callback(GVolumeMonitor *monitor, GMount *mount,
                                   CajaApplication *application) {
  ...
  /* Check and see if any of the open windows are displaying contents from the
   * unmounted mount */
  window_list = gtk_application_get_windows(GTK_APPLICATION(application));
  root = g_mount_get_root(mount);
  /* Construct a list of windows to be closed. Do not add the non-closable
   * windows to the list. */
  ...
  if (caja_application_desktop_windows == NULL && !unclosed_slot) {
...
static char *caja_application_get_session_data(CajaApplication *self) {
  ...
  for (l = caja_get_history_list(); l != NULL; l = l->next) {
    ...
    bookmark_node =
        xmlNewChild(history_node, NULL, (const xmlChar *)"bookmark", NULL);
    ...
    if (caja_bookmark_get_has_custom_name(bookmark)) {
      xmlNewProp(bookmark_node, (const xmlChar *)"has_custom_name",
                 (const xmlChar *)"TRUE");
    }
  ...
  for (l = window_list; l != NULL; l = l->next) {
    ...
    xmlNewProp(win_node, (const xmlChar *)"type",
               CAJA_IS_NAVIGATION_WINDOW(window) ? (const xmlChar *)"navigation"
                                                 : (const xmlChar *)"spatial");

    if (CAJA_IS_NAVIGATION_WINDOW(
            window)) { /* spatial windows store their state as file metadata */
      ...
      if (gdk_window &&
        ...
        xmlNewProp(win_node, (const xmlChar *)"maximized",
                   (const xmlChar *)"TRUE");
      }

      if (gdk_window &&
          gdk_window_get_state(gdk_window) & GDK_WINDOW_STATE_STICKY) {
        xmlNewProp(win_node, (const xmlChar *)"sticky",
                   (const xmlChar *)"TRUE");
      }

      if (gdk_window &&
          gdk_window_get_state(gdk_window) & GDK_WINDOW_STATE_ABOVE) {
        xmlNewProp(win_node, (const xmlChar *)"keep-above",
                   (const xmlChar *)"TRUE");
      }
    }
    ...
    for (m = slots; m != NULL; m = m->next) {
      ...
      if (slot == active_slot) {
        xmlNewProp(slot_node, (const xmlChar *)"active",
                   (const xmlChar *)"TRUE");
      }
    }
  ...
  if (xmlSaveDoc(ctx, doc) < 0 || xmlSaveFlush(ctx) < 0) {
...

{
  ...
  data = g_key_file_get_string(state_file, "Caja", "documents", NULL);
  if (data == NULL) {
    return;
  }
    ...
    for (node = root_node->children; node != NULL; node = node->next) {
      ...
      if (xmlStrcmp(node->name, (const xmlChar *)"history") == 0) {
        ...
        for (bookmark_node = node->children; bookmark_node != NULL;
          ...
          if (xmlStrcmp(bookmark_node->name, (const xmlChar *)"bookmark") ==
              0) {
            ...
            has_custom_name =
                xmlHasProp(bookmark_node, (const xmlChar *)"has_custom_name")
                    ? TRUE
                    : FALSE;
            ...
            emit_change |= caja_add_to_history_list_no_notify(
                location, (const char *)name, has_custom_name, icon);
            ...
            g_message("unexpected bookmark node %s while parsing session data",
                      bookmark_node->name);
            bail = TRUE;
            continue;
          }
...

      {
        ...
        if (xmlStrcmp(type, (const xmlChar *)"navigation") == 0) {
          ...
          if (geometry != NULL && *geometry != '\0') {
            eel_gtk_window_set_initial_geometry_from_string(
                GTK_WINDOW(window), (const char *)geometry,
                CAJA_NAVIGATION_WINDOW_MIN_WIDTH,
                CAJA_NAVIGATION_WINDOW_MIN_HEIGHT, FALSE);
          }
          xmlFree(geometry);
          ...
          for (i = 0, slot_node = node->children; slot_node != NULL;
            ...
            if (slot_node->name &&
                xmlStrcmp(slot_node->name, (const xmlChar *)"slot") == 0) {
              xmlChar *slot_uri;
              ...
              if (slot_uri != NULL) {
                ...
                if (i == 0) {
                  ...
                  slot = caja_window_open_slot(window->details->active_pane,
          ...
          if (i == 0) {
            ...
            caja_window_slot_open_location(
                window->details->active_pane->active_slot, location, FALSE);
            g_object_unref(location);
          }
        } else if (xmlStrcmp(type, (const xmlChar *)"spatial") == 0) {
          location = g_file_new_for_uri((const char *)location_uri);
          window = caja_application_get_spatial_window(
              application, NULL, NULL, location, gdk_screen_get_default(),
              NULL);
          caja_window_go_to(window, location);
          g_object_unref(location);
        } else {
          g_message("unknown window type \"%s\" while parsing session data",
                    type);
          bail = TRUE;
        }
...
static gboolean do_cmdline_sanity_checks(CajaApplication *self,
                                         gboolean perform_self_check,
                                         gboolean version, gboolean kill_shell,
  ...
  if (self->priv->geometry != NULL && remaining != NULL &&
...
static void select_items_ready_cb(GObject *source, GAsyncResult *res,
                                  gpointer user_data) {
...
static void caja_application_select(CajaApplication *self, GFile **files,
                                    gint len) {
  ...
  g_dbus_connection_call(
      connection, CAJA_FDO_DBUS_NAME, CAJA_FDO_DBUS_PATH, CAJA_FDO_DBUS_IFACE,
      "ShowItems", g_variant_new("(ass)", &builder, ""), NULL,
      G_DBUS_CALL_FLAGS_NONE, G_MAXINT, NULL, select_items_ready_cb, self);
...
static gboolean running_as_root(void) { return geteuid() == 0; }

static gboolean caja_application_local_command_line(GApplication *application,
                                                    gchar ***arguments,
                                                    gint *exit_status) {
  ...
  const GOptionEntry options[] = {
      ...
      {"no-desktop", '\0', 0, G_OPTION_ARG_NONE, &self->priv->no_desktop,
       N_("Do not manage the desktop (ignore the preference set in the "
          "preferences dialog)."),
       NULL},
      {"force-desktop", '\0', 0, G_OPTION_ARG_NONE, &self->priv->force_desktop,
       N_("Manage the desktop regardless of set preferences or environment (on "
          "new startup only)"),
       NULL},
      ...
      {"quit", 'q', 0, G_OPTION_ARG_NONE, &kill_shell, N_("Quit Caja."), NULL},
      {"select", 's', 0, G_OPTION_ARG_NONE, &select_uris,
       N_("Select specified URI in parent folder."), NULL},
      {G_OPTION_REMAINING, 0, 0, G_OPTION_ARG_STRING_ARRAY, &remaining, NULL,
       N_("[URI...]")},
  ...
  *exit_status = EXIT_SUCCESS;
  ...
  if (!do_cmdline_sanity_checks(self, perform_self_check, version, kill_shell,
                                select_uris, remaining)) {
    *exit_status = EXIT_FAILURE;
    goto out;
  }
  ...
  g_debug(
      "Parsing local command line, no_default_window %d, quit %d, "
      "self checks %d, no_desktop %d",
      no_default_window, kill_shell, perform_self_check,
      self->priv->no_desktop);
  ...
  if (select_uris) {
    ...
    if (self->priv->geometry == NULL) {
      ...
      g_snprintf(concatOptions, 64, "%d=%s=%d", browser_window,
                 self->priv->geometry, open_in_tabs);
    }
...
static void load_custom_css(GtkCssProvider *provider, const gchar *filename,
                            guint priority) {
  GError *error = NULL;
  ...
  if (error != NULL) {
    ...
    gtk_style_context_add_provider_for_screen(
        gdk_screen_get_default(), GTK_STYLE_PROVIDER(provider), priority);
  }
...
static void reload_theme_css(GtkSettings *settings, GParamSpec *unused,
  ...
  if (g_file_test(path, G_FILE_TEST_EXISTS))
    load_custom_css(provider, css_theme_name,
...
static void init_icons_and_styles(void) {
  GtkSettings *settings = gtk_settings_get_default();
  GtkCssProvider *provider;

  /* add our custom CSS provider */
  load_custom_css(NULL, "caja.css", GTK_STYLE_PROVIDER_PRIORITY_THEME);
  /* add our desktop CSS provider,  ensures the desktop background does not get
   * covered */
  load_custom_css(NULL, "caja-desktop.css",
                  GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
  ...
  g_signal_connect_data(settings, "notify::gtk-theme-name",
                        G_CALLBACK(reload_theme_css), provider,
...
static void init_desktop(CajaApplication *self) {
  ...
  if (running_as_root() || !running_in_mate()) {
    /* do not manage desktop when running as root or on other desktops unless
     * forced */
    self->priv->no_desktop = TRUE;
  }
...
static gboolean caja_application_save_accel_map(gpointer data) {
...
static void queue_accel_map_save_callback(GtkAccelMap *object,
                                          gchar *accel_path, guint accel_key,
...
static void init_gtk_accels(void) {
  char *accel_map_filename;
...
static void caja_application_startup(GApplication *app) {
  CajaApplication *self = CAJA_APPLICATION(app);
  ...
  /* TODO-gio: This should be using the UNMOUNTED feature of GFileMonitor
   * instead */
  ...
  self->automount_idle_id = g_idle_add_full(
      G_PRIORITY_LOW, automount_all_volumes_idle_cb, self, NULL);
  ...
  /* exit_with_last_window is already set to TRUE, and we need to keep that
   * value on other desktops, running from the command line,  or when running
   * caja as root. Otherwise, we read the value from the configuration.
   */
  ...
  if (running_in_mate() && !running_as_root()) {
    ...
    exit_with_last_window = g_settings_get_boolean(
        caja_preferences, CAJA_PREFERENCES_EXIT_WITH_LAST_WINDOW);
...
static void caja_application_quit_mainloop(GApplication *app) {
...
static void caja_application_class_init(CajaApplicationClass *class) {
...
CajaApplication *caja_application_new(void) {
  return g_object_new(CAJA_TYPE_APPLICATION, "application-id", "org.mate.Caja",
                      "register-session", TRUE, "flags",
