...
static AtkAttributeSet *desktop_get_attributes(AtkObject *accessible) {
  AtkAttributeSet *attributes;
  AtkAttribute *is_desktop;

  attributes = ATK_OBJECT_CLASS(caja_desktop_window_accessible_parent_class)
                   ->get_attributes(accessible);
...
static void caja_desktop_window_accessible_init(
    CajaDesktopWindowAccessible *window) {}

static void caja_desktop_window_accessible_class_init(
    CajaDesktopWindowAccessibleClass *klass) {
...
static void caja_desktop_window_init(CajaDesktopWindow *window) {
  GtkAction *action;
...
static gint caja_desktop_window_delete_event(CajaDesktopWindow *window) {
...
static void caja_desktop_window_screen_size_changed(GdkScreen *screen,
  ...
  g_object_set(window, "width_request", width_request, "height_request",
               height_request, NULL);
}
...
CajaDesktopWindow *caja_desktop_window_new(CajaApplication *application,
  ...
  window = CAJA_DESKTOP_WINDOW(gtk_widget_new(
      caja_desktop_window_get_type(), "app", application, "width_request",
      width_request, "height_request", height_request, "screen", screen, NULL));
  ...
  g_signal_connect(window, "delete_event",
                   G_CALLBACK(caja_desktop_window_delete_event), NULL);
...
static void set_wmspec_desktop_hint(GdkWindow *window) {
  ...
  gdk_property_change(window, gdk_atom_intern("_NET_WM_WINDOW_TYPE", FALSE),
                      gdk_x11_xatom_to_atom(XA_ATOM), 32, GDK_PROP_MODE_REPLACE,
                      (guchar *)&atom, 1);
}

static void set_desktop_window_id(CajaDesktopWindow *window,
                                  GdkWindow *gdkwindow) {
...
static void realize(GtkWidget *widget) {
  ...
  details->size_changed_id = g_signal_connect(
      gtk_window_get_screen(GTK_WINDOW(window)), "size_changed",
...
static void caja_desktop_window_class_init(CajaDesktopWindowClass *klass) {
