...
#define CAJA_ZOOM_CONTROL_CLASS(klass)                      \
  (G_TYPE_CHECK_CLASS_CAST((klass), CAJA_TYPE_ZOOM_CONTROL, \
...
#define CAJA_ZOOM_CONTROL_GET_CLASS(obj)                    \
  (G_TYPE_INSTANCE_GET_CLASS((obj), CAJA_TYPE_ZOOM_CONTROL, \
                             CajaZoomControlClass))
...
struct CajaZoomControlClass {
  ...
  void (*zoom_to_level)(CajaZoomControl *control, CajaZoomLevel zoom_level);
  void (*zoom_to_default)(CajaZoomControl *control);

  /* Action signal for keybindings, do not connect to this */
  void (*change_value)(CajaZoomControl *control, GtkScrollType scroll);
};
