...
static void caja_sidebar_title_init(CajaSidebarTitle *sidebar_title) {
  sidebar_title->details =
      caja_sidebar_title_get_instance_private(sidebar_title);

  gtk_orientable_set_orientation(GTK_ORIENTABLE(sidebar_title),
                                 GTK_ORIENTATION_VERTICAL);

  /* Create the icon */
  sidebar_title->details->icon = gtk_image_new();
  gtk_box_pack_start(GTK_BOX(sidebar_title), sidebar_title->details->icon, 0, 0,
                     0);
  gtk_widget_show(sidebar_title->details->icon);

  /* Create the title label */
  sidebar_title->details->title_label = sidebar_title_create_title_label();
  gtk_box_pack_start(GTK_BOX(sidebar_title),
                     sidebar_title->details->title_label, 0, 0, 0);
  ...
  gtk_box_pack_start(GTK_BOX(sidebar_title),
                     sidebar_title->details->more_info_label, 0, 0, 0);
  gtk_widget_show(sidebar_title->details->more_info_label);

  sidebar_title->details->emblem_box =
      gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0);
  gtk_widget_show(sidebar_title->details->emblem_box);
  gtk_box_pack_start(GTK_BOX(sidebar_title), sidebar_title->details->emblem_box,
                     0, 0, 0);
  ...
  g_signal_connect_swapped(
      caja_preferences, "changed::" CAJA_PREFERENCES_SHOW_DIRECTORY_ITEM_COUNTS,
      G_CALLBACK(update_more_info), sidebar_title);
}

/* destroy by throwing away private storage */
static void release_file(CajaSidebarTitle *sidebar_title) {
  if (sidebar_title->details->file_changed_connection != 0) {
    if (g_signal_handler_is_connected(
            G_OBJECT(sidebar_title->details->file),
...
static void caja_sidebar_title_finalize(GObject *object) {
  CajaSidebarTitle *sidebar_title;
  ...
  g_signal_handlers_disconnect_by_func(caja_preferences, update_more_info,
                                       sidebar_title);
...
static void setup_gc_with_fg(CajaSidebarTitle *sidebar_title, int idx,
                             GdkRGBA *color) {
...
void caja_sidebar_title_select_text_color(CajaSidebarTitle *sidebar_title,
  ...
  /* read the info colors from the current theme; use a reasonable default if
   * undefined */
  style = gtk_widget_get_style_context(GTK_WIDGET(sidebar_title));
  gtk_style_context_get_style(style, "light_info_color", &light_info_color,
                              "dark_info_color", &dark_info_color, NULL);
  ...
  setup_gc_with_fg(
      sidebar_title, LABEL_INFO_COLOR_HIGHLIGHT,
  ...
  setup_gc_with_fg(
      sidebar_title, LABEL_INFO_COLOR_ACTIVE,
      eel_gdk_rgba_is_dark(&color) ? light_info_color : dark_info_color);
  ...
  if (!eel_background_is_set(background)) {
    ...
    setup_gc_with_fg(
        sidebar_title, LABEL_INFO_COLOR,
        eel_gdk_rgba_is_dark(&color) ? light_info_color : dark_info_color);
...
static char *get_property_from_component(CajaSidebarTitle *sidebar_title,
                                         const char *property) {
...
static void update_icon(CajaSidebarTitle *sidebar_title) {
  ...
  if (icon_name != NULL && icon_name[0] != '\0') {
    ...
    surface = caja_file_get_icon_surface(
        sidebar_title->details->file,
        sidebar_title->details->best_icon_size * icon_scale, TRUE, icon_scale,
  ...
  if (!leave_surface_unchanged) {
    gtk_image_set_from_surface(GTK_IMAGE(sidebar_title->details->icon),
                               surface);
  }
...
static void override_title_font(GtkWidget *widget, const gchar *font) {
  gchar *css;
  ...
  css = g_strdup_printf("label { font-family: %s; font-size: %spt; }", tempfont,
                        tempsize);
...
static void update_title_font(CajaSidebarTitle *sidebar_title) {
  ...
  gtk_style_context_get(context, state, GTK_STYLE_PROPERTY_FONT, &title_font,
                        NULL);
  max_style_font_size =
      pango_font_description_get_size(title_font) * 1.8 / PANGO_SCALE;
...
static void update_title(CajaSidebarTitle *sidebar_title) {
  GtkLabel *label;
  const char *text;
...
static void append_and_eat(GString *string, const char *separator,
                           char *new_string) {
  if (new_string == NULL) {
    return;
  }
...
static int measure_width_callback(const char *string, gpointer callback_data) {
  PangoLayout *layout;
  int width;
...
static void update_more_info(CajaSidebarTitle *sidebar_title) {
  ...
  if (component_info != NULL && strlen(component_info) > 0) {
    ...
    if (sidebar_width > MINIMUM_INFO_WIDTH) {
      ...
      layout = pango_layout_copy(gtk_label_get_layout(
          GTK_LABEL(sidebar_title->details->more_info_label)));
...
static void add_emblem(CajaSidebarTitle *sidebar_title, GdkPixbuf *pixbuf) {
  GtkWidget *image_widget;

  image_widget = gtk_image_new_from_pixbuf(pixbuf);
  gtk_widget_show(image_widget);
  gtk_container_add(GTK_CONTAINER(sidebar_title->details->emblem_box),
                    image_widget);
}

static void update_emblems(CajaSidebarTitle *sidebar_title) {
  GList *pixbufs, *p;
  ...
  pixbufs = caja_file_get_emblem_pixbufs(
      sidebar_title->details->file,
      caja_icon_get_emblem_size_for_icon_size(CAJA_ICON_SIZE_STANDARD), FALSE,
      NULL);
...
static gboolean item_count_ready(CajaSidebarTitle *sidebar_title) {
  return sidebar_title->details->file != NULL &&
         caja_file_get_directory_item_count(sidebar_title->details->file, NULL,
                                            NULL) != 0;
}

static void monitor_add(CajaSidebarTitle *sidebar_title) {
  ...
  caja_file_monitor_add(sidebar_title->details->file, sidebar_title,
                        attributes);
}
...
void caja_sidebar_title_set_file(CajaSidebarTitle *sidebar_title,
  ...
  if (file != sidebar_title->details->file) {
    ...
    /* attach file */
    if (file != NULL) {
      sidebar_title->details->file_changed_connection = g_signal_connect_object(
          sidebar_title->details->file, "changed", G_CALLBACK(update_all),
          sidebar_title, G_CONNECT_SWAPPED);
      monitor_add(sidebar_title);
    }
  }
...
static void caja_sidebar_title_size_allocate(GtkWidget *widget,
  ...
  GTK_WIDGET_CLASS(caja_sidebar_title_parent_class)
...
gboolean caja_sidebar_title_hit_test_icon(CajaSidebarTitle *sidebar_title,
  ...
  gtk_widget_get_allocation(GTK_WIDGET(sidebar_title->details->icon),
                            allocation);
