...
static void caja_window_info_base_init(gpointer g_class) {
  static gboolean initialized = FALSE;

  if (!initialized) {
    caja_window_info_signals[LOADING_URI] = g_signal_new(
        "loading_uri", CAJA_TYPE_WINDOW_INFO, G_SIGNAL_RUN_LAST,
        G_STRUCT_OFFSET(CajaWindowInfoIface, loading_uri), NULL, NULL,
        g_cclosure_marshal_VOID__STRING, G_TYPE_NONE, 1, G_TYPE_STRING);

    caja_window_info_signals[SELECTION_CHANGED] = g_signal_new(
        "selection_changed", CAJA_TYPE_WINDOW_INFO, G_SIGNAL_RUN_LAST,
        G_STRUCT_OFFSET(CajaWindowInfoIface, selection_changed), NULL, NULL,
        g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

    caja_window_info_signals[TITLE_CHANGED] = g_signal_new(
        "title_changed", CAJA_TYPE_WINDOW_INFO, G_SIGNAL_RUN_LAST,
        G_STRUCT_OFFSET(CajaWindowInfoIface, title_changed), NULL, NULL,
        g_cclosure_marshal_VOID__STRING, G_TYPE_NONE, 1, G_TYPE_STRING);

    caja_window_info_signals[HIDDEN_FILES_MODE_CHANGED] = g_signal_new(
        "hidden_files_mode_changed", CAJA_TYPE_WINDOW_INFO, G_SIGNAL_RUN_LAST,
        G_STRUCT_OFFSET(CajaWindowInfoIface, hidden_files_mode_changed), NULL,
        NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

    caja_window_info_signals[BACKUP_FILES_MODE_CHANGED] = g_signal_new(
        "backup_files_mode_changed", CAJA_TYPE_WINDOW_INFO, G_SIGNAL_RUN_LAST,
        G_STRUCT_OFFSET(CajaWindowInfoIface, backup_files_mode_changed), NULL,
        NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
...
GType caja_window_info_get_type(void) {
  ...
  if (!type) {
    const GTypeInfo info = {sizeof(CajaWindowInfoIface),
    ...
    type = g_type_register_static(G_TYPE_INTERFACE, "CajaWindowInfo", &info, 0);
...
void caja_window_info_report_load_underway(CajaWindowInfo *window,
  ...
  (*CAJA_WINDOW_INFO_GET_IFACE(window)->report_load_underway)(window, view);
}
...
void caja_window_info_report_load_complete(CajaWindowInfo *window,
  ...
  (*CAJA_WINDOW_INFO_GET_IFACE(window)->report_load_complete)(window, view);
}
...
void caja_window_info_report_view_failed(CajaWindowInfo *window,
  ...
  (*CAJA_WINDOW_INFO_GET_IFACE(window)->report_view_failed)(window, view);
}
...
void caja_window_info_view_visible(CajaWindowInfo *window, CajaView *view) {
  g_return_if_fail(CAJA_IS_WINDOW_INFO(window));
...
void caja_window_info_push_status(CajaWindowInfo *window, const char *status) {
  g_return_if_fail(CAJA_IS_WINDOW_INFO(window));

  (*CAJA_WINDOW_INFO_GET_IFACE(window)->push_status)(window, status);
}
...
CajaWindowShowHiddenFilesMode caja_window_info_get_hidden_files_mode(
    CajaWindowInfo *window) {
  g_return_val_if_fail(CAJA_IS_WINDOW_INFO(window),
                       CAJA_WINDOW_SHOW_HIDDEN_FILES_DEFAULT);

  return (*CAJA_WINDOW_INFO_GET_IFACE(window)->get_hidden_files_mode)(window);
}

void caja_window_info_set_hidden_files_mode(
    CajaWindowInfo *window, CajaWindowShowHiddenFilesMode mode) {
  g_return_if_fail(CAJA_IS_WINDOW_INFO(window));

  (*CAJA_WINDOW_INFO_GET_IFACE(window)->set_hidden_files_mode)(window, mode);
}

CajaWindowShowBackupFilesMode caja_window_info_get_backup_files_mode(
    CajaWindowInfo *window) {
  g_return_val_if_fail(CAJA_IS_WINDOW_INFO(window),
...
void caja_window_info_set_backup_files_mode(
    CajaWindowInfo *window, CajaWindowShowBackupFilesMode mode) {
  g_return_if_fail(CAJA_IS_WINDOW_INFO(window));

  (*CAJA_WINDOW_INFO_GET_IFACE(window)->set_backup_files_mode)(window, mode);
}
...
void caja_window_info_set_initiated_unmount(CajaWindowInfo *window,
  ...
  (*CAJA_WINDOW_INFO_GET_IFACE(window)->set_initiated_unmount)(
      window, initiated_unmount);
}
