...
static void set_directory_location(CajaDirectory *directory, GFile *location);
...
static void caja_directory_class_init(CajaDirectoryClass *klass) {
  ...
  signals[FILES_ADDED] = g_signal_new(
      "files_added", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaDirectoryClass, files_added), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);
  signals[FILES_CHANGED] = g_signal_new(
      "files_changed", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaDirectoryClass, files_changed), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);
  signals[DONE_LOADING] = g_signal_new(
      "done_loading", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaDirectoryClass, done_loading), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
  signals[LOAD_ERROR] = g_signal_new(
      "load_error", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaDirectoryClass, load_error), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);
 ...
 */
CajaDirectory *caja_directory_get_internal(GFile *location, gboolean create) {
  CajaDirectory *directory;
 ...
 */
CajaFile *caja_directory_get_existing_corresponding_file(
    CajaDirectory *directory) {
  CajaFile *file;
  char *uri;
...
char *caja_directory_get_name_for_self_as_new_file(CajaDirectory *directory) {
  ...
  if (CAJA_DIRECTORY_GET_CLASS(directory)->get_name_for_self_as_new_file ==
      NULL)
    return NULL;
  else
    return CAJA_DIRECTORY_GET_CLASS(directory)->get_name_for_self_as_new_file(
        directory);
}
...
static CajaDirectory *caja_directory_new(GFile *location) {
  ...
  if (eel_uri_is_desktop(uri)) {
    ...
    directory =
        CAJA_DIRECTORY(caja_search_directory_new_from_saved_search(uri));
  } else {
...
static void add_to_hash_table(CajaDirectory *directory, CajaFile *file,
                              GList *node) {
  const char *name;
...
static GList *extract_from_hash_table(CajaDirectory *directory,
                                      CajaFile *file) {
  const char *name;
  GList *node;
...
void caja_directory_add_file(CajaDirectory *directory, CajaFile *file) {
  GList *node;
...
void caja_directory_remove_file(CajaDirectory *directory, CajaFile *file) {
  GList *node;
...
void caja_directory_emit_load_error(CajaDirectory *directory, GError *error) {
  g_signal_emit(directory, signals[LOAD_ERROR], 0, error);
}

/* Return a directory object for this one's parent. */
static CajaDirectory *get_parent_directory(GFile *location) {
  GFile *parent;
 ...
 */
static CajaDirectory *get_parent_directory_if_exists(GFile *location) {
  GFile *parent;
...
static void call_files_changed_common(CajaDirectory *directory,
                                      GList *file_list) {
  GList *node;
  CajaFile *file = NULL;
...
void caja_directory_notify_files_changed(GList *files) {
  ...
  for (node = files; node != NULL; node = node->next) {
    ...
    if (file != NULL) {
      ...
      hash_table_list_prepend(changed_lists, file->details->directory, file);
    }
  }
...
void caja_directory_notify_files_removed(GList *files) {
  ...
  for (p = files; p != NULL; p = p->next) {
    ...
    if (file != NULL && !caja_file_rename_in_progress(file)) {
      ...
      hash_table_list_prepend(changed_lists, file->details->directory,
                              caja_file_ref(file));
    }
    caja_file_unref(file);
  }
...
static void change_directory_location(CajaDirectory *directory,
                                      GFile *new_location) {
...
static GList *caja_directory_moved_internal(GFile *old_location,
                                            GFile *new_location) {
    ...
    if (new_directory_location) {
      ...
      if (directory->details->as_file != NULL) {
        affected_files = g_list_prepend(
            affected_files, caja_file_ref(directory->details->as_file));
      }
      affected_files = g_list_concat(
          affected_files, caja_file_list_copy(directory->details->file_list));
    }
...
void caja_directory_moved(const char *old_uri, const char *new_uri) {
  GList *list, *node;
  GHashTable *hash;
  GFile *old_location;
  GFile *new_location;
  ...
  for (node = list; node != NULL; node = node->next) {
    ...
    hash_table_list_prepend(hash, file->details->directory,
                            caja_file_ref(file));
  }
...
void caja_directory_notify_files_moved(GList *file_pairs) {
  ...
  for (p = file_pairs; p != NULL; p = p->next) {
    ...
    if (file != NULL) {
      ...
      hash_table_list_prepend(changed_lists, new_directory, file);
      collect_parent_directories(parent_directories, new_directory);
    }

    /* Update any directory objects that are affected. */
    affected_files = caja_directory_moved_internal(from_location, to_location);
    ...
    if (file == NULL) {
      ...
      caja_directory_cancel_loading_file_attributes(old_directory, file,
                                                    cancel_attributes);
      ...
      caja_file_update_name_and_directory(file, name, new_directory);
      g_free(name);
...
void caja_directory_schedule_position_set(GList *position_setting_list) {
  ...
  for (p = position_setting_list; p != NULL; p = p->next) {
    ...
    caja_file_set_metadata(file, CAJA_METADATA_KEY_ICON_POSITION, NULL, str);
    ...
    if (item->set) {
      ...
      caja_file_set_time_metadata(
          file, CAJA_METADATA_KEY_ICON_POSITION_TIMESTAMP, UNDEFINED_TIME);
    }
    ...
    caja_file_set_metadata(file, CAJA_METADATA_KEY_SCREEN, NULL, str);
...
void caja_directory_call_when_ready(CajaDirectory *directory,
  ...
  if (CAJA_DIRECTORY_GET_CLASS(directory)->call_when_ready != NULL) {
    CAJA_DIRECTORY_GET_CLASS(directory)->call_when_ready(
        directory, file_attributes, wait_for_all_files, callback,
        callback_data);
  }
}
...
void caja_directory_cancel_callback(CajaDirectory *directory,
  ...
  if (CAJA_DIRECTORY_GET_CLASS(directory)->cancel_callback != NULL) {
    CAJA_DIRECTORY_GET_CLASS(directory)->cancel_callback(directory, callback,
                                                         callback_data);
  }
}
...
void caja_directory_file_monitor_add(CajaDirectory *directory,
  ...
  if (CAJA_DIRECTORY_GET_CLASS(directory)->file_monitor_add != NULL) {
    CAJA_DIRECTORY_GET_CLASS(directory)->file_monitor_add(
        directory, client, monitor_hidden_files, file_attributes, callback,
        callback_data);
  }
}
...
static gboolean is_tentative(gpointer data, gpointer callback_data) {
  CajaFile *file;
...
static GList *real_get_file_list(CajaDirectory *directory) {
  ...
  tentative_files =
      eel_g_list_partition(g_list_copy(directory->details->file_list),
...
static gboolean real_is_editable(CajaDirectory *directory) { return TRUE; }
 ...
 **/
GList *caja_directory_list_ref(GList *list) {
 ...
 * @list: GList of directories.
 **/
void caja_directory_list_unref(GList *list) {
 ...
 * @list: GList of directories.
 **/
void caja_directory_list_free(GList *list) {
 ...
 * @list: GList of directories.
 **/
GList *caja_directory_list_copy(GList *list) {
  return g_list_copy(caja_directory_list_ref(list));
}

static int compare_by_uri(CajaDirectory *a, CajaDirectory *b) {
  char *uri_a, *uri_b;
  int res;
 ...
 * @list: GList of directories.
 **/
GList *caja_directory_list_sort_by_uri(GList *list) {
...
static void got_files_callback(CajaDirectory *directory, GList *files,
                               gpointer callback_data) {
  g_assert(CAJA_IS_DIRECTORY(directory));
...
void caja_self_check_directory(void) {
  CajaDirectory *directory;
  CajaFile *file;
  ...
  caja_directory_file_monitor_add(directory, &data_dummy, TRUE, 0, NULL, NULL);
  ...
  EEL_CHECK_STRING_RESULT (caja_file_get_metadata (file, "test", "default"),
  "value");

  caja_file_set_boolean_metadata (file, "test_boolean", TRUE, TRUE);
  EEL_CHECK_BOOLEAN_RESULT (caja_file_get_boolean_metadata (file,
  "test_boolean", TRUE), TRUE); caja_file_set_boolean_metadata (file,
  "test_boolean", TRUE, FALSE); EEL_CHECK_BOOLEAN_RESULT
  (caja_file_get_boolean_metadata (file, "test_boolean", TRUE), FALSE);
  EEL_CHECK_BOOLEAN_RESULT (caja_file_get_boolean_metadata (NULL,
  "test_boolean", TRUE), TRUE);

  caja_file_set_integer_metadata (file, "test_integer", 0, 17);
  EEL_CHECK_INTEGER_RESULT (caja_file_get_integer_metadata (file,
  "test_integer", 0), 17); caja_file_set_integer_metadata (file, "test_integer",
  0, -1); EEL_CHECK_INTEGER_RESULT (caja_file_get_integer_metadata (file,
  "test_integer", 0), -1); caja_file_set_integer_metadata (file, "test_integer",
  42, 42); EEL_CHECK_INTEGER_RESULT (caja_file_get_integer_metadata (file,
  "test_integer", 42), 42); EEL_CHECK_INTEGER_RESULT
  (caja_file_get_integer_metadata (NULL, "test_integer", 42), 42);
  EEL_CHECK_INTEGER_RESULT (caja_file_get_integer_metadata (file,
  "nonexistent_key", 42), 42);
  */

  EEL_CHECK_BOOLEAN_RESULT(
      caja_directory_get_by_uri("file:///etc") == directory, TRUE);
  caja_directory_unref(directory);
