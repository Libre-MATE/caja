...
static gboolean call_consume_changes_idle_cb(gpointer not_used) {
  caja_file_changes_consume_changes(TRUE);
  call_consume_changes_idle_id = 0;
  return FALSE;
}

static void schedule_call_consume_changes(void) {
...
static void dir_changed(GFileMonitor *monitor, GFile *child, GFile *other_file,
...
CajaMonitor *caja_monitor_directory(GFile *location) {
  ...
  */
  if (ret->monitor != NULL) {
    g_signal_connect(ret->monitor, "changed", G_CALLBACK(dir_changed), ret);
  }
...
void caja_monitor_cancel(CajaMonitor *monitor) {
  if (monitor->monitor != NULL) {
    g_signal_handlers_disconnect_by_func(monitor->monitor, dir_changed,
                                         monitor);
  ...
  if (monitor->volume_monitor != NULL) {
    g_signal_handlers_disconnect_by_func(monitor->volume_monitor, mount_removed,
                                         monitor);
