...
enum { COLUMN_APPINFO, COLUMN_DEFAULT, COLUMN_ICON, COLUMN_NAME, NUM_COLUMNS };

G_DEFINE_TYPE(CajaMimeApplicationChooser, caja_mime_application_chooser,
              GTK_TYPE_BOX);
...
static void mime_type_data_changed_cb(GObject *signaller, gpointer user_data);

static void caja_mime_application_chooser_finalize(GObject *object) {
...
static void caja_mime_application_chooser_class_init(
    CajaMimeApplicationChooserClass *class) {
...
static void default_toggled_cb(GtkCellRendererToggle *renderer,
                               const char *path_str, gpointer user_data) {
  CajaMimeApplicationChooser *chooser;
  GtkTreeIter iter;
  GtkTreePath *path;
  GError *error;
  ...
  if (gtk_tree_model_get_iter(GTK_TREE_MODEL(chooser->details->model), &iter,
                              path)) {
    gboolean is_default;
    gboolean success;
    GAppInfo *info;

    gtk_tree_model_get(GTK_TREE_MODEL(chooser->details->model), &iter,
                       COLUMN_DEFAULT, &is_default, COLUMN_APPINFO, &info, -1);

    if (!is_default && info != NULL) {
      error = NULL;
      if (chooser->details->extension) {
        success = g_app_info_set_as_default_for_extension(
            info, chooser->details->extension, &error);
      } else {
        success = g_app_info_set_as_default_for_type(
            info, chooser->details->content_type, &error);
      }
      ...
      if (!success) {
        ...
        message = g_strdup_printf(
            _("Could not set application as the default: %s"), error->message);
        eel_show_error_dialog(
            _("Could not set as default application"), message,
...
static GAppInfo *get_selected_application(CajaMimeApplicationChooser *chooser) {
  GtkTreeIter iter;
  GtkTreeSelection *selection;
  GAppInfo *info;

  selection =
      gtk_tree_view_get_selection(GTK_TREE_VIEW(chooser->details->treeview));

  info = NULL;
  if (gtk_tree_selection_get_selected(selection, NULL, &iter)) {
    gtk_tree_model_get(GTK_TREE_MODEL(chooser->details->model), &iter,
                       COLUMN_APPINFO, &info, -1);
  }
...
static void selection_changed_cb(GtkTreeSelection *selection,
  ...
  if (info) {
    ...
    gtk_widget_set_sensitive(chooser->details->remove_button, FALSE);
  }
}

static GtkWidget *create_tree_view(CajaMimeApplicationChooser *chooser) {
  GtkWidget *treeview;
  GtkListStore *store;
  ...
  store = gtk_list_store_new(NUM_COLUMNS, G_TYPE_APP_INFO, G_TYPE_BOOLEAN,
                             G_TYPE_ICON, G_TYPE_STRING);
  gtk_tree_sortable_set_sort_column_id(GTK_TREE_SORTABLE(store), COLUMN_NAME,
                                       GTK_SORT_ASCENDING);
  gtk_tree_view_set_model(GTK_TREE_VIEW(treeview), GTK_TREE_MODEL(store));
  chooser->details->model = store;

  renderer = gtk_cell_renderer_toggle_new();
  g_signal_connect(renderer, "toggled", G_CALLBACK(default_toggled_cb),
                   chooser);
  gtk_cell_renderer_toggle_set_radio(GTK_CELL_RENDERER_TOGGLE(renderer), TRUE);

  column = gtk_tree_view_column_new_with_attributes(
      _("Default"), renderer, "active", COLUMN_DEFAULT, NULL);
  ...
  column = gtk_tree_view_column_new_with_attributes(_("Icon"), renderer,
                                                    "gicon", COLUMN_ICON, NULL);
  gtk_tree_view_append_column(GTK_TREE_VIEW(treeview), column);

  renderer = gtk_cell_renderer_text_new();
  column = gtk_tree_view_column_new_with_attributes(
      _("Name"), renderer, "markup", COLUMN_NAME, NULL);
  gtk_tree_view_append_column(GTK_TREE_VIEW(treeview), column);
...
static void add_clicked_cb(GtkButton *button, gpointer user_data) {
  ...
  if (chooser->details->for_multiple_files) {
    dialog = caja_add_application_dialog_new_for_multiple_files(
        chooser->details->extension, chooser->details->orig_mime_type);
  } else {
...
static void remove_clicked_cb(GtkButton *button, gpointer user_data) {
  CajaMimeApplicationChooser *chooser;
  GError *error;
  GAppInfo *info;
  ...
  if (info) {
    error = NULL;
    if (!g_app_info_remove_supports_type(info, chooser->details->content_type,
                                         &error)) {
      eel_show_error_dialog(
          _("Could not remove application"), error->message,
          GTK_WINDOW(gtk_widget_get_toplevel(GTK_WIDGET(chooser))));
      g_error_free(error);
    }
    g_signal_emit_by_name(caja_signaller_get_current(), "mime_data_changed");
...
static void caja_mime_application_chooser_init(
    CajaMimeApplicationChooser *chooser) {
  GtkWidget *box;
  GtkWidget *scrolled;
  GtkWidget *button;
  ...
  gtk_box_pack_start(GTK_BOX(chooser), chooser->details->label, FALSE, FALSE,
                     0);
  ...
  gtk_container_add(GTK_CONTAINER(scrolled), chooser->details->treeview);
  ...
  g_signal_connect(button, "clicked", G_CALLBACK(add_clicked_cb), chooser);
  ...
  g_signal_connect(button, "clicked", G_CALLBACK(remove_clicked_cb), chooser);
  ...
  g_signal_connect(button, "clicked", G_CALLBACK(reset_clicked_cb), chooser);

  gtk_widget_show(button);
  gtk_container_add(GTK_CONTAINER(box), button);

  g_signal_connect(caja_signaller_get_current(), "mime_data_changed",
                   G_CALLBACK(mime_type_data_changed_cb), chooser);
}
...
static gboolean refresh_model_timeout(gpointer data) {
...
static void refresh_model_soon(CajaMimeApplicationChooser *chooser) {
  if (chooser->details->refresh_timeout != 0) return;

  chooser->details->refresh_timeout =
      g_timeout_add(300, refresh_model_timeout, chooser);
}
...
static void refresh_model(CajaMimeApplicationChooser *chooser) {
  ...
  column =
      gtk_tree_view_get_column(GTK_TREE_VIEW(chooser->details->treeview), 0);
  ...
  default_app =
      g_app_info_get_default_for_type(chooser->details->content_type, FALSE);
  ...
  for (l = applications; l != NULL; l = l->next) {
    ...
    escaped =
        g_markup_escape_text(g_app_info_get_display_name(application), -1);

    icon = g_app_info_get_icon(application);

    gtk_list_store_append(chooser->details->model, &iter);
    gtk_list_store_set(chooser->details->model, &iter, COLUMN_APPINFO,
                       application, COLUMN_DEFAULT, is_default, COLUMN_ICON,
                       icon, COLUMN_NAME, escaped, -1);
  ...
  if (applications) {
    g_object_set(chooser->details->toggle_renderer, "visible", TRUE, NULL);
    ...
    gtk_list_store_set(chooser->details->model, &iter, COLUMN_NAME, name,
                       COLUMN_APPINFO, NULL, -1);
    g_free(name);
...
static void set_extension_and_description(CajaMimeApplicationChooser *chooser,
...
static gboolean set_uri_and_type(CajaMimeApplicationChooser *chooser,
                                 const char *uri, const char *mime_type) {
  char *label;
  char *name;
  char *emname;
  char *extension;
...
static char *get_extension_from_file(CajaFile *nfile) {
  char *name;
  char *extension;
...
static gboolean set_uri_and_type_for_multiple_files(
    CajaMimeApplicationChooser *chooser, GList *uris, const char *mime_type) {
  char *label;
  ...
  if (!same_extension) {
    set_extension_and_description(CAJA_MIME_APPLICATION_CHOOSER(chooser), NULL,
                                  mime_type);
  } else {
    set_extension_and_description(CAJA_MIME_APPLICATION_CHOOSER(chooser),
