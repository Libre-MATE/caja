...
static void lock(void) { g_mutex_lock(&log_mutex); }

static void unlock(void) { g_mutex_unlock(&log_mutex); }
...
static gboolean is_domain_enabled(const char *domain) {
  /* User actions are always logged */
  if (strcmp(domain, CAJA_DEBUG_LOG_DOMAIN_USER) == 0) return TRUE;

  if (!domains_hash) return FALSE;
...
static void add_to_ring(char *str) {
  ensure_ring();
...
static void add_to_milestones(const char *str) {
  char *str_copy;
...
void caja_debug_logv(gboolean is_milestone, const char *domain,
  ...
  if (!(is_milestone || is_domain_enabled(domain))) goto out;
  ...
  debug_str =
      g_strdup_printf("%p %s (%s): %s", g_thread_self(), date_str, domain, str);
  g_free(date_str);
#else
  gettimeofday(&tv, NULL);
  tm = *localtime(&tv.tv_sec);
  debug_str = g_strdup_printf("%p %04d/%02d/%02d %02d:%02d:%02d.%04d (%s): %s",
                              g_thread_self(), tm.tm_year + 1900, tm.tm_mon + 1,
                              tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec,
                              (int)(tv.tv_usec / 100), domain, str);
#endif
  g_free(str);
  ...
  if (uris) {
    ...
    new_str = g_new(
        char, debug_str_len + 1 + uris_len + 1); /* plus 1 for newline & zero */
  ...
  if (is_milestone) add_to_milestones(debug_str);
...
void caja_debug_log_with_file_list(gboolean is_milestone, const char *domain,
  ...
  if (!(is_milestone || caja_debug_log_is_domain_enabled(domain))) {
    return;
  }
...
gboolean caja_debug_log_load_configuration(const char *filename,
  ...
  strings = g_key_file_get_string_list(
      key_file, KEY_FILE_GROUP, KEY_FILE_DOMAINS_KEY, &num_strings, &my_error);
  if (my_error)
    g_error_free(my_error);
  else {
    gsize i;

    for (i = 0; i < num_strings; i++) strings[i] = g_strstrip(strings[i]);
  ...
  my_error = NULL;
  num = g_key_file_get_integer(key_file, KEY_FILE_GROUP, KEY_FILE_MAX_LINES_KEY,
                               &my_error);
  if (my_error)
    g_error_free(my_error);
  else
...
void caja_debug_log_enable_domains(const char **domains, int n_domains) {
  ...
  if (!domains_hash) domains_hash = g_hash_table_new(g_str_hash, g_str_equal);
...
static void domains_foreach_dump_cb(gpointer key, gpointer value,
                                    gpointer data) {
...
static GKeyFile *make_key_file_from_configuration(void) {
  GKeyFile *key_file;
  struct domains_dump_closure closure;
  ...
  g_key_file_set_integer(key_file, KEY_FILE_GROUP, KEY_FILE_MAX_LINES_KEY,
                         ring_buffer_max_lines);

  return key_file;
}

static gboolean write_string(const char *filename, FILE *file, const char *str,
                             GError **error) {
...
static gboolean dump_configuration(const char *filename, FILE *file,
                                   GError **error) {
  GKeyFile *key_file;
  char *data;
  gsize length;
...
static gboolean dump_milestones(const char *filename, FILE *file,
                                GError **error) {
  GSList *l;
...
static gboolean dump_ring_buffer(const char *filename, FILE *file,
                                 GError **error) {
  int start_index;
  int i;
...
void caja_debug_log_set_max_lines(int num_lines) {
  ...
  if (num_lines == ring_buffer_max_lines) goto out;
  ...
  if (ring_buffer) {
    ...
    if (ring_buffer_num_lines == ring_buffer_max_lines)
      start_index =
          (ring_buffer_next_index + ring_buffer_max_lines - lines_to_copy) %
          ring_buffer_max_lines;
    else
    ...
    for (i = 0; i < ring_buffer_max_lines; i++) g_free(ring_buffer[i]);
