...
enum { COLUMN_VISIBLE, COLUMN_LABEL, COLUMN_NAME, NUM_COLUMNS };
...
G_DEFINE_TYPE_WITH_PRIVATE(CajaColumnChooser, caja_column_chooser,
                           GTK_TYPE_BOX);

static void caja_column_chooser_constructed(GObject *object);

static void caja_column_chooser_set_property(GObject *object, guint param_id,
                                             const GValue *value,
                                             GParamSpec *pspec) {
...
static void caja_column_chooser_class_init(
    CajaColumnChooserClass *chooser_class) {
  GObjectClass *oclass;
  ...
  signals[CHANGED] = g_signal_new(
      "changed", G_TYPE_FROM_CLASS(chooser_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaColumnChooserClass, changed), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  signals[USE_DEFAULT] = g_signal_new(
      "use_default", G_TYPE_FROM_CLASS(chooser_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaColumnChooserClass, use_default), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
...
static void update_buttons(CajaColumnChooser *chooser) {
  ...
  if (gtk_tree_selection_get_selected(selection, NULL, &iter)) {
    ...
    gtk_tree_model_get(GTK_TREE_MODEL(chooser->details->store), &iter,
                       COLUMN_VISIBLE, &visible, -1);

    path =
        gtk_tree_model_get_path(GTK_TREE_MODEL(chooser->details->store), &iter);
...
static void list_changed(CajaColumnChooser *chooser) {
...
static void visible_toggled_callback(GtkCellRendererToggle *cell,
  ...
  gtk_tree_model_get(GTK_TREE_MODEL(chooser->details->store), &iter,
                     COLUMN_VISIBLE, &visible, -1);
  gtk_list_store_set(chooser->details->store, &iter, COLUMN_VISIBLE, !visible,
                     -1);
  gtk_tree_path_free(path);
...
static void add_tree_view(CajaColumnChooser *chooser) {
  GtkWidget *scrolled;
  GtkWidget *view;
  GtkListStore *store;
  ...
  g_signal_connect(selection, "changed", G_CALLBACK(selection_changed_callback),
                   chooser);
  ...
  gtk_tree_view_insert_column_with_attributes(
      GTK_TREE_VIEW(view), -1, NULL, cell, "active", COLUMN_VISIBLE, NULL);
...
static void move_up_clicked_callback(GtkWidget *button, gpointer user_data) {
  ...
  if (gtk_tree_selection_get_selected(selection, NULL, &iter)) {
    ...
    if (gtk_tree_model_get_iter(GTK_TREE_MODEL(chooser->details->store), &prev,
                                path)) {
      gtk_list_store_move_before(chooser->details->store, &iter, &prev);
    }
    gtk_tree_path_free(path);
  }
...
static void move_down_clicked_callback(GtkWidget *button, gpointer user_data) {
  ...
  if (gtk_tree_selection_get_selected(selection, NULL, &iter)) {
    ...
    if (gtk_tree_model_iter_next(GTK_TREE_MODEL(chooser->details->store),
                                 &next)) {
      gtk_list_store_move_after(chooser->details->store, &iter, &next);
    }
  }
...
static void use_default_clicked_callback(GtkWidget *button,
                                         gpointer user_data) {
  g_signal_emit(CAJA_COLUMN_CHOOSER(user_data), signals[USE_DEFAULT], 0);
}

static GtkWidget *button_new_with_mnemonic(const gchar *icon_name,
                                           const gchar *str) {
  GtkWidget *image;
  GtkWidget *button;
...
static void add_buttons(CajaColumnChooser *chooser) {
  GtkWidget *box;
  GtkWidget *separator;
  ...
  chooser->details->move_up_button =
      button_new_with_mnemonic("go-up", _("Move _Up"));
  g_signal_connect(chooser->details->move_up_button, "clicked",
                   G_CALLBACK(move_up_clicked_callback), chooser);
  gtk_widget_show_all(chooser->details->move_up_button);
  gtk_widget_set_sensitive(chooser->details->move_up_button, FALSE);
  gtk_box_pack_start(GTK_BOX(box), chooser->details->move_up_button, FALSE,
                     FALSE, 0);

  chooser->details->move_down_button =
      button_new_with_mnemonic("go-down", _("Move Dow_n"));
  g_signal_connect(chooser->details->move_down_button, "clicked",
                   G_CALLBACK(move_down_clicked_callback), chooser);
  gtk_widget_show_all(chooser->details->move_down_button);
  gtk_widget_set_sensitive(chooser->details->move_down_button, FALSE);
  gtk_box_pack_start(GTK_BOX(box), chooser->details->move_down_button, FALSE,
                     FALSE, 0);
  ...
  chooser->details->use_default_button =
      gtk_button_new_with_mnemonic(_("Use De_fault"));
  g_signal_connect(chooser->details->use_default_button, "clicked",
                   G_CALLBACK(use_default_clicked_callback), chooser);
  gtk_widget_show(chooser->details->use_default_button);
  gtk_box_pack_start(GTK_BOX(box), chooser->details->use_default_button, FALSE,
                     FALSE, 0);

  gtk_box_pack_start(GTK_BOX(chooser), box, FALSE, FALSE, 0);
}

static void populate_tree(CajaColumnChooser *chooser) {
  GList *columns;
  GList *l;
  ...
  for (l = columns; l != NULL; l = l->next) {
    ...
    gtk_list_store_set(chooser->details->store, &iter, COLUMN_VISIBLE, FALSE,
                       COLUMN_LABEL, label, COLUMN_NAME, name, -1);
...
static void caja_column_chooser_init(CajaColumnChooser *chooser) {
  chooser->details = caja_column_chooser_get_instance_private(chooser);

  g_object_set(G_OBJECT(chooser), "homogeneous", FALSE, "spacing", 8,
...
static void set_visible_columns(CajaColumnChooser *chooser,
  ...
  if (gtk_tree_model_get_iter_first(GTK_TREE_MODEL(chooser->details->store),
    ...
    do {
      ...
      gtk_list_store_set(chooser->details->store, &iter, COLUMN_VISIBLE,
                         visible, -1);
      g_free(name);

    } while (gtk_tree_model_iter_next(GTK_TREE_MODEL(chooser->details->store),
                                      &iter));
  }
...
static char **get_column_names(CajaColumnChooser *chooser,
  ...
  if (gtk_tree_model_get_iter_first(GTK_TREE_MODEL(chooser->details->store),
    ...
    do {
      ...
      gboolean visible;
      gtk_tree_model_get(GTK_TREE_MODEL(chooser->details->store), &iter,
                         COLUMN_VISIBLE, &visible, COLUMN_NAME, &name, -1);
...
static gboolean get_column_iter(CajaColumnChooser *chooser, CajaColumn *column,
                                GtkTreeIter *iter) {
  char *column_name;
...
static void set_column_order(CajaColumnChooser *chooser, char **column_order)
    ...
    if (get_column_iter(chooser, CAJA_COLUMN(l->data), &iter)) {
      ...
      if (path) {
        gtk_tree_model_get_iter(GTK_TREE_MODEL(chooser->details->store),
                                &before, path);
        gtk_list_store_move_after(chooser->details->store, &iter, &before);
        gtk_tree_path_next(path);

      } else {
        gtk_list_store_move_after(chooser->details->store, &iter, NULL);
      }
    }
  }
  gtk_tree_path_free(path);
