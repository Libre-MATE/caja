...
G_DEFINE_TYPE_WITH_PRIVATE(CajaFileConflictDialog, caja_file_conflict_dialog,
                           GTK_TYPE_DIALOG);

static void file_icons_changed(CajaFile *file, GtkWidget *widget) {
  cairo_surface_t *surface;

  surface = caja_file_get_icon_surface(file, CAJA_ICON_SIZE_LARGE, FALSE,
...
static void file_list_ready_cb(GList *files, gpointer user_data) {
  ...
  if (dest_is_dir) {
    if (source_is_dir) {
      primary_text = g_strdup_printf(_("Merge folder \"%s\"?"), dest_name);
  ...
  surface = caja_file_get_icon_surface(
      dest, CAJA_ICON_SIZE_LARGE, TRUE,
      gtk_widget_get_scale_factor(fcd->details->titles_vbox),
      CAJA_FILE_ICON_FLAGS_USE_THUMBNAILS);
  details->dest_image = gtk_image_new_from_surface(surface);
  gtk_box_pack_start(GTK_BOX(details->first_hbox), details->dest_image, FALSE,
                     FALSE, 0);
  gtk_widget_show(details->dest_image);
  cairo_surface_destroy(surface);

  surface = caja_file_get_icon_surface(
      src, CAJA_ICON_SIZE_LARGE, TRUE,
      gtk_widget_get_scale_factor(fcd->details->titles_vbox),
      CAJA_FILE_ICON_FLAGS_USE_THUMBNAILS);
  details->src_image = gtk_image_new_from_surface(surface);
  gtk_box_pack_start(GTK_BOX(details->second_hbox), details->src_image, FALSE,
                     FALSE, 0);
  ...
  date = caja_file_get_string_attribute(dest, "date_modified");
  ...
  /* Second label */
  label = gtk_label_new(NULL);
  date = caja_file_get_string_attribute(src, "date_modified");
  ...
  g_signal_connect_object(src, "changed", G_CALLBACK(file_icons_changed),
                          fcd->details->src_image, 0);
  g_signal_connect_object(dest, "changed", G_CALLBACK(file_icons_changed),
                          fcd->details->dest_image, 0);
}

static void build_dialog_appearance(CajaFileConflictDialog *fcd) {
  GList *files = NULL;
  ...
  caja_file_list_call_when_ready(files, CAJA_FILE_ATTRIBUTES_FOR_ICON,
                                 &details->handle, file_list_ready_cb, fcd);
  g_list_free(files);
}

static void set_source_and_destination(GtkWidget *w, GFile *source,
...
static void entry_text_changed_cb(GtkEditable *entry,
  ...
  if (g_strcmp0(gtk_entry_get_text(GTK_ENTRY(entry)), "") != 0 &&
      g_strcmp0(gtk_entry_get_text(GTK_ENTRY(entry)), details->conflict_name) !=
          0) {
...
static void expander_activated_cb(GtkExpander *w,
                                  CajaFileConflictDialog *dialog) {
  CajaFileConflictDialogPrivate *details;
  ...
  if (!gtk_expander_get_expanded(w)) {
    if (g_strcmp0(gtk_entry_get_text(GTK_ENTRY(details->entry)),
      ...
      eel_filename_get_rename_region(details->conflict_name, &start_pos,
                                     &end_pos);
      gtk_editable_select_region(GTK_EDITABLE(details->entry), start_pos,
                                 end_pos);
    }
  }
}
...
static void checkbox_toggled_cb(GtkToggleButton *t,
  ...
  gtk_widget_set_sensitive(details->expander, !gtk_toggle_button_get_active(t));
...
static void reset_button_clicked_cb(GtkButton *w,
  ...
  gtk_widget_grab_focus(details->entry);
  eel_filename_get_rename_region(details->conflict_name, &start_pos, &end_pos);
  gtk_editable_select_region(GTK_EDITABLE(details->entry), start_pos, end_pos);
}

static void diff_button_clicked_cb(GtkButton *w,
                                   CajaFileConflictDialog *dialog) {
  CajaFileConflictDialogPrivate *details;
  details = dialog->details;
  ...
  if (command) {
    ...
    if (!g_spawn_async_with_pipes(
            ...
            NULL /* user_data */, NULL, NULL, NULL, NULL, &error)) {
...
static void caja_file_conflict_dialog_init(CajaFileConflictDialog *fcd) {
  ...
  details->expander = gtk_expander_new_with_mnemonic(
      _("Select a new name for the _destination"));
  ...
  g_signal_connect(widget, "changed", G_CALLBACK(entry_text_changed_cb),
                   dialog);
  ...
  g_signal_connect(widget, "clicked", G_CALLBACK(reset_button_clicked_cb),
                   dialog);
  ...
  gtk_button_set_image(
      GTK_BUTTON(details->diff_button),
  ...
  widget = gtk_check_button_new_with_mnemonic(
      _("Apply this action to all files and folders"));

  gtk_box_pack_start(GTK_BOX(vbox), widget, FALSE, FALSE, 0);
  details->checkbox = widget;
  g_signal_connect(widget, "toggled", G_CALLBACK(checkbox_toggled_cb), dialog);
  ...
  details->rename_button =
      gtk_dialog_add_button(dialog, _("Re_name"), CONFLICT_RESPONSE_RENAME);
  gtk_widget_hide(details->rename_button);

  details->replace_button =
      gtk_dialog_add_button(dialog, _("Replace"), CONFLICT_RESPONSE_REPLACE);
...
static void do_dispose(GObject *self) {
  CajaFileConflictDialogPrivate *details =
  ...
  if (details->handle != NULL) {
    ...
    if (details->source) caja_file_monitor_remove(details->source, self);
...
static void caja_file_conflict_dialog_class_init(
    CajaFileConflictDialogClass *klass) {
...
char *caja_file_conflict_dialog_get_new_name(CajaFileConflictDialog *dialog) {
  return g_strdup(gtk_entry_get_text(GTK_ENTRY(dialog->details->entry)));
}

gboolean caja_file_conflict_dialog_get_apply_to_all(
    CajaFileConflictDialog *dialog) {
  return gtk_toggle_button_get_active(
      GTK_TOGGLE_BUTTON(dialog->details->checkbox));
}

GtkWidget *caja_file_conflict_dialog_new(GtkWindow *parent, GFile *source,
                                         GFile *destination, GFile *dest_dir) {
  GtkWidget *dialog;
  CajaFile *src, *dest;
  ...
  if (source_is_dir) {
    dialog = GTK_WIDGET(g_object_new(
        CAJA_TYPE_FILE_CONFLICT_DIALOG, "title",
        dest_is_dir ? _("Merge Folder") : _("File and Folder conflict"), NULL));
  } else {
    dialog = GTK_WIDGET(g_object_new(
        CAJA_TYPE_FILE_CONFLICT_DIALOG, "title",
        dest_is_dir ? _("File and Folder conflict") : _("File conflict"),
        NULL));
  }
