...
enum { APPEARANCE_CHANGED, CONTENTS_CHANGED, LAST_SIGNAL };
...
static void caja_bookmark_finalize(GObject *object) {
...
static void caja_bookmark_class_init(CajaBookmarkClass *class) {
  G_OBJECT_CLASS(class)->finalize = caja_bookmark_finalize;

  signals[APPEARANCE_CHANGED] = g_signal_new(
      "appearance_changed", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaBookmarkClass, appearance_changed), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  signals[CONTENTS_CHANGED] = g_signal_new(
      "contents_changed", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaBookmarkClass, contents_changed), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
}

static void caja_bookmark_init(CajaBookmark *bookmark) {
 ...
 * (GCompareFunc style)
 **/
int caja_bookmark_compare_with(gconstpointer a, gconstpointer b) {
  ...
  if (g_strcmp0(bookmark_a->details->name, bookmark_b->details->name) != 0) {
    return 1;
  }
 ...
 * (GCompareFunc style)
 **/
int caja_bookmark_compare_uris(gconstpointer a, gconstpointer b) {
  CajaBookmark *bookmark_a;
  CajaBookmark *bookmark_b;
...
CajaBookmark *caja_bookmark_copy(CajaBookmark *bookmark) {
  ...
  return caja_bookmark_new(bookmark->details->location, bookmark->details->name,
 ...
 **/
gboolean caja_bookmark_set_name(CajaBookmark *bookmark, const char *new_name) {
...
static gboolean caja_bookmark_icon_is_different(CajaBookmark *bookmark,
                                                GIcon *new_icon) {
 ...
 */
static gboolean caja_bookmark_update_icon(CajaBookmark *bookmark) {
  GIcon *new_icon;
...
static void bookmark_file_changed_callback(CajaFile *file,
                                           CajaBookmark *bookmark) {
  GFile *location;
  ...
  if (caja_file_is_gone(file) || caja_file_is_in_trash(file)) {
 ...
 */
static void caja_bookmark_set_icon_to_default(CajaBookmark *bookmark) {
...
static void caja_bookmark_disconnect_file(CajaBookmark *bookmark) {
  g_assert(CAJA_IS_BOOKMARK(bookmark));
...
static void caja_bookmark_connect_file(CajaBookmark *bookmark) {
  char *display_name;
  ...
  if (!bookmark->details->has_custom_name && bookmark->details->file &&
      caja_file_check_if_ready(bookmark->details->file,
...
CajaBookmark *caja_bookmark_new(GFile *location, const char *name,
                                gboolean has_custom_name, GIcon *icon) {
...
static GtkWidget *bookmark_image_menu_item_new_from_surface(
    cairo_surface_t *icon_surface, const gchar *label_name) {
  GtkWidget *icon;
  GtkLabel *label;
  gchar *concat;
...
GtkWidget *caja_bookmark_menu_item_new(CajaBookmark *bookmark) {
  ...
  if (strlen(bookmark->details->name) > 0) {
    ...
    menu_item = bookmark_image_menu_item_new_from_surface(
        surface, bookmark->details->name);
    if (surface != NULL) {
...
void caja_bookmark_set_scroll_pos(CajaBookmark *bookmark, const char *uri) {
  g_free(bookmark->details->scroll_file);
