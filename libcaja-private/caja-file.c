...
static GQuark attribute_name_q, attribute_size_q, attribute_size_on_disk_q,
    attribute_type_q, attribute_creation_date_q, attribute_date_created_q,
    attribute_modification_date_q, attribute_date_modified_q,
    attribute_accessed_date_q, attribute_date_accessed_q, attribute_emblems_q,
    attribute_extension_q, attribute_mime_type_q, attribute_size_detail_q,
    attribute_size_on_disk_detail_q, attribute_deep_size_q,
    attribute_deep_size_on_disk_q, attribute_deep_file_count_q,
    attribute_deep_directory_count_q, attribute_deep_total_count_q,
    attribute_date_changed_q, attribute_trashed_on_q,
    attribute_trash_orig_path_q, attribute_date_permissions_q,
    attribute_permissions_q, attribute_selinux_context_q,
    attribute_octal_permissions_q, attribute_owner_q, attribute_group_q,
    attribute_uri_q, attribute_where_q, attribute_link_target_q,
    attribute_volume_q, attribute_free_space_q;
...
static gboolean update_info_and_name(CajaFile *file, GFileInfo *info);
...
static void caja_file_init(CajaFile *file) {
...
static GObject *caja_file_constructor(GType type, guint n_construct_properties,
                                      GObjectConstructParam *construct_params) {
  GObject *object;
  CajaFile *file;

  object = (*G_OBJECT_CLASS(caja_file_parent_class)->constructor)(
...
gboolean caja_file_set_display_name(CajaFile *file, const char *display_name,
                                    const char *edit_name, gboolean custom) {
  gboolean changed;
...
static void caja_file_clear_display_name(CajaFile *file) {
  g_clear_pointer(&file->details->display_name, g_ref_string_release);
...
static void clear_metadata(CajaFile *file) {
...
gboolean caja_file_update_metadata_from_info(CajaFile *file, GFileInfo *info) {
...
static CajaFile *caja_file_new_from_filename(CajaDirectory *directory,
                                             const char *filename,
                                             gboolean self_owned) {
  CajaFile *file;
...
static void modify_link_hash_table(CajaFile *file,
  ...
  if (file->details->symlink_name == NULL ||
      !caja_file_is_symbolic_link(file)) {
    return;
  }
...
static void add_to_link_hash_table_list(GList **list, CajaFile *file) {
  if (g_list_find(*list, file) != NULL) {
    g_warning(
        "Adding file to symlink_table multiple times. "
        "Please add feedback of what you were doing at "
        "https://bugzilla.gnome.org/show_bug.cgi?id=358172\n");
    return;
  }
...
static void add_to_link_hash_table(CajaFile *file) {
  modify_link_hash_table(file, add_to_link_hash_table_list);
}

static void remove_from_link_hash_table_list(GList **list, CajaFile *file) {
  if (g_list_find(*list, file) != NULL) {
...
static void remove_from_link_hash_table(CajaFile *file) {
  modify_link_hash_table(file, remove_from_link_hash_table_list);
}

CajaFile *caja_file_new_from_info(CajaDirectory *directory, GFileInfo *info) {
  CajaFile *file;
  ...
  if (mime_type && strcmp(mime_type, CAJA_SAVED_SEARCH_MIMETYPE) == 0) {
...
static CajaFile *caja_file_get_internal(GFile *location, gboolean create) {
  gboolean self_owned;
...
GDriveStartStopType caja_file_get_start_stop_type(CajaFile *file) {
  ...
  if (ret != G_DRIVE_START_STOP_TYPE_UNKNOWN) goto out;
...
void caja_file_mount(CajaFile *file, GMountOperation *mount_op,
  ...
  if (CAJA_FILE_GET_CLASS(file)->mount == NULL) {
    ...
    CAJA_FILE_GET_CLASS(file)->mount(file, mount_op, cancellable, callback,
                                     callback_data);
  }
}
...
void caja_file_unmount(CajaFile *file, GMountOperation *mount_op,
                       GCancellable *cancellable,
                       CajaFileOperationCallback callback,
                       gpointer callback_data) {
  GError *error;
  ...
  if (file->details->can_unmount) {
    if (CAJA_FILE_GET_CLASS(file)->unmount != NULL) {
      CAJA_FILE_GET_CLASS(file)->unmount(file, mount_op, cancellable, callback,
                                         callback_data);
    } else {
      if (callback) {
        error = NULL;
    ...
    caja_file_operations_unmount_mount_full(NULL, file->details->mount, FALSE,
                                            TRUE, unmount_done, data);
  } else if (callback) {
    callback(file, NULL, NULL, callback_data);
  }
}

void caja_file_eject(CajaFile *file, GMountOperation *mount_op,
                     GCancellable *cancellable,
                     CajaFileOperationCallback callback,
                     gpointer callback_data) {
  GError *error;
  ...
  if (file->details->can_eject) {
    if (CAJA_FILE_GET_CLASS(file)->eject != NULL) {
      CAJA_FILE_GET_CLASS(file)->eject(file, mount_op, cancellable, callback,
                                       callback_data);
    } else {
      if (callback) {
        error = NULL;
    ...
    data->callback = callback;
    data->callback_data = callback_data;
    caja_file_operations_unmount_mount_full(NULL, file->details->mount, TRUE,
                                            TRUE, unmount_done, data);
  } else if (callback) {
    callback(file, NULL, NULL, callback_data);
  }
...
void caja_file_start(CajaFile *file, GMountOperation *start_op,
                     GCancellable *cancellable,
                     CajaFileOperationCallback callback,
                     gpointer callback_data) {
  GError *error;
  ...
  if ((file->details->can_start || file->details->can_start_degraded) &&
    ...
    CAJA_FILE_GET_CLASS(file)->start(file, start_op, cancellable, callback,
                                     callback_data);
  } else {
    if (callback) {
      error = NULL;
...
void caja_file_stop(CajaFile *file, GMountOperation *mount_op,
                    GCancellable *cancellable,
                    CajaFileOperationCallback callback,
                    gpointer callback_data) {
  GError *error;
  ...
  if (CAJA_FILE_GET_CLASS(file)->stop != NULL) {
    if (file->details->can_stop) {
      CAJA_FILE_GET_CLASS(file)->stop(file, mount_op, cancellable, callback,
                                      callback_data);
    } else {
      if (callback) {
        error = NULL;
...
static gboolean is_desktop_file(CajaFile *file) {
  return caja_file_is_mime_type(file, "application/x-desktop");
}

static gboolean can_rename_desktop_file(CajaFile *file) {
  GFile *location;
  gboolean res;
...
static void caja_file_operation_remove(CajaFileOperation *op) {
  op->file->details->operations_in_progress =
...
void caja_file_operation_complete(CajaFileOperation *op, GFile *result_file,
                                  GError *error) {
  ...
  if (new_info != NULL) {
    ...
    if (op->file->details->got_custom_display_name) {
      caja_file_invalidate_attributes(
          op->file, CAJA_FILE_ATTRIBUTE_INFO | CAJA_FILE_ATTRIBUTE_LINK_INFO);
    }
...
static void rename_callback(GObject *source_object, GAsyncResult *res,
  ...
  if (new_file != NULL) {
    ...
    caja_undostack_manager_data_set_rename_information(
        op->undo_redo_data, G_FILE(source_object), new_file);
    // End UNDO-REDO
    g_file_query_info_async(new_file, CAJA_FILE_DEFAULT_ATTRIBUTES, 0,
...
static gboolean name_is(CajaFile *file, const char *new_name) {
  const char *old_name;
  old_name = file->details->name;
  return strcmp(new_name, old_name) == 0;
}

void caja_file_rename(CajaFile *file, const char *new_name,
                      CajaFileOperationCallback callback,
                      gpointer callback_data) {
  CajaFileOperation *op;
  char *old_name;
   ...
   */
  if (!CAJA_IS_DESKTOP_ICON_FILE(file) && !is_renameable_desktop_file &&
  ...
  if (!caja_undostack_manager_is_undo_redo(caja_undostack_manager_instance())) {
    op->undo_redo_data =
        caja_undostack_manager_data_new(CAJA_UNDOSTACK_RENAME, 1);
  }
  // End UNDO-REDO
...
void caja_file_cancel(CajaFile *file, CajaFileOperationCallback callback,
                      gpointer callback_data) {
  GList *node, *next;
  CajaFileOperation *op = NULL;
...
gboolean caja_file_matches_uri(CajaFile *file, const char *match_uri) {
...
int caja_file_compare_location(CajaFile *file_1, CajaFile *file_2) {
  GFile *loc_a, *loc_b;
  gboolean res;
...
static void update_link(CajaFile *link_file, CajaFile *target_file) {
...
static GList *get_link_files(CajaFile *target_file) {
  GList **link_files;
...
static void update_links_if_target(CajaFile *target_file) {
  GList *link_files, *p;
...
static gboolean update_info_internal(CajaFile *file, GFileInfo *info,
  ...
  changed |=
      caja_file_set_display_name(file, g_file_info_get_display_name(info),
  ...
  if (g_file_info_has_attribute(info,
    ...
    size_on_disk = g_file_info_get_attribute_uint64(
        info, G_FILE_ATTRIBUTE_STANDARD_ALLOCATED_SIZE);
  }
  ...
  if (file->details->atime != atime || file->details->mtime != mtime ||
      file->details->ctime != ctime || file->details->btime != btime) {
  ...
  trash_orig_path = g_file_info_get_attribute_byte_string(
  ...
  if (update_name) {
    ...
    if (file->details->name == NULL || strcmp(file->details->name, name) != 0) {
      ...
      node =
          caja_directory_begin_file_name_change(file->details->directory, file);
      ...
      caja_directory_end_file_name_change(file->details->directory, file, node);
    }
  }
...
gboolean caja_file_update_info(CajaFile *file, GFileInfo *info) {
...
static gboolean update_name_internal(CajaFile *file, const char *name,
...
gboolean caja_file_update_name(CajaFile *file, const char *name) {
  gboolean ret;
...
gboolean caja_file_update_name_and_directory(CajaFile *file, const char *name,
...
void caja_file_set_directory(CajaFile *file, CajaDirectory *new_directory) {
  caja_file_update_name_and_directory(file, NULL, new_directory);
}

static Knowledge get_item_count(CajaFile *file, guint *count) {
  gboolean known, unreadable;

  known = caja_file_get_directory_item_count(file, count, &unreadable);
  if (!known) {
    return UNKNOWN;
  }
  if (unreadable) {
...
static Knowledge get_size(CajaFile *file, goffset *size,
...
static Knowledge get_time(CajaFile *file, time_t *time_out, CajaDateType type) {
  time_t time;
...
static int compare_directories_by_count(CajaFile *file_1, CajaFile *file_2) {
  /* Sort order:
...
static int compare_files_by_size(CajaFile *file_1, CajaFile *file_2,
                                 gboolean size_on_disk) {
  /* Sort order:
...
static int compare_by_size(CajaFile *file_1, CajaFile *file_2,
                           gboolean size_on_disk) {
  /* Sort order:
   *   Directories with n items
   *   Directories with 0 items
...
static int compare_by_display_name(CajaFile *file_1, CajaFile *file_2) {
...
static int compare_by_directory_name(CajaFile *file_1, CajaFile *file_2) {
...
static gboolean file_has_note(CajaFile *file) {
  char *note;
  gboolean res;
...
static GList *prepend_automatic_keywords(CajaFile *file, GList *names) {
  ...
  if (caja_file_is_in_trash(file)) {
    names = g_list_prepend(names, g_strdup(CAJA_FILE_EMBLEM_NAME_TRASH));
  }
#endif
  if (file_has_note(file)) {
    names = g_list_prepend(names, g_strdup(CAJA_FILE_EMBLEM_NAME_NOTE));
  }

  /* Trash files are assumed to be read-only,
   * so we want to ignore them here. */
  if (!caja_file_can_write(file) && !caja_file_is_in_trash(file) &&
      (parent == NULL || caja_file_can_write(parent))) {
    names = g_list_prepend(names, g_strdup(CAJA_FILE_EMBLEM_NAME_CANT_WRITE));
  }
  if (!caja_file_can_read(file)) {
    names = g_list_prepend(names, g_strdup(CAJA_FILE_EMBLEM_NAME_CANT_READ));
  }
  if (caja_file_is_symbolic_link(file)) {
    names =
        g_list_prepend(names, g_strdup(CAJA_FILE_EMBLEM_NAME_SYMBOLIC_LINK));
  }
...
static void fill_emblem_cache_if_needed(CajaFile *file) {
  GList *node, *keywords;
  char *scanner;
  size_t length;
  ...
  file->details->compare_by_emblem_cache =
      g_malloc(sizeof(CajaFileSortByEmblemCache) + length);
...
static int compare_by_emblems(CajaFile *file_1, CajaFile *file_2) {
...
static int compare_by_type(CajaFile *file_1, CajaFile *file_2) {
...
static int compare_by_time(CajaFile *file_1, CajaFile *file_2,
                           CajaDateType type) {
  /* Sort order:
...
static int compare_by_full_path(CajaFile *file_1, CajaFile *file_2) {
  int compare;
 ...
 */
static char *prev_extension_segment(char *basename, int *rem_chars) {
  if (*basename == '.') {
    *basename = 0;
    basename--;
    (*rem_chars)--;
 ...
 */
static gboolean is_valid_extension_segment(const char *segment,
                                           int segment_index) {
  gboolean result;
...
static int compare_by_extension_segments(CajaFile *file_1, CajaFile *file_2) {
  char *name_1, *name_2;
  ...
  do {
    ...
    done_1 = rem_chars_1 <= 0 ||
             !is_valid_extension_segment(segment_1, segment_index);
    done_2 = rem_chars_2 <= 0 ||
             !is_valid_extension_segment(segment_2, segment_index);
    if (done_1 && !done_2) {
      compare = -1;
      break;
...
static gchar *caja_file_get_extension_as_string(CajaFile *file) {
  int rem_chars;
  char *segment;
  ...
  if (!caja_file_is_directory(file)) {
    ...
    if (rem_chars > 0 && is_valid_extension_segment(segment, 0)) {
      ...
      do {
        ...
        if (rem_chars > 0 &&
            is_valid_extension_segment(segment, segment_index)) {
...
static int caja_file_compare_for_sort_internal(CajaFile *file_1,
                                               CajaFile *file_2,
 ...
 **/
int caja_file_compare_for_sort(CajaFile *file_1, CajaFile *file_2,
                               CajaFileSortType sort_type,
                               gboolean directories_first, gboolean reversed) {
  int result;
  ...
  result = caja_file_compare_for_sort_internal(file_1, file_2,
...
int caja_file_compare_for_sort_by_attribute_q(CajaFile *file_1,
  ...
  if (attribute == 0 || attribute == attribute_name_q) {
    ...
    return caja_file_compare_for_sort(file_1, file_2, CAJA_FILE_SORT_BY_SIZE,
                                      directories_first, reversed);
    ...
    return caja_file_compare_for_sort(file_1, file_2, CAJA_FILE_SORT_BY_TYPE,
                                      directories_first, reversed);
  } else if (attribute == attribute_modification_date_q ||
             attribute == attribute_date_modified_q) {
    return caja_file_compare_for_sort(file_1, file_2, CAJA_FILE_SORT_BY_MTIME,
                                      directories_first, reversed);
  } else if (attribute == attribute_creation_date_q ||
             attribute == attribute_date_created_q) {
    return caja_file_compare_for_sort(file_1, file_2, CAJA_FILE_SORT_BY_BTIME,
                                      directories_first, reversed);
  } else if (attribute == attribute_accessed_date_q ||
             attribute == attribute_date_accessed_q) {
    return caja_file_compare_for_sort(file_1, file_2, CAJA_FILE_SORT_BY_ATIME,
                                      directories_first, reversed);
    ...
    return caja_file_compare_for_sort(file_1, file_2, CAJA_FILE_SORT_BY_EMBLEMS,
                                      directories_first, reversed);
  ...
  if (result == 0) {
    ...
    value_1 = caja_file_get_string_attribute_q(file_1, attribute);
    value_2 = caja_file_get_string_attribute_q(file_2, attribute);
...
int caja_file_compare_for_sort_by_attribute(CajaFile *file_1, CajaFile *file_2,
                                            const char *attribute,
                                            gboolean directories_first,
                                            gboolean reversed) {
  return caja_file_compare_for_sort_by_attribute_q(
 ...
 **/
int caja_file_compare_display_name(CajaFile *file, const char *pattern) {
  const char *name;
  int result;
...
static gboolean caja_file_is_backup_file(CajaFile *file) {
 ...
 */
gboolean caja_file_should_show(CajaFile *file, gboolean show_hidden,
                               gboolean show_foreign, gboolean show_backup) {
  ...
  if (caja_file_is_in_trash(file)) {
    ...
    return (show_hidden || !caja_file_is_hidden_file(file)) &&
           ...
           (show_foreign || !(caja_file_is_in_desktop(file) &&
                              caja_file_is_foreign_link(file)));
  }
}
...
gboolean caja_file_is_in_desktop(CajaFile *file) {
  if (file->details->directory->details->location) {
    return caja_is_desktop_directory(
        file->details->directory->details->location);
  }
  return FALSE;
}

static gboolean filter_hidden_partition_callback(gpointer data,
                                                 gpointer callback_data) {
  CajaFile *file;
  ...
  return caja_file_should_show(file, options & SHOW_HIDDEN, TRUE,
                               options & SHOW_BACKUP);
}

GList *caja_file_list_filter_hidden(GList *files, gboolean show_hidden) {
  ...
  filtered_files = eel_g_list_partition(
      filtered_files, filter_hidden_partition_callback,
      GINT_TO_POINTER((show_hidden ? SHOW_HIDDEN : 0)), &removed_files);
  caja_file_list_free(removed_files);

  return filtered_files;
}

char *caja_file_get_metadata(CajaFile *file, const char *key,
...
GList *caja_file_get_metadata_list(CajaFile *file, const char *key) {
  guint id;
  char **value;
...
void caja_file_set_metadata(CajaFile *file, const char *key,
                            const char *default_metadata,
                            const char *metadata) {
  const char *val;
  ...
  EEL_CALL_METHOD(CAJA_FILE_CLASS, file, set_metadata, (file, key, val));
}

void caja_file_set_metadata_list(CajaFile *file, const char *key, GList *list) {
  char **val;
  int len, i;
  GList *l;
  ...
  EEL_CALL_METHOD(CAJA_FILE_CLASS, file, set_metadata_as_list,
                  (file, key, val));

  g_free(val);
}

gboolean caja_file_get_boolean_metadata(CajaFile *file, const char *key,
  ...
  result_as_string =
      caja_file_get_metadata(file, key, default_metadata ? "true" : "false");
...
int caja_file_get_integer_metadata(CajaFile *file, const char *key,
  ...
  g_snprintf(default_as_string, sizeof(default_as_string), "%d",
             default_metadata);
  result_as_string = caja_file_get_metadata(file, key, default_as_string);
...
time_t caja_file_get_time_metadata(CajaFile *file, const char *key) {
  time_t time;
  char *time_string;
...
void caja_file_set_time_metadata(CajaFile *file, const char *key, time_t time) {
  char time_str[21];
  char *metadata;
...
void caja_file_set_boolean_metadata(CajaFile *file, const char *key,
                                    gboolean default_metadata,
                                    gboolean metadata) {
  g_return_if_fail(CAJA_IS_FILE(file));
  g_return_if_fail(key != NULL);
  ...
  caja_file_set_metadata(file, key, default_metadata ? "true" : "false",
                         metadata ? "true" : "false");
}

void caja_file_set_integer_metadata(CajaFile *file, const char *key,
  ...
  caja_file_set_metadata(file, key, default_as_string, value_as_string);
}
...
static const char *caja_file_peek_display_name(CajaFile *file) {
  ...
  Subject: Prevent a crash in nautilus_file_peek_display_name() on invalid
  NautilusFile This is more a workaround only, expect assert failures at other
  ...
  */
  if (file == NULL || caja_file_is_gone(file)) return "";
  ...
  if (file->details->display_name == NULL) {
    ...
    if (g_utf8_validate(name, -1, NULL)) {
      caja_file_set_display_name(file, name, NULL, FALSE);
    } else {
      char *escaped_name;

      escaped_name =
          g_uri_escape_string(name, G_URI_RESERVED_CHARS_ALLOWED_IN_PATH, TRUE);
      caja_file_set_display_name(file, escaped_name, NULL, FALSE);
      g_free(escaped_name);
    }
  }
...
void caja_file_monitor_add(CajaFile *file, gconstpointer client,
                           CajaFileAttributes attributes) {
  g_return_if_fail(CAJA_IS_FILE(file));
  g_return_if_fail(client != NULL);

  EEL_CALL_METHOD(CAJA_FILE_CLASS, file, monitor_add,
                  (file, client, attributes));
}

void caja_file_monitor_remove(CajaFile *file, gconstpointer client) {
  g_return_if_fail(CAJA_IS_FILE(file));
  g_return_if_fail(client != NULL);

  EEL_CALL_METHOD(CAJA_FILE_CLASS, file, monitor_remove, (file, client));
}
...
char *caja_file_get_drop_target_uri(CajaFile *file) {
  ...
  if (caja_file_is_caja_link(file)) {
    location = caja_file_get_location(file);
    /* FIXME bugzilla.gnome.org 43020: This does sync. I/O and works only
     * locally. */
...
static char *get_custom_icon_metadata_uri(CajaFile *file) {
  char *custom_icon_uri;
  char *uri;

  uri = caja_file_get_metadata(file, CAJA_METADATA_KEY_CUSTOM_ICON, NULL);
  if (uri != NULL && caja_file_is_directory(file) && is_uri_relative(uri)) {
    char *dir_uri;
...
static GIcon *get_custom_icon(CajaFile *file) {
  char *custom_icon_uri;
  GFile *icon_file;
  GIcon *icon;
  ...
  if (icon == NULL && file->details->got_link_info &&
...
GIcon *caja_file_get_gicon(CajaFile *file, CajaFileIconFlags flags) {
  ...
  if (file->details->icon) {
    ...
    if (((flags & CAJA_FILE_ICON_FLAGS_EMBEDDING_TEXT) ||
      ...
      if (is_folder && (flags & CAJA_FILE_ICON_FLAGS_IGNORE_VISITING) == 0 &&
          caja_file_has_open_window(file)) {
        g_ptr_array_add(prepend_array, "folder-visiting");
      }
      if (is_folder && (flags & CAJA_FILE_ICON_FLAGS_FOR_DRAG_ACCEPT)) {
    ...
    if ((flags & CAJA_FILE_ICON_FLAGS_USE_MOUNT_ICON) && mount_icon != NULL) {
      g_object_unref(icon);
      icon = mount_icon;
...
static GIcon *get_default_file_icon(CajaFileIconFlags flags) {
  ...
  if (fallback_icon == NULL) {
    ...
    g_themed_icon_append_name(G_THEMED_ICON(fallback_icon_preview),
                              "text-x-generic");
  }
  if (flags & CAJA_FILE_ICON_FLAGS_EMBEDDING_TEXT) {
...
CajaIconInfo *caja_file_get_icon(CajaFile *file, int size, int scale,
  ...
  if (flags & CAJA_FILE_ICON_FLAGS_USE_THUMBNAILS &&
    ...
    if (flags & CAJA_FILE_ICON_FLAGS_FORCE_THUMBNAIL_SIZE) {
      ...
      modified_size =
          size * scale * cached_thumbnail_size / CAJA_ICON_SIZE_STANDARD;
    }
    ...
    if (file->details->thumbnail) {
      ...
      scaled_pixbuf =
          gdk_pixbuf_scale_simple(raw_pixbuf, MAX(w * thumb_scale, 1),
      ...
      return icon;
               ...
               file->details->can_read && !file->details->is_thumbnailing &&
...
cairo_surface_t *caja_file_get_icon_surface(CajaFile *file, int size,
...
gboolean caja_file_get_date(CajaFile *file, CajaDateType date_type,
                            time_t *date) {
  if (date != NULL) {
    *date = 0;
  }
  ...
  g_return_val_if_fail(date_type == CAJA_DATE_TYPE_CHANGED ||
                           ...
                           date_type == CAJA_DATE_TYPE_PERMISSIONS_CHANGED,
                       FALSE);
  ...
  return EEL_CALL_METHOD_WITH_RETURN_VALUE(CAJA_FILE_CLASS, file, get_date,
                                           (file, date_type, date));
}

static char *caja_file_get_where_string(CajaFile *file) {
  if (file == NULL) {
    return NULL;
  }
  ...
  return EEL_CALL_METHOD_WITH_RETURN_VALUE(CAJA_FILE_CLASS, file,
...
static char *caja_file_fit_date_as_string(
    CajaFile *file, CajaDateType date_type, int width,
    CajaWidthMeasureCallback measure_callback,
    CajaTruncateCallback truncate_callback, void *measure_context) {
   ...
   * measuring the resulting string width directly, measure the width of a
   * template that represents the widest possible version of a date in a given
   * format. This is done by using M, m and 0 for the variable letters/digits
   * respectively.
   */
  format = NULL;
 ...
 * @truncate_callback: The callback used to truncate the string to a desired
 *width.
 ...
 *
 **/
char *caja_file_fit_modified_date_as_string(
    CajaFile *file, int width, CajaWidthMeasureCallback measure_callback,
    CajaTruncateCallback truncate_callback, void *measure_context) {
  return caja_file_fit_date_as_string(file, CAJA_DATE_TYPE_MODIFIED, width,
                                      measure_callback, truncate_callback,
                                      measure_context);
}

static char *caja_file_get_trash_original_file_parent_as_string(
    CajaFile *file) {
 ...
 *
 **/
static char *caja_file_get_date_as_string(CajaFile *file,
                                          CajaDateType date_type) {
  return caja_file_fit_date_as_string(file, date_type, 0, NULL, NULL, NULL);
}

static CajaSpeedTradeoffValue show_directory_item_count;
static CajaSpeedTradeoffValue show_text_in_icons;

static void show_text_in_icons_changed_callback(gpointer callback_data) {
  show_text_in_icons = g_settings_get_enum(caja_preferences,
...
static void show_directory_item_count_changed_callback(gpointer callback_data) {
  show_directory_item_count = g_settings_get_enum(
      caja_preferences, CAJA_PREFERENCES_SHOW_DIRECTORY_ITEM_COUNTS);
}

static gboolean get_speed_tradeoff_preference_for_file(
    CajaFile *file, CajaSpeedTradeoffValue value) {
  GFilesystemPreviewType use_preview;
...
gboolean caja_file_should_show_type(CajaFile *file) {
  ...
  ret = ((strcmp(uri, "computer:///") != 0) &&
         (strcmp(uri, "network:///") != 0) && (strcmp(uri, "smb:///") != 0));
  g_free(uri);
...
gboolean caja_file_should_get_top_left_text(CajaFile *file) {
  ...
  if (!show_text_in_icons_callback_added) {
    g_signal_connect_swapped(
        ...
        G_CALLBACK(show_text_in_icons_changed_callback), NULL);
 ...
 *
 **/
gboolean caja_file_get_directory_item_count(CajaFile *file, guint *count,
  ...
  return EEL_CALL_METHOD_WITH_RETURN_VALUE(
      CAJA_FILE_CLASS, file, get_item_count, (file, count, count_unreadable));
}
 ...
 *
 **/
CajaRequestStatus caja_file_get_deep_counts(
    CajaFile *file, guint *directory_count, guint *file_count,
    guint *unreadable_directory_count, goffset *total_size,
    goffset *total_size_on_disk, gboolean force) {
  ...
  return EEL_CALL_METHOD_WITH_RETURN_VALUE(
      CAJA_FILE_CLASS, file, get_deep_counts,
...
goffset caja_file_get_size(CajaFile *file) {
  ...
  if (file->details->size == -1) return 0;
...
goffset caja_file_get_size_on_disk(CajaFile *file) {
  /* Before we have info on the file, we don't know the size. */
  if (file->details->size_on_disk == -1) return 0;
  return file->details->size_on_disk;
}

time_t caja_file_get_mtime(CajaFile *file) { return file->details->mtime; }
...
static void set_attributes_callback(GObject *source_object,
  ...
  if (res) {
    g_file_query_info_async(G_FILE(source_object), CAJA_FILE_DEFAULT_ATTRIBUTES,
...
void caja_file_set_attributes(CajaFile *file, GFileInfo *attributes,
                              CajaFileOperationCallback callback,
                              gpointer callback_data) {
  CajaFileOperation *op;
  GFile *location;
...
gboolean caja_file_can_set_permissions(CajaFile *file) {
  ...
  if (file->details->uid != -1 && caja_file_is_local(file)) {
    /* Check the user. */
    user_id = geteuid();
 ...
 **/
void caja_file_set_permissions(CajaFile *file, guint32 new_permissions,
                               CajaFileOperationCallback callback,
                               gpointer callback_data) {
  GFileInfo *info;
  ...
  if (!caja_undostack_manager_is_undo_redo(caja_undostack_manager_instance())) {
    CajaUndoStackActionData *undo_redo_data =
        caja_undostack_manager_data_new(CAJA_UNDOSTACK_SETPERMISSIONS, 1);
    caja_undostack_manager_data_set_file_permissions(
        undo_redo_data, caja_file_get_uri(file), file->details->permissions,
        new_permissions);
    caja_undostack_manager_add_action(caja_undostack_manager_instance(),
                                      undo_redo_data);
  }
...
static char *get_real_name(const char *name, const char *gecos) {
  ...
  if (eel_str_is_empty(real_name) || eel_strcmp(name, real_name) == 0 ||
 ...
 */
void caja_file_set_owner(CajaFile *file, const char *user_name_or_id,
                         CajaFileOperationCallback callback,
                         gpointer callback_data) {
  GError *error;
  GFileInfo *info;
  ...
  if (!caja_undostack_manager_is_undo_redo(caja_undostack_manager_instance())) {
    ...
    CajaUndoStackActionData *undo_redo_data =
        caja_undostack_manager_data_new(CAJA_UNDOSTACK_CHANGEOWNER, 1);
    caja_undostack_manager_data_set_owner_change_information(
        undo_redo_data, caja_file_get_uri(file), current_owner,
        user_name_or_id);
    caja_undostack_manager_add_action(caja_undostack_manager_instance(),
                                      undo_redo_data);
 ...
 */
static GList *caja_get_group_names_for_user(void) {
  GList *list;
  int count, i;
 ...
 * @callback_data: Parameter passed back with callback function.
 */
void caja_file_set_group(CajaFile *file, const char *group_name_or_id,
                         CajaFileOperationCallback callback,
                         gpointer callback_data) {
  GError *error;
  GFileInfo *info;
  ...
  if (!caja_undostack_manager_is_undo_redo(caja_undostack_manager_instance())) {
    ...
    CajaUndoStackActionData *undo_redo_data =
        caja_undostack_manager_data_new(CAJA_UNDOSTACK_CHANGEGROUP, 1);
    caja_undostack_manager_data_set_group_change_information(
        undo_redo_data, caja_file_get_uri(file), current_group,
        group_name_or_id);
    caja_undostack_manager_add_action(caja_undostack_manager_instance(),
                                      undo_redo_data);
 ...
 *
 **/
static char *caja_file_get_octal_permissions_as_string(CajaFile *file) {
  guint32 permissions;
 ...
 *
 **/
static char *caja_file_get_permissions_as_string(CajaFile *file) {
  guint32 permissions;
...
static char *caja_file_get_owner_as_string(CajaFile *file,
  ...
  if (file->details->owner_real == NULL) {
    ...
    user_name = g_strdup_printf("%s - %s", file->details->owner,
 ...
 * @file: CajaFile representing the file in question.
 * @size_on_disk: If TRUE will return the size on disk. If FALSE return file
 *size.
 *
 * Returns: Newly allocated string ready to display to the user.
 *
 **/
static char *caja_file_get_size_as_string(CajaFile *file,
                                          gboolean size_on_disk) {
  guint item_count;
  gboolean count_unreadable;
  goffset size;
  ...
  if (caja_file_is_directory(file)) {
    if (!caja_file_get_directory_item_count(file, &item_count,
                                            &count_unreadable)) {
      return NULL;
    }
 ...
 *
 **/
static char *caja_file_get_size_as_string_with_real_size(
    CajaFile *file, gboolean size_on_disk) {
  guint item_count;
  gboolean count_unreadable;
  goffset size;
  char *formatted;
  ...
  formatted_plus_real =
      g_strdup_printf(_("%s (%s bytes)"), formatted, real_size);
  g_free(real_size);
  g_free(formatted);
  return formatted_plus_real;
}

static char *caja_file_get_deep_count_as_string_internal(
    CajaFile *file, gboolean report_size, gboolean report_size_on_disk,
    gboolean report_directory_count, gboolean report_file_count) {
  ...
  /* Can't ask for more than one of those: size, size on disk or (directory
   * and/or file count) */
  ...
  g_assert(report_size || report_size_on_disk || report_directory_count ||
           report_file_count);
  ...
  status = caja_file_get_deep_counts(file, &directory_count, &file_count,
                                     &unreadable_count, &total_size,
                                     &total_size_on_disk, FALSE);
 ...
 * is responsible for g_free-ing this string.
 * @file: CajaFile representing the file in question.
 * @size_on_disk: if TRUE will return the size on disk, else return size of
 *file.
 *
 * Returns: Newly allocated string ready to display to the user.
 *
 **/
static char *caja_file_get_deep_size_as_string(CajaFile *file,
                                               gboolean size_on_disk) {
  if (size_on_disk) {
 ...
 *
 **/
static char *caja_file_get_deep_total_count_as_string(CajaFile *file) {
 ...
 *
 **/
static char *caja_file_get_deep_file_count_as_string(CajaFile *file) {
  return caja_file_get_deep_count_as_string_internal(file, FALSE, FALSE, FALSE,
                                                     TRUE);
}
 ...
 *
 **/
static char *caja_file_get_deep_directory_count_as_string(CajaFile *file) {
  return caja_file_get_deep_count_as_string_internal(file, FALSE, FALSE, TRUE,
                                                     FALSE);
}
 ...
 * set includes "name", "type", "mime_type", "size", "size_on_disk",
 *"deep_size", "deep_size_on_disk", "deep_directory_count", "deep_file_count",
 *"deep_total_count", "date_modified", "date_changed", "date_accessed",
 *"date_permissions", "owner", "group", "permissions", "octal_permissions",
 *"uri", "where", "link_target", "volume", "free_space", "selinux_context",
 *"trashed_on", "trashed_orig_path"
 *
 * Returns: Newly allocated string ready to display to the user, or NULL
 * if the value is unknown or @attribute_name is not supported.
 *
 **/
char *caja_file_get_string_attribute_q(CajaFile *file, GQuark attribute_q) {
  ...
  if (attribute_q == attribute_date_modified_q) {
    return caja_file_get_date_as_string(file, CAJA_DATE_TYPE_MODIFIED);
  }
  if (attribute_q == attribute_date_changed_q) {
    return caja_file_get_date_as_string(file, CAJA_DATE_TYPE_CHANGED);
  }
  if (attribute_q == attribute_date_accessed_q) {
    return caja_file_get_date_as_string(file, CAJA_DATE_TYPE_ACCESSED);
  }
  if (attribute_q == attribute_date_created_q) {
    return caja_file_get_date_as_string(file, CAJA_DATE_TYPE_CREATED);
  }
  if (attribute_q == attribute_trashed_on_q) {
    return caja_file_get_date_as_string(file, CAJA_DATE_TYPE_TRASHED);
  }
...
char *caja_file_get_string_attribute(CajaFile *file,
  ...
  return caja_file_get_string_attribute_q(file,
...
char *caja_file_get_string_attribute_with_default_q(CajaFile *file,
  ...
  if (attribute_q == attribute_deep_size_q) {
    status =
        caja_file_get_deep_counts(file, NULL, NULL, NULL, NULL, NULL, FALSE);
...
char *caja_file_get_string_attribute_with_default(CajaFile *file,
                                                  const char *attribute_name) {
  return caja_file_get_string_attribute_with_default_q(
      file, g_quark_from_string(attribute_name));
}

gboolean caja_file_is_date_sort_attribute_q(GQuark attribute_q) {
 ...
 *
 **/
static char *get_description(CajaFile *file) {
  const char *mime_type;
  char *description;
  ...
  if (g_content_type_is_unknown(mime_type) && caja_file_is_executable(file)) {
...
static char *update_description_for_link(CajaFile *file, char *string) {
  if (caja_file_is_symbolic_link(file)) {
    char *res;
 ...
 *
 **/
gboolean caja_file_is_mime_type(CajaFile *file, const char *mime_type) {
  g_return_val_if_fail(CAJA_IS_FILE(file), FALSE);
...
gboolean caja_file_is_launchable(CajaFile *file) {
  ...
  return type_can_be_executable && caja_file_can_get_permissions(file) &&
         caja_file_can_execute(file) && caja_file_is_executable(file) &&
 ...
 *
 **/
GList *caja_file_get_emblem_icons(CajaFile *file, char **exclude) {
  GList *keywords, *l;
  GList *icons;
  char *icon_names[2];
  char *keyword;
...
GList *caja_file_get_emblem_pixbufs(CajaFile *file, int size,
                                    gboolean force_size, char **exclude) {
  GList *icons, *l;
  GList *pixbufs;
  GdkPixbuf *pixbuf;
  GIcon *icon = NULL;
...
GList *caja_file_get_keywords(CajaFile *file) {
  ...
  keywords = caja_file_get_metadata_list(file, CAJA_METADATA_KEY_EMBLEMS);

  keywords =
      g_list_concat(keywords, g_list_copy_deep(file->details->extension_emblems,
 ...
 *
 **/
void caja_file_set_keywords(CajaFile *file, GList *keywords) {
  ...
  canonical_keywords =
      sort_keyword_list_and_remove_duplicates(g_list_copy(keywords));
  caja_file_set_metadata_list(file, CAJA_METADATA_KEY_EMBLEMS,
                              canonical_keywords);
...
void caja_file_set_mount(CajaFile *file, GMount *mount) {
  if (file->details->mount) {
    g_signal_handlers_disconnect_by_func(file->details->mount,
                                         file_mount_unmounted, file);
    g_object_unref(file->details->mount);
...
char *caja_file_get_volume_free_space(CajaFile *file) {
  ...
  if (directory->details->free_space != (guint64)-1) {
    if (g_settings_get_boolean(caja_preferences,
                               CAJA_PREFERENCES_USE_IEC_UNITS))
      res = g_format_size_full(directory->details->free_space,
 ...
 *
 **/
char *caja_file_peek_top_left_text(CajaFile *file, gboolean need_large_text,
...
void caja_file_mark_gone(CajaFile *file) {
  ...
  if (file->details->is_gone) return;
...
void caja_file_changed(CajaFile *file) {
  ...
  if (caja_file_is_self_owned(file)) {
    ...
    fake_list.next = NULL;
    fake_list.prev = NULL;
    caja_directory_emit_change_signals(file->details->directory, &fake_list);
  }
}
...
gboolean caja_file_check_if_ready(CajaFile *file,
  ...
  return EEL_CALL_METHOD_WITH_RETURN_VALUE(
      CAJA_FILE_CLASS, file, check_if_ready, (file, file_attributes));
}
...

{
  ...
  EEL_CALL_METHOD(CAJA_FILE_CLASS, file, call_when_ready,
                  (file, file_attributes, callback, callback_data));
}

void caja_file_cancel_call_when_ready(CajaFile *file, CajaFileCallback callback,
                                      gpointer callback_data) {
  g_return_if_fail(callback != NULL);
  ...
  EEL_CALL_METHOD(CAJA_FILE_CLASS, file, cancel_call_when_ready,
                  (file, callback, callback_data));
}

static void invalidate_directory_count(CajaFile *file) {
  file->details->directory_count_is_up_to_date = FALSE;
}

static void invalidate_deep_counts(CajaFile *file) {
  file->details->deep_counts_status = CAJA_REQUEST_NOT_STARTED;
}

static void invalidate_mime_list(CajaFile *file) {
  file->details->mime_list_is_up_to_date = FALSE;
}

static void invalidate_top_left_text(CajaFile *file) {
  file->details->top_left_text_is_up_to_date = FALSE;
}

static void invalidate_file_info(CajaFile *file) {
  file->details->file_info_is_up_to_date = FALSE;
}

static void invalidate_link_info(CajaFile *file) {
  file->details->link_info_is_up_to_date = FALSE;
}

static void invalidate_thumbnail(CajaFile *file) {
  file->details->thumbnail_is_up_to_date = FALSE;
}

static void invalidate_mount(CajaFile *file) {
...
void caja_file_invalidate_attributes_internal(
    CajaFile *file, CajaFileAttributes file_attributes) {
  Request request;
...
void caja_file_set_has_open_window(CajaFile *file, gboolean has_open_window) {
...
void caja_file_set_is_thumbnailing(CajaFile *file, gboolean is_thumbnailing) {
  g_return_if_fail(CAJA_IS_FILE(file));
...
void caja_file_invalidate_attributes(CajaFile *file,
  ...
  caja_directory_cancel_loading_file_attributes(file->details->directory, file,
                                                file_attributes);
...
CajaFileAttributes caja_file_get_all_attributes(void) {
  return CAJA_FILE_ATTRIBUTE_INFO | CAJA_FILE_ATTRIBUTE_LINK_INFO |
         ...
         CAJA_FILE_ATTRIBUTE_EXTENSION_INFO | CAJA_FILE_ATTRIBUTE_THUMBNAIL |
 ...
 * @list: GList of files.
 **/
GList *caja_file_list_ref(GList *list) {
 ...
 * @list: GList of files.
 **/
void caja_file_list_unref(GList *list) {
 ...
 * @list: GList of files.
 **/
void caja_file_list_free(GList *list) {
 ...
 * @list: GList of files.
 **/
GList *caja_file_list_copy(GList *list) {
  return g_list_copy(caja_file_list_ref(list));
}

static gboolean get_attributes_for_default_sort_type(CajaFile *file,
  ...
  *is_trash = FALSE;
  ...
  if (file && caja_file_is_directory(file)) {
    ...
    is_trash_dir = caja_file_is_in_trash(file);
 ...
 * @list: GList of files.
 **/
GList *caja_file_list_sort_by_display_name(GList *list) {
...
static FileListReadyData *file_list_ready_data_new(
    GList *file_list, CajaFileListCallback callback, gpointer callback_data) {
...
static void file_list_file_ready_callback(CajaFile *file, gpointer user_data) {
  FileListReadyData *data;
...
void caja_file_list_call_when_ready(GList *file_list,
  ...
  while (l != NULL) {
    ...
    if (file)
      caja_file_call_when_ready(file, attributes, file_list_file_ready_callback,
                                data);
  }
}
...
static char *try_to_make_utf8(const char *text, int *length) {
  ...
  for (i = 0; i < n_encodings_to_try; i++) {
    ...
    utf8_text = g_convert(text, *length, "UTF-8", encodings_to_try[i], NULL,
                          &converted_length, &conversion_error);
    if (utf8_text != NULL) {
...
char *caja_extract_top_left_text(const char *text, gboolean large, int length) {
  GString *buffer;
  const gchar *in;
  const gchar *end;
  int line, i;
  ...
  if (text != NULL) {
    ...
    if (!validated && !(length >= max_bytes && text + length - utf8_end < 6)) {
...
static void thumbnail_limit_changed_callback(gpointer user_data) {
  g_settings_get(caja_preferences, CAJA_PREFERENCES_IMAGE_FILE_THUMBNAIL_LIMIT,
...
static void thumbnail_size_changed_callback(gpointer user_data) {
  cached_thumbnail_size = g_settings_get_int(
      caja_icon_view_preferences, CAJA_PREFERENCES_ICON_VIEW_THUMBNAIL_SIZE);
...
static void show_thumbnails_changed_callback(gpointer user_data) {
  show_image_thumbs = g_settings_get_enum(
      caja_preferences, CAJA_PREFERENCES_SHOW_IMAGE_FILE_THUMBNAILS);
...
static void caja_file_class_init(CajaFileClass *class) {
  ...
  attribute_size_on_disk_detail_q =
      g_quark_from_static_string("size_on_disk_detail");
  attribute_deep_size_q = g_quark_from_static_string("deep_size");
  attribute_deep_size_on_disk_q =
      g_quark_from_static_string("deep_size_on_disk");
  ...
  signals[CHANGED] =
      g_signal_new("changed", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(CajaFileClass, changed), NULL, NULL,
                   g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  signals[UPDATED_DEEP_COUNT_IN_PROGRESS] = g_signal_new(
      "updated_deep_count_in_progress", G_TYPE_FROM_CLASS(class),
      G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaFileClass, updated_deep_count_in_progress), NULL,
      NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  eel_g_settings_add_auto_enum(caja_preferences, CAJA_PREFERENCES_DATE_FORMAT,
                               &date_format_pref);

  thumbnail_limit_changed_callback(NULL);
  g_signal_connect_swapped(
      caja_preferences, "changed::" CAJA_PREFERENCES_IMAGE_FILE_THUMBNAIL_LIMIT,
  ...
  g_signal_connect_swapped(
      ...
      G_CALLBACK(thumbnail_size_changed_callback), NULL);
  ...
  g_signal_connect(caja_signaller_get_current(), "mime_data_changed",
                   G_CALLBACK(mime_type_data_changed_callback), NULL);
}

static void caja_file_add_emblem(CajaFile *file, const char *emblem_name) {
  if (file->details->pending_info_providers) {
...
static void caja_file_add_string_attribute(CajaFile *file,
                                           const char *attribute_name,
                                           const char *value) {
  if (file->details->pending_info_providers) {
...
static void caja_file_invalidate_extension_info(CajaFile *file) {
...
void caja_self_check_file(void) {
  CajaFile *file_1;
  CajaFile *file_2;
  GList *list;
  ...
  EEL_CHECK_BOOLEAN_RESULT(caja_file_get_by_uri("file:///home/") == file_1,
                           TRUE);
  caja_file_unref(file_1);
  ...
  EEL_CHECK_BOOLEAN_RESULT(
      caja_file_compare_for_sort(file_1, file_2, CAJA_FILE_SORT_BY_DISPLAY_NAME,
                                 FALSE, FALSE) < 0,
      TRUE);
  EEL_CHECK_BOOLEAN_RESULT(
      caja_file_compare_for_sort(file_1, file_2, CAJA_FILE_SORT_BY_DISPLAY_NAME,
      ...
      TRUE);
  EEL_CHECK_BOOLEAN_RESULT(
      caja_file_compare_for_sort(file_1, file_1, CAJA_FILE_SORT_BY_DISPLAY_NAME,
                                 FALSE, FALSE) == 0,
      TRUE);
  EEL_CHECK_BOOLEAN_RESULT(
      caja_file_compare_for_sort(file_1, file_1, CAJA_FILE_SORT_BY_DISPLAY_NAME,
