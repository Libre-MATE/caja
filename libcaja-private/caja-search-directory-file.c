...
G_DEFINE_TYPE(CajaSearchDirectoryFile, caja_search_directory_file,
              CAJA_TYPE_FILE);

static void search_directory_file_monitor_add(CajaFile *file,
  ...
  caja_search_directory_file_update_display_name(
      CAJA_SEARCH_DIRECTORY_FILE(file));
}

static void search_directory_file_monitor_remove(CajaFile *file,
                                                 gconstpointer client) {
  /* Do nothing here, we don't have any monitors */
}

static void search_directory_file_call_when_ready(
...
static void search_directory_file_cancel_call_when_ready(
    CajaFile *file, CajaFileCallback callback, gpointer callback_data) {
  /* Do nothing here, we don't have any pending calls */
}

static gboolean search_directory_file_check_if_ready(
    CajaFile *file, CajaFileAttributes attributes) {
  return TRUE;
}

static gboolean search_directory_file_get_item_count(
...
static CajaRequestStatus search_directory_file_get_deep_counts(
...
static char *search_directory_file_get_where_string(CajaFile *file) {
...
static void caja_search_directory_file_init(
    CajaSearchDirectoryFile *search_file) {
  CajaFile *file;
...
static void caja_search_directory_file_class_init(
    CajaSearchDirectoryFileClass *klass) {
