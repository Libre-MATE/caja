...
void caja_application_notify_unmount_show(const gchar *message) {
  ...
  if (!g_settings_get_boolean(caja_preferences,
                              CAJA_PREFERENCES_SHOW_NOTIFICATIONS))
    return;

  if (unmount_notify == NULL) {
    unmount_notify =
        notify_notification_new(strings[0], strings[1], "media-removable");

    notify_notification_set_hint(unmount_notify, "transient",
                                 g_variant_new_boolean(TRUE));
    notify_notification_set_urgency(unmount_notify, NOTIFY_URGENCY_CRITICAL);
  } else {
    notify_notification_update(unmount_notify, strings[0], strings[1],
                               "media-removable");
  }
...
static char *format_time(int seconds) {
  ...
  return g_strdup_printf(
      ngettext("approximately %'d hour", "approximately %'d hours", hours),
      hours);
}
...
static char *get_link_name(const char *name, int count, int max_length) {
  ...
  if (max_length > 0 &&
      (unshortened_length = strlen(result)) > (size_t)max_length) {
    char *new_name;

    new_name =
        shorten_utf8_string(name, ((int)unshortened_length) - max_length);
    if (new_name) {
      g_free(result);
...
/* Translators: if in your language there's no difference between 1st, 2nd, 3rd
...
static char *make_next_duplicate_name(const char *base, const char *suffix,
  ...
  if (max_length > 0 &&
    ...
    new_base =
        shorten_utf8_string(base, ((int)unshortened_length) - max_length);
    if (new_base) {
      g_free(result);
...
static void custom_full_name_skip(va_list *va) { (void)va_arg(*va, GFile *); }
...
static void custom_basename_skip(va_list *va) { (void)va_arg(*va, GFile *); }
...
static void custom_size_skip(va_list *va) { (void)va_arg(*va, goffset); }
...
static void custom_time_skip(va_list *va) { (void)va_arg(*va, int); }
...
static void custom_mount_skip(va_list *va) { (void)va_arg(*va, GMount *); }
...
#define op_job_new(__type, parent_window, should_start, can_pause)     \
  ((__type *)(init_common(sizeof(__type), parent_window, should_start, \
                          can_pause)))
...
static gpointer init_common(gsize job_size, GtkWindow *parent_window,
  ...
  /* expected warning with Clang static analyzer: * "Cast a region whose size is
   ...
   * * (e.g. a TrashJob), but only initializing the common part of it
   * (CommonJob)  * which is a subset of all "real" job structures, structures
   * that all start   * with a CommonJob, and that thus can be used as such. */
...
static gboolean do_run_simple_dialog(gpointer _data) {
  ...
  for (response_id = 0; data->button_titles[response_id] != NULL;
    ...
    if (g_strcmp0(button_title, CANCEL) == 0)
      eel_dialog_add_button(GTK_DIALOG(dialog), button_title, "process-stop",
                            response_id);
    else if (g_strcmp0(button_title, DELETE) == 0)
      eel_dialog_add_button(GTK_DIALOG(dialog), button_title, "edit-delete",
                            response_id);
    else
...
static int run_simple_dialog_va(CommonJob *job, gboolean ignore_close_box,
  ...
  g_io_scheduler_job_send_to_mainloop(job->io_job, do_run_simple_dialog, data,
                                      NULL);
...
static gboolean should_confirm_move_to_trash(void) {
  GSettings *prefs;
  gboolean confirm_trash;

  prefs = g_settings_new("org.mate.caja.preferences");
  confirm_trash =
      g_settings_get_boolean(prefs, CAJA_PREFERENCES_CONFIRM_MOVE_TO_TRASH);
  g_object_unref(prefs);
  return confirm_trash;
}
...
static gboolean confirm_delete_from_trash(CommonJob *job, GList *files) {
  ...
  response = run_warning(
      job, prompt, f(_("If you delete an item, it will be permanently lost.")),
      NULL, FALSE, CANCEL, DELETE, NULL);
...
static gboolean confirm_empty_trash(CommonJob *job) {
  ...
  response = run_warning(
      job, prompt, f(_("All items in the Trash will be permanently deleted.")),
      NULL, FALSE, CANCEL, _("Empty _Trash"), NULL);
...
static gboolean confirm_trash(CommonJob *job, GList *files) {
  char *prompt;
  int file_count;
  int response;
  ...
  if (file_count == 1) {
    prompt = f(_("Are you sure you want to trash \"%B\"?"), files->data);
  } else {
  ...
  response = run_warning(job, prompt,
                         f(_("Items moved to the trash may be recovered until "
                             "the trash is emptied.")),
                         NULL, FALSE, CANCEL, _("Move to _Trash"), NULL);
...
static void report_delete_progress(CommonJob *job, SourceInfo *source_info,
  ...
  files_left_s = f(ngettext("%'d file left to delete",
                            "%'d files left to delete", files_left),
                   files_left);

  caja_progress_info_take_status(job->progress, f(_("Deleting files")));
  ...
  if (elapsed < SECONDS_NEEDED_FOR_RELIABLE_TRANSFER_RATE) {
     ...
     * (i.e. the %T argument).
     */
    time_left_s = f(ngettext("%T left", "%T left",
  ...
  if (source_info->num_files != 0) {
    caja_progress_info_set_progress(job->progress, transfer_info->num_files,
                                    source_info->num_files);
  }
}

static void delete_file(CommonJob *job, GFile *file, gboolean *skipped_file,
                        SourceInfo *source_info, TransferInfo *transfer_info,
                        gboolean toplevel);

static void delete_dir(CommonJob *job, GFile *dir, gboolean *skipped_file,
                       SourceInfo *source_info, TransferInfo *transfer_info,
                       gboolean toplevel) {
  GFileInfo *info;
  GError *error;
...
retry:
  ...
  if (enumerator) {
    ...
    while (!job_aborted(job) && (info = g_file_enumerator_next_file(
      ...
      delete_file(job, file, &local_skipped_file, source_info, transfer_info,
                  FALSE);
      g_object_unref(file);
      g_object_unref(info);
    }
    ...
    if (error && IS_IO_ERROR(error, CANCELLED)) {
      ...
      if (IS_IO_ERROR(error, PERMISSION_DENIED)) {
        ...
        secondary = f(_("There was an error getting information about the "
                        "files in the folder \"%B\"."),
                      dir);
  ...
  if (!job_aborted(job) &&
    ...
    if (!g_file_delete(dir, job->cancellable, &error)) {
      ...
      response =
          run_warning(job, primary, secondary, details,
...
static void delete_file(CommonJob *job, GFile *file, gboolean *skipped_file,
  ...
  if (IS_IO_ERROR(error, NOT_EMPTY)) {
    g_error_free(error);
    delete_dir(job, file, skipped_file, source_info, transfer_info, toplevel);
    return;
...
static void delete_files(CommonJob *job, GList *files, guint *files_skipped) {
  GList *l;
  ...
  for (l = files; l != NULL && !job_aborted(job); l = l->next) {
    ...
    skipped_file = FALSE;
    delete_file(job, file, &skipped_file, &source_info, &transfer_info, TRUE);
    if (skipped_file) {
      (*files_skipped)++;
    }
  }
...
static void report_trash_progress(CommonJob *job, guint files_trashed,
                                  guint total_files) {
  guint files_left;
  char *s;
  ...
  caja_progress_info_take_status(job->progress, f(_("Moving files to trash")));

  s = f(
      ngettext("%'d file left to trash", "%'d files left to trash", files_left),
      files_left);
...
static void trash_files(CommonJob *job, GList *files, guint *files_skipped) {
  GList *l;
  GFile *file;
  GList *to_delete;
  GError *error;
  ...
  for (l = files; l != NULL && !job_aborted(job); l = l->next) {
    ...
    if (!g_file_trash(file, job->cancellable, &error)) {
      ...
      response = run_question(job, primary, secondary, details,
                              (total_files - files_trashed) > 1, CANCEL,
                              SKIP_ALL, SKIP, DELETE_ALL, DELETE, NULL);
    ...
    skip:
      ...
      // Start UNDO-REDO
      caja_undostack_manager_data_add_trashed_file(job->undo_redo_data, file,
                                                   mtime);
      // End UNDO-REDO
...
static gboolean delete_job_done(gpointer user_data) {
  DeleteJob *job;
...
static gboolean delete_job(GIOSchedulerJob *io_job, GCancellable *cancellable,
                           gpointer user_data) {
  ...
  g_io_scheduler_job_send_to_mainloop_async(io_job, delete_job_done, job, NULL);

  return FALSE;
}

static void trash_or_delete_internal(GList *files, GtkWindow *parent_window,
                                     gboolean try_trash,
                                     CajaDeleteCallback done_callback,
                                     gpointer done_callback_data) {
  DeleteJob *job;
  ...
  // Start UNDO-REDO
  // FIXME: Disabled, because of missing mechanism to restore a file from trash
  // in a clean way see
  // https://www.mail-archive.com/nautilus-list@gnome.org/msg04664.html
  if (try_trash &&
      !caja_undostack_manager_is_undo_redo(caja_undostack_manager_instance())) {
    job->common.undo_redo_data = caja_undostack_manager_data_new(
        CAJA_UNDOSTACK_MOVETOTRASH, g_list_length(files));
    GFile *src_dir = g_file_get_parent(files->data);
    caja_undostack_manager_data_set_src_dir(job->common.undo_redo_data,
                                            src_dir);
  }
  // End UNDO-REDO
...
void caja_file_operations_trash_or_delete(GList *files,
                                          ...
                                          CajaDeleteCallback done_callback,
                                          gpointer done_callback_data) {
  trash_or_delete_internal(files, parent_window, TRUE, done_callback,
                           done_callback_data);
}

void caja_file_operations_delete(GList *files, GtkWindow *parent_window,
                                 CajaDeleteCallback done_callback,
                                 gpointer done_callback_data) {
  trash_or_delete_internal(files, parent_window, FALSE, done_callback,
                           done_callback_data);
}
...
static gint prompt_empty_trash(GtkWindow *parent_window) {
  ...
  gtk_dialog_add_button(GTK_DIALOG(dialog), _("Do _not Empty Trash"),
                        GTK_RESPONSE_REJECT);

  eel_dialog_add_button(GTK_DIALOG(dialog), CANCEL, "process-stop",
                        GTK_RESPONSE_CANCEL);

  gtk_dialog_add_button(GTK_DIALOG(dialog), _("Empty _Trash"),
                        GTK_RESPONSE_ACCEPT);
...
void caja_file_operations_unmount_mount_full(GtkWindow *parent_window,
  ...
  if (check_trash && has_trash_files(mount)) {
    ...
    if (response == GTK_RESPONSE_ACCEPT) {
      ...
      g_io_scheduler_push_job(empty_trash_job, job, NULL, 0, NULL);
      return;
...
void caja_file_operations_unmount_mount(GtkWindow *parent_window, GMount *mount,
                                        gboolean eject, gboolean check_trash) {
...
static void volume_mount_cb(GObject *source_object, GAsyncResult *res,
  ...
  mount_callback = (CajaMountCallback)g_object_get_data(G_OBJECT(mount_op),
                                                        "mount-callback");
  mount_callback_data_object =
      g_object_get_data(G_OBJECT(mount_op), "mount-callback-data");

  if (mount_callback != NULL) {
    (*mount_callback)(G_VOLUME(source_object), mount_callback_data_object);
...
void caja_file_operations_mount_volume(GtkWindow *parent_window,
  ...
  caja_file_operations_mount_volume_full(parent_window, volume, allow_autorun,
                                         NULL, NULL);
}

void caja_file_operations_mount_volume_full(
    GtkWindow *parent_window, GVolume *volume, gboolean allow_autorun,
    CajaMountCallback mount_callback, GObject *mount_callback_data_object) {
  GMountOperation *mount_op;
  ...
  if (allow_autorun) caja_allow_autorun_for_volume(volume);
  g_volume_mount(volume, 0, mount_op, NULL, volume_mount_cb, mount_op);
}

static void report_count_progress(CommonJob *job, SourceInfo *source_info) {
  char *s;
  ...
  switch (source_info->op) {
    ...
    case OP_KIND_COPY:
      s = f(
          ngettext("Preparing to copy %'d file (%S)",
                   "Preparing to copy %'d files (%S)", source_info->num_files),
          source_info->num_files, source_info->num_bytes);
      break;
    case OP_KIND_MOVE:
      s = f(
          ngettext("Preparing to move %'d file (%S)",
                   "Preparing to move %'d files (%S)", source_info->num_files),
          source_info->num_files, source_info->num_bytes);
      break;
    case OP_KIND_DELETE:
    ...
    case OP_KIND_TRASH:
      s = f(ngettext("Preparing to trash %'d file",
                     "Preparing to trash %'d files", source_info->num_files),
            source_info->num_files);
      break;
  }
...
static void count_file(GFileInfo *info, CommonJob *job,
                       SourceInfo *source_info) {
...
retry:
  ...
  if (info == NULL) {
    ...
    if (IS_IO_ERROR(error, PERMISSION_DENIED)) {
      secondary =
          f(_("You do not have permissions to access the destination folder."));
    } else {
      secondary =
  ...
  if (file_type != G_FILE_TYPE_DIRECTORY) {
    ...
    response = run_error(job, primary, secondary, NULL, FALSE, CANCEL, NULL);
  ...
  if (required_size > 0 &&
    ...
    if (free_size < (guint64)required_size) {
      primary = f(_("Error while copying to \"%B\"."), dest);
      secondary =
          f(_("There is not enough space on the destination. Try to remove "
              "files to make space."));

      details = f(_("There is %" G_GUINT64_FORMAT
                    " available, but %" G_GOFFSET_FORMAT " is required."),
...
static void report_copy_progress(CopyMoveJob *copy_job, SourceInfo *source_info,
  ...
  if (files_left != transfer_info->last_reported_files_left ||
    ...
    if (source_info->num_files == 1) {
      ...
      if (copy_job->destination != NULL) {
        caja_progress_info_take_status(
            job->progress,
            f(is_move ? ngettext("Moving %'d file to \"%B\"",
                                 "Moving %'d files to \"%B\"", files_left)
                      : ngettext("Copying %'d file to \"%B\"",
                                 "Copying %'d files to \"%B\"", files_left),
              files_left, copy_job->destination));
      } else {
        caja_progress_info_take_status(
            job->progress, f(ngettext("Duplicating %'d file",
                                      "Duplicating %'d files", files_left),
                             files_left));
      }
    }
  }
  ...
  if (elapsed < SECONDS_NEEDED_FOR_RELIABLE_TRANSFER_RATE &&
     ...
     * time duration like "2 minutes". So the whole thing will be something like
     * "2 kb of 4 MB -- 2 hours left (4kb/sec)"
     *
     * The singular/plural form will be used depending on the remaining time
     * (i.e. the %T argument).
     */
    s = f(ngettext("%S of %S \xE2\x80\x94 %T left (%S/sec)",
                   ...
                   seconds_count_format_time_units(remaining_time)),
          transfer_info->num_bytes, total_size, remaining_time,
          (goffset)transfer_rate);
    caja_progress_info_take_details(job->progress, s);
  }

  caja_progress_info_set_progress(job->progress, transfer_info->num_bytes,
                                  total_size);
}
...
static int get_max_name_length(GFile *file_dir) {
  ...
  if (!g_file_has_uri_scheme(file_dir, "file")) return max_length;

  dir = g_file_get_path(file_dir);
  if (!dir) return max_length;
...
static gboolean str_replace(char *str, const char *chars_to_replace,
                            char replacement) {
  gboolean success;
  int i;
...
static GFile *get_target_file(GFile *src, GFile *dest_dir,
  ...
  if (!same_fs) {
    ...
    info = g_file_query_info(src, G_FILE_ATTRIBUTE_STANDARD_COPY_NAME, 0, NULL,
                             NULL);
...
static void copy_move_file(CopyMoveJob *job, GFile *src, GFile *dest_dir,
                           ...
                           TransferInfo *transfer_info,
                           GHashTable *debuting_files, GdkPoint *point,
                           gboolean overwrite, gboolean *skipped_file,
                           gboolean readonly_source_fs, gboolean last_item);
...
retry:
  ...
  // Start UNDO-REDO
  caja_undostack_manager_data_add_origin_target_pair(job->undo_redo_data, src,
                                                     *dest);
  // End UNDO-REDO
 ...
 * the new destination.
 */
static gboolean copy_move_directory(
    CopyMoveJob *copy_job, GFile *src, GFile **dest, gboolean same_fs,
    gboolean create_dest, char **parent_dest_fs_type, SourceInfo *source_info,
...
retry:
  ...
  if (enumerator) {
    ...
    nextinfo = g_file_enumerator_next_file(enumerator, job->cancellable,
                                           skip_error ? NULL : &error);
    while (!job_aborted(job) && (info = nextinfo) != NULL) {
      caja_progress_info_get_ready(job->progress);
      ...
      copy_move_file(copy_job, src_file, *dest, same_fs, FALSE, &dest_fs_type,
                     source_info, transfer_info, NULL, NULL, FALSE,
                     &local_skipped_file, readonly_source_fs, last_item);
      g_object_unref(src_file);
      g_object_unref(info);
    }
    if (nextinfo) g_object_unref(nextinfo);
  ...
  if (create_dest) {
    flags =
        (readonly_source_fs)
            ? G_FILE_COPY_NOFOLLOW_SYMLINKS | G_FILE_COPY_TARGET_DEFAULT_PERMS
...
static gboolean remove_target_recursively(CommonJob *job, GFile *src,
                                          GFile *toplevel_dest, GFile *file) {
  GFileEnumerator *enumerator;
  GError *error;
  GFile *child;
  gboolean stop;
  ...
  if (enumerator) {
    ...
    secondary = f(
        _("Could not remove files from the already existing folder %F."), file);
    details = error->message;
...
static gboolean is_trusted_desktop_file(GFile *file,
                                        GCancellable *cancellable) {
  char *basename;
  gboolean res;
  GFileInfo *info;
  ...
  info = g_file_query_info(
      file,
      G_FILE_ATTRIBUTE_STANDARD_TYPE "," G_FILE_ATTRIBUTE_ACCESS_CAN_EXECUTE,
...
static gboolean do_run_conflict_dialog(gpointer _data) {
  ConflictDialogData *data = _data;
  GtkWidget *dialog;
  int response;

  dialog = caja_file_conflict_dialog_new(data->parent, data->src, data->dest,
                                         data->dest_dir);
  response = gtk_dialog_run(GTK_DIALOG(dialog));

  if (response == CONFLICT_RESPONSE_RENAME) {
    data->resp_data->new_name = caja_file_conflict_dialog_get_new_name(
        CAJA_FILE_CONFLICT_DIALOG(dialog));
  } else if (response != GTK_RESPONSE_CANCEL && response != GTK_RESPONSE_NONE) {
    data->resp_data->apply_to_all = caja_file_conflict_dialog_get_apply_to_all(
        CAJA_FILE_CONFLICT_DIALOG(dialog));
  }
...
static ConflictResponseData *run_conflict_dialog(CommonJob *job, GFile *src,
                                                 GFile *dest, GFile *dest_dir) {
  ...
  caja_progress_info_pause(job->progress);
  g_io_scheduler_job_send_to_mainloop(job->io_job, do_run_conflict_dialog, data,
                                      NULL);
  caja_progress_info_resume(job->progress);
...
static void conflict_response_data_free(ConflictResponseData *data) {
  g_free(data->new_name);
  g_slice_free(ConflictResponseData, data);
}

static GFile *get_target_file_for_display_name(GFile *dir, char *name) {
  GFile *dest;
...
static void copy_move_file(CopyMoveJob *copy_job, GFile *src, GFile *dest_dir,
                           ...
                           char **dest_fs_type, SourceInfo *source_info,
                           TransferInfo *transfer_info,
                           GHashTable *debuting_files, GdkPoint *position,
                           gboolean overwrite, gboolean *skipped_file,
                           gboolean readonly_source_fs, gboolean last_item) {
  GFile *dest, *new_dest;
  GError *error;
  ...
  if (test_dir_is_parent(dest_dir, src)) {
    ...
    response =
        run_warning(job, primary, secondary, NULL,
                    (source_info->num_files - transfer_info->num_files) > 1,
                    CANCEL, SKIP_ALL, SKIP, NULL);
...
retry:
  ...
  if (res) {
    if (!copy_job->is_move) {
      /* Ignore errors here. Failure to copy metadata is not a hard error */
      g_file_copy_attributes(src, dest, flags | G_FILE_COPY_ALL_METADATA,
                             job->cancellable, NULL);
    }
    ...
    if (debuting_files) {
      if (position) {
        caja_file_changes_queue_schedule_position_set(dest, *position,
                                                      job->screen_num);
      } else {
  ...
  /* Needs to recurse */
    ...
    if (overwrite && would_recurse) {
      ...
      if (!g_file_delete(dest, job->cancellable, &error) &&
        ...
        secondary = f(_("Could not remove the already existing file with the "
                        "same name in %F."),
                      dest_dir);
        details = error->message;
    ...
    if (!copy_move_directory(copy_job, src, &dest, same_fs, would_recurse,
                             ...
                             debuting_files, skipped_file, readonly_source_fs,
                             last_item)) {
  ...
  for (l = job->files; l != NULL && !job_aborted(common); l = l->next) {
    ...
    if (dest) {
      ...
      copy_move_file(job, src, dest, same_fs, unique_names, &dest_fs_type,
                     source_info, transfer_info, job->debuting_files, point,
                     FALSE, &skipped_file, readonly_source_fs, !l->next);
      g_object_unref(dest);
    }
    i++;
  }
...
static gboolean copy_job_done(gpointer user_data) {
  CopyMoveJob *job;
...
static gboolean copy_job(GIOSchedulerJob *io_job, GCancellable *cancellable,
                         gpointer user_data) {
  CopyMoveJob *job;
  CommonJob *common;
  SourceInfo source_info;
...
aborted:
  ...
  g_io_scheduler_job_send_to_mainloop_async(io_job, copy_job_done, job, NULL);

  return FALSE;
}

static gboolean contains_multiple_items(GList *files) {
  GFile *first;
...
void caja_file_operations_copy(GList *files, GArray *relative_item_points,
                               GFile *target_dir, GtkWindow *parent_window,
                               CajaCopyCallback done_callback,
                               gpointer done_callback_data) {
  CopyMoveJob *job;

  job = op_job_new(CopyMoveJob, parent_window, FALSE,
                   contains_multiple_items(files));
  ...
  if (!caja_undostack_manager_is_undo_redo(caja_undostack_manager_instance())) {
    job->common.undo_redo_data = caja_undostack_manager_data_new(
        CAJA_UNDOSTACK_COPY, g_list_length(files));
    GFile *src_dir = g_file_get_parent(files->data);
    caja_undostack_manager_data_set_src_dir(job->common.undo_redo_data,
                                            src_dir);
    g_object_ref(target_dir);
    caja_undostack_manager_data_set_dest_dir(job->common.undo_redo_data,
                                             target_dir);
  }
  // End UNDO-REDO

  g_io_scheduler_push_job(copy_job, job, NULL, /* destroy notify */
                          0, job->common.cancellable);
}

static void report_move_progress(CopyMoveJob *move_job, int total, int left) {
  CommonJob *job;
  ...
  caja_progress_info_take_status(
      job->progress,
      f(_("Preparing to Move to \"%B\""), move_job->destination));

  caja_progress_info_take_details(
      job->progress, f(ngettext("Preparing to move %'d file",
...
static void move_file_prepare(CopyMoveJob *move_job, GFile *src,
                              ...
                              GdkPoint *position, GList **fallback_files,
                              int files_left) {
  GFile *dest, *new_dest;
  GError *error;
  CommonJob *job;
...
static void move_files_prepare(CopyMoveJob *job, const char *dest_fs_id,
  ...
  for (l = job->files; l != NULL && !job_aborted(common); l = l->next) {
    ...
    if (last_item)
      /* this is the last file and there are no fallbacks to process, cannot
       * pause anymore */
    ...
    move_file_prepare(job, src, job->destination, same_fs, dest_fs_type,
                      job->debuting_files, point, fallbacks, left);
    report_move_progress(job, total, --left);
    i++;
  }
...
static void move_files(CopyMoveJob *job, GList *fallbacks,
  ...
  for (l = fallbacks; l != NULL && !job_aborted(common); l = l->next) {
    ...
    copy_move_file(job, src, job->destination, same_fs, FALSE, dest_fs_type,
                   source_info, transfer_info, job->debuting_files, point,
                   fallback->overwrite, &skipped_file, FALSE, !l->next);
    i++;
  }
}

static gboolean move_job_done(gpointer user_data) {
  CopyMoveJob *job;
...
static gboolean move_job(GIOSchedulerJob *io_job, GCancellable *cancellable,
                         gpointer user_data) {
  CopyMoveJob *job;
  CommonJob *common;
  GList *fallbacks;
...
void caja_file_operations_move(GList *files, GArray *relative_item_points,
                               GFile *target_dir, GtkWindow *parent_window,
                               CajaCopyCallback done_callback,
                               gpointer done_callback_data) {
  CopyMoveJob *job;
  ...
  if (!caja_undostack_manager_is_undo_redo(caja_undostack_manager_instance())) {
    if (g_file_has_uri_scheme(g_list_first(files)->data, "trash")) {
      job->common.undo_redo_data = caja_undostack_manager_data_new(
          CAJA_UNDOSTACK_RESTOREFROMTRASH, g_list_length(files));
    } else {
      job->common.undo_redo_data = caja_undostack_manager_data_new(
          CAJA_UNDOSTACK_MOVE, g_list_length(files));
    }
    GFile *src_dir = g_file_get_parent(files->data);
    caja_undostack_manager_data_set_src_dir(job->common.undo_redo_data,
                                            src_dir);
  ...
  g_io_scheduler_push_job(move_job, job, NULL, /* destroy notify */
                          0, job->common.cancellable);
}

static void report_link_progress(CopyMoveJob *link_job, int total, int left) {
  CommonJob *job;

  job = (CommonJob *)link_job;

  caja_progress_info_take_status(
      job->progress, f(_("Creating links in \"%B\""), link_job->destination));

  caja_progress_info_take_details(
      job->progress,
      f(ngettext("Making link to %'d file", "Making links to %'d files", left),
        left));

  caja_progress_info_set_progress(job->progress, left, total);
}

static char *get_abs_path_for_symlink(GFile *file) {
  GFile *root, *parent;
  char *relative, *abs;
...
static void link_file(CopyMoveJob *job, GFile *src, GFile *dest_dir,
                      char **dest_fs_type, GHashTable *debuting_files,
                      GdkPoint *position, int files_left) {
...
retry:
  ...
  if (path == NULL) {
    not_local = TRUE;
                                       ...
                                       &error)) {
    // Start UNDO-REDO
    caja_undostack_manager_data_add_origin_target_pair(common->undo_redo_data,
                                                       src, dest);
    // End UNDO-REDO
    g_free(path);
    if (debuting_files) {
...
static gboolean link_job_done(gpointer user_data) {
  CopyMoveJob *job;
...
static gboolean link_job(GIOSchedulerJob *io_job, GCancellable *cancellable,
                         gpointer user_data) {
  CopyMoveJob *job;
  CommonJob *common;
  GFile *src;
...
void caja_file_operations_link(GList *files, GArray *relative_item_points,
                               GFile *target_dir, GtkWindow *parent_window,
                               CajaCopyCallback done_callback,
                               gpointer done_callback_data) {
  CopyMoveJob *job;
  ...
  if (!caja_undostack_manager_is_undo_redo(caja_undostack_manager_instance())) {
    job->common.undo_redo_data = caja_undostack_manager_data_new(
        CAJA_UNDOSTACK_CREATELINK, g_list_length(files));
    GFile *src_dir = g_file_get_parent(files->data);
    caja_undostack_manager_data_set_src_dir(job->common.undo_redo_data,
                                            src_dir);
    g_object_ref(target_dir);
  ...
  g_io_scheduler_push_job(link_job, job, NULL, /* destroy notify */
                          0, job->common.cancellable);
}

void caja_file_operations_duplicate(GList *files, GArray *relative_item_points,
                                    GtkWindow *parent_window,
                                    CajaCopyCallback done_callback,
                                    gpointer done_callback_data) {
  CopyMoveJob *job;
  ...
  if (!caja_undostack_manager_is_undo_redo(caja_undostack_manager_instance())) {
    job->common.undo_redo_data = caja_undostack_manager_data_new(
        CAJA_UNDOSTACK_DUPLICATE, g_list_length(files));
    GFile *src_dir = g_file_get_parent(files->data);
    caja_undostack_manager_data_set_src_dir(job->common.undo_redo_data,
                                            src_dir);
...
static gboolean set_permissions_job_done(gpointer user_data) {
...
static void set_permissions_file(SetPermissionsJob *job, GFile *file,
  ...
  if (!job_aborted(common) &&
    ...
    // Start UNDO-REDO
    caja_undostack_manager_data_add_file_permissions(common->undo_redo_data,
                                                     file, current);
    // End UNDO-REDO
...
static gboolean set_permissions_job(GIOSchedulerJob *io_job,
                                    GCancellable *cancellable,
                                    gpointer user_data) {
  ...
  caja_progress_info_set_status(common->progress, _("Setting permissions"));

  caja_progress_info_start(job->common.progress);

  set_permissions_file(job, job->file, NULL);

  g_io_scheduler_job_send_to_mainloop_async(io_job, set_permissions_job_done,
                                            job, NULL);

  return FALSE;
}

void caja_file_set_permissions_recursive(
    const char *directory, guint32 file_permissions, guint32 file_mask,
    guint32 dir_permissions, guint32 dir_mask, CajaOpCallback callback,
    gpointer callback_data) {
  SetPermissionsJob *job;
  ...
  if (!caja_undostack_manager_is_undo_redo(caja_undostack_manager_instance())) {
    job->common.undo_redo_data = caja_undostack_manager_data_new(
        CAJA_UNDOSTACK_RECURSIVESETPERMISSIONS, 1);
    g_object_ref(job->file);
    caja_undostack_manager_data_set_dest_dir(job->common.undo_redo_data,
                                             job->file);
    caja_undostack_manager_data_set_recursive_permissions(
        job->common.undo_redo_data, file_permissions, file_mask,
        dir_permissions, dir_mask);
  }
  // End UNDO-REDO

  g_io_scheduler_push_job(set_permissions_job, job, NULL, 0, NULL);
}
...
static void callback_for_move_to_trash(GHashTable *debuting_uris,
  ...
  if (data->real_callback) data->real_callback(debuting_uris, data->real_data);
  g_slice_free(MoveTrashCBData, data);
}

void caja_file_operations_copy_move(
    const GList *item_uris, GArray *relative_item_points,
    const char *target_dir, GdkDragAction copy_action, GtkWidget *parent_view,
    CajaCopyCallback done_callback, gpointer done_callback_data) {
  GList *locations;
  GList *p;
  GFile *src_dir;
  GFile *dest;
  ...
  if (copy_action == GDK_ACTION_COPY) {
    if (target_dir == NULL ||
      ...
      caja_file_operations_duplicate(locations, relative_item_points,
                                     parent_window, done_callback,
                                     done_callback_data);
    } else {
      caja_file_operations_copy(locations, relative_item_points, dest,
                                parent_window, done_callback,
                                done_callback_data);
    }
    ...
    if (g_file_has_uri_scheme(dest, "trash")) {
      ...
      caja_file_operations_trash_or_delete(
          locations, parent_window,
          (CajaDeleteCallback)callback_for_move_to_trash, cb_data);
    } else if (src_dir == NULL || !g_file_equal(src_dir, dest)) {
      caja_file_operations_move(locations, relative_item_points, dest,
                                parent_window, done_callback,
                                done_callback_data);
    }
  } else {
    caja_file_operations_link(locations, relative_item_points, dest,
...
static gboolean create_job_done(gpointer user_data) {
  CreateJob *job;
...
static gboolean create_job(GIOSchedulerJob *io_job, GCancellable *cancellable,
                           gpointer user_data) {
  CreateJob *job;
  CommonJob *common;
  int count;
...
retry:
  ...
  if (job->make_dir) {
    ...
    if (res) {
      caja_undostack_manager_data_set_create_data(common->undo_redo_data,
                                                  g_file_get_uri(dest), NULL);
    }
    // End UNDO-REDO
  } else {
    if (job->src) {
      res = g_file_copy(job->src, dest, G_FILE_COPY_NONE, common->cancellable,
                        NULL, NULL, &error);
      // Start UNDO-REDO
      if (res) {
        caja_undostack_manager_data_set_create_data(common->undo_redo_data,
      ...
      if (out) {
        ...
        if (res) {
          res = g_output_stream_close(G_OUTPUT_STREAM(out), common->cancellable,
                                      &error);
          // Start UNDO-REDO
          if (res) {
            caja_undostack_manager_data_set_create_data(
                common->undo_redo_data, g_file_get_uri(dest), g_strdup(data));
          }
          // End UNDO-REDO
        }
  ...
  if (res) {
    ...
    caja_file_changes_queue_file_added(dest);
    if (job->has_position) {
      caja_file_changes_queue_schedule_position_set(dest, job->position,
                                                    common->screen_num);
    } else {
      caja_file_changes_queue_schedule_position_remove(dest);
    }
      ...
      if (count == 1) {
        ...
        if (max_length > 0 && unshortened_length > (size_t)max_length) {
          new_filename = shorten_utf8_string(
              filename2, ((int)unshortened_length) - max_length);
        }
    ...
    /* Other error */
      ...
      secondary = f(_("There was an error creating the directory in %F."),
                    job->dest_dir);
      details = error->message;
...
aborted:
  ...
  g_free(filename);
  g_free(dest_fs_type);
  g_io_scheduler_job_send_to_mainloop_async(io_job, create_job_done, job, NULL);
...
void caja_file_operations_new_folder(GtkWidget *parent_view,
  ...
  if (!caja_undostack_manager_is_undo_redo(caja_undostack_manager_instance())) {
    job->common.undo_redo_data =
        caja_undostack_manager_data_new(CAJA_UNDOSTACK_CREATEFOLDER, 1);
  }
  // End UNDO-REDO

  g_io_scheduler_push_job(create_job, job, NULL, /* destroy notify */
                          0, job->common.cancellable);
}

void caja_file_operations_new_file_from_template(
    GtkWidget *parent_view, GdkPoint *target_point, const char *parent_dir,
    const char *target_filename, const char *template_uri,
    CajaCreateCallback done_callback, gpointer done_callback_data) {
  CreateJob *job;
  GtkWindow *parent_window;
  ...
  if (!caja_undostack_manager_is_undo_redo(caja_undostack_manager_instance())) {
    job->common.undo_redo_data = caja_undostack_manager_data_new(
        CAJA_UNDOSTACK_CREATEFILEFROMTEMPLATE, 1);
  }
  // End UNDO-REDO
...
void caja_file_operations_new_file(
    GtkWidget *parent_view, GdkPoint *target_point, const char *parent_dir,
    const char *target_filename, const char *initial_contents, int length,
    CajaCreateCallback done_callback, gpointer done_callback_data) {
  CreateJob *job;
  GtkWindow *parent_window;
  ...
  if (!caja_undostack_manager_is_undo_redo(caja_undostack_manager_instance())) {
    job->common.undo_redo_data =
        caja_undostack_manager_data_new(CAJA_UNDOSTACK_CREATEEMPTYFILE, 1);
  }
  // End UNDO-REDO
...
static gboolean empty_trash_job_done(gpointer user_data) {
  EmptyTrashJob *job;
...
static gboolean empty_trash_job(GIOSchedulerJob *io_job,
  ...
  g_io_scheduler_job_send_to_mainloop_async(io_job, empty_trash_job_done, job,
                                            NULL);
...
static gboolean mark_trusted_job_done(gpointer user_data) {
...
retry:
  ...
  error = NULL;
  if (!g_file_load_contents(file, cancellable, &contents, &length, NULL,
                            &error)) {
    if (interactive) {
      response = run_error(
          common, g_strdup(_("Unable to mark launcher trusted (executable)")),
          error->message, NULL, FALSE, CANCEL, RETRY, NULL);
    } else {
      response = 0;
    }
  ...
  if (!g_str_has_prefix(contents, "#!")) {
    ...
    if (!g_file_replace_contents(file, new_contents, new_length, NULL, FALSE, 0,
  ...
  info = g_file_query_info(
      file, G_FILE_ATTRIBUTE_STANDARD_TYPE "," G_FILE_ATTRIBUTE_UNIX_MODE,
      G_FILE_QUERY_INFO_NOFOLLOW_SYMLINKS, common->cancellable, &error);
  ...
  g_file_set_attribute_string(
      file, "metadata::" CAJA_METADATA_KEY_TRUSTED_LAUNCHER, "true",
      G_FILE_QUERY_INFO_NONE, cancellable, NULL);
out:;
}

static gboolean mark_trusted_job(GIOSchedulerJob *io_job,
                                 GCancellable *cancellable,
                                 gpointer user_data) {
  MarkTrustedJob *job = user_data;
  CommonJob *common;
  ...
  g_io_scheduler_job_send_to_mainloop_async(io_job, mark_trusted_job_done, job,
                                            NULL);

  return FALSE;
}

void caja_file_mark_desktop_file_trusted(GFile *file, GtkWindow *parent_window,
                                         gboolean interactive,
                                         CajaOpCallback done_callback,
                                         gpointer done_callback_data) {
  MarkTrustedJob *job;
  ...
  g_io_scheduler_push_job(mark_trusted_job, job, NULL, 0, NULL);
}

#if !defined(CAJA_OMIT_SELF_CHECK)

void caja_self_check_file_operations(void) {
  ...
  EEL_CHECK_STRING_RESULT(get_duplicate_name("foo (another copy)", 1, -1),
                          "foo (3rd copy)");
  EEL_CHECK_STRING_RESULT(get_duplicate_name("foo (another copy).txt", 1, -1),
                          "foo (3rd copy).txt");
  ...
  EEL_CHECK_STRING_RESULT(get_duplicate_name("foo (13th copy)", 1, -1),
                          "foo (14th copy)");
  EEL_CHECK_STRING_RESULT(get_duplicate_name("foo (13th copy).txt", 1, -1),
                          "foo (14th copy).txt");
  EEL_CHECK_STRING_RESULT(get_duplicate_name("foo (21st copy)", 1, -1),
                          "foo (22nd copy)");
  EEL_CHECK_STRING_RESULT(get_duplicate_name("foo (21st copy).txt", 1, -1),
                          "foo (22nd copy).txt");
  EEL_CHECK_STRING_RESULT(get_duplicate_name("foo (22nd copy)", 1, -1),
                          "foo (23rd copy)");
  EEL_CHECK_STRING_RESULT(get_duplicate_name("foo (22nd copy).txt", 1, -1),
                          "foo (23rd copy).txt");
  EEL_CHECK_STRING_RESULT(get_duplicate_name("foo (23rd copy)", 1, -1),
                          "foo (24th copy)");
  EEL_CHECK_STRING_RESULT(get_duplicate_name("foo (23rd copy).txt", 1, -1),
                          "foo (24th copy).txt");
  EEL_CHECK_STRING_RESULT(get_duplicate_name("foo (24th copy)", 1, -1),
                          "foo (25th copy)");
  EEL_CHECK_STRING_RESULT(get_duplicate_name("foo (24th copy).txt", 1, -1),
                          "foo (25th copy).txt");
  EEL_CHECK_STRING_RESULT(get_duplicate_name("foo foo (24th copy)", 1, -1),
                          "foo foo (25th copy)");
  EEL_CHECK_STRING_RESULT(get_duplicate_name("foo foo (24th copy).txt", 1, -1),
                          "foo foo (25th copy).txt");
  EEL_CHECK_STRING_RESULT(
      get_duplicate_name("foo foo (100000000000000th copy).txt", 1, -1),
      "foo foo (copy).txt");
  EEL_CHECK_STRING_RESULT(get_duplicate_name("foo (10th copy)", 1, -1),
                          "foo (11th copy)");
  EEL_CHECK_STRING_RESULT(get_duplicate_name("foo (10th copy).txt", 1, -1),
                          "foo (11th copy).txt");
  EEL_CHECK_STRING_RESULT(get_duplicate_name("foo (11th copy)", 1, -1),
                          "foo (12th copy)");
  EEL_CHECK_STRING_RESULT(get_duplicate_name("foo (11th copy).txt", 1, -1),
                          "foo (12th copy).txt");
  EEL_CHECK_STRING_RESULT(get_duplicate_name("foo (12th copy)", 1, -1),
                          "foo (13th copy)");
  EEL_CHECK_STRING_RESULT(get_duplicate_name("foo (12th copy).txt", 1, -1),
                          "foo (13th copy).txt");
  EEL_CHECK_STRING_RESULT(get_duplicate_name("foo (110th copy)", 1, -1),
                          "foo (111th copy)");
  EEL_CHECK_STRING_RESULT(get_duplicate_name("foo (110th copy).txt", 1, -1),
                          "foo (111th copy).txt");
  EEL_CHECK_STRING_RESULT(get_duplicate_name("foo (122nd copy)", 1, -1),
                          "foo (123rd copy)");
  EEL_CHECK_STRING_RESULT(get_duplicate_name("foo (122nd copy).txt", 1, -1),
                          "foo (123rd copy).txt");
  EEL_CHECK_STRING_RESULT(get_duplicate_name("foo (123rd copy)", 1, -1),
                          "foo (124th copy)");
  EEL_CHECK_STRING_RESULT(get_duplicate_name("foo (123rd copy).txt", 1, -1),
                          "foo (124th copy).txt");
