...
static const GtkTargetEntry drop_types[] = {
    ...
    {CAJA_ICON_DND_XDNDDIRECTSAVE_TYPE, 0,
     CAJA_ICON_DND_XDNDDIRECTSAVE}, /* XDS Protocol Type */
...
static EelCanvasItem *create_selection_shadow(CajaIconContainer *container,
                                              GList *list) {
  ...
  /* Create a group, so that it's easier to move all the items around at
         once.  */
  group = EEL_CANVAS_GROUP(eel_canvas_item_new(
      EEL_CANVAS_GROUP(canvas->root), eel_canvas_group_get_type(), NULL));
  ...
  for (p = list; p != NULL; p = p->next) {
    ...
    if (x2 >= min_x && x1 <= max_x && y2 >= min_y && y1 <= max_y)
      eel_canvas_item_new(group, eel_canvas_rect_get_type(), "x1", (double)x1,
                          "y1", (double)y1, "x2", (double)x2, "y2", (double)y2,
                          "outline-color-rgba", &black, "outline-stippling",
                          TRUE, "width_pixels", 1, NULL);
  }
 ...
 */
static void set_shadow_position(EelCanvasItem *shadow, double x, double y) {
  eel_canvas_item_set(shadow, "x", x, "y", y, NULL);
}
...
static void canvas_rect_world_to_widget(EelCanvas *canvas, EelDRect *world_rect,
  ...
  eel_canvas_world_to_window(canvas, world_rect->x0, world_rect->y0,
                             &window_rect.x0, &window_rect.y0);
  eel_canvas_world_to_window(canvas, world_rect->x1, world_rect->y1,
...
static void canvas_widget_to_world(EelCanvas *canvas, double widget_x,
                                   double widget_y, double *world_x,
                                   double *world_y) {
  eel_canvas_window_to_world(
      canvas,
      widget_x + gtk_adjustment_get_value(
                     gtk_scrollable_get_hadjustment(GTK_SCROLLABLE(canvas))),
      widget_y + gtk_adjustment_get_value(
                     gtk_scrollable_get_vadjustment(GTK_SCROLLABLE(canvas))),
      world_x, world_y);
}

static gboolean icon_get_data_binder(CajaIcon *icon, gpointer data) {
  ...
  widget_rect = eel_irect_offset_by(
      widget_rect, -container->details->dnd_info->drag_info.start_x,
  ...
  context->iteratee(uri, (int)widget_rect.x0, (int)widget_rect.y0,
                    widget_rect.x1 - widget_rect.x0,
                    widget_rect.y1 - widget_rect.y0, context->iteratee_data);
 ...
 */
static void caja_icon_container_each_selected_icon(
    CajaIconContainer *container,
 ...
 */
static void each_icon_get_data_binder(CajaDragEachSelectedItemDataGet iteratee,
                                      gpointer iterator_context,
                                      gpointer data) {
  IconGetDataBinderContext context;
  CajaIconContainer *container;
  ...
  caja_icon_container_each_selected_icon(container, icon_get_data_binder,
                                         &context);
}
...
static void drag_data_get_callback(GtkWidget *widget, GdkDragContext *context,
   ...
   */
  caja_drag_drag_data_get(widget, context, selection_data, info, time, widget,
                          each_icon_get_data_binder);
}

/* Target-side handling of the drag.  */

static void caja_icon_container_position_shadow(CajaIconContainer *container,
                                                int x, int y) {
...
static void caja_icon_container_dropped_icon_feedback(GtkWidget *widget,
  ...
  dnd_info->shadow =
      create_selection_shadow(container, dnd_info->drag_info.selection_list);
...
static void set_direct_save_uri(GtkWidget *widget, GdkDragContext *context,
                                CajaDragInfo *drag_info, int x, int y) {
  ...
  drop_target = caja_icon_container_find_drop_target(
      CAJA_ICON_CONTAINER(widget), context, x, y, NULL, TRUE);
...
static void get_data_on_first_target_we_support(GtkWidget *widget,
                                                GdkDragContext *context,
                                                guint32 time, int x, int y) {
  GtkTargetList *list;
  GdkAtom target;

  if (drop_types_list == NULL) {
    drop_types_list =
        gtk_target_list_new(drop_types, G_N_ELEMENTS(drop_types) - 1);
    gtk_target_list_add_text_targets(drop_types_list, CAJA_ICON_DND_TEXT);
  }
  if (drop_types_list_root == NULL) {
    drop_types_list_root =
        gtk_target_list_new(drop_types, G_N_ELEMENTS(drop_types));
...
static void caja_icon_container_ensure_drag_data(CajaIconContainer *container,
                                                 GdkDragContext *context,
                                                 guint32 time) {
  CajaIconDndInfo *dnd_info;
  ...
  if (!dnd_info->drag_info.got_drop_data_type) {
    get_data_on_first_target_we_support(GTK_WIDGET(container), context, time, 0,
                                        0);
  }
}
...
static CajaIcon *caja_icon_container_item_at(CajaIconContainer *container,
                                             int x, int y) {
  GList *p;
  int size;
  EelDRect point;
  ...
  /* build the hit-test rectangle. Base the size on the scale factor to ensure
   * that it is non-empty even at the smallest scale factor
   */
  ...
  for (p = container->details->icons; p != NULL; p = p->next) {
    ...
    eel_canvas_w2c(EEL_CANVAS(container), point.x0, point.y0, &canvas_point.x0,
                   &canvas_point.y0);
    eel_canvas_w2c(EEL_CANVAS(container), point.x1, point.y1, &canvas_point.x1,
                   &canvas_point.y1);
...
static char *get_container_uri(CajaIconContainer *container) {
  char *uri;
...
static gboolean caja_icon_container_selection_items_local(
    CajaIconContainer *container, GList *items) {
...
static GdkDragAction get_background_drag_action(CajaIconContainer *container,
  ...
  if (action == GDK_ACTION_ASK) {
    ...
    if (!eel_background_is_desktop(
            eel_get_widget_background(GTK_WIDGET(container)))) {
      valid_actions |= CAJA_DND_ACTION_SET_AS_GLOBAL_BACKGROUND;
    }

    action =
        caja_drag_drop_background_ask(GTK_WIDGET(container), valid_actions);
  }

  return action;
}

static void receive_dropped_color(CajaIconContainer *container, int x, int y,
                                  GdkDragAction action,
                                  GtkSelectionData *data) {
  ...
  if (action > 0) {
    ...
    eel_background_set_dropped_color(
        eel_get_widget_background(GTK_WIDGET(container)), GTK_WIDGET(container),
        action, x, y, data);
  }
}

/* handle dropped tile images */
static void receive_dropped_tile_image(CajaIconContainer *container,
                                       GdkDragAction action,
                                       GtkSelectionData *data) {
  g_assert(data != NULL);
  ...
  if (action > 0) {
    ...
    eel_background_set_dropped_image(
        eel_get_widget_background(GTK_WIDGET(container)), action,
        gtk_selection_data_get_data(data));
  }
}

/* handle dropped keywords */
static void receive_dropped_keyword(CajaIconContainer *container,
                                    const char *keyword, int x, int y) {
  char *uri;
  double world_x, world_y;
  ...
  caja_debug_log(FALSE, CAJA_DEBUG_LOG_DOMAIN_USER,
                 "dropped emblem '%s' on icon container URI: %s", keyword, uri);
...
/* handle dropped url */
static void receive_dropped_netscape_url(CajaIconContainer *container,
  ...
  drop_target = caja_icon_container_find_drop_target(container, context, x, y,
                                                     NULL, TRUE);

  g_signal_emit_by_name(container, "handle_netscape_url", encoded_url,
                        drop_target,
...
static void receive_dropped_uri_list(CajaIconContainer *container,
                                     const char *uri_list,
  ...
  g_signal_emit_by_name(container, "handle_uri_list", uri_list, drop_target,
                        gdk_drag_context_get_selected_action(context), x, y);
...
/* handle dropped text */
static void receive_dropped_text(CajaIconContainer *container, const char *text,
                                 GdkDragContext *context, int x, int y) {
  char *drop_target;
  ...
  g_signal_emit_by_name(container, "handle_text", text, drop_target,
                        gdk_drag_context_get_selected_action(context), x, y);
...
static void receive_dropped_raw(CajaIconContainer *container,
  ...
  g_signal_emit_by_name(container, "handle_raw", raw_data, length, drop_target,
                        direct_save_uri,
                        gdk_drag_context_get_selected_action(context), x, y);
...
static int auto_scroll_timeout_callback(gpointer data) {
  CajaIconContainer *container;
  GtkWidget *widget;
  ...
  if (container->details->dnd_info->drag_info.waiting_to_autoscroll &&
      container->details->dnd_info->drag_info.start_auto_scroll_in >
  ...
  caja_drag_autoscroll_calculate_delta(widget, &x_scroll_delta,
                                       &y_scroll_delta);
  ...
  if (!caja_icon_container_scroll(container, (int)x_scroll_delta,
                                  (int)y_scroll_delta)) {
  ...
  /* Due to a glitch in GtkLayout, whe need to do an explicit draw of the
   * exposed area. Calculate the size of the area we need to draw
   */
...
static void set_up_auto_scroll_if_needed(CajaIconContainer *container) {
  caja_drag_autoscroll_start(&container->details->dnd_info->drag_info,
                             GTK_WIDGET(container),
                             auto_scroll_timeout_callback, container);
}

static void stop_auto_scroll(CajaIconContainer *container) {
  caja_drag_autoscroll_stop(&container->details->dnd_info->drag_info);
}

static void handle_local_move(CajaIconContainer *container, double world_x,
                              double world_y) {
  GList *moved_icons, *p;
  CajaFile *file;
  ...
  for (p = container->details->dnd_info->drag_info.selection_list; p != NULL;
    ...
    icon = caja_icon_container_get_icon_by_uri(container, item->uri);
    ...
    if (icon == NULL) {
      ...
      caja_file_set_metadata(file, CAJA_METADATA_KEY_SCREEN, NULL,
                             screen_string);
      caja_file_set_time_metadata(
          file, CAJA_METADATA_KEY_ICON_POSITION_TIMESTAMP, now);
    ...
    if (item->got_icon_position) {
      caja_icon_container_move_icon(container, icon, world_x + item->icon_x,
                                    world_y + item->icon_y, icon->scale, TRUE,
                                    TRUE, TRUE);
    }
    moved_icons = g_list_prepend(moved_icons, icon);
  }
  caja_icon_container_select_list_unselect_others(container, moved_icons);
...
static void handle_nonlocal_move(CajaIconContainer *container,
                                 GdkDragAction action, int x, int y,
                                 const char *target_uri, gboolean icon_hit) {
  GList *source_uris, *p;
  ...
  for (p = container->details->dnd_info->drag_info.selection_list; p != NULL;
    ...
    source_uris =
        g_list_prepend(source_uris, ((CajaDragSelectionItem *)p->data)->uri);
  }
  ...
  if (!icon_hit) {
     ...
     */
    source_item_locations = g_array_set_size(
  ...
  /* start the copy */
  g_signal_emit_by_name(container, "move_copy_items", source_uris,
                        source_item_locations, target_uri, action, x, y);
...
static char *caja_icon_container_find_drop_target(CajaIconContainer *container,
  ...
  if (drop_target_icon == NULL) {
    ...
    if (rewrite_desktop && container_uri != NULL &&
  ...
  return caja_icon_container_get_icon_drop_target_uri(container,
                                                      drop_target_icon);
}

static gboolean selection_is_image_file(GList *selection_list) {
  ...
  if (selection_list == NULL || selection_list->next != NULL) {
    return FALSE;
  }
  ...
  info = g_file_query_info(location, G_FILE_ATTRIBUTE_STANDARD_CONTENT_TYPE, 0,
                           NULL, NULL);
  if (info) {
...
static void caja_icon_container_receive_dropped_icons(
    CajaIconContainer *container, GdkDragContext *context, int x, int y) {
  char *drop_target;
  ...
  if (real_action == GDK_ACTION_ASK) {
    /* FIXME bugzilla.gnome.org 42485: This belongs in FMDirectoryView, not
     * here. */
    /* Check for special case items in selection list */
    if (caja_drag_selection_includes_special_link(
            container->details->dnd_info->drag_info.selection_list)) {
  ...
  if (real_action == (GdkDragAction)CAJA_DND_ACTION_SET_AS_BACKGROUND) {
    ...
    eel_background_set_dropped_image(
        eel_get_widget_background(GTK_WIDGET(container)), real_action,
        selected_item->uri);
    return;
  }
  ...
  if (real_action > 0) {
    ...
    if (local_move_only) {
      ...
      handle_nonlocal_move(container, real_action, world_x, world_y,
                           drop_target, icon_hit);
    }
  }
...
static void caja_icon_container_get_drop_action(CajaIconContainer *container,
                                                GdkDragContext *context, int x,
                                                int y, int *action) {
  char *drop_target;
  gboolean icon_hit;
  ...
  *action = 0;
  ...
  switch (container->details->dnd_info->drag_info.data_type) {
    case CAJA_ICON_DND_MATE_ICON_LIST:
      ...
      drop_target = caja_icon_container_find_drop_target(container, context, x,
                                                         y, &icon_hit, FALSE);
      if (!drop_target) {
        return;
      }
      caja_drag_default_drop_action_for_icons(
          context, drop_target,
          container->details->dnd_info->drag_info.selection_list, action);
      g_free(drop_target);
      break;
    case CAJA_ICON_DND_URI_LIST:
      drop_target = caja_icon_container_find_drop_target(container, context, x,
                                                         y, &icon_hit, FALSE);
      *action =
          caja_drag_default_drop_action_for_uri_list(context, drop_target);
...
static void set_drop_target(CajaIconContainer *container, CajaIcon *icon) {
  CajaIcon *old_icon;
...
static void caja_icon_dnd_update_drop_target(CajaIconContainer *container,
                                             GdkDragContext *context, int x,
                                             int y) {
  CajaIcon *icon;
  double world_x, world_y;
  ...
  if (icon != NULL && (container->details->dnd_info->drag_info.data_type !=
...
static void caja_icon_container_free_drag_data(CajaIconContainer *container) {
  CajaIconDndInfo *dnd_info;
...
static void drag_leave_callback(GtkWidget *widget, GdkDragContext *context,
  ...
  if (dnd_info->shadow != NULL) eel_canvas_item_hide(dnd_info->shadow);
...
static void drag_begin_callback(GtkWidget *widget, GdkDragContext *context,
  ...
  surface = caja_icon_canvas_item_get_drag_surface(
...
void caja_icon_dnd_begin_drag(CajaIconContainer *container,
                              GdkDragAction actions, int button,
  ...
  */
  dnd_info->drag_info.start_x =
      start_x - gtk_adjustment_get_value(
                    gtk_scrollable_get_hadjustment(GTK_SCROLLABLE(container)));
  dnd_info->drag_info.start_y =
      start_y - gtk_adjustment_get_value(
                    gtk_scrollable_get_vadjustment(GTK_SCROLLABLE(container)));

  /* start the drag */
  gtk_drag_begin_with_coordinates(
      GTK_WIDGET(container), dnd_info->drag_info.target_list, actions, button,
      (GdkEvent *)event, dnd_info->drag_info.start_x,
      dnd_info->drag_info.start_y);
}

static gboolean drag_highlight_draw(GtkWidget *widget, cairo_t *cr,
                                    gpointer user_data) {
  gint width, height;
  GdkWindow *window;
...
static void dnd_highlight_queue_redraw(GtkWidget *widget) {
   ...
   */
  gtk_widget_queue_draw_area(widget, 0, 0, width, 10);
  gtk_widget_queue_draw_area(widget, 0, 0, 10, height);
  gtk_widget_queue_draw_area(widget, 0, height - 10, width, 10);
  gtk_widget_queue_draw_area(widget, width - 10, 0, 10, height);
}

static void start_dnd_highlight(GtkWidget *widget) {
  CajaIconDndInfo *dnd_info;
  GtkWidget *toplevel;
  ...
  if (!dnd_info->highlighted) {
    ...
    g_signal_connect_after(widget, "draw", G_CALLBACK(drag_highlight_draw),
                           NULL);
    dnd_highlight_queue_redraw(widget);
  }
}
...
static void stop_dnd_highlight(GtkWidget *widget) {
  ...
  if (dnd_info->highlighted) {
    g_signal_handlers_disconnect_by_func(widget, drag_highlight_draw, NULL);
    dnd_highlight_queue_redraw(widget);
...
static gboolean drag_motion_callback(GtkWidget *widget, GdkDragContext *context,
  ...
  caja_icon_container_ensure_drag_data(CAJA_ICON_CONTAINER(widget), context,
                                       time);
   ...
   */
  action = 0;
  caja_icon_container_get_drop_action(CAJA_ICON_CONTAINER(widget), context, x,
                                      y, &action);
  if (action != 0) {
...
static void drag_data_received_callback(GtkWidget *widget,
  ...
  switch (info) {
    ...
    case CAJA_ICON_DND_RAW:
      ...
      /* Netscape keeps sending us the data, even though we accept the first
       * drag */
    case CAJA_ICON_DND_NETSCAPE_URL:
      if (drag_info->selection_data != NULL) {
        gtk_selection_data_free(drag_info->selection_data);
      ...
      break;
    case CAJA_ICON_DND_ROOTWINDOW_DROP:
      /* Do nothing, this won't even happen, since we don't want to call
       * get_data twice */
      break;
  }
  ...
  if (drag_info->drop_occured) {
    ...
    switch (info) {
      case CAJA_ICON_DND_MATE_ICON_LIST:
        caja_icon_container_receive_dropped_icons(CAJA_ICON_CONTAINER(widget),
                                                  context, x, y);
        break;
      case CAJA_ICON_DND_COLOR:
        receive_dropped_color(CAJA_ICON_CONTAINER(widget), x, y,
      ...
      case CAJA_ICON_DND_BGIMAGE:
        receive_dropped_tile_image(
            ...
            gdk_drag_context_get_selected_action(context), data);
        break;
      case CAJA_ICON_DND_KEYWORD:
        receive_dropped_keyword(CAJA_ICON_CONTAINER(widget),
                                (char *)gtk_selection_data_get_data(data), x,
                                y);
        break;
      case CAJA_ICON_DND_NETSCAPE_URL:
        receive_dropped_netscape_url(CAJA_ICON_CONTAINER(widget),
                                     (char *)gtk_selection_data_get_data(data),
                                     context, x, y);
        success = TRUE;
        break;
      case CAJA_ICON_DND_URI_LIST:
        receive_dropped_uri_list(CAJA_ICON_CONTAINER(widget),
                                 (char *)gtk_selection_data_get_data(data),
                                 context, x, y);
        success = TRUE;
        break;
      case CAJA_ICON_DND_TEXT:
        ...
        receive_dropped_text(CAJA_ICON_CONTAINER(widget), (char *)tmp, context,
                             x, y);
        success = TRUE;
        g_free(tmp);
        break;
      case CAJA_ICON_DND_RAW:
        ...
        receive_dropped_raw(CAJA_ICON_CONTAINER(widget), tmp_raw, length,
                            drag_info->direct_save_uri, context, x, y);
        success = TRUE;
        break;
      case CAJA_ICON_DND_RESET_BACKGROUND:
      ...
      case CAJA_ICON_DND_XDNDDIRECTSAVE: {
        ...
        selection_length =
            gtk_selection_data_get_length(drag_info->selection_data);
        selection_format =
            gtk_selection_data_get_format(drag_info->selection_data);

        if (selection_format == 8 && selection_length == 1 &&
            selection_data[0] == 'F') {
          gtk_drag_get_data(widget, context,
                            gdk_atom_intern(CAJA_ICON_DND_RAW_TYPE, FALSE),
                            time);
          return;
          ...
          caja_file_changes_queue_schedule_position_set(
              location, p,
              gdk_x11_screen_get_screen_number(gtk_widget_get_screen(widget)));
          g_object_unref(location);
...
void caja_icon_dnd_init(CajaIconContainer *container) {
  ...
  caja_drag_init(&container->details->dnd_info->drag_info, drag_types,
                 G_N_ELEMENTS(drag_types), TRUE);
  ...
  gtk_drag_dest_set(
      GTK_WIDGET(container), 0, drop_types, n_elements,
  ...
  g_signal_connect(container, "drag_begin", G_CALLBACK(drag_begin_callback),
                   NULL);
  g_signal_connect(container, "drag_data_get",
                   G_CALLBACK(drag_data_get_callback), NULL);
  g_signal_connect(container, "drag_end", G_CALLBACK(drag_end_callback), NULL);

  /* Messages for incoming drag. */
  g_signal_connect(container, "drag_data_received",
                   G_CALLBACK(drag_data_received_callback), NULL);
  g_signal_connect(container, "drag_motion", G_CALLBACK(drag_motion_callback),
                   NULL);
  g_signal_connect(container, "drag_drop", G_CALLBACK(drag_drop_callback),
                   NULL);
  g_signal_connect(container, "drag_leave", G_CALLBACK(drag_leave_callback),
                   NULL);
}
