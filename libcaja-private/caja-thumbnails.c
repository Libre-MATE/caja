...
static void thumbnail_thread_func(GTask *task, gpointer source_object,
                                  gpointer task_data,
                                  GCancellable *cancellable);

/* structure used for making thumbnails, associating a uri with where the
...
static gboolean get_file_mtime(const char *file_uri, time_t *mtime) {
  GFile *file;
  GFileInfo *info;
  gboolean ret;
...
static void free_thumbnail_info(CajaThumbnailInfo *info) {
  g_free(info->image_uri);
  g_free(info->mime_type);
  g_free(info);
}

static MateDesktopThumbnailFactory *get_thumbnail_factory(void) {
  static MateDesktopThumbnailFactory *thumbnail_factory = NULL;
...
static gboolean thumbnail_thread_starter_cb(gpointer data) {
  GTask *task;
...
/* This is a one-shot idle callback called from the main loop to call
   ...
   thread-safe. */
static gboolean thumbnail_thread_notify_file_changed(gpointer image_uri) {
  CajaFile *file;
  ...
  if (file != NULL) {
    ...
    caja_file_invalidate_attributes(
        file, CAJA_FILE_ATTRIBUTE_THUMBNAIL | CAJA_FILE_ATTRIBUTE_INFO);
...
static GHashTable *get_types_table(void) {
  ...
  if (image_mime_types == NULL) {
    ...
    for (l = format_list; l; l = l->next) {
      ...
      for (i = 0; types[i] != NULL; i++) {
        g_hash_table_insert(image_mime_types, types[i], GUINT_TO_POINTER(1));
      }
...
static gboolean pixbuf_can_load_type(const char *mime_type) {
...
gboolean caja_can_thumbnail(CajaFile *file) {
  ...
  res = mate_desktop_thumbnail_factory_can_thumbnail(factory, uri, mime_type,
                                                     mtime);
  g_free(mime_type);
  g_free(uri);
...
void caja_create_thumbnail(CajaFile *file) {
  ...
  if (file->details->got_file_info && file->details->file_info_is_up_to_date &&
  ...
  if (existing == NULL) {
    ...
    if (thumbnail_thread_is_running == FALSE &&
        thumbnail_thread_starter_id == 0) {
      thumbnail_thread_starter_id = g_idle_add_full(
          G_PRIORITY_LOW, thumbnail_thread_starter_cb, NULL, NULL);
    }
  } else {
    ...
    */
    ...
    if (pixbuf) {
      ...
      mate_desktop_thumbnail_factory_save_thumbnail(
          thumbnail_factory, pixbuf, info->image_uri, current_orig_mtime);
      ...
      mate_desktop_thumbnail_factory_create_failed_thumbnail(
          thumbnail_factory, info->image_uri, current_orig_mtime);
    }
    ...
    g_idle_add_full(G_PRIORITY_HIGH_IDLE, thumbnail_thread_notify_file_changed,
