...
static void caja_vfs_directory_init(gpointer object, gpointer klass);
static void caja_vfs_directory_class_init(gpointer klass);

EEL_CLASS_BOILERPLATE(CajaVFSDirectory, caja_vfs_directory, CAJA_TYPE_DIRECTORY)

static void caja_vfs_directory_init(gpointer object, gpointer klass) {}

static gboolean vfs_contains_file(CajaDirectory *directory, CajaFile *file) {
...
static void vfs_call_when_ready(CajaDirectory *directory,
  ...
  caja_directory_call_when_ready_internal(directory, NULL, file_attributes,
                                          wait_for_file_list, callback, NULL,
                                          callback_data);
}

static void vfs_cancel_callback(CajaDirectory *directory,
                                CajaDirectoryCallback callback,
                                gpointer callback_data) {
  g_assert(CAJA_IS_VFS_DIRECTORY(directory));

  caja_directory_cancel_callback_internal(directory, NULL, callback, NULL,
                                          callback_data);
}

static void vfs_file_monitor_add(CajaDirectory *directory, gconstpointer client,
  ...
  caja_directory_monitor_add_internal(directory, NULL, client,
                                      monitor_hidden_files, file_attributes,
                                      callback, callback_data);
}

static void vfs_file_monitor_remove(CajaDirectory *directory,
...
static void vfs_force_reload(CajaDirectory *directory) {
  CajaFileAttributes all_attributes;

  g_assert(CAJA_IS_DIRECTORY(directory));

  all_attributes = caja_file_get_all_attributes();
  caja_directory_force_reload_internal(directory, all_attributes);
}

static gboolean vfs_are_all_files_seen(CajaDirectory *directory) {
  g_assert(CAJA_IS_VFS_DIRECTORY(directory));

  return directory->details->directory_loaded;
}

static gboolean vfs_is_not_empty(CajaDirectory *directory) {
  g_assert(CAJA_IS_VFS_DIRECTORY(directory));
