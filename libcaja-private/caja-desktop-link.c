...
static void create_icon_file(CajaDesktopLink *link) {
  link->details->icon_file = caja_desktop_icon_file_new(link);
}

static void caja_desktop_link_changed(CajaDesktopLink *link) {
  if (link->details->icon_file != NULL) {
    caja_desktop_icon_file_update(link->details->icon_file);
  }
}

static void mount_changed_callback(GMount *mount, CajaDesktopLink *link) {
...
static void caja_desktop_link_ensure_display_name(CajaDesktopLink *link) {
...
static void trash_state_changed_callback(CajaTrashMonitor *trash_monitor,
                                         gboolean state,
...
static void home_name_changed(gpointer callback_data) {
  CajaDesktopLink *link;
  ...
  link->details->display_name = g_settings_get_string(
      caja_desktop_preferences, CAJA_PREFERENCES_DESKTOP_HOME_NAME);

  caja_desktop_link_ensure_display_name(link);
  caja_desktop_link_changed(link);
}

static void computer_name_changed(gpointer callback_data) {
  CajaDesktopLink *link;
  ...
  g_free(link->details->display_name);
  link->details->display_name = g_settings_get_string(
      caja_desktop_preferences, CAJA_PREFERENCES_DESKTOP_COMPUTER_NAME);

  caja_desktop_link_ensure_display_name(link);
  caja_desktop_link_changed(link);
}

static void trash_name_changed(gpointer callback_data) {
  CajaDesktopLink *link;
  ...
  g_free(link->details->display_name);
  link->details->display_name = g_settings_get_string(
      caja_desktop_preferences, CAJA_PREFERENCES_DESKTOP_TRASH_NAME);

  caja_desktop_link_ensure_display_name(link);
  caja_desktop_link_changed(link);
}

static void network_name_changed(gpointer callback_data) {
  CajaDesktopLink *link;
  ...
  g_free(link->details->display_name);
  link->details->display_name = g_settings_get_string(
      caja_desktop_preferences, CAJA_PREFERENCES_DESKTOP_NETWORK_NAME);
...
CajaDesktopLink *caja_desktop_link_new(CajaDesktopLinkType type) {
  ...
  switch (type) {
    case CAJA_DESKTOP_LINK_HOME:
      ...
      link->details->display_name = g_settings_get_string(
          caja_desktop_preferences, CAJA_PREFERENCES_DESKTOP_HOME_NAME);
      link->details->activation_location =
      ...
      g_signal_connect_swapped(caja_desktop_preferences,
                               ...
                               G_CALLBACK(home_name_changed), link);
    ...
    case CAJA_DESKTOP_LINK_COMPUTER:
      ...
      g_signal_connect_swapped(
          ...
          G_CALLBACK(computer_name_changed), link);
    ...
    case CAJA_DESKTOP_LINK_TRASH:
      ...
      g_signal_connect_swapped(caja_desktop_preferences,
                               ...
                               G_CALLBACK(trash_name_changed), link);
      link->details->signal_handler_obj = G_OBJECT(caja_trash_monitor_get());
      link->details->signal_handler = g_signal_connect_object(
    ...
    case CAJA_DESKTOP_LINK_NETWORK:
      ...
      g_signal_connect_swapped(
          ...
          G_CALLBACK(network_name_changed), link);
      break;
...
CajaDesktopLink *caja_desktop_link_new_from_mount(GMount *mount) {
  ...
  link->details->filename = caja_desktop_link_monitor_make_filename_unique(
      caja_desktop_link_monitor_get(), filename);
  g_free(filename);
  g_free(name);
  ...
  link->details->signal_handler = g_signal_connect(
      mount, "changed", G_CALLBACK(mount_changed_callback), link);
  ...
  switch (link->details->type) {
    case CAJA_DESKTOP_LINK_HOME:
      g_settings_set_string(caja_desktop_preferences,
                            CAJA_PREFERENCES_DESKTOP_HOME_NAME, name);
      break;
    case CAJA_DESKTOP_LINK_COMPUTER:
      g_settings_set_string(caja_desktop_preferences,
                            CAJA_PREFERENCES_DESKTOP_COMPUTER_NAME, name);
      break;
    case CAJA_DESKTOP_LINK_TRASH:
      g_settings_set_string(caja_desktop_preferences,
                            CAJA_PREFERENCES_DESKTOP_TRASH_NAME, name);
      break;
    case CAJA_DESKTOP_LINK_NETWORK:
      g_settings_set_string(caja_desktop_preferences,
                            CAJA_PREFERENCES_DESKTOP_NETWORK_NAME, name);
      break;
    default:
      g_assert_not_reached();
...
static void caja_desktop_link_init(CajaDesktopLink *link) {
  link->details = caja_desktop_link_get_instance_private(link);
}

static void desktop_link_finalize(GObject *object) {
  CajaDesktopLink *link;
...
static void caja_desktop_link_class_init(CajaDesktopLinkClass *klass) {
