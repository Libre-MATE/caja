...
G_DEFINE_TYPE_WITH_PRIVATE(CajaDesktopIconFile, caja_desktop_icon_file,
                           CAJA_TYPE_FILE)

static void desktop_icon_file_monitor_add(CajaFile *file, gconstpointer client,
                                          CajaFileAttributes attributes) {
  caja_directory_monitor_add_internal(file->details->directory, file, client,
                                      TRUE, attributes, NULL, NULL);
}

static void desktop_icon_file_monitor_remove(CajaFile *file,
                                             gconstpointer client) {
  caja_directory_monitor_remove_internal(file->details->directory, file,
                                         client);
}

static void desktop_icon_file_call_when_ready(CajaFile *file,
                                              CajaFileAttributes attributes,
                                              CajaFileCallback callback,
                                              gpointer callback_data) {
  caja_directory_call_when_ready_internal(file->details->directory, file,
                                          attributes, FALSE, NULL, callback,
                                          callback_data);
}

static void desktop_icon_file_cancel_call_when_ready(CajaFile *file,
                                                     CajaFileCallback callback,
                                                     gpointer callback_data) {
...
static gboolean desktop_icon_file_check_if_ready(
    CajaFile *file, CajaFileAttributes attributes) {
  return caja_directory_check_if_ready_internal(file->details->directory, file,
                                                attributes);
}

static gboolean desktop_icon_file_get_item_count(CajaFile *file, guint *count,
...
static CajaRequestStatus desktop_icon_file_get_deep_counts(
...
static gboolean desktop_icon_file_get_date(CajaFile *file,
  ...
  return caja_desktop_link_get_date(desktop_file->details->link, date_type,
                                    date);
}

static char *desktop_icon_file_get_where_string(CajaFile *file) {
  return g_strdup(_("on the desktop"));
}

static void caja_desktop_icon_file_init(CajaDesktopIconFile *desktop_file) {
  desktop_file->details =
      caja_desktop_icon_file_get_instance_private(desktop_file);
}

static void update_info_from_link(CajaDesktopIconFile *icon_file) {
  CajaFile *file;
  CajaDesktopLink *link;
  char *display_name;
  GMount *mount;
  ...
  caja_file_set_display_name(file, display_name, NULL, TRUE);
  g_free(display_name);
 ...
 */
CajaDesktopLink *caja_desktop_icon_file_get_link(
    CajaDesktopIconFile *icon_file) {
...
static void caja_desktop_icon_file_unmount(CajaFile *file,
...
static void caja_desktop_icon_file_eject(CajaFile *file,
                                         GMountOperation *mount_op,
                                         GCancellable *cancellable,
                                         CajaFileOperationCallback callback,
                                         gpointer callback_data) {
...
static void caja_desktop_icon_file_set_metadata(CajaFile *file, const char *key,
                                                const char *value) {
  caja_desktop_set_metadata_string(file, file->details->name, key, value);
}

static void caja_desktop_icon_file_set_metadata_as_list(CajaFile *file,
                                                        const char *key,
                                                        char **value) {
  caja_desktop_set_metadata_stringv(file, file->details->name, key,
                                    (const gchar **)value);
}

static void caja_desktop_icon_file_class_init(CajaDesktopIconFileClass *klass) {
  ...
  file_class->set_metadata_as_list =
      caja_desktop_icon_file_set_metadata_as_list;
