...
enum { TRASH_STATE_CHANGED, LAST_SIGNAL };
...
static void caja_trash_monitor_finalize(GObject *object) {
...
static void caja_trash_monitor_class_init(CajaTrashMonitorClass *klass) {
  ...
  signals[TRASH_STATE_CHANGED] = g_signal_new(
      "trash_state_changed", G_TYPE_FROM_CLASS(object_class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaTrashMonitorClass, trash_state_changed), NULL, NULL,
      g_cclosure_marshal_VOID__BOOLEAN, G_TYPE_NONE, 1, G_TYPE_BOOLEAN);
}

static void update_info_cb(GObject *source_object, GAsyncResult *res,
                           gpointer user_data) {
  CajaTrashMonitor *trash_monitor;
  GFileInfo *info;
  gboolean empty;
  ...
  if (info != NULL) {
    ...
    if (icon) {
      ...
      if (trash_monitor->details->empty != empty) {
        ...
        g_signal_emit(trash_monitor, signals[TRASH_STATE_CHANGED], 0,
...
static void schedule_update_info(CajaTrashMonitor *trash_monitor) {
  GFile *location;

  location = g_file_new_for_uri("trash:///");

  g_file_query_info_async(location, G_FILE_ATTRIBUTE_STANDARD_ICON, 0, 0, NULL,
                          update_info_cb, g_object_ref(trash_monitor));

  g_object_unref(location);
}

static void file_changed(GFileMonitor *monitor, GFile *child, GFile *other_file,
...
static void caja_trash_monitor_init(CajaTrashMonitor *trash_monitor) {
  GFile *location;

  trash_monitor->details =
      caja_trash_monitor_get_instance_private(trash_monitor);
  ...
  trash_monitor->details->file_monitor =
      g_file_monitor_file(location, 0, NULL, NULL);
...
static void unref_trash_monitor(void) { g_object_unref(caja_trash_monitor); }
...
CajaTrashMonitor *caja_trash_monitor_get(void) {
  if (caja_trash_monitor == NULL) {
    ...
    caja_trash_monitor =
        CAJA_TRASH_MONITOR(g_object_new(CAJA_TYPE_TRASH_MONITOR, NULL));
