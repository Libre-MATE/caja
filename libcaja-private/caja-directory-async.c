...
static gboolean request_is_satisfied(CajaDirectory *directory, CajaFile *file,
                                     Request request);
...
static void link_info_done(CajaDirectory *directory, CajaFile *file,
                           const char *uri, const char *name, const char *icon,
                           gboolean is_launcher, gboolean is_foreign);
...
static void move_file_to_extension_queue(CajaDirectory *directory,
                                         CajaFile *file);
static void caja_directory_invalidate_file_attributes(
    CajaDirectory *directory, CajaFileAttributes file_attributes);
 ...
 */
static gboolean async_job_start(CajaDirectory *directory, const char *job) {
#ifdef DEBUG_ASYNC_JOBS
  char *key;
#endif
  ...
  if (async_job_count >= MAX_ASYNC_JOBS) {
    if (waiting_directories == NULL) {
      waiting_directories = eel_g_hash_table_new_free_at_exit(
          NULL, NULL, "caja-directory-async.c: waiting_directories");
    }
...
#ifdef DEBUG_ASYNC_JOBS
  {
    ...
    if (async_jobs == NULL) {
      async_jobs = eel_g_hash_table_new_free_at_exit(
          g_str_hash, g_str_equal, "caja-directory-async.c: async_jobs");
    }
...
/* End a job. */
static void async_job_end(CajaDirectory *directory, const char *job) {
#ifdef DEBUG_ASYNC_JOBS
  char *key;
...
static void async_job_wake_up(void) {
  ...
  while (async_job_count < MAX_ASYNC_JOBS) {
    ...
    caja_directory_async_state_changed(CAJA_DIRECTORY(value));
  }
  already_waking_up = FALSE;
}

static void directory_count_cancel(CajaDirectory *directory) {
  if (directory->details->count_in_progress != NULL) {
    g_cancellable_cancel(directory->details->count_in_progress->cancellable);
  }
}

static void deep_count_cancel(CajaDirectory *directory) {
...
static void mime_list_cancel(CajaDirectory *directory) {
...
static void top_left_cancel(CajaDirectory *directory) {
...
static void link_info_cancel(CajaDirectory *directory) {
...
static void thumbnail_cancel(CajaDirectory *directory) {
...
static void mount_cancel(CajaDirectory *directory) {
...
static void file_info_cancel(CajaDirectory *directory) {
...
static void new_files_cancel(CajaDirectory *directory) {
  GList *l;
...
static GList *find_monitor(CajaDirectory *directory, CajaFile *file,
                           gconstpointer client) {
  Monitor monitor;
  ...
  return g_list_find_custom(directory->details->monitor_list, &monitor,
                            monitor_key_compare);
}

static void remove_monitor_link(CajaDirectory *directory, GList *link) {
  if (link != NULL) {
    Monitor *monitor;
...
static void remove_monitor(CajaDirectory *directory, CajaFile *file,
                           gconstpointer client) {
...
static void mime_db_changed_callback(GObject *ignore, CajaDirectory *dir) {
  CajaFileAttributes attrs;

  g_assert(dir != NULL);
  g_assert(dir->details != NULL);

  attrs = CAJA_FILE_ATTRIBUTE_INFO | CAJA_FILE_ATTRIBUTE_LINK_INFO |
...
void caja_directory_monitor_add_internal(CajaDirectory *directory,
                                         CajaFile *file, gconstpointer client,
  ...
  if (directory->details->monitor == NULL) {
    directory->details->monitor =
        caja_monitor_directory(directory->details->location);
  }
  ...
  if (REQUEST_WANTS_TYPE(monitor->request, REQUEST_FILE_INFO) &&
    ...
    directory->details->mime_db_monitor = g_signal_connect_object(
        caja_signaller_get_current(), "mime_data_changed",
...
static void set_file_unconfirmed(CajaFile *file, gboolean unconfirmed) {
  CajaDirectory *directory;
...
static gboolean should_skip_file(CajaDirectory *directory, GFileInfo *info) {
  static gboolean show_hidden_files_changed_callback_installed = FALSE;

  /* Add the callback once for the life of our process */
  if (!show_hidden_files_changed_callback_installed) {
    g_signal_connect_swapped(
        caja_preferences, "changed::" CAJA_PREFERENCES_SHOW_HIDDEN_FILES,
        G_CALLBACK(show_hidden_files_changed_callback), NULL);
...
static void directory_load_one(CajaDirectory *directory, GFileInfo *info) {
  if (info == NULL) {
    return;
  }
  ...
  if (g_file_info_get_name(info) == NULL) {
    ...
    uri = caja_directory_get_uri(directory);
    g_warning(
        "Got GFileInfo with NULL name in %s, ignoring. This shouldn't happen "
        "unless the gvfs backend is broken.\n",
        uri);
    g_free(uri);
...
static void directory_load_cancel(CajaDirectory *directory) {
...
static void file_list_cancel(CajaDirectory *directory) {
...
static void directory_load_done(CajaDirectory *directory, GError *error) {
  GList *node;
...
void caja_directory_add_file_monitors(CajaDirectory *directory, CajaFile *file,
...
static void ready_callback_call(CajaDirectory *directory,
...
void caja_directory_call_when_ready_internal(
    CajaDirectory *directory, CajaFile *file,
    CajaFileAttributes file_attributes, gboolean wait_for_file_list,
    CajaDirectoryCallback directory_callback, CajaFileCallback file_callback,
    gpointer callback_data) {
...
gboolean caja_directory_check_if_ready_internal(
    CajaDirectory *directory, CajaFile *file,
    CajaFileAttributes file_attributes) {
  Request request;
...
static void remove_callback_link_keep_data(CajaDirectory *directory,
                                           GList *link) {
  ReadyCallback *callback;
...
static void remove_callback_link(CajaDirectory *directory, GList *link) {
  ReadyCallback *callback;
...
void caja_directory_cancel_callback_internal(
    CajaDirectory *directory, CajaFile *file,
    CajaDirectoryCallback directory_callback, CajaFileCallback file_callback,
    gpointer callback_data) {
  ReadyCallback callback;
...
void caja_directory_get_info_for_new_files(CajaDirectory *directory,
  ...
  for (l = location_list; l != NULL; l = l->next) {
    ...
    g_file_query_info_async(location, CAJA_FILE_DEFAULT_ATTRIBUTES, 0,
...
static gboolean lacks_large_top_left(CajaFile *file) {
  return file->details->file_info_is_up_to_date &&
         (!file->details->top_left_text_is_up_to_date ||
...
static gboolean lacks_info(CajaFile *file) {
  return !file->details->file_info_is_up_to_date && !file->details->is_gone;
}

static gboolean lacks_filesystem_info(CajaFile *file) {
  return !file->details->filesystem_info_is_up_to_date;
}

static gboolean lacks_deep_count(CajaFile *file) {
  return file->details->deep_counts_status != CAJA_REQUEST_DONE;
}

static gboolean lacks_mime_list(CajaFile *file) {
  return !file->details->mime_list_is_up_to_date;
}

static gboolean should_get_mime_list(CajaFile *file) {
  return lacks_mime_list(file) && !file->details->loading_directory;
}

static gboolean lacks_link_info(CajaFile *file) {
...
static gboolean lacks_extension_info(CajaFile *file) {
  return file->details->pending_info_providers != NULL;
}

static gboolean lacks_thumbnail(CajaFile *file) {
  return caja_file_should_show_thumbnail(file) &&
         file->details->thumbnail_path != NULL &&
         !file->details->thumbnail_is_up_to_date;
}

static gboolean lacks_mount(CajaFile *file) {
...
static gboolean has_problem(CajaDirectory *directory, CajaFile *file,
                            FileCheck problem) {
  GList *node;
...
static void schedule_call_ready_callbacks(CajaDirectory *directory) {
 ...
 * before then */
static gboolean call_ready_callbacks(CajaDirectory *directory) {
  gboolean found_any;
  GList *node, *next;
  ReadyCallback *callback = NULL;
...
gboolean caja_directory_has_active_request_for_file(CajaDirectory *directory,
  ...
  for (node = directory->details->monitor_list; node != NULL;
       node = node->next) {
    monitor = node->data;
...
gboolean caja_directory_is_anyone_monitoring_file_list(
    CajaDirectory *directory) {
...
static void mark_all_files_unconfirmed(CajaDirectory *directory) {
  GList *node;
  CajaFile *file = NULL;
...
static void start_monitoring_file_list(CajaDirectory *directory) {
  DirectoryLoadState *state;
  ...
  state->load_directory_file = caja_directory_get_corresponding_file(directory);
  state->load_directory_file->details->loading_directory = TRUE;

#ifdef DEBUG_LOAD_DIRECTORY
  g_message("load_directory called to monitor file list of %p",
  ...
  g_file_enumerate_children_async(
      directory->details->location, CAJA_FILE_DEFAULT_ATTRIBUTES, 0, /* flags */
...
static void file_list_start_or_stop(CajaDirectory *directory) {
...
static gboolean monitor_includes_file(const Monitor *monitor, CajaFile *file) {
  if (monitor->file == file) {
    return TRUE;
  }
  ...
  return caja_file_should_show(file, monitor->monitor_hidden_files, TRUE,
                               FALSE);
}

static gboolean is_needy(CajaFile *file, FileCheck check_missing,
...
static void directory_count_stop(CajaDirectory *directory) {
  if (directory->details->count_in_progress != NULL) {
    CajaFile *file;
...
static void count_children_done(CajaDirectory *directory, CajaFile *count_file,
                                gboolean succeeded, int count) {
...
static void directory_count_start(CajaDirectory *directory, CajaFile *file,
                                  gboolean *doing_io) {
  DirectoryCountState *state;
  GFile *location;
...
static void deep_count_one(DeepCountState *state, GFileInfo *info) {
  ...
  /* Count the disk size. */
  if (!is_seen_inode && g_file_info_has_attribute(
                            info, G_FILE_ATTRIBUTE_STANDARD_ALLOCATED_SIZE)) {
    file->details->deep_size_on_disk += g_file_info_get_attribute_uint64(
        info, G_FILE_ATTRIBUTE_STANDARD_ALLOCATED_SIZE);
  }
}
...
static void deep_count_stop(CajaDirectory *directory) {
  if (directory->details->deep_count_in_progress != NULL) {
    CajaFile *file;
...
static void deep_count_start(CajaDirectory *directory, CajaFile *file,
                             gboolean *doing_io) {
  GFile *location;
  DeepCountState *state;
...
static void mime_list_stop(CajaDirectory *directory) {
  if (directory->details->mime_list_in_progress != NULL) {
    CajaFile *file;
...
static void mime_list_start(CajaDirectory *directory, CajaFile *file,
                            gboolean *doing_io) {
  MimeListState *state;
  GFile *location;
...
static void top_left_stop(CajaDirectory *directory) {
  if (directory->details->top_left_read_state != NULL) {
    CajaFile *file;
    ...
    if (file != NULL) {
      g_assert(CAJA_IS_FILE(file));
      g_assert(file->details->directory == directory);
      if (is_needy(file, lacks_top_left, REQUEST_TOP_LEFT_TEXT) ||
          is_needy(file, lacks_large_top_left, REQUEST_LARGE_TOP_LEFT_TEXT)) {
        return;
      }
    }
...
static void top_left_read_state_free(TopLeftTextReadState *state) {
  g_object_unref(state->cancellable);
  g_free(state);
}

static void top_left_read_callback(GObject *source_object, GAsyncResult *res,
                                   gpointer callback_data) {
  ...
  if (g_file_load_partial_contents_finish(
          G_FILE(source_object), res, &file_contents, &file_size, NULL, NULL)) {
    file_details->top_left_text =
        caja_extract_top_left_text(file_contents, state->large, file_size);
...
static gboolean top_left_read_more_callback(const char *file_contents,
                                            goffset bytes_read,
                                            gpointer callback_data) {
  TopLeftTextReadState *state;
  ...
  if (state->large) {
    return bytes_read < CAJA_FILE_LARGE_TOP_LEFT_TEXT_MAXIMUM_BYTES &&
           count_lines(file_contents, bytes_read) <=
               CAJA_FILE_LARGE_TOP_LEFT_TEXT_MAXIMUM_LINES;
  } else {
    return bytes_read < CAJA_FILE_TOP_LEFT_TEXT_MAXIMUM_BYTES &&
           count_lines(file_contents, bytes_read) <=
               CAJA_FILE_TOP_LEFT_TEXT_MAXIMUM_LINES;
  }
}

static void top_left_start(CajaDirectory *directory, CajaFile *file,
                           gboolean *doing_io) {
  GFile *location;
  ...
  if (is_needy(file, lacks_large_top_left, REQUEST_LARGE_TOP_LEFT_TEXT)) {
    needs_large = TRUE;
  }

  /* Figure out which file to read the top left for. */
  if (!(needs_large || is_needy(file, lacks_top_left, REQUEST_TOP_LEFT_TEXT))) {
    return;
  }
  *doing_io = TRUE;
  ...
  location = caja_file_get_location(file);
  g_file_load_partial_contents_async(location, state->cancellable,
                                     top_left_read_more_callback,
                                     top_left_read_callback, state);
  g_object_unref(location);
}
...
static void file_info_stop(CajaDirectory *directory) {
  if (directory->details->get_info_in_progress != NULL) {
    CajaFile *file;
...
static void file_info_start(CajaDirectory *directory, CajaFile *file,
                            gboolean *doing_io) {
  GFile *location;
  GetInfoState *state;
...
static gboolean is_link_trusted(CajaFile *file, gboolean is_launcher) {
  gboolean res = FALSE;
  ...
  if (caja_file_is_local(file)) {
    ...
    if (!res) {
      res = caja_file_get_boolean_metadata(
          file, CAJA_METADATA_KEY_TRUSTED_LAUNCHER, FALSE);
    }
...
static gboolean should_read_link_info_sync(CajaFile *file) {
...
static void link_info_stop(CajaDirectory *directory) {
  if (directory->details->link_info_read_state != NULL) {
    CajaFile *file;
...
static void link_info_got_data(CajaDirectory *directory, CajaFile *file,
                               gboolean result, goffset bytes_read,
                               char *file_contents) {
  char *uri, *name, *icon;
  ...
  if (result) {
    ...
    caja_link_get_link_info_given_file_contents(file_contents, bytes_read,
                                                link_uri, &uri, &name, &icon,
                                                &is_launcher, &is_foreign);
    g_free(link_uri);
  } else {
...
static void link_info_caja_link_read_callback(GObject *source_object,
                                              GAsyncResult *res,
                                              gpointer user_data) {
  ...
  link_info_got_data(state->directory, state->file, result, file_size,
                     file_contents);
...
static void link_info_start(CajaDirectory *directory, CajaFile *file,
                            gboolean *doing_io) {
  GFile *location;
  ...
  *doing_io = TRUE;
  ...
  if (!caja_style_link) {
    ...
    result = g_file_load_contents(location, NULL, &file_contents, &file_size,
                                  NULL, NULL);
    ...
    g_file_load_contents_async(location, state->cancellable,
                               link_info_caja_link_read_callback, state);
  }
  g_object_unref(location);
}

static void thumbnail_done(CajaDirectory *directory, CajaFile *file,
...
static void thumbnail_stop(CajaDirectory *directory) {
  if (directory->details->thumbnail_state != NULL) {
    CajaFile *file;
...
static void thumbnail_got_pixbuf(CajaDirectory *directory, CajaFile *file,
                                 GdkPixbuf *pixbuf, gboolean tried_original) {
  caja_directory_ref(directory);
...
static void thumbnail_loader_size_prepared(GdkPixbufLoader *loader, int width,
  ...
  max_thumbnail_size =
      CAJA_ICON_SIZE_LARGEST * cached_thumbnail_size / CAJA_ICON_SIZE_STANDARD;
...
static void thumbnail_start(CajaDirectory *directory, CajaFile *file,
                            gboolean *doing_io) {
  GFile *location;
  ThumbnailState *state;
...
static void mount_stop(CajaDirectory *directory) {
  if (directory->details->mount_state != NULL) {
    CajaFile *file;
...
static void mount_start(CajaDirectory *directory, CajaFile *file,
                        gboolean *doing_io) {
  GFile *location;
  MountState *state;
...
static void filesystem_info_cancel(CajaDirectory *directory) {
  if (directory->details->filesystem_info_state != NULL) {
    g_cancellable_cancel(
...
static void filesystem_info_stop(CajaDirectory *directory) {
  if (directory->details->filesystem_info_state != NULL) {
    CajaFile *file;
  ...
  if (info != NULL) {
    ...
    file->details->filesystem_readonly =
        (g_file_info_get_attribute_boolean(
             info, G_FILE_ATTRIBUTE_FILESYSTEM_READONLY) != FALSE);
  }
...
static void filesystem_info_start(CajaDirectory *directory, CajaFile *file,
                                  gboolean *doing_io) {
  GFile *location;
  FilesystemInfoState *state;
  ...
  *doing_io = TRUE;
  ...
  g_file_query_filesystem_info_async(
      ...
      G_PRIORITY_DEFAULT, state->cancellable, query_filesystem_info_callback,
      state);
  g_object_unref(location);
}

static void extension_info_cancel(CajaDirectory *directory) {
...
static void extension_info_stop(CajaDirectory *directory) {
  if (directory->details->extension_info_in_progress != NULL) {
    CajaFile *file;
...
static void finish_info_provider(CajaDirectory *directory, CajaFile *file,
                                 CajaInfoProvider *provider) {
...
static gboolean info_provider_idle_callback(gpointer user_data) {
  ...
  if (response->handle != directory->details->extension_info_in_progress ||
    ...
    g_warning(
        "Unexpected plugin response.  This probably indicates a bug in a Caja "
        "extension: handle=%p",
        response->handle);
  } else {
    CajaFile *file;
...
static void info_provider_callback(CajaInfoProvider *provider,
                                   CajaOperationHandle *handle,
                                   CajaOperationResult result,
                                   gpointer user_data) {
  InfoProviderResponse *response;
...
static void extension_info_start(CajaDirectory *directory, CajaFile *file,
                                 gboolean *doing_io) {
  CajaInfoProvider *provider;
  CajaOperationResult result;
  CajaOperationHandle *handle;
  ...
  *doing_io = TRUE;
  ...
  g_closure_set_marshal(update_complete, caja_marshal_VOID__POINTER_ENUM);

  result = caja_info_provider_update_file_info(provider, CAJA_FILE_INFO(file),
                                               update_complete, &handle);

  g_closure_unref(update_complete);

  if (result == CAJA_OPERATION_COMPLETE || result == CAJA_OPERATION_FAILED) {
...
static void start_or_stop_io(CajaDirectory *directory) {
  CajaFile *file;
  gboolean doing_io;
...
static void cancel_directory_count_for_file(CajaDirectory *directory,
                                            CajaFile *file) {
  if (directory->details->count_in_progress != NULL &&
      directory->details->count_in_progress->count_file == file) {
    directory_count_cancel(directory);
...
static void cancel_deep_counts_for_file(CajaDirectory *directory,
                                        CajaFile *file) {
  if (directory->details->deep_count_file == file) {
    deep_count_cancel(directory);
  }
...
static void cancel_mime_list_for_file(CajaDirectory *directory,
                                      CajaFile *file) {
  if (directory->details->mime_list_in_progress != NULL &&
      directory->details->mime_list_in_progress->mime_list_file == file) {
    mime_list_cancel(directory);
...
static void cancel_top_left_text_for_file(CajaDirectory *directory,
                                          CajaFile *file) {
  if (directory->details->top_left_read_state != NULL &&
      directory->details->top_left_read_state->file == file) {
    top_left_cancel(directory);
...
static void cancel_file_info_for_file(CajaDirectory *directory,
                                      CajaFile *file) {
  if (directory->details->get_info_file == file) {
    file_info_cancel(directory);
  }
...
static void cancel_thumbnail_for_file(CajaDirectory *directory,
                                      CajaFile *file) {
  if (directory->details->thumbnail_state != NULL &&
      directory->details->thumbnail_state->file == file) {
    thumbnail_cancel(directory);
...
static void cancel_mount_for_file(CajaDirectory *directory, CajaFile *file) {
  if (directory->details->mount_state != NULL &&
      directory->details->mount_state->file == file) {
    mount_cancel(directory);
  }
...
static void cancel_filesystem_info_for_file(CajaDirectory *directory,
                                            CajaFile *file) {
  if (directory->details->filesystem_info_state != NULL &&
      directory->details->filesystem_info_state->file == file) {
    filesystem_info_cancel(directory);
...
static void cancel_link_info_for_file(CajaDirectory *directory,
                                      CajaFile *file) {
  if (directory->details->link_info_read_state != NULL &&
      directory->details->link_info_read_state->file == file) {
    link_info_cancel(directory);
...
void caja_directory_cancel_loading_file_attributes(
    CajaDirectory *directory, CajaFile *file,
    CajaFileAttributes file_attributes) {
  Request request;
...
void caja_directory_add_file_to_work_queue(CajaDirectory *directory,
  ...
  caja_file_queue_enqueue(directory->details->high_priority_queue, file);
}
...
void caja_directory_remove_file_from_work_queue(CajaDirectory *directory,
                                                CajaFile *file) {
  caja_file_queue_remove(directory->details->high_priority_queue, file);
  caja_file_queue_remove(directory->details->low_priority_queue, file);
  caja_file_queue_remove(directory->details->extension_queue, file);
}

static void move_file_to_low_priority_queue(CajaDirectory *directory,
                                            CajaFile *file) {
  /* Must add before removing to avoid ref underflow */
  caja_file_queue_enqueue(directory->details->low_priority_queue, file);
  caja_file_queue_remove(directory->details->high_priority_queue, file);
}
...
static void move_file_to_extension_queue(CajaDirectory *directory,
                                         CajaFile *file) {
  /* Must add before removing to avoid ref underflow */
  caja_file_queue_enqueue(directory->details->extension_queue, file);
  caja_file_queue_remove(directory->details->low_priority_queue, file);
}
