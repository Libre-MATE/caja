 ...
 * visible. If this is 0, you can get into trouble with extra scrolling after
 * holding down the arrow key for awhile when there are many items.
 */
...
#define SNAP_HORIZONTAL(func, x)                                              \
  ((func((double)((x)-DESKTOP_PAD_HORIZONTAL) / SNAP_SIZE_X) * SNAP_SIZE_X) + \
   DESKTOP_PAD_HORIZONTAL)
#define SNAP_VERTICAL(func, y)                                              \
  ((func((double)((y)-DESKTOP_PAD_VERTICAL) / SNAP_SIZE_Y) * SNAP_SIZE_Y) + \
...
enum { ACTION_ACTIVATE, ACTION_MENU, LAST_ACTION };
...
typedef struct _CajaIconContainerAccessibleClass
    CajaIconContainerAccessibleClass;
...
static void activate_selected_items_alternate(CajaIconContainer *container,
                                              CajaIcon *icon);
static void compute_stretch(StretchState *start, StretchState *current);
static CajaIcon *get_first_selected_icon(CajaIconContainer *container);
static CajaIcon *get_nth_selected_icon(CajaIconContainer *container, int index);
...
static void icon_destroy(CajaIconContainer *container, CajaIcon *icon);
static void end_renaming_mode(CajaIconContainer *container, gboolean commit);
...
static inline void icon_get_bounding_box(CajaIcon *icon, int *x1_return,
                                         int *y1_return, int *x2_return,
                                         int *y2_return,
...
static void caja_icon_container_stop_monitor_top_left(
    CajaIconContainer *container, CajaIconData *data, gconstpointer client);
static void caja_icon_container_start_monitor_top_left(
    CajaIconContainer *container, CajaIconData *data, gconstpointer client,
    gboolean large_text);
...
static void handle_vadjustment_changed(GtkAdjustment *adjustment,
                                       CajaIconContainer *container);
static GList *caja_icon_container_get_selected_icons(
    CajaIconContainer *container);
static void caja_icon_container_update_visible_icons(
    CajaIconContainer *container);
static void reveal_icon(CajaIconContainer *container, CajaIcon *icon);
...
static double get_mirror_x_position(CajaIconContainer *container,
                                    CajaIcon *icon, double x);
static void text_ellipsis_limit_changed_container_callback(
    gpointer callback_data);

static int compare_icons_horizontal(CajaIconContainer *container,
                                    CajaIcon *icon_a, CajaIcon *icon_b);
...
static const char *caja_icon_container_accessible_action_descriptions[] = {
    "Activate selected items", "Popup context menu", NULL};
...
static void icon_free(CajaIcon *icon) {
  /* Destroy this canvas item; the parent will unref it. */
  eel_canvas_item_destroy(EEL_CANVAS_ITEM(icon->item));
  g_free(icon);
}

static gboolean icon_is_positioned(const CajaIcon *icon) {
  return icon->x != ICON_UNPOSITIONED_VALUE &&
         icon->y != ICON_UNPOSITIONED_VALUE;
}

/* x, y are the top-left coordinates of the icon. */
static void icon_set_position(CajaIcon *icon, double x, double y) {
  CajaIconContainer *container;
  int x1, x2, y1, y2;
  ...
  if (caja_icon_container_get_is_fixed_size(container)) {
    ...
    */
    ...
    max_y =
        container_bottom - DESKTOP_PAD_VERTICAL - item_height + height_above;
  ...
  eel_canvas_item_move(EEL_CANVAS_ITEM(icon->item), x - icon->x, y - icon->y);

  icon->x = x;
  icon->y = y;
}

static void icon_get_size(CajaIconContainer *container, CajaIcon *icon,
                          guint *size) {
  if (size != NULL) {
    *size =
        MAX(caja_get_icon_size_for_zoom_level(container->details->zoom_level) *
                icon->scale,
 ...
 */
static void icon_set_size(CajaIconContainer *container, CajaIcon *icon,
                          guint icon_size, gboolean snap,
  ...
  scale = (double)icon_size /
          caja_get_icon_size_for_zoom_level(container->details->zoom_level);
  caja_icon_container_move_icon(container, icon, icon->x, icon->y, scale, FALSE,
                                snap, update_position);
}

static void icon_raise(CajaIcon *icon) {
  EelCanvasItem *item, *band;

  item = EEL_CANVAS_ITEM(icon->item);
  band = CAJA_ICON_CONTAINER(item->canvas)
             ->details->rubberband_info.selection_rectangle;

  eel_canvas_item_send_behind(item, band);
}

static void emit_stretch_started(CajaIconContainer *container, CajaIcon *icon) {
  g_signal_emit(container, signals[ICON_STRETCH_STARTED], 0, icon->data);
}

static void emit_stretch_ended(CajaIconContainer *container, CajaIcon *icon) {
  g_signal_emit(container, signals[ICON_STRETCH_ENDED], 0, icon->data);
}

static void icon_toggle_selected(CajaIconContainer *container, CajaIcon *icon) {
  end_renaming_mode(container, TRUE);

  icon->is_selected = !icon->is_selected;
  eel_canvas_item_set(EEL_CANVAS_ITEM(icon->item), "highlighted_for_selection",
                      (gboolean)icon->is_selected, NULL);
  ...
  if (icon == container->details->stretch_icon) {
    ...
    if (container->details->keep_aligned) {
      caja_icon_container_move_icon(container, icon, icon->x, icon->y,
                                    icon->scale, FALSE, TRUE, TRUE);
    }
...
static gboolean icon_set_selected(CajaIconContainer *container, CajaIcon *icon,
                                  gboolean select) {
...
static inline void icon_get_bounding_box(CajaIcon *icon, int *x1_return,
  ...
  if (usage == BOUNDS_USAGE_FOR_DISPLAY) {
    eel_canvas_item_get_bounds(EEL_CANVAS_ITEM(icon->item), &x1, &y1, &x2, &y2);
  } else if (usage == BOUNDS_USAGE_FOR_LAYOUT) {
    caja_icon_canvas_item_get_bounds_for_layout(icon->item, &x1, &y1, &x2, &y2);
  } else if (usage == BOUNDS_USAGE_FOR_ENTIRE_ITEM) {
    caja_icon_canvas_item_get_bounds_for_entire_item(icon->item, &x1, &y1, &x2,
                                                     &y2);
  } else {
...
gboolean caja_icon_container_scroll(CajaIconContainer *container, int delta_x,
                                    int delta_y) {
  ...
  return gtk_adjustment_get_value(hadj) != old_h_value ||
         gtk_adjustment_get_value(vadj) != old_v_value;
}

static void pending_icon_to_reveal_destroy_callback(
    CajaIconCanvasItem *item, CajaIconContainer *container) {
...
static CajaIcon *get_pending_icon_to_reveal(CajaIconContainer *container) {
  return container->details->pending_icon_to_reveal;
}

static void set_pending_icon_to_reveal(CajaIconContainer *container,
                                       CajaIcon *icon) {
  CajaIcon *old_icon;
  ...
  if (old_icon != NULL) {
    g_signal_handlers_disconnect_by_func(
        old_icon->item, G_CALLBACK(pending_icon_to_reveal_destroy_callback),
        container);
  }
...
static void item_get_canvas_bounds(EelCanvasItem *item, EelIRect *bounds,
                                   gboolean safety_pad) {
  EelDRect world_rect;

  eel_canvas_item_get_bounds(item, &world_rect.x0, &world_rect.y0,
                             &world_rect.x1, &world_rect.y1);
  eel_canvas_item_i2w(item->parent, &world_rect.x0, &world_rect.y0);
  eel_canvas_item_i2w(item->parent, &world_rect.x1, &world_rect.y1);
  if (safety_pad) {
  ...
  eel_canvas_w2c(item->canvas, world_rect.x0, world_rect.y0, &bounds->x0,
                 &bounds->y0);
  eel_canvas_w2c(item->canvas, world_rect.x1, world_rect.y1, &bounds->x1,
                 &bounds->y1);
}

static void icon_get_row_and_column_bounds(CajaIconContainer *container,
                                           CajaIcon *icon, EelIRect *bounds,
                                           gboolean safety_pad) {
  GList *p;
  EelIRect one_bounds;
...
static void process_pending_icon_to_reveal(CajaIconContainer *container) {
...
static gboolean keyboard_icon_reveal_timeout_callback(gpointer data) {
  CajaIconContainer *container;
  CajaIcon *icon;
...
static void unschedule_keyboard_icon_reveal(CajaIconContainer *container) {
...
static void schedule_keyboard_icon_reveal(CajaIconContainer *container,
                                          CajaIcon *icon) {
  CajaIconContainerDetails *details;
  ...
  details->keyboard_icon_reveal_timer_id =
      g_timeout_add(KEYBOARD_ICON_REVEAL_TIMEOUT,
                    keyboard_icon_reveal_timeout_callback, container);
}

static void clear_keyboard_focus(CajaIconContainer *container) {
  if (container->details->keyboard_focus != NULL) {
    eel_canvas_item_set(
        EEL_CANVAS_ITEM(container->details->keyboard_focus->item),
        "highlighted_as_keyboard_focus", 0, NULL);
  }

  container->details->keyboard_focus = NULL;
}

static inline void emit_atk_focus_state_change(CajaIcon *icon,
                                               gboolean focused) {
  AtkObject *atk_object =
      atk_gobject_accessible_for_object(G_OBJECT(icon->item));
...
static void set_keyboard_focus(CajaIconContainer *container, CajaIcon *icon) {
  g_assert(icon != NULL);
  ...
  eel_canvas_item_set(EEL_CANVAS_ITEM(container->details->keyboard_focus->item),
                      "highlighted_as_keyboard_focus", 1, NULL);

  emit_atk_focus_state_change(icon, TRUE);
}

static void set_keyboard_rubberband_start(CajaIconContainer *container,
                                          CajaIcon *icon) {
  container->details->keyboard_rubberband_start = icon;
}

static void clear_keyboard_rubberband_start(CajaIconContainer *container) {
 ...
 */
static void get_icon_bounds_for_canvas_bounds(
    EelCanvasGroup *group, double *x1, double *y1, double *x2, double *y2,
    CajaIconCanvasItemBoundsUsage usage) {
  EelCanvasItem *child;
  GList *list;
  ...
  for (list = group->item_list; list; list = list->next) {
    ...
    if (child->flags & EEL_CANVAS_ITEM_VISIBLE) {
      ...
      if (!CAJA_IS_ICON_CANVAS_ITEM(child) ||
        ...
        caja_icon_canvas_item_get_bounds_for_layout(
            CAJA_ICON_CANVAS_ITEM(child), &minx, &miny, &maxx, &maxy);
      } else if (usage == BOUNDS_USAGE_FOR_ENTIRE_ITEM) {
        caja_icon_canvas_item_get_bounds_for_entire_item(
            CAJA_ICON_CANVAS_ITEM(child), &minx, &miny, &maxx, &maxy);
      } else {
        g_assert_not_reached();
      }
  ...
  for (; list; list = list->next) {
    ...
    if (!(child->flags & EEL_CANVAS_ITEM_VISIBLE)) continue;

    if (!CAJA_IS_ICON_CANVAS_ITEM(child) || usage == BOUNDS_USAGE_FOR_DISPLAY) {
      ...
      caja_icon_canvas_item_get_bounds_for_entire_item(
          CAJA_ICON_CANVAS_ITEM(child), &tx1, &ty1, &tx2, &ty2);
    } else {
      g_assert_not_reached();
    }
...
static void get_all_icon_bounds(CajaIconContainer *container, double *x1,
                                double *y1, double *x2, double *y2,
                                CajaIconCanvasItemBoundsUsage usage) {
  /* FIXME bugzilla.gnome.org 42477: Do we have to do something about the
   * rubberband here? Any other non-icon items?
   */
  get_icon_bounds_for_canvas_bounds(
      EEL_CANVAS_GROUP(EEL_CANVAS(container)->root), x1, y1, x2, y2, usage);
}
...
/* Set a new scroll region without eliminating any of the currently-visible
 * area. */
static void canvas_set_scroll_region_include_visible_area(EelCanvas *canvas,
                                                          double x1, double y1,
                                                          double x2,
                                                          double y2) {
  ...
  old_scroll_x = gtk_adjustment_get_value(
      gtk_scrollable_get_hadjustment(GTK_SCROLLABLE(canvas)));
  old_scroll_y = gtk_adjustment_get_value(
  ...
  eel_canvas_set_scroll_region(canvas, x1, y1, x2, y2);
}
...
void caja_icon_container_update_scroll_region(CajaIconContainer *container) {
  ...
  if (caja_icon_container_get_is_fixed_size(container)) {
    ...
    eel_canvas_set_scroll_region(
        ...
        ((double)(allocation.width - 1) - container->details->left_margin -
         container->details->right_margin) /
            pixels_per_unit,
        ((double)(allocation.height - 1) - container->details->top_margin -
         container->details->bottom_margin) /
            pixels_per_unit);
    return;
  }
  ...
  if (reset_scroll_region) {
    ...
    canvas_set_scroll_region_include_visible_area(EEL_CANVAS(container), x1, y1,
                                                  x2, y2);
  }
  ...
  step_increment =
      caja_get_icon_size_for_zoom_level(container->details->zoom_level) / 4;
...
static int compare_icons(gconstpointer a, gconstpointer b,
...
static void sort_icons(CajaIconContainer *container, GList **icons) {
  CajaIconContainerClass *klass;
...
static void resort(CajaIconContainer *container) {
...
static void lay_down_one_line(CajaIconContainer *container, GList *line_start,
                              GList *line_end, double y, double max_height,
                              GArray *positions, gboolean whole_text) {
  GList *p;
  double x, y_offset;
  ...
  for (p = line_start; p != line_end; p = p->next) {
    ...
    icon_set_position(
        icon,
        is_rtl ? get_mirror_x_position(container, icon, x + position->x_offset)
               : x + position->x_offset,
        y + y_offset);
    caja_icon_canvas_item_set_entire_text(icon->item, whole_text);

    icon->saved_ltr_x =
        is_rtl ? get_mirror_x_position(container, icon, icon->x) : icon->x;

    x += position->width;
  }
}

static void lay_down_one_column(CajaIconContainer *container, GList *line_start,
                                GList *line_end, double x, double y_start,
                                double y_iter, GArray *positions) {
  GList *p;
  double y;
  int i;
  gboolean is_rtl;
...
static void lay_down_icons_horizontal(CajaIconContainer *container,
                                      GList *icons, double start_y) {
  GList *p, *line_start;
  CajaIcon *icon;
  ...
  if (container->details->label_position == CAJA_ICON_LABEL_POSITION_BESIDE) {
    ...
    for (p = icons; p != NULL; p = p->next) {
      ...
      icon_bounds = caja_icon_canvas_item_get_icon_rectangle(icon->item);
      max_icon_width =
          MAX(max_icon_width, ceil(icon_bounds.x1 - icon_bounds.x0));

      text_bounds = caja_icon_canvas_item_get_text_rectangle(icon->item, TRUE);
      max_text_width =
          MAX(max_text_width, ceil(text_bounds.x1 - text_bounds.x0));
    }

    grid_width =
        max_icon_width + max_text_width + ICON_PAD_LEFT + ICON_PAD_RIGHT;
  } else {
    int num_columns;
  ...
  for (p = icons; p != NULL; p = p->next) {
     ...
     */
    caja_icon_canvas_item_get_bounds_for_layout(
        icon->item, &bounds.x0, &bounds.y0, &bounds.x1, &bounds.y1);
    ...
    if (line_start != p && line_width + icon_width >= canvas_width) {
      ...
      lay_down_one_line(container, line_start, p, y, max_height_above,
                        positions, FALSE);
    ...
    if (container->details->label_position == CAJA_ICON_LABEL_POSITION_BESIDE) {
      if (gridded_layout) {
        position->x_offset = max_icon_width + ICON_PAD_LEFT + ICON_PAD_RIGHT -
                             (icon_bounds.x1 - icon_bounds.x0);
      } else {
        position->x_offset = icon_width - ((icon_bounds.x1 - icon_bounds.x0) +
  ...
  if (line_start != NULL) {
    ...
    lay_down_one_line(container, line_start, NULL, y, max_height_above,
                      positions, TRUE);
...
static void get_max_icon_dimensions(GList *icon_start, GList *icon_end,
  ...
  *max_bounds_height = 0.0;
  ...
  for (p = icon_start; p != icon_end; p = p->next) {
    ...
    *max_icon_width =
        MAX(*max_icon_width, ceil(icon_bounds.x1 - icon_bounds.x0));
    *max_icon_height =
        MAX(*max_icon_height, ceil(icon_bounds.y1 - icon_bounds.y0));

    text_bounds = caja_icon_canvas_item_get_text_rectangle(icon->item, TRUE);
    *max_text_width =
        MAX(*max_text_width, ceil(text_bounds.x1 - text_bounds.x0));
    *max_text_height =
        MAX(*max_text_height, ceil(text_bounds.y1 - text_bounds.y0));

    caja_icon_canvas_item_get_bounds_for_layout(icon->item, NULL, &y1, NULL,
                                                &y2);
    *max_bounds_height = MAX(*max_bounds_height, y2 - y1);
  }
}

/* column-wise layout. At the moment, this only works with label-beside-icon
 * (used by "Compact View"). */
static void lay_down_icons_vertical(CajaIconContainer *container, GList *icons,
                                    double start_y) {
  GList *p, *line_start;
  ...
  g_assert(CAJA_IS_ICON_CONTAINER(container));
  g_assert(container->details->label_position ==
           CAJA_ICON_LABEL_POSITION_BESIDE);
  ...
  get_max_icon_dimensions(icons, NULL, &max_icon_width, &max_icon_height,
  ...
  for (p = icons; p != NULL; p = p->next) {
    ...
    /* If this icon doesn't fit, it's time to lay out the column that's queued
     * up. */

    /* We use the bounds height here, since for wrapping we also want to
     * consider overlapping emblems at the bottom. We may wrap a little bit too
     * early since the icon with the max. bounds height may actually not be in
     * the last row, but it is better than visual glitches
     */
    if (line_start != p &&
        line_height + (max_bounds_height_with_borders - 1) >= canvas_height) {
      x += ICON_PAD_LEFT;
      ...
      lay_down_one_column(container, line_start, p, x, CONTAINER_PAD_TOP,
                          max_height_with_borders, positions);
    ...
    max_width_in_column =
        MAX(max_width_in_column, ceil(icon_bounds.x1 - icon_bounds.x0) +
    ...
    position->x_offset +=
        max_icon_width - ceil(icon_bounds.x1 - icon_bounds.x0);

    height = MAX(ceil(icon_bounds.y1 - icon_bounds.y0),
                 ceil(text_bounds.y1 - text_bounds.y0));
  ...
  if (line_start != NULL) {
    x += ICON_PAD_LEFT;
    lay_down_one_column(container, line_start, NULL, x, CONTAINER_PAD_TOP,
                        max_height_with_borders, positions);
  }
...
static void snap_position(CajaIconContainer *container, CajaIcon *icon, int *x,
                          int *y) {
  int center_x;
  int baseline_y;
  int icon_width;
  ...
  if (*x + icon_width / 2 >
      total_width - (DESKTOP_PAD_HORIZONTAL + SNAP_SIZE_X)) {
    *x =
        total_width - (DESKTOP_PAD_HORIZONTAL + SNAP_SIZE_X + (icon_width / 2));
  }
  ...
  if (*y + icon_height > total_height - (DESKTOP_PAD_VERTICAL + SNAP_SIZE_Y)) {
    *y =
        total_height - (DESKTOP_PAD_VERTICAL + SNAP_SIZE_Y + (icon_height / 2));
  }
...
static int compare_icons_by_position(gconstpointer a, gconstpointer b) {
  ...
  icon_get_bounding_box(icon_a, &x1, &y1, &x2, &y2, BOUNDS_USAGE_FOR_DISPLAY);
  center_a = x1 + (x2 - x1) / 2;
  icon_get_bounding_box(icon_b, &x1, &y1, &x2, &y2, BOUNDS_USAGE_FOR_DISPLAY);
  center_b = x1 + (x2 - x1) / 2;

  return center_a == center_b ? icon_a->y - icon_b->y : center_a - center_b;
}

static PlacementGrid *placement_grid_new(CajaIconContainer *container,
                                         gboolean tight) {
  PlacementGrid *grid;
  int width, height;
  int num_columns;
...
static void placement_grid_free(PlacementGrid *grid) {
  g_free(grid->icon_grid);
  g_free(grid->grid_memory);
  g_free(grid);
}

static gboolean placement_grid_position_is_free(PlacementGrid *grid,
                                                EelIRect pos) {
  int x, y;
...
static void placement_grid_mark(PlacementGrid *grid, EelIRect pos) {
  int x, y;
...
static void canvas_position_to_grid_position(PlacementGrid *grid,
   ...
   * during a snap. */
  if (grid->tight) {
    grid_position->x0 = ceil(
        (double)(canvas_position.x0 - DESKTOP_PAD_HORIZONTAL) / SNAP_SIZE_X);
    grid_position->y0 =
        ceil((double)(canvas_position.y0 - DESKTOP_PAD_VERTICAL) / SNAP_SIZE_Y);
    grid_position->x1 = floor(
        (double)(canvas_position.x1 - DESKTOP_PAD_HORIZONTAL) / SNAP_SIZE_X);
    grid_position->y1 = floor(
        (double)(canvas_position.y1 - DESKTOP_PAD_VERTICAL) / SNAP_SIZE_Y);
  } else {
    grid_position->x0 = floor(
        (double)(canvas_position.x0 - DESKTOP_PAD_HORIZONTAL) / SNAP_SIZE_X);
    grid_position->y0 = floor(
        (double)(canvas_position.y0 - DESKTOP_PAD_VERTICAL) / SNAP_SIZE_Y);
    grid_position->x1 = floor(
        (double)(canvas_position.x1 - DESKTOP_PAD_HORIZONTAL) / SNAP_SIZE_X);
    grid_position->y1 = floor(
        (double)(canvas_position.y1 - DESKTOP_PAD_VERTICAL) / SNAP_SIZE_Y);
  ...
  grid_position->x1 =
      CLAMP(grid_position->x1, grid_position->x0, grid->num_columns - 1);
  grid_position->y1 =
      CLAMP(grid_position->y1, grid_position->y0, grid->num_rows - 1);
}

static void placement_grid_mark_icon(PlacementGrid *grid, CajaIcon *icon) {
  EelIRect icon_pos;
  EelIRect grid_pos;

  icon_get_bounding_box(icon, &icon_pos.x0, &icon_pos.y0, &icon_pos.x1,
                        &icon_pos.y1, BOUNDS_USAGE_FOR_LAYOUT);
  canvas_position_to_grid_position(grid, icon_pos, &grid_pos);
  placement_grid_mark(grid, grid_pos);
}

static void find_empty_location(CajaIconContainer *container,
                                PlacementGrid *grid, CajaIcon *icon,
                                int start_x, int start_y, int *x, int *y) {
  ...
  icon_get_bounding_box(icon, NULL, &icon_position.y0, NULL, &icon_position.y1,
                        BOUNDS_USAGE_FOR_ENTIRE_ITEM);
  ...
  do {
    ...
    canvas_position_to_grid_position(grid, icon_position, &grid_position);

    need_new_column =
        icon_position.y0 + height_for_bound_check + DESKTOP_PAD_VERTICAL >
        canvas_height;
    ...
    if (need_new_column ||
      ...
      if (need_new_column) {
        ...
        icon_position.y0 = DESKTOP_PAD_VERTICAL + SNAP_SIZE_Y -
                           (pixbuf_rect.y1 - pixbuf_rect.y0);
...
static void align_icons(CajaIconContainer *container) {
  ...
  unplaced_icons = g_list_sort(unplaced_icons, compare_icons_by_position);
...
static double get_mirror_x_position(CajaIconContainer *container,
  ...
  return CANVAS_WIDTH(container, allocation) - x -
         (icon_bounds.x1 - icon_bounds.x0);
}

static void caja_icon_container_set_rtl_positions(
    CajaIconContainer *container) {
  GList *l;
  CajaIcon *icon = NULL;
...
static void lay_down_icons_vertical_desktop(CajaIconContainer *container,
                                            GList *icons) {
  ...
  if (placed > 0) {
    ...
    if (grid) {
      for (p = placed_icons; p != NULL; p = p->next) {
        placement_grid_mark_icon(grid, (CajaIcon *)p->data);
      }
    ...
    while (icons != NULL) {
      ...
      should_snap = !(container->details->tighter_layout &&
                      !container->details->keep_aligned);
      ...
      for (p = icons; p != NULL; p = p->next) {
        ...
        if (y != DESKTOP_PAD_VERTICAL &&
            y + icon_height_for_bound_check > height) {
          break;
        }
      ...
      for (p = icons; p != NULL; p = p->next) {
        ...
        /* Check and see if we need to move to a new column */
        if (y != DESKTOP_PAD_VERTICAL &&
            y > height - icon_height_for_bound_check &&
            /* Make sure we lay out at least one icon per column, to make
               progress */
            p != icons) {
        ...
        icon_set_position(icon, center_x - (icon_rect.x1 - icon_rect.x0) / 2,
                          y);
...
static void lay_down_icons(CajaIconContainer *container, GList *icons,
                           double start_y) {
...
static void redo_layout_internal(CajaIconContainer *container) {
...
static gboolean redo_layout_callback(gpointer callback_data) {
  CajaIconContainer *container;
...
static void unschedule_redo_layout(CajaIconContainer *container) {
...
static void schedule_redo_layout(CajaIconContainer *container) {
  if (container->details->idle_id == 0 &&
      container->details->has_been_allocated) {
    container->details->idle_id = g_idle_add(redo_layout_callback, container);
  }
}

static void redo_layout(CajaIconContainer *container) {
  unschedule_redo_layout(container);
  redo_layout_internal(container);
}

static void reload_icon_positions(CajaIconContainer *container) {
  ...
  for (p = container->details->icons; p != NULL; p = p->next) {
    ...
    g_signal_emit(container, signals[GET_STORED_ICON_POSITION], 0, icon->data,
                  &position, &have_stored_position);
    if (have_stored_position) {
      icon_set_position(icon, position.x, position.y);
      item = EEL_CANVAS_ITEM(icon->item);
      caja_icon_canvas_item_get_bounds_for_layout(
          icon->item, &bounds.x0, &bounds.y0, &bounds.x1, &bounds.y1);
      eel_canvas_item_i2w(item->parent, &bounds.x0, &bounds.y0);
      eel_canvas_item_i2w(item->parent, &bounds.x1, &bounds.y1);
      if (bounds.y1 > bottom) {
        bottom = bounds.y1;
      }
    } else {
...
static void invalidate_label_sizes(CajaIconContainer *container) {
  GList *p;
  CajaIcon *icon = NULL;
...
static void invalidate_labels(CajaIconContainer *container) {
  GList *p;
  CajaIcon *icon = NULL;
...
static gboolean select_range(CajaIconContainer *container, CajaIcon *icon1,
  ...
  for (p = container->details->icons; p != NULL; p = p->next) {
    ...
    if (select || unselect_outside_range) {
      selection_changed |= icon_set_selected(container, icon, select);
    }
...
static gboolean select_one_unselect_others(CajaIconContainer *container,
  ...
  for (p = container->details->icons; p != NULL; p = p->next) {
    ...
    selection_changed |=
        icon_set_selected(container, icon, icon == icon_to_select);
  }
...
static gboolean unselect_all(CajaIconContainer *container) {
  return select_one_unselect_others(container, NULL);
}

void caja_icon_container_move_icon(CajaIconContainer *container, CajaIcon *icon,
                                   int x, int y, double scale, gboolean raise,
                                   gboolean snap, gboolean update_position) {
  CajaIconContainerDetails *details;
  ...
  if (!details->auto_layout && !details->lock_icons_position) {
    ...
    icon->saved_ltr_x = caja_icon_container_is_layout_rtl(container)
                            ? get_mirror_x_position(container, icon, icon->x)
                            : icon->x;
  }
  ...
  if (emit_signal) {
    ...
    g_signal_emit(container, signals[ICON_POSITION_CHANGED], 0, icon->data,
                  &position);
  }
...
static void rubberband_select(CajaIconContainer *container,
  ...
  for (p = container->details->icons; p != NULL; p = p->next) {
    ...
    if (!canvas_rect_calculated) {
      ...
      eel_canvas_w2c(canvas, current_rect->x0, current_rect->y0,
                     &canvas_rect.x0, &canvas_rect.y0);
      eel_canvas_w2c(canvas, current_rect->x1, current_rect->y1,
                     &canvas_rect.x1, &canvas_rect.y1);
    ...
    selection_changed |= icon_set_selected(
        container, icon, is_in ^ icon->was_selected_before_rubberband);
  }
...
static int rubberband_timeout_callback(gpointer data) {
  CajaIconContainer *container;
  GtkWidget *widget;
  ...
  adj_x = gtk_adjustment_get_value(
  ...
  adj_y = gtk_adjustment_get_value(
  ...
  if (y_scroll == 0 && x_scroll == 0 && (int)band_info->prev_x == x &&
      (int)band_info->prev_y == y && !adj_changed) {
    return TRUE;
  }
  ...
  eel_canvas_item_set(band_info->selection_rectangle, "x1", x1, "y1", y1, "x2",
                      x2, "y2", y2, NULL);
  ...
  rubberband_select(container, &band_info->prev_rect, &selection_rect);
...
/*borrowed from Nemo, makes Caja rubberbanding follow same selectors as Nemo and
 * presumably Nautilus */
static void start_rubberbanding(CajaIconContainer *container,
  ...
  gtk_style_context_get(context, GTK_STATE_FLAG_NORMAL,
                        GTK_STYLE_PROPERTY_BORDER_COLOR, &c, NULL);
  ...
  band_info->selection_rectangle = eel_canvas_item_new(
      eel_canvas_root(EEL_CANVAS(container)), EEL_TYPE_CANVAS_RECT, "x1",
      band_info->start_x, "y1", band_info->start_y, "x2", band_info->start_x,
      "y2", band_info->start_y, "fill_color_rgba", &bg_color,
      "outline_color_rgba", &border_color, "width_pixels", 1, NULL);
  ...
  if (band_info->timer_id == 0) {
    band_info->timer_id = g_timeout_add(RUBBERBAND_TIMEOUT_INTERVAL,
                                        rubberband_timeout_callback, container);
  }

  eel_canvas_item_grab(
      band_info->selection_rectangle,
      (GDK_POINTER_MOTION_MASK | GDK_BUTTON_RELEASE_MASK | GDK_SCROLL_MASK),
      NULL, (GdkEvent *)event);
}

static void stop_rubberbanding(CajaIconContainer *container) {
  CajaIconRubberbandInfo *band_info;
  GList *icons;
  ...
  g_signal_emit(container, signals[BAND_SELECT_ENDED], 0);
}
...
static CajaIcon *find_best_icon(CajaIconContainer *container,
                                CajaIcon *start_icon,
                                IsBetterIconFunction function, void *data) {
  GList *p;
...
static CajaIcon *find_best_selected_icon(CajaIconContainer *container,
                                         CajaIcon *start_icon,
...
static int compare_icons_by_uri(CajaIconContainer *container, CajaIcon *icon_a,
                                CajaIcon *icon_b) {
  char *uri_a, *uri_b;
  int result;
...
static int get_cmp_point_x(CajaIconContainer *container, EelDRect icon_rect) {
  if (container->details->label_position == CAJA_ICON_LABEL_POSITION_BESIDE) {
...
static int get_cmp_point_y(CajaIconContainer *container, EelDRect icon_rect) {
  if (container->details->label_position == CAJA_ICON_LABEL_POSITION_BESIDE) {
...
static int compare_icons_horizontal(CajaIconContainer *container,
  ...
  eel_canvas_w2c(EEL_CANVAS(container), get_cmp_point_x(container, world_rect),
                 get_cmp_point_y(container, world_rect), &ax, NULL);
  world_rect = caja_icon_canvas_item_get_icon_rectangle(icon_b->item);
  eel_canvas_w2c(EEL_CANVAS(container), get_cmp_point_x(container, world_rect),
                 get_cmp_point_y(container, world_rect), &bx, NULL);
...
static int compare_icons_vertical(CajaIconContainer *container,
  ...
  world_rect = caja_icon_canvas_item_get_icon_rectangle(icon_a->item);
  eel_canvas_w2c(EEL_CANVAS(container), get_cmp_point_x(container, world_rect),
                 get_cmp_point_y(container, world_rect), NULL, &ay);
  world_rect = caja_icon_canvas_item_get_icon_rectangle(icon_b->item);
  eel_canvas_w2c(EEL_CANVAS(container), get_cmp_point_x(container, world_rect),
                 get_cmp_point_y(container, world_rect), NULL, &by);
...
static int compare_icons_horizontal_first(CajaIconContainer *container,
                                          CajaIcon *icon_a, CajaIcon *icon_b) {
  EelDRect world_rect;
  int ax, ay, bx, by;
  ...
  world_rect = caja_icon_canvas_item_get_icon_rectangle(icon_a->item);
  eel_canvas_w2c(EEL_CANVAS(container), get_cmp_point_x(container, world_rect),
                 get_cmp_point_y(container, world_rect), &ax, &ay);
  world_rect = caja_icon_canvas_item_get_icon_rectangle(icon_b->item);
  eel_canvas_w2c(EEL_CANVAS(container), get_cmp_point_x(container, world_rect),
                 get_cmp_point_y(container, world_rect), &bx, &by);
...
static int compare_icons_vertical_first(CajaIconContainer *container,
                                        CajaIcon *icon_a, CajaIcon *icon_b) {
  EelDRect world_rect;
  int ax, ay, bx, by;
...
static gboolean leftmost_in_top_row(CajaIconContainer *container,
                                    CajaIcon *start_icon, CajaIcon *best_so_far,
                                    CajaIcon *candidate, void *data) {
  if (best_so_far == NULL) {
    return TRUE;
  }
...
static gboolean rightmost_in_top_row(CajaIconContainer *container,
                                     CajaIcon *start_icon,
                                     CajaIcon *best_so_far, CajaIcon *candidate,
                                     void *data) {
  if (best_so_far == NULL) {
    return TRUE;
  }
...
static gboolean rightmost_in_bottom_row(CajaIconContainer *container,
                                        CajaIcon *start_icon,
                                        CajaIcon *best_so_far,
                                        CajaIcon *candidate, void *data) {
  if (best_so_far == NULL) {
...
static int compare_with_start_row(CajaIconContainer *container,
                                  CajaIcon *icon) {
  EelCanvasItem *item;
...
static int compare_with_start_column(CajaIconContainer *container,
                                     CajaIcon *icon) {
  EelCanvasItem *item;
...
static gboolean same_row_right_side_leftmost(CajaIconContainer *container,
                                             CajaIcon *start_icon,
                                             CajaIcon *best_so_far,
                                             CajaIcon *candidate, void *data) {
  ...
  if (best_so_far != NULL) {
    if (compare_icons_horizontal_first(container, best_so_far, candidate) < 0) {
      return FALSE;
    }
  }

  /* Candidate to the left of the start do not qualify. */
  if (compare_icons_horizontal_first(container, candidate, start_icon) <= 0) {
    return FALSE;
  }
...
static gboolean same_row_left_side_rightmost(CajaIconContainer *container,
                                             CajaIcon *start_icon,
                                             CajaIcon *best_so_far,
                                             CajaIcon *candidate, void *data) {
  /* Candidates not on the start row do not qualify. */
...
static gboolean next_row_leftmost(CajaIconContainer *container,
                                  CajaIcon *start_icon, CajaIcon *best_so_far,
                                  CajaIcon *candidate, void *data) {
  ...
  if (best_so_far != NULL) {
    if (compare_icons_vertical_first(container, best_so_far, candidate) > 0) {
...
static gboolean next_row_rightmost(CajaIconContainer *container,
                                   CajaIcon *start_icon, CajaIcon *best_so_far,
                                   CajaIcon *candidate, void *data) {
  /* sort out icons that are not below the current row */
  if (compare_with_start_row(container, candidate) >= 0) {
...
static gboolean next_column_bottommost(CajaIconContainer *container,
                                       CajaIcon *start_icon,
                                       CajaIcon *best_so_far,
                                       CajaIcon *candidate, void *data) {
...
static gboolean previous_row_rightmost(CajaIconContainer *container,
                                       CajaIcon *start_icon,
                                       CajaIcon *best_so_far,
                                       CajaIcon *candidate, void *data) {
...
static gboolean same_column_above_lowest(CajaIconContainer *container,
                                         CajaIcon *start_icon,
                                         CajaIcon *best_so_far,
                                         CajaIcon *candidate, void *data) {
  ...
  if (compare_icons_vertical_first(container, candidate, start_icon) >= 0) {
    return FALSE;
  }
...
static gboolean same_column_below_highest(CajaIconContainer *container,
                                          CajaIcon *start_icon,
                                          CajaIcon *best_so_far,
                                          CajaIcon *candidate, void *data) {
  /* Candidates not on the start column do not qualify. */
...
static gboolean previous_column_highest(CajaIconContainer *container,
                                        CajaIcon *start_icon,
                                        CajaIcon *best_so_far,
                                        CajaIcon *candidate, void *data) {
  ...
  if (best_so_far != NULL) {
    if (compare_icons_horizontal(container, best_so_far, candidate) < 0) {
      /* candidate is right of the best choice, but left of the current column
       */
      return TRUE;
    }

    if (compare_icons_vertical(container, best_so_far, candidate) > 0) {
      return TRUE;
    }
  }
...
static gboolean next_column_highest(CajaIconContainer *container,
                                    CajaIcon *start_icon, CajaIcon *best_so_far,
                                    CajaIcon *candidate, void *data) {
...
static gboolean previous_column_lowest(CajaIconContainer *container,
                                       CajaIcon *start_icon,
                                       CajaIcon *best_so_far,
                                       CajaIcon *candidate, void *data) {
  /* sort out icons that are not before the current column */
...
static gboolean last_column_lowest(CajaIconContainer *container,
                                   CajaIcon *start_icon, CajaIcon *best_so_far,
                                   CajaIcon *candidate, void *data) {
  if (best_so_far == NULL) {
    return TRUE;
...
static gboolean closest_in_90_degrees(CajaIconContainer *container,
                                      CajaIcon *start_icon,
                                      CajaIcon *best_so_far,
                                      CajaIcon *candidate, void *data) {
  EelDRect world_rect;
  ...
  eel_canvas_w2c(EEL_CANVAS(container), get_cmp_point_x(container, world_rect),
                 get_cmp_point_y(container, world_rect), &x, &y);
...
static EelDRect get_rubberband(CajaIcon *icon1, CajaIcon *icon2) {
  EelDRect rect1;
  EelDRect rect2;
  EelDRect ret;

  eel_canvas_item_get_bounds(EEL_CANVAS_ITEM(icon1->item), &rect1.x0, &rect1.y0,
                             &rect1.x1, &rect1.y1);
  eel_canvas_item_get_bounds(EEL_CANVAS_ITEM(icon2->item), &rect2.x0, &rect2.y0,
                             &rect2.x1, &rect2.y1);
...
static void keyboard_move_to(CajaIconContainer *container, CajaIcon *icon,
                             CajaIcon *from, GdkEventKey *event) {
  if (icon == NULL) {
    return;
  }
  ...
  if (event != NULL && (event->state & GDK_CONTROL_MASK) != 0 &&
    ...
    if (icon && container->details->keyboard_rubberband_start) {
      rect =
          get_rubberband(container->details->keyboard_rubberband_start, icon);
      rubberband_select(container, NULL, &rect);
    }
  } else if (event != NULL && (event->state & GDK_CONTROL_MASK) == 0 &&
             (event->state & GDK_SHIFT_MASK) != 0) {
    /* Select range */
    CajaIcon *start_icon;
...
static void keyboard_home(CajaIconContainer *container, GdkEventKey *event) {
  CajaIcon *from;
  CajaIcon *to;
  ...
  from =
      find_best_selected_icon(container, NULL, rightmost_in_bottom_row, NULL);
  to = find_best_icon(container, NULL, leftmost_in_top_row, NULL);

  keyboard_move_to(container, to, from, event);
}

static void keyboard_end(CajaIconContainer *container, GdkEventKey *event) {
  CajaIcon *to;
  CajaIcon *from;
   ...
   */
  from = find_best_selected_icon(container, NULL, leftmost_in_top_row, NULL);
...
static void record_arrow_key_start(CajaIconContainer *container, CajaIcon *icon,
...
static void keyboard_arrow_key(
    CajaIconContainer *container, GdkEventKey *event,
    GtkDirectionType direction, IsBetterIconFunction better_start,
    IsBetterIconFunction empty_start, IsBetterIconFunction better_destination,
  ...
  if (from == NULL) {
    if (has_multiple_selection(container)) {
      if (all_selected(container)) {
        from = find_best_selected_icon(container, NULL, empty_start, NULL);
      } else {
        from = find_best_selected_icon(container, NULL, better_start, NULL);
      }
    } else {
   ...
   */
  if (from == NULL) {
    to = from = find_best_icon(container, NULL, empty_start, NULL);
  } else {
    record_arrow_key_start(container, from, direction);

    to = find_best_icon(container, from,
                        container->details->auto_layout
                            ? better_destination
    ...
    if (to == NULL && better_destination_fallback != NULL) {
      to = find_best_icon(container, from, better_destination_fallback, &data);
    }
     ...
     * Should select 4.
     */
    if (to == NULL && container->details->auto_layout &&
        better_destination_fallback_fallback != NULL) {
      to = find_best_icon(container, from, better_destination_fallback_fallback,
                          &data);
    }
...
static gboolean is_rectangle_selection_event(GdkEventKey *event) {
  return (event->state & GDK_CONTROL_MASK) != 0 &&
         (event->state & GDK_SHIFT_MASK) != 0;
}

static void keyboard_right(CajaIconContainer *container, GdkEventKey *event) {
   ...
   */
  keyboard_arrow_key(container, event, GTK_DIR_RIGHT, rightmost_in_bottom_row,
                     caja_icon_container_is_layout_rtl(container)
                         ? rightmost_in_top_row
                         : leftmost_in_top_row,
                     same_row_right_side_leftmost, fallback,
                     next_column_fallback, closest_in_90_degrees);
}

static void keyboard_left(CajaIconContainer *container, GdkEventKey *event) {
  IsBetterIconFunction fallback;
   ...
   */
  keyboard_arrow_key(container, event, GTK_DIR_LEFT, rightmost_in_bottom_row,
                     caja_icon_container_is_layout_rtl(container)
                         ? rightmost_in_top_row
                         : leftmost_in_top_row,
                     same_row_left_side_rightmost, fallback,
                     previous_column_fallback, closest_in_90_degrees);
}

static void keyboard_down(CajaIconContainer *container, GdkEventKey *event) {
  IsBetterIconFunction fallback;
   ...
   */
  keyboard_arrow_key(container, event, GTK_DIR_DOWN, rightmost_in_bottom_row,
                     caja_icon_container_is_layout_rtl(container)
                         ? rightmost_in_top_row
                         : leftmost_in_top_row,
                     same_column_below_highest, fallback, next_row_fallback,
                     closest_in_90_degrees);
}

static void keyboard_up(CajaIconContainer *container, GdkEventKey *event) {
  IsBetterIconFunction fallback;
   ...
   */
  keyboard_arrow_key(
      container, event, GTK_DIR_UP, rightmost_in_bottom_row,
      caja_icon_container_is_layout_rtl(container) ? rightmost_in_top_row
                                                   : leftmost_in_top_row,
      same_column_above_lowest, fallback, NULL, closest_in_90_degrees);
}

static void keyboard_space(CajaIconContainer *container, GdkEventKey *event) {
  CajaIcon *icon;

  if (!has_selection(container) && container->details->keyboard_focus != NULL) {
    keyboard_move_to(container, container->details->keyboard_focus, NULL, NULL);
    ...
    if (container->details->keyboard_focus != NULL) {
      ...
      if (container->details->keyboard_focus->is_selected) {
        container->details->range_selection_base_icon =
            container->details->keyboard_focus;
      }
    } else {
      icon =
          find_best_selected_icon(container, NULL, leftmost_in_top_row, NULL);
      if (icon == NULL) {
        icon = find_best_icon(container, NULL, leftmost_in_top_row, NULL);
      }
      if (icon != NULL) {
        set_keyboard_focus(container, icon);
      }
...
static void select_previous_or_next_icon(CajaIconContainer *container,
                                         gboolean next, GdkEventKey *event) {
  CajaIcon *icon;
  const GList *item;
  ...
  if (icon != NULL) {
    ...
    if (item == NULL) {
      item = next ? g_list_first(container->details->icons)
                  : g_list_last(container->details->icons);
    }
...
static void finalize(GObject *object) {
  ...
  g_signal_handlers_disconnect_by_func(
      ...
      text_ellipsis_limit_changed_container_callback, object);
  g_signal_handlers_disconnect_by_func(
      caja_desktop_preferences, text_ellipsis_limit_changed_container_callback,
      object);
...
static gboolean clear_size_allocation_count(gpointer data) {
  CajaIconContainer *container;
...
static void size_allocate(GtkWidget *widget, GtkAllocation *allocation) {
   ...
   */
  if (container->details->size_allocation_count_id == 0) {
    container->details->size_allocation_count_id = g_idle_add_full(
        G_PRIORITY_HIGH, clear_size_allocation_count, container, NULL);
  }
  ...
  GTK_WIDGET_CLASS(caja_icon_container_parent_class)
...
static void get_prefered_width(GtkWidget *widget, gint *minimum_size,
                               gint *natural_size) {
  EelCanvasGroup *root;
  double x1, x2;
  int cx1, cx2;
  ...
  eel_canvas_item_get_bounds(EEL_CANVAS_ITEM(root), &x1, NULL, &x2, NULL);
...
static void get_prefered_height(GtkWidget *widget, gint *minimum_size,
                                gint *natural_size) {
  EelCanvasGroup *root;
  double y1, y2;
  int cy1, cy2;
  ...
  root = eel_canvas_root(EEL_CANVAS(widget));
  eel_canvas_item_get_bounds(EEL_CANVAS_ITEM(root), NULL, &y1, NULL, &y2);
...
static gboolean draw(GtkWidget *widget, cairo_t *cr) {
  ...
  return GTK_WIDGET_CLASS(caja_icon_container_parent_class)->draw(widget, cr);
}
...
static void style_updated(GtkWidget *widget) {
  ...
  container = CAJA_ICON_CONTAINER(widget);
  container->details->use_drop_shadows =
      container->details->drop_shadows_requested;
...
static gboolean button_press_event(GtkWidget *widget, GdkEventButton *event) {
  ...
  clicked_on_icon = GTK_WIDGET_CLASS(caja_icon_container_parent_class)
  ...
  if (!gtk_widget_has_focus(widget) &&
      !(is_renaming(container) || is_renaming_pending(container))) {
  ...
  if (event->button == DRAG_BUTTON && event->type == GDK_BUTTON_PRESS) {
    /* Clear the last click icon for double click */
    container->details->double_click_icon[1] =
        container->details->double_click_icon[0];
  ...
  g_signal_emit(widget, signals[BUTTON_PRESS], 0, event, &return_value);
  return return_value;
}

static void caja_icon_container_did_not_drag(CajaIconContainer *container,
                                             GdkEventButton *event) {
  CajaIconContainerDetails *details;
  gboolean selection_changed;
  ...
  if (details->icon_selected_on_button_down &&
    ...
    if (button_event_modifies_selection(event)) {
      ...
      selection_changed =
          select_one_unselect_others(container, details->drag_icon);
  ...
  if (details->drag_icon != NULL &&
      (details->single_click_mode || event->button == MIDDLE_BUTTON)) {
...
static gboolean clicked_within_double_click_interval(
    CajaIconContainer *container) {
  static gint64 last_click_time = 0;
  static gint click_count = 0;
  gint double_click_time;
...
static void clear_drag_state(CajaIconContainer *container) {
  container->details->drag_icon = NULL;
  container->details->drag_state = DRAG_STATE_INITIAL;
}

static gboolean start_stretching(CajaIconContainer *container,
                                 GdkEvent *event) {
  CajaIconContainerDetails *details;
  CajaIcon *icon;
  ...
  if (!caja_icon_canvas_item_hit_test_stretch_handles(icon->item, world_point,
                                                      &corner)) {
    return FALSE;
  }
  ...
  eel_canvas_w2c(EEL_CANVAS(container), details->drag_x, details->drag_y,
                 &details->stretch_start.pointer_x,
                 &details->stretch_start.pointer_y);
  eel_canvas_w2c(EEL_CANVAS(container), icon->x, icon->y,
                 &details->stretch_start.icon_x,
                 &details->stretch_start.icon_y);
  icon_get_size(container, icon, &details->stretch_start.icon_size);
...
static gboolean update_stretch_at_idle(CajaIconContainer *container) {
  CajaIconContainerDetails *details;
  CajaIcon *icon;
  double world_x, world_y;
  ...
  eel_canvas_w2c(EEL_CANVAS(container), details->world_x, details->world_y,
                 &stretch_state.pointer_x, &stretch_state.pointer_y);

  compute_stretch(&details->stretch_start, &stretch_state);

  eel_canvas_c2w(EEL_CANVAS(container), stretch_state.icon_x,
                 stretch_state.icon_y, &world_x, &world_y);
...
static void continue_stretching(CajaIconContainer *container, double world_x,
                                double world_y) {
  ...
  if (container->details->stretch_idle_id == 0) {
    container->details->stretch_idle_id =
        g_idle_add((GSourceFunc)update_stretch_at_idle, container);
  }
}

static gboolean keyboard_stretching(CajaIconContainer *container,
                                    GdkEventKey *event) {
  CajaIcon *icon;
  guint size;
  ...
  switch (event->keyval) {
    ...
    case GDK_KEY_0:
    case GDK_KEY_KP_0:
      caja_icon_container_move_icon(container, icon, icon->x, icon->y, 1.0,
                                    FALSE, TRUE, TRUE);
      break;
  }
...
static void ungrab_stretch_icon(CajaIconContainer *container) {
  eel_canvas_item_ungrab(
      EEL_CANVAS_ITEM(container->details->stretch_icon->item));
}

static void end_stretching(CajaIconContainer *container, double world_x,
                           double world_y) {
  CajaIconPosition position;
  CajaIcon *icon;
...
static gboolean undo_stretching(CajaIconContainer *container) {
  ...
  caja_icon_canvas_item_set_show_stretch_handles(stretched_icon->item, FALSE);

  icon_set_position(stretched_icon, container->details->stretch_initial_x,
                    container->details->stretch_initial_y);
  icon_set_size(container, stretched_icon,
                container->details->stretch_initial_size, TRUE, TRUE);
...
static gboolean button_release_event(GtkWidget *widget, GdkEventButton *event) {
  ...
  return GTK_WIDGET_CLASS(caja_icon_container_parent_class)
      ->button_release_event(widget, event);
}

static int motion_notify_event(GtkWidget *widget, GdkEventMotion *event) {
  CajaIconContainer *container;
  CajaIconContainerDetails *details;
  double world_x, world_y;
  ...
  if (details->drag_button != 0) {
    switch (details->drag_state) {
      case DRAG_STATE_MOVE_OR_COPY:
        ...
        if (gtk_drag_check_threshold(widget, details->drag_x, details->drag_y,
                                     world_x, world_y)) {
          ...
          eel_canvas_w2c(EEL_CANVAS(container), details->drag_x,
                         details->drag_y, &canvas_x, &canvas_y);

          actions = GDK_ACTION_COPY | GDK_ACTION_LINK | GDK_ACTION_ASK;

          if (container->details->drag_allow_moves) {
            actions |= GDK_ACTION_MOVE;
          }

          caja_icon_dnd_begin_drag(container, actions, details->drag_button,
                                   event, canvas_x, canvas_y);
          caja_debug_log(FALSE, CAJA_DEBUG_LOG_DOMAIN_USER,
...
static void caja_icon_container_search_position_func(
    CajaIconContainer *container, GtkWidget *search_dialog) {
  gint x, y;
  gint cont_x, cont_y;
  ...
  monitor_num = gdk_display_get_monitor_at_window(
      gdk_screen_get_display(screen), cont_window);
  ...
  if (cont_x + cont_width - requisition.width >
      WidthOfScreen(gdk_x11_screen_get_xscreen(screen)) / scale) {
    x = WidthOfScreen(gdk_x11_screen_get_xscreen(screen)) / scale -
        requisition.width;
...
static gboolean caja_icon_container_real_search_enable_popdown(gpointer data) {
...
static void caja_icon_container_search_enable_popdown(GtkWidget *widget,
                                                      gpointer data) {
  CajaIconContainer *container = (CajaIconContainer *)data;
...
static void caja_icon_container_search_disable_popdown(GtkEntry *entry,
                                                       GtkMenu *menu,
                                                       gpointer data) {
  CajaIconContainer *container = (CajaIconContainer *)data;
  ...
  g_signal_connect(menu, "hide",
                   G_CALLBACK(caja_icon_container_search_enable_popdown), data);
}
...
static void caja_icon_container_search_dialog_hide(
    GtkWidget *search_dialog, CajaIconContainer *container) {
...
static gboolean caja_icon_container_search_entry_flush_timeout(
    CajaIconContainer *container) {
 ...
 * callback.
 */
static void caja_icon_container_search_preedit_changed(
    GtkEntry *entry, gchar *preedit, CajaIconContainer *container) {
  container->details->imcontext_changed = 1;
  if (container->details->typeselect_flush_timeout) {
    g_source_remove(container->details->typeselect_flush_timeout);
    container->details->typeselect_flush_timeout = g_timeout_add_seconds(
        CAJA_ICON_CONTAINER_SEARCH_DIALOG_TIMEOUT,
        (GSourceFunc)caja_icon_container_search_entry_flush_timeout, container);
  }
}

static void caja_icon_container_search_activate(GtkEntry *entry,
                                                CajaIconContainer *container) {
  caja_icon_container_search_dialog_hide(container->details->search_window,
                                         container);
...
static gboolean caja_icon_container_search_delete_event(
    GtkWidget *widget, GdkEventAny *event, CajaIconContainer *container) {
...
static gboolean caja_icon_container_search_button_press_event(
    GtkWidget *widget, GdkEventButton *event, CajaIconContainer *container) {
  g_assert(GTK_IS_WIDGET(widget));
...
static void caja_icon_container_get_icon_text(CajaIconContainer *container,
                                              CajaIconData *data,
                                              char **editable_text,
...
static gboolean caja_icon_container_search_iter(CajaIconContainer *container,
  ...
  for (p = container->details->icons; p != NULL && count != n; p = p->next) {
    icon = p->data;
    caja_icon_container_get_icon_text(container, icon->data, &name, NULL, TRUE);
...
static void caja_icon_container_search_move(GtkWidget *window,
                                            CajaIconContainer *container,
                                            gboolean up) {
  gboolean ret;
  gint len;
  ...
  ret = caja_icon_container_search_iter(
      container, text,
      up ? ((container->details->selected_iter) - 1)
...
static gboolean caja_icon_container_search_scroll_event(
    GtkWidget *widget, GdkEventScroll *event, CajaIconContainer *container) {
...
static gboolean caja_icon_container_search_key_press_event(
    GtkWidget *widget, GdkEventKey *event, CajaIconContainer *container) {
  gboolean retval = FALSE;
...
static void caja_icon_container_search_init(GtkWidget *entry,
                                            CajaIconContainer *container) {
  gint ret;
  gint len;
  const gchar *text;
...
static void caja_icon_container_ensure_interactive_directory(
    CajaIconContainer *container) {
  ...
  g_signal_connect(container->details->search_entry, "activate",
                   G_CALLBACK(caja_icon_container_search_activate), container);
 ...
 * started this by typing the start_interactive_search keybinding.  Otherwise,
 * it came from
 */
static gboolean caja_icon_container_real_start_interactive_search(
    CajaIconContainer *container, gboolean keybinding) {
  ...
  /* done, show it */
  caja_icon_container_search_position_func(container,
                                           container->details->search_window);
  gtk_widget_show(container->details->search_window);
  if (container->details->search_entry_changed_id == 0) {
    container->details->search_entry_changed_id = g_signal_connect(
        container->details->search_entry, "changed",
        G_CALLBACK(caja_icon_container_search_init), container);
  }
   ...
   * really non-kosher. */
  entry_parent_class = g_type_class_peek_parent(
      GTK_ENTRY_GET_CLASS(container->details->search_entry));
...
static gboolean caja_icon_container_start_interactive_search(
    CajaIconContainer *container) {
  return caja_icon_container_real_start_interactive_search(container, TRUE);
}

static gboolean handle_popups(CajaIconContainer *container, GdkEventKey *event,
                              const char *signal) {
...
static int key_press_event(GtkWidget *widget, GdkEventKey *event) {
  ...
  if (is_renaming(container) || is_renaming_pending(container)) {
    ...
    switch (event->keyval) {
      ...
      case GDK_KEY_ISO_Left_Tab:
        select_previous_or_next_icon(
            container, (event->state & GDK_SHIFT_MASK) == 0, event);
        handled = TRUE;
        break;
#endif
      case GDK_KEY_Return:
      ...
      case GDK_KEY_F10:
         ...
         */
        if (event->state & GDK_CONTROL_MASK) {
          handled = handle_popups(container, event, "context_click_background");
        }
        break;
      case GDK_KEY_v:
  ...
  if (!handled) {
    handled = GTK_WIDGET_CLASS(caja_icon_container_parent_class)
  ...
  if (!handled &&
      event->keyval !=
          GDK_KEY_slash /* don't steal slash key event, used for "go to" */
    ...
    old_text = g_strdup(
        gtk_entry_get_text(GTK_ENTRY(container->details->search_entry)));
    new_event = gdk_event_copy((GdkEvent *)event);
    window = ((GdkEventKey *)new_event)->window;
    ((GdkEventKey *)new_event)->window =
        gtk_widget_get_window(container->details->search_entry);
    ...
    scale = gtk_widget_get_scale_factor(GTK_WIDGET(container));
    gtk_window_move(
        GTK_WINDOW(container->details->search_window),
...
static gboolean popup_menu(GtkWidget *widget) {
  ...
  if (has_selection(container)) {
    handle_popups(container, NULL, "context_click_selection");
  } else {
...
static void draw_canvas_background(EelCanvas *canvas, cairo_t *cr) {
  /* Don't chain up to the parent to avoid clearing and redrawing */
}

static void grab_notify_cb(GtkWidget *widget, gboolean was_grabbed) {
  CajaIconContainer *container;

  container = CAJA_ICON_CONTAINER(widget);

  if (container->details->rubberband_info.active && !was_grabbed) {
...
static GObject *caja_icon_container_constructor(
    GType type, guint n_construct_params,
    GObjectConstructParam *construct_params) {
  CajaIconContainer *container;
  GObject *object;

  object = G_OBJECT_CLASS(caja_icon_container_parent_class)
  ...
  if (caja_icon_container_get_is_desktop(container)) {
    g_signal_connect_swapped(
        ...
        G_CALLBACK(text_ellipsis_limit_changed_container_callback), container);
  } else {
    g_signal_connect_swapped(
        caja_icon_view_preferences,
...
static void caja_icon_container_class_init(CajaIconContainerClass *class) {
  ...
  signals[SELECTION_CHANGED] = g_signal_new(
      ...
      G_STRUCT_OFFSET(CajaIconContainerClass, selection_changed), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
  signals[BUTTON_PRESS] = g_signal_new(
      "button_press", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaIconContainerClass, button_press), NULL, NULL,
      caja_marshal_BOOLEAN__POINTER, G_TYPE_BOOLEAN, 1, GDK_TYPE_EVENT);
  signals[ACTIVATE] = g_signal_new(
      "activate", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaIconContainerClass, activate), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);
  signals[ACTIVATE_ALTERNATE] = g_signal_new(
      "activate_alternate", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaIconContainerClass, activate_alternate), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);
  signals[CONTEXT_CLICK_SELECTION] = g_signal_new(
      "context_click_selection", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaIconContainerClass, context_click_selection), NULL,
      NULL, g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);
  signals[CONTEXT_CLICK_BACKGROUND] = g_signal_new(
      "context_click_background", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaIconContainerClass, context_click_background), NULL,
      NULL, g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);
  signals[MIDDLE_CLICK] = g_signal_new(
      "middle_click", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaIconContainerClass, middle_click), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);
  signals[ICON_POSITION_CHANGED] = g_signal_new(
      "icon_position_changed", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaIconContainerClass, icon_position_changed), NULL,
      NULL, caja_marshal_VOID__POINTER_POINTER, G_TYPE_NONE, 2, G_TYPE_POINTER,
      G_TYPE_POINTER);
  signals[ICON_TEXT_CHANGED] = g_signal_new(
      "icon_text_changed", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaIconContainerClass, icon_text_changed), NULL, NULL,
      caja_marshal_VOID__POINTER_STRING, G_TYPE_NONE, 2, G_TYPE_POINTER,
      G_TYPE_STRING);
  signals[ICON_STRETCH_STARTED] = g_signal_new(
      "icon_stretch_started", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaIconContainerClass, icon_stretch_started), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);
  signals[ICON_STRETCH_ENDED] = g_signal_new(
      "icon_stretch_ended", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaIconContainerClass, icon_stretch_ended), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);
  signals[RENAMING_ICON] = g_signal_new(
      "renaming_icon", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaIconContainerClass, renaming_icon), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);
  signals[GET_ICON_URI] = g_signal_new(
      "get_icon_uri", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaIconContainerClass, get_icon_uri), NULL, NULL,
      caja_marshal_STRING__POINTER, G_TYPE_STRING, 1, G_TYPE_POINTER);
  signals[GET_ICON_DROP_TARGET_URI] = g_signal_new(
      "get_icon_drop_target_uri", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaIconContainerClass, get_icon_drop_target_uri), NULL,
      NULL, caja_marshal_STRING__POINTER, G_TYPE_STRING, 1, G_TYPE_POINTER);
  signals[MOVE_COPY_ITEMS] = g_signal_new(
      "move_copy_items", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaIconContainerClass, move_copy_items), NULL, NULL,
      caja_marshal_VOID__POINTER_POINTER_POINTER_ENUM_INT_INT, G_TYPE_NONE, 6,
      G_TYPE_POINTER, G_TYPE_POINTER, G_TYPE_POINTER, GDK_TYPE_DRAG_ACTION,
      G_TYPE_INT, G_TYPE_INT);
  signals[HANDLE_NETSCAPE_URL] = g_signal_new(
      "handle_netscape_url", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaIconContainerClass, handle_netscape_url), NULL, NULL,
      caja_marshal_VOID__STRING_STRING_ENUM_INT_INT, G_TYPE_NONE, 5,
      G_TYPE_STRING, G_TYPE_STRING, GDK_TYPE_DRAG_ACTION, G_TYPE_INT,
      G_TYPE_INT);
  signals[HANDLE_URI_LIST] = g_signal_new(
      "handle_uri_list", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaIconContainerClass, handle_uri_list), NULL, NULL,
      caja_marshal_VOID__STRING_STRING_ENUM_INT_INT, G_TYPE_NONE, 5,
      G_TYPE_STRING, G_TYPE_STRING, GDK_TYPE_DRAG_ACTION, G_TYPE_INT,
      G_TYPE_INT);
  signals[HANDLE_TEXT] =
      g_signal_new("handle_text", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(CajaIconContainerClass, handle_text), NULL,
                   NULL, caja_marshal_VOID__STRING_STRING_ENUM_INT_INT,
                   G_TYPE_NONE, 5, G_TYPE_STRING, G_TYPE_STRING,
                   GDK_TYPE_DRAG_ACTION, G_TYPE_INT, G_TYPE_INT);
  signals[HANDLE_RAW] = g_signal_new(
      "handle_raw", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaIconContainerClass, handle_raw), NULL, NULL,
      caja_marshal_VOID__POINTER_INT_STRING_STRING_ENUM_INT_INT, G_TYPE_NONE, 7,
      G_TYPE_POINTER, G_TYPE_INT, G_TYPE_STRING, G_TYPE_STRING,
      GDK_TYPE_DRAG_ACTION, G_TYPE_INT, G_TYPE_INT);
  signals[GET_CONTAINER_URI] = g_signal_new(
      "get_container_uri", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaIconContainerClass, get_container_uri), NULL, NULL,
      caja_marshal_STRING__VOID, G_TYPE_STRING, 0);
  signals[CAN_ACCEPT_ITEM] = g_signal_new(
      "can_accept_item", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaIconContainerClass, can_accept_item), NULL, NULL,
      caja_marshal_INT__POINTER_STRING, G_TYPE_INT, 2, G_TYPE_POINTER,
      G_TYPE_STRING);
  signals[GET_STORED_ICON_POSITION] = g_signal_new(
      "get_stored_icon_position", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaIconContainerClass, get_stored_icon_position), NULL,
      NULL, caja_marshal_BOOLEAN__POINTER_POINTER, G_TYPE_BOOLEAN, 2,
      G_TYPE_POINTER, G_TYPE_POINTER);
  signals[GET_STORED_LAYOUT_TIMESTAMP] = g_signal_new(
      "get_stored_layout_timestamp", G_TYPE_FROM_CLASS(class),
      G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaIconContainerClass, get_stored_layout_timestamp),
      NULL, NULL, caja_marshal_BOOLEAN__POINTER_POINTER, G_TYPE_BOOLEAN, 2,
      G_TYPE_POINTER, G_TYPE_POINTER);
  signals[STORE_LAYOUT_TIMESTAMP] = g_signal_new(
      "store_layout_timestamp", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaIconContainerClass, store_layout_timestamp), NULL,
      NULL, caja_marshal_BOOLEAN__POINTER_POINTER, G_TYPE_BOOLEAN, 2,
      G_TYPE_POINTER, G_TYPE_POINTER);
  signals[LAYOUT_CHANGED] = g_signal_new(
      "layout_changed", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaIconContainerClass, layout_changed), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
  signals[PREVIEW] =
      g_signal_new("preview", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(CajaIconContainerClass, preview), NULL, NULL,
                   caja_marshal_INT__POINTER_BOOLEAN, G_TYPE_INT, 2,
                   G_TYPE_POINTER, G_TYPE_BOOLEAN);
  signals[BAND_SELECT_STARTED] = g_signal_new(
      "band_select_started", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaIconContainerClass, band_select_started), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
  signals[BAND_SELECT_ENDED] = g_signal_new(
      "band_select_ended", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaIconContainerClass, band_select_ended), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
  signals[ICON_ADDED] = g_signal_new(
      "icon_added", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaIconContainerClass, icon_added), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);
  signals[ICON_REMOVED] = g_signal_new(
      "icon_removed", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaIconContainerClass, icon_removed), NULL, NULL,
      g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);

  signals[CLEARED] =
      g_signal_new("cleared", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(CajaIconContainerClass, cleared), NULL, NULL,
                   g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  signals[START_INTERACTIVE_SEARCH] = g_signal_new(
      "start_interactive_search", G_TYPE_FROM_CLASS(class),
      G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
      G_STRUCT_OFFSET(CajaIconContainerClass, start_interactive_search), NULL,
      NULL, caja_marshal_BOOLEAN__VOID, G_TYPE_BOOLEAN, 0);
  ...
  gtk_widget_class_set_accessible_type(
      widget_class, caja_icon_container_accessible_get_type());

  canvas_class = EEL_CANVAS_CLASS(class);
  canvas_class->draw_background = draw_canvas_background;
  class->start_interactive_search =
      caja_icon_container_start_interactive_search;

  gtk_widget_class_install_style_property(
      widget_class,
      g_param_spec_boxed("selection_box_rgba", "Selection Box RGBA",
                         "Color of the selection box", GDK_TYPE_RGBA,
                         G_PARAM_READABLE));
  gtk_widget_class_install_style_property(
      widget_class,
      g_param_spec_boxed(
  ...
  gtk_widget_class_install_style_property(
      ...
      g_param_spec_boolean("activate_prelight_icon_label",
                           ...
                           "Whether icon labels should make use of its "
                           "prelight color in prelight state",
...
static void update_selected(CajaIconContainer *container) {
  GList *node;
  CajaIcon *icon = NULL;
...
static gboolean handle_focus_in_event(GtkWidget *widget, GdkEventFocus *event,
                                      gpointer user_data) {
...
static gboolean handle_focus_out_event(GtkWidget *widget, GdkEventFocus *event,
                                       gpointer user_data) {
...
static void handle_scale_factor_changed(GObject *object, GParamSpec *pspec,
                                        gpointer user_data) {
...
static gboolean get_text_ellipsis_limit_for_zoom(char **strs,
...
static void text_ellipsis_limit_changed_callback(gpointer callback_data) {
  char **pref;
  unsigned int i;
  int one_limit;

  pref = g_settings_get_strv(caja_icon_view_preferences,
  ...
  for (i = 0; i < G_N_ELEMENTS(zoom_level_names); i++) {
    if (get_text_ellipsis_limit_for_zoom(pref, zoom_level_names[i],
                                         &one_limit)) {
      text_ellipsis_limits[i] = one_limit;
    }
  }
...
static void desktop_text_ellipsis_limit_changed_callback(
    gpointer callback_data) {
  int pref;

  pref = g_settings_get_int(caja_desktop_preferences,
                            CAJA_PREFERENCES_DESKTOP_TEXT_ELLIPSIS_LIMIT);
  desktop_text_ellipsis_limit = pref;
}

static void caja_icon_container_init(CajaIconContainer *container) {
  CajaIconContainerDetails *details;
  ...
  if (!setup_prefs) {
    g_signal_connect_swapped(
        caja_icon_view_preferences,
        "changed::" CAJA_PREFERENCES_ICON_VIEW_TEXT_ELLIPSIS_LIMIT,
        G_CALLBACK(text_ellipsis_limit_changed_callback), NULL);
    text_ellipsis_limit_changed_callback(NULL);

    g_signal_connect_swapped(
        caja_icon_view_preferences,
        "changed::" CAJA_PREFERENCES_DESKTOP_TEXT_ELLIPSIS_LIMIT,
        G_CALLBACK(desktop_text_ellipsis_limit_changed_callback), NULL);
...
static gboolean handle_icon_double_click(CajaIconContainer *container,
                                         CajaIcon *icon,
                                         GdkEventButton *event) {
  CajaIconContainerDetails *details;
...
static gboolean handle_icon_button_press(CajaIconContainer *container,
                                         CajaIcon *icon,
                                         GdkEventButton *event) {
  CajaIconContainerDetails *details;
  ...
  if (event->button == DRAG_BUTTON || event->button == DRAG_MENU_BUTTON) {
...
static int item_event_callback(EelCanvasItem *item, GdkEvent *event,
                               gpointer data) {
  CajaIconContainer *container;
  CajaIcon *icon;
...
void caja_icon_container_clear(CajaIconContainer *container) {
  CajaIconContainerDetails *details;
  GList *p;
  CajaIcon *icon = NULL;
  ...
  for (p = details->icons; p != NULL; p = p->next) {
    ...
    if (icon->is_monitored) {
      caja_icon_container_stop_monitor_top_left(container, icon->data, icon);
    }
    icon_free(p->data);
  }
...
CajaIconData *caja_icon_container_get_first_visible_icon(
    CajaIconContainer *container) {
  GList *l;
  ...
  hadj_v = gtk_adjustment_get_value(
      gtk_scrollable_get_hadjustment(GTK_SCROLLABLE(container)));
  vadj_v = gtk_adjustment_get_value(
      gtk_scrollable_get_vadjustment(GTK_SCROLLABLE(container)));
  h_page_size = gtk_adjustment_get_page_size(
      gtk_scrollable_get_hadjustment(GTK_SCROLLABLE(container)));
  ...
  eel_canvas_c2w(EEL_CANVAS(container), x, y, &x, &y);
  ...
  while (l != NULL) {
    ...
    if (icon_is_positioned(icon)) {
      eel_canvas_item_get_bounds(EEL_CANVAS_ITEM(icon->item), &x1, &y1, &x2,
                                 &y2);
...
void caja_icon_container_scroll_to_icon(CajaIconContainer *container,
  ...
  while (l != NULL) {
    ...
    if (icon->data == data && icon_is_positioned(icon)) {
      if (caja_icon_container_is_auto_layout(container)) {
        /* ensure that we reveal the entire row/column */
        icon_get_row_and_column_bounds(container, icon, &bounds, TRUE);
      } else {
...
static void call_icon_callback(gpointer data, gpointer callback_data) {
  CajaIcon *icon;
...
void caja_icon_container_for_each(CajaIconContainer *container,
                                  CajaIconCallback callback,
                                  gpointer callback_data) {
  CallbackAndData callback_and_data;
  ...
  g_list_foreach(container->details->icons, call_icon_callback,
                 &callback_and_data);
}

static int selection_changed_at_idle_callback(gpointer data) {
  CajaIconContainer *container;
...
static void icon_destroy(CajaIconContainer *container, CajaIcon *icon) {
  ...
  if (details->keyboard_focus == icon || details->keyboard_focus == NULL) {
  ...
  if (was_selected) {
    ...
    details->selection_changed_id =
        g_idle_add(selection_changed_at_idle_callback, container);
  }
}
...
static void activate_selected_items(CajaIconContainer *container) {
  ...
  if (selection != NULL) {
    g_signal_emit(container, signals[ACTIVATE], 0, selection);
  }
...
static void activate_selected_items_alternate(CajaIconContainer *container,
  ...
  if (selection != NULL) {
    g_signal_emit(container, signals[ACTIVATE_ALTERNATE], 0, selection);
  }
  g_list_free(selection);
}
...
static CajaIconInfo *caja_icon_container_get_icon_images(
    CajaIconContainer *container, CajaIconData *data, int size,
    GList **emblem_pixbufs, char **embedded_text, gboolean for_drag_accept,
    gboolean need_large_embeddded_text, gboolean *embedded_text_needs_loading,
  ...
  return klass->get_icon_images(
      container, data, size, emblem_pixbufs, embedded_text, for_drag_accept,
      need_large_embeddded_text, embedded_text_needs_loading, has_open_window);
}

static void caja_icon_container_freeze_updates(CajaIconContainer *container) {
  CajaIconContainerClass *klass;
...
static void caja_icon_container_unfreeze_updates(CajaIconContainer *container) {
  CajaIconContainerClass *klass;
...
static void caja_icon_container_prioritize_thumbnailing(
    CajaIconContainer *container, CajaIcon *icon) {
  CajaIconContainerClass *klass;
...
static void caja_icon_container_update_visible_icons(
  ...
  eel_canvas_c2w(EEL_CANVAS(container), min_x, min_y, &min_x, &min_y);
  eel_canvas_c2w(EEL_CANVAS(container), max_x, max_y, &max_x, &max_y);

  /* Do the iteration in reverse to get the render-order from top to
   * bottom for the prioritized thumbnails.
   */
  for (node = g_list_last(container->details->icons); node != NULL;
       node = node->prev) {
    icon = node->data;

    if (icon_is_positioned(icon)) {
      eel_canvas_item_get_bounds(EEL_CANVAS_ITEM(icon->item), &x0, &y0, &x1,
                                 &y1);
      ...
      if (visible) {
        ...
        caja_icon_container_prioritize_thumbnailing(container, icon);
      } else {
...
void caja_icon_container_update_icon(CajaIconContainer *container,
  ...
  max_image_size =
      MAX(MAXIMUM_IMAGE_SIZE * EEL_CANVAS(container)->pixels_per_unit,
          CAJA_ICON_MAXIMUM_SIZE);
  ...
  icon_info = caja_icon_container_get_icon_images(
      container, icon->data, icon_size, &emblem_pixbufs, &embedded_text,
      icon == details->drop_target, large_embedded_text,
  ...
  has_embedded_text_rect =
      caja_icon_info_get_embedded_rect(icon_info, &embedded_text_rect);

  if (has_embedded_text_rect && embedded_text_needs_loading) {
    icon->is_monitored = TRUE;
    caja_icon_container_start_monitor_top_left(container, icon->data, icon,
                                               large_embedded_text);
  }

  caja_icon_container_get_icon_text(container, icon->data, &editable_text,
                                    &additional_text, FALSE);

  /* If name of icon being renamed was changed from elsewhere, end renaming
   * mode. Alternatively, we could replace the characters in the editable text
   * widget with the new name, but that could cause timing problems if the user
   * just happened to be typing at that moment.
   */
  if (icon == get_icon_being_renamed(container) &&
  ...
  eel_canvas_item_set(EEL_CANVAS_ITEM(icon->item), "editable_text",
                      editable_text, "additional_text", additional_text,
                      "highlighted_for_drop", icon == details->drop_target,
                      NULL);

  caja_icon_canvas_item_set_image(icon->item, pixbuf);
  caja_icon_canvas_item_set_attach_points(icon->item, attach_points,
                                          n_attach_points);
...
static gboolean assign_icon_position(CajaIconContainer *container,
                                     CajaIcon *icon) {
  gboolean have_stored_position;
  CajaIconPosition position;
...
static void finish_adding_icon(CajaIconContainer *container, CajaIcon *icon) {
  caja_icon_container_update_icon(container, icon);
  eel_canvas_item_show(EEL_CANVAS_ITEM(icon->item));

  g_signal_connect_object(icon->item, "event", G_CALLBACK(item_event_callback),
                          container, 0);
...
static void finish_adding_new_icons(CajaIconContainer *container) {
  ...
  if (semi_position_icons != NULL) {
    ...
    for (p = semi_position_icons; p != NULL; p = p->next) {
      ...
      g_signal_emit(container, signals[ICON_POSITION_CHANGED], 0, icon->data,
                    &position);
      g_signal_emit(container, signals[STORE_LAYOUT_TIMESTAMP], 0, icon->data,
                    &now, &dummy);
  ...
  if (container->details->store_layout_timestamps_when_finishing_new_icons) {
    ...
    container->details->store_layout_timestamps_when_finishing_new_icons =
        FALSE;
  }
}

static gboolean is_old_or_unknown_icon_data(CajaIconContainer *container,
                                            CajaIconData *data) {
  time_t timestamp;
  gboolean success;
  ...
  g_signal_emit(container, signals[GET_STORED_LAYOUT_TIMESTAMP], 0, data,
                &timestamp, &success);
 ...
 **/
gboolean caja_icon_container_add(CajaIconContainer *container,
                                 CajaIconData *data) {
  CajaIconContainerDetails *details;
  CajaIcon *icon;
  ...
  icon->has_lazy_position =
      (is_old_or_unknown_icon_data(container, data) != FALSE);
  icon->scale = 1.0;
  icon->item = CAJA_ICON_CANVAS_ITEM(eel_canvas_item_new(
      EEL_CANVAS_GROUP(EEL_CANVAS(container)->root),
      caja_icon_canvas_item_get_type(), "visible", FALSE, NULL));
...
void caja_icon_container_layout_now(CajaIconContainer *container) {
  ...
  gtk_container_check_resize(
      GTK_CONTAINER(gtk_widget_get_parent(GTK_WIDGET(container))));
}
 ...
 **/
gboolean caja_icon_container_remove(CajaIconContainer *container,
                                    CajaIconData *data) {
  CajaIcon *icon;
 ...
 **/
void caja_icon_container_request_update(CajaIconContainer *container,
                                        CajaIconData *data) {
  CajaIcon *icon;
...
CajaZoomLevel caja_icon_container_get_zoom_level(CajaIconContainer *container) {
  return container->details->zoom_level;
}

void caja_icon_container_set_zoom_level(CajaIconContainer *container,
                                        int new_level) {
  CajaIconContainerDetails *details;
  int pinned_level;
  double pixels_per_unit;
 ...
 **/
GList *caja_icon_container_get_selection(CajaIconContainer *container) {
  GList *list, *p;
 ...
 *
 **/
void caja_icon_container_invert_selection(CajaIconContainer *container) {
  GList *p;
...
static GArray *caja_icon_container_get_icon_locations(
    CajaIconContainer *container, GList *icons) {
  GArray *result;
  GList *node;
  int index;
  ...
  for (index = 0, node = icons; node != NULL; index++, node = node->next) {
    g_array_index(result, GdkPoint, index).x = ((CajaIcon *)node->data)->x;
    g_array_index(result, GdkPoint, index).y = ((CajaIcon *)node->data)->y;
  }
 ...
 **/
GArray *caja_icon_container_get_selected_icon_locations(
    CajaIconContainer *container) {
  GArray *result;
  GList *icons;
 ...
 **/
void caja_icon_container_select_all(CajaIconContainer *container) {
  gboolean selection_changed;
  GList *p;
  CajaIcon *icon = NULL;
 ...
 **/
void caja_icon_container_set_selection(CajaIconContainer *container,
                                       GList *selection) {
  gboolean selection_changed;
  GHashTable *hash;
  GList *p;
  ...
  for (p = container->details->icons; p != NULL; p = p->next) {
    ...
    selection_changed |= icon_set_selected(
        container, icon, g_hash_table_lookup(hash, icon->data) != NULL);
  }
 ...
 **/
void caja_icon_container_select_list_unselect_others(
    CajaIconContainer *container, GList *selection) {
  gboolean selection_changed;
  GHashTable *hash;
  GList *p;
  CajaIcon *icon = NULL;
  ...
  for (p = container->details->icons; p != NULL; p = p->next) {
    ...
    selection_changed |= icon_set_selected(
        container, icon, g_hash_table_lookup(hash, icon) != NULL);
  }
  g_hash_table_destroy(hash);
 ...
 **/
void caja_icon_container_unselect_all(CajaIconContainer *container) {
...
CajaIcon *caja_icon_container_get_icon_by_uri(CajaIconContainer *container,
  ...
  */
  ...
  for (p = details->icons; p != NULL; p = p->next) {
    ...
    icon_uri = caja_icon_container_get_icon_uri(container, icon);
...
void caja_icon_container_show_stretch_handles(CajaIconContainer *container) {
  ...
  if (details->stretch_icon != NULL) {
    caja_icon_canvas_item_set_show_stretch_handles(details->stretch_icon->item,
                                                   FALSE);
    ungrab_stretch_icon(container);
  ...
  /* only need to keep size in one dimension, since they are constrained to be
   * the same */
...
char *caja_icon_container_get_icon_uri(CajaIconContainer *container,
  ...
  uri = NULL;
  g_signal_emit(container, signals[GET_ICON_URI], 0, icon->data, &uri);
  return uri;
}
...
char *caja_icon_container_get_icon_drop_target_uri(CajaIconContainer *container,
  ...
  uri = NULL;
  g_signal_emit(container, signals[GET_ICON_DROP_TARGET_URI], 0, icon->data,
                &uri);
  return uri;
}
 ...
 */
static void reset_scroll_region_if_not_empty(CajaIconContainer *container) {
  if (!caja_icon_container_is_empty(container)) {
...
gboolean caja_icon_container_is_lock_icons_position(
    CajaIconContainer *container) {
  return container->details->lock_icons_position;
}

static gboolean align_icons_callback(gpointer callback_data) {
  CajaIconContainer *container;
...
static void unschedule_align_icons(CajaIconContainer *container) {
  if (container->details->align_idle_id != 0) {
    g_source_remove(container->details->align_idle_id);
    container->details->align_idle_id = 0;
  }
...
static void schedule_align_icons(CajaIconContainer *container) {
  if (container->details->align_idle_id == 0 &&
      container->details->has_been_allocated) {
    container->details->align_idle_id =
        g_idle_add(align_icons_callback, container);
  }
}
...
static void pending_icon_to_rename_destroy_callback(
    CajaIconCanvasItem *item, CajaIconContainer *container) {
...
static CajaIcon *get_pending_icon_to_rename(CajaIconContainer *container) {
  return container->details->pending_icon_to_rename;
}

static void set_pending_icon_to_rename(CajaIconContainer *container,
                                       CajaIcon *icon) {
  CajaIcon *old_icon;
  ...
  if (old_icon != NULL) {
    g_signal_handlers_disconnect_by_func(
        old_icon->item, G_CALLBACK(pending_icon_to_rename_destroy_callback),
        container);
  }
...
static void process_pending_icon_to_rename(CajaIconContainer *container) {
  ...
  if (pending_icon_to_rename != NULL) {
    if (pending_icon_to_rename->is_selected &&
 ...
 **/
void caja_icon_container_start_renaming_selected_item(
    CajaIconContainer *container, gboolean select_all) {
  CajaIconContainerDetails *details;
  CajaIcon *icon;
  EelDRect icon_rect;
  EelDRect text_rect;
  ...
  /* Freeze updates so files added while renaming don't cause rename to loose
   * focus, bug #318373 */
  ...
  if (details->rename_widget == NULL) {
    ...
    eel_editable_label_set_line_wrap(EEL_EDITABLE_LABEL(details->rename_widget),
                                     TRUE);
    eel_editable_label_set_line_wrap_mode(
        EEL_EDITABLE_LABEL(details->rename_widget), PANGO_WRAP_WORD_CHAR);
    eel_editable_label_set_draw_outline(
        EEL_EDITABLE_LABEL(details->rename_widget), TRUE);
    ...
    gtk_layout_put(GTK_LAYOUT(container), details->rename_widget, 0, 0);
  }
  ...
  if (details->font) {
    ...
    pango_font_description_set_size(
        desc, pango_font_description_get_size(desc) +
                  container->details
                      ->font_size_table[container->details->zoom_level]);
  }
  eel_editable_label_set_font_description(
      EEL_EDITABLE_LABEL(details->rename_widget), desc);
  ...
  if (details->label_position == CAJA_ICON_LABEL_POSITION_BESIDE) {
    eel_canvas_w2c(EEL_CANVAS_ITEM(icon->item)->canvas, text_rect.x0,
                   text_rect.y0, &x, &y);
  } else {
    eel_canvas_w2c(EEL_CANVAS_ITEM(icon->item)->canvas,
                   (icon_rect.x0 + icon_rect.x1) / 2, icon_rect.y1, &x, &y);
    x = x - width / 2 - 1;
  }
  ...
  g_signal_emit(container, signals[RENAMING_ICON], 0,
...
static void end_renaming_mode(CajaIconContainer *container, gboolean commit) {
  ...
  if (commit) {
    ...
    changed_text = eel_editable_label_get_text(
        EEL_EDITABLE_LABEL(container->details->rename_widget));
    if (strcmp(container->details->original_text, changed_text) != 0) {
      AtkObject *accessible_icon;

      g_signal_emit(container, signals[ICON_TEXT_CHANGED], 0, icon->data,
                    changed_text);
...
gboolean caja_icon_container_emit_preview_signal(
    CajaIconContainer *icon_container, CajaIcon *icon, gboolean start_flag) {
  gboolean result;
  ...
  result = FALSE;
  g_signal_emit(icon_container, signals[PREVIEW], 0, icon->data, start_flag,
                &result);

  return result;
}

gboolean caja_icon_container_has_stored_icon_positions(
    CajaIconContainer *container) {
  GList *p;
  gboolean have_stored_position;
  CajaIconPosition position;
...
void caja_icon_container_set_font_size_table(
    CajaIconContainer *container,
...
void caja_icon_container_set_all_columns_same_width(
    CajaIconContainer *container, gboolean all_columns_same_width) {
  g_return_if_fail(CAJA_IS_ICON_CONTAINER(container));
 ...
 **/
void caja_icon_container_set_highlighted_for_clipboard(
    CajaIconContainer *container, GList *clipboard_icon_data) {
  GList *l;
  ...
  for (l = container->details->icons; l != NULL; l = l->next) {
    icon = l->data;
    highlighted_for_clipboard =
        (g_list_find(clipboard_icon_data, icon->data) != NULL);
...
static CajaIconContainerAccessiblePrivate *accessible_get_priv(
  ...
  priv =
      g_object_get_qdata(G_OBJECT(accessible), accessible_private_data_quark);
...
static gboolean caja_icon_container_accessible_do_action(AtkAction *accessible,
                                                         int i) {
  GtkWidget *widget;
  CajaIconContainer *container;
  GList *selection;
  ...
  switch (i) {
    ...
    default:
      g_warning(
          "Invalid action passed to CajaIconContainerAccessible::do_action");
      return FALSE;
  }
  return TRUE;
}

static int caja_icon_container_accessible_get_n_actions(AtkAction *accessible) {
  return LAST_ACTION;
}

static const char *caja_icon_container_accessible_action_get_description(
    AtkAction *accessible, int i) {
  CajaIconContainerAccessiblePrivate *priv;
...
static const char *caja_icon_container_accessible_action_get_name(
    AtkAction *accessible, int i) {
  g_assert(i < LAST_ACTION);
...
static const char *caja_icon_container_accessible_action_get_keybinding(
    AtkAction *accessible, int i) {
  g_assert(i < LAST_ACTION);
...
static gboolean caja_icon_container_accessible_action_set_description(
    AtkAction *accessible, int i, const char *description) {
  CajaIconContainerAccessiblePrivate *priv;
...
static void caja_icon_container_accessible_action_interface_init(
  ...
  iface->get_description =
      caja_icon_container_accessible_action_get_description;
  ...
  iface->set_description =
      caja_icon_container_accessible_action_set_description;
}

/* AtkSelection interface */

static void caja_icon_container_accessible_update_selection(
    AtkObject *accessible) {
  CajaIconContainer *container;
  CajaIconContainerAccessiblePrivate *priv;
  GList *l;
  ...
  for (l = container->details->icons; l != NULL; l = l->next) {
    icon = l->data;
    if (icon->is_selected) {
      priv->selection = g_list_prepend(priv->selection, icon);
    }
  }
...
static void caja_icon_container_accessible_selection_changed_cb(
    CajaIconContainer *container, gpointer data) {
  g_signal_emit_by_name(data, "selection_changed");
}

static void caja_icon_container_accessible_icon_added_cb(
    CajaIconContainer *container, CajaIconData *icon_data, gpointer data) {
  CajaIcon *icon;
  ...
  if (icon) {
    ...
    g_signal_emit_by_name(atk_parent, "children_changed::add", index, atk_child,
                          NULL);
  }
}

static void caja_icon_container_accessible_icon_removed_cb(
    CajaIconContainer *container, CajaIconData *icon_data, gpointer data) {
  CajaIcon *icon;
  ...
  if (icon) {
    ...
    g_signal_emit_by_name(atk_parent, "children_changed::remove", index,
                          atk_child, NULL);
  }
}

static void caja_icon_container_accessible_cleared_cb(
    CajaIconContainer *container, gpointer data) {
  g_signal_emit_by_name(data, "children_changed", 0, NULL, NULL);
}

static gboolean caja_icon_container_accessible_add_selection(
    AtkSelection *accessible, int i) {
  GtkWidget *widget;
  CajaIconContainer *container;
  CajaIcon *icon;
  ...
  if (icon) {
    ...
    selection = caja_icon_container_get_selection(container);
    selection = g_list_prepend(selection, icon->data);
...
static gboolean caja_icon_container_accessible_clear_selection(
    AtkSelection *accessible) {
  GtkWidget *widget;
  CajaIconContainer *container;
...
static AtkObject *caja_icon_container_accessible_ref_selection(
    AtkSelection *accessible, int i) {
  CajaIconContainerAccessiblePrivate *priv;
  CajaIcon *icon;
...
static int caja_icon_container_accessible_get_selection_count(
    AtkSelection *accessible) {
  int count;
  CajaIconContainerAccessiblePrivate *priv;
...
static gboolean caja_icon_container_accessible_is_child_selected(
    AtkSelection *accessible, int i) {
  CajaIconContainer *container;
  CajaIcon *icon;
  GtkWidget *widget;
...
static gboolean caja_icon_container_accessible_remove_selection(
    AtkSelection *accessible, int i) {
  CajaIconContainer *container;
  CajaIconContainerAccessiblePrivate *priv;
  CajaIcon *icon;
...
static gboolean caja_icon_container_accessible_select_all_selection(
    AtkSelection *accessible) {
  CajaIconContainer *container;
  GtkWidget *widget;
...
void caja_icon_container_widget_to_file_operation_position(
    CajaIconContainer *container, GdkPoint *position) {
  double x, y;
  ...
  position->x -=
      caja_get_icon_size_for_zoom_level(container->details->zoom_level) / 2;
  position->y -=
      caja_get_icon_size_for_zoom_level(container->details->zoom_level) / 2;
}

static void caja_icon_container_accessible_selection_interface_init(
  ...
  iface->get_selection_count =
      caja_icon_container_accessible_get_selection_count;
  ...
  iface->select_all_selection =
      caja_icon_container_accessible_select_all_selection;
}

static gint caja_icon_container_accessible_get_n_children(
    AtkObject *accessible) {
  CajaIconContainer *container;
  GtkWidget *widget;
  gint i;
...
static AtkObject *caja_icon_container_accessible_ref_child(
    AtkObject *accessible, int i) {
  AtkObject *atk_object;
  CajaIconContainer *container;
  CajaIcon *icon;
  GtkWidget *widget;
  ...
  if (icon) {
    ...
    if (i == g_list_length(container->details->icons)) {
      if (container->details->rename_widget) {
        atk_object =
            gtk_widget_get_accessible(container->details->rename_widget);
        g_object_ref(atk_object);
...
static void caja_icon_container_accessible_initialize(AtkObject *accessible,
                                                      gpointer data) {
  CajaIconContainerAccessiblePrivate *priv;
...
static void caja_icon_container_accessible_finalize(GObject *object) {
  CajaIconContainerAccessiblePrivate *priv;
  int i;
...
G_DEFINE_TYPE_WITH_CODE(
    CajaIconContainerAccessible, caja_icon_container_accessible,
...
static void caja_icon_container_accessible_init(
    CajaIconContainerAccessible *accessible) {}

static void caja_icon_container_accessible_class_init(
    CajaIconContainerAccessibleClass *klass) {
...
static char *check_compute_stretch(int icon_x, int icon_y, int icon_size,
  ...
  return g_strdup_printf("%d,%d:%d", current.icon_x, current.icon_y,
                         current.icon_size);
}

void caja_self_check_icon_container(void) {
  EEL_CHECK_STRING_RESULT(check_compute_stretch(0, 0, 16, 0, 0, 0, 0),
                          "0,0:16");
  EEL_CHECK_STRING_RESULT(check_compute_stretch(0, 0, 16, 16, 16, 17, 17),
                          "0,0:17");
  EEL_CHECK_STRING_RESULT(check_compute_stretch(0, 0, 16, 16, 16, 17, 16),
                          "0,0:16");
  EEL_CHECK_STRING_RESULT(check_compute_stretch(100, 100, 64, 105, 105, 40, 40),
                          "35,35:129");
}
...
int caja_icon_container_get_max_layout_lines_for_pango(
    CajaIconContainer *container) {
  int limit;
...
void caja_icon_container_begin_loading(CajaIconContainer *container) {
  ...
  if (caja_icon_container_get_store_layout_timestamps(container)) {
    ...
    g_signal_emit(container, signals[GET_STORED_LAYOUT_TIMESTAMP], 0, NULL,
                  &container->details->layout_timestamp, &dummy);
  }
}
...
static void store_layout_timestamps_now(CajaIconContainer *container) {
  ...
  g_signal_emit(container, signals[STORE_LAYOUT_TIMESTAMP], 0, NULL,
                &container->details->layout_timestamp, &dummy);
...
gboolean caja_icon_container_get_store_layout_timestamps(
    CajaIconContainer *container) {
  return container->details->store_layout_timestamps;
}

void caja_icon_container_set_store_layout_timestamps(
    CajaIconContainer *container, gboolean store_layout_timestamps) {
  container->details->store_layout_timestamps =
      (store_layout_timestamps != FALSE);
}
