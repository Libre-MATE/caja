...
static gboolean is_reserved_keyword(const char *keyword) {
  GList *available;
  char *icon_name;
  gboolean result;
  ...
  result = g_list_find_custom(available, (char *)icon_name,
...
GdkPixbuf *caja_emblem_load_pixbuf_for_emblem(GFile *emblem) {
  ...
  scaled = eel_gdk_pixbuf_scale_down_to_fit(pixbuf, CAJA_ICON_SIZE_STANDARD,
...
/* utility to make sure the passed-in keyword only contains alphanumeric
 * characters */
static gboolean emblem_keyword_valid(const char *keyword) {
  const char *p;
  gunichar c;
...
gboolean caja_emblem_verify_keyword(GtkWindow *parent_window,
  ...
  if (keyword == NULL || strlen(keyword) == 0) {
    eel_show_error_dialog(_("The emblem cannot be installed."),
                          _("Sorry, but you must specify a non-blank keyword "
                            "for the new emblem."),
                          GTK_WINDOW(parent_window));
    return FALSE;
  } else if (!emblem_keyword_valid(keyword)) {
    eel_show_error_dialog(_("The emblem cannot be installed."),
                          _("Sorry, but emblem keywords can only contain "
                            "letters, spaces and numbers."),
                          GTK_WINDOW(parent_window));
    return FALSE;
    ...
    error_string = g_strdup_printf(
        _("Sorry, but there is already an emblem named \"%s\"."), display_name);
    eel_show_error_dialog(_("Please choose a different emblem name."),
                          error_string, GTK_WINDOW(parent_window));
    g_free(error_string);
    return FALSE;
  }
...
void caja_emblem_install_custom_emblem(GdkPixbuf *pixbuf, const char *keyword,
  ...
  dir = g_build_filename(g_get_home_dir(), ".icons", "hicolor", "48x48",
                         "emblems", NULL);
  stat_dir = g_build_filename(g_get_home_dir(), ".icons", "hicolor", NULL);
...
gboolean caja_emblem_remove_emblem(const char *keyword) {
  ...
  dir = g_strdup_printf("%s/.icons/hicolor/48x48/emblems", g_get_home_dir());
  stat_dir = g_strdup_printf("%s/.icons/hicolor", g_get_home_dir());
...
gboolean caja_emblem_rename_emblem(const char *keyword, const char *name) {
