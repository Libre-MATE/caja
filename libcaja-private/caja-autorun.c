...
void caja_autorun_get_preferences(const char *x_content_type,
  ...
  *pref_open_folder = FALSE;
  x_content_start_app =
      g_settings_get_strv(caja_media_preferences,
                          CAJA_PREFERENCES_MEDIA_AUTORUN_X_CONTENT_START_APP);
  x_content_ignore = g_settings_get_strv(
      caja_media_preferences, CAJA_PREFERENCES_MEDIA_AUTORUN_X_CONTENT_IGNORE);
  x_content_open_folder =
      g_settings_get_strv(caja_media_preferences,
                          CAJA_PREFERENCES_MEDIA_AUTORUN_X_CONTENT_OPEN_FOLDER);
  if (x_content_start_app != NULL) {
    *pref_start_app =
        eel_g_strv_find(x_content_start_app, x_content_type) != -1;
  }
  ...
  if (x_content_open_folder != NULL) {
    *pref_open_folder =
        eel_g_strv_find(x_content_open_folder, x_content_type) != -1;
  }
...
static void remove_elem_from_str_array(char **v, const char *s) {
  int n, m;
...
static char **add_elem_to_str_array(char **v, const char *s) {
  guint len;
  char **r;
  ...
  if (v) memcpy(r, v, len * sizeof(char *));
...
void caja_autorun_set_preferences(const char *x_content_type,
                                  gboolean pref_start_app, gboolean pref_ignore,
                                  gboolean pref_open_folder) {
  char **x_content_start_app;
  char **x_content_ignore;
  char **x_content_open_folder;
  ...
  if (pref_start_app) {
    x_content_start_app =
        add_elem_to_str_array(x_content_start_app, x_content_type);
  }
  g_settings_set_strv(caja_media_preferences,
                      CAJA_PREFERENCES_MEDIA_AUTORUN_X_CONTENT_START_APP,
  ...
  g_settings_set_strv(caja_media_preferences,
                      ...
                      (const gchar *const *)x_content_ignore);

  remove_elem_from_str_array(x_content_open_folder, x_content_type);
  if (pref_open_folder) {
    x_content_open_folder =
        add_elem_to_str_array(x_content_open_folder, x_content_type);
  }
  g_settings_set_strv(caja_media_preferences,
                      CAJA_PREFERENCES_MEDIA_AUTORUN_X_CONTENT_OPEN_FOLDER,
...
static gboolean combo_box_separator_func(GtkTreeModel *model, GtkTreeIter *iter,
                                         gpointer data) {
  char *str;
...
static void caja_autorun_combobox_data_destroy(CajaAutorunComboBoxData *data) {
  /* signal handler may be automatically disconnected by destroying the widget
   */
  if (g_signal_handler_is_connected(G_OBJECT(data->combo_box),
                                    data->changed_signal_id)) {
    g_signal_handler_disconnect(G_OBJECT(data->combo_box),
                                data->changed_signal_id);
  }
...
static void other_application_selected(CajaOpenWithDialog *dialog,
                                       GAppInfo *app_info,
  ...
  if (data->update_settings) {
    ...
    g_app_info_set_as_default_for_type(app_info, data->x_content_type, NULL);
...
static void handle_dialog_closure(CajaAutorunComboBoxData *data) {
...
static void dialog_destroy_cb(GtkWidget *object,
                              CajaAutorunComboBoxData *data) {
  handle_dialog_closure(data);
}

static void combo_box_changed(GtkComboBox *combo_box,
                              CajaAutorunComboBoxData *data) {
  GtkTreeIter iter;
  GtkTreeModel *model;
  GAppInfo *app_info;
  ...
  gtk_tree_model_get(model, &iter, COLUMN_AUTORUN_APP_INFO, &app_info,
                     COLUMN_AUTORUN_X_CONTENT_TYPE, &x_content_type,
                     COLUMN_AUTORUN_ITEM_TYPE, &type, -1);
  ...
  switch (type) {
    ...
    case AUTORUN_APP:
      ...
      if (data->update_settings) {
        ...
        g_app_info_set_as_default_for_type(app_info, x_content_type, NULL);
      }
      break;
    ...
    case AUTORUN_OTHER_APP: {
      ...
      g_signal_connect(dialog, "application_selected",
                       G_CALLBACK(other_application_selected), data);
...
static void caja_autorun_rebuild_combo_box(GtkWidget *combo_box) {
  ...
  caja_autorun_prepare_combo_box(combo_box, x_content_type, data->include_ask,
                                 data->include_open_with_other_app,
                                 data->update_settings, data->changed_cb,
                                 data->user_data);
  g_free(x_content_type);
}
...
void caja_autorun_prepare_combo_box(
    GtkWidget *combo_box, const char *x_content_type, gboolean include_ask,
    gboolean include_open_with_other_app, gboolean update_settings,
    CajaAutorunComboBoxChanged changed_cb, gpointer user_data) {
  GList *l;
  GList *app_info_list;
  ...
  caja_autorun_get_preferences(x_content_type, &pref_start_app, &pref_ignore,
                               &pref_open_folder);
  ...
  list_store = gtk_list_store_new(5, CAIRO_GOBJECT_TYPE_SURFACE, G_TYPE_STRING,
                                  G_TYPE_APP_INFO, G_TYPE_STRING, G_TYPE_INT);

  /* no apps installed */
  if (num_apps == 0) {
    gtk_list_store_append(list_store, &iter);
    surface = gtk_icon_theme_load_surface(gtk_icon_theme_get_default(),
                                          "dialog-error", icon_size, icon_scale,
                                          NULL, 0, NULL);

    /* TODO: integrate with PackageKit-mate to find applications */

    gtk_list_store_set(list_store, &iter, COLUMN_AUTORUN_SURFACE, surface,
                       COLUMN_AUTORUN_NAME, _("No applications found"),
                       COLUMN_AUTORUN_APP_INFO, NULL,
                       COLUMN_AUTORUN_X_CONTENT_TYPE, x_content_type,
                       COLUMN_AUTORUN_ITEM_TYPE, AUTORUN_ASK, -1);
    cairo_surface_destroy(surface);
  } else {
    if (include_ask) {
      gtk_list_store_append(list_store, &iter);
      surface = gtk_icon_theme_load_surface(gtk_icon_theme_get_default(),
                                            "dialog-question", icon_size,
    ...
    gtk_list_store_append(list_store, &iter);
    surface = gtk_icon_theme_load_surface(gtk_icon_theme_get_default(),
                                          "window-close", icon_size, icon_scale,
                                          NULL, 0, NULL);
    gtk_list_store_set(list_store, &iter, COLUMN_AUTORUN_SURFACE, surface,
                       COLUMN_AUTORUN_NAME, _("Do Nothing"),
                       COLUMN_AUTORUN_APP_INFO, NULL,
                       COLUMN_AUTORUN_X_CONTENT_TYPE, x_content_type,
                       COLUMN_AUTORUN_ITEM_TYPE, AUTORUN_IGNORE, -1);
    cairo_surface_destroy(surface);

    gtk_list_store_append(list_store, &iter);
    surface =
        gtk_icon_theme_load_surface(gtk_icon_theme_get_default(), "folder-open",
    ...
    gtk_list_store_append(list_store, &iter);
    gtk_list_store_set(list_store, &iter, COLUMN_AUTORUN_SURFACE, NULL,
                       COLUMN_AUTORUN_NAME, NULL, COLUMN_AUTORUN_APP_INFO, NULL,
                       COLUMN_AUTORUN_X_CONTENT_TYPE, NULL,
                       COLUMN_AUTORUN_ITEM_TYPE, AUTORUN_SEP, -1);

    int n;

    for (l = app_info_list, n = include_ask ? 4 : 3; l != NULL;
         l = l->next, n++) {
      GIcon *icon;
      ...
      open_string =
          g_strdup_printf(_("Open %s"), g_app_info_get_display_name(app_info));
      ...
      gtk_list_store_set(list_store, &iter, COLUMN_AUTORUN_SURFACE, surface,
                         ...
                         COLUMN_AUTORUN_X_CONTENT_TYPE, x_content_type,
                         COLUMN_AUTORUN_ITEM_TYPE, AUTORUN_APP, -1);
      if (surface != NULL) {
        cairo_surface_destroy(surface);
      }
      g_free(open_string);
  ...
  if (include_open_with_other_app) {
    ...
    gtk_list_store_append(list_store, &iter);
    surface = gtk_icon_theme_load_surface(gtk_icon_theme_get_default(),
                                          "application-x-executable", icon_size,
                                          icon_scale, NULL, 0, NULL);
    gtk_list_store_set(list_store, &iter, COLUMN_AUTORUN_SURFACE, surface,
                       COLUMN_AUTORUN_NAME, _("Open with other Application..."),
                       COLUMN_AUTORUN_APP_INFO, NULL,
                       COLUMN_AUTORUN_X_CONTENT_TYPE, x_content_type,
                       COLUMN_AUTORUN_ITEM_TYPE, AUTORUN_OTHER_APP, -1);
    cairo_surface_destroy(surface);
  }
  ...
  gtk_cell_layout_set_attributes(GTK_CELL_LAYOUT(combo_box), renderer,
                                 "surface", COLUMN_AUTORUN_SURFACE, NULL);
  ...
  gtk_combo_box_set_row_separator_func(GTK_COMBO_BOX(combo_box),
                                       combo_box_separator_func, NULL, NULL);
  ...
  if (num_apps == 0) {
    ...
    if (data->changed_signal_id == 0) {
      data->changed_signal_id = g_signal_connect(
          G_OBJECT(combo_box), "changed", G_CALLBACK(combo_box_changed), data);
    }
  }

  if (new_data) {
    g_object_set_data_full(G_OBJECT(combo_box), "caja_autorun_combobox_data",
                           data,
                           (GDestroyNotify)caja_autorun_combobox_data_destroy);
  }
}

static gboolean is_shift_pressed(void) {
  gboolean ret;
  GdkDisplay *display;
  XkbStateRec state;
  Bool status;
  ...
  status = XkbGetState(GDK_DISPLAY_XDISPLAY(display), XkbUseCoreKbd, &state);
...
enum { AUTORUN_DIALOG_RESPONSE_EJECT = 0 };
...
void caja_autorun_launch_for_mount(GMount *mount, GAppInfo *app_info) {
  ...
  caja_launch_application(app_info, files, NULL); /* TODO: what to set here? */
  g_object_unref(file);
  g_list_free(files);
}

static void autorun_dialog_mount_unmounted(GMount *mount,
                                           AutorunDialogData *data);

static void autorun_dialog_destroy(AutorunDialogData *data) {
  g_signal_handlers_disconnect_by_func(
      G_OBJECT(data->mount), G_CALLBACK(autorun_dialog_mount_unmounted), data);
...
static void autorun_dialog_response(GtkDialog *dialog, gint response,
                                    AutorunDialogData *data) {
  switch (response) {
    case AUTORUN_DIALOG_RESPONSE_EJECT:
      caja_file_operations_unmount_mount(GTK_WINDOW(dialog), data->mount,
    ...
    case GTK_RESPONSE_OK:
      ...
      if (data->remember) {
        ...
        caja_autorun_set_preferences(data->x_content_type, TRUE,
                                     data->selected_ignore,
                                     data->selected_open_folder);
        if (!data->selected_ignore && !data->selected_open_folder &&
            data->selected_app != NULL) {
          g_app_info_set_as_default_for_type(data->selected_app,
                                             data->x_content_type, NULL);
        }
      } else {
...
static void autorun_combo_changed(gboolean selected_ask,
                                  gboolean selected_ignore,
                                  gboolean selected_open_folder,
                                  GAppInfo *selected_app, gpointer user_data) {
...
static void autorun_always_toggled(GtkToggleButton *togglebutton,
                                   AutorunDialogData *data) {
  data->remember = gtk_toggle_button_get_active(togglebutton);
}

static gboolean combo_box_enter_ok(GtkWidget *togglebutton, GdkEventKey *event,
                                   GtkDialog *dialog) {
...
static gboolean do_autorun_for_content_type(
    GMount *mount, const char *x_content_type,
    CajaAutorunOpenWindow open_window_func, gpointer user_data) {
  AutorunDialogData *data;
  GtkWidget *dialog;
  GtkWidget *hbox;
  GtkWidget *vbox;
...
show_dialog:
  ...
  if (strcmp(x_content_type, "x-content/audio-cdda") == 0) {
    ...
    media_greeting =
        _("You have just inserted a medium with software intended to be "
          "automatically started.");
  } else {
    /* fallback to generic greeting */
    media_greeting = _("You have just inserted a medium.");
  }
  markup = g_strdup_printf("<big><b>%s %s</b></big>", media_greeting,
  ...
  markup =
      g_strdup_printf(_("Select how to open \"%s\" and whether to perform this "
                        "action in the future for other media of type \"%s\"."),
                      mount_name, content_description);
  ...
  action_area = gtk_widget_get_parent(eel_dialog_add_button(
      GTK_DIALOG(dialog), _("_OK"), "gtk-ok", GTK_RESPONSE_OK));
  ...
  if (g_mount_can_eject(mount)) {
    ...
    surface = gtk_icon_theme_load_surface(
        gtk_icon_theme_get_default(), "media-eject",
        caja_get_icon_size_for_stock_size(GTK_ICON_SIZE_BUTTON), icon_scale,
        NULL, 0, NULL);
  ...
  gtk_dialog_add_action_widget(GTK_DIALOG(dialog), eject_button,
                               AUTORUN_DIALOG_RESPONSE_EJECT);
  gtk_button_box_set_child_secondary(GTK_BUTTON_BOX(action_area), eject_button,
                                     TRUE);
...
static void autorun_guessed_content_type_callback(GObject *source_object,
  ...
  guessed_content_type =
      g_mount_guess_content_type_finish(G_MOUNT(source_object), res, &error);
  g_object_set_data_full(source_object, "caja-content-type-cache",
...
void caja_autorun(GMount *mount, CajaAutorunOpenWindow open_window_func,
                  gpointer user_data) {
  AutorunData *data;
  ...
  g_mount_guess_content_type(mount, FALSE, NULL,
                             autorun_guessed_content_type_callback, data);
}
...
void caja_autorun_get_x_content_types_for_mount_async(
    GMount *mount, CajaAutorunGetContent callback, GCancellable *cancellable,
    gpointer user_data) {
  char **cached;
  GetContentTypesData *data;
...
static gboolean remove_allow_volume(gpointer data) {
  GVolume *volume = data;
...
void caja_allow_autorun_for_volume_finish(GVolume *volume) {
  if (g_object_get_data(G_OBJECT(volume), "caja-allow-autorun") != NULL) {
    g_timeout_add_seconds_full(0, INHIBIT_AUTORUN_SECONDS, remove_allow_volume,
                               g_object_ref(volume), g_object_unref);
  }
}

static gboolean should_skip_native_mount_root(GFile *root) {
  char *path;
...
static gboolean should_autorun_mount(GMount *mount) {
  ...
  if (enclosing_volume != NULL) {
    if (g_object_get_data(G_OBJECT(enclosing_volume), "caja-allow-autorun") !=
        NULL) {
  ...
  /* only do autorun on local files or files where g_volume_should_automount()
   * returns TRUE */
  ignore_autorun = TRUE;
  if ((g_file_is_native(root) && !should_skip_native_mount_root(root)) ||
      (enclosing_volume != NULL &&
       g_volume_should_automount(enclosing_volume))) {
    ignore_autorun = FALSE;
  }
  if (enclosing_volume != NULL) {
    g_object_unref(enclosing_volume);
