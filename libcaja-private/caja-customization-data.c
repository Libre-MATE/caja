...
static char *format_name_for_display(CajaCustomizationData *data,
                                     const char *name);
static void load_name_map_hash_table(CajaCustomizationData *data);

static gboolean read_all_children(char *filename, const char *attributes,
                                  GList **list_out) {
...
CajaCustomizationData *caja_customization_data_new(
    const char *customization_name, gboolean show_public_customizations,
    int maximum_icon_height, int maximum_icon_width) {
  ...
  if (show_public_customizations) {
    ...
    public_result =
        read_all_children(public_directory_path,
...
gboolean caja_customization_data_get_next_element_for_display(
    CajaCustomizationData *data, char **emblem_name, GdkPixbuf **pixbuf_out,
    char **label_out) {
  ...
  if (data->current_file_list == NULL) {
    if (data->reading_mode == READ_PUBLIC_CUSTOMIZATIONS) {
      ...
      data->reading_mode = READ_PRIVATE_CUSTOMIZATIONS;
      data->current_file_list = data->private_file_list;
      return caja_customization_data_get_next_element_for_display(
  ...
  if (!eel_istr_has_prefix(g_file_info_get_content_type(current_file_info),
                           "image/") ||
  ...
  image_file_name =
      get_file_path_for_mode(data, g_file_info_get_name(current_file_info));
  orig_pixbuf = gdk_pixbuf_new_from_file_at_scale(
      image_file_name, data->maximum_icon_width, data->maximum_icon_height,
      TRUE, NULL);
  ...
  is_reset_image =
      g_strcmp0(g_file_info_get_name(current_file_info), RESET_IMAGE_NAME) == 0;
  ...
  *emblem_name = g_strdup(g_file_info_get_name(current_file_info));
  ...
  if (strcmp(data->customization_name, "patterns") == 0 &&
    ...
    pixbuf = caja_customization_make_pattern_chit(
        orig_pixbuf, data->pattern_frame, FALSE, is_reset_image);
  } else {
    pixbuf = eel_gdk_pixbuf_scale_down_to_fit(
        orig_pixbuf, data->maximum_icon_width, data->maximum_icon_height);
  }
  ...
  *label_out =
      format_name_for_display(data, g_file_info_get_name(current_file_info));
...
gboolean caja_customization_data_private_data_was_displayed(
    CajaCustomizationData *data) {
  return data->private_data_was_displayed;
}

void caja_customization_data_destroy(CajaCustomizationData *data) {
  g_assert(data->public_file_list != NULL || data->private_file_list != NULL);
...
static char *get_global_customization_path(const char *customization_name) {
  return g_build_filename(CAJA_DATADIR, customization_name, NULL);
}
...
static char *get_private_customization_path(const char *customization_name) {
  ...
  directory_path = g_build_filename(user_directory, customization_name, NULL);
...
GdkPixbuf *caja_customization_make_pattern_chit(GdkPixbuf *pattern_tile,
  ...
  gdk_pixbuf_scale(
      pattern_tile, pixbuf, 2, 2, frame_width - 8, frame_height - 8, 0, 0,
      (double)(frame_width - 8 + 1) / pattern_width,
      (double)(frame_height - 8 + 1) / pattern_height, GDK_INTERP_BILINEAR);

  /* if we're dragging, get rid of the light-colored halo */
  if (dragging) {
    temp_pixbuf = gdk_pixbuf_new(GDK_COLORSPACE_RGB, TRUE, 8, frame_width - 8,
                                 frame_height - 8);
    gdk_pixbuf_copy_area(pixbuf, 2, 2, frame_width - 8, frame_height - 8,
                         temp_pixbuf, 0, 0);
...
/* utility to format the passed-in name for display by stripping the extension,
   mapping underscore and capitalizing as necessary */
...
static char *format_name_for_display(CajaCustomizationData *data,
  ...
  /* map file names to display names using the mappings defined in the hash
   * table */
...
static void load_name_map_hash_table(CajaCustomizationData *data) {
  ...
  if ((browser_data = xmlParseFile(CAJA_DATADIR "/browser.xml")) != NULL) {
    ...
    category_node = eel_xml_get_root_child_by_name_and_property(
        browser_data, (const xmlChar *)"category", (const xmlChar *)"name",
