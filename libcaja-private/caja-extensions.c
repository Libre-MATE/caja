...
static Extension *extension_new(gchar *filename, gboolean state,
                                gboolean python, GObject *module) {
  GError *error = NULL;
  Extension *ext;
  ...
  extension_filename =
      g_strdup_printf(CAJA_DATADIR "/extensions/%s.caja-extension", filename);
  if (g_key_file_load_from_file(extension_file, extension_filename,
                                G_KEY_FILE_NONE, &error)) {
    ext->name = g_key_file_get_locale_string(
        extension_file, CAJA_EXTENSION_GROUP, "Name", NULL, NULL);
    ext->description = g_key_file_get_locale_string(
        extension_file, CAJA_EXTENSION_GROUP, "Description", NULL, NULL);
    ext->icon = g_key_file_get_string(extension_file, CAJA_EXTENSION_GROUP,
                                      "Icon", NULL);
    ext->author = g_key_file_get_string_list(
        extension_file, CAJA_EXTENSION_GROUP, "Author", NULL, NULL);
    ext->copyright = g_key_file_get_locale_string(
        extension_file, CAJA_EXTENSION_GROUP, "Copyright", NULL, NULL);
    ext->version = g_key_file_get_string(extension_file, CAJA_EXTENSION_GROUP,
                                         "Version", NULL);
    ext->website = g_key_file_get_string(extension_file, CAJA_EXTENSION_GROUP,
                                         "Website", NULL);
    g_key_file_free(extension_file);
  } else {
    caja_debug_log(FALSE, CAJA_DEBUG_LOG_DOMAIN_USER,
                   "Error loading keys from file \"%s\": %s\n",
...
static gboolean gsettings_key_has_value(const gchar *value) {
  gchar **list;
  gint i;
...
static gboolean gsettings_append_to_list(const char *value) {
  gchar **current;
  gchar **new;
  gint size;
  gboolean retval;
...
static gboolean gsettings_remove_from_list(const char *value) {
  gchar **current;
  GArray *array;
  gint i;
  gboolean retval;
...
static gboolean caja_extension_get_state(const gchar *extname) {
...
GList *caja_extensions_get_list(void) { return caja_extensions; }
