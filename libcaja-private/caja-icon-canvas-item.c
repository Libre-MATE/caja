 ...
 *
 *  + text_height_for_entire_text: height needed to display the entire
 * PangoLayout, if it wasn't ellipsized.
 */
...
struct _CajaIconCanvasItemPrivate {
  ...
  int text_height;
  /* actual size that would be required for rendering the entire text if it
   * wasn't ellipsized */
...
typedef enum { RIGHT_SIDE, BOTTOM_SIDE, LEFT_SIDE, TOP_SIDE } RectangleSide;
...
static void caja_icon_canvas_item_text_interface_init(
    EelAccessibleTextIface *iface);
static GType caja_icon_canvas_item_accessible_factory_get_type(void);

G_DEFINE_TYPE_WITH_CODE(
    CajaIconCanvasItem, caja_icon_canvas_item, EEL_TYPE_CANVAS_ITEM,
    G_ADD_PRIVATE(CajaIconCanvasItem)
        G_IMPLEMENT_INTERFACE(EEL_TYPE_ACCESSIBLE_TEXT,
                              caja_icon_canvas_item_text_interface_init));

/* private */
static void draw_label_text(CajaIconCanvasItem *item, cairo_t *cr,
                            gboolean create_mask, EelIRect icon_rect);
static void measure_label_text(CajaIconCanvasItem *item);
static void get_icon_canvas_rectangle(CajaIconCanvasItem *item, EelIRect *rect);
static void emblem_layout_reset(EmblemLayout *layout,
                                CajaIconCanvasItem *icon_item,
                                EelIRect icon_rect, gboolean is_rtl);
static gboolean emblem_layout_next(EmblemLayout *layout,
                                   GdkPixbuf **emblem_pixbuf,
                                   EelIRect *emblem_rect, gboolean is_rtl);
static void draw_pixbuf(GdkPixbuf *pixbuf, cairo_t *cr, int x, int y);
...
static void draw_embedded_text(CajaIconCanvasItem *icon_item, cairo_t *cr,
                               int x, int y);

static void caja_icon_canvas_item_ensure_bounds_up_to_date(
    CajaIconCanvasItem *icon_item);

/* Object initialization function for the icon item. */
static void caja_icon_canvas_item_init(CajaIconCanvasItem *icon_item) {
...
static void caja_icon_canvas_item_finalize(GObject *object) {
 ...
 */
static gboolean pixbuf_is_acceptable(GdkPixbuf *pixbuf) {
...
static void caja_icon_canvas_item_invalidate_bounds_cache(
    CajaIconCanvasItem *item) {
...
static void caja_icon_canvas_item_set_property(GObject *object,
                                               guint property_id,
                                               const GValue *value,
                                               GParamSpec *pspec) {
  CajaIconCanvasItem *item;
  ...
  switch (property_id) {
    case PROP_EDITABLE_TEXT:
      if (g_strcmp0(details->editable_text, g_value_get_string(value)) == 0) {
        return;
      }
      ...
      if (details->text_util) {
        gail_text_util_text_setup(details->text_util, details->editable_text);
      }
    ...
    case PROP_ADDITIONAL_TEXT:
      if (g_strcmp0(details->additional_text, g_value_get_string(value)) == 0) {
        return;
      }
    ...
    case PROP_HIGHLIGHTED_FOR_CLIPBOARD:
      if (!details->is_highlighted_for_clipboard ==
          !g_value_get_boolean(value)) {
        return;
      }
...
static void caja_icon_canvas_item_get_property(GObject *object,
...
static void get_scaled_icon_size(CajaIconCanvasItem *item, gint *width,
                                 gint *height) {
...
cairo_surface_t *caja_icon_canvas_item_get_drag_surface(
    CajaIconCanvasItem *item) {
  ...
  eel_canvas_world_to_window(canvas, item->details->x, item->details->y,
                             &item_x, &item_y);
  ...
  surface = gdk_window_create_similar_surface(
      gtk_widget_get_window(GTK_WIDGET(canvas)), CAIRO_CONTENT_COLOR_ALPHA,
      width, height);

  cr = cairo_create(surface);

  drag_surface = gdk_cairo_surface_create_from_pixbuf(
      item->details->pixbuf, gtk_widget_get_scale_factor(GTK_WIDGET(canvas)),
      gtk_widget_get_window(GTK_WIDGET(canvas)));
  gtk_render_icon_surface(context, cr, drag_surface, item_offset_x,
                          item_offset_y);
  ...
  while (emblem_layout_next(&emblem_layout, &emblem_pixbuf, &emblem_rect,
                            is_rtl)) {
    gdk_cairo_set_source_pixbuf(cr, emblem_pixbuf, emblem_rect.x0,
                                emblem_rect.y0);
  ...
  draw_embedded_text(item, cr, item_offset_x, item_offset_y);
...
void caja_icon_canvas_item_set_attach_points(CajaIconCanvasItem *item,
  ...
  if (attach_points != NULL && n_attach_points != 0) {
    item->details->attach_points =
        g_memdup(attach_points, n_attach_points * sizeof(GdkPoint));
...
void caja_icon_canvas_item_set_embedded_text_rect(
    CajaIconCanvasItem *item, const GdkRectangle *text_rect) {
 ...
 */
static void recompute_bounding_box(CajaIconCanvasItem *icon_item, double i2w_dx,
                                   double i2w_dy) {
  ...
  eel_canvas_item_get_bounds(item, &top_left.x, &top_left.y, &bottom_right.x,
                             &bottom_right.y);
  ...
  eel_canvas_w2c_d(item->canvas, top_left.x, top_left.y, &item->x1, &item->y1);
  eel_canvas_w2c_d(item->canvas, bottom_right.x, bottom_right.y, &item->x2,
                   &item->y2);
}

static EelIRect compute_text_rectangle(const CajaIconCanvasItem *item,
                                       EelIRect icon_rectangle,
                                       gboolean canvas_coords,
                                       CajaIconCanvasItemBoundsUsage usage) {
  EelIRect text_rectangle;
  double pixels_per_unit;
  double text_width, text_height, text_height_for_layout,
      text_height_for_entire_text, real_text_height, text_dx;
  ...
  if (canvas_coords) {
    ...
    text_height_for_layout =
        item->details->text_height_for_layout / pixels_per_unit;
    text_height_for_entire_text =
        item->details->text_height_for_entire_text / pixels_per_unit;
    text_dx = item->details->text_dx / pixels_per_unit;
  }

  if (CAJA_ICON_CONTAINER(EEL_CANVAS_ITEM(item)->canvas)
          ->details->label_position == CAJA_ICON_LABEL_POSITION_BESIDE) {
    if (!caja_icon_container_is_layout_rtl(
            CAJA_ICON_CONTAINER(EEL_CANVAS_ITEM(item)->canvas))) {
    ...
    text_rectangle.x0 =
        (icon_rectangle.x0 + icon_rectangle.x1) / 2 - (int)text_width / 2;
    ...
    text_rectangle.y1 =
        text_rectangle.y0 + real_text_height + LABEL_OFFSET / pixels_per_unit;
  }

  return text_rectangle;
}

static EelIRect get_current_canvas_bounds(EelCanvasItem *item) {
  EelIRect bounds;
...
void caja_icon_canvas_item_update_bounds(CajaIconCanvasItem *item,
  ...
  item->details->text_rect = compute_text_rectangle(
      item, item->details->canvas_rect, TRUE, BOUNDS_USAGE_FOR_DISPLAY);
  ...
  while (emblem_layout_next(&emblem_layout, &emblem_pixbuf, &emblem_rect,
                            is_rtl)) {
    eel_irect_union(&item->details->emblem_rect, &item->details->emblem_rect,
                    &emblem_rect);
  }

  /* queue a redraw. */
  eel_canvas_request_redraw(canvas_item->canvas, before.x0, before.y0,
                            before.x1 + 1, before.y1 + 1);
}

/* Update handler for the icon canvas item. */
static void caja_icon_canvas_item_update(EelCanvasItem *item, double i2w_dx,
                                         double i2w_dy, gint flags) {
  caja_icon_canvas_item_update_bounds(CAJA_ICON_CANVAS_ITEM(item), i2w_dx,
                                      i2w_dy);

  eel_canvas_item_request_redraw(EEL_CANVAS_ITEM(item));

  EEL_CANVAS_ITEM_CLASS(caja_icon_canvas_item_parent_class)
      ->update(item, i2w_dx, i2w_dy, flags);
}

/* Rendering */
static gboolean in_single_click_mode(void) {
  return click_policy_auto_value == CAJA_CLICK_POLICY_SINGLE;
}

/* Keep these for a bit while we work on performance of
 ...
 */
static void layout_get_full_size(PangoLayout *layout, int *width, int *height,
                                 int *dx) {
  ...
  total_width =
      (logical_rect.x + logical_rect.width + PANGO_SCALE / 2) / PANGO_SCALE;
...
static void layout_get_size_for_layout(PangoLayout *layout,
  ...
  if (pango_layout_get_line_count(layout) <= max_layout_line_count) {
    ...
    *height_for_layout = 0;
    ...
    /* VOODOO-TODO, determine number of lines based on the icon size for text
     * besides icon. cf. compute_text_rectangle() */
    for (i = 0; i < max_layout_line_count; i++) {
      pango_layout_iter_get_line_extents(iter, NULL, &logical_rect);
      *height_for_layout +=
          (logical_rect.height + PANGO_SCALE / 2) / PANGO_SCALE;
...
static void prepare_pango_layout_width(CajaIconCanvasItem *item,
                                       PangoLayout *layout) {
  if (caja_icon_canvas_item_get_max_text_width(item) < 0) {
    ...
    pango_layout_set_width(
        layout,
        floor(caja_icon_canvas_item_get_max_text_width(item)) * PANGO_SCALE);
    pango_layout_set_ellipsize(layout, PANGO_ELLIPSIZE_END);
  }
}

static void prepare_pango_layout_for_measure_entire_text(
    CajaIconCanvasItem *item, PangoLayout *layout) {
...
static void prepare_pango_layout_for_draw(CajaIconCanvasItem *item,
                                          PangoLayout *layout) {
  CajaIconCanvasItemPrivate *details;
  CajaIconContainer *container;
  ...
  needs_highlight =
      details->is_highlighted_for_selection || details->is_highlighted_for_drop;

  if (IS_COMPACT_VIEW(container)) {
    pango_layout_set_height(layout, -1);
  } else if (needs_highlight || details->is_highlighted_as_keyboard_focus ||
             details->entire_text ||
    ...
    /* TODO? we might save some resources, when the re-layout is not neccessary
     * in case the layout height already fits into max. layout lines. But pango
     * should figure this out itself (which it doesn't ATM).
     */
    pango_layout_set_height(
        layout, caja_icon_container_get_max_layout_lines_for_pango(container));
  }
}
...
static void measure_label_text(CajaIconCanvasItem *item) {
  CajaIconCanvasItemPrivate *details;
  CajaIconContainer *container;
  gint editable_height, editable_height_for_layout,
      editable_height_for_entire_text, editable_width, editable_dx;
  ...
  have_editable =
      details->editable_text != NULL && details->editable_text[0] != '\0';
  have_additional =
      details->additional_text != NULL && details->additional_text[0] != '\0';
  ...
  if (have_editable) {
     ...
     * then, measure text height applicable for layout:
     * editable_height_for_layout next, measure actually displayed height:
     * editable_height
     */
    ...
    layout_get_full_size(editable_layout, NULL,
                         &editable_height_for_entire_text, NULL);
    layout_get_size_for_layout(
        editable_layout, caja_icon_container_get_max_layout_lines(container),
        editable_height_for_entire_text, &editable_height_for_layout);
  ...
  if (have_additional) {
    details->text_height =
        editable_height + LABEL_LINE_SPACING + additional_height;
    details->text_height_for_layout =
        editable_height_for_layout + LABEL_LINE_SPACING + additional_height;
  ...
  /* add some extra space for highlighting even when we don't highlight so
   * things won't move */
...
static void draw_label_text(CajaIconCanvasItem *item, cairo_t *cr,
  ...
  if (details->text_height == 0 || details->text_width == 0) {
    return;
  }
  ...
  text_rect =
      compute_text_rectangle(item, icon_rect, TRUE, BOUNDS_USAGE_FOR_DISPLAY);
  ...
  gtk_widget_style_get(GTK_WIDGET(container), "activate_prelight_icon_label",
                       &prelight_label, NULL);

  /* if the icon needs a background, do some set-up */
  if (!needs_highlight && have_editable && details->text_width > 0 &&
      details->text_height > 0 && prelight_label && item->details->is_prelit) {
    ...
    if (needs_highlight) state |= GTK_STATE_FLAG_SELECTED;

    frame_x = is_rtl_label_beside ? text_rect.x0 + item->details->text_dx
                                  : text_rect.x0;
    frame_y = text_rect.y0;
    frame_w = is_rtl_label_beside
                  ? text_rect.x1 - text_rect.x0 - item->details->text_dx
  ...
  if (draw_frame) {
    ...
    gtk_render_frame(context, cr, frame_x, frame_y, frame_w, frame_h);
    gtk_render_background(context, cr, frame_x, frame_y, frame_w, frame_h);
  ...
  if (have_editable && !details->is_renaming) {
    state = base_state;
    ...
    editable_layout = get_label_layout(&item->details->editable_text_layout,
                                       item, item->details->editable_text);
    prepare_pango_layout_for_draw(item, editable_layout);

    gtk_style_context_save(context);
    gtk_style_context_set_state(context, state);

    gtk_render_layout(context, cr, x, text_rect.y0 + TEXT_BACK_PADDING_Y,
                      editable_layout);

    gtk_style_context_restore(context);
  }

  if (have_additional && !details->is_renaming) {
    state = base_state;
    ...
    additional_layout = get_label_layout(&item->details->additional_text_layout,
                                         item, item->details->additional_text);
    prepare_pango_layout_for_draw(item, additional_layout);
    ...
    gtk_render_layout(context, cr, x,
                      text_rect.y0 + details->editable_text_height +
                          LABEL_LINE_SPACING + TEXT_BACK_PADDING_Y,
                      additional_layout);
  ...
  if (!create_mask && item->details->is_highlighted_as_keyboard_focus) {
    ...
    gtk_render_focus(context, cr, text_rect.x0, text_rect.y0,
                     text_rect.x1 - text_rect.x0, text_rect.y1 - text_rect.y0);
...
void caja_icon_canvas_item_set_is_visible(CajaIconCanvasItem *item,
                                          gboolean visible) {
  if (item->details->is_visible == visible) return;
...
static void draw_stretch_handles(CajaIconCanvasItem *item, cairo_t *cr,
                                 const EelIRect *rect) {
  GtkWidget *widget;
  GdkPixbuf *knob_pixbuf;
  ...
  cairo_rectangle(cr, rect->x0 + 0.5, rect->y0 + 0.5, rect->x1 - rect->x0 - 1,
                  rect->y1 - rect->y0 - 1);
  cairo_stroke(cr);
...
static cairo_surface_t *real_map_surface(CajaIconCanvasItem *icon_item) {
  EelCanvas *canvas;
  ...
  if (icon_item->details->is_prelit ||
    ...
    /* if the icon is currently being previewed, superimpose an image to
     * indicate that */
    /* audio is the only kind of previewing right now, so this code isn't as
    ...
    if (icon_item->details->is_active) {
      ...
      emblem_size = caja_icon_get_emblem_size_for_icon_size(
          gdk_pixbuf_get_width(temp_pixbuf));
      /* Load the audio symbol. */
      audio_filename = caja_pixmap_file("audio.svg");
      if (audio_filename != NULL) {
        audio_pixbuf = gdk_pixbuf_new_from_file_at_scale(
            audio_filename, emblem_size, emblem_size, TRUE, NULL);
      } else {
        audio_pixbuf = NULL;
      }
      ...
      if (audio_pixbuf != NULL) {
        gdk_pixbuf_composite(audio_pixbuf, temp_pixbuf, 0, 0,
                             gdk_pixbuf_get_width(audio_pixbuf),
                             gdk_pixbuf_get_height(audio_pixbuf), 0, 0, 1.0,
                             1.0, GDK_INTERP_BILINEAR, 0xFF);
  ...
  surface = gdk_cairo_surface_create_from_pixbuf(
      temp_pixbuf, gtk_widget_get_scale_factor(GTK_WIDGET(canvas)),
      gtk_widget_get_window(GTK_WIDGET(canvas)));
...
static cairo_surface_t *map_surface(CajaIconCanvasItem *icon_item) {
  if (!(icon_item->details->rendered_surface != NULL &&
        icon_item->details->rendered_is_active ==
            icon_item->details->is_active &&
        icon_item->details->rendered_is_prelit ==
            icon_item->details->is_prelit &&
        icon_item->details->rendered_is_highlighted_for_selection ==
            icon_item->details->is_highlighted_for_selection &&
        icon_item->details->rendered_is_highlighted_for_drop ==
            icon_item->details->is_highlighted_for_drop &&
        icon_item->details->rendered_is_highlighted_for_clipboard ==
            icon_item->details->is_highlighted_for_clipboard &&
        (icon_item->details->is_highlighted_for_selection &&
         icon_item->details->rendered_is_focused ==
             gtk_widget_has_focus(
                 GTK_WIDGET(EEL_CANVAS_ITEM(icon_item)->canvas))))) {
    if (icon_item->details->rendered_surface != NULL) {
    ...
    icon_item->details->rendered_is_highlighted_for_clipboard =
        icon_item->details->is_highlighted_for_clipboard;
    icon_item->details->rendered_is_focused =
        (gtk_widget_has_focus(GTK_WIDGET(EEL_CANVAS_ITEM(icon_item)->canvas)) !=
         FALSE);
  }
...
static void draw_embedded_text(CajaIconCanvasItem *item, cairo_t *cr, int x,
                               int y) {
  PangoLayout *layout;
  GtkWidget *widget;
  ...
  cairo_rectangle(cr, x + item->details->embedded_text_rect.x,
                  y + item->details->embedded_text_rect.y,
                  item->details->embedded_text_rect.width,
                  item->details->embedded_text_rect.height);
  cairo_clip(cr);

  gtk_render_layout(style_context, cr, x + item->details->embedded_text_rect.x,
                    y + item->details->embedded_text_rect.y, layout);
...
static void caja_icon_canvas_item_draw(EelCanvasItem *item, cairo_t *cr,
  ...
  gtk_render_icon_surface(context, cr, temp_surface, icon_rect.x0,
                          icon_rect.y0);
...
static PangoLayout *create_label_layout(CajaIconCanvasItem *item,
                                        const char *text) {
  PangoLayout *layout;
  PangoContext *context;
  PangoFontDescription *desc;
...
static PangoLayout *get_label_layout(PangoLayout **layout_cache,
                                     CajaIconCanvasItem *item,
                                     const char *text) {
  PangoLayout *layout;
...
/* handle events */
static int caja_icon_canvas_item_event(EelCanvasItem *item, GdkEvent *event) {
  CajaIconCanvasItem *icon_item;
  GdkWindow *cursor_window;
  ...
  switch (event->type) {
    case GDK_ENTER_NOTIFY:
      if (!icon_item->details->is_prelit) {
        ...
        eel_canvas_item_send_behind(
            item, CAJA_ICON_CONTAINER(item->canvas)
        ...
        icon_item->details->is_active =
            (caja_icon_container_emit_preview_signal(
                 ...
                 CAJA_ICON_CANVAS_ITEM(item)->user_data, TRUE) != FALSE);
      }
      return TRUE;
    ...
    case GDK_LEAVE_NOTIFY:
      if (icon_item->details->is_prelit ||
        ...
        caja_icon_container_emit_preview_signal(
            CAJA_ICON_CONTAINER(item->canvas),
            CAJA_ICON_CANVAS_ITEM(item)->user_data, FALSE);
...
static gboolean hit_test_pixbuf(GdkPixbuf *pixbuf, EelIRect pixbuf_location,
                                EelIRect probe_rect) {
  ...
  for (x = relative_rect.x0; x < relative_rect.x1; x++) {
    for (y = relative_rect.y0; y < relative_rect.y1; y++) {
      pixel = gdk_pixbuf_get_pixels(pixbuf) +
              y * gdk_pixbuf_get_rowstride(pixbuf) + x * 4;
      if (pixel[3] > 1) {
        return TRUE;
      }
    }
...
static gboolean hit_test(CajaIconCanvasItem *icon_item, EelIRect canvas_rect) {
  CajaIconCanvasItemPrivate *details;
  EelIRect emblem_rect;
  EmblemLayout emblem_layout;
  GdkPixbuf *emblem_pixbuf;
  ...
  is_rtl = caja_icon_container_is_layout_rtl(
      CAJA_ICON_CONTAINER(EEL_CANVAS_ITEM(icon_item)->canvas));

  /* Check for hit in the emblem pixbufs. */
  emblem_layout_reset(&emblem_layout, icon_item,
                      icon_item->details->canvas_rect, is_rtl);
  while (emblem_layout_next(&emblem_layout, &emblem_pixbuf, &emblem_rect,
                            is_rtl)) {
...
static double caja_icon_canvas_item_point(EelCanvasItem *item, double x,
...
static void caja_icon_canvas_item_translate(EelCanvasItem *item, double dx,
                                            double dy) {
  CajaIconCanvasItem *icon_item;
  CajaIconCanvasItemPrivate *details;
...
void caja_icon_canvas_item_get_bounds_for_layout(CajaIconCanvasItem *icon_item,
                                                 double *x1, double *y1,
                                                 double *x2, double *y2) {
  CajaIconCanvasItemPrivate *details;
  EelIRect *total_rect;
...
void caja_icon_canvas_item_get_bounds_for_entire_item(
    CajaIconCanvasItem *icon_item, double *x1, double *y1, double *x2,
    double *y2) {
  CajaIconCanvasItemPrivate *details;
  EelIRect *total_rect;
...
static void caja_icon_canvas_item_bounds(EelCanvasItem *item, double *x1,
...
static void caja_icon_canvas_item_ensure_bounds_up_to_date(
  ...
  if (!details->bounds_cached) {
    ...
    text_rect_for_layout = compute_text_rectangle(icon_item, icon_rect, FALSE,
                                                  BOUNDS_USAGE_FOR_LAYOUT);
    text_rect_for_entire_text = compute_text_rectangle(
        icon_item, icon_rect, FALSE, BOUNDS_USAGE_FOR_ENTIRE_ITEM);

    is_rtl =
        caja_icon_container_is_layout_rtl(CAJA_ICON_CONTAINER(item->canvas));
    ...
    eel_irect_union(&total_rect_for_entire_text, &icon_rect,
                    &text_rect_for_entire_text);
    ...
    while (emblem_layout_next(&emblem_layout, &emblem_pixbuf, &emblem_rect,
      ...
      eel_irect_union(&total_rect_for_layout, &total_rect_for_layout,
                      &emblem_rect);
      eel_irect_union(&total_rect_for_entire_text, &total_rect_for_entire_text,
                      &emblem_rect);
    }
...
EelDRect caja_icon_canvas_item_get_icon_rectangle(
    const CajaIconCanvasItem *item) {
  EelDRect rectangle;
  double pixels_per_unit;
  gint width, height;
  ...
  eel_canvas_item_i2w(EEL_CANVAS_ITEM(item), &rectangle.x0, &rectangle.y0);
  eel_canvas_item_i2w(EEL_CANVAS_ITEM(item), &rectangle.x1, &rectangle.y1);
...
EelDRect caja_icon_canvas_item_get_text_rectangle(CajaIconCanvasItem *item,
  ...
  text_rectangle = compute_text_rectangle(
      item, icon_rectangle, FALSE,
  ...
  eel_canvas_item_i2w(EEL_CANVAS_ITEM(item), &ret.x0, &ret.y0);
  eel_canvas_item_i2w(EEL_CANVAS_ITEM(item), &ret.x1, &ret.y1);
...
static void get_icon_canvas_rectangle(CajaIconCanvasItem *item,
  ...
  eel_canvas_w2c(EEL_CANVAS_ITEM(item)->canvas, item->details->x,
                 item->details->y, &rect->x0, &rect->y0);
...
void caja_icon_canvas_item_set_show_stretch_handles(
    CajaIconCanvasItem *item, gboolean show_stretch_handles) {
  g_return_if_fail(CAJA_IS_ICON_CANVAS_ITEM(item));
  g_return_if_fail(show_stretch_handles == FALSE ||
...
static gboolean hit_test_stretch_handle(CajaIconCanvasItem *item,
  ...
  if (corner) *corner = hit_corner;

  return hit_corner != -1;
}

gboolean caja_icon_canvas_item_hit_test_stretch_handles(
    CajaIconCanvasItem *item, EelDPoint world_point, GtkCornerType *corner) {
  EelIRect canvas_rect;

  g_return_val_if_fail(CAJA_IS_ICON_CANVAS_ITEM(item), FALSE);

  eel_canvas_w2c(EEL_CANVAS_ITEM(item)->canvas, world_point.x, world_point.y,
...
const char *caja_icon_canvas_item_get_editable_text(
    CajaIconCanvasItem *icon_item) {
...
void caja_icon_canvas_item_set_renaming(CajaIconCanvasItem *item,
                                        gboolean state) {
  g_return_if_fail(CAJA_IS_ICON_CANVAS_ITEM(item));
    ...
    if (container->details->label_position == CAJA_ICON_LABEL_POSITION_BESIDE) {
      if (container->details->layout_mode == CAJA_ICON_LAYOUT_T_B_L_R ||
        ...
        if (container->details->all_columns_same_width) {
          return MAX_TEXT_WIDTH_BESIDE_TOP_TO_BOTTOM *
                 canvas_item->canvas->pixels_per_unit;
        } else {
          return -1;
        }
      } else {
...
void caja_icon_canvas_item_set_entire_text(CajaIconCanvasItem *item,
...
static void caja_icon_canvas_item_class_init(CajaIconCanvasItemClass *class) {
  ...
  g_object_class_install_property(
      object_class, PROP_EDITABLE_TEXT,
      g_param_spec_string("editable_text", "editable text",
                          "the editable label", "", G_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_ADDITIONAL_TEXT,
      g_param_spec_string("additional_text", "additional text",
                          "some more text", "", G_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_HIGHLIGHTED_FOR_SELECTION,
      g_param_spec_boolean("highlighted_for_selection",
                           "highlighted for selection",
                           "whether we are highlighted for a selection", FALSE,
                           G_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_HIGHLIGHTED_AS_KEYBOARD_FOCUS,
      g_param_spec_boolean(
          "highlighted_as_keyboard_focus", "highlighted as keyboard focus",
          "whether we are highlighted to render keyboard focus", FALSE,
          G_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_HIGHLIGHTED_FOR_DROP,
      g_param_spec_boolean("highlighted_for_drop", "highlighted for drop",
                           "whether we are highlighted for a D&D drop", FALSE,
                           G_PARAM_READWRITE));

  g_object_class_install_property(
      object_class, PROP_HIGHLIGHTED_FOR_CLIPBOARD,
      g_param_spec_boolean("highlighted_for_clipboard",
                           "highlighted for clipboard",
                           "whether we are highlighted for a clipboard paste "
                           "(after we have been cut)",
  ...
  atk_registry_set_factory_type(
      atk_get_default_registry(), CAJA_TYPE_ICON_CANVAS_ITEM,
...
static const char *caja_icon_canvas_item_accessible_action_descriptions[] = {
    "Open item", "Popup context menu", NULL};

enum { ACTION_OPEN, ACTION_MENU, LAST_ACTION };
...
#define GET_PRIV(o)                                                           \
  G_TYPE_INSTANCE_GET_PRIVATE(o, caja_icon_canvas_item_accessible_get_type(), \
                              CajaIconCanvasItemAccessiblePrivate);

/* accessible AtkAction interface */

static gboolean caja_icon_canvas_item_accessible_idle_do_action(gpointer data) {
  CajaIconCanvasItem *item;
  ...
  while (!g_queue_is_empty(container->details->a11y_item_action_queue)) {
    ...
    switch (action_number) {
      ...
      case ACTION_MENU:
        ...
        g_list_free(selection);
        g_signal_emit_by_name(container, "context_click_selection",
                              &button_event);
        break;
      default:
        g_assert_not_reached();
...
static gboolean caja_icon_canvas_item_accessible_do_action(
    ...
    case ACTION_MENU:
      ...
      if (container->details->a11y_item_action_idle_handler == 0) {
        container->details->a11y_item_action_idle_handler = g_idle_add(
            caja_icon_canvas_item_accessible_idle_do_action, container);
      }
      break;
    default:
      g_warning(
          "Invalid action passed to CajaIconCanvasItemAccessible::do_action");
      return FALSE;
  }
...
static int caja_icon_canvas_item_accessible_get_n_actions(
    AtkAction *accessible) {
  return LAST_ACTION;
}

static const char *caja_icon_canvas_item_accessible_action_get_description(
    AtkAction *accessible, int i) {
...
static const char *caja_icon_canvas_item_accessible_action_get_name(
    AtkAction *accessible, int i) {
  g_assert(i < LAST_ACTION);
...
static const char *caja_icon_canvas_item_accessible_action_get_keybinding(
    AtkAction *accessible, int i) {
  g_assert(i < LAST_ACTION);
...
static gboolean caja_icon_canvas_item_accessible_action_set_description(
...
static void caja_icon_canvas_item_accessible_action_interface_init(
  ...
  iface->get_description =
      caja_icon_canvas_item_accessible_action_get_description;
...
static const gchar *caja_icon_canvas_item_accessible_get_name(
    AtkObject *accessible) {
  CajaIconCanvasItem *item;
...
static const gchar *caja_icon_canvas_item_accessible_get_description(
    AtkObject *accessible) {
  CajaIconCanvasItem *item;
...
static AtkObject *caja_icon_canvas_item_accessible_get_parent(
    AtkObject *accessible) {
  CajaIconCanvasItem *item;
...
static int caja_icon_canvas_item_accessible_get_index_in_parent(
    AtkObject *accessible) {
  CajaIconCanvasItem *item;
  CajaIconContainer *container;
  GList *l;
...
static const gchar *caja_icon_canvas_item_accessible_get_image_description(
    AtkImage *image) {
  CajaIconCanvasItemAccessiblePrivate *priv;
  CajaIconCanvasItem *item;
  char *description;
  ...
  if (priv->image_description) {
    ...
    item = CAJA_ICON_CANVAS_ITEM(
        atk_gobject_accessible_get_object(ATK_GOBJECT_ACCESSIBLE(image)));
    ...
    container = CAJA_ICON_CONTAINER(EEL_CANVAS_ITEM(item)->canvas);
    description =
        caja_icon_container_get_icon_description(container, icon->data);
...
static void caja_icon_canvas_item_accessible_get_image_size(AtkImage *image,
                                                            gint *width,
                                                            gint *height) {
  CajaIconCanvasItem *item;
...
static void caja_icon_canvas_item_accessible_get_image_position(
  ...
  if (!item->details->canvas_rect.x0 && !item->details->canvas_rect.x1) {
    ...
    if (item->details->text_width) {
      itmp = item->details->canvas_rect.x0 - item->details->text_rect.x0;
      if (itmp > x_offset) {
        x_offset = itmp;
      }
      itmp = item->details->canvas_rect.y0 - item->details->text_rect.y0;
      if (itmp > y_offset) {
        y_offset = itmp;
      }
    }
    if (item->details->emblem_pixbufs) {
      itmp = item->details->canvas_rect.x0 - item->details->emblem_rect.x0;
      if (itmp > x_offset) {
        x_offset = itmp;
      }
      itmp = item->details->canvas_rect.y0 - item->details->emblem_rect.y0;
      if (itmp > y_offset) {
        y_offset = itmp;
      }
    }
...
static gboolean caja_icon_canvas_item_accessible_set_image_description(
...
static void caja_icon_canvas_item_accessible_image_interface_init(
    AtkImageIface *iface) {
  iface->get_image_description =
      caja_icon_canvas_item_accessible_get_image_description;
  iface->set_image_description =
  ...
  iface->get_image_position =
      caja_icon_canvas_item_accessible_get_image_position;
}

static gint caja_icon_canvas_item_accessible_get_offset_at_point(
  ...
  atk_component_get_extents(ATK_COMPONENT(text), &real_x, &real_y, &real_width,
                            &real_height, coords);
  ...
  item = CAJA_ICON_CANVAS_ITEM(
      atk_gobject_accessible_get_object(ATK_GOBJECT_ACCESSIBLE(text)));
  ...
  have_additional = item->details->additional_text != NULL &&
  ...
  if (have_editable) {
    ...
    if (y >= editable_height && have_additional) {
      prepare_pango_layout_for_draw(item, editable_layout);
      additional_layout =
          get_label_layout(&item->details->additional_text_layout, item,
  ...
  x += text_offset;
  if (!pango_layout_xy_to_index(layout, x * PANGO_SCALE, y * PANGO_SCALE,
                                &index, NULL)) {
    if (x < 0 || y < 0) {
      index = 0;
    } else {
...
static void caja_icon_canvas_item_accessible_get_character_extents(
    AtkText *text, gint offset, gint *x, gint *y, gint *width, gint *height,
    AtkCoordType coords) {
  gint pos_x, pos_y;
  ...
  atk_component_get_extents(ATK_COMPONENT(text), &pos_x, &pos_y, NULL, NULL,
                            coords);
  item = CAJA_ICON_CANVAS_ITEM(
      atk_gobject_accessible_get_object(ATK_GOBJECT_ACCESSIBLE(text)));
...
static void caja_icon_canvas_item_accessible_text_interface_init(
    AtkTextIface *iface) {
  ...
  iface->get_character_extents =
      caja_icon_canvas_item_accessible_get_character_extents;
  iface->get_offset_at_point =
      caja_icon_canvas_item_accessible_get_offset_at_point;
}
...
G_DEFINE_TYPE_WITH_CODE(
    CajaIconCanvasItemAccessible, caja_icon_canvas_item_accessible,
    eel_canvas_item_accessible_get_type(),
    G_ADD_PRIVATE(CajaIconCanvasItemAccessible) G_IMPLEMENT_INTERFACE(
        ATK_TYPE_IMAGE, caja_icon_canvas_item_accessible_image_interface_init)
        G_IMPLEMENT_INTERFACE(
            ATK_TYPE_TEXT, caja_icon_canvas_item_accessible_text_interface_init)
            G_IMPLEMENT_INTERFACE(
                ATK_TYPE_ACTION,
                caja_icon_canvas_item_accessible_action_interface_init));

static AtkStateSet *caja_icon_canvas_item_accessible_ref_state_set(
    AtkObject *accessible) {
  ...
  state_set = ATK_OBJECT_CLASS(caja_icon_canvas_item_accessible_parent_class)
...
static void caja_icon_canvas_item_accessible_finalize(GObject *object) {
  CajaIconCanvasItemAccessiblePrivate *priv;
  int i;
  ...
  G_OBJECT_CLASS(caja_icon_canvas_item_accessible_parent_class)
      ->finalize(object);
}

static void caja_icon_canvas_item_accessible_initialize(AtkObject *accessible,
                                                        gpointer widget) {
  ATK_OBJECT_CLASS(caja_icon_canvas_item_accessible_parent_class)
      ->initialize(accessible, widget);

  atk_object_set_role(accessible, ATK_ROLE_ICON);
}

static void caja_icon_canvas_item_accessible_class_init(
    CajaIconCanvasItemAccessibleClass *klass) {
  ...
  aclass->get_index_in_parent =
      caja_icon_canvas_item_accessible_get_index_in_parent;
  aclass->ref_state_set = caja_icon_canvas_item_accessible_ref_state_set;
}

static void caja_icon_canvas_item_accessible_init(
    CajaIconCanvasItemAccessible *self) {}
...
G_DEFINE_TYPE(CajaIconCanvasItemAccessibleFactory,
              caja_icon_canvas_item_accessible_factory,
...
static AtkObject *caja_icon_canvas_item_accessible_factory_create_accessible(
  ...
  if (item->details->additional_text) {
    if (item_text->len > 0) g_string_append_c(item_text, ' ');
    g_string_append(item_text, item->details->additional_text);
  }
  item->details->text_util = gail_text_util_new();
  gail_text_util_text_setup(item->details->text_util, item_text->str);
...
static GType caja_icon_canvas_item_accessible_factory_get_accessible_type(
    void) {
  return caja_icon_canvas_item_accessible_get_type();
}

static void caja_icon_canvas_item_accessible_factory_init(
    CajaIconCanvasItemAccessibleFactory *self) {}

static void caja_icon_canvas_item_accessible_factory_class_init(
    CajaIconCanvasItemAccessibleFactoryClass *klass) {
  klass->create_accessible =
      caja_icon_canvas_item_accessible_factory_create_accessible;
  klass->get_accessible_type =
      caja_icon_canvas_item_accessible_factory_get_accessible_type;
}
