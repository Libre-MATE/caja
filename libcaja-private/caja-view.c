...
static void caja_view_base_init(gpointer g_class) {
  static gboolean initialized = FALSE;

  if (!initialized) {
    caja_view_signals[TITLE_CHANGED] =
        g_signal_new("title_changed", CAJA_TYPE_VIEW, G_SIGNAL_RUN_LAST,
                     G_STRUCT_OFFSET(CajaViewIface, title_changed), NULL, NULL,
                     g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

    caja_view_signals[ZOOM_LEVEL_CHANGED] =
        g_signal_new("zoom_level_changed", CAJA_TYPE_VIEW, G_SIGNAL_RUN_LAST,
                     G_STRUCT_OFFSET(CajaViewIface, zoom_level_changed), NULL,
...
GType caja_view_get_type(void) {
  ...
  if (!type) {
    const GTypeInfo info = {sizeof(CajaViewIface),
                            caja_view_base_init,
                            NULL,
                            NULL,
                            NULL,
    ...
    type = g_type_register_static(G_TYPE_INTERFACE, "CajaView", &info, 0);
...
void caja_view_load_location(CajaView *view, const char *location_uri) {
  g_return_if_fail(CAJA_IS_VIEW(view));
  g_return_if_fail(location_uri != NULL);

  (*CAJA_VIEW_GET_IFACE(view)->load_location)(view, location_uri);
}
...
void caja_view_set_selection(CajaView *view, GList *list) {
  g_return_if_fail(CAJA_IS_VIEW(view));

  (*CAJA_VIEW_GET_IFACE(view)->set_selection)(view, list);
}

void caja_view_set_is_active(CajaView *view, gboolean is_active) {
  g_return_if_fail(CAJA_IS_VIEW(view));

  (*CAJA_VIEW_GET_IFACE(view)->set_is_active)(view, is_active);
}
...
void caja_view_scroll_to_file(CajaView *view, const char *uri) {
  g_return_if_fail(CAJA_IS_VIEW(view));
...
void caja_view_bump_zoom_level(CajaView *view, int zoom_increment) {
  g_return_if_fail(CAJA_IS_VIEW(view));

  (*CAJA_VIEW_GET_IFACE(view)->bump_zoom_level)(view, zoom_increment);
}

void caja_view_zoom_to_level(CajaView *view, CajaZoomLevel level) {
  g_return_if_fail(CAJA_IS_VIEW(view));

  (*CAJA_VIEW_GET_IFACE(view)->zoom_to_level)(view, level);
}
...
void caja_view_pop_up_location_context_menu(CajaView *view,
  ...
  if (CAJA_VIEW_GET_IFACE(view)->pop_up_location_context_menu != NULL) {
    (*CAJA_VIEW_GET_IFACE(view)->pop_up_location_context_menu)(view, event,
                                                               location);
  }
}

void caja_view_drop_proxy_received_uris(CajaView *view, GList *uris,
                                        const char *target_location,
                                        GdkDragAction action) {
  g_return_if_fail(CAJA_IS_VIEW(view));

  if (CAJA_VIEW_GET_IFACE(view)->drop_proxy_received_uris != NULL) {
    (*CAJA_VIEW_GET_IFACE(view)->drop_proxy_received_uris)(
        view, uris, target_location, action);
  }
}
...
void caja_view_drop_proxy_received_netscape_url(CajaView *view,
  ...
  if (CAJA_VIEW_GET_IFACE(view)->drop_proxy_received_netscape_url != NULL) {
    (*CAJA_VIEW_GET_IFACE(view)->drop_proxy_received_netscape_url)(
        view, source_url, target_location, action);
  }
}
