...
char *caja_compute_title_for_location(GFile *location) {
  char *title;
...
char *caja_get_user_directory(void) {
   ...
   */
  char *user_directory =
      g_build_filename(g_get_user_config_dir(), "caja", NULL);
    ...
    if (g_chmod(user_directory, DEFAULT_CAJA_DIRECTORY_MODE) != 0) {
      GtkWidget *dialog =
          gtk_message_dialog_new(NULL, GTK_DIALOG_DESTROY_WITH_PARENT,
                                 ...
                                 "The path for the directory containing caja "
                                 "settings need read and write permissions: %s",
                                 user_directory);
...
static XdgDirEntry *parse_xdg_dirs(const char *config_file) {
  GArray *array;
  ...
  if (config_file == NULL) {
    config_file_free =
        g_build_filename(g_get_user_config_dir(), "user-dirs.dirs", NULL);
  ...
  if (g_file_get_contents(config_file, &data, NULL, NULL)) {
    ...
    for (i = 0; lines[i] != NULL; i++) {
      ...
      *value++ = 0;
      ...
      if (!g_str_has_prefix(p, "XDG_")) continue;
      if (!g_str_has_suffix(p, "_DIR")) continue;
      type_start = p + 4;
...
static void xdg_dir_changed(CajaFile *file, XdgDirEntry *dir) {
...
static void xdg_dir_cache_changed_cb(GFileMonitor *monitor, GFile *file,
                                     GFile *other_file,
...
static gboolean emit_user_dirs_changed_idle(gpointer data) {
  g_signal_emit_by_name(caja_signaller_get_current(), "user_dirs_changed");
...
static void unschedule_user_dirs_changed(void) {
  if (user_dirs_changed_tag != 0) {
...
static void free_xdg_dir_cache(void) {
  if (cached_xdg_dirs != NULL) {
    int i;

    for (i = 0; cached_xdg_dirs[i].type != NULL; i++) {
      if (cached_xdg_dirs[i].file != NULL) {
        caja_file_monitor_remove(cached_xdg_dirs[i].file, &cached_xdg_dirs[i]);
...
static void destroy_xdg_dir_cache(void) {
  free_xdg_dir_cache();
...
static void update_xdg_dir_cache(void) {
  ...
  for (i = 0; cached_xdg_dirs[i].type != NULL; i++) {
    ...
    if (strcmp(cached_xdg_dirs[i].path, g_get_home_dir()) != 0) {
      ...
      caja_file_monitor_add(cached_xdg_dirs[i].file, &cached_xdg_dirs[i],
                            CAJA_FILE_ATTRIBUTE_INFO);
      g_signal_connect(cached_xdg_dirs[i].file, "changed",
                       G_CALLBACK(xdg_dir_changed), &cached_xdg_dirs[i]);
      g_free(uri);
    }
  }
...
static void desktop_dir_changed_callback(gpointer callback_data) {
  desktop_dir_changed();
}

static void update_desktop_dir(void) {
  char *path;
  char *dirname;
...
gboolean caja_is_home_directory_file(GFile *dir, const char *filename) {
...
gboolean caja_is_desktop_directory_file(GFile *dir, const char *file) {
  if (!desktop_dir_changed_callback_installed) {
    g_signal_connect_swapped(caja_preferences,
                             "changed::" CAJA_PREFERENCES_DESKTOP_IS_HOME_DIR,
                             G_CALLBACK(desktop_dir_changed_callback), NULL);
...
char *caja_pixmap_file(const char *partial_path) {
  ...
  if (g_file_test(path, G_FILE_TEST_EXISTS)) {
    ...
    g_debug(
        "Failed to locate \"%s\" in Caja pixmap path \"%s\". Incomplete "
        "installation?",
        partial_path, tmp);
    g_free(tmp);
  }
...
char *caja_ensure_unique_file_name(const char *directory_uri,
  ...
  filename = g_strdup_printf("%s%s", base_name, extension);
  child = g_file_get_child(dir, filename);
  g_free(filename);

  copy = 1;
  while ((info = g_file_query_info(child, G_FILE_ATTRIBUTE_STANDARD_TYPE, 0,
                                   NULL, NULL)) != NULL) {
    g_object_unref(info);
    g_object_unref(child);

    filename = g_strdup_printf("%s-%d%s", base_name, copy, extension);
    child = g_file_get_child(dir, filename);
    g_free(filename);
 ...
 */
int caja_inhibit_power_manager(const char *message) {
  ...
  result = g_dbus_connection_call_sync(
      connection, GSM_NAME, GSM_PATH, GSM_INTERFACE, "Inhibit",
      g_variant_new("(susu)", "Caja", (guint)0, message,
                    (guint)(INHIBIT_LOGOUT | INHIBIT_SUSPEND)),
      G_VARIANT_TYPE("(u)"), G_DBUS_CALL_FLAGS_NO_AUTO_START, -1, NULL, &error);
 ...
 */
void caja_uninhibit_power_manager(gint cookie) {
  GDBusConnection *connection;
  GVariant *result;
  GError *error = NULL;
  ...
  result = g_dbus_connection_call_sync(
      connection, GSM_NAME, GSM_PATH, GSM_INTERFACE, "Uninhibit",
      g_variant_new("(u)", (guint)cookie), NULL,
...
gboolean caja_is_in_desktop_dir(GFile *file) {
  ...
  path = g_file_get_path(file);
  dirname = g_path_get_dirname(path);
  if (g_strcmp0(dirname, g_get_user_special_dir(G_USER_DIRECTORY_DESKTOP)) ==
      0) {
    res = TRUE;
  }
...
GHashTable *caja_trashed_files_get_original_directories(
    GList *files, GList **unhandled_files) {
    ...
    if (original_dir != NULL) {
      if (directories == NULL) {
        directories = g_hash_table_new_full(
            g_direct_hash, g_direct_equal, (GDestroyNotify)caja_file_unref,
...
void caja_restore_files_from_trash(GList *files, GtkWindow *parent_window) {
  ...
  original_dirs_hash =
      caja_trashed_files_get_original_directories(files, &unhandled_files);
  ...
  for (l = unhandled_files; l != NULL; l = l->next) {
    ...
    eel_show_warning_dialog(
        message, _("The item cannot be restored from trash"), parent_window);
    g_free(message);
  }
  ...
  if (original_dirs_hash != NULL) {
    ...
    for (l = original_dirs; l != NULL; l = l->next) {
      ...
      caja_file_operations_move(locations, NULL, original_dir_location,
...
char *caja_get_filesystem_id_by_location(GFile *location, gboolean follow) {
  ...
  info = g_file_query_info(location, G_FILE_ATTRIBUTE_ID_FILESYSTEM, flags,
                           NULL, NULL);
  if (info) {
    if (g_file_info_has_attribute(info, G_FILE_ATTRIBUTE_ID_FILESYSTEM)) {
      filesystem_id = g_strdup(g_file_info_get_attribute_string(
...
void caja_self_check_file_utilities(void) {}
