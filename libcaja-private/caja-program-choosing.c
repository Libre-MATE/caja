 ...
 *  */
static void application_cannot_open_location(GAppInfo *application,
                                             CajaFile *file,
  ...
  if (caja_mime_has_any_applications_for_file(file)) {
    ...
    if (application != NULL) {
      ...
      message =
          g_strdup_printf(_("\"%s\" cannot open \"%s\" because \"%s\" cannot "
                            "access files at \"%s\" "
                            "locations."),
      ...
      message = g_strdup_printf(_("The default action cannot open \"%s\" "
                                  "because it cannot access files at \"%s\" "
                                  "locations."),
                                file_name, uri_scheme);
    }

    message_dialog = eel_show_yes_no_dialog(prompt, message, "gtk-ok",
    ...
    if (response == GTK_RESPONSE_YES) {
      ...
      caja_choose_application_for_file(
          file, parent_window, launch_application_callback, launch_parameters);
    }
    g_free(message);
  } else {
    if (application != NULL) {
      prompt =
          g_strdup_printf(_("\"%s\" cannot open \"%s\" because \"%s\" cannot "
                          ...
                          g_app_info_get_display_name(application), uri_scheme);
      message =
          _("No other applications are available to view this file. "
            "If you copy this file onto your computer, you may be able to open "
            "it.");
    } else {
      prompt = g_strdup_printf(_("The default action cannot open \"%s\" "
                                 "because it cannot access files at \"%s\" "
                                 "locations."),
                               file_name, uri_scheme);
      message =
          _("No other actions are available to view this file. "
            "If you copy this file onto your computer, you may be able to open "
            "it.");
    }
 ...
 * @uris: The files whose locations should be passed as a parameter to the
 * application.
 * @parent_window: A window to use as the parent for any error dialogs.
 */
void caja_launch_application(GAppInfo *application, GList *files,
                             GtkWindow *parent_window) {
  GList *uris, *l;
  ...
  caja_launch_application_by_uri(application, uris, parent_window);
...
static void gather_pid_callback(GDesktopAppInfo *appinfo, GPid pid,
                                gpointer data) {
  g_child_watch_add(pid, dummy_child_watch, NULL);
}

void caja_launch_application_by_uri(GAppInfo *application, GList *uris,
                                    GtkWindow *parent_window) {
  CajaFile *file;
  gboolean result;
  GError *error;
  ...
  icon = caja_file_get_icon(
      file, 48, gtk_widget_get_scale_factor(GTK_WIDGET(parent_window)), 0);
  ...
  result = g_desktop_app_info_launch_uris_as_manager(
      G_DESKTOP_APP_INFO(application), uris,
      G_APP_LAUNCH_CONTEXT(launch_context),
      G_SPAWN_SEARCH_PATH | G_SPAWN_DO_NOT_REAP_CHILD, NULL, NULL,
      gather_pid_callback, application, &error);
  ...
  if (!result) {
    if (error->domain == G_IO_ERROR &&
      ...
      application_cannot_open_location(application, file, uri_scheme,
                                       parent_window);
      g_free(uri_scheme);
    } else {
#ifdef NEW_MIME_COMPLETE
 ...
 */
void caja_launch_application_from_command(GdkScreen *screen, const char *name,
  ...
  if (use_terminal) {
    ...
    app_info = g_app_info_create_from_commandline(full_command, NULL,
 ...
 * @command_string: The application to be launched, with any desired
 * command-line options.
 * @parameters: Passed as parameters to the application after quoting each of
 * them.
 */
...
void caja_launch_desktop_file(GdkScreen *screen, const char *desktop_file_uri,
  ...
  if (!g_file_is_native(desktop_file)) {
    ...
    eel_show_error_dialog(_("Sorry, but you cannot execute commands from "
                            "a remote site."),
  ...
  context = gdk_display_get_app_launch_context(
      gtk_widget_get_display(GTK_WIDGET(parent_window)));

  /* TODO: Ideally we should accept a timestamp here instead of using
   * GDK_CURRENT_TIME */
  gdk_app_launch_context_set_timestamp(context, GDK_CURRENT_TIME);
  gdk_app_launch_context_set_screen(context,
                                    gtk_window_get_screen(parent_window));
  g_desktop_app_info_launch_uris_as_manager(
      app_info, (GList *)parameter_uris, G_APP_LAUNCH_CONTEXT(context),
      G_SPAWN_SEARCH_PATH | G_SPAWN_DO_NOT_REAP_CHILD, NULL, NULL,
      gather_pid_callback, app_info, &error);
  if (error != NULL) {
    char *message;

    message = g_strconcat(_("Details: "), error->message, NULL);
    eel_show_error_dialog(_("There was an error launching the application."),
                          message, parent_window);
