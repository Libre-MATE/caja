...
void caja_drag_init(CajaDragInfo *drag_info, const GtkTargetEntry *drag_types,
                    int drag_type_count, gboolean add_text_targets) {
...
static void drag_selection_item_destroy(CajaDragSelectionItem *item) {
  g_free(item->uri);
  g_free(item);
}

void caja_drag_destroy_selection_list(GList *list) {
  GList *p;

  if (list == NULL) return;

  for (p = list; p != NULL; p = p->next) drag_selection_item_destroy(p->data);
...
GList *caja_drag_build_selection_list(GtkSelectionData *data) {
  ...
  while (size > 0) {
    ...
    if (*p == '\n' || *p == '\0') {
      ...
      if (p == 0) {
        g_warning(
            "Invalid x-special/mate-icon-list data received: "
    ...
    item->got_icon_position =
        sscanf(p, "%d:%d:%d:%d%*s", &item->icon_x, &item->icon_y,
...
static gboolean caja_drag_file_local_internal(const char *target_uri_string,
...
gboolean caja_drag_items_local(const char *target_uri_string,
                               const GList *selection_list) {
  /* must have at least one item */
  g_assert(selection_list);

  return caja_drag_file_local_internal(
      target_uri_string, ((CajaDragSelectionItem *)selection_list->data)->uri);
}
...
GdkDragAction caja_drag_default_drop_action_for_netscape_url(
...
static gboolean check_same_fs(const char *target_uri, CajaFile *target_file,
                              const char *dropped_uri, CajaFile *dropped_file) {
  gboolean result;
...
void caja_drag_default_drop_action_for_icons(GdkDragContext *context,
  ...
  same_fs =
      check_same_fs(target_uri_string, target_file, dropped_uri, dropped_file);
...
GdkDragAction caja_drag_default_drop_action_for_uri_list(
    GdkDragContext *context, const char *target_uri_string) {
  if (eel_uri_is_trash(target_uri_string) &&
...
*/
static void add_one_mate_icon(const char *uri, int x, int y, int w, int h,
                              gpointer data) {
  GString *result;
...
static gboolean is_path_that_mate_uri_list_extract_filenames_can_parse(
    const char *path) {
   ...
   * of these.
   */
  if (strchr(path, '#') != NULL || strchr(path, '\r') != NULL ||
      strchr(path, '\n') != NULL) {
    return FALSE;
  }
 ...
 */
static void add_one_compatible_uri(const char *uri, int x, int y, int w, int h,
                                   gpointer data) {
  GString *result;
...
gboolean caja_drag_drag_data_get(
  ...
  switch (info) {
    case CAJA_ICON_DND_MATE_ICON_LIST:
      ...
      (*each_selected_item_iterator)(add_one_mate_icon, container_context,
                                     result);
      break;
...
static void menu_deactivate_callback(GtkWidget *menu, gpointer data) {
  ...
  if (g_main_loop_is_running(damd->loop)) g_main_loop_quit(damd->loop);
}
...
GdkDragAction caja_drag_drop_action_ask(GtkWidget *widget,
  ...
  append_drop_action_menu_item(
      menu, _("Set as _Background"), CAJA_DND_ACTION_SET_AS_BACKGROUND,
      (actions & CAJA_DND_ACTION_SET_AS_BACKGROUND) != 0, &damd);
...
GdkDragAction caja_drag_drop_background_ask(GtkWidget *widget,
  ...
  append_drop_action_menu_item(
      menu, _("Set as background for _all folders"),
      CAJA_DND_ACTION_SET_AS_GLOBAL_BACKGROUND,
      (actions & CAJA_DND_ACTION_SET_AS_GLOBAL_BACKGROUND) != 0, &damd);

  append_drop_action_menu_item(
      menu, _("Set as background for _this folder"),
      CAJA_DND_ACTION_SET_AS_FOLDER_BACKGROUND,
      (actions & CAJA_DND_ACTION_SET_AS_FOLDER_BACKGROUND) != 0, &damd);
...
void caja_drag_autoscroll_start(CajaDragInfo *drag_info, GtkWidget *widget,
  ...
  if (drag_info->have_data && drag_info->have_valid_data) {
    if (drag_info->info == CAJA_ICON_DND_MATE_ICON_LIST) {
      caja_drag_default_drop_action_for_icons(
          context, target_uri, drag_info->data.selection_list, &action);
...
static void drag_info_clear(CajaDragSlotProxyInfo *drag_info) {
  if (!drag_info->have_data) {
    goto out;
  }
...
static void slot_proxy_handle_drop(GtkWidget *widget, GdkDragContext *context,
  ...
  if (target_slot != NULL && target_view != NULL) {
    if (drag_info->info == CAJA_ICON_DND_MATE_ICON_LIST) {
      ...
      uri_list = caja_drag_uri_list_from_selection_list(
          drag_info->data.selection_list);
      g_assert(uri_list != NULL);

      caja_view_drop_proxy_received_uris(
    ...
    if (drag_info->info == CAJA_ICON_DND_NETSCAPE_URL) {
      caja_view_drop_proxy_received_netscape_url(
          target_view, drag_info->data.netscape_url, target_uri,
          gdk_drag_context_get_selected_action(context));
    }
...
static void slot_proxy_drag_data_received(GtkWidget *widget,
  ...
  if (info == CAJA_ICON_DND_MATE_ICON_LIST) {
    ...
    drag_info->data.netscape_url =
        g_strdup((char *)gtk_selection_data_get_data(data));
