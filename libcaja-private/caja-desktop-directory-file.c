...
static void caja_desktop_directory_file_init(gpointer object, gpointer klass);
static void caja_desktop_directory_file_class_init(gpointer klass);

EEL_CLASS_BOILERPLATE(CajaDesktopDirectoryFile, caja_desktop_directory_file,
                      CAJA_TYPE_FILE)

static guint desktop_callback_hash(gconstpointer desktop_callback_as_pointer) {
...
static gboolean desktop_callback_equal(
    gconstpointer desktop_callback_as_pointer,
...
static void real_file_changed_callback(CajaFile *real_file,
                                       gpointer callback_data) {
  CajaDesktopDirectoryFile *desktop_file;
...
static CajaFileAttributes get_delegated_attributes_mask(void) {
...
static void partition_attributes(CajaFileAttributes attributes,
                                 CajaFileAttributes *delegated_attributes,
                                 CajaFileAttributes *non_delegated_attributes) {
...
static void desktop_directory_file_monitor_add(CajaFile *file,
  ...
  if (monitor != NULL) {
    ...
    g_hash_table_insert(desktop_file->details->monitors, (gpointer)client,
                        monitor);
  }

  partition_attributes(attributes, &monitor->delegated_attributes,
                       &monitor->non_delegated_attributes);

  /* Pawn off partioned attributes to real dir file */
  caja_file_monitor_add(desktop_file->details->real_dir_file, monitor,
                        monitor->delegated_attributes);

  /* Do the rest ourself */
  caja_directory_monitor_add_internal(file->details->directory, file, client,
                                      TRUE, monitor->non_delegated_attributes,
                                      NULL, NULL);
}

static void desktop_directory_file_monitor_remove(CajaFile *file,
                                                  gconstpointer client) {
  CajaDesktopDirectoryFile *desktop_file;
  DesktopMonitor *monitor;
  ...
  caja_directory_monitor_remove_internal(file->details->directory, file,
                                         client);
}

static void desktop_callback_destroy(DesktopCallback *desktop_callback) {
...
static void desktop_callback_check_done(DesktopCallback *desktop_callback) {
  ...
  caja_desktop_update_metadata_from_keyfile(
      CAJA_FILE(desktop_callback->desktop_file), "directory");
...
static void desktop_callback_remove_file(DesktopCallback *desktop_callback,
                                         CajaFile *file) {
  desktop_callback->non_ready_files =
      g_list_remove(desktop_callback->non_ready_files, file);
  desktop_callback_check_done(desktop_callback);
}

static void ready_callback(CajaFile *file, gpointer callback_data) {
...
static void desktop_directory_file_call_when_ready(
    CajaFile *file, CajaFileAttributes attributes, CajaFileCallback callback,
    gpointer callback_data)

{
  ...
  if (g_hash_table_lookup(desktop_file->details->callbacks, &search_key) !=
      NULL) {
  ...
  partition_attributes(attributes, &desktop_callback->delegated_attributes,
                       &desktop_callback->non_delegated_attributes);

  desktop_callback->non_ready_files =
      g_list_prepend(desktop_callback->non_ready_files, file);
  desktop_callback->non_ready_files = g_list_prepend(
      desktop_callback->non_ready_files, desktop_file->details->real_dir_file);

  /* Put it in the hash table. */
  g_hash_table_insert(desktop_file->details->callbacks, desktop_callback,
                      desktop_callback);

  /* Now connect to each file's call_when_ready. */
  caja_directory_call_when_ready_internal(
      file->details->directory, file,
      desktop_callback->non_delegated_attributes, FALSE, NULL, ready_callback,
      desktop_callback);
  caja_file_call_when_ready(desktop_file->details->real_dir_file,
...
static void desktop_directory_file_cancel_call_when_ready(
    CajaFile *file, CajaFileCallback callback, gpointer callback_data) {
  CajaDesktopDirectoryFile *desktop_file;
  DesktopCallback search_key, *desktop_callback;
  ...
  search_key.callback_data = callback_data;
  desktop_callback =
      g_hash_table_lookup(desktop_file->details->callbacks, &search_key);
  ...
  caja_directory_cancel_callback_internal(file->details->directory, file, NULL,
                                          ready_callback, desktop_callback);

  caja_file_cancel_call_when_ready(desktop_file->details->real_dir_file,
                                   ready_callback, desktop_callback);

  desktop_callback_destroy(desktop_callback);
}

static gboolean real_check_if_ready(CajaFile *file,
                                    CajaFileAttributes attributes) {
  return caja_directory_check_if_ready_internal(file->details->directory, file,
                                                attributes);
}

static gboolean desktop_directory_file_check_if_ready(
    CajaFile *file, CajaFileAttributes attributes) {
  CajaFileAttributes delegated_attributes, non_delegated_attributes;
  CajaDesktopDirectoryFile *desktop_file;

  desktop_file = CAJA_DESKTOP_DIRECTORY_FILE(file);

  partition_attributes(attributes, &delegated_attributes,
...
static gboolean desktop_directory_file_get_item_count(
    CajaFile *file, guint *count, gboolean *count_unreadable) {
  CajaDesktopDirectoryFile *desktop_file;
  gboolean got_count;

  desktop_file = CAJA_DESKTOP_DIRECTORY_FILE(file);

  got_count = caja_file_get_directory_item_count(
      desktop_file->details->real_dir_file, count, count_unreadable);
...
static CajaRequestStatus desktop_directory_file_get_deep_counts(
  ...
  status = caja_file_get_deep_counts(
      desktop_file->details->real_dir_file, directory_count, file_count,
      unreadable_directory_count, total_size, total_size_on_disk, TRUE);
...
static gboolean desktop_directory_file_get_date(CajaFile *file,
  ...
  return caja_file_get_date(desktop_file->details->real_dir_file, date_type,
                            date);
}

static char *desktop_directory_file_get_where_string(CajaFile *file) {
  return g_strdup(_("on the desktop"));
}

static void monitor_destroy(gpointer data) {
...
static void caja_desktop_directory_file_set_metadata(CajaFile *file,
                                                     const char *key,
                                                     const char *value) {
  caja_desktop_set_metadata_string(file, "directory", key, value);
}

static void caja_desktop_directory_file_set_metadata_as_list(CajaFile *file,
                                                             const char *key,
                                                             char **value) {
  caja_desktop_set_metadata_stringv(file, "directory", key,
                                    (const gchar **)value);
}
...
static void caja_desktop_directory_file_init(gpointer object, gpointer klass) {
  ...
  desktop_directory =
      CAJA_DESKTOP_DIRECTORY(caja_directory_get_by_uri(EEL_DESKTOP_URI));
  ...
  desktop_file->details->callbacks =
      g_hash_table_new(desktop_callback_hash, desktop_callback_equal);
  desktop_file->details->monitors =
      g_hash_table_new_full(NULL, NULL, NULL, monitor_destroy);
  ...
  g_signal_connect_object(real_dir_file, "changed",
                          G_CALLBACK(real_file_changed_callback), desktop_file,
                          0);
}

static void desktop_callback_remove_file_cover(gpointer key, gpointer value,
                                               gpointer callback_data) {
  desktop_callback_remove_file(value, CAJA_FILE(callback_data));
}
...
static void desktop_finalize(GObject *object) {
  ...
  /* Todo: ghash now safe? */
  eel_g_hash_table_safe_for_each(desktop_file->details->callbacks,
  ...
  if (g_hash_table_size(desktop_file->details->callbacks) != 0) {
    g_warning(
        "call_when_ready still pending when desktop virtual file is destroyed");
  }
...
static void caja_desktop_directory_file_class_init(gpointer klass) {
  ...
  file_class->cancel_call_when_ready =
      desktop_directory_file_cancel_call_when_ready;
  ...
  file_class->set_metadata_as_list =
      caja_desktop_directory_file_set_metadata_as_list;
}
