...
G_DEFINE_TYPE(CajaSearchDirectory, caja_search_directory, CAJA_TYPE_DIRECTORY);

static void search_engine_hits_added(CajaSearchEngine *engine, GList *hits,
                                     CajaSearchDirectory *search);
static void search_engine_hits_subtracted(CajaSearchEngine *engine, GList *hits,
                                          CajaSearchDirectory *search);
static void search_engine_finished(CajaSearchEngine *engine,
                                   CajaSearchDirectory *search);
static void search_engine_error(CajaSearchEngine *engine, const char *error,
                                CajaSearchDirectory *search);
...
static void ensure_search_engine(CajaSearchDirectory *search) {
  if (!search->details->engine) {
    search->details->engine = caja_search_engine_new();
    g_signal_connect(search->details->engine, "hits-added",
                     G_CALLBACK(search_engine_hits_added), search);
    g_signal_connect(search->details->engine, "hits-subtracted",
                     G_CALLBACK(search_engine_hits_subtracted), search);
    g_signal_connect(search->details->engine, "finished",
                     G_CALLBACK(search_engine_finished), search);
    g_signal_connect(search->details->engine, "error",
                     G_CALLBACK(search_engine_error), search);
  }
}

static void reset_file_list(CajaSearchDirectory *search) {
...
static void start_or_stop_search_engine(CajaSearchDirectory *search,
                                        gboolean adding) {
  if (adding &&
      (search->details->monitor_list ||
       search->details->pending_callback_list) &&
      search->details->query && !search->details->search_running) {
    ...
    caja_search_engine_set_query(search->details->engine,
    ...
    caja_search_engine_start(search->details->engine);
             ...
             !search->details->pending_callback_list &&
             search->details->engine && search->details->search_running) {
...
static void search_monitor_add(CajaDirectory *directory, gconstpointer client,
                               gboolean monitor_hidden_files,
  ...
  search->details->monitor_list =
      g_list_prepend(search->details->monitor_list, monitor);
...
static void search_monitor_remove_file_monitors(SearchMonitor *monitor,
                                                CajaSearchDirectory *search) {
  GList *list;
  CajaFile *file = NULL;
...
static void search_monitor_destroy(SearchMonitor *monitor,
                                   CajaSearchDirectory *search) {
...
static void search_monitor_remove(CajaDirectory *directory,
                                  gconstpointer client) {
  CajaSearchDirectory *search;
  GList *list;
  ...
  for (list = search->details->monitor_list; list != NULL; list = list->next) {
    ...
    if (monitor->client == client) {
      search->details->monitor_list =
          g_list_delete_link(search->details->monitor_list, list);
...
static void cancel_call_when_ready(gpointer key, gpointer value,
                                   gpointer user_data) {
...
static void search_callback_destroy(SearchCallback *search_callback) {
  if (search_callback->non_ready_hash) {
    g_hash_table_foreach(search_callback->non_ready_hash,
                         cancel_call_when_ready, search_callback);
...
static void search_callback_invoke_and_destroy(
  ...
  search_callback->search_directory->details->callback_list =
      g_list_remove(search_callback->search_directory->details->callback_list,
                    search_callback);
...
static void search_callback_add_file_callbacks(SearchCallback *callback) {
  ...
  for (list = file_list_copy; list != NULL; list = list->next) {
    ...
    caja_file_call_when_ready(file, callback->wait_for_attributes,
                              search_callback_file_ready_callback, callback);
  }
  g_list_free(file_list_copy);
}

static SearchCallback *search_callback_find(CajaSearchDirectory *search,
                                            CajaDirectoryCallback callback,
                                            gpointer callback_data) {
  GList *list;
...
static SearchCallback *search_callback_find_pending(
    CajaSearchDirectory *search, CajaDirectoryCallback callback,
    gpointer callback_data) {
  GList *list;
  SearchCallback *search_callback = NULL;

  for (list = search->details->pending_callback_list; list != NULL;
       list = list->next) {
    search_callback = list->data;
...
static GHashTable *file_list_to_hash_table(GList *file_list) {
  GList *list;
  GHashTable *table;
...
static void search_call_when_ready(CajaDirectory *directory,
                                   CajaFileAttributes file_attributes,
                                   gboolean wait_for_file_list,
                                   CajaDirectoryCallback callback,
                                   gpointer callback_data) {
  ...
  if (search_callback == NULL) {
    search_callback =
        search_callback_find_pending(search, callback, callback_data);
  }
  ...
  if (wait_for_file_list && !search->details->search_finished) {
    ...
    search_callback->non_ready_hash =
        file_list_to_hash_table(search->details->files);
    ...
    if (!search_callback->non_ready_hash) {
      ...
      */
      ...
      search->details->callback_list =
          g_list_prepend(search->details->callback_list, search_callback);
      search_callback_add_file_callbacks(search_callback);
    }
  }
}

static void search_cancel_callback(CajaDirectory *directory,
                                   CajaDirectoryCallback callback,
                                   gpointer callback_data) {
  CajaSearchDirectory *search;
  SearchCallback *search_callback;
  ...
  if (search_callback) {
    search->details->callback_list =
        g_list_remove(search->details->callback_list, search_callback);
  ...
  if (search_callback) {
    search->details->pending_callback_list =
        g_list_remove(search->details->pending_callback_list, search_callback);
...
static void search_callback_add_pending_file_callbacks(
    SearchCallback *callback) {
  callback->file_list =
      caja_file_list_copy(callback->search_directory->details->files);
  callback->non_ready_hash =
      file_list_to_hash_table(callback->search_directory->details->files);

  search_callback_add_file_callbacks(callback);
}

static void search_engine_error(CajaSearchEngine *engine,
                                const char *error_message,
                                CajaSearchDirectory *search) {
  GError *error;

  error = g_error_new_literal(G_IO_ERROR, G_IO_ERROR_FAILED, error_message);
  caja_directory_emit_load_error(CAJA_DIRECTORY(search), error);
  g_error_free(error);
}
...
static void search_engine_finished(CajaSearchEngine *engine,
  ...
  search->details->callback_list = g_list_concat(
      search->details->callback_list, search->details->pending_callback_list);

  g_list_free(search->details->pending_callback_list);
  search->details->pending_callback_list = NULL;
}

static void search_force_reload(CajaDirectory *directory) {
  CajaSearchDirectory *search;
...
static gboolean search_are_all_files_seen(CajaDirectory *directory) {
  CajaSearchDirectory *search;

  search = CAJA_SEARCH_DIRECTORY(directory);

  return (!search->details->query || search->details->search_finished);
}

static gboolean search_contains_file(CajaDirectory *directory, CajaFile *file) {
  CajaSearchDirectory *search;
...
static GList *search_get_file_list(CajaDirectory *directory) {
  CajaSearchDirectory *search;
...
static gboolean search_is_editable(CajaDirectory *directory) { return FALSE; }
...
static void search_dispose(GObject *object) {
  ...
  if (search->details->monitor_list) {
    for (list = search->details->monitor_list; list != NULL;
         list = list->next) {
...
static void caja_search_directory_init(CajaSearchDirectory *search) {
  search->details = g_new0(CajaSearchDirectoryDetails, 1);
}

static void caja_search_directory_class_init(CajaSearchDirectoryClass *class) {
...
void caja_search_directory_set_query(CajaSearchDirectory *search,
  ...
  if (as_file != NULL) {
    caja_search_directory_file_update_display_name(
        CAJA_SEARCH_DIRECTORY_FILE(as_file));
  }
}
...
CajaSearchDirectory *caja_search_directory_new_from_saved_search(
  ...
  search =
      CAJA_SEARCH_DIRECTORY(g_object_new(CAJA_TYPE_SEARCH_DIRECTORY, NULL));
...
void caja_search_directory_save_search(CajaSearchDirectory *search) {
  ...
  caja_search_directory_save_to_file(search, search->details->saved_search_uri);
  search->details->modified = FALSE;
}
