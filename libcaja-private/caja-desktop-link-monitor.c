...
static void caja_desktop_link_monitor_init(gpointer object, gpointer klass);
static void caja_desktop_link_monitor_class_init(gpointer klass);

EEL_CLASS_BOILERPLATE(CajaDesktopLinkMonitor, caja_desktop_link_monitor,
                      G_TYPE_OBJECT)

static CajaDesktopLinkMonitor *the_link_monitor = NULL;

static void destroy_desktop_link_monitor(void) {
...
static gboolean volume_file_name_used(CajaDesktopLinkMonitor *monitor,
                                      const char *name) {
  GList *l;
  gboolean same;
...
static gboolean has_mount(CajaDesktopLinkMonitor *monitor, GMount *mount) {
  gboolean ret;
  GList *l;
...
static void create_mount_link(CajaDesktopLinkMonitor *monitor, GMount *mount) {
  if (has_mount(monitor, mount)) return;

  if ((!g_mount_is_shadowed(mount)) &&
      g_settings_get_boolean(caja_desktop_preferences,
                             CAJA_PREFERENCES_DESKTOP_VOLUMES_VISIBLE)) {
    CajaDesktopLink *link;

    link = caja_desktop_link_new_from_mount(mount);
    monitor->details->mount_links =
        g_list_prepend(monitor->details->mount_links, link);
  }
}

static void remove_mount_link(CajaDesktopLinkMonitor *monitor, GMount *mount) {
  GList *l;
  CajaDesktopLink *link;
  GMount *other_mount = NULL;
  ...
  if (link) {
    monitor->details->mount_links =
        g_list_remove(monitor->details->mount_links, link);
    g_object_unref(link);
  }
}
...
static void update_link_visibility(CajaDesktopLinkMonitor *monitor,
...
static void desktop_home_visible_changed(gpointer callback_data) {
  CajaDesktopLinkMonitor *monitor;

  monitor = CAJA_DESKTOP_LINK_MONITOR(callback_data);

  update_link_visibility(CAJA_DESKTOP_LINK_MONITOR(monitor),
                         &monitor->details->home_link, CAJA_DESKTOP_LINK_HOME,
                         CAJA_PREFERENCES_DESKTOP_HOME_VISIBLE);
}

static void desktop_computer_visible_changed(gpointer callback_data) {
  CajaDesktopLinkMonitor *monitor;
...
static void desktop_trash_visible_changed(gpointer callback_data) {
  CajaDesktopLinkMonitor *monitor;

  monitor = CAJA_DESKTOP_LINK_MONITOR(callback_data);

  update_link_visibility(CAJA_DESKTOP_LINK_MONITOR(callback_data),
                         &monitor->details->trash_link, CAJA_DESKTOP_LINK_TRASH,
                         CAJA_PREFERENCES_DESKTOP_TRASH_VISIBLE);
}

static void desktop_network_visible_changed(gpointer callback_data) {
  CajaDesktopLinkMonitor *monitor;

  monitor = CAJA_DESKTOP_LINK_MONITOR(callback_data);

  update_link_visibility(
      CAJA_DESKTOP_LINK_MONITOR(callback_data), &monitor->details->network_link,
      CAJA_DESKTOP_LINK_NETWORK, CAJA_PREFERENCES_DESKTOP_NETWORK_VISIBLE);
}

static void desktop_volumes_visible_changed(gpointer callback_data) {
  CajaDesktopLinkMonitor *monitor;
  GList *l, *mounts;

  monitor = CAJA_DESKTOP_LINK_MONITOR(callback_data);

  if (g_settings_get_boolean(caja_desktop_preferences,
                             CAJA_PREFERENCES_DESKTOP_VOLUMES_VISIBLE)) {
...
static void create_link_and_add_preference(CajaDesktopLink **link_ref,
                                           CajaDesktopLinkType link_type,
                                           const char *preference_key,
                                           GCallback callback,
  ...
  g_signal_connect_swapped(caja_desktop_preferences, detailed_signal, callback,
                           callback_data);
...
static void caja_desktop_link_monitor_init(gpointer object, gpointer klass) {
  ...
  create_link_and_add_preference(
      &monitor->details->home_link, CAJA_DESKTOP_LINK_HOME,
      CAJA_PREFERENCES_DESKTOP_HOME_VISIBLE,
      G_CALLBACK(desktop_home_visible_changed), monitor);

  create_link_and_add_preference(
      &monitor->details->computer_link, CAJA_DESKTOP_LINK_COMPUTER,
      CAJA_PREFERENCES_DESKTOP_COMPUTER_VISIBLE,
      G_CALLBACK(desktop_computer_visible_changed), monitor);

  create_link_and_add_preference(
      &monitor->details->trash_link, CAJA_DESKTOP_LINK_TRASH,
      CAJA_PREFERENCES_DESKTOP_TRASH_VISIBLE,
      G_CALLBACK(desktop_trash_visible_changed), monitor);

  create_link_and_add_preference(
      &monitor->details->network_link, CAJA_DESKTOP_LINK_NETWORK,
...
static void remove_link_and_preference(CajaDesktopLink **link_ref,
                                       const char *preference_key,
                                       GCallback callback,
                                       gpointer callback_data) {
  if (*link_ref != NULL) {
  ...
  g_signal_handlers_disconnect_by_func(caja_desktop_preferences, callback,
                                       callback_data);
}

static void desktop_link_monitor_finalize(GObject *object) {
  CajaDesktopLinkMonitor *monitor;
  ...
  remove_link_and_preference(
      &monitor->details->trash_link, CAJA_PREFERENCES_DESKTOP_TRASH_VISIBLE,
      G_CALLBACK(desktop_trash_visible_changed), monitor);

  remove_link_and_preference(
      &monitor->details->network_link, CAJA_PREFERENCES_DESKTOP_NETWORK_VISIBLE,
      G_CALLBACK(desktop_network_visible_changed), monitor);
  ...
  g_signal_handlers_disconnect_by_func(
      caja_desktop_preferences, desktop_volumes_visible_changed, monitor);
