...
/* CajaDirectory is a class that manages the model for a directory,
   real or virtual, for Caja, mainly the file-manager component. The directory
   is responsible for managing both real data and cached metadata. On top of the
   file system independence provided by gio, the directory object also provides:
...
typedef void (*CajaDirectoryCallback)(CajaDirectory *directory, GList *files,
...
typedef struct {
   ...
   * discovers new files.
   */
  void (*files_added)(CajaDirectory *directory, GList *added_files);
   ...
   */
  void (*files_changed)(CajaDirectory *directory, GList *changed_files);
  ...
  void (*load_error)(CajaDirectory *directory, GError *error);

  /*** Virtual functions for subclasses to override. ***/
  gboolean (*contains_file)(CajaDirectory *directory, CajaFile *file);
  ...
  void (*cancel_callback)(CajaDirectory *directory,
                          CajaDirectoryCallback callback,
                          gpointer callback_data);
  void (*file_monitor_add)(CajaDirectory *directory, gconstpointer client,
                           gboolean monitor_hidden_files,
                           CajaFileAttributes monitor_attributes,
                           CajaDirectoryCallback initial_files_callback,
                           gpointer callback_data);
  void (*file_monitor_remove)(CajaDirectory *directory, gconstpointer client);
...
gboolean caja_directory_contains_file(CajaDirectory *directory, CajaFile *file);
...
void caja_directory_file_monitor_add(
    CajaDirectory *directory, gconstpointer client,
    gboolean monitor_hidden_files, CajaFileAttributes attributes,
    CajaDirectoryCallback initial_files_callback, gpointer callback_data);
...
GList *caja_directory_match_pattern(CajaDirectory *directory, const char *glob);
...
/* Convenience functions for dealing with a list of CajaDirectory objects that
