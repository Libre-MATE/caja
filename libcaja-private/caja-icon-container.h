...
#define CAJA_ICON_CONTAINER(obj)                               \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), CAJA_TYPE_ICON_CONTAINER, \
                              CajaIconContainer))
#define CAJA_ICON_CONTAINER_CLASS(klass)                      \
  (G_TYPE_CHECK_CLASS_CAST((klass), CAJA_TYPE_ICON_CONTAINER, \
...
#define CAJA_ICON_CONTAINER_GET_CLASS(obj)                    \
  (G_TYPE_INSTANCE_GET_CLASS((obj), CAJA_TYPE_ICON_CONTAINER, \
                             CajaIconContainerClass))

#define CAJA_ICON_CONTAINER_ICON_DATA(pointer) ((CajaIconData *)(pointer))
...
typedef struct {
  ...
  int (*button_press)(CajaIconContainer *container, GdkEventButton *event);
  void (*context_click_background)(CajaIconContainer *container,
                                   GdkEventButton *event);
  void (*middle_click)(CajaIconContainer *container, GdkEventButton *event);

  /* Operations on icons. */
  void (*activate)(CajaIconContainer *container, CajaIconData *data);
  void (*activate_alternate)(CajaIconContainer *container, CajaIconData *data);
  void (*context_click_selection)(CajaIconContainer *container,
                                  GdkEventButton *event);
  void (*move_copy_items)(CajaIconContainer *container, const GList *item_uris,
                          GdkPoint *relative_item_points,
                          const char *target_uri, GdkDragAction action, int x,
                          int y);
  void (*handle_netscape_url)(CajaIconContainer *container, const char *url,
                              const char *target_uri, GdkDragAction action,
                              int x, int y);
  void (*handle_uri_list)(CajaIconContainer *container, const char *uri_list,
                          const char *target_uri, GdkDragAction action, int x,
                          int y);
  void (*handle_text)(CajaIconContainer *container, const char *text,
                      const char *target_uri, GdkDragAction action, int x,
                      int y);
  void (*handle_raw)(CajaIconContainer *container, char *raw_data, int length,
  ...
  CajaIconInfo *(*get_icon_images)(CajaIconContainer *container,
                                   CajaIconData *data, int icon_size,
                                   GList **emblem_pixbufs, char **embedded_text,
                                   gboolean for_drag_accept,
                                   gboolean need_large_embeddded_text,
                                   gboolean *embedded_text_needs_loading,
                                   gboolean *has_window_open);
  void (*get_icon_text)(CajaIconContainer *container, CajaIconData *data,
                        char **editable_text, char **additional_text,
                        gboolean include_invisible);
  char *(*get_icon_description)(CajaIconContainer *container,
                                CajaIconData *data);
  int (*compare_icons)(CajaIconContainer *container, CajaIconData *icon_a,
                       CajaIconData *icon_b);
  int (*compare_icons_by_name)(CajaIconContainer *container,
                               CajaIconData *icon_a, CajaIconData *icon_b);
  ...
  void (*start_monitor_top_left)(CajaIconContainer *container,
                                 CajaIconData *data, gconstpointer client,
                                 gboolean large_text);
  ...
  gboolean (*can_accept_item)(CajaIconContainer *container,
                              CajaIconData *target, const char *item_uri);
  gboolean (*get_stored_icon_position)(CajaIconContainer *container,
                                       CajaIconData *data,
                                       CajaIconPosition *position);
  char *(*get_icon_uri)(CajaIconContainer *container, CajaIconData *data);
  char *(*get_icon_drop_target_uri)(CajaIconContainer *container,
                                    CajaIconData *data);

  /* If icon data is NULL, the layout timestamp of the container should be
   * retrieved. That is the time when the container displayed a fully loaded
   * directory with all icon positions assigned.
   *
   * If icon data is not NULL, the position timestamp of the icon should be
   * retrieved. That is the time when the file (i.e. icon data payload) was last
   * displayed in a fully loaded directory with all icon positions assigned.
   */
  gboolean (*get_stored_layout_timestamp)(CajaIconContainer *container,
                                          CajaIconData *data, time_t *time);
  /* If icon data is NULL, the layout timestamp of the container should be
   * stored. If icon data is not NULL, the position timestamp of the container
   * should be stored.
   */
  gboolean (*store_layout_timestamp)(CajaIconContainer *container,
                                     CajaIconData *data, const time_t *time);
  ...
  void (*icon_text_changed)(CajaIconContainer *container, CajaIconData *data,
                            const char *text);
  ...
  void (*icon_stretch_started)(CajaIconContainer *container,
                               CajaIconData *data);
  void (*icon_stretch_ended)(CajaIconContainer *container, CajaIconData *data);
  int (*preview)(CajaIconContainer *container, CajaIconData *data,
                 gboolean start_flag);
  void (*icon_added)(CajaIconContainer *container, CajaIconData *data);
  void (*icon_removed)(CajaIconContainer *container, CajaIconData *data);
...
gboolean caja_icon_container_add(CajaIconContainer *view, CajaIconData *data);
...
#define CANVAS_WIDTH(container, allocation)              \
  ((allocation.width - container->details->left_margin - \
    container->details->right_margin) /                  \
   EEL_CANVAS(container)->pixels_per_unit)

#define CANVAS_HEIGHT(container, allocation)             \
  ((allocation.height - container->details->top_margin - \
