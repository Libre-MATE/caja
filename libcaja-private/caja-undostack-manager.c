...
struct _CajaUndoStackManagerPrivate {
  ...
  guint index;
  GMutex
      mutex; /* Used to protect access to stack (because of async file ops) */
 ...
 ***************************************************************** */
enum { PROP_UNDOSTACK_MANAGER_0, PROP_UNDO_LEVELS };

static void caja_undostack_manager_set_property(GObject *object, guint prop_id,
                                                const GValue *value,
                                                GParamSpec *pspec);

static void caja_undostack_manager_get_property(GObject *object, guint prop_id,
                                                GValue *value,
                                                GParamSpec *pspec);
...
static CajaUndoStackActionData *stack_scroll_left(
    CajaUndoStackManagerPrivate *priv);

static CajaUndoStackActionData *stack_scroll_right(
    CajaUndoStackManagerPrivate *priv);

static CajaUndoStackActionData *get_next_redo_action(
    CajaUndoStackManagerPrivate *priv);

static CajaUndoStackActionData *get_next_undo_action(
    CajaUndoStackManagerPrivate *priv);
...
static gchar *get_first_target_short_name(CajaUndoStackActionData *action);
...
/* *****************************************************************
 Base functions
 ***************************************************************** */
static void caja_undostack_manager_class_init(
    CajaUndoStackManagerClass *klass) {
  GParamSpec *undo_levels;
  GObjectClass *g_object_class;

  /* Create properties */
  undo_levels = g_param_spec_uint(
      "undo-levels", "undo levels", "Number of undo levels to be stored", 1,
      UINT_MAX, 30, G_PARAM_READWRITE | G_PARAM_CONSTRUCT);
  ...
  g_signal_new("request-menu-update", G_TYPE_FROM_CLASS(klass),
...
static void caja_undostack_manager_init(CajaUndoStackManager *self) {
  CajaUndoStackManagerPrivate *priv;
  ...
  if (action != NULL) {
    ...
    switch (action->type) {
      case CAJA_UNDOSTACK_COPY: {
        ...
        caja_file_operations_copy(uris, NULL, action->dest_dir, NULL,
      ...
      case CAJA_UNDOSTACK_CREATEFILEFROMTEMPLATE: {
        ...
        caja_file_operations_new_file_from_template(
            NULL, NULL, puri, new_name, action->template,
      ...
      case CAJA_UNDOSTACK_DUPLICATE: {
        ...
        uris = construct_gfile_list(action->sources, action->src_dir);
        caja_file_operations_duplicate(
            uris, NULL, NULL, undo_redo_done_transfer_callback, action);
        g_list_free_full(uris, g_object_unref);
        break;
      }
      ...
      case CAJA_UNDOSTACK_MOVE: {
        ...
        uris = construct_gfile_list(action->sources, action->src_dir);
        caja_file_operations_move(uris, NULL, action->dest_dir, NULL,
                                  undo_redo_done_transfer_callback, action);
        g_list_free_full(uris, g_object_unref);
        break;
      }
      case CAJA_UNDOSTACK_RENAME: {
        ...
        caja_file_rename(file, new_name, undo_redo_done_rename_callback,
                         action);
        g_object_unref(file);
        g_free(new_name);
        break;
      ...
      case CAJA_UNDOSTACK_CREATEEMPTYFILE: {
        ...
        puri = get_uri_parent(action->target_uri);
        new_name = get_uri_basename(action->target_uri);
        caja_file_operations_new_file(NULL, NULL, puri, new_name,
                                      action->template, 0,
                                      undo_redo_done_create_callback, action);
        g_free(puri);
        g_free(new_name);
      ...
      case CAJA_UNDOSTACK_CREATELINK: {
        ...
        uris = construct_gfile_list(action->sources, action->src_dir);
        caja_file_operations_link(uris, NULL, action->dest_dir, NULL,
                                  undo_redo_done_transfer_callback, action);
        g_list_free_full(uris, g_object_unref);
        break;
      }
      case CAJA_UNDOSTACK_SETPERMISSIONS: {
        ...
        caja_file_set_permissions(file, action->new_permissions,
      ...
      case CAJA_UNDOSTACK_RECURSIVESETPERMISSIONS: {
        ...
        caja_file_set_permissions_recursive(
            puri, action->file_permissions, action->file_mask,
            action->dir_permissions, action->dir_mask, undo_redo_op_callback,
            action);
        g_free(puri);
        break;
      }
      case CAJA_UNDOSTACK_CHANGEGROUP: {
        ...
        file = caja_file_get_by_uri(action->target_uri);
        caja_file_set_group(file, action->new_group_name_or_id,
                            undo_redo_done_rename_callback, action);
        g_object_unref(file);
        break;
      }
      case CAJA_UNDOSTACK_CHANGEOWNER: {
        ...
        file = caja_file_get_by_uri(action->target_uri);
        caja_file_set_owner(file, action->new_user_name_or_id,
                            undo_redo_done_rename_callback, action);
        g_object_unref(file);
        break;
      }
  ...
  if (action != NULL) {
    ...
    switch (action->type) {
      ...
      case CAJA_UNDOSTACK_CREATELINK:
        ...
        caja_file_operations_delete(uris, NULL, undo_redo_done_delete_callback,
                                    action);
        g_list_free_full(uris, g_object_unref);
      ...
      case CAJA_UNDOSTACK_MOVETOTRASH: {
        ...
        if (g_hash_table_size(files_to_restore) > 0) {
          ...
          for (l = gfiles_in_trash; l != NULL; l = l->next) {
            ...
            g_file_move(item, dest, G_FILE_COPY_NOFOLLOW_SYMLINKS, NULL, NULL,
                        NULL, NULL);
            g_object_unref(dest);
          }
      ...
      case CAJA_UNDOSTACK_MOVE:
        uris = construct_gfile_list(action->destinations, action->dest_dir);
        caja_file_operations_move(uris, NULL, action->src_dir, NULL,
                                  undo_redo_done_transfer_callback, action);
        g_list_free_full(uris, g_object_unref);
        break;
      case CAJA_UNDOSTACK_RENAME: {
      ...
      case CAJA_UNDOSTACK_SETPERMISSIONS: {
        ...
        file = caja_file_get_by_uri(action->target_uri);
        caja_file_set_permissions(file, action->current_permissions,
                                  undo_redo_done_rename_callback, action);
        g_object_unref(file);
        break;
      }
      ...
      case CAJA_UNDOSTACK_CHANGEGROUP: {
        ...
        file = caja_file_get_by_uri(action->target_uri);
        caja_file_set_group(file, action->original_group_name_or_id,
                            undo_redo_done_rename_callback, action);
        g_object_unref(file);
        break;
      }
      case CAJA_UNDOSTACK_CHANGEOWNER: {
        ...
        file = caja_file_get_by_uri(action->target_uri);
        caja_file_set_owner(file, action->original_user_name_or_id,
                            undo_redo_done_rename_callback, action);
        g_object_unref(file);
        break;
      }
...
static GList *get_all_trashed_items(GQueue *stack) {
  CajaUndoStackActionData *action = NULL;
  GList *trash = NULL;
  GList *l;
  GQueue *tmp_stack = g_queue_copy(stack);

  while ((action = (CajaUndoStackActionData *)g_queue_pop_tail(tmp_stack)) !=
         NULL)
    if (action->trashed)
...
static gboolean is_destination_uri_action_partof_trashed(GList *trash,
                                                         GList *g) {
  GList *l;
  char *uri;
...
CajaUndoStackActionData *caja_undostack_manager_data_new(
  ...
  CajaUndoStackActionData *data = g_slice_new0(CajaUndoStackActionData);
  data->type = type;
...
/** ****************************************************************
 * Pushes an trashed file with modification time in an existing undo data
 *container
 ** ****************************************************************/
...
static void clear_redo_actions(CajaUndoStackManagerPrivate *priv) {
  while (priv->index > 0) {
    CajaUndoStackActionData *head =
        (CajaUndoStackActionData *)g_queue_pop_head(priv->stack);
    ...
    if (action->undo_description == NULL) {
      ...
      switch (action->type) {
        ...
        case CAJA_UNDOSTACK_MOVE:
          if (count != 1) {
            description =
                g_strdup_printf(_("Move %d items back to '%s'"), count, source);
          } else {
            gchar *name = get_first_target_short_name(action);
            description =
        ...
        case CAJA_UNDOSTACK_RECURSIVESETPERMISSIONS: {
          ...
          description = g_strdup_printf(
              _("Restore original permissions of items enclosed in '%s'"),
              name);
          g_free(name);
        } break;
        case CAJA_UNDOSTACK_SETPERMISSIONS: {
        ...
        case CAJA_UNDOSTACK_CHANGEGROUP: {
          char *name = get_uri_basename(action->target_uri);
          description =
              g_strdup_printf(_("Restore group of '%s' to '%s'"), name,
    ...
    if (action->redo_description == NULL) {
      ...
      switch (action->type) {
        case CAJA_UNDOSTACK_COPY:
          if (count != 1) {
            description =
                g_strdup_printf(_("Copy %d items to '%s'"), count, destination);
          } else {
            gchar *name = get_first_target_short_name(action);
            description =
        ...
        case CAJA_UNDOSTACK_DUPLICATE:
          if (count != 1) {
            description = g_strdup_printf(_("Duplicate of %d items in '%s'"),
                                          count, destination);
          } else {
            gchar *name = get_first_target_short_name(action);
            description =
        ...
        case CAJA_UNDOSTACK_MOVE:
          if (count != 1) {
            description =
                g_strdup_printf(_("Move %d items to '%s'"), count, destination);
          } else {
            gchar *name = get_first_target_short_name(action);
            description =
        ...
        case CAJA_UNDOSTACK_RECURSIVESETPERMISSIONS: {
          char *name = g_file_get_path(action->dest_dir);
          description = g_strdup_printf(
              _("Set permissions of items enclosed in '%s'"), name);
          g_free(name);
        } break;
        case CAJA_UNDOSTACK_SETPERMISSIONS: {
          char *name = get_uri_basename(action->target_uri);
        ...
        case CAJA_UNDOSTACK_CHANGEGROUP: {
          char *name = get_uri_basename(action->target_uri);
          description = g_strdup_printf(_("Set group of '%s' to '%s'"), name,
                                        action->new_group_name_or_id);
          g_free(name);
        } break;
        case CAJA_UNDOSTACK_CHANGEOWNER: {
          char *name = get_uri_basename(action->target_uri);
          description = g_strdup_printf(_("Set owner of '%s' to '%s'"), name,
      ...
      switch (action->type) {
        case CAJA_UNDOSTACK_COPY:
          label = g_strdup_printf(ngettext("_Undo copy of %d item",
                                           "_Undo copy of %d items", count),
                                  count);
          break;
        case CAJA_UNDOSTACK_DUPLICATE:
          label =
              g_strdup_printf(ngettext("_Undo duplicate of %d item",
                                       "_Undo duplicate of %d items", count),
                              count);
          break;
        case CAJA_UNDOSTACK_MOVE:
          label = g_strdup_printf(ngettext("_Undo move of %d item",
                                           "_Undo move of %d items", count),
                                  count);
          break;
        case CAJA_UNDOSTACK_RENAME:
          label = g_strdup_printf(ngettext("_Undo rename of %d item",
                                           "_Undo rename of %d items", count),
                                  count);
          break;
        case CAJA_UNDOSTACK_CREATEEMPTYFILE:
        ...
        case CAJA_UNDOSTACK_CREATEFOLDER:
          label =
              g_strdup_printf(ngettext("_Undo creation of %d folder",
                                       "_Undo creation of %d folders", count),
                              count);
          break;
        case CAJA_UNDOSTACK_MOVETOTRASH:
          label = g_strdup_printf(
              ngettext("_Undo move to trash of %d item",
                       "_Undo move to trash of %d items", count),
              count);
          break;
        case CAJA_UNDOSTACK_RESTOREFROMTRASH:
          label = g_strdup_printf(
              ngettext("_Undo restore from trash of %d item",
                       "_Undo restore from trash of %d items", count),
              count);
          break;
        case CAJA_UNDOSTACK_CREATELINK:
          label =
              g_strdup_printf(ngettext("_Undo create link to %d item",
                                       "_Undo create link to %d items", count),
                              count);
          break;
        case CAJA_UNDOSTACK_DELETE:
          label = g_strdup_printf(ngettext("_Undo delete of %d item",
                                           "_Undo delete of %d items", count),
                                  count);
          break;
        case CAJA_UNDOSTACK_RECURSIVESETPERMISSIONS:
          label = g_strdup_printf(
              ngettext("Undo recursive change permissions of %d item",
                       "Undo recursive change permissions of %d items", count),
              count);
          break;
        case CAJA_UNDOSTACK_SETPERMISSIONS:
          label = g_strdup_printf(
              ngettext("Undo change permissions of %d item",
                       "Undo change permissions of %d items", count),
              count);
          break;
        case CAJA_UNDOSTACK_CHANGEGROUP:
          label =
              g_strdup_printf(ngettext("Undo change group of %d item",
                                       "Undo change group of %d items", count),
                              count);
          break;
        case CAJA_UNDOSTACK_CHANGEOWNER:
          label =
              g_strdup_printf(ngettext("Undo change owner of %d item",
                                       "Undo change owner of %d items", count),
                              count);
          break;
        default:
          break;
      ...
      switch (action->type) {
        case CAJA_UNDOSTACK_COPY:
          label = g_strdup_printf(ngettext("_Redo copy of %d item",
                                           "_Redo copy of %d items", count),
                                  count);
          break;
        case CAJA_UNDOSTACK_DUPLICATE:
          label =
              g_strdup_printf(ngettext("_Redo duplicate of %d item",
                                       "_Redo duplicate of %d items", count),
                              count);
          break;
        case CAJA_UNDOSTACK_MOVE:
          label = g_strdup_printf(ngettext("_Redo move of %d item",
                                           "_Redo move of %d items", count),
                                  count);
          break;
        case CAJA_UNDOSTACK_RENAME:
          label = g_strdup_printf(ngettext("_Redo rename of %d item",
                                           "_Redo rename of %d items", count),
                                  count);
          break;
        case CAJA_UNDOSTACK_CREATEEMPTYFILE:
        ...
        case CAJA_UNDOSTACK_CREATEFOLDER:
          label =
              g_strdup_printf(ngettext("_Redo creation of %d folder",
                                       "_Redo creation of %d folders", count),
                              count);
          break;
        case CAJA_UNDOSTACK_MOVETOTRASH:
          label = g_strdup_printf(
              ngettext("_Redo move to trash of %d item",
                       "_Redo move to trash of %d items", count),
              count);
          break;
        case CAJA_UNDOSTACK_RESTOREFROMTRASH:
          label = g_strdup_printf(
              ngettext("_Redo restore from trash of %d item",
                       "_Redo restore from trash of %d items", count),
              count);
          break;
        case CAJA_UNDOSTACK_CREATELINK:
          label =
              g_strdup_printf(ngettext("_Redo create link to %d item",
                                       "_Redo create link to %d items", count),
                              count);
          break;
        case CAJA_UNDOSTACK_DELETE:
          label = g_strdup_printf(ngettext("_Redo delete of %d item",
                                           "_Redo delete of %d items", count),
                                  count);
          break;
        case CAJA_UNDOSTACK_RECURSIVESETPERMISSIONS:
          label = g_strdup_printf(
              ngettext("Redo recursive change permissions of %d item",
                       "Redo recursive change permissions of %d items", count),
              count);
          break;
        case CAJA_UNDOSTACK_SETPERMISSIONS:
          label = g_strdup_printf(
              ngettext("Redo change permissions of %d item",
                       "Redo change permissions of %d items", count),
              count);
          break;
        case CAJA_UNDOSTACK_CHANGEGROUP:
          label =
              g_strdup_printf(ngettext("Redo change group of %d item",
                                       "Redo change group of %d items", count),
                              count);
          break;
        case CAJA_UNDOSTACK_CHANGEOWNER:
          label =
              g_strdup_printf(ngettext("Redo change owner of %d item",
                                       "Redo change owner of %d items", count),
                              count);
          break;
        default:
          break;
...
static void free_undostack_action(gpointer data, gpointer user_data) {
  ...
  if (action->src_dir) g_object_unref(action->src_dir);
  if (action->dest_dir) g_object_unref(action->dest_dir);

  if (action) g_slice_free(CajaUndoStackActionData, action);
}
...
static CajaUndoStackActionData *get_next_redo_action(
  ...
  CajaUndoStackActionData *action =
      g_queue_peek_nth(priv->stack, priv->index - 1);
...
static CajaUndoStackActionData *get_next_undo_action(
  ...
  CajaUndoStackActionData *action = g_queue_peek_nth(priv->stack, priv->index);
...
static GHashTable *retrieve_files_to_restore(GHashTable *trashed) {
  ...
  GHashTable *to_restore = g_hash_table_new_full(g_direct_hash, g_direct_equal,
  ...
  if (enumerator) {
    ...
    while ((info = g_file_enumerator_next_file(enumerator, NULL, NULL)) !=
      ...
      const char *origpath = g_file_info_get_attribute_byte_string(
          info, G_FILE_ATTRIBUTE_TRASH_ORIG_PATH);
      if (origpath == NULL) {
        g_warning(
            "The item cannot be restored from trash: could not determine "
            "original location");
        continue;
      }
      ...
      if (lookupvalue) {
        ...
        guint64 mtime_item = g_file_info_get_attribute_uint64(
            info, G_FILE_ATTRIBUTE_TIME_MODIFIED);
        if (*mtime == mtime_item) {
          GFile *item = g_file_get_child(
              trash, g_file_info_get_name(info)); /* File in the trash */
