...
enum { CHANGED, PROGRESS_CHANGED, STARTED, FINISHED, LAST_SIGNAL };
...
static void caja_progress_info_finalize(GObject *object) {
...
static void caja_progress_info_dispose(GObject *object) {
  CajaProgressInfo *info;
...
static void caja_progress_info_class_init(CajaProgressInfoClass *klass) {
  ...
  signals[CHANGED] =
      g_signal_new("changed", CAJA_TYPE_PROGRESS_INFO, G_SIGNAL_RUN_LAST, 0,
                   NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  signals[PROGRESS_CHANGED] = g_signal_new(
      "progress-changed", CAJA_TYPE_PROGRESS_INFO, G_SIGNAL_RUN_LAST, 0, NULL,
      NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  signals[STARTED] =
      g_signal_new("started", CAJA_TYPE_PROGRESS_INFO, G_SIGNAL_RUN_LAST, 0,
                   NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

  signals[FINISHED] =
      g_signal_new("finished", CAJA_TYPE_PROGRESS_INFO, G_SIGNAL_RUN_LAST, 0,
                   NULL, NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
}
...
static void status_icon_activate_cb(GtkStatusIcon *icon,
...
static GtkWidget *get_progress_window(void) {
  static GtkWidget *progress_window = NULL;
  GtkWidget *vbox;

  if (progress_window != NULL) return progress_window;
  ...
  gtk_window_set_title(GTK_WINDOW(progress_window), _("File Operations"));

  gtk_window_set_position(GTK_WINDOW(progress_window), GTK_WIN_POS_CENTER);

  gtk_window_set_icon_name(GTK_WINDOW(progress_window), "system-file-manager");
  ...
  g_signal_connect(progress_window, "delete_event", (GCallback)delete_event,
                   NULL);

  status_icon = gtk_status_icon_new_from_icon_name("system-file-manager");
  g_signal_connect(status_icon, "activate", (GCallback)status_icon_activate_cb,
                   progress_window);
...
static gboolean is_op_paused(ProgressWidgetState state) {
...
static void progress_widget_data_free(ProgressWidgetData *data) {
  g_object_unref(data->info);
  g_free(data);
}

static void update_data(ProgressWidgetData *data) {
...
static GtkWidget *get_widgets_container(void) {
  GtkWidget *window = get_progress_window();
  return gtk_bin_get_child(GTK_BIN(window));
}
static void foreach_get_running_operations(GtkWidget *widget, int *n) {
  ProgressWidgetData *data =
      (ProgressWidgetData *)g_object_get_data(G_OBJECT(widget), "data");

  if (!is_op_paused(data->state)) (*n)++;
}

static int get_running_operations(void) {
...
static void foreach_get_queued_widget(GtkWidget *widget, GtkWidget **out) {
  if (*out == NULL) {
    ProgressWidgetData *data;

    data = (ProgressWidgetData *)g_object_get_data(G_OBJECT(widget), "data");
...
static GtkWidget *get_first_queued_widget(void) {
  GtkWidget *container = get_widgets_container();
  GtkWidget *out = NULL;
...
static void start_button_update_view(ProgressWidgetData *data) {
  ...
  if (state == STATE_RUNNING || state == STATE_QUEUING) {
    toapply = g_object_get_data(G_OBJECT(button), STARTBT_DATA_IMAGE_PAUSE);
    atk_object_set_name(gtk_widget_get_accessible(button), _("Pause"));
    gtk_widget_set_tooltip_text(button, _("Pause"));
    as_pause = TRUE;
  } else {
    toapply = g_object_get_data(G_OBJECT(button), STARTBT_DATA_IMAGE_RESUME);
  ...
  if (curimage != toapply) {
    if (curimage != NULL) gtk_container_remove(GTK_CONTAINER(button), curimage);
...
static void queue_button_update_view(ProgressWidgetData *data) {
...
static void progress_info_set_waiting(CajaProgressInfo *info,
                                      gboolean waiting) {
  G_LOCK(progress_info);
...
static void widget_reposition_as_queued(GtkWidget *widget) {
  gtk_box_reorder_child(GTK_BOX(get_widgets_container()), widget,
                        n_progress_ops - 1);
}

/* Reposition the widget so that it sits right before the first stopped widget
 */
static void widget_reposition_as_paused(GtkWidget *widget) {
  int i;
  ...
  for (child = children; child && !abort; child = child->next) {
    data =
        (ProgressWidgetData *)g_object_get_data(G_OBJECT(child->data), "data");
  ...
  gtk_box_reorder_child(GTK_BOX(container), widget, i);
}

/* Reposition the widget so that it sits right after the last running widget */
static void widget_reposition_as_running(GtkWidget *widget) {
  GList *children, *child;
  ProgressWidgetData *data = NULL;
  gboolean abort = FALSE;
  int i, mypos = -1;
...
static void widget_state_transit_to(ProgressWidgetData *data,
                                    ProgressWidgetState newstate) {
  data->state = newstate;

  if (newstate == STATE_PAUSING || newstate == STATE_QUEUING ||
...
static void update_queue(void) {
  if (get_running_operations() == 0) {
    ...
    if (next != NULL) {
      ...
      data = (ProgressWidgetData *)g_object_get_data(G_OBJECT(next), "data");
      widget_state_transit_to(data, STATE_RUNNING);
    }
  }
}

static void update_progress(ProgressWidgetData *data) {
  double progress;
...
static void update_status_icon_and_window(void) {
  ...
  tooltip =
      g_strdup_printf(ngettext("%'d file operation active",
                               "%'d file operations active", n_progress_ops),
                      n_progress_ops);
  ...
  if (!toshow) {
    ...
    if (window_shown) {
      if (g_settings_get_boolean(caja_preferences,
        ...
        notification = notify_notification_new("caja", _("Process completed"),
                                               "system-file-manager");
...
static void op_finished(ProgressWidgetData *data) {
...
static int do_disable_pause(CajaProgressInfo *info) {
...
static void cancel_clicked(GtkWidget *button, ProgressWidgetData *data) {
  caja_progress_info_cancel(data->info);
  gtk_widget_set_sensitive(button, FALSE);
  do_disable_pause(data->info);
}

static void progress_widget_invalid_state(ProgressWidgetData *data) {
  // TODO give more info: current state, buttons
  g_warning("Invalid ProgressWidgetState");
}

static int widget_state_notify_paused_callback(ProgressWidgetData *data) {
  if (data != NULL) {
  ...
  if (info->waiting) {
    ...
    if (info->waiting) {
      ...
      g_source_set_callback(source,
                            (GSourceFunc)widget_state_notify_paused_callback,
                            info->widget, NULL);
      g_source_attach(source, NULL);
...
static void start_clicked(GtkWidget *startbt, ProgressWidgetData *data) {
  switch (data->state) {
    case STATE_RUNNING:
    case STATE_QUEUING:
...
static void queue_clicked(GtkWidget *queuebt, ProgressWidgetData *data) {
  switch (data->state) {
    case STATE_RUNNING:
    case STATE_PAUSING:
...
static void unref_callback(gpointer data) { g_object_unref(data); }

static void start_button_init(ProgressWidgetData *data) {
  GtkWidget *pauseImage, *resumeImage;
  GtkWidget *button = gtk_button_new();
  data->btstart = button;

  pauseImage = gtk_image_new_from_icon_name("media-playback-pause",
                                            GTK_ICON_SIZE_BUTTON);
  resumeImage = gtk_image_new_from_icon_name("media-playback-start",
                                             GTK_ICON_SIZE_BUTTON);

  g_object_ref(pauseImage);
  g_object_ref(resumeImage);

  g_object_set_data_full(G_OBJECT(button), STARTBT_DATA_IMAGE_PAUSE, pauseImage,
                         unref_callback);
...
static void queue_button_init(ProgressWidgetData *data) {
...
static GtkWidget *progress_widget_new(CajaProgressInfo *info) {
  ProgressWidgetData *data;
  ...
  data->widget = vbox;
  g_object_set_data_full(G_OBJECT(data->widget), "data", data,
  ...
  gtk_box_pack_start(GTK_BOX(box), progress_bar, TRUE, FALSE, 0);

  start_button_init(data);
  queue_button_init(data);

  gtk_box_pack_start(GTK_BOX(hbox), btcancel, FALSE, FALSE, 0);
  gtk_box_pack_start(GTK_BOX(hbox), box, TRUE, TRUE, 0);
  gtk_box_pack_start(GTK_BOX(hbox), data->btstart, FALSE, FALSE, 0);
  gtk_box_pack_start(GTK_BOX(hbox), data->btqueue, FALSE, FALSE, 0);
  ...
  g_signal_connect_swapped(data->info, "progress_changed",
                           (GCallback)update_progress, data);
  g_signal_connect_swapped(data->info, "finished", (GCallback)op_finished,
                           data);
...
static void handle_new_progress_info(CajaProgressInfo *info) {
  GtkWidget *window, *progress;

  window = get_progress_window();

  progress = progress_widget_new(info);
  gtk_box_pack_start(GTK_BOX(gtk_bin_get_child(GTK_BIN(window))), progress,
                     FALSE, FALSE, 6);
...
static gboolean delayed_window_showup(CajaProgressInfo *info) {
...
static void new_op_started(CajaProgressInfo *info) {
  ...
  if (!caja_progress_info_get_is_finished(info)) {
    ...
    if (info->waiting) {
      if (get_running_operations() == 0) progress_info_set_waiting(info, FALSE);
    }

    g_timeout_add_seconds(2, (GSourceFunc)delayed_window_showup,
                          g_object_ref(info));
  }
}

static void caja_progress_info_init(CajaProgressInfo *info) {
...
static gboolean idle_callback(gpointer data) {
  ...
  */
  ...
  if (start_at_idle) {
    g_signal_emit(info, signals[STARTED], 0);
  }

  if (changed_at_idle) {
    g_signal_emit(info, signals[CHANGED], 0);
  }

  if (progress_at_idle) {
    g_signal_emit(info, signals[PROGRESS_CHANGED], 0);
  }

  if (finish_at_idle) {
    g_signal_emit(info, signals[FINISHED], 0);
  }
...
static void queue_idle(CajaProgressInfo *info, gboolean now) {
  if (info->idle_source == NULL || (now && !info->source_is_now)) {
    if (info->idle_source) {
      g_source_destroy(info->idle_source);
      g_source_unref(info->idle_source);
      info->idle_source = NULL;
...
void caja_progress_info_take_status(CajaProgressInfo *info, char *status) {
  G_LOCK(progress_info);
...
void caja_progress_info_set_status(CajaProgressInfo *info, const char *status) {
  G_LOCK(progress_info);
...
void caja_progress_info_take_details(CajaProgressInfo *info, char *details) {
  G_LOCK(progress_info);
...
void caja_progress_info_set_progress(CajaProgressInfo *info, double current,
                                     double total) {
  ...
  if (info->activity_mode || /* emit on switch from activity mode */
      fabs(current_percent - info->progress) >
          0.005 /* Emit on change of 0.5 percent */
  ) {
