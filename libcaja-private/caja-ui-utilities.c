...
void caja_ui_unmerge_ui(GtkUIManager *ui_manager, guint *merge_id,
...
char *caja_get_ui_directory(void) { return g_strdup(DATADIR "/caja/ui"); }
...
const char *caja_ui_string_get(const char *filename) {
  ...
  if (ui == NULL) {
    ...
    g_free(path);
    g_hash_table_insert(ui_cache, g_strdup(filename), ui);
  }
...
static void extension_action_sensitive_callback(CajaMenuItem *item,
                                                GParamSpec *arg1,
                                                gpointer user_data) {
  gboolean value;
...
GtkAction *caja_action_from_menu_item(CajaMenuItem *item,
  ...
  if (icon_name != NULL) {
    ...
    surface = get_action_icon(
        icon_name, caja_get_icon_size_for_stock_size(GTK_ICON_SIZE_MENU),
        parent_widget);
    if (surface != NULL) {
...
GtkAction *caja_toolbar_action_from_menu_item(CajaMenuItem *item,
  ...
  if (icon_name != NULL) {
    ...
    surface = get_action_icon(
        ...
        parent_widget);
    if (surface != NULL) {
      g_object_set_data_full(G_OBJECT(action), "toolbar-icon", surface,
                             (GDestroyNotify)cairo_surface_destroy);
    }
  }
...
static GdkPixbuf *caja_get_thumbnail_frame(void) {
...
void caja_ui_frame_image(GdkPixbuf **pixbuf) {
  ...
  pixbuf_with_frame = eel_embed_image_in_frame(
      *pixbuf, frame, left_offset, top_offset, right_offset, bottom_offset);
