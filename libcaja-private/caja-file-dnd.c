...
static gboolean caja_drag_can_accept_files(CajaFile *drop_target_item) {
  ...
  if (caja_is_engrampa_installed() && caja_file_is_archive(drop_target_item)) {
    return TRUE;
  }
...
gboolean caja_drag_can_accept_items(CajaFile *drop_target_item,
  ...
  if (drop_target_item == NULL) return FALSE;
  ...
  for (max = 100; items != NULL && max >= 0; items = items->next, max--) {
    if (!caja_drag_can_accept_item(
            drop_target_item, ((CajaDragSelectionItem *)items->data)->uri)) {
      return FALSE;
    }
  }
...
gboolean caja_drag_can_accept_info(CajaFile *drop_target_item,
  ...
  switch (drag_type) {
    ...
    case CAJA_ICON_DND_RAW:
      return caja_drag_can_accept_files(
