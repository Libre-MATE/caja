...
static GList *get_file_list_for_launch_locations(GList *locations) {
  ...
  for (l = locations; l != NULL; l = l->next) {
    ...
    files = g_list_prepend(files, caja_file_ref(location->file));
  }
  return g_list_reverse(files);
}

static LaunchLocation *launch_location_from_file(CajaFile *file) {
...
static ApplicationLaunchParameters *application_launch_parameters_new(
    GAppInfo *application, GList *uris) {
...
static GList *filter_caja_handler(GList *apps) {
  GList *l, *next;
  ...
  while (l != NULL) {
    ...
    if (id != NULL && strcmp(id, "caja-folder-handler.desktop") == 0) {
...
static GList *filter_non_uri_apps(GList *apps) {
  GList *l, *next;
  GAppInfo *app = NULL;
...
static gboolean caja_mime_actions_check_if_required_attributes_ready(
    CajaFile *file) {
...
static gboolean file_has_local_path(CajaFile *file) {
  GFile *location;
  char *path;
  gboolean res;
...
static int file_compare_by_mime_type(CajaFile *file_a, CajaFile *file_b) {
...
static int file_compare_by_parent_uri(CajaFile *file_a, CajaFile *file_b) {
...
static int application_compare_by_name(const GAppInfo *app_a,
...
static int application_compare_by_id(const GAppInfo *app_a,
                                     const GAppInfo *app_b) {
...
static GList *intersect_application_lists(GList *a, GList *b) {
  GList *l, *m;
  GList *ret;
  GAppInfo *a_app = NULL;
  GAppInfo *b_app = NULL;
...
GList *caja_mime_get_applications_for_files(GList *files) {
  GList *l, *sorted_files;
  GList *one_ret, *ret;
  CajaFile *file = NULL;
...
static void trash_or_delete_files(GtkWindow *parent_window, const GList *files,
  ...
  caja_file_operations_trash_or_delete(locations, parent_window, NULL, NULL);
...
static ActivationAction get_executable_text_file_action(
    GtkWindow *parent_window, CajaFile *file) {
  GtkDialog *dialog;
  char *file_name;
  char *prompt;
  char *detail;
  ...
  preferences_value = g_settings_get_enum(
      caja_preferences, CAJA_PREFERENCES_EXECUTABLE_TEXT_ACTIVATION);
  switch (preferences_value) {
    ...
    default:
      ...
      g_warning(
          "Unknown value %d for CAJA_PREFERENCES_EXECUTABLE_TEXT_ACTIVATION",
          preferences_value);
  }

  file_name = caja_file_get_display_name(file);
  prompt = g_strdup_printf(
      _("Do you want to run \"%s\", or display its contents?"), file_name);
  detail = g_strdup_printf(_("\"%s\" is an executable text file."), file_name);
  g_free(file_name);

  dialog = eel_create_question_dialog(prompt, detail, _("Run in _Terminal"),
                                      RESPONSE_RUN_IN_TERMINAL, _("_Display"),
                                      RESPONSE_DISPLAY, parent_window);

  eel_dialog_add_button(dialog, _("_Cancel"), "process-stop",
                        GTK_RESPONSE_CANCEL);
...
static ActivationAction get_activation_action(CajaFile *file) {
...
static gboolean file_was_cancelled(CajaFile *file) {
  GError *error;
...
static gboolean file_was_not_mounted(CajaFile *file) {
  GError *error;
...
static void activation_parameters_free(ActivateParameters *parameters) {
  ...
  if (parameters->slot_info) {
    g_object_remove_weak_pointer(G_OBJECT(parameters->slot_info),
...
static gboolean confirm_multiple_windows(GtkWindow *parent_window, int count,
                                         gboolean use_tabs) {
  GtkDialog *dialog;
  char *prompt;
  char *detail;
...
static void activate_parameters_install_free(
  ...
  if (parameters_install->slot_info) {
    g_object_remove_weak_pointer(G_OBJECT(parameters_install->slot_info),
...
static char *get_application_no_mime_type_handler_message(CajaFile *file,
                                                          char *uri) {
...
static void choose_program(GtkDialog *message_dialog, int response,
  ...
  g_object_set_data_full(G_OBJECT(dialog), "mime-action:file",
                         caja_file_ref(file), (GDestroyNotify)caja_file_unref);
  ...
  g_signal_connect_object(dialog, "application_selected",
                          G_CALLBACK(application_selected_cb), callback_data,
                          0);
...
static void show_unhandled_type_error(ActivateParametersInstall *parameters) {
  ...
  if (g_content_type_is_unknown(mime_type)) {
    dialog = gtk_message_dialog_new(parameters->parent_window,
                                    ...
                                    GTK_MESSAGE_ERROR, 0, NULL);
    g_object_set(dialog, "text", error_message, "secondary-text",
                 _("The file is of an unknown type"), NULL);
  } else {
    char *text;
    char *description;
    ...
    dialog = gtk_message_dialog_new(parameters->parent_window,
                                    GTK_DIALOG_DESTROY_WITH_PARENT,
                                    GTK_MESSAGE_ERROR, 0, NULL);
    g_object_set(dialog, "text", error_message, "secondary-text", text, NULL);
  ...
  g_signal_connect(dialog, "response", G_CALLBACK(choose_program),
...
static void search_for_application_dbus_call_notify_cb(GDBusProxy *proxy,
  ...
  caja_mime_activate_files(
      parameters_install->parent_window, parameters_install->slot_info,
      parameters_install->files, parameters_install->activation_directory,
      parameters_install->mode, parameters_install->flags,
      parameters_install->user_confirmation);

  activate_parameters_install_free(parameters_install);
}

static void search_for_application_mime_type(
    ActivateParametersInstall *parameters_install, const gchar *mime_type) {
  GdkWindow *window;
  guint xid = 0;
  ...
  g_dbus_proxy_call(
      ...
      g_variant_new("(u^ass)", xid, mime_types, "hide-confirm-search"),
...
static void pk_proxy_appeared_cb(GObject *source, GAsyncResult *res,
  ...
  dialog = gtk_message_dialog_new(parameters_install->parent_window, 0,
                                  GTK_MESSAGE_ERROR, GTK_BUTTONS_YES_NO, "%s",
                                  error_message);
  description = g_content_type_get_description(mime_type);
...
static void application_unhandled_uri(ActivateParameters *parameters,
  ...
  g_object_add_weak_pointer(G_OBJECT(parameters_install->slot_info),
                            (gpointer *)&parameters_install->slot_info);
  if (parameters->parent_window) {
  ...
  show_install_mime = g_settings_get_boolean(
      caja_preferences, CAJA_PREFERENCES_INSTALL_MIME_ACTIVATION);
#else
  ...
  g_dbus_proxy_new_for_bus(G_BUS_TYPE_SESSION, G_DBUS_PROXY_FLAGS_NONE, NULL,
                           ...
                           "/org/freedesktop/PackageKit",
                           "org.freedesktop.PackageKit.Modify", NULL,
...
static void activate_parameters_desktop_free(
    ActivateParametersDesktop *parameters_desktop) {
  if (parameters_desktop->parent_window) {
    g_object_remove_weak_pointer(
        G_OBJECT(parameters_desktop->parent_window),
...
static void untrusted_launcher_response_callback(
    GtkDialog *dialog, int response_id, ActivateParametersDesktop *parameters) {
  GdkScreen *screen;
  char *uri;
  GFile *file;
  ...
  switch (response_id) {
    case RESPONSE_RUN:
      ...
      caja_launch_desktop_file(screen, uri, NULL, parameters->parent_window);
      g_free(uri);
      break;
    case RESPONSE_MARK_TRUSTED:
      file = caja_file_get_location(parameters->file);
      caja_file_mark_desktop_file_trusted(file, parameters->parent_window, TRUE,
                                          NULL, NULL);
      g_object_unref(file);
      break;
    default:
...
static void activate_desktop_file(ActivateParameters *parameters,
                                  CajaFile *file) {
  ActivateParametersDesktop *parameters_desktop;
  GdkScreen *screen;
  char *uri;
  ...
  if (!caja_file_is_trusted_link(file)) {
    ...
    if (parameters->parent_window) {
      ...
      g_object_add_weak_pointer(G_OBJECT(parameters_desktop->parent_window),
                                (gpointer *)&parameters_desktop->parent_window);
    }
    ...
    secondary = g_strdup_printf(
        _("The application launcher \"%s\" has not been marked as trusted. "
          "If you do not know the source of this file, launching it may be "
          "unsafe."),
        display_name);

    dialog =
        gtk_message_dialog_new(parameters->parent_window, 0,
    ...
    if (caja_file_can_set_permissions(file)) {
      gtk_dialog_add_button(GTK_DIALOG(dialog), _("Mark as _Trusted"),
                            RESPONSE_MARK_TRUSTED);
    }
...
static void activate_files(ActivateParameters *parameters) {
    ...
    switch (action) {
      ...
      case ACTIVATION_ACTION_LAUNCH_IN_TERMINAL:
        launch_in_terminal_files =
            g_list_prepend(launch_in_terminal_files, file);
        break;
  ...
  for (l = launch_files; l != NULL; l = l->next) {
    ...
    caja_launch_application_from_command(screen, name, quoted_path, FALSE,
                                         NULL);
    g_free(name);
    g_free(quoted_path);
    g_free(executable_path);
  ...
  if (parameters->slot_info != NULL &&
      (!parameters->user_confirmation ||
       confirm_multiple_windows(
           parameters->parent_window, count,
    ...
    for (l = open_in_view_files; l != NULL; l = l->next) {
      ...
      caja_window_slot_info_open_location(parameters->slot_info, f,
                                          parameters->mode, flags, NULL);
      g_object_unref(f);
      g_free(uri);
    }
  }
...
static void activation_mount_not_mounted(ActivateParameters *parameters) {
  ...
  caja_file_list_call_when_ready(
      files,
      caja_mime_actions_get_required_file_attributes() |
  ...
  for (l = parameters->locations; l != NULL; l = next) {
    ...
    if (file_was_not_mounted(file)) {
      if (parameters->tried_mounting) {
        ...
        parameters->not_mounted =
            g_list_prepend(parameters->not_mounted, caja_file_ref(file));
      }
      continue;
    }
  }
...
static void activation_get_activation_uris(ActivateParameters *parameters) {
  ...
  files = get_file_list_for_launch_locations(parameters->locations);
  caja_file_list_call_when_ready(
      files, CAJA_FILE_ATTRIBUTE_INFO | CAJA_FILE_ATTRIBUTE_LINK_INFO,
      &parameters->files_handle, activate_activation_uris_ready_callback,
      parameters);
  caja_file_list_free(files);
}

static void activation_mountable_mounted(CajaFile *file, GFile *result_location,
                                         GError *error,
                                         gpointer callback_data) {
  ActivateParameters *parameters = callback_data;
  LaunchLocation *location;
...
static void activation_mount_mountables(ActivateParameters *parameters) {
  if (parameters->mountables != NULL) {
    ...
    g_signal_connect(mount_op, "notify::is-showing",
                     G_CALLBACK(activate_mount_op_active), parameters);
    caja_file_mount(file, mount_op, parameters->cancellable,
...
static void activation_mountable_started(CajaFile *file, GFile *gfile_of_file,
                                         GError *error,
                                         gpointer callback_data) {
  ActivateParameters *parameters = callback_data;
  LaunchLocation *location;
  ...
  if (error == NULL) {
    ...
    if (error->domain != G_IO_ERROR ||
        (error->code != G_IO_ERROR_CANCELLED &&
         error->code != G_IO_ERROR_FAILED_HANDLED)) {
      eel_show_error_dialog(_("Unable to start location"), error->message,
                            NULL);
    }
...
static void activation_start_mountables(ActivateParameters *parameters) {
  if (parameters->start_mountables != NULL) {
    ...
    g_signal_connect(start_op, "notify::is-showing",
                     G_CALLBACK(activate_mount_op_active), parameters);
    caja_file_start(file, start_op, parameters->cancellable,
...
void caja_mime_activate_files(GtkWindow *parent_window,
  ...
  g_object_add_weak_pointer(G_OBJECT(parameters->slot_info),
                            (gpointer *)&parameters->slot_info);
  if (parent_window) {
  ...
  for (l = parameters->locations; l != NULL; l = next) {
    ...
    if (caja_file_can_mount(file)) {
      parameters->mountables =
          g_list_prepend(parameters->mountables, caja_file_ref(file));
    }

    if (caja_file_can_start(file)) {
      parameters->start_mountables =
          g_list_prepend(parameters->start_mountables, caja_file_ref(file));
    }
  }

  activation_start_timed_cancel(parameters);
  if (parameters->mountables != NULL) activation_mount_mountables(parameters);
  if (parameters->start_mountables != NULL)
    activation_start_mountables(parameters);
  if (parameters->mountables == NULL && parameters->start_mountables == NULL)
    activation_get_activation_uris(parameters);
...
void caja_mime_activate_file(GtkWindow *parent_window,
  ...
  caja_mime_activate_files(parent_window, slot_info, files, launch_directory,
                           mode, flags, FALSE);
  g_list_free(files);
}
