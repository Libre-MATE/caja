...
G_DEFINE_TYPE(CajaIconInfo, caja_icon_info, G_TYPE_OBJECT);

static void caja_icon_info_init(CajaIconInfo *icon) {
...
static void caja_icon_info_finalize(GObject *object) {
  CajaIconInfo *icon;
...
static void caja_icon_info_class_init(CajaIconInfoClass *icon_info_class) {
...
CajaIconInfo *caja_icon_info_new_for_pixbuf(GdkPixbuf *pixbuf, gint scale) {
  CajaIconInfo *icon;
...
static CajaIconInfo *caja_icon_info_new_for_icon_info(GtkIconInfo *icon_info,
                                                      gint scale) {
  CajaIconInfo *icon;
  GdkPoint *points;
  gint n_points;
  ...
  icon->got_embedded_rect =
      gtk_icon_info_get_embedded_rect(icon_info, &icon->embedded_rect);
...
static guint icon_key_hash(IconKey *key) {
  return g_icon_hash(key->icon) ^ key->size;
}

static gboolean icon_key_equal(const IconKey *a, const IconKey *b) {
  return a->size == b->size && a->scale == b->scale &&
         g_icon_equal(a->icon, b->icon);
}

static IconKey *icon_key_new(GIcon *icon, int scale, int size) {
  IconKey *key;
...
static void icon_key_free(IconKey *key) {
  g_object_unref(key->icon);
  g_slice_free(IconKey, key);
}

CajaIconInfo *caja_icon_info_lookup(GIcon *icon, int size, int scale) {
  ...
  if (G_IS_LOADABLE_ICON(icon)) {
    ...
    if (loadable_icon_cache == NULL) {
      loadable_icon_cache = g_hash_table_new_full(
          (GHashFunc)icon_key_hash, (GEqualFunc)icon_key_equal,
          (GDestroyNotify)icon_key_free, (GDestroyNotify)g_object_unref);
    }
    ...
    if (!pixbuf) {
      gtkicon_info = gtk_icon_theme_lookup_icon_for_scale(
          icon_theme, "text-x-generic", size, scale,
    ...
    gtkicon_info = gtk_icon_theme_lookup_by_gicon_for_scale(
        icon_theme, icon, size, scale, GTK_ICON_LOOKUP_FORCE_SIZE);
...
CajaIconInfo *caja_icon_info_lookup_from_name(const char *name, int size,
                                              int scale) {
  GIcon *icon;
  CajaIconInfo *info;
...
CajaIconInfo *caja_icon_info_lookup_from_path(const char *path, int size,
                                              int scale) {
  GFile *icon_file;
  GIcon *icon;
  CajaIconInfo *info;
...
cairo_surface_t *caja_icon_info_get_surface_nodefault(CajaIconInfo *icon) {
  ...
  surface =
      gdk_cairo_surface_create_from_pixbuf(pixbuf, icon->orig_scale, NULL);
...
GdkPixbuf *caja_icon_info_get_pixbuf(CajaIconInfo *icon) {
  ...
  if (res == NULL) {
    res = gdk_pixbuf_new_from_data(
        caja_default_file_icon, GDK_COLORSPACE_RGB, TRUE, 8,
        caja_default_file_icon_width, caja_default_file_icon_height,
...
cairo_surface_t *caja_icon_info_get_surface_nodefault_at_size(
    CajaIconInfo *icon, gsize forced_size) {
  GdkPixbuf *pixbuf;
  cairo_surface_t *surface;
...
gboolean caja_icon_info_get_attach_points(CajaIconInfo *icon, GdkPoint **points,
                                          gint *n_points) {
