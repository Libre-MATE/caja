...
enum { RESPONSE_OPEN, RESPONSE_REMOVE };

enum { APPLICATION_SELECTED, LAST_SIGNAL };

static guint signals[LAST_SIGNAL] = {0};
G_DEFINE_TYPE(CajaOpenWithDialog, caja_open_with_dialog, GTK_TYPE_DIALOG);

static void caja_open_with_dialog_finalize(GObject *object) {
 ...
 */
static gboolean check_application(CajaOpenWithDialog *dialog) {
  char *command;
  char *path = NULL;
  char **argv = NULL;
  int argc;
  ...
  if (dialog->details->selected_app_info != NULL) {
    command =
        g_strdup(g_app_info_get_executable(dialog->details->selected_app_info));
  }
  ...
  if (error) {
    eel_show_error_dialog(_("Could not run application"), error->message,
                          GTK_WINDOW(dialog));
    g_error_free(error);
    retval = FALSE;
    goto cleanup;
  ...
  if (!path) {
    ...
    error_message = g_strdup_printf(_("Could not find '%s'"), argv[0]);

    eel_show_error_dialog(_("Could not find application"), error_message,
                          GTK_WINDOW(dialog));
...
static char *get_app_name(const char *commandline, GError **error) {
  char *basename;
  char *unquoted;
  char **argv;
  int argc;
 ...
 */
static GAppInfo *add_or_find_application(CajaOpenWithDialog *dialog) {
  GAppInfo *app;
  GError *error;
  ...
  if (dialog->details->selected_app_info) {
    ...
    if (app_name != NULL) {
      app = g_app_info_create_from_commandline(commandline, app_name,
  ...
  if (app == NULL) {
    message = g_strdup_printf(
        _("Could not add application to the application database: %s"),
        error ? error->message : _("Unknown error"));
    eel_show_error_dialog(_("Could not add application"), message,
                          GTK_WINDOW(dialog));
    g_free(message);
  ...
  if (should_set_default) {
    if (dialog->details->content_type) {
      success = g_app_info_set_as_default_for_type(
          app, dialog->details->content_type, &error);
    } else {
      success = g_app_info_set_as_default_for_extension(
          app, dialog->details->extension, &error);
    }
  } else {
    GList *applications;
    ...
    if (dialog->details->content_type && applications != NULL) {
      ...
      g_app_info_add_supports_type(app, dialog->details->content_type, NULL);
    }
  ...
  if (!success && should_set_default) {
    message = g_strdup_printf(_("Could not set application as the default: %s"),
                              error->message);
    eel_show_error_dialog(_("Could not set as default application"), message,
                          GTK_WINDOW(dialog));
    g_free(message);
    g_error_free(error);
  }

  g_signal_emit_by_name(caja_signaller_get_current(), "mime_data_changed");
  return app;
}

static void emit_application_selected(CajaOpenWithDialog *dialog,
                                      GAppInfo *application) {
  g_signal_emit(G_OBJECT(dialog), signals[APPLICATION_SELECTED], 0,
                application);
}

static void response_cb(CajaOpenWithDialog *dialog, int response_id,
                        gpointer data) {
  switch (response_id) {
    case RESPONSE_OPEN:
          ...
          if (gtk_tree_model_get_iter_first(model, &iter)) {
            do {
              ...
              if (g_app_info_equal(selected, info)) {
                gtk_list_store_remove(dialog->details->program_list_store,
                                      &iter);
                break;
              }
...
static void caja_open_with_dialog_class_init(CajaOpenWithDialogClass *class) {
  GObjectClass *gobject_class;

  gobject_class = G_OBJECT_CLASS(class);
  gobject_class->finalize = caja_open_with_dialog_finalize;

  signals[APPLICATION_SELECTED] = g_signal_new(
      "application_selected", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaOpenWithDialogClass, application_selected), NULL,
      NULL, g_cclosure_marshal_VOID__POINTER, G_TYPE_NONE, 1, G_TYPE_POINTER);
}

static void chooser_response_cb(GtkFileChooser *chooser, int response,
                                gpointer user_data) {
  CajaOpenWithDialog *dialog;
  ...
  if (response == GTK_RESPONSE_OK) {
    ...
    if (filename) {
      ...
      gtk_entry_set_text(GTK_ENTRY(dialog->details->entry), quoted_text);
...
static void browse_clicked_cb(GtkWidget *button, gpointer user_data) {
  CajaOpenWithDialog *dialog;
  GtkWidget *chooser;

  dialog = CAJA_OPEN_WITH_DIALOG(user_data);

  chooser = eel_file_chooser_dialog_new(
      _("Select an Application"), GTK_WINDOW(dialog),
      GTK_FILE_CHOOSER_ACTION_OPEN, "process-stop", GTK_RESPONSE_CANCEL,
      "document-open", GTK_RESPONSE_OK, NULL);
...
static void entry_changed_cb(GtkWidget *entry, CajaOpenWithDialog *dialog) {
...
static cairo_surface_t *get_surface_for_icon(GIcon *icon) {
  ...
  if (G_IS_FILE_ICON(icon)) {
    ...
    if (filename) {
      ...
      pixbuf = gdk_pixbuf_new_from_file_at_size(
          filename, CAJA_OPEN_WITH_DIALOG_ICON_SIZE * icon_scale,
          CAJA_OPEN_WITH_DIALOG_ICON_SIZE * icon_scale, NULL);
    ...
    if (names != NULL && names[0] != NULL) {
      ...
      surface = gtk_icon_theme_load_surface(
          gtk_icon_theme_get_default(), icon_no_extension,
          CAJA_OPEN_WITH_DIALOG_ICON_SIZE, icon_scale, NULL,
...
static gboolean caja_open_with_dialog_add_icon_idle(
    CajaOpenWithDialog *dialog) {
  cairo_surface_t *surface;
  GIcon *icon;
  ...
  do {
    ...
    dialog->details->add_icon_paths = g_slist_delete_link(
        dialog->details->add_icon_paths, dialog->details->add_icon_paths);

    if (!gtk_tree_model_get_iter(
            GTK_TREE_MODEL(dialog->details->program_list_store), &iter, path)) {
    ...
    gtk_tree_model_get(GTK_TREE_MODEL(dialog->details->program_list_store),
                       &iter, COLUMN_GICON, &icon, -1);
...
static gboolean caja_open_with_search_equal_func(GtkTreeModel *model,
                                                 int column, const char *key,
                                                 GtkTreeIter *iter,
                                                 gpointer user_data) {
  char *name;
  char *path;
  ...
  if (key != NULL) {
    ...
    gtk_tree_model_get(model, iter, COLUMN_NAME, &name, COLUMN_EXEC, &path, -1);
    ...
    if (name != NULL) {
      ...
      if (strncmp(normalized_name, normalized_key, strlen(normalized_key)) ==
          0) {
        ret = FALSE;
      }
    ...
    if (ret && path != NULL) {
      ...
      if (strncmp(normalized_path, normalized_key, strlen(normalized_key)) ==
              0 ||
          strncmp(normalized_basename, normalized_key,
                  strlen(normalized_key)) == 0) {
        ret = FALSE;
      }
...
static gboolean caja_open_with_dialog_add_items_idle(
    CajaOpenWithDialog *dialog) {
  ...
  /* create list store */
  dialog->details->program_list_store = gtk_list_store_new(
      NUM_COLUMNS, G_TYPE_APP_INFO, CAIRO_GOBJECT_TYPE_SURFACE, G_TYPE_ICON,
      G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING);
  sort = gtk_tree_model_sort_new_with_model(
  ...
  for (l = all_applications; l; l = l->next) {
    ...
    gtk_list_store_set(dialog->details->program_list_store, &iter,
                       COLUMN_APP_INFO, app, COLUMN_ICON, NULL, COLUMN_GICON,
                       g_app_info_get_icon(app), COLUMN_NAME,
                       g_app_info_get_display_name(app), COLUMN_COMMENT,
                       g_app_info_get_description(app), COLUMN_EXEC,
                       g_app_info_get_executable, -1);

    path = gtk_tree_model_get_path(
        GTK_TREE_MODEL(dialog->details->program_list_store), &iter);
    if (path != NULL) {
      dialog->details->add_icon_paths =
          g_slist_prepend(dialog->details->add_icon_paths, path);
    }
  }
  ...
  gtk_tree_view_set_model(GTK_TREE_VIEW(dialog->details->program_list),
                          GTK_TREE_MODEL(sort));
  gtk_tree_sortable_set_sort_column_id(GTK_TREE_SORTABLE(sort), COLUMN_NAME,
                                       GTK_SORT_ASCENDING);
  gtk_tree_view_set_search_equal_func(
      GTK_TREE_VIEW(dialog->details->program_list),
      caja_open_with_search_equal_func, NULL, NULL);

  renderer = gtk_cell_renderer_pixbuf_new();
  column = gtk_tree_view_column_new();
  gtk_tree_view_column_pack_start(column, renderer, FALSE);
  gtk_tree_view_column_set_attributes(column, renderer, "surface", COLUMN_ICON,
                                      NULL);
  ...
  gtk_tree_view_append_column(GTK_TREE_VIEW(dialog->details->program_list),
                              column);

  dialog->details->add_icon_paths =
      g_slist_reverse(dialog->details->add_icon_paths);

  if (!dialog->details->add_icons_idle_id) {
    dialog->details->add_icons_idle_id = g_idle_add_full(
        G_PRIORITY_DEFAULT_IDLE,
        (GSourceFunc)caja_open_with_dialog_add_icon_idle, dialog, NULL);
  }
...
static void program_list_selection_changed(GtkTreeSelection *selection,
  ...
  if (!gtk_tree_selection_get_selected(selection, &model, &iter)) {
    gtk_widget_set_sensitive(dialog->details->button, FALSE);
    gtk_dialog_set_response_sensitive(GTK_DIALOG(dialog), RESPONSE_REMOVE,
                                      FALSE);
    return;
  }
...
static void program_list_selection_activated(GtkTreeView *view,
  ...
  selection =
      gtk_tree_view_get_selection(GTK_TREE_VIEW(dialog->details->program_list));
  program_list_selection_changed(selection, dialog);

  gtk_dialog_response(GTK_DIALOG(&dialog->parent), RESPONSE_OPEN);
}

static void expander_toggled(GtkWidget *expander, CajaOpenWithDialog *dialog) {
...
static void caja_open_with_dialog_init(CajaOpenWithDialog *dialog) {
  GtkWidget *hbox;
  GtkWidget *vbox;
  GtkWidget *vbox2;
  GtkWidget *label;
  ...
  gtk_box_pack_start(GTK_BOX(vbox2), dialog->details->label, FALSE, FALSE, 0);
  ...
  gtk_tree_view_set_headers_visible(
      GTK_TREE_VIEW(dialog->details->program_list), FALSE);
  gtk_container_add(GTK_CONTAINER(scrolled_window),
                    dialog->details->program_list);

  gtk_box_pack_start(GTK_BOX(vbox2), scrolled_window, TRUE, TRUE, 0);

  dialog->details->desc_label =
      gtk_label_new(_("Select an application to view its description."));
  gtk_label_set_xalign(GTK_LABEL(dialog->details->desc_label), 0.0);
  gtk_label_set_justify(GTK_LABEL(dialog->details->desc_label),
                        GTK_JUSTIFY_LEFT);
  ...
  gtk_box_pack_start(GTK_BOX(vbox2), dialog->details->desc_label, FALSE, FALSE,
                     0);
  ...
  dialog->details->add_items_idle_id = g_idle_add_full(
      G_PRIORITY_DEFAULT_IDLE,
      (GSourceFunc)caja_open_with_dialog_add_items_idle, dialog, NULL);
  ...
  g_signal_connect_after(expander, "activate", G_CALLBACK(expander_toggled),
                         dialog);
  ...
  gtk_box_pack_start(GTK_BOX(hbox), dialog->details->entry, TRUE, TRUE, 0);
  ...
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(dialog->details->checkbox),
                               TRUE);
  ...
  eel_dialog_add_button(GTK_DIALOG(dialog), _("_Remove"), "list-remove",
                        RESPONSE_REMOVE);

  gtk_dialog_set_response_sensitive(GTK_DIALOG(dialog), RESPONSE_REMOVE, FALSE);
  ...
  gtk_label_set_mnemonic_widget(GTK_LABEL(label),
                                GTK_WIDGET(dialog->details->button));
  ...
  gtk_dialog_add_action_widget(GTK_DIALOG(dialog), dialog->details->button,
                               RESPONSE_OPEN);

  gtk_dialog_set_default_response(GTK_DIALOG(dialog), RESPONSE_OPEN);

  g_signal_connect(dialog, "response", G_CALLBACK(response_cb), dialog);
}

static char *get_extension(const char *basename) {
  char *p;
...
static void set_uri_and_type(CajaOpenWithDialog *dialog, const char *uri,
                             const char *mime_type,
                             const char *passed_extension, gboolean add_mode) {
  char *label;
  char *emname;
  char *name, *extension;
  char *checkbox_text;
  ...
  if (extension != NULL && g_content_type_is_unknown(mime_type)) {
    ...
    if (name != NULL) {
      ...
      if (add_mode) {
        ...
        checkbox_text =
            g_strdup_printf(_("_Remember this application for %s documents"),
                            dialog->details->extension);

        gtk_button_set_label(GTK_BUTTON(dialog->details->checkbox),
                             checkbox_text);
    ...
    if (name != NULL) {
      ...
      if (add_mode) {
        ...
        checkbox_text = g_strdup_printf(
            _("_Remember this application for \"%s\" files"), description);
...
static GtkWidget *real_caja_open_with_dialog_new(const char *uri,
                                                 const char *mime_type,
  ...
  set_uri_and_type(CAJA_OPEN_WITH_DIALOG(dialog), uri, mime_type, extension,
                   add_mode);
...
GtkWidget *caja_add_application_dialog_new(const char *uri,
  ...
  dialog = CAJA_OPEN_WITH_DIALOG(
      real_caja_open_with_dialog_new(uri, mime_type, NULL, TRUE));
...
GtkWidget *caja_add_application_dialog_new_for_multiple_files(
  ...
  dialog = CAJA_OPEN_WITH_DIALOG(
      real_caja_open_with_dialog_new(NULL, mime_type, extension, TRUE));
