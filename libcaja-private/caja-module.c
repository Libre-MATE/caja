...
#define CAJA_MODULE(obj) \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), CAJA_TYPE_MODULE, CajaModule))
...
static gboolean caja_module_load(GTypeModule *gmodule) {
  CajaModule *module;

  module = CAJA_MODULE(gmodule);

  module->library =
      g_module_open(module->path, G_MODULE_BIND_LAZY | G_MODULE_BIND_LOCAL);
  ...
  if (!g_module_symbol(module->library, "caja_module_initialize",
                       (gpointer *)&module->initialize) ||
      !g_module_symbol(module->library, "caja_module_shutdown",
                       (gpointer *)&module->shutdown) ||
      !g_module_symbol(module->library, "caja_module_list_types",
  ...
  g_module_symbol(module->library, "caja_module_list_pyfiles",
...
static void caja_module_unload(GTypeModule *gmodule) {
  CajaModule *module;
...
static void caja_module_finalize(GObject *object) {
  CajaModule *module;
...
static void caja_module_init(CajaModule *module) {}

static void caja_module_class_init(CajaModuleClass *class) {
...
static void add_module_objects(CajaModule *module) {
  GObject *object = NULL;
  GList *pyfiles = NULL;
  gchar *filename = NULL;
...
static CajaModule *caja_module_load_file(const char *filename) {
  CajaModule *module;
...
static void load_module_dir(const char *dirname) {
  GDir *dir;
...
static void free_module_objects(void) {
  GList *l, *next;
...
void caja_module_setup(void) {
  ...
  if (!initialized) {
    ...
    if (caja_extension_dirs) {
      gchar **dir_vector =
          g_strsplit(caja_extension_dirs, G_SEARCHPATH_SEPARATOR_S, 0);

      for (gchar **dir = dir_vector; *dir != NULL; ++dir) load_module_dir(*dir);
...
GList *caja_module_get_extensions_for_type(GType type) {
  ...
  for (l = module_objects; l != NULL; l = l->next) {
    if (G_TYPE_CHECK_INSTANCE_TYPE(G_OBJECT(l->data), type)) {
      g_object_ref(l->data);
...
void caja_module_extension_list_free(GList *extensions) {
  GList *l, *next;
