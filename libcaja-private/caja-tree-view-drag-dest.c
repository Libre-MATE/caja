...
G_DEFINE_TYPE(CajaTreeViewDragDest, caja_tree_view_drag_dest, G_TYPE_OBJECT);
...
static void remove_scroll_timeout(CajaTreeViewDragDest *dest) {
...
static void remove_expand_timeout(CajaTreeViewDragDest *dest) {
...
static gboolean highlight_draw(GtkWidget *widget, cairo_t *cr, gpointer data) {
...
static void set_widget_highlight(CajaTreeViewDragDest *dest,
                                 gboolean highlight) {
  ...
  if (highlight && !dest->details->highlight_id) {
    dest->details->highlight_id = g_signal_connect_object(
        dest->details->tree_view, "draw", G_CALLBACK(highlight_draw), dest, 0);
    gtk_widget_queue_draw(GTK_WIDGET(dest->details->tree_view));
  }
}

static void set_drag_dest_row(CajaTreeViewDragDest *dest, GtkTreePath *path) {
  if (path) {
...
static void clear_drag_dest_row(CajaTreeViewDragDest *dest) {
  gtk_tree_view_set_drag_dest_row(dest->details->tree_view, NULL, 0);
  set_widget_highlight(dest, FALSE);
}

static gboolean get_drag_data(CajaTreeViewDragDest *dest,
...
static void free_drag_data(CajaTreeViewDragDest *dest) {
...
static char *get_root_uri(CajaTreeViewDragDest *dest) {
  char *uri;
...
static CajaFile *file_for_path(CajaTreeViewDragDest *dest, GtkTreePath *path) {
  CajaFile *file;
...
static GtkTreePath *get_drop_path(CajaTreeViewDragDest *dest,
                                  GtkTreePath *path) {
  CajaFile *file;
  GtkTreePath *ret;
  ...
  while (file == NULL /* dummy row */ ||
         !caja_drag_can_accept_info(file, dest->details->drag_type,
...
static char *get_drop_target_uri_for_path(CajaTreeViewDragDest *dest,
                                          GtkTreePath *path) {
  CajaFile *file;
  char *target;
...
static guint get_drop_action(CajaTreeViewDragDest *dest,
  ...
  switch (dest->details->drag_type) {
    case CAJA_ICON_DND_MATE_ICON_LIST:
      ...
      caja_drag_default_drop_action_for_icons(
          context, drop_target, dest->details->drag_list, &action);
...
static char *get_drop_target_uri_at_pos(CajaTreeViewDragDest *dest, int x,
                                        int y) {
  char *drop_target;
  GtkTreePath *path;
  GtkTreePath *drop_path;
...
static void receive_uris(CajaTreeViewDragDest *dest, GdkDragContext *context,
                         GList *source_uris, int x, int y) {
  char *drop_target;
  ...
  if (real_action == GDK_ACTION_ASK) {
    ...
    real_action =
        caja_drag_drop_action_ask(GTK_WIDGET(dest->details->tree_view), action);
  }
...
static void receive_dropped_icons(CajaTreeViewDragDest *dest,
  ...
  for (l = dest->details->drag_list; l != NULL; l = l->next) {
    source_uris =
        g_list_prepend(source_uris, ((CajaDragSelectionItem *)l->data)->uri);
  }
...
static void receive_dropped_uri_list(CajaTreeViewDragDest *dest,
                                     GdkDragContext *context, int x, int y) {
  char *drop_target;
...
static void receive_dropped_text(CajaTreeViewDragDest *dest,
                                 GdkDragContext *context, int x, int y) {
  char *drop_target;
  char *text;
...
static void receive_dropped_raw(CajaTreeViewDragDest *dest,
...
static void receive_dropped_netscape_url(CajaTreeViewDragDest *dest,
...
static void receive_dropped_keyword(CajaTreeViewDragDest *dest,
                                    GdkDragContext *context, int x, int y) {
...
static gboolean receive_xds(CajaTreeViewDragDest *dest, GtkWidget *widget,
  ...
  if (selection_format == 8 && selection_length == 1 &&
    ...
    gtk_drag_get_data(widget, context,
                      gdk_atom_intern(CAJA_ICON_DND_RAW_TYPE, FALSE), time);
    return FALSE;
...
static gboolean drag_data_received_callback(
  ...
  if (!dest->details->have_drag_data) {
    ...
    if (info == CAJA_ICON_DND_MATE_ICON_LIST) {
      dest->details->drag_list = caja_drag_build_selection_list(selection_data);
    }
  }
...
static gboolean set_direct_save_uri(CajaTreeViewDragDest *dest,
                                    GdkDragContext *context, int x, int y) {
  char *drop_uri;
  char *uri;
...
static void caja_tree_view_drag_dest_dispose(GObject *object) {
  CajaTreeViewDragDest *dest;
...
static void caja_tree_view_drag_dest_finalize(GObject *object) {
  CajaTreeViewDragDest *dest;
...
static void caja_tree_view_drag_dest_class_init(
    CajaTreeViewDragDestClass *class) {
  ...
  signals[GET_ROOT_URI] =
      g_signal_new("get_root_uri", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(CajaTreeViewDragDestClass, get_root_uri),
                   NULL, NULL, caja_marshal_STRING__VOID, G_TYPE_STRING, 0);
  signals[GET_FILE_FOR_PATH] = g_signal_new(
      "get_file_for_path", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaTreeViewDragDestClass, get_file_for_path), NULL, NULL,
      caja_marshal_OBJECT__BOXED, CAJA_TYPE_FILE, 1, GTK_TYPE_TREE_PATH);
  signals[MOVE_COPY_ITEMS] = g_signal_new(
      "move_copy_items", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaTreeViewDragDestClass, move_copy_items), NULL, NULL,

      caja_marshal_VOID__POINTER_STRING_ENUM_INT_INT, G_TYPE_NONE, 5,
      G_TYPE_POINTER, G_TYPE_STRING, GDK_TYPE_DRAG_ACTION, G_TYPE_INT,
      G_TYPE_INT);
  signals[HANDLE_NETSCAPE_URL] = g_signal_new(
      "handle_netscape_url", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(CajaTreeViewDragDestClass, handle_netscape_url), NULL,
      NULL, caja_marshal_VOID__STRING_STRING_ENUM_INT_INT, G_TYPE_NONE, 5,
  ...
  signals[HANDLE_URI_LIST] = g_signal_new(
      ...
      G_STRUCT_OFFSET(CajaTreeViewDragDestClass, handle_uri_list), NULL, NULL,
  ...
  signals[HANDLE_TEXT] =
      g_signal_new("handle_text", G_TYPE_FROM_CLASS(class), G_SIGNAL_RUN_LAST,
                   G_STRUCT_OFFSET(CajaTreeViewDragDestClass, handle_text),
                   NULL, NULL, caja_marshal_VOID__STRING_STRING_ENUM_INT_INT,
  ...
  signals[HANDLE_RAW] = g_signal_new(
      ...
      G_STRUCT_OFFSET(CajaTreeViewDragDestClass, handle_raw), NULL, NULL,
...
CajaTreeViewDragDest *caja_tree_view_drag_dest_new(GtkTreeView *tree_view) {
  ...
  g_signal_connect_object(tree_view, "drag_motion",
                          G_CALLBACK(drag_motion_callback), dest, 0);
  g_signal_connect_object(tree_view, "drag_leave",
                          G_CALLBACK(drag_leave_callback), dest, 0);
  g_signal_connect_object(tree_view, "drag_drop",
                          G_CALLBACK(drag_drop_callback), dest, 0);
  g_signal_connect_object(tree_view, "drag_data_received",
