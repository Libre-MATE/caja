...
enum { ACTIVE, INACTIVE, LAST_SIGNAL };

static guint caja_window_slot_info_signals[LAST_SIGNAL] = {0};

static void caja_window_slot_info_base_init(gpointer g_class) {
  static gboolean initialized = FALSE;

  if (!initialized) {
    caja_window_slot_info_signals[ACTIVE] =
        g_signal_new("active", CAJA_TYPE_WINDOW_SLOT_INFO, G_SIGNAL_RUN_LAST,
                     G_STRUCT_OFFSET(CajaWindowSlotInfoIface, active), NULL,
                     NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);

    caja_window_slot_info_signals[INACTIVE] =
        g_signal_new("inactive", CAJA_TYPE_WINDOW_SLOT_INFO, G_SIGNAL_RUN_LAST,
                     G_STRUCT_OFFSET(CajaWindowSlotInfoIface, inactive), NULL,
                     NULL, g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
...
GType caja_window_slot_info_get_type(void) {
  ...
  if (!type) {
    const GTypeInfo info = {sizeof(CajaWindowSlotInfoIface),
    ...
    type = g_type_register_static(G_TYPE_INTERFACE, "CajaWindowSlotInfo", &info,
                                  0);
...
void caja_window_slot_info_set_status(CajaWindowSlotInfo *slot,
  ...
  (*CAJA_WINDOW_SLOT_INFO_GET_IFACE(slot)->set_status)(slot, status);
}
...
void caja_window_slot_info_open_location_full(
  ...
  (*CAJA_WINDOW_SLOT_INFO_GET_IFACE(slot)->open_location)(
      slot, location, mode, flags, selection, callback, user_data);
}
