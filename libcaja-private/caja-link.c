...
static gboolean is_link_mime_type(const char *mime_type) {
  if (mime_type != NULL &&
...
static gboolean is_local_file_a_link(const char *uri) {
  gboolean link;
  GFile *file;
  GFileInfo *info;
  GError *error;
...
static gboolean _g_key_file_save_to_gfile(GKeyFile *key_file, GFile *file,
                                          GError **error) {
  char *data;
  gsize len;
...
static char *slurp_key_string(const char *uri, const char *keyname,
                              gboolean localize) {
  GKeyFile *key_file;
  char *result;
  ...
  if (localize) {
    result =
        g_key_file_get_locale_string(key_file, MAIN_GROUP, keyname, NULL, NULL);
  } else {
...
gboolean caja_link_local_create(const char *directory_uri,
  ...
  if (eel_uri_is_trash(directory_uri) || eel_uri_is_search(directory_uri)) {
    return FALSE;
  }
  ...
  if (unique_filename) {
    ...
    uri =
        caja_ensure_unique_file_name(real_directory_uri, base_name, ".desktop");
    if (uri == NULL) {
  ...
  contents = g_strdup_printf(
      ...
      "URL=%s\n"
      "%s%s\n",
      display_name, target_uri, image != NULL ? "Icon=" : "",
      image != NULL ? image : "");

  if (!g_file_replace_contents(file, contents, strlen(contents), NULL, FALSE,
...
static gboolean caja_link_local_set_key(const char *uri, const char *key,
                                        const char *value, gboolean localize) {
  gboolean success;
  GKeyFile *key_file;
  GFile *file;
  ...
  file = g_file_new_for_uri(uri);
  key_file = g_key_file_new();
  if (!_g_key_file_load_from_gfile(key_file, file, G_KEY_FILE_KEEP_COMMENTS,
                                   NULL)) {
    g_key_file_free(key_file);
    g_object_unref(file);
    return FALSE;
  ...
  if (localize) {
    g_key_file_set_locale_string(key_file, MAIN_GROUP, key, get_language(),
                                 value);
  } else {
...
char *caja_link_local_get_text(const char *path) {
  return slurp_key_string(path, "Name", TRUE);
}

char *caja_link_local_get_additional_text(const char *path) {
...
static char *caja_link_get_link_uri_from_desktop(GKeyFile *key_file,
  ...
  if (strcmp(type, "URL") == 0) {
    /* Some old broken desktop files use this nonstandard feature, we need
     * handle it though */
...
static char *caja_link_get_link_name_from_desktop(GKeyFile *key_file) {
  return g_key_file_get_locale_string(key_file, MAIN_GROUP, "Name", NULL, NULL);
}

static char *caja_link_get_link_icon_from_desktop(GKeyFile *key_file) {
  ...
  if (icon != NULL) {
    if (!g_path_is_absolute(icon)) {
      ...
      /* Strip out any extension on non-filename icons. Old desktop files may
       * have this */
      p = strchr(icon, '.');
      /* Only strip known icon extensions */
      if ((p != NULL) && ((g_ascii_strcasecmp(p, ".png") == 0) ||
...
static gboolean string_array_contains(char **array, const char *str) {
  char **p;
...
void caja_link_get_link_info_given_file_contents(
  ...
  key_file = g_key_file_new();
  if (!g_key_file_load_from_data(key_file, file_contents, link_file_size,
  ...
  *is_foreign = FALSE;
  only_show_in = g_key_file_get_string_list(key_file, MAIN_GROUP, "OnlyShowIn",
                                            NULL, NULL);
  ...
  not_show_in =
      g_key_file_get_string_list(key_file, MAIN_GROUP, "NotShowIn", NULL, NULL);
